...
TL;DR: 研究发现，针对人脸识别中常见的图像质量问题，选择性应用CFR-GAN结合CodeFormer的修复方法能有效提升识别准确率，而简单应用现有修复技术反而会降低性能。


<details>
  <summary>Details</summary>
Motivation: 生物特征人脸识别模型在处理现实世界图像时，常因图像质量差、非正面姿态和遮挡等问题导致准确率显著下降。本研究旨在探索通过AI驱动的头部姿态校正和图像修复技术是否能改善识别准确性。

Method: 使用模型无关的大规模法证评估流程，评估三种修复方法的影响：3D重建（NextFace）、2D正面化（CFR-GAN）和特征增强（CodeFormer）。研究分析了这些技术的单独应用和组合应用效果。

Result: 简单应用这些修复技术会显著降低人脸识别准确率。但选择性应用CFR-GAN结合CodeFormer的方法能产生有意义的改进效果。

Conclusion: 对于现实世界中质量较差的人脸图像，需要谨慎选择和应用修复技术。选择性组合使用2D正面化和特征增强技术是提升人脸识别性能的有效策略。
...
TL;DR: Flux4D是一个简单可扩展的4D动态场景重建框架，通过直接预测3D高斯及其运动动态，以完全无监督方式重建传感器观测，无需预训练模型或先验知识。
...
Conclusion: Flux4D提供了一个简单且可扩展的框架，用于大规模动态场景的4D重建，通过完全无监督的方式实现了高效、可扩展且泛化能力强的动态场景重建。
...
TL;DR: 利用多模态大语言模型GPT-4o和GPT-5进行零样本物体计数，仅使用文本提示，无需视觉示例或监督训练
...
Result: 在FSC-147和CARPK数据集上的评估显示，模型性能与最先进的零样本方法相当，在某些情况下甚至超过现有方法

Conclusion: 多模态大语言模型仅使用文本提示就能有效执行零样本物体计数任务，展示了其强大的视觉推理能力
...
TL;DR: 提出一种基于大语言模型的两阶段方法，从带有粗分割的3D点云直接推断材质组成，无需特定任务训练。


<details>
  <summary>Details</summary>
Motivation: 现有3D形状数据集和模型主要关注几何形状，忽略了决定物体外观的材质属性。缺乏可靠的材质标注数据，需要一种能够从几何推理到材质理解的方法。

Method: 采用两阶段大语言模型方法：第一阶段预测物体语义类别，第二阶段根据推断的语义为每个几何段分配合理的材质。两个阶段都以零样本方式运行，无需特定任务训练。

Result: 在Fusion/ABS和ShapeNet的1000个形状上，该方法实现了较高的语义和材质合理性。使用DeepEval中的LLM-as-a-Judge进行评估，证明了方法的有效性。

Conclusion: 语言模型可以作为通用先验，在3D数据中桥接几何推理和材质理解，展示了从几何信息推断材质组成的可行性。
...
Conclusion: 提出了一种通用实用的噪声合成方法，仅需最少数据即可生成真实噪声，解决了低光去噪训练数据收集难题
...
<details>
  <summary>Details</summary>
Motivation: 潜在扩散模型在图像修复和局部编辑方面取得了显著进展，但固有的潜在压缩会引入像素级不一致问题，如色移、纹理不匹配和边界可见接缝。现有解决方案在实践中通常无法完全消除这些伪影，并且无法很好地泛化到不同的潜在表示或任务。
...
Conclusion: PixPerfect提供了一个像素级细化框架，能够跨不同LDM架构和任务实现无缝、高保真的局部编辑，解决了现有方法在处理像素级不一致问题上的局限性。
...
TL;DR: 本文系统评估了多种深度学习架构用于多类火灾分类，并提出了PyroFocus两阶段管道，在保持精度的同时显著降低推理时间和计算成本，适用于机载实时火灾监测。


<details>
  <summary>Details</summary>
Motivation: 随着野火频率和严重程度的增加，需要低延迟、计算高效的机载实时火灾检测方法。多光谱和高光谱热成像仪提供丰富光谱信息，但高数据维度和有限的机载资源使实时处理具有挑战性。

Method: 系统评估了包括自定义CNN和Transformer模型在内的多种深度学习架构，并提出了PyroFocus两阶段管道：第一阶段进行火灾分类（无火、活跃火、后火），第二阶段进行火辐射功率回归或分割。使用NASA的MASTER数据进行实验。

Result: 实验结果表明，提出的两阶段管道在速度和精度之间实现了良好的权衡，显著降低了推理时间和计算成本，显示出在实时边缘部署方面的巨大潜力。

Conclusion: PyroFocus两阶段管道为未来野火监测任务中的实时机载部署提供了有效的解决方案，在保持检测精度的同时满足了实时处理的计算约束要求。
...
TL;DR: H²U3D是一个用于房屋尺度3D场景理解的数据集，包含多楼层环境；SpatialReasoner是一个主动感知框架，通过两阶段训练策略在H²U3D上实现SOTA性能，仅需3-4张图像。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要局限于房间尺度场景，在大规模3D环境中的空间推理能力不足，需要专门针对房屋尺度的3D场景理解数据集和相应方法。

Method: 1) 提出H²U3D数据集：通过自动化标注流程构建分层粗到细的视觉表示，生成多样化问答对和思维链标注；2) 提出SpatialReasoner框架：主动调用空间工具探索3D场景，采用两阶段训练策略（监督冷启动+带自适应探索奖励的强化学习）。

Result: SpatialReasoner在H²U3D上达到最先进性能，超越GPT-4o和Gemini-2.5-Pro等强基线。仅需平均3-4张图像，而基线需要16+张图像，证明了粗到细主动探索范式的有效性。
...
TL;DR: NavMapFusion：基于扩散模型的在线地图构建框架，融合低精度导航地图与高精度传感器数据，为自动驾驶提供实时准确的环境表征


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要准确的环境表征，传统高精地图需要在线构建，而广泛可用的标准导航地图精度不足但可作为先验指导在线地图构建

Method: 提出NavMapFusion扩散模型框架，以低精度导航地图为条件，结合高精度传感器数据进行迭代去噪，将先验地图与在线感知的差异对应为扩散过程中的噪声

Result: 在nuScenes基准测试中，使用OpenStreetMap道路线作为条件，在100米范围内实现21.4%的相对提升，更大感知范围提升更显著，同时保持实时能力

Conclusion: 扩散模型为地图融合提供了稳健框架，通过融合低精度先验与高精度传感器数据，生成准确且最新的环境表征，推动更安全可靠的自动驾驶
...
TL;DR: 提出SLEDGE方法，通过分层逐步生成设计，模拟设计师逐步修改的过程，相比现有单步生成方法更好地处理设计复杂性。


<details>
  <summary>Details</summary>
Motivation: 现有设计生成方法主要将设计合成视为单步生成问题，低估了创意过程的固有复杂性。设计本质上是逐步修改和完善的过程，需要更符合实际设计流程的方法。

Method: 提出"逐步分层设计生成"问题设置，要求模型根据设计师的指令序列生成设计。开发SLEDGE方法，利用多模态LLM将每次设计更新建模为基于前一个状态的原子化、分层变化，同时以指令为基础。

Result: 建立了新的评估套件（包括数据集和基准），通过详尽的实验分析和与最先进方法的比较，证明了所提方法的有效性。

Conclusion: SLEDGE方法能够更好地模拟实际设计过程，填补了现有方法的不足，希望这项工作能吸引更多对这一实用但未被充分探索的研究领域的关注。
...
TL;DR: 提出ProtoEFNet原型学习模型，通过动态时空原型捕获心脏运动模式，用于连续射血分数回归预测，在保持准确性的同时提供临床可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统射血分数评估需要人工标注且耗时，现有深度学习模型多为黑盒缺乏透明度，临床信任度低。后验解释方法无法指导模型内部推理，临床应用可靠性有限。

Method: 提出ProtoEFNet视频原型学习模型，学习动态时空原型捕获临床有意义的心脏运动模式，并设计原型角度分离损失增强连续EF谱的判别性表示。

Result: 在EchonetDynamic数据集上，ProtoEFNet达到与非可解释模型相当的准确性，同时提供临床相关洞察。消融研究表明，提出的损失函数将F1分数从77.67±2.68提升至79.64±2.10。

Conclusion: ProtoEFNet通过原型学习实现了准确且可解释的射血分数预测，原型角度分离损失有效提升性能，为临床提供透明可靠的决策支持。
...
TL;DR: HalluGen是一个基于扩散的框架，用于合成逼真的幻觉（图像中不存在但看似合理的结构），并创建了首个大规模幻觉数据集，用于评估安全关键图像恢复中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 生成模型在图像恢复中容易产生幻觉（看似合理但实际不存在的结构），这在医疗影像、工业检测和遥感等安全关键领域尤为危险，可能导致严重诊断错误。然而，评估幻觉需要标注数据，而这类标注成本高且主观，阻碍了相关研究进展。

Method: 提出了HalluGen框架，基于扩散模型合成具有可控类型、位置和严重程度的逼真幻觉。使用该框架构建了首个大规模幻觉数据集，包含4,350张标注图像（源自1,450张脑部MR图像）。还开发了SHAFE（语义幻觉评估特征评估）指标，采用基于特征的软注意力池化方法提高幻觉敏感性。

Result: HalluGen能够生成感知逼真但语义错误的输出（分割IoU从0.86降至0.36）。创建的数据集支持系统评估幻觉检测和缓解方法。SHAFE指标在幻觉敏感性上优于传统指标。训练的无参考幻觉检测器能够泛化到真实恢复失败场景。

Conclusion: HalluGen及其开放数据集为安全关键图像恢复中的幻觉评估建立了首个可扩展的基础设施，解决了评估幻觉的循环依赖问题，为开发更可靠的图像恢复系统提供了重要工具。
...
Conclusion: SeeU提出的2D→4D→2D框架能够从稀疏2D观测中学习连续4D世界动态，实现更优的视觉内容生成，为视觉理解、预测和生成任务提供了新思路。
...
TL;DR: FireSentry是一个省级规模的多模态野火数据集，具有亚米级空间分辨率和亚秒级时间分辨率，用于细粒度野火蔓延预测。基于该数据集，提出了FiReDiff双模态范式，通过先预测红外视频序列再分割火掩模，在多项指标上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有野火预测研究主要关注粗时空尺度，依赖低分辨率卫星数据，只能捕捉宏观火灾状态，限制了高精度局部火灾动态建模能力。需要细粒度数据集来提升应急响应和决策精度。

Method: 1) 创建FireSentry数据集：使用同步无人机平台采集可见光和红外视频流、现场环境测量数据及人工验证的火掩模；2) 建立包含物理模型、数据驱动模型和生成模型的综合基准；3) 提出FiReDiff双模态范式：先预测红外模态的未来视频序列，再基于生成的动态在掩模模态中精确分割火掩模。

Result: FiReDiff在生成模型上取得显著性能提升：视频质量指标PSNR提升39.2%、SSIM提升36.1%、LPIPS提升50.0%、FVD提升29.4%；掩模精度指标AUPRC提升3.3%、F1分数提升59.1%、IoU提升42.9%、MSE提升62.5%。

Conclusion: FireSentry基准数据集和FiReDiff范式共同推进了细粒度野火预测和动态灾害模拟。数据集已公开可用，为相关研究提供了高质量基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于高斯的方法存在局限性：要么使用带标注的3D标签监督封闭集语义高斯，忽略了渲染能力；要么通过纯2D自监督学习开放集高斯表示，导致几何质量下降且仅限于相机设置。需要充分利用高斯表示的潜力。

Method: 提出多模态高斯变换器，使高斯能够从多种传感器模态查询特征；提出Shelf监督学习范式，在2D图像和3D场景层面联合优化高斯特征；利用现成的视觉基础模型进行监督。

Result: 在Occ3D-nuScenes数据集上展示了最先进的零样本语义占据预测性能；在无人地面车辆上评估了其在各种城市场景中的野外性能。

Conclusion: ShelfGaussian成功整合了多模态传感器数据和视觉基础模型，实现了高效且强大的3D场景理解，在感知和规划任务中表现出色。
...
TL;DR: MOS框架通过模态一致表示学习和跨模态数据生成融合，有效缓解光学与SAR图像间的模态差异，显著提升跨模态船舶重识别性能


<details>
  <summary>Details</summary>
Motivation: 光学与合成孔径雷达(SAR)图像间的跨模态船舶重识别是海事智能监控的关键任务，但两种模态间的显著差异给鲁棒识别带来巨大挑战

Method: 提出MOS框架，包含两个核心组件：1) 模态一致表示学习(MCRL)，通过SAR图像去噪处理和类级模态对齐损失来对齐跨模态的类内特征分布；2) 跨模态数据生成与特征融合(CDGF)，利用布朗桥扩散模型合成跨模态样本，在推理时与原始特征融合以增强对齐性和判别性

Result: 在HOSS ReID数据集上的实验表明，MOS在所有评估协议中都显著超越现有最佳方法，在ALL to ALL、Optical to SAR和SAR to Optical设置下，R1准确率分别提升了+3.0%、+6.2%和+16.4%
...
TL;DR: 该论文提出了视频差异描述任务ViDiC及其对应的ViDiC-1K数据集，用于评估多模态大语言模型在描述视频对之间相似性和差异性的能力，填补了现有图像差异描述方法无法捕捉动态场景变化的空白。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言系统在理解动态场景之间的视觉差异方面存在不足，特别是对运动连续性、事件演化和编辑一致性的捕捉。虽然图像差异描述任务能够描述静态图像之间的语义变化，但无法处理视频中的时间维度变化。

Method: 1. 提出视频差异描述任务ViDiC；2. 构建ViDiC-1K数据集，包含1000个精心策划的视频对，标注了超过4000个比较检查项，涵盖7个类别；3. 采用双重检查框架，基于LLM-as-a-Judge协议分别测量相似性和差异性的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 当前可供性学习方法大多只关注"如何使用"物体，忽略了"是什么"和"在哪里"这两个方面。而其他方法将物体检测和可供性学习作为两个独立任务处理，缺乏有效交互和实时能力。
...
Conclusion: YOLOA在准确性和效率之间实现了优秀的平衡，通过LLM适配器联合处理物体检测和可供性学习，解决了可供性检测中的"是什么-在哪里-如何使用"挑战。
...
TL;DR: 提出一种利用预训练扩散模型进行标签高效高光谱图像分类的框架，通过提取早期去噪时间步的空间特征并与光谱信息融合，在稀疏标注下实现优越性能
...
Result: 在两个最新的高光谱数据集上，该方法仅使用稀疏训练标签就超越了现有最先进方法，消融研究进一步验证了扩散模型特征和光谱感知融合的优势
...
TL;DR: LM-CartSeg是一个全自动的膝关节MRI分割和放射组学分析流程，通过几何规则改进分割质量，提供稳定的内外侧分区，并提取具有诊断价值的放射组学特征。


<details>
  <summary>Details</summary>
Motivation: 膝关节MRI放射组学研究需要稳健且具有解剖学意义的ROI区域，现有方法多依赖手动标注且缺乏质量控制。本研究旨在开发一个全自动的软骨/骨分割、内外侧分区和放射组学分析流程。

Method: 使用两个3D nnU-Net模型分别在SKM-TEA和OAIZIB-CM数据集上训练，测试时融合零样本预测并通过几何规则精炼：连通分量清理、构建10mm软骨下骨带、基于PCA和k-means的数据驱动胫骨内外侧分割。从10个ROI中提取4650个非形状放射组学特征。

Result: 后处理显著改善了分割质量：OAIZIB-CM上的ASSD从2.63mm降至0.36mm，HD95从25.2mm降至3.35mm，DSC达0.91；SKI-10上的零样本DSC为0.80。几何内外侧规则在不同数据集中产生稳定分区，而直接使用nnU-Net会产生域依赖的侧向交换。只有6-12%的特征与体积或厚度强相关。

Conclusion: LM-CartSeg能够自动生成经过质量控制的ROI和放射组学特征，这些特征携带了超越简单形态测量的判别信息，为多中心膝关节骨关节炎放射组学研究提供了实用基础。
...
TL;DR: 提出无监督学习3D关键点框架，用于条件生成扩散模型，在点云数据上实现结构化表示和形状重建
...
Method: 从点云数据中无监督学习空间结构化的3D关键点，作为紧凑可解释表示，用于条件化Elucidated Diffusion Model（EDM）重建完整形状

Result: 学习到的关键点在不同物体实例间具有可重复的空间结构，支持关键点空间的平滑插值，表明其能捕捉几何变化；在多样物体类别上表现优异，关键点一致性比先前方法提高6个百分点

Conclusion: 该框架成功填补了无监督关键点学习与无条件生成模型之间的空白，为3D生成提供了结构化、可解释的表示方法
...
Method: 提出ThinkDeeper框架，核心是空间感知世界模型(SA-WM)，将当前场景提炼为指令感知的潜在状态，并展开未来潜在状态序列。配合超图引导的解码器，分层融合这些状态与多模态输入，捕获高阶空间依赖关系。

Result: 在六个基准测试中表现优异：在Talk2Car排行榜排名第一，在DrivePilot、MoCAD和RefCOCO/+/g基准上超越最先进基线。在挑战性场景（长文本、多智能体、模糊性）中表现出强大鲁棒性和效率，即使使用50%数据训练仍保持优越性能。

Conclusion: ThinkDeeper通过前瞻性空间推理有效解决了自动驾驶中的视觉定位问题，展示了世界模型在理解复杂空间关系和场景演变方面的价值，为自动驾驶系统处理自然语言指令提供了更可靠的解决方案。
...
TL;DR: TPI（文本打印图像）方法通过将文本直接渲染到白色画布上生成合成图像，实现低成本的大规模视觉语言模型训练，无需真实图像数据。
...
Result: 在4个模型和7个基准测试中，TPI比扩散模型生成的合成图像更有效地支持文本中心训练。TPI作为低成本数据增强策略也显示出实用价值。

Conclusion: 文本中心训练具有显著潜力，TPI方法为LVLM的完全自动化数据生成开辟了新路径，解决了图像收集的限制问题。
...
TL;DR: 该论文提出在视频生成中引入几何正则化损失，通过增强潜在扩散模型实现逐帧深度预测，以解决现有方法缺乏3D结构建模导致的时序不一致和结构伪影问题。
...
Method: 在潜在扩散模型中增强逐帧深度预测，采用深度作为几何表示，并提出多视角几何损失来对齐共享3D坐标系中跨帧的预测深度图。

Result: 在多个数据集上的实验表明，该方法相比现有基线能产生显著更稳定和几何一致的结果，改善了时空连贯性、形状一致性和物理合理性。

Conclusion: 该方法弥合了外观生成和3D结构建模之间的差距，通过几何正则化损失提升了视频生成的几何一致性和物理合理性。
...
TL;DR: NAS-LoRA是一种新的参数高效微调方法，通过在LoRA的编码器和解码器之间加入轻量级神经架构搜索模块，动态优化先验知识，帮助SAM模型更好地适应医学和农业等特定领域。


<details>
  <summary>Details</summary>
Motivation: SAM作为强大的视觉基础模型在图像分割中表现出色，但在适应医学和农业等特定下游任务时仍面临挑战。虽然LoRA及其变体被广泛用于提升SAM的适应性能，但Transformer编码器缺乏图像块内的空间先验，可能阻碍高级语义信息的获取。

Method: 提出NAS-LoRA方法，在LoRA的编码器和解码器组件之间加入轻量级神经架构搜索模块，动态优化先验知识集成到权重更新中。同时提出分阶段优化策略，帮助ViT编码器平衡权重更新和架构调整，逐步学习高级语义信息。

Result: 实验表明NAS-LoRA改进了现有的参数高效微调方法，同时在不增加推理成本的情况下减少了24.14%的训练成本，展示了神经架构搜索在增强视觉基础模型参数高效微调方面的潜力。

Conclusion: NAS-LoRA通过引入神经架构搜索模块和分阶段优化策略，有效解决了SAM模型在特定领域适应中的语义鸿沟问题，为视觉基础模型的高效微调提供了新思路。
...
TL;DR: PRIS提出了一种在推理时自适应修改提示词的方法，通过分析生成视觉内容中的失败模式来重新设计提示词，从而提高文本到视觉生成的准确性。


<details>
  <summary>Details</summary>
Motivation: 文本到视觉生成中，用户意图与生成结果之间的精确对齐是一个核心挑战。现有方法主要通过扩大视觉生成规模（如增加采样步骤或种子数）来改进，但这种方法很快会遇到质量瓶颈，因为提示词在整个过程中保持不变。

Method: 提出PRIS框架，在推理时自适应地修改提示词。核心思想是：1）审查生成的视觉内容；2）识别跨视觉内容的重复失败模式；3）相应地重新设计提示词；4）用修订后的提示词重新生成视觉内容。引入元素级事实校正验证器，在细粒度层面评估提示属性与生成视觉内容之间的对齐。

Result: 在文本到图像和文本到视频基准测试中进行了广泛实验，证明了方法的有效性，包括在VBench 2.0上获得15%的性能提升。结果表明，联合扩展提示词和视觉内容是充分利推理时扩展定律的关键。

Conclusion: PRIS框架通过推理时自适应修改提示词，显著提高了文本到视觉生成的对齐质量。该方法突破了传统仅扩展视觉生成规模的限制，展示了提示词与视觉内容联合扩展的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在处理结构化多步骤场景（如菜谱插图）时存在困难，无法适应菜谱长度的自然变化，通常生成固定数量的图像而不考虑实际指令结构。

Method: 提出了三个关键组件：1) 步骤区域控制(SRC)，在单个去噪过程中将文本步骤与对应图像区域对齐；2) 灵活RoPE，增强时间连贯性和空间多样性的步骤感知位置编码机制；3) 跨步骤一致性控制(CSCC)，保持步骤间细粒度食材一致性。

Result: 在菜谱插图基准测试中，CookAnything在基于训练和无训练设置下均优于现有方法，支持复杂多步骤指令的可扩展高质量视觉合成。

Conclusion: 该框架在指令媒体和程序性内容创作等领域具有广泛应用潜力，能够生成连贯、语义分明的图像序列来可视化任意长度的烹饪指令。
...
TL;DR: V-ITI是一个轻量级视觉推理时干预框架，通过检测视觉忽视并仅在需要时进行干预，有效减少多模态大语言模型的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但存在幻觉问题，生成与输入视觉内容不一致的内容，这在精度敏感领域严重影响可靠性。现有方法通常通过干预注意力分数或输出logits来缓解幻觉，但只关注"如何干预"而忽略了"何时干预"这一前提，导致"过度干预"问题，引入新的幻觉和不必要的计算开销。

Method: 首先研究视觉忽视机制，发现可以通过MLLMs中头级激活模式准确检测视觉忽视。提出V-ITI框架，包含：1）视觉忽视检测器：通过头级判别探针识别视觉忽视；2）视觉回忆干预器：仅在检测到视觉忽视时，使用预存储的视觉激活信息调制激活。

Result: 在八个基准测试和不同MLLM家族上的广泛实验表明，V-ITI能够持续缓解与视觉相关的幻觉，同时保持一般任务性能。

Conclusion: V-ITI通过智能检测视觉忽视并在需要时进行针对性干预，有效解决了多模态大语言模型的幻觉问题，避免了过度干预带来的负面影响。
...
TL;DR: 论文提出Action Effect Modeling (AEM)框架，通过联合建模动作执行和结果来改进程序性任务中的错误检测，在EgoPER和CaptainCook4D基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要分析动作执行方式，忽略了动作产生的结果（动作效果）。许多错误并不体现在执行过程中，而是体现在结果状态上，如物体状态或空间排列错误。
...
Result: 在EgoPER和CaptainCook4D基准的one-class classification (OCC)设置下取得最先进性能，证明联合建模执行和结果能实现更可靠的错误检测。

Conclusion: 建模动作执行和结果能提升错误检测的可靠性，效果感知表示有潜力为更广泛的下游应用带来益处。
...
### [30] [Dynamic Content Moderation in Livestreams: Combining Supervised Classification with MLLM-Boosted Similarity Matching](https://arxiv.org/abs/2512.03553)
*Wei Chee Yew,Hailun Xu,Sanjay Saha,Xiaotian Fan,Hiok Hian Ong,David Yuchen Wang,Kanchan Sarkar,Zhenheng Yang,Danhui Guan*

Main category: cs.CV

TL;DR: 本文提出了一种用于直播视频内容审核的混合框架，结合了监督分类和基于相似性的匹配，通过多模态大语言模型提升准确性，在生产环境中显著减少了不良内容的观看。
...
Conclusion: 该框架展示了一种可扩展且适应性的多模态内容治理方法，能够同时处理明确违规和新兴对抗行为，为大规模直播平台提供了有效的审核解决方案。
...
TL;DR: 提出公平感知的低秩适应方法，结合参数效率和显式公平优化，通过可微的MaxAccGap损失函数优化跨人口群体的准确率公平性，在青光眼眼底图像上显著减少诊断准确率差异。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医学影像任务中表现出专家级性能，但在不同人口群体间存在显著的诊断准确率差异，需要解决医疗AI的公平性问题。

Method: 提出三种方法：FR-LoRA将MaxAccGap正则化整合到训练目标中；GR-LoRA应用逆频率加权平衡梯度贡献；Hybrid-LoRA结合两种机制。使用可微的MaxAccGap损失函数进行端到端优化。

Result: 在10,000张青光眼眼底图像上评估，GR-LoRA将诊断准确率差异减少69%，同时保持53.15%的整体准确率。仅需0.24%的可训练参数，在资源受限的医疗环境中具有实际部署可行性。

Conclusion: 该方法成功结合了参数效率和公平优化，显著减少了医疗AI在不同人口群体间的诊断差异，为资源受限的医疗环境提供了实用的公平医疗AI解决方案。
...
TL;DR: 论文提出了CartoMapQA基准测试，专门评估视觉语言模型在地图理解方面的能力，包含2000多个样本，涵盖从符号识别到路线推理的多层次地图解读技能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视觉和文本信息融合方面展现出潜力，但其在地图解读方面的能力尚未得到充分探索。地图理解对于导航、地理搜索和城市规划等实际应用至关重要，需要专门的评估工具来指导模型改进。

Method: 创建了CartoMapQA基准测试数据集，包含2000多个样本，每个样本由地图图像、问题（开放式或多选题）和正确答案组成。任务涵盖低、中、高三个层次的地图解读技能：符号识别、信息提取、比例尺解读和路线推理。对开源和专有视觉语言模型进行了评估。

Result: 评估发现现有模型存在持续挑战：经常难以理解地图特定语义，地理空间推理能力有限，容易产生OCR相关错误。这些弱点表明当前模型在地图理解方面仍有明显不足。

Conclusion: CartoMapQA基准测试为评估和改进视觉语言模型的地图理解能力提供了有价值的工具，有助于开发更适合实际应用（如导航、地理搜索和城市规划）的模型。数据集和源代码已开源供研究社区使用。
...
TL;DR: EEA是一个通过语义引导和层次树搜索实现探索-利用平衡的长视频理解框架，能够高效定位关键信息并减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 当前长视频理解方法存在两个主要问题：一是密集预处理导致计算开销大，二是难以有效平衡探索与利用，导致信息覆盖不全和效率低下。

Method: EEA框架通过语义引导的层次树搜索过程实现探索-利用平衡。它自主发现并动态更新任务相关的语义查询，收集与这些查询匹配的视频帧作为语义锚点。在树搜索过程中，优先探索语义相关帧，同时确保未知片段有足够覆盖。此外，通过显式建模不确定性，将视觉语言模型的内在奖励与语义先验自适应结合。
...
TL;DR: 该论文质疑DeepSeek-OCR提出的视觉压缩对语言建模有效的假设，通过实验发现简单替代方法在相同压缩率下能达到或超越视觉压缩的重建效果，且视觉压缩在语言建模任务中表现不佳。


<details>
  <summary>Details</summary>
Motivation: DeepSeek-OCR展示了从少量视觉标记中高保真重建渲染文本的能力，引发了视觉压缩对语言模型有用的兴奋。但现有评估仅停留在重建层面，未验证这些表示是否真正有助于语言建模。

Method: 将DeepSeek-OCR的视觉编码器与两种简单替代方法进行比较：参数无关的平均池化和学习的分层编码器。在匹配的压缩率下评估文本重建质量，并测试这些压缩表示在语言建模任务中的实际效果。

Result: 简单方法（平均池化和分层编码器）在文本重建方面达到或超越了视觉编码器的性能。更重要的是，在语言建模任务中，视觉压缩方法未能超越简单的截断方法，而简单替代方法表现更好。

Conclusion: 关于视觉压缩对语言模型有用的兴奋超出了现有证据支持。视觉压缩在文本重建方面没有独特优势，且未能证明对语言建模有帮助。代码和检查点已开源。
...
TL;DR: ToG-Bench是首个面向任务的时空视频定位基准，针对第一人称视频，强调任务导向而非描述性指令，包含显式-隐式双重定位和多对象定位，基于ScanNet构建了100个标注视频和2704个任务指令。


<details>
  <summary>Details</summary>
Motivation: 现有时空视频定位研究主要局限于对象中心和描述性指令，忽视了任务导向推理对于具身智能体实现目标导向交互的重要性。需要建立连接感知与交互的基准来评估具身智能。

Method: 基于ScanNet视频构建ToG-Bench基准，包含100个标注视频片段和2704个任务导向定位指令。采用半自动化流程，结合基础模型标注和人工精炼。引入任务级评估指标，专门针对多对象和显式-隐式对象定位。

Result: 基准测试了7个最先进的多模态大语言模型，实验揭示了任务导向时空视频定位的内在挑战，以及在显式-隐式定位和多对象定位方面存在的显著性能差距。

Conclusion: ToG-Bench凸显了在具身场景中连接感知与交互的困难，为评估具身智能的任务导向定位能力提供了重要基准，揭示了现有模型在任务推理方面的局限性。
...
TL;DR: DPMFormer是一个用于领域泛化语义分割的新框架，通过领域感知提示学习和对比学习来解决视觉与文本上下文语义不对齐问题，在各种DGSS基准测试中达到最先进水平。
...
Method: 提出DPMFormer框架：1）领域感知提示学习促进视觉与文本线索的语义对齐；2）领域感知对比学习结合纹理扰动多样化可观察域；3）领域鲁棒一致性学习最小化原始图像与增强图像预测的差异。

Result: 该框架在各种领域泛化语义分割基准测试中建立了新的最先进水平，证明了其优越性。

Conclusion: DPMFormer通过解决视觉-文本语义不对齐问题，提供了一个对多样化环境变化具有鲁棒性的领域泛化语义分割框架。
...
TL;DR: CodeVision：一个基于代码即工具框架的多模态大语言模型，通过生成代码作为通用接口调用任意图像操作，提升模型在方向变化和自然损坏图像上的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在工具使用方面存在局限性，依赖有限的工具集，且对图像方向变化和自然损坏等简单变化表现出惊人的脆弱性，性能显著下降，需要更鲁棒的工具推理方法。

Method: 提出CodeVision框架，将代码作为通用工具接口；采用两阶段训练方法：先在高质量数据集上进行监督微调，然后使用带有密集过程奖励函数的强化学习来鼓励策略性和高效的工具使用。

Result: 在Qwen2.5-VL和Qwen3-VL系列上的实验表明，该方法显著提升了模型性能，并促成了灵活工具组合、高效链式执行和从运行时反馈中鲁棒错误恢复等新兴能力。

Conclusion: CodeVision提供了一个灵活且可扩展的代码即工具框架，解决了当前多模态大语言模型在工具使用和鲁棒性方面的局限性，为更强大的视觉推理系统奠定了基础。
...
TL;DR: BlackCAtt是一种黑盒对抗攻击方法，通过识别因果像素集来生成可解释、不可察觉、可复现且架构无关的对抗样本，针对目标检测器实现边界框的删除、修改或添加。


<details>
  <summary>Details</summary>
Motivation: 现有对抗扰动方法多为白盒且架构特定，难以理解其工作原理。需要开发能够解释攻击机制的黑盒方法，帮助开发者理解和分析攻击，并微调模型进行防御。

Method: BlackCAtt结合目标检测器产生的边界框和因果像素集，构建黑盒对抗攻击。该方法使用最小、因果充分的像素集来创建攻击，不依赖检测器内部架构，将其视为黑盒处理。

Result: 在COCO测试数据集上，BlackCAtt在删除检测方面比基线好2.7倍，在修改检测方面好3.86倍，在触发新虚假检测方面好5.75倍。生成的攻击与原始图像非常接近，几乎不可察觉。

Conclusion: 因果像素识别能够产生更精确、更不可察觉的对抗攻击。BlackCAtt展示了因果像素在创建架构无关、可解释的黑盒对抗攻击方面的强大能力。
...
TL;DR: 本文提出了一种基于计算机视觉的舞蹈动作自动分析框架，结合YOLOv8/v11进行舞者检测和SAM进行精确分割，实现了无标记的舞蹈动作跟踪和量化分析。
...
Method: 提出概念验证框架：使用YOLOv8/v11进行舞者检测，结合Segment Anything Model (SAM)进行精确像素级分割，实现对舞蹈动作的跟踪、步数统计、空间覆盖模式分析和节奏一致性测量。

Result: 在49秒的加纳AfroBeats舞蹈视频测试中，系统检测精度约94%，召回率约89%；SAM分割的IoU约83%。分析显示主要舞者比次要舞者多执行23%的舞步，运动强度高37%，使用空间多42%。

Conclusion: 该框架证明了技术可行性，为定量舞蹈指标提供了有前景的方向，但仍是早期研究，存在单视频验证、缺乏系统标注和与现有姿态估计方法比较等局限性，为未来系统验证研究奠定了基础。
...
Method: 提出AdaptVision范式：1）从低分辨率图像处理压缩视觉token；2）必要时调用边界框工具裁剪关键区域获取额外视觉信息；3）使用解耦轮次策略优化（DTPO）的强化学习框架，将学习目标分解为工具学习和准确性改进两个组件，并分别计算优势估计。

Result: 在多个VQA基准测试中，AdaptVision相比现有高效VLM方法，在消耗显著更少视觉token的同时，实现了更优越的性能表现。

Conclusion: AdaptVision通过自适应视觉token获取机制和DTPO优化框架，成功实现了视觉语言模型在准确性和效率之间的更好平衡，为高效VLM提供了新范式。
...
TL;DR: 提出Stable Signer模型，将手语生成重新定义为包含文本理解和Pose2Vid的端到端分层生成任务，通过SLUL和SLP-MoE模块显著提升生成质量


<details>
  <summary>Details</summary>
Motivation: 传统手语生成方法存在文本转换不准确、姿势生成误差、姿势到视频渲染问题，导致误差累积，进展缓慢。需要简化冗余结构，优化任务目标

Method: 1. 重新定义SLP为分层端到端任务（文本理解+ Pose2Vid）；2. 提出SLUL（手语理解链接器）进行文本理解；3. 使用SLP-MoE手势渲染专家块生成手势；4. 训练SLUL时采用新开发的SAGM损失函数

Result: 性能相比当前SOTA生成方法提升了48.6%，能够端到端生成高质量、多风格的手语视频

Conclusion: Stable Signer通过简化传统冗余结构、优化任务目标，实现了更准确的手语视频生成，显著提升了生成质量
...
TL;DR: FloodDiffusion是一个用于文本驱动的流式人体运动生成的新框架，通过改进的扩散强制方法在时间变化文本提示下生成对齐、无缝的运动序列，在HumanML3D基准上达到0.057的FID分数。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖分块或自回归模型处理流式运动生成，存在局限性。需要开发能够处理时间变化文本提示、生成无缝运动序列且具有实时延迟的新框架。

Method: 采用扩散强制框架建模时间序列生成任务，但发现原始扩散强制方法无法建模真实运动分布。改进包括：使用双向注意力而非因果注意力；实现下三角时间调度器而非随机调度；采用连续时间变化方式引入文本条件。

Result: 在流式运动生成任务上达到最先进性能，HumanML3D基准上FID为0.057，首次证明扩散强制框架在该任务上的有效性。

Conclusion: FloodDiffusion通过改进的扩散强制框架成功解决了文本驱动的流式人体运动生成问题，实现了高质量、无缝的运动序列生成，为相关领域提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注静态高质量图像，忽略了时序退化和错误传播这两个关键失效模式，即瞬时视觉损坏引发的幻觉会在后续帧中持续存在，这对于自动驾驶等安全关键应用中的VLM部署至关重要。

Method: 1) 提出DIQ-H基准，应用基于物理的视觉退化（运动模糊、传感器噪声、压缩伪影）；2) 通过多轮问答任务测量幻觉持续性、错误恢复和时序一致性；3) 提出不确定性引导迭代精炼（UIR）方法，使用轻量级VLM通过不确定性过滤生成可靠的伪真值标注。

Result: 1) UIR方法实现了15.3%的准确率提升；2) 在16个最先进的VLM上实验显示显著的鲁棒性差距：即使是GPT-4o也只有78.5%的恢复率，而开源模型在时序一致性上低于60%；3) DIQ-H为评估真实世界部署中的VLM可靠性提供了全面平台。

Conclusion: DIQ-H基准填补了现有评估在动态视觉退化方面的空白，揭示了VLM在时序序列中的鲁棒性不足，为未来模型改进和真实世界部署提供了重要评估工具。
...
Method: 提出文本嵌入扰动方法：1）引入基于步骤的文本嵌入扰动，结合频率引导的噪声调度与空间噪声扰动；2）根据频率特定贡献和扰动容忍度选择性调整扰动强度。
...
Result: 在PartNet-Mobility数据集上的大量定性和定量实验证明了方法的有效性，相比先前方法取得了更优性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在长视频理解中面临上下文长度限制和密集视频标记处理成本高的问题，现有查询感知帧选择方法计算开销大，需要更高效的解决方案。
...
Conclusion: 复杂搜索机制并非普遍必要，DIG通过查询类型自适应策略实现了高效且有效的长视频理解帧选择，为LMM在长视频应用提供了实用解决方案。
...
TL;DR: GLASTE是一个端到端的场景文本编辑框架，通过全局-局部感知机制解决了现有方法在一致性和长度敏感性方面的挑战，在保持文本样式和背景纹理的同时，能够处理编辑前后文本长度的显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有场景文本编辑方法存在两个主要挑战：不一致性和长度不敏感性。它们经常无法保持编辑后的局部补丁与周围区域的一致性，并且难以处理编辑前后文本长度的显著差异。

Method: 提出了GLASTE框架，采用全局-局部组合结构，结合全局和局部损失，增强文本图像特征。将文本样式表示为与图像大小无关的向量，可转移到不同大小的目标文本图像。使用仿射融合将目标文本图像填充到编辑补丁中，同时保持其纵横比不变。

Result: 在真实世界数据集上的大量实验验证表明，GLASTE模型在定量指标和定性结果上都优于先前的方法，并有效缓解了两个主要挑战。

Conclusion: GLASTE通过全局-局部感知机制成功解决了场景文本编辑中的一致性和长度敏感性挑战，实现了更高质量的文本编辑效果。
...
TL;DR: 该研究探讨了草图表示学习中时间顺序的重要性，发现绝对坐标优于相对坐标，非自回归解码器优于自回归解码器，且时间重要性取决于具体顺序和任务。


<details>
  <summary>Details</summary>
Motivation: 尽管草图表示学习领域已有显著进展，但研究者对时间顺序在草图表示质量中的真正相关性仍缺乏深入理解。本研究旨在探究将草图视为序列是否合理，以及哪些内部顺序更具相关性。

Method: 研究通过比较不同顺序表示方法（绝对坐标vs相对坐标）和解码器类型（自回归vs非自回归）来评估草图的时间顺序重要性。实验分析了不同顺序对草图表示质量的影响。

Result: 结果显示：1）虽然传统位置编码对将草图建模为序列是有效的，但绝对坐标始终优于相对坐标；2）非自回归解码器优于自回归解码器；3）时间顺序的重要性取决于所考虑的顺序和评估的任务。

Conclusion: 草图的时间顺序确实对表示质量有重要影响，但并非所有顺序都同等重要。绝对坐标表示和非自回归解码器在草图表示学习中表现更优，且时间顺序的重要性具有任务依赖性。
...
<details>
  <summary>Details</summary>
Motivation: 不同于基于注意力的压缩方法，本文从信息论角度出发，旨在约束计算预算下最大化视频表示的信息保真度，解决视觉压缩中的冗余问题

Method: 提出信息独特性概念衡量token间内在冗余，设计三个模块：帧组融合、token分配和空间动态压缩，逐步进行语义帧分组、自适应资源分配和细粒度空间压缩

Result: 大量实验表明，UniComp在有限计算预算下持续优于现有压缩方法，能更好地保留关键视觉token，证明了信息独特性在token压缩中的关键作用

Conclusion: 信息独特性驱动的视频压缩框架UniComp通过优化条件熵最小化，在约束计算资源下实现了更有效的视觉信息保留，为视频压缩提供了新视角
...
TL;DR: PSA是一种新型稀疏注意力机制，通过多级池化KV表示替代二值掩码，在保持计算效率的同时减少信息损失，适用于视频理解和生成任务。
...
Result: 在视频理解和生成基准测试中，PSA在保持上下文信息和视觉保真度方面表现优异，在效率和质量的权衡上优于现有稀疏注意力基线方法，或达到相当性能。

Conclusion: PSA通过多级池化KV表示有效缓解了稀疏注意力中的信息损失问题，在有限计算预算下实现了更好的效率-质量平衡，为视频理解和生成任务提供了有效的稀疏注意力解决方案。
...
TL;DR: 该研究提出了一种跨染色对比学习框架，利用多染色病理切片数据提升H&E染色的通用表征能力，通过两阶段预训练解决染色间错位问题，在多种病理任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 计算病理学需要通用的全切片图像表征。虽然多标记染色（如免疫组化）能丰富H&E染色特征，但现有数据集中染色间错位问题严重，导致特征不一致，限制了跨染色表征学习的发展。

Method: 首先构建了五染色对齐数据集（H&E、HER2、KI67、ER、PGR），然后提出两阶段预训练框架：1）使用轻量适配器进行补丁级对比对齐，提升H&E特征与IHC特征的兼容性；2）采用多实例学习进行切片级表征学习，包含跨染色注意力融合模块和全局对齐模块。

Result: 在癌症亚型分类、IHC生物标志物状态分类和生存预测等任务上均取得一致提升，生成了高质量、可迁移的H&E切片级表征。

Conclusion: 提出的跨染色对比学习框架有效解决了多染色病理数据中的错位问题，显著提升了H&E染色的表征能力，为计算病理学提供了更强大的通用表征工具。
...
TL;DR: DOT-BI是一种利用人类运动感知来区分人类和机器人的动态光学测试方法，通过隐藏数字与背景的纹理相同但运动模式不同来实现人类可识别而算法难检测。


<details>
  <summary>Details</summary>
Motivation: 当前在线调查和流程中需要有效区分人类用户和自动化系统（机器人），现有方法可能被先进AI模型绕过，因此需要一种基于人类感知特性的验证方法。

Method: DOT-BI显示一个"隐藏"数字，该数字与其背景具有相同的随机黑白像素纹理，但通过数字与背景之间的运动和尺度差异，使人类能够在多帧中感知到数字，而逐帧算法处理无法获得有意义的信号。

Result: 1. 最先进的视频多模态模型（GPT-5-Thinking和Gemini 2.5 Pro）即使被告知机制也无法提取正确值；2. 在线调查中99.5%参与者成功完成任务，平均完成时间10.7秒；实验室研究中未发现对感知易用性或完成时间的负面影响。

Conclusion: DOT-BI是一种快速简便的机器人识别方法，基于人类运动感知特性，能有效区分人类和自动化系统，且用户体验良好，已发布代码和预渲染变体以促进采用。
...
TL;DR: 该论文在生成模型效率和实际计算机视觉应用两方面做出贡献：第一部分通过6项创新改进归一化流架构；第二部分展示了5个实际应用案例，包括农业质量评估、地质制图、隐私保护和艺术修复。


<details>
  <summary>Details</summary>
Motivation: 论文旨在解决生成模型（特别是归一化流）的效率问题，并将其应用于实际计算机视觉挑战中，包括农业质量评估、地质制图、自动驾驶隐私保护和艺术修复等现实世界问题。

Method: 第一部分：提出6项归一化流架构改进：1）可逆3x3卷积层及其可逆性数学证明；2）更高效的Quad-coupling层；3）kxk卷积层快速并行反演算法；4）卷积逆运算的高效反向传播算法；5）在Inverse-Flow中使用卷积逆进行前向传播；6）Affine-StableSR超分辨率模型。第二部分：1）使用条件GAN的农业产品质量评估系统；2）基于堆叠自编码器的无监督地质制图框架；3）基于人脸检测和图像修复的自动驾驶数据集隐私保护方法；4）使用Stable Diffusion进行图像修复以保护隐私；5）适应多种退化类型的艺术修复扩散模型。

Result: 提出的归一化流改进显著提升了模型效率；农业质量评估系统在种子纯度测试中取得良好准确率；地质制图框架相比传统方法改进了特征提取；隐私保护方法有效保护了自动驾驶数据集中的敏感信息；艺术修复模型能够统一处理多种退化类型。

Conclusion: 论文在生成模型的理论改进和实际应用两方面都做出了重要贡献，不仅提升了归一化流的计算效率，还成功将生成模型应用于多个现实世界的计算机视觉问题，展示了生成模型在解决实际挑战中的强大潜力。
...
<details>
  <summary>Details</summary>
Motivation: RNA逆折叠问题是RNA设计中的关键挑战，需要找到能够折叠成特定二级结构的核苷酸序列，这对分子稳定性和功能至关重要。由于序列与结构之间的复杂关系，这一任务具有内在的复杂性。

Method: HyperRNA框架包含三个主要组件：预处理、编码和解码。预处理阶段基于3-bead粗粒度表示提取RNA骨架原子坐标构建图结构；编码阶段使用注意力嵌入模块和超图编码器处理这些图，捕捉高阶依赖和复杂生物分子相互作用；解码阶段以自回归方式生成RNA序列。

Result: 在PDBBind和RNAsolo数据集上进行的定量和定性实验表明，HyperRNA在RNA序列生成和RNA-蛋白质复合物序列生成的逆折叠任务中，不仅优于现有的RNA设计方法，还突显了利用超图在RNA工程中的潜力。

Conclusion: HyperRNA是一个有效的RNA逆折叠框架，通过超图建模能够更好地捕捉RNA结构中的复杂关系，为RNA工程提供了新的方法。
...
TL;DR: CloseUpAvatar是一种新颖的关节人体化身表示方法，通过使用两组可学习纹理（低频和高频细节）以及根据相机距离自动切换纹理的方法，在更广泛的相机运动范围内保持渲染质量，特别是近距离视图的渲染效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理更广泛的相机运动时，难以保持近距离视图的渲染质量。需要一种能够根据相机距离自动调整渲染细节的方法，以在更广泛的相机方向范围内实现逼真渲染。

Method: 使用带纹理的平面集合表示化身，包含两组可学习纹理：低频和高频细节纹理。系统根据相机与化身表面的距离自动切换到高频纹理（近距离时），随着相机远离逐渐减少其影响。通过限制所需图元数量来保持高帧率。

Result: 在ActorsHQ数据集的高分辨率输入图像上进行实验，CloseUpAvatar在从新颖的广泛相机位置渲染方面，相比现有方法在定性和定量上都显示出改进，同时通过限制图元数量保持高FPS。

Conclusion: CloseUpAvatar通过基于相机距离的纹理切换机制，能够在更广泛的相机运动范围内保持渲染质量，特别是改善了近距离视图的渲染效果，同时保持实时性能。
...
<details>
  <summary>Details</summary>
Motivation: 牙齿部分点云常因遮挡和扫描视角限制存在大面积缺失区域，这会偏置编码器的全局特征并迫使解码器产生幻觉结构

Method: 提出检索增强框架，将原型记忆库集成到标准编码器-解码器流程中。编码部分输入后，从可学习记忆库中检索最近邻流形原型，通过置信度门控加权与查询特征融合，然后解码。记忆库端到端优化，无需牙齿位置标签即可自组织为可重用牙齿形状原型
...
TL;DR: Motion4D：一个将2D基础模型先验集成到统一4D高斯泼溅表示中的框架，解决了2D模型在动态场景分析中缺乏3D一致性的问题，显著提升了点追踪、视频对象分割和新视角合成等任务的性能。
...
Method: Motion4D采用两部分迭代优化框架：1）顺序优化，分阶段更新运动和语义场以保持局部一致性；2）全局优化，联合优化所有属性以实现长期一致性。还引入了3D置信度图动态调整运动先验，以及基于像素RGB和语义误差的自适应重采样过程。通过迭代细化过程增强语义一致性。

Result: 广泛评估表明，Motion4D在点追踪、视频对象分割和新视角合成等多种场景理解任务上，显著优于2D基础模型和现有的3D方法。

Conclusion: Motion4D成功地将2D基础模型的先验知识集成到统一的4D表示中，有效解决了3D一致性问题，为动态场景理解提供了更准确和连贯的解决方案。
...
TL;DR: LAMP使用大语言模型作为运动规划器，将自然语言描述转换为动态物体和相机的3D轨迹，通过运动领域特定语言实现程序合成，建立了首个直接从自然语言生成物体和相机运动的框架。


<details>
  <summary>Details</summary>
Motivation: 视频生成在视觉保真度和可控性方面取得了显著进展，但运动控制（指定物体动态和相机轨迹）对于创作复杂的电影场景至关重要，而现有接口仍然有限。需要一种能够将自然语言描述直接转换为3D运动轨迹的方法。
...
Result: 实验表明，与最先进的替代方案相比，LAMP在运动可控性和与用户意图的对齐方面表现更好，建立了首个直接从自然语言规范生成物体和相机运动的框架。

Conclusion: LAMP通过将大语言模型作为运动规划器，成功实现了从自然语言到3D运动轨迹的转换，为视频生成中的运动控制提供了新的有效方法，特别是在物体动态和相机轨迹的精确控制方面取得了突破。
...
TL;DR: ReCamDriving是一个纯视觉、相机控制的新轨迹视频生成框架，利用密集完整的3DGS渲染提供几何指导，实现精确的相机可控生成


<details>
  <summary>Details</summary>
Motivation: 现有修复方法无法恢复复杂伪影，LiDAR方法依赖稀疏不完整的线索，需要一种能提供精确相机控制和结构一致性的视频生成方法

Method: 采用两阶段训练范式：第一阶段使用相机姿态进行粗略控制，第二阶段加入3DGS渲染进行细粒度视角和几何指导；提出3DGS跨轨迹数据策展策略消除训练测试差距

Result: 构建了ParaDrive数据集（包含11万+平行轨迹视频对），实验表明ReCamDriving在相机可控性和结构一致性方面达到最先进水平

Conclusion: ReCamDriving通过利用3DGS渲染的密集几何指导，实现了精确的相机可控视频生成，解决了现有方法的局限性
...
TL;DR: Colon-X是一个推进结肠镜多模态智能的开源项目，包含ColonVQA数据集（110万+视觉问答条目）和ColonReason推理数据集，开发了ColonR1模型，在数据稀缺条件下表现优于监督微调25.22%。


<details>
  <summary>Details</summary>
Motivation: 推动结肠镜多模态智能发展，解决从多模态理解到临床推理的关键过渡问题，建立社区数据基础并提升临床输出的鲁棒性和可信度。

Method: 1) 构建ColonVQA数据集（76个临床发现，18个多模态任务）；2) 评估22个多模态大语言模型的泛化能力和扰动可靠性；3) 通过多专家辩论流程标注ColonReason推理数据集；4) 开发ColonR1模型，采用任务自适应奖励和梯度稳定优化技术。

Result: 1) 当前领先MLLMs的临床输出远未达到鲁棒可信；2) ColonR1在数据稀缺条件下达到56.61%总体准确率，比监督微调提升25.22%；3) 为多模态结肠镜分析建立了新的推理基准。

Conclusion: Colon-X项目成功构建了全面的结肠镜多模态数据集和推理模型，填补了从多模态理解到临床推理的空白，为社区提供了开源数据资源和新的推理基准。
...
TL;DR: ConvRot是一种基于分组旋转的量化方法，通过正则Hadamard变换抑制扩散变换器中的行列异常值，实现W4A4推理，无需重新训练即可保持图像质量，速度提升2.26倍，内存减少4.05倍。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在生成高质量图像方面表现出色，但随着模型规模增大，内存占用和推理延迟显著增加，给实际部署带来挑战。现有基于旋转的量化方法存在较大开销，且难以处理扩散变换器中的行级异常值。

Method: 提出ConvRot方法，采用分组旋转量化，利用正则Hadamard变换抑制行列异常值，将复杂度从二次降低到线性。在此基础上设计ConvLinear4bit模块，集成旋转、量化、GEMM和反量化操作，实现无需重新训练的W4A4推理。

Result: 在FLUX.1-dev上的实验显示，ConvRot实现了2.26倍的速度提升和4.05倍的内存减少，同时保持了图像保真度。这是首次将基于旋转的量化应用于扩散变换器的即插即用W4A4推理。

Conclusion: ConvRot通过创新的分组旋转量化方法，有效解决了扩散变换器部署中的内存和延迟问题，为高效图像生成提供了实用的解决方案。
...
TL;DR: GaussianBlender是一个前馈式文本驱动3D风格化框架，能够实时推理编辑，无需逐资产优化，解决了现有方法的多视角不一致问题。
...
Result: GaussianBlender不仅能够实现即时、高保真、几何保持、多视角一致的风格化，而且超越了需要逐实例测试时优化的方法，实现了可扩展的实用3D风格化。
...
<details>
  <summary>Details</summary>
Motivation: 传统被动视觉系统在复杂环境中信息获取不足，需要开发能够主动与环境交互、根据目标调整行为的智能感知系统，以应对机器人、自动驾驶等领域的实际需求。

Method: 本文采用综述研究方法，系统分析主动视觉感知的理论基础、技术实现和应用案例，探讨其核心机制（注意力引导、传感器移动、物体交互）以及面临的挑战。

Result: 论文全面梳理了主动视觉感知在多个应用领域的重要作用，识别了实时处理复杂视觉数据、动态环境决策、多模态传感器融合等关键技术挑战，并指出了未来研究方向。

Conclusion: 主动视觉感知是智能系统发展的关键方向，虽然面临诸多技术挑战，但其在提升环境理解能力和适应性方面具有巨大潜力，需要进一步研究以推动更广泛的应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有感知相似性评分方法存在局限性：深度感知损失（如LPIPS）虽然与人类视觉对齐良好，但依赖复杂非线性特征且缺乏可解释性；手工设计的方法（如SSIM）可解释但缺少关键感知特性。需要一种既与人类感知对齐又具有可解释性的方法。

Method: 提出结构化不确定性相似性评分（SUSS）：1）将图像建模为一组感知组件，每个组件用结构化多元正态分布表示；2）通过生成式自监督训练，使模型为人类不可感知的增强分配高似然；3）最终评分是组件对数概率的加权和，权重从人类感知数据集中学习；4）在像素空间学习图像特定的线性残差变换，支持通过去相关残差和采样进行透明检查。

Result: SUSS与人类感知判断高度对齐，在不同失真类型上表现出强大的感知校准能力，并提供局部化、可解释的相似性评估解释。作为感知损失函数用于下游成像任务时，表现出稳定的优化行为和竞争性性能。

Conclusion: SUSS成功结合了深度感知损失的良好对齐性和手工设计方法的可解释性，通过结构化概率建模在像素空间提供透明、可检查的感知相似性评估，为计算机视觉模型的训练和评估提供了更好的工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决从非结构化互联网图像进行大规模3D重建时的图像匹配挑战，特别是在处理大规模图像集合时如何有效匹配图像对。

Method: 结合数据集自适应图像配对策略与旋转感知关键点提取匹配：1) 使用DINO检索语义相关图像对；2) 通过旋转增强捕捉方向依赖的局部特征，使用ALIKED和Light Glue进行匹配。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在大型数据集上训练时，会学习并复制数据中存在的偏见，导致生成刻板印象的输出。现有方法通常需要人工标注数据集或针对每个生成概念训练外部分类器，限制了实际应用。

Method: SelfDebias在推理过程中识别图像编码器嵌入空间中的语义聚类，使用这些聚类来引导扩散过程，最小化输出分布与均匀分布之间的KL散度。该方法完全无监督，不需要人工标注数据或外部分类器。

Result: 实验表明SelfDebias能够泛化到不同的提示词和扩散模型架构，包括条件模型和无条件模型。它不仅能有效减少关键人口统计维度上的偏见，同时保持生成图像的视觉保真度，还能处理更抽象概念的偏见识别。

Conclusion: SelfDebias提供了一种完全无监督的测试时去偏解决方案，能够自动识别语义模式并减少扩散模型中的偏见，为生成更公平、多样化的图像提供了一种有效方法。
...
TL;DR: 提出LSRS方法，通过在推理阶段对VAR模型的潜在尺度进行渐进式拒绝采样，使用轻量级评分模型评估候选token映射，选择高质量映射指导后续尺度生成，显著提升生成质量且计算开销很小。


<details>
  <summary>Details</summary>
Motivation: VAR模型在并行处理同一尺度内多个token时可能导致结构错误，产生次优生成图像。需要一种方法在推理阶段缓解自回归误差累积，同时保持计算效率。

Method: 提出潜在尺度拒绝采样(LSRS)：1) 在每个尺度采样多个候选token映射；2) 使用轻量级评分模型评估候选质量；3) 选择高质量映射指导后续尺度生成；4) 优先处理对结构一致性关键的早期尺度。

Result: LSRS显著提升VAR生成质量：VAR-d30模型的FID从1.95降至1.78，推理时间仅增加1%；当推理时间增加15%时，FID可进一步降至1.66。在最小计算开销下实现质量提升。

Conclusion: LSRS为VAR模型提供了高效的测试时扩展解决方案，有效缓解自回归误差累积，显著提升生成质量，同时保持计算效率，是增强VAR生成能力的实用方法。
...
<details>
  <summary>Details</summary>
Motivation: 古埃及象形文字完全由图画组成，翻译面临单个字符多义性等挑战。深度学习翻译应用快速发展，为自动翻译象形文字提供了可能。

Method: 采用三阶段方法：1) 使用Contour和Detectron2进行分割；2) 将符号映射到Gardiner编码；3) 使用CNN模型进行翻译。使用Morris Franken和EgyptianTranslation两个数据集进行分类和翻译。

Result: 模型取得了42.2的BLEU评分，相比先前研究有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视频的呼吸率测量方法要么在理想条件下测试，要么精度不足，需要一种更鲁棒的方法来应对受试者运动干扰

Method: 结合数学变换的方法，硬件要求极低，仅需普通摄像头，能够抵抗受试者运动引起的失真
...
TL;DR: CoDA提出了一种无需目标数据集预训练扩散模型的数据集蒸馏框架，通过识别目标数据集的内在核心分布并引导生成过程与之对齐，在ImageNet-1K等基准上取得了SOTA性能。
...
Result: CoDA在不依赖目标数据集特定训练生成模型的情况下，在所有基准测试（包括ImageNet-1K及其子集）上达到或超越了依赖此类模型的方法性能，在ImageNet-1K的50-IPC设置下创下了60.4%的新SOTA准确率。

Conclusion: CoDA成功解决了数据集蒸馏中预训练模型依赖和分布不匹配的核心问题，通过核心分布对齐机制实现了高效的数据集蒸馏，为使用现成文本到图像模型进行数据集蒸馏提供了有效解决方案。
...
TL;DR: 该研究公开了一个来自伊拉克埃尔比勒的339张前列腺穿刺活检全切片图像数据集，包含185名患者的Gleason评分和ISUP分级，旨在支持AI模型在不同人群中的开发和验证。


<details>
  <summary>Details</summary>
Motivation: 当前公开的病理学数据集稀缺且主要代表西方人群，AI模型对中东等数字化程度较低地区人群的泛化能力未知，因此需要发布多样化的数据集来支持全球病理AI模型的开发和验证。

Method: 收集了伊拉克埃尔比勒185名患者的339张前列腺穿刺活检全切片图像，由三位病理学家独立分配Gleason评分和ISUP分级，使用Leica、Hamamatsu和Grundium三种扫描仪扫描，所有切片均已去标识化并以原始格式提供。

Result: 创建了一个包含339张前列腺活检全切片图像的数据集，来自中东地区（伊拉克）的185名患者，具有三位病理学家独立标注的Gleason评分和ISUP分级，使用三种不同扫描仪获取，为AI模型验证提供了多样化数据。

Conclusion: 该数据集填补了中东地区公开病理数据集的空白，支持AI模型在不同人群中的泛化性评估、分级一致性分析、颜色归一化和跨扫描仪鲁棒性评估，将促进全球病理AI模型的公平发展。
...
<details>
  <summary>Details</summary>
Motivation: 虽然基于Transformer的架构在计算机视觉和NLP领域取得了巨大成功，但它们通常需要大量参数和训练数据才能获得强大性能。本研究旨在探索在小型5M参数视觉Transformer上，不同预训练、中间微调和下游数据集及训练目标的边际效益。

Method: 使用三种不同的预训练、中间微调和下游数据集及训练目标进行实验，评估它们对小型5M参数视觉Transformer性能的影响。研究特别关注中间微调阶段对最终下游任务表现的影响。

Result: 研究发现：1）预训练和微调总是有帮助，但收益递减；2）中间微调可能对下游性能产生有害影响，这可能是因为任务机制的不相似性；3）不当的中间任务堆叠会浪费计算资源并降低性能。

Conclusion: 小型视觉Transformer最受益于有针对性的预训练和仔细的数据选择，而不加区分的中间任务堆叠会浪费计算资源甚至降低性能。这为资源受限环境下的模型训练提供了重要指导。
...
TL;DR: CaravelMetrics是一个用于自动化脑血管分析的框架，通过骨架化图表示建模血管形态，计算15个形态、拓扑、分形和几何特征，支持全局和区域多尺度分析。


<details>
  <summary>Details</summary>
Motivation: 开发一个可扩展、全自动的脑血管定量特征提取框架，支持规范建模和群体水平的血管健康与衰老研究，解决传统脑血管分析方法的局限性。

Method: 整合基于图谱的区域分割、中心线提取和图构建，通过骨架化图表示建模血管形态，计算15个形态测量、拓扑、分形和几何特征，支持全局和动脉区域多尺度分析。

Result: 在IXI数据集的570个3D TOF-MRA扫描（年龄20-86岁）上应用，获得可重复的血管图，捕捉到年龄和性别相关变化，以及教育程度相关的血管复杂性增加，与文献报道一致。

Conclusion: CaravelMetrics提供了一个可扩展、全自动的脑血管定量特征提取方法，支持规范建模和群体水平的血管健康与衰老研究，为脑血管组织分析提供了有效工具。
...
TL;DR: 提出SSDCA模型，结合纵向内窥镜图像，在直肠癌等待观察期间区分完全缓解与局部复发


<details>
  <summary>Details</summary>
Motivation: 直肠癌患者在总新辅助治疗后临床完全缓解时采用等待观察策略，但需要客观准确的方法从随访内窥镜图像中早期检测局部复发，以管理护理并预防远处转移

Method: 开发了具有双交叉注意力的Siamese Swin Transformer（SSDCA），结合重新分期和随访时的纵向内窥镜图像，利用预训练的Swin transformer提取领域无关特征，增强对成像变化的鲁棒性，通过双交叉注意力强调两次扫描的特征，无需图像空间对齐
...
Conclusion: SSDCA能够有效结合纵向内窥镜图像，准确区分临床完全缓解和局部复发，为直肠癌等待观察期间的监测提供了可靠工具
...
<details>
  <summary>Details</summary>
Motivation: 现有的零样本方法主要关注在注意力机制中集成帧间对应关系，但软约束不足以识别有效特征，导致时间不一致性问题。

Method: FRESCO整合了帧内对应关系和帧间对应关系，形成更鲁棒的时空约束，确保语义相似内容在帧间的一致转换，并显式优化特征。

Result: 在视频到视频转换和文本引导视频编辑两个零样本任务上验证了有效性，实验表明能生成高质量、连贯的视频，显著优于当前零样本方法。

Conclusion: FRESCO通过增强的时空约束机制，在保持零样本优势的同时，显著提升了视频操作的视觉一致性和质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法主要关注以另一种模态为条件生成单一模态，或将其中一种与其他模态（如文本、音频）集成。统一2D视频和3D运动进行同时优化和生成仍然未被充分探索，由于它们在结构和分布上的显著差异，这带来了重大挑战。

Method: 1. 受LLM统一不同模态能力的启发，将视频和3D运动建模为统一的token序列，使用单独的嵌入层来缓解分布差距
2. 设计序列建模策略，在单一框架中集成两个不同的任务
...
TL;DR: TempR1是一个针对多模态大语言模型的时间感知多任务强化学习框架，通过系统化的多任务训练增强模型的时间理解能力，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 增强多模态大语言模型的时间理解能力对于推进长视频分析至关重要，但现有的强化学习方法局限于有限的任务类型和数据，难以适应多样化的时间理解场景。

Method: 提出了TempR1框架，构建多任务语料库暴露模型于多样化的时间结构和语义，基于GRPO算法实现稳定有效的跨任务优化。将时间任务分为三种预测区间与真实实例对应类型，并为每种类型设计定制化的定位奖励。

Result: 在多个基准测试中达到最先进性能，联合优化互补任务产生强大的协同效应，既增强了泛化能力也提升了单任务性能。

Conclusion: TempR1为MLLMs中的时间推理建立了一个可扩展且原则性的范式，通过系统化的多任务强化学习有效增强了模型的时间理解能力。
...
Method: 提出UniID统一框架，采用身份聚焦学习方案训练两个分支专门捕获身份特征，在推理时通过归一化重缩放机制恢复基础扩散模型的文本控制能力，同时让互补的身份信号相互增强。

Result: 与六种最先进方法相比，UniID在身份保持和文本控制方面都表现出优越性能。

Conclusion: UniID通过统一两种范式并采用原则性设计，实现了高保真人脸个性化与灵活文本控制的平衡。
...
TL;DR: DirectDrag是一个无需手动遮罩和文本提示的基于拖拽的图像编辑框架，通过自动软遮罩生成和读取引导的特征对齐机制，实现了高保真度的精确图像编辑。
...
Method: DirectDrag提出两个关键创新：1) 自动软遮罩生成模块，从点位移智能推断可编辑区域，自动沿移动路径定位变形；2) 读取引导的特征对齐机制，利用中间扩散激活来保持基于点编辑时的结构一致性。

Result: 尽管无需手动遮罩或提示，DirectDrag在图像质量方面优于现有方法，同时保持了有竞争力的拖拽精度。在DragBench和真实场景上的广泛实验证明了其有效性和实用性。

Conclusion: DirectDrag是一个新颖的无需遮罩和提示的编辑框架，实现了精确高效的图像操作，同时保持了高图像保真度和准确的点对齐，为高质量交互式图像处理提供了实用解决方案。
...
TL;DR: 研究发现现有前馈3D重建模型VGGT具有内在的噪声图像识别能力，无需额外训练即可过滤干扰图像


<details>
  <summary>Details</summary>
Motivation: 在野外图像集合中进行3D重建时，常受到"噪声"图像（与其他图像重叠度低或无关的输入）的干扰。传统SfM管道通过几何验证和异常值拒绝处理此类问题，但前馈3D重建模型缺乏这些显式机制，导致在野外条件下性能下降。

Method: 通过在不同比例合成干扰图像下的深入分析，发现VGGT模型的特定层自然表现出异常值抑制行为。进一步探索发现该层编码了具有区分性的内部表示，使其具备有效的噪声过滤能力，可直接用于前馈3D重建中的异常视图拒绝，无需额外微调或监督。

Result: 在受控和野外数据集上的大量实验表明，这种隐式过滤机制具有一致性，并能很好地泛化到不同场景中。

Conclusion: 现有前馈3D重建模型具有内在的噪声图像识别能力，无需显式异常值拒绝机制或噪声感知训练，即可有效过滤干扰图像，提高野外条件下的3D重建可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在潜在空间中应用群理论先验或建模变换时，通常需要手动将潜在变量划分为等变和不变子空间，这限制了在表示空间中稳健学习和操作群作用的能力。

Method: 使用可学习的二元掩码配合直通估计，动态地将潜在表示划分为变换敏感和不变分量；在统一优化框架中联合学习潜在解缠和群变换映射，可与任何标准编码器-解码器架构无缝集成。
...
TL;DR: NVRC-Lite：一种轻量级神经视频压缩框架，通过多尺度特征网格和八叉树上下文模型改进NVRC，在保持低计算复杂度的同时提升性能并加速编解码速度。


<details>
  <summary>Details</summary>
Motivation: 现有基于隐式神经表示（INR）的视频压缩方法中，NVRC虽然性能优异但计算复杂度较高，而轻量级INR方法如C3虽然复杂度低但性能有限。同时，现有INR通常使用自回归模型进行熵编码，虽然有效但编码速度过慢，不实用。

Method: 1. 将多尺度特征网格集成到轻量级神经表示中，使用更高分辨率网格提升低复杂度INR的性能；2. 提出基于八叉树的上下文模型用于高维特征网格的熵编码，替代传统的自回归模型以加速熵编码模块。

Result: NVRC-Lite在PSNR和MS-SSIM指标上分别比当前最佳轻量级INR视频编解码器C3节省高达21.03%和23.06%的BD-rate，同时实现8.4倍编码加速和2.5倍解码加速。

Conclusion: NVRC-Lite成功将NVRC框架扩展到轻量级表示，通过多尺度特征网格和八叉树上下文模型，在保持低计算复杂度的同时显著提升了性能并大幅加速了编解码过程，为实用化的轻量级神经视频压缩提供了有效解决方案。
...
Method: 提出C3G框架，使用可学习token通过自注意力机制聚合多视图特征来指导高斯生成，确保每个高斯都能整合跨视图的相关视觉特征，并利用学习到的注意力模式进行高效特征提升。

Result: 在无姿态新视图合成、3D开放词汇分割和视图不变特征聚合等任务上的实验表明，该方法在内存效率和特征保真度方面优于现有方法。

Conclusion: 紧凑但几何意义明确的表示足以实现高质量的场景重建和理解，证明了该方法在效率和性能上的优越性。
...
TL;DR: RELIC是一个统一的交互式世界建模框架，通过压缩的历史潜在令牌实现实时长时记忆、空间一致性和精确用户控制，在16 FPS下实现实时生成。


<details>
  <summary>Details</summary>
Motivation: 构建真正的交互式世界模型需要三个关键要素：实时长时流式生成、一致的空间记忆和精确的用户控制。现有方法通常只解决其中一个方面，同时实现三者极具挑战性，例如长时记忆机制往往会降低实时性能。

Method: 基于自回归视频扩散蒸馏技术，使用高度压缩的历史潜在令牌（包含相对动作和绝对相机姿态）在KV缓存中表示长时记忆。采用新的内存高效自强制范式，将双向教师视频模型转换为因果学生生成器，支持全上下文蒸馏和长时自滚动。

Result: 作为140亿参数模型，在Unreal Engine渲染数据集上训练，RELIC以16 FPS实现实时生成，相比先前工作展现出更准确的动作跟随、更稳定的长时流式生成和更鲁棒的空间记忆检索能力。

Conclusion: RELIC为下一代交互式世界建模奠定了坚实基础，统一解决了实时长时流式生成、空间记忆一致性和用户控制这三个关键挑战。
...
Conclusion: PosterCopilot通过创新的训练策略和工作流程，解决了现有自动化图形设计方法的局限性，实现了几何精确、美学优良且支持专业迭代编辑的设计框架。
...
Method: 提出SimFlow方法，将VAE编码器输出的方差固定为常数（如0.5），这样既能让编码器输出更广泛的token分布，又能让解码器学习从增强的token分布中重建干净图像，同时简化了VAE的证据下界，使NF与VAE能够稳定地联合训练。

Result: 在ImageNet 256×256生成任务上，SimFlow获得了2.15的gFID分数，优于当前最佳方法STARFlow（2.40）。与REPA-E方法结合后，gFID进一步提升至1.91，在归一化流方法中创造了新的SOTA。

Conclusion: 通过简单地将VAE方差固定为常数，SimFlow有效解决了现有归一化流方法的两个关键限制，实现了更简洁的流程和更好的生成性能，为NF与VAE的联合训练提供了新的思路。
...
### [87] [Exploring Syntropic Frameworks in AI Alignment: A Philosophical Investigation](https://arxiv.org/abs/2512.03048)
*Austin Spizzirri*

Main category: cs.AI

TL;DR: 本文主张将AI对齐重新构想为通过基于过程、多智能体、发展性机制来构建具有熵减和理性响应能力的智能体，而非编码固定的人类价值观内容。


<details>
  <summary>Details</summary>
Motivation: 传统基于内容的价值规范方法存在结构不稳定性问题，这是由于"是-应当"鸿沟、价值多元主义和扩展框架问题共同导致的"规范陷阱"。需要寻找更稳健的AI对齐框架。

Method: 提出三个哲学贡献：1) 阐述"规范陷阱"论证；2) 提出"熵减"作为理解多智能体对齐动态的信息理论框架；3) 基于兼容主义指导控制理论建立真实与模拟道德能力的功能区分，并设计具身实验范式和验证机制。

Result: 该框架产生了关于人工系统中价值涌现和道德能动性的具体、可证伪的预测，但实证验证仍在进行中，是更广泛研究计划的哲学组成部分。

Conclusion: AI对齐应该从编码固定价值内容转向构建具有熵减特性和理性响应能力的智能体，通过过程性、多智能体、发展性机制实现更稳健的对齐。
...
### [88] [Beyond the Black Box: A Cognitive Architecture for Explainable and Aligned AI](https://arxiv.org/abs/2512.03072)
*Hu Keyi*

Main category: cs.AI

TL;DR: 提出了一种名为"权重计算主义"的新型认知架构，基于逻辑原子和基本操作构建，通过可解释的权重计算模型实现决策，为AGI提供了透明、可追溯的理论基础。
...
Result: 该架构实现了透明、类人的推理能力，并在前所未有的场景中展现出稳健的学习能力，为构建可信赖和对齐的AGI奠定了实践和理论基础。

Conclusion: 权重计算主义作为一种基于第一原理的认知架构，通过原子分解实现了根本的可解释性、对新情境的内在通用性以及可追溯的价值对齐，为AGI发展提供了可行的路径。
...
### [89] [When Do Symbolic Solvers Enhance Reasoning in Large Language Models?](https://arxiv.org/abs/2512.03272)
*Zhiyuan He,Dingmin Wang*

Main category: cs.AI

TL;DR: 该论文探讨了符号求解器集成方法何时能增强传统长链思维推理，发现符号求解器仅在问题需要有限隐式推理但涉及充足搜索空间时有效，而最新LLMs在浅推理深度的演绎问题上表现更好。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成长链思维在复杂推理任务上表现良好，但这种方法可能导致大量token开销，特别是当模型"过度思考"产生冗长推理链时，甚至会导致错误答案。符号求解器集成方法利用LLM的代码生成能力将推理任务转换为可执行代码，然后用符号求解器解决，但何时这种传统长链思维能被符号求解器增强仍是一个开放问题。

Method: 研究探索符号求解器集成方法何时能增强传统长链思维推理，通过实验分析不同问题类型（需要有限隐式推理但涉及充足搜索空间的问题 vs 浅推理深度的演绎问题）的性能表现，比较最新LLMs（如GPT-4o）与符号求解器集成方法在不同类型推理任务上的表现。

Result: 实验结果显示：1）符号求解器集成方法仅在问题需要有限隐式推理但涉及充足搜索空间时有效；2）最新LLMs（如GPT-4o）在浅推理深度的演绎问题上表现更好；3）符号求解器集成方法显著提高LLMs在需要重复回溯的约束满足问题上的性能；4）当提供声明性示例时，即使是CodeLlama-13B也能在困难的斑马谜题上超越GPT-4o。

Conclusion: 符号求解器集成方法对传统长链思维推理的增强效果取决于问题类型：对于需要有限隐式推理但涉及充足搜索空间的问题（特别是约束满足问题），符号求解器集成方法能显著提升性能；而对于浅推理深度的演绎问题，最新LLMs本身已表现良好。声明性示例的提供可以进一步提升符号求解器集成方法的性能。
...
TL;DR: 该论文提出了一种评估LLM智能体在零样本混合动机环境中合作能力的方法，使用Concordia自然语言多智能体模拟环境，通过NeurIPS 2024竞赛发现当前智能体在需要说服和规范执行等场景中的合作泛化能力存在显著差距。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在社交互动方面展现出强大能力，并越来越多地部署在与人类和人工智能体互动的场景中。然而，现有评估方法无法衡量这些能力在新颖社交情境中的泛化能力，这成为LLM智能体发展的关键前沿问题。

Method: 提出了一种使用Concordia自然语言多智能体模拟环境来评估LLM智能体在零样本混合动机环境中合作能力的方法。该方法通过测试智能体在不同合作伙伴和情境中识别和利用互利机会的能力来衡量一般合作智能。

Result: 通过NeurIPS 2024 Concordia竞赛的实证结果显示，当前智能体在谈判到集体行动问题等多样化场景中实现互利的能力存在显著差距。特别是在需要说服和规范执行的场景中，智能体的合作泛化能力远未达到稳健可靠的水平。

Conclusion: LLM智能体在合作能力方面仍需显著改进，特别是在需要复杂社交推理和适应新情境的场景中。当前智能体在需要说服和规范执行等复杂社交互动中的泛化能力存在明显不足，这为未来研究指明了方向。
...
TL;DR: 提出了一种通信受限的多智能体强化学习框架，通过区分有损和无损消息，将通信影响量化为全局奖励，解决了现实场景中通信损失问题。
...
### [92] [EnCompass: Enhancing Agent Programming with Search Over Program Execution Paths](https://arxiv.org/abs/2512.03571)
*Zhening Li,Armando Solar-Lezama,Yisong Yue,Stephan Zheng*

Main category: cs.AI

TL;DR: 提出了一种名为"概率天使非确定性"（PAN）的LLM智能体编程新方法，通过分离核心工作流逻辑和推理时策略，简化智能体开发。
...
Method: 引入PAN编程模型，使用Python装饰器将智能体工作流程序编译为搜索空间，实现工作流逻辑与推理策略的解耦。

Result: 开发了EnCompass框架，通过三个案例研究证明该框架能快速提升智能体可靠性，并轻松切换不同推理策略，只需少量额外代码。

Conclusion: PAN编程模型和EnCompass框架为LLM智能体开发提供了更灵活、可实验的方法，显著简化了智能体编程过程。
...
### [93] [DeepRule: An Integrated Framework for Automated Business Rule Generation via Deep Predictive Modeling and Hybrid Search Optimization](https://arxiv.org/abs/2512.03607)
*Yusen Wu,Xiaotie Deng*

Main category: cs.AI

TL;DR: DeepRule是一个集成框架，用于零售品类和定价优化的自动化业务规则生成，通过三层架构解决理论模型与现实经济复杂性之间的系统性错配问题。
...
### [94] [RoCo: Role-Based LLMs Collaboration for Automatic Heuristic Design](https://arxiv.org/abs/2512.03762)
...
Method: 提出RoCo多智能体角色协作系统，包含四个专门角色：探索者（创造性思维）、利用者（效率优化）、批评者（评估反馈）、整合者（平衡创新与利用）。这些智能体通过结构化的多轮交互过程进行协作，包括反馈、精炼和精英突变。

Result: 在五个不同的组合优化问题上，无论是白盒还是黑盒设置下，RoCo都表现出优越性能，生成的启发式方法超越了ReEvo和HSEvo等现有方法。

Conclusion: 基于角色的协作范式为鲁棒且高性能的自动启发式设计建立了新标准，证明了多角色协作在增强启发式多样性和质量方面的有效性。
...
### [95] [Omni-AutoThink: Adaptive Multimodal Reasoning via Reinforcement Learning](https://arxiv.org/abs/2512.03783)
*Dongchao Yang,Songxiang Liu,Disong Wang,Yuanyuan Wang,Guanglu Wan,Helen Meng*

Main category: cs.AI

TL;DR: Omni-AutoThink：一种自适应推理框架，可根据任务难度动态调整模型推理深度，提升多模态模型的灵活推理能力
...
Conclusion: Omni-AutoThink框架成功解决了现有Omni模型推理行为僵化的问题，通过自适应调整推理深度实现了更灵活高效的多模态推理。
...
### [96] [Benchmark for Planning and Control with Large Language Model Agents: Blocksworld with Model Context Protocol](https://arxiv.org/abs/2512.03955)
*Niklas Jobs,Luis Miguel Vieira da Silva,Jayanth Somashekaraiah,Maximilian Weigand,David Kube,Felix Gehlhoff*

Main category: cs.AI

TL;DR: 该论文提出了一个基于Blocksworld问题的基准测试框架，用于系统评估LLM智能体在工业自动化任务中的规划和执行能力，通过MCP标准化接口实现不同智能体架构的无缝对接和比较。
...
Method: 引入一个包含可执行模拟环境的基准测试框架，基于Blocksworld问题提供五个复杂度类别，通过集成模型上下文协议(MCP)作为标准化工具接口，使不同智能体架构能够无需特定实现修改即可连接和评估。

Result: 开发了一个单智能体实现来展示基准测试的适用性，建立了用于比较基于LLM的规划和执行方法的定量指标。

Conclusion: 该基准测试框架为评估LLM智能体在工业自动化任务中的自适应规划和执行能力提供了标准化平台，通过MCP接口实现了不同智能体架构的系统比较，为未来研究提供了定量评估基础。
...
### [97] [Entropy-Based Measurement of Value Drift and Alignment Work in Large Language Models](https://arxiv.org/abs/2512.03047)
*Samih Fadli*

Main category: cs.CL

TL;DR: 该研究将伦理熵作为衡量LLM安全性的状态变量，开发了行为分类器和监测管道来实时检测价值漂移


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全评估主要依赖静态基准测试，但关键失效模式（如分布偏移下的价值漂移、越狱攻击、部署中缓慢的对齐退化）是动态的，需要实时监测框架

Method: 基于智能第二定律，将伦理熵作为状态变量；定义五维行为分类法，训练分类器从模型转录中估计伦理熵S(t)；测量四种前沿模型在压力测试下的熵动态；开发包含S(t)和有效对齐工作率gamma_eff的监测管道

Result: 基础模型显示持续熵增长，而经过指令调优的变体抑制了漂移，将伦理熵降低了约80%；从轨迹中估计出有效对齐工作率gamma_eff；监测管道能在熵漂移超过稳定性阈值时发出警报

Conclusion: 伦理熵框架为LLM安全提供了可操作的动态监测方法，能够实时检测价值漂移，为运行时监督提供了有效工具
...
### [98] [Watermarks for Embeddings-as-a-Service Large Language Models](https://arxiv.org/abs/2512.03079)
*Anudeex Shetty*

Main category: cs.CL

TL;DR: 该论文研究了嵌入即服务(EaaS)中的水印技术，发现现有水印技术易受改写攻击，并提出了一种新的线性变换水印方法WET来防御模仿攻击。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的发展，企业开始提供嵌入即服务(EaaS)，但EaaS容易受到模仿攻击。虽然已有水印技术用于保护知识产权，但现有水印存在新的安全漏洞。

Method: 首先揭示现有EaaS水印可通过输入文本改写被移除的漏洞，然后提出WET水印技术，使用线性变换嵌入，通过反向变换和相似度比较进行水印验证。

Result: 研究发现改写攻击能有效绕过当前最先进的EaaS水印，而提出的WET水印技术对改写攻击具有鲁棒性，验证准确率接近完美。

Conclusion: 现有EaaS水印存在改写攻击漏洞，提出的WET水印技术能有效防御此类攻击，为EaaS知识产权保护提供了更安全的解决方案。
...
### [99] [Alleviating Choice Supportive Bias in LLM with Reasoning Dependency Generation](https://arxiv.org/abs/2512.03082)
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在评估时表现出选择支持性偏见，会系统性偏向自己选择的选项，这可能影响AI辅助决策的客观性。虽然已有方法主要针对人口统计和社会偏见，但解决LLM中认知偏见的方法仍未被充分探索。

Method: 提出推理依赖生成框架，通过自动构建平衡的推理问答对来生成无偏见推理数据，明确建模选择、证据和理由之间的依赖关系。该方法能生成跨领域的大规模数据集，包含上下文依赖数据和依赖解耦数据。
...
### [100] [InvertiTune: High-Quality Data Synthesis for Cost-Effective Single-Shot Text-to-Knowledge Graph Generation](https://arxiv.org/abs/2512.03197)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的文本到知识图谱构建方法通常依赖迭代提示，导致计算成本高昂且容易遗漏文本中分布的复杂关系，需要更高效、准确的解决方案。

Method: 提出InvertiTune框架：1）受控数据生成管道：从大型知识库中提取子图，进行噪声过滤，利用LLM生成对应的自然文本描述；2）监督微调：使用生成的配对数据集（长文本+大KG）对轻量级模型进行微调，实现单次推理的KG构建。

Result: 在CE12k数据集上，InvertiTune超越了更大的非微调LLM以及最先进的Text2KG方法，同时在CrossEval-1200测试集上表现出更强的跨数据集泛化能力。

Conclusion: 研究表明，现实、高质量的训练数据对于推进高效、高性能的文本到知识图谱系统至关重要，InvertiTune框架为此提供了有效的解决方案。
...
### [101] [Identifying attributions of causality in political text](https://arxiv.org/abs/2512.03214)
*Paulina Garcia-Corral*

Main category: cs.CL

TL;DR: 作者提出了一个检测和解析政治文本中解释的框架，使用轻量级因果语言模型提取因果对，用于大规模分析政治解释。


<details>
  <summary>Details</summary>
Motivation: 解释是理解政治世界的基础，但政治科学中缺乏对解释的系统分析，现有方法分散且常限于特定议题。

Method: 训练轻量级因果语言模型，从政治文本中提取结构化因果对数据集，用于下游分析。

Result: 方法展示了大规模研究因果解释的可行性，具有适度的标注需求、良好的泛化能力和相对于人工编码的准确性。

Conclusion: 该框架为政治科学中系统分析解释提供了可行工具，能够处理大规模文本并提取结构化因果信息。
...
### [102] [Randomized Masked Finetuning: An Efficient Way to Mitigate Memorization of PIIs in LLMs](https://arxiv.org/abs/2512.03310)
*Kunj Joshi,David A. Smith*

Main category: cs.CL

TL;DR: RMFT是一种隐私保护微调技术，通过随机掩码减少LLMs对训练数据中个人身份信息的记忆，同时最小化性能影响


<details>
  <summary>Details</summary>
Motivation: 当前自然语言模型（特别是大语言模型）存在严重的安全和隐私风险，模型倾向于记忆训练数据中的个人身份信息，这可能导致隐私泄露

Method: 提出随机掩码微调（RMFT）技术，在微调过程中随机掩码部分输入以减少PII记忆；同时提出MaxTER评估框架，用于评估隐私-效用权衡

Result: 在Enron邮件数据集上，RMFT相比基线微调实现了80.81%的总提取率降低和80.17%的已见提取率降低，性能损失仅为5.73%的困惑度增加，优于去重方法

Conclusion: RMFT是一种有效的隐私保护微调方法，能够在显著减少PII记忆的同时保持模型性能，为LLMs的安全部署提供了实用解决方案
...
### [103] [Modeling Topics and Sociolinguistic Variation in Code-Switched Discourse: Insights from Spanish-English and Spanish-Guaraní](https://arxiv.org/abs/2512.03334)
*Nemika Tyagi,Nelvin Licona Guevara,Olga Kellert*

Main category: cs.CL

TL;DR: LLM辅助标注管道用于西班牙语-英语和西班牙语-瓜拉尼语双语话语的社会语言学和主题分析，自动标注主题、体裁和语用功能，揭示性别、语言优势和语用功能之间的系统性关联。


<details>
  <summary>Details</summary>
Motivation: 传统社会语言学分析依赖人工标注，成本高且难以扩展到大规模语料。本研究旨在利用大语言模型自动恢复传统上只能通过人工标注获得的可解释社会语言学模式，推进跨语言和低资源双语研究的计算方法。

Method: 使用大语言模型构建自动标注管道，对3,691个语码转换句子进行主题、体裁和语用功能标注，整合迈阿密双语语料库的人口统计元数据，并为西班牙语-瓜拉尼语数据集添加新的主题标注。

Result: 迈阿密数据揭示了性别、语言优势和语用功能之间的系统性关联；巴拉圭文本显示了正式瓜拉尼语和非正式西班牙语之间的明显双语分工。这些发现用语料库规模的定量证据复制并扩展了早期的互动和社会语言学观察。
...
### [104] [PERCS: Persona-Guided Controllable Biomedical Summarization Dataset](https://arxiv.org/abs/2512.03340)
*Rohan Charudatt Salvi,Chirag Chawla,Dhruv Jain,Swapnil Panigrahi,Md Shad Akhtar,Shweta Yadav*

Main category: cs.CL

TL;DR: PERCS数据集：针对四种不同医疗知识水平用户（普通大众、医预科学生、非医学研究人员、医学专家）的个性化生物医学摘要简化数据集，支持可控摘要生成研究。


<details>
  <summary>Details</summary>
Motivation: 现有医学文本简化资源通常假设单一通用受众，忽视了不同用户群体在医疗素养和信息需求上的显著差异，需要针对特定受众的个性化摘要方法。

Method: 创建PERCS数据集，包含生物医学摘要及其针对四种不同医疗知识水平用户的个性化摘要；由医生使用详细错误分类法进行事实准确性和用户匹配度审核；对四种大语言模型进行基准测试。

Result: 技术验证显示不同用户群体的摘要在可读性、词汇选择和内容深度上存在明显差异；建立了自动评估指标（全面性、可读性、忠实度）的基线结果；数据集和评估材料已公开。

Conclusion: PERCS数据集填补了针对不同医疗知识水平用户的个性化生物医学摘要研究的空白，为可控摘要生成和个性化医学传播研究提供了重要资源。
...
### [105] [Idea-Gated Transformers: Enforcing Semantic Coherence via Differentiable Vocabulary Pruning](https://arxiv.org/abs/2512.03343)
*Darshan Fofadiya*

Main category: cs.CL

TL;DR: 本文提出了一种新颖的Idea-Gated Transformer架构，通过分离语义规划和语法生成来解决自回归语言模型中的"主题漂移"问题，使用"概念向量"实时门控词汇选择，提高生成内容的主题一致性。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型在下一个词预测训练中经常出现"主题漂移"问题，生成内容会偏离初始提示，这是因为模型过度依赖局部关联而非全局规划。虽然增大模型规模可以缓解这一问题，但NTP目标固有的短视性仍然存在。

Method: 提出了Idea-Gated Transformer架构，将语义规划与语法生成分离。引入辅助的"Idea Head"来预测未来上下文窗口的词袋分布，生成潜在的"概念向量"，该向量在生成过程中主动门控主词汇选择。提出了可微分门控机制，抑制语义不相关的标记，实时修剪搜索空间。

Result: 在WikiText-103上的实验表明，Idea-Gated模型在验证困惑度上与标准GPT-2基线相当，但在领域保持性方面表现出显著优势。定性和定量分析显示，门控机制成功将生成锁定在特定语义簇（如金融、科学）中，有效抵抗关联漂移。
...
### [106] [From Hypothesis to Premises: LLM-based Backward Logical Reasoning with Selective Symbolic Translation](https://arxiv.org/abs/2512.03360)
*Qingchuan Li,Mingyue Cheng,Zirui Liu,Daoyu Wang,Yuting Zeng,Tongxuan Liu*

Main category: cs.CL

TL;DR: 提出HBLR框架，通过置信度感知的符号翻译和假设驱动的向后推理，结合翻译和推理反思模块，在五个推理基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型主要依赖前向推理范式，存在推理路径冗余、幻觉步骤和语义漂移问题，导致推理效率低且不可靠。需要更接近人类逻辑推理的方法。

Method: HBLR框架包含两个核心阶段：1）置信度感知符号翻译，仅将高置信度文本转换为逻辑形式，保留不确定内容为自然语言，通过翻译反思确保语义保真度；2）假设驱动的向后推理，模拟人类演绎思维，假设结论为真并递归验证前提，通过推理反思识别和修正错误推理步骤。

Result: 在五个推理基准上的广泛实验表明，HBLR在准确性和效率方面一致优于强基线方法。

Conclusion: HBLR通过结合符号翻译和向后推理，提供了一种更高效可靠的逻辑推理框架，模拟了人类的演绎思维过程，解决了前向推理方法的局限性。
...
### [107] [Nexus: Higher-Order Attention Mechanisms in Transformers](https://arxiv.org/abs/2512.03377)
*Hanting Chen,Chong Zhu,Kai Han,Yuchuan Tian,Yuchen Liang,Tianyu Guo,Xinghao Chen,Dacheng Tao,Yunhe Wang*

Main category: cs.CL

TL;DR: Nexus是一种新型Transformer架构，通过递归框架增强表示能力，打破标准注意力机制的低秩瓶颈，能够捕捉复杂多跳关系。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer的一阶注意力机制存在低秩瓶颈问题，难以在单层内捕捉复杂的多跳关系，限制了模型的表达能力。

Method: 提出Nexus架构，采用递归框架动态精炼查询和键向量。查询和键向量本身是内部注意力循环的输出，允许token在最终注意力计算前聚合全局上下文并建模高阶相关性。采用参数高效的权重共享策略，确保增强表达能力的同时只增加O(1)参数。

Result: 理论分析表明该方法打破了标准注意力的线性瓶颈。实证结果显示Nexus在多个基准测试中优于标准Transformer。

Conclusion: Nexus通过递归注意力机制有效增强了Transformer的表示能力，能够捕捉复杂关系，同时保持参数效率，为改进注意力机制提供了新思路。
...
### [108] [Characterizing Language Use in a Collaborative Situated Game](https://arxiv.org/abs/2512.03381)
...
TL;DR: 研究者收集了Portal 2合作模式中11.5小时的口语对话语料库，包含24.5K条话语，分析了复杂空间指代等独特语言现象，并公开了包含视频、音频、游戏状态数据的完整语料库。


<details>
  <summary>Details</summary>
Motivation: 合作视频游戏中的语言数据具有丰富的研究价值，但现有闲聊或任务导向对话语料库很少包含复杂环境下的协调、不确定推理和空间指代等语言现象。

Method: 收集Portal 2合作模式中11.5小时的玩家口语对话，形成包含24.5K条话语的Portal Dialogue Corpus，包含玩家视频、音频、转录文本、游戏状态数据，并进行手动和自动标注。
...
### [109] [Dual LoRA: Enhancing LoRA with Magnitude and Direction Updates](https://arxiv.org/abs/2512.03402)
*Yixing Xu,Chao Li,Xuanwu Yin,Spandan Tiwari,Dong Li,Ashish Sirasao,Emad Barsoum*

Main category: cs.CL

TL;DR: Dual LoRA通过将低秩矩阵分为幅度组和方向组来改进LoRA性能，添加ReLU和符号函数来模拟全参数微调的梯度更新过程，在多个NLP任务上优于原始LoRA及其变体。


<details>
  <summary>Details</summary>
Motivation: LoRA作为参数高效微调方法在适应预训练大语言模型到下游任务时很流行，但由于其低秩假设，训练出的模型性能往往不令人满意。需要改进LoRA以提升性能。

Method: 提出Dual LoRA方法，将低秩矩阵分为两组：幅度组控制是否更新参数及更新幅度，方向组决定参数更新方向（前进或后退）。通过添加ReLU函数到幅度组和符号函数到方向组，更好地模拟基于梯度优化算法的全参数微调过程。

Result: 在广泛的NLP任务上进行了实验，包括自然语言生成、理解和常识推理数据集，使用GPT-2、RoBERTa、DeBERTa和LLaMA-1/2/3作为基线模型。结果表明，在相同可训练参数数量下，Dual LoRA始终优于原始LoRA及其最先进的变体。

Conclusion: Dual LoRA通过引入归纳偏置，将低秩矩阵分解为幅度和方向组件，有效改进了LoRA的性能，在多个模型和任务上展现出优于现有方法的性能表现。
...
### [110] [PretrainZero: Reinforcement Active Pretraining](https://arxiv.org/abs/2512.03442)
*Xingrun Xing,Zhiyuan Fan,Jie Lou,Guoqi Li,Jiajun Zhang,Debing Zhang*

Main category: cs.CL

TL;DR: PretrainZero是一个基于预训练语料的强化主动学习框架，将强化学习从领域特定的后训练扩展到通用预训练，通过主动识别信息内容、自我监督学习和验证扩展来提升基础模型的通用推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的大思考模型虽然在某些专业领域表现出专家级能力，但仍严重依赖特定领域可验证的奖励信号，这限制了通用推理能力的边界扩展。需要突破验证数据壁垒，实现更通用的推理能力。

Method: 1) 主动预训练：学习统一推理策略，主动从预训练语料中识别合理且信息丰富的内容，并通过强化学习预测这些内容。2) 自我监督学习：无需可验证标签、预训练奖励模型或监督微调，直接在通用维基百科语料上使用强化学习预训练推理器。3) 验证扩展：通过处理越来越具有挑战性的掩码跨度，增强预训练基础模型的通用推理能力。

Result: 在强化预训练中，PretrainZero将Qwen3-4B-Base在MMLU-Pro、SuperGPQA和数学平均基准测试上分别提升了8.43、5.96和10.60分。预训练模型还可以作为下游RLVR任务的推理基础模型。

Conclusion: PretrainZero成功将强化学习从领域特定的后训练扩展到通用预训练，通过主动学习和自我监督机制突破了验证数据壁垒，显著提升了基础模型的通用推理能力，为实现更通用的人工智能推理能力提供了新途径。
...
### [111] [A Preliminary Study on the Promises and Challenges of Native Top-$k$ Sparse Attention](https://arxiv.org/abs/2512.03494)
*Di Xiu,Hongyin Tang,Bolin Rong,Lizhi Yan,Jingang Wang,Yifan Lu,Xunliang Cai*

Main category: cs.CL

TL;DR: Top-k注意力机制在解码和训练阶段的有效性研究，验证了在解码阶段仅保留与查询相似度最高的关键键值能达到与完整注意力相当或更好的性能，并通过训练-推理一致性进一步提升模型表现。
...
Method: 研究Top-k注意力机制在解码和训练阶段的有效性，包括精确Top-k解码实验、原生Top-k注意力训练策略、近似Top-k算法精度影响分析，以及从熵的角度进行理论解释。

Result: 实验表明：1）解码阶段仅保留与查询最相似的关键键值能在HELMET和LongBench v2等下游任务上达到与完整注意力相当或更好的性能；2）训练-推理一致性有助于进一步释放Top-k解码潜力；3）下游任务性能与近似算法精度呈正相关；4）Top-k注意力SFT模型在下游任务中表现出明显的熵减少现象。

Conclusion: Top-k注意力机制能有效降低大语言模型的计算成本，低熵状态更适合Top-k解码，训练-推理一致性策略能进一步提升模型性能，为高效长上下文建模提供了有前景的解决方案。
...
### [112] [Understanding LLM Reasoning for Abstractive Summarization](https://arxiv.org/abs/2512.03503)
*Haohan Yuan,Siu Cheung Hui,Haopeng Zhang*

Main category: cs.CL

TL;DR: 研究发现LLMs的推理能力在摘要生成中存在质量与忠实度的权衡，显式推理提升流畅性但降低事实准确性，隐式推理则相反，增加推理预算反而可能损害事实一致性。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型在数学和代码生成等分析任务中表现出色，但其在抽象摘要生成方面的效用被广泛假设但未经验证。本研究旨在填补这一空白，验证推理策略在摘要任务中的实际效果。

Method: 首先将通用推理策略定制到摘要领域，然后对8种推理策略和3个大型推理模型在8个不同数据集上进行大规模比较研究，评估摘要质量和忠实度。
...
Conclusion: 有效的摘要生成需要忠实的压缩而非创造性的过度思考，推理在摘要任务中的效果具有情境依赖性，需要根据具体需求选择合适的策略。
...
### [113] [AlignCheck: a Semantic Open-Domain Metric for Factual Consistency Assessment](https://arxiv.org/abs/2512.03634)
*Ahmad Aghaebrahimian*

Main category: cs.CL

TL;DR: 提出了一种用于评估文本事实一致性的可解释框架，通过将文本分解为原子事实并引入灵活的、无模式的方法论，改进了现有评估指标的不足。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理任务中取得显著进展，但仍容易生成错误或误导性但看似合理的论点（幻觉问题）。这在临床等高风险领域尤其令人担忧，因为事实不准确可能带来严重后果。现有评估指标未能充分评估事实一致性且缺乏可解释性，使得诊断和缓解错误变得困难。

Method: 提出一个可解释的事实一致性评估框架，将文本分解为原子事实，采用灵活的无模式方法。与之前使用绝对度量的方法不同，引入了加权度量来增强事实评估。此外，还提出了一种机制来控制复杂领域中的评估复杂性。

Result: 在流行的通用和临床数据集上对方法进行了基准测试，并发布了代码以支持未来研究中事实感知模型的训练。

Conclusion: 该框架解决了现有事实一致性评估指标的局限性，为高风险领域（如临床应用）提供了更可靠、可解释的评估工具，有助于诊断和缓解语言模型的幻觉问题。
...
### [114] [Generative AI Practices, Literacy, and Divides: An Empirical Analysis in the Italian Context](https://arxiv.org/abs/2512.03671)
...
TL;DR: 意大利首次全面实证研究显示，生成式AI在意大利广泛采用，正取代其他技术成为主要信息来源，但存在数字素养不足、性别鸿沟等问题。


<details>
  <summary>Details</summary>
Motivation: 研究旨在了解生成式AI在意大利的采用情况、使用模式和数字素养水平，以评估AI技术可能加剧数字鸿沟的风险，特别是考虑到用户对其局限性的认识不足。

Method: 基于新收集的1,906名意大利语成年人的调查数据，进行首次全面的实证映射，分析生成式AI的采用率、使用模式和数字素养。

Result: 研究发现：1) 生成式AI在工作和个人用途中广泛采用，包括情感支持和医疗建议等敏感任务；2) 正取代其他技术成为主要信息来源，但用户数字素养低，难以识别错误信息；3) 存在显著的性别鸿沟，女性采用率仅为男性一半，使用频率也更低；4) 数字素养是采用的关键预测因素，但只能部分解释性别差异。

Conclusion: 需要有针对性的教育计划，并进一步调查仅靠能力无法解释的公平参与障碍，以应对生成式AI带来的数字鸿沟风险。
...
### [115] [Evaluating Hydro-Science and Engineering Knowledge of Large Language Models](https://arxiv.org/abs/2512.03672)
...
TL;DR: 该研究提出了Hydro-SE Bench基准测试，包含4000道选择题，用于评估大语言模型在水科学与工程领域的知识与应用能力。评估结果显示商业LLM准确率在0.74-0.80之间，小参数模型在0.41-0.68之间，模型在自然物理科学相关领域表现良好，但在行业标准等专业领域存在不足。
...
Method: 提出了Hydro-SE Bench评估基准，包含4000道选择题，涵盖9个子领域，能够评估LLM在基本概念知识、工程应用能力、推理计算能力三个方面的表现。对商业LLM和小参数模型进行了系统性评估。

Result: 商业LLM准确率在0.74-0.80之间，小参数模型在0.41-0.68之间。模型在自然物理科学相关子领域表现良好，但在行业标准、水工结构等专业领域存在困难。模型扩展主要提升了推理计算能力，但在实际工程应用方面仍有很大提升空间。

Conclusion: 该研究揭示了LLM在Hydro-SE任务中的优势和不足，为模型开发者提供了明确的训练目标，为Hydro-SE研究者提供了应用LLM的实践指导，强调了模型在实际工程应用方面的改进潜力。
...
### [116] [Different types of syntactic agreement recruit the same units within large language models](https://arxiv.org/abs/2512.03676)
*Daria Kryvosheieva,Andrea de Varda,Evelina Fedorenko,Greta Tuckute*

Main category: cs.CL

TL;DR: 该研究通过认知神经科学启发的功能定位方法，在大语言模型中识别了对67种英语句法现象最敏感的单元，发现不同句法一致性现象（如主谓一致、照应一致、限定词-名词一致）会招募重叠的单元集，表明句法一致性在大语言模型中构成了有意义的句法功能类别。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型能够可靠地区分合乎语法和不合语法的句子，但语法知识在模型内部如何表征仍然是一个开放性问题。研究者希望探究不同的句法现象在大语言模型中是否共享或使用不同的组件。

Method: 采用认知神经科学启发的功能定位方法，在7个开源权重的大语言模型中识别对67种英语句法现象最敏感的单元。通过跨句子的一致性测试和因果干预实验验证这些单元对模型句法性能的因果支持作用。

Result: 研究发现：1）不同句法一致性现象（主谓一致、照应一致、限定词-名词一致）会招募重叠的单元集；2）这一模式在英语、俄语和中文中都成立；3）在57种语言的跨语言分析中，结构更相似的语言在主谓一致上共享更多单元。

Conclusion: 句法一致性——作为句法依赖关系的关键标记——在大语言模型的表征空间中构成了有意义的类别，揭示了模型内部句法知识的组织方式。
...
### [117] [AITutor-EvalKit: Exploring the Capabilities of AI Tutors](https://arxiv.org/abs/2512.03688)
*Numaan Naeem,Kaushal Kumar Maurya,Kseniia Petukhova,Ekaterina Kochmar*

Main category: cs.CL

TL;DR: 开发了一个名为AITutor-EvalKit的应用，利用语言技术评估AI导师的教学质量，提供演示、评估、模型检查和数据可视化功能。


<details>
  <summary>Details</summary>
Motivation: 为教育利益相关者和ACL社区提供一个工具，用于评估AI导师的教学质量，支持学习过程，并收集用户反馈和标注数据。

Method: 开发了一个集成语言技术的应用程序，包含AI导师质量评估、软件演示、模型检查和数据可视化等功能模块。

Result: 成功创建了AITutor-EvalKit工具，能够有效评估AI导师的教学质量，并为教育研究提供数据收集和可视化支持。

Conclusion: AITutor-EvalKit是一个实用的工具，不仅有助于评估AI导师的教学效果，还能促进教育技术研究和用户反馈收集。
...
### [118] [DZ-TDPO: Non-Destructive Temporal Alignment for Mutable State Tracking in Long-Context Dialogue](https://arxiv.org/abs/2512.03704)
*Yijun Liao*

Main category: cs.CL

TL;DR: DZ-TDPO是一个解决长对话系统中状态惯性问题的非破坏性对齐框架，通过动态KL约束和可学习时间注意力偏置，在保持模型通用能力的同时实现更好的对话状态更新。


<details>
  <summary>Details</summary>
Motivation: 长对话系统存在"状态惯性"问题，静态约束导致模型无法解决用户意图演变与历史上下文之间的冲突，需要一种既能有效更新对话状态又不破坏模型通用能力的解决方案。

Method: 提出DZ-TDPO框架，结合冲突感知的动态KL约束和可学习的时间注意力偏置，通过精确的注意力调节而非破坏性权重更新来缓解状态惯性问题。

Result: 在Multi-Session Chat数据集上，DZ-TDPO达到最先进的胜率（Phi-3.5上86.2%），Qwen2.5-7B模型实现近乎完美的对齐（99.4%胜率）且困惑度开销可忽略。发现了"容量-稳定性权衡"现象。

Conclusion: 通过精确的注意力调节而非破坏性权重更新可以有效缓解状态惯性问题，同时保持模型的通用能力。大模型在实现完美对齐时困惑度开销更小，展示了框架的可扩展性优势。
...
### [119] [In-Context Representation Hijacking](https://arxiv.org/abs/2512.03771)
...
Method: 通过系统地将有害关键词替换为良性标记，在多个上下文示例中训练模型，使良性标记的内部表示收敛到有害语义，实现语义覆盖

Result: 攻击在闭源和开源系统上均有效，在Llama-3.3-70B-Instruct上单句上下文覆盖达到74%攻击成功率，证明当前对齐策略不足

Conclusion: 揭示了LLMs潜在空间中的新攻击面，表明当前对齐策略应在表示层面进行改进，而不仅仅是表面文本层面
...
### [120] [Enhancing Instruction-Following Capabilities in Seq2Seq Models: DoLA Adaptations for T5](https://arxiv.org/abs/2512.03803)
*Huey Sun,Anabel Yong,Lorenzo Gilly,Felipe Jin*

Main category: cs.CL

TL;DR: 将DoLa对比解码方法从仅解码器架构扩展到T5/FLAN-T5编码器-解码器架构，评估其对指令遵循能力的影响，发现对某些任务有改进但对其他任务有损害


<details>
  <summary>Details</summary>
Motivation: 现有对比解码方法（如DoLa）仅在解码器架构中实现，主要研究其对事实性的改进，需要探索其在编码器-解码器架构中的应用及其对指令遵循能力的影响

Method: 将DoLa方法适配到T5和FLAN-T5模型家族，通过层对层分析logit演化来量化DoLa对token输出概率的影响

Result: DoLa改进了某些类别任务的文本生成忠实度，但对其他任务有损害；通过logit演化分析量化了DoLa对输出概率的具体影响

Conclusion: 成功将对比解码方法扩展到编码器-解码器架构，首次在T5/FLAN-T5中实现DoLa，揭示了其对不同任务指令遵循能力的差异化影响
...
### [121] [Improving Alignment Between Human and Machine Codes: An Empirical Assessment of Prompt Engineering for Construct Identification in Psychology](https://arxiv.org/abs/2512.03818)
*Kylie L. Anglin,Stephanie Milan,Brittney Hernandez,Claudia Ventura*

Main category: cs.CL

TL;DR: 本文提出一个优化LLM在心理学文本分类任务中提示工程的实证框架，发现构造定义、任务框架和示例对性能影响最大，推荐结合人工和自动方法生成多个提示变体并基于训练集性能选择最佳方案。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在文本分类方面表现良好，但其输出严重依赖提示的措辞。现有研究很少关注心理学等领域的分类任务，这些领域的构造具有精确的理论驱动定义，可能在预训练数据中代表性不足。需要系统的方法来优化LLM在这些专业领域的性能。

Method: 提出了一个优化LLM提示性能的实证框架，实验评估了五种提示策略：代码本引导的实证提示选择、自动提示工程、角色提示、思维链推理和解释性提示，结合零样本和少样本分类方法。

Result: 研究发现角色提示、思维链和解释并不能完全解决提示措辞不当带来的性能损失。影响提示性能的最关键特征是构造定义、任务框架，以及较小程度的示例。在三个构造和两个模型中，与专家判断最一致的结果来自结合代码本引导的实证提示选择和自动提示工程的少样本提示。

Conclusion: 建议研究人员尽可能生成和评估多个提示变体（无论是人工制作、自动生成还是两者结合），并基于训练数据集中的实证性能选择提示和示例，在保留集上验证最终方法。这为在专家判断对齐至关重要的场景中优化LLM提示提供了实用、系统和理论驱动的方法。
...
### [122] [Training and Evaluation of Guideline-Based Medical Reasoning in LLMs](https://arxiv.org/abs/2512.03838)
*Michael Staniek,Artem Sokolov,Stefan Riezler*

Main category: cs.CL

TL;DR: 该研究提出通过微调LLMs遵循医学共识指南进行逐步推理，以提升医学早期预测的可解释性和准确性，并以Sepsis-3为例验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前医学早期预测模型过于关注准确性而忽视了可解释性，这影响了医疗从业者的信任。医学共识指南普遍存在但未被充分利用来指导AI模型的推理过程。

Method: 将医学共识指南转化为可执行的推理规则，用这些规则的实例化数据微调LLMs，使模型能够遵循共识指南进行逐步推理。同时采用多模态方法整合时间序列预测模型来改善稀疏不规则临床变量的预测。

Result: 微调后的小模型在未见患者数据上对规则（及例外）的推导正确性接近完美，表现优于使用显式定义进行单次学习的大模型以及训练在包含共识定义的医学文本上的模型。多模态设置进一步改善了时间序列预测结果。

Conclusion: 通过微调LLMs遵循医学共识指南进行推理，可以有效提升医学早期预测的可解释性和准确性。早期预测的主要瓶颈不是分布外泛化，而是对稀疏不规则临床变量的时间序列预测问题，多模态方法可以改善这一问题。
...
### [123] [Reconstructing KV Caches with Cross-layer Fusion For Enhanced Transformers](https://arxiv.org/abs/2512.03870)
...
<details>
  <summary>Details</summary>
Motivation: Transformer解码器在长序列任务中面临KV缓存内存瓶颈问题，现有的跨层KV缓存共享方法（如YOCO、CLA）性能通常不如层内方法（如GQA），需要找到更有效的解决方案

Method: 通过分析顶层KV的信息流，发现值主要来自底层，键来自底层和中层。提出FusedKV方法，顶层KV缓存是底层和中层最有信息量的KV缓存的可学习融合，直接在RoPE后的键上操作以保留位置信息。还提出更高效的FusedKV-Lite版本，顶层KV缓存直接从底层值和中层键派生

Result: 在332M到4B参数的LLM实验中，该方法减少50%缓存内存，同时达到比标准Transformer解码器更低的验证困惑度，成为内存高效的高性能架构替代方案

Conclusion: FusedKV通过智能融合底层和中层KV缓存信息，有效解决了Transformer解码器的内存瓶颈问题，在保持性能的同时显著减少内存占用，为长序列处理提供了实用的解决方案
...
### [124] [BERnaT: Basque Encoders for Representing Natural Textual Diversity](https://arxiv.org/abs/2512.03903)
...
TL;DR: 该研究构建了包含巴斯克语标准、社交媒体和历史文本的多样化语料库，训练了BERnaT模型系列，发现结合多样化数据训练的语言模型在保持标准基准性能的同时，在所有任务类型上表现更优。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型依赖经过质量过滤的大规模文本语料库，这一过程可能无意中排除非标准语言变体，降低模型鲁棒性并强化代表性偏见。研究者认为语言模型应捕捉语言变异的完整频谱（方言、历史、非正式等），而不仅依赖标准化文本。

Method: 聚焦于形态丰富且资源稀缺的巴斯克语，构建了结合标准、社交媒体和历史来源的新语料库，预训练了BERnaT系列的编码器模型，包括标准配置、多样化配置和组合配置。提出了将自然语言理解任务分为标准和多样化子集的评估框架，以评估语言泛化能力。

Result: 结果显示，在标准和多样化数据上训练的模型始终优于仅在标准语料库上训练的模型，在所有任务类型上提高了性能，同时没有损害标准基准的准确性。

Conclusion: 这些发现强调了语言多样性在构建包容性、可泛化语言模型中的重要性，表明结合多样化语言变体训练能提升模型性能而不影响标准任务表现。
...
### [125] [Is Lying Only Sinful in Islam? Exploring Religious Bias in Multilingual Large Language Models Across Major Religions](https://arxiv.org/abs/2512.03943)
*Kazi Abrab Hossain,Jannatul Somiya Mahmud,Maria Hossain Tuli,Anik Mitra,S. M. Taiabul Haque,Farig Y. Sadeque*

Main category: cs.CL

TL;DR: 本文介绍了BRAND数据集，用于评估多语言大语言模型在宗教敏感话题上的偏见问题，特别关注南亚四大宗教，发现模型在英语表现优于孟加拉语，且对伊斯兰教存在系统性偏见。
...
Method: 引入BRAND（双语宗教问责规范数据集），专注于南亚四大宗教（佛教、基督教、印度教、伊斯兰教），包含2400多个条目，使用英语和孟加拉语三种不同类型的提示来评估模型表现。

Result: 模型在英语中的表现优于孟加拉语，即使在回答宗教中立问题时也持续显示出对伊斯兰教的偏见。这些发现突显了多语言模型在不同语言中回答相似问题时存在的持续偏见。

Conclusion: 研究结果强调了多语言模型在宗教敏感话题上的偏见问题，并将这些发现与人机交互领域中宗教和灵性相关的更广泛问题联系起来，表明需要更公平的多语言模型开发。
...
### [126] [Adapting Large Language Models to Low-Resource Tibetan: A Two-Stage Continual and Supervised Fine-Tuning Study](https://arxiv.org/abs/2512.03976)
*Lifeng Chen,Ryan Lai,Tianming Liu*

Main category: cs.CL

TL;DR: 该研究采用两阶段方法将Qwen2.5-3B模型适配到藏语：先通过持续预训练建立藏语语言基础，再通过监督微调进行任务和翻译专业化，显著提升了藏语处理性能。
...
Method: 采用两阶段适配方法：1）持续预训练（CPT）建立藏语语言基础；2）监督微调（SFT）进行任务和翻译专业化。对Qwen3-4B的435层进行层间分析。

Result: 困惑度从2.98降至1.54；汉藏翻译质量显著提升（BLEU从0.046提升至0.261，chrF从2.2提升至6.6）。适配主要集中在嵌入层和输出头，中后期MLP投影编码领域特定转换。

Conclusion: CPT构建藏语语义流形，SFT以最小表示扰动优化任务对齐。该研究首次定量探索了藏语适配动态，为低资源语言的多语言基础模型扩展提供了开放可复现框架。
...
### [127] [Teaching Old Tokenizers New Words: Efficient Tokenizer Adaptation for Pre-trained Models](https://arxiv.org/abs/2512.03989)
*Taido Purason,Pavel Chizhov,Ivan P. Yamshchikov,Mark Fishel*

Main category: cs.CL

TL;DR: 该论文提出两种互补的tokenizer适应方法：继续BPE训练用于词汇扩展，以及基于叶子的词汇剪枝，以提高tokenizer在新领域或语言中的效率。


<details>
  <summary>Details</summary>
Motivation: 当前tokenizer适应方法在将预训练语言模型迁移到新领域或语言时存在不足。常用的词汇扩展方法是在领域特定文本上训练新tokenizer并添加非重叠词汇，但这会导致许多不可达或从未使用的token。

Method: 提出两种互补方法：1) 继续BPE训练：通过在新增数据上继续BPE合并学习过程来适应预训练tokenizer；2) 基于叶子的词汇剪枝：移除冗余token同时保持模型质量。

Result: 在多语言和多种模型家族上的实验表明，继续BPE训练方法提高了tokenization效率并更好地利用了新增词汇。词汇剪枝方法能有效移除冗余token。

Conclusion: 这两种方法为受控词汇修改提供了实用工具，作者已将其作为开源包发布。这些方法能有效改善tokenizer在新领域或语言中的适应效果。
...
### [128] [AugServe: Adaptive Request Scheduling for Augmented Large Language Model Inference Serving](https://arxiv.org/abs/2512.04013)
...
<details>
  <summary>Details</summary>
Motivation: 随着增强型大语言模型在Web应用中的普及，提升推理服务效率和优化服务水平目标变得至关重要。现有系统面临两个主要挑战：1）先到先服务调度导致严重的队头阻塞，使许多请求的排队延迟超过SLO；2）静态批处理令牌限制无法适应波动的负载和硬件条件，这两者都降低了有效吞吐量和服务质量。
...
Conclusion: AugServe通过创新的调度策略和动态批处理机制，显著提升了增强型LLM推理服务的效率和性能，为实际部署提供了有效的解决方案。
...
TL;DR: Jina-VLM是一个24亿参数的视觉语言模型，在开放2B规模VLM中实现了最先进的多语言视觉问答性能，结合了SigLIP2视觉编码器和Qwen3语言骨干，支持任意分辨率图像的高效处理。


<details>
  <summary>Details</summary>
Motivation: 开发一个在2B参数规模下具有领先多语言视觉问答能力的开放视觉语言模型，同时保持文本性能的竞争力，为研究社区提供高质量的VLM资源。

Method: 采用SigLIP2视觉编码器与Qwen3语言骨干相结合，通过注意力池化连接器实现任意分辨率图像的标记高效处理，模型参数为24亿。

Result: 在标准VQA基准测试和多语言评估中取得领先结果，同时保持竞争力的纯文本性能，模型权重和代码已公开发布。

Conclusion: Jina-VLM在2B规模VLM中实现了最先进的多语言视觉问答性能，为研究社区提供了高质量的开放视觉语言模型资源。
...
### [130] [SkillFactory: Self-Distillation For Learning Cognitive Behaviors](https://arxiv.org/abs/2512.04072)
*Zayne Sprague,Jack Lu,Manya Wadhwa,Sedrick Keh,Mengye Ren,Greg Durrett*

Main category: cs.CL

TL;DR: SkillFactory是一种在强化学习前通过监督微调让模型学习认知技能的方法，即使基础模型不具备这些技能


<details>
  <summary>Details</summary>
Motivation: 如何让模型掌握基础模型不具备的认知技能（如验证答案、回溯、尝试替代方法等），而不是依赖从更强模型的蒸馏

Method: 通过重新排列模型自身的输出来创建"银色"SFT训练数据，在强化学习前进行监督微调，让模型初步学习认知技能

Result: 1) SkillFactory SFT初始化帮助模型在强化学习后泛化到更难任务变体；2) 模型确实使用了认知技能；3) SkillFactory模型在域外任务上比基础模型更稳健

Conclusion: 强化学习前学习的归纳偏置有助于模型学习稳健的认知技能使用
