...
TL;DR: 提出了MeasureBench基准测试，用于评估视觉语言模型在读取测量仪器方面的能力，发现当前最先进的模型在指针定位等精细空间感知任务上存在显著困难。


<details>
  <summary>Details</summary>
Motivation: 人类读取测量仪器相对容易且不需要太多专业知识，但当前视觉语言模型在此任务上表现不佳，需要专门的基准测试来推动模型在视觉测量阅读方面的进步。

Method: 开发了MeasureBench基准测试，包含真实世界和合成图像，并建立了可扩展的数据合成流程，能够程序化生成具有可控视觉外观的测量仪器，包括指针、刻度、字体、光照和背景等关键细节的变化。

Result: 评估显示即使是最先进的视觉语言模型在测量阅读任务上普遍表现不佳，主要失败模式是指针定位问题，模型能够识别数字但无法准确定位指针位置，导致数值错误。初步的强化学习实验在合成数据上有一定效果，但对真实图像效果有限。

Conclusion: 当前视觉语言模型在精细空间感知方面存在根本性局限，该基准测试资源有望推动未来在视觉基础计算和精确空间感知方面的进展，弥合数字识别与世界测量之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 传统的自适应密度控制方法仅基于位置梯度的大小进行基元分裂，忽略了梯度的方向信息，导致冗余分裂和次优的分裂位置。本文旨在通过引入梯度方向一致性来更好地捕捉局部结构复杂性，提高效率和质量。

Method: 提出方向一致性驱动的自适应密度控制（DC4GS），通过梯度的角度一致性来评估方向一致性，避免不必要的分裂。当需要分裂时，利用方向一致性定义最优分裂位置，使子基元更好地对齐局部结构，而不是传统的随机放置。

Result: 实验结果表明，DC4GS相比现有自适应密度控制方法，基元数量减少了高达30%，同时显著提高了重建保真度。

Conclusion: 方向一致性驱动的自适应密度控制方法能够有效减少3D高斯泼溅中的基元数量，同时提升重建质量，为3D重建提供了更高效的解决方案。
...
Method: SACL通过自适应epoch调度、困难样本注入和尺度感知优化三个关键机制，根据可用数据规模动态调整课程设计。

Result: 在LUNA25数据集上，SACL在完整数据集上达到与静态课程学习相当的mAP50性能，但在数据受限条件下显著优于基线，在10%、20%和50%训练数据时分别提升4.6%、3.5%和2.0%。

Conclusion: SACL能够在不同数据规模下实现稳健训练，无需架构修改，为医疗机构在标注资源有限的情况下开发有效的肺结节检测系统提供了实用解决方案。
...
TL;DR: 提出了一种统一的多流SYNAPSE-Net框架，用于多模态MRI中异质性脑病变的自动分割，在三个公开数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型通常是缺乏泛化能力和高性能方差的专门化'点解决方案'，限制了其临床可靠性。

Method: 构建了集成多流CNN编码器、Swin Transformer瓶颈、动态跨模态注意力融合机制和分层门控解码器的混合架构，采用结合病理特定数据增强和难度感知采样方法的方差减少策略进行训练。

Result: 在WMH数据集上获得DSC值0.831和HD95值3.03；在ISLES 2022上获得最佳边界精度（HD95值9.69）；在BraTS 2020上获得肿瘤核心区域最高DSC值0.8651。
...
TL;DR: 提出了一种基于语义帧聚合的Transformer模型（SFAT），用于直播视频评论生成。该模型通过CLIP视觉-文本多模态知识生成评论，并根据语义相关性为视频帧分配权重，强调信息丰富的帧，减少无关帧的影响。
...
Method: SFAT模型利用CLIP的视觉-文本多模态知识，基于语义相关性为视频帧分配权重，采用加权帧求和技术强调信息帧，并使用带有跨注意力机制的评论解码器。

Result: 构建了一个大规模、多样化的英语视频评论数据集，涵盖11个视频类别，总计438小时和320万条评论。SFAT模型在生成直播视频和对话上下文评论方面优于现有方法。

Conclusion: SFAT模型通过语义帧聚合和跨注意力机制，能够有效生成上下文适当的直播视频评论，解决了现有方法在帧优先级排序方面的不足。
...
TL;DR: MoME是一种用于医学图像分割的视觉语言混合专家模型，将MoE范式从大语言模型成功应用于医学视觉语言任务，通过动态专家选择和多尺度视觉特征提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 将在大语言模型中广泛成功的混合专家(MoE)范式应用于医学视觉语言任务，利用多尺度视觉特征和文本嵌入来解决医学图像的复杂性。

Method: 采用混合专家架构，动态选择专家，有效利用多尺度视觉特征和文本嵌入，在10个数据集共3410个CT扫描上进行训练和评估。

Result: 在综合性医学图像分割基准测试中表现出强大性能，在多个数据集上获得有竞争力的精度。

Conclusion: MoME探索了基础模型在医学成像中的集成，展示了MoE架构在医学图像分析中实现稳健结果的新颖方法。
...
TL;DR: 本文提出了一种无样本增量关系蒸馏（IRD）框架来解决增量人-物交互检测（IHOID）问题，该框架通过解耦对象和关系学习，并使用两种独特的蒸馏损失来学习跨不同HOI组合的不变关系特征。


<details>
  <summary>Details</summary>
Motivation: 在开放世界环境中，人-物交互持续演变，挑战传统的封闭世界HOI检测模型。受人类逐步获取知识能力的启发，探索增量HOI检测以开发能够在动态环境中识别人-物关系的智能体。

Method: 提出无样本增量关系蒸馏（IRD）框架，解耦对象和关系的学习，引入两种独特的蒸馏损失来学习跨不同HOI组合的不变关系特征。

Result: 在HICO-DET和V-COCO数据集上的广泛实验表明，该方法在缓解遗忘、增强对交互漂移的鲁棒性以及零样本HOI泛化方面优于最先进的基线方法。

Conclusion: IRD框架有效解决了增量HOI检测中的挑战，包括灾难性遗忘、交互漂移和零样本HOI组合检测问题，为动态环境中的关系识别提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个更准确、更全面的远程生理信号监测模型，能够从面部视频中同时估计多种生理参数，为远程健康监测提供更强大的工具。

Method: 采用新的模型架构，并大幅增加训练数据的规模和多样性，训练数据包含1,413个独特个体。在422个独特个体的组合测试集上进行评估。

Result: VitalLens 2.0实现了新的最先进性能：心率MAE为1.57 bpm，呼吸率MAE为1.08 bpm，HRV-SDNN MAE为10.18 ms，HRV-RMSSD MAE为16.45 ms，显著优于先前方法。

Conclusion: VitalLens 2.0在远程生理信号估计方面取得了重大突破，为开发者提供了通过API访问的先进生理监测工具。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶场景具有复杂的空间和几何特性，需要专门优化的语义分割模型来处理道路场景的挑战。

Method: 采用双编码器架构结合SAM的ViT-H全局语义和ResNet-50局部细节，使用可变形融合模块对齐多尺度特征，通过可变形注意力进行多阶段精炼，并采用混合损失函数优化训练。

Result: 在Cityscapes上达到68.1 mIoU，BDD100K上达到59.5 mIoU，分别比基准模型提升22.9和19.2 mIoU，具有更强的跨域泛化能力和数据效率。

Conclusion: 针对基础模型进行架构和优化增强能够实现可靠且可扩展的自动驾驶感知系统。
...
TL;DR: HiT是一种新颖的分层神经场表示方法，通过无监督学习从粗到细地学习3D形状的通用层次结构。核心贡献是分层变换器，使用压缩码本学习树层次结构中的父子关系，能自动识别不同形状类别间的共同子结构。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常限制在固定层次结构（如二叉树），无法灵活适应数据的自然层次结构。HiT旨在从数据中直接推断层次结构，支持更通用和复杂的层次表示。

Method: 提出分层变换器（HiT），每个层级使用压缩码本学习父子关系。不限制具体层次结构类型，只限制每层节点总数，让模型从数据中自动学习层次结构。

Result: 在55个ShapeNet类别上进行无监督形状分割任务，成功将形状分割为多个粒度级别，证明了方法的有效性。

Conclusion: HiT能够从数据中学习有意义的层次结构，捕获父子节点间的包含关系，在无监督形状分割任务中表现出色，支持多粒度分割。
...
TL;DR: 本文提出了WildfireX-SLAM数据集，这是一个用于森林和野火环境中SLAM研究的大规模合成数据集，包含5.5k张低空RGB-D航空图像，覆盖16平方公里森林区域。


<details>
  <summary>Details</summary>
Motivation: 开发3DGS-based SLAM方法在大型森林场景中具有重要应用价值（如野火应急响应和森林管理），但缺乏高质量数据集阻碍了相关研究。

Method: 利用Unreal Engine 5 Electric Dreams环境样本项目开发数据采集流水线，可灵活控制光照、天气和野火条件等环境因素，收集航空和地面视图的多种数据模态。

Result: 创建了包含5.5k张低空RGB-D航空图像的大规模数据集，并进行了全面基准测试，揭示了森林环境中3DGS-based SLAM的独特挑战。
...
TL;DR: 提出了E-MMDiT，一种仅304M参数的高效轻量级多模态扩散模型，通过令牌压缩、位置强化和交替子区域注意力等技术，在单节点8个AMD MI300X GPU上仅用1.5天训练即可实现快速图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通常需要大规模训练数据和大量计算资源，或者结构复杂延迟高。为了降低训练资源需求并提升效率，需要开发更轻量高效的模型。

Method: 采用高度压缩的视觉令牌化器生成紧凑表示；提出多路径压缩模块进一步压缩令牌；引入位置强化技术保持空间一致性；使用交替子区域注意力降低计算成本；提出AdaLN-affine轻量模块计算调制参数。

Result: 在512px图像生成任务中，仅使用2500万公开数据训练1.5天，在GenEval上达到0.66分，通过GRPO等后训练技术可提升至0.72分。
...
TL;DR: 提出跨视图和跨注意力模块(CVCAM)与多头空间注意力模块(MHSAM)，解决跨视图物体地理定位中信息传递不足和空间关系特征图细化不够的问题，并创建了G2D数据集填补地面到无人机定位任务的空白。


<details>
  <summary>Details</summary>
Motivation: 现有方法在跨视图物体地理定位中未能有效传递视图间信息，且未进一步细化空间关系特征图，导致模型错误关注无关边缘噪声，影响定位性能。

Method: 引入CVCAM模块进行多轮视图间交互，实现上下文信息的持续交换和学习；集成MHSAM模块使用不同尺寸卷积核提取多尺度空间特征；创建G2D数据集填补地面到无人机定位任务空白。

Result: 在CVOGL和G2D数据集上的大量实验表明，所提方法实现了高定位精度，超越了当前最先进方法。

Conclusion: 提出的CVCAM和MHSAM模块有效提升了跨视图物体地理定位性能，通过多视图交互和多尺度特征提取解决了现有方法的局限性，新数据集为地面到无人机定位任务提供了重要资源。
...
TL;DR: HiGS是一个分层生成框架，通过多步骤关联语义空间组合实现3D场景生成，让用户能够迭代扩展场景，选择关键语义对象，同时模型自动完成周边区域。
...
Method: 提出渐进式分层空间语义图(PHiSSG)，动态组织空间关系和语义依赖，保持图节点与生成对象的一对一映射，支持递归布局优化。
...
TL;DR: 提出AFM-Net框架，通过CNN和Mamba双路径实现局部与全局特征的有效融合，解决遥感图像场景分类中多尺度特征建模的挑战，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 遥感图像场景分类面临复杂空间结构和多尺度特征的挑战，现有方法中CNN擅长局部纹理建模而Transformer擅长全局上下文捕捉，但Transformer计算成本高，需要高效融合两者的方法。

Method: 提出AFM-Net框架，包含CNN分支提取层次化视觉先验和Mamba分支进行高效全局序列建模，通过分层融合机制逐步聚合多尺度特征，实现动态跨层特征交互和上下文重构，最后通过专家混合分类器进行细粒度场景识别。

Result: 在AID、NWPU-RESISC45和UC Merced数据集上分别达到93.72%、95.54%和96.92%的准确率，超越了现有最优方法，在性能和效率上达到平衡。

Conclusion: AFM-Net通过CNN和Mamba的有效融合，成功解决了遥感图像场景分类中的多尺度特征建模问题，在多个基准数据集上表现出优越性能，为相关研究提供了新的思路。
...
TL;DR: 本研究探索了使用现有深度学习模型通过模块化、基于规则的框架来近似Banff病变评分的可行性，发现当前AI流程在复制专家级计算评分方面存在局限性。
...
Conclusion: 结果揭示了当前AI流程在复制计算专家级评分方面的局限性，并强调了模块化评估和计算Banff分级标准在指导移植病理学未来模型开发中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在预训练时通常使用低分辨率输入，当处理高分辨率图像时会丢失视觉细节和遗漏重要物体，导致生成的描述不准确和不详细。
...
Conclusion: 提出的多阶段流程通过整合VLM、LLM和物体检测系统，显著提升了高分辨率图像描述的质量，在细节丰富性和可靠性方面都有明显改善。
...
TL;DR: M^3Detection是一个统一的多帧3D目标检测框架，通过相机和4D成像雷达的多模态数据融合，在第二阶段进行全局和局部特征聚合，提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有相机-雷达融合方法大多局限于单帧输入，只能捕捉场景的部分信息，加上图像退化和4D雷达稀疏性，限制了检测性能。多帧融合虽然能提供更丰富的时空信息，但面临跨帧跨模态特征融合和计算成本高的挑战。

Method: 利用基线检测器的中间特征和跟踪器生成的参考轨迹，在第二阶段设计全局级对象间特征聚合模块（雷达信息引导）和局部级网格间特征聚合模块（沿参考轨迹扩展），最后通过轨迹级多帧时空推理模块编码跨帧交互。

Result: 在VoD和TJ4DRadSet数据集上的大量实验表明，M^3Detection实现了最先进的3D检测性能。

Conclusion: 该框架在相机-4D成像雷达融合的多帧检测中具有有效性，通过多级特征融合和时空推理提升了检测性能。
...
TL;DR: 提出DANCER框架，基于稳定视频扩散模型实现单人生成舞蹈视频，通过外观增强模块和姿态渲染模块提升生成质量，并构建TikTok-3K数据集增强训练。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在视觉生成任务中表现出色，但视频生成特别是涉及人体舞蹈的内容更具挑战性，需要保证视频连续性和人体运动的高自由度。

Method: 基于稳定视频扩散模型，引入外观增强模块（AEM）关注参考图像细节，姿态渲染模块（PRM）从额外域捕获姿态条件，并构建TikTok-3K数据集增强训练。

Result: 在真实世界数据集上的广泛实验表明，该模型性能优于现有最先进方法。

Conclusion: DANCER框架能够有效生成高质量的单人舞蹈视频，在舞蹈合成任务中表现出优越性能。
...
TL;DR: H2-Cache是一种新颖的分层缓存机制，通过将去噪过程分离为结构定义和细节精炼两个阶段，使用双阈值系统选择性缓存，实现了扩散模型推理的显著加速（最高5.08倍），同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面表现优异，但其迭代去噪过程计算成本高，阻碍了实际部署。现有的缓存技术在速度和保真度之间存在权衡，存在质量下降和计算开销大的问题。

Method: 提出H2-Cache分层缓存机制，基于去噪过程可分离为结构定义阶段和细节精炼阶段的洞察，采用双阈值系统选择性缓存每个阶段。引入池化特征摘要（PFS）技术进行轻量级相似性估计。
...
TL;DR: 本文提出了一种双层次渐进式难度感知重加权策略（DPHR），用于解决无人机与卫星图像跨视角地理定位中的严重视角差异和困难负样本问题。该方法在样本级别评估相对难度并分配细粒度权重，在批次级别利用训练进度信号来衰减噪声梯度并逐步增强困难负样本挖掘。


<details>
  <summary>Details</summary>
Motivation: 跨视角地理定位面临严重视角差异和困难负样本问题，现有方法使用静态加权策略对分布偏移敏感，容易过早过度强调困难样本，导致噪声梯度和不稳定收敛。

Method: 提出双层次渐进式难度感知重加权策略：1）样本级别的比率难度感知模块评估相对难度并分配细粒度权重；2）批次级别的渐进式自适应损失加权机制利用训练进度信号，在早期优化中衰减噪声梯度，随着训练成熟逐步增强困难负样本挖掘。

Result: 在University-1652和SUES-200基准测试上的实验表明，所提出的DPHR方法有效且鲁棒，相比最先进方法实现了持续改进。

Conclusion: DPHR策略通过双层次渐进式难度感知重加权，有效解决了跨视角地理定位中的困难负样本问题，提高了方法的鲁棒性和性能。
...
TL;DR: 该论文提出了多模态交互式真实性评估（MIVA）任务，基于狼人杀游戏构建了包含视频和文本的多模态数据集，评估了当前最先进的多模态大语言模型在识别谎言方面的表现，发现即使GPT-4o等强大模型也难以可靠地区分真假。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越多地融入人类生活，赋予它们强大的社会智能已成为关键前沿。识别真假是社交智能的重要方面，但在动态、多方对话中的自动欺骗检测仍是一个重大挑战。

Method: 引入了新的多模态交互式真实性评估（MIVA）任务，并基于狼人杀游戏构建了一个新颖的多模态数据集，包含同步的视频、文本和可验证的真实性标签。建立了全面的基准来评估最先进的多模态大语言模型。

Result: 评估结果显示存在显著的性能差距：即使是像GPT-4o这样强大的模型也难以可靠地区分真假。失败模式分析表明这些模型未能有效地将语言与视觉社交线索相结合，并且可能在对其齐方面过于保守。

Conclusion: 迫切需要新的方法来构建更具洞察力和可信度的AI系统，当前的多模态大语言模型在识别谎言方面存在明显不足，需要进一步改进其对社会线索的理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像诊断中构建鲁棒且泛化性强的模型面临挑战，主要由于大规模准确标注数据稀缺以及动态医疗环境中的领域偏移问题，特别是在胸部CT中窗口设置差异导致的领域偏移，且传统方法因隐私限制无法重用历史数据。
...
Conclusion: 该框架能有效学习多窗口CT图像的多样化特征，同时确保数据隐私，为医学图像诊断提供了鲁棒且泛化性强的解决方案。
...
TL;DR: YOLOv11在卫星图像建筑实例分割和高度分类任务中表现出色，在DFC2023数据集上达到60.4% mAP@50和38.3% mAP@50-95，优于先前多任务框架，适合实时大规模城市测绘。
...
Method: 使用YOLOv11深度学习模型，结合多尺度特征融合改进架构，在DFC2023 Track 2数据集（包含12个城市的125,000多个标注建筑）上进行评估，采用精度、召回率、F1分数和mAP等指标。

Result: YOLOv11在实例分割方面表现优异，达到60.4% mAP@50和38.3% mAP@50-95，在五个预定义高度层级上保持稳健的分类精度，特别擅长处理遮挡、复杂建筑形状和类别不平衡问题。

Conclusion: YOLOv11在检测精度和推理速度上均优于早期多任务框架，具有推进语义城市重建的潜力，为遥感和地理空间智能的未来发展提供了可行见解。
...
TL;DR: MoRE是一个基于混合专家架构的密集3D视觉基础模型，通过动态路由特征到任务特定专家来增强可扩展性和适应性，结合置信度深度细化模块和语义特征集成，在多个几何任务上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决3D模型扩展面临的几何监督复杂性和3D数据多样性挑战，旨在开发一个既能大规模训练又能在真实世界条件下保持鲁棒性的3D视觉基础模型。

Method: 采用混合专家架构动态路由特征，集成置信度深度细化模块稳定几何估计，结合密集语义特征与全局对齐的3D骨干表示，并使用定制损失函数进行多任务鲁棒学习。

Result: 在多个基准测试中达到最先进性能，支持有效的下游应用而无需额外计算开销。

Conclusion: MoRE通过混合专家架构和精心设计的模块，成功解决了3D模型扩展的挑战，为3D视觉几何重建提供了一个强大且可扩展的基础模型。
...
TL;DR: RegionRAG是一个多模态检索增强生成框架，通过从文档级检索转向区域级检索来解决视觉文档中无关内容的问题，提高了检索和问答的准确性。
...
Method: 提出从文档级检索转向区域级检索的新范式，在训练中使用混合监督策略从有标签和无标签数据中定位相关图像块，在推理时通过动态管道将显著图像块智能分组为完整语义区域。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像中，预训练模型缺乏模态特异性，而微调专家模型在模态偏移下性能下降，现有模型融合方法在多样医学模态中无法提供一致增益。

Method: 提出T^3框架，无需反向传播，通过计算两个模型输出分布的Jensen-Shannon散度来获得每个样本的插值系数，动态平衡局部精度和泛化鲁棒性。

Result: 在跨四个模态的严格评估中，T^3在Top-1准确率和错误率降低方面达到新的最优水平，显著优于基线方法。

Conclusion: T^3为临床环境中的自适应多模态视觉语言模型部署铺平了道路，在保持效率的同时实现了卓越性能。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型将图像和视频帧表示为视觉token，但将单图像扩展到小时级视频时，token数量会超出实际限制。现有的关键帧选择方法要么均匀采样，要么使用检索式评分，但需要预过滤且可能错过最具信息量的时刻。

Method: FOCUS将关键帧选择建模为多臂老虎机中的组合纯探索问题：将短时间片段视为臂，使用经验均值和Bernstein置信半径识别信息丰富区域，同时保留不确定区域的探索。采用两阶段探索-利用过程，首先识别高价值时间区域，然后在每个区域内选择得分最高的帧。

Result: 在两个长视频问答基准测试中，FOCUS仅处理不到2%的视频帧就实现了显著的准确率提升。对于超过20分钟的视频，在LongVideoBench上准确率提高了11.9%。

Conclusion: FOCUS作为一种关键帧选择方法，为使用MLLM进行可扩展的长视频理解提供了一个简单通用的解决方案，在严格token预算下有效提升了长视频理解的性能。
...
TL;DR: 本文提出S-GRACE方法，通过语义引导生成对抗样本来改进扩散模型的概念擦除性能，相比现有方法提升26%擦除效果，减少90%训练时间。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法在扩散模型中使用对抗训练时忽略概念语义，导致无法有效覆盖目标概念空间，要么擦除不彻底，要么干扰其他概念空间。

Method: 提出S-GRACE方法，在概念空间内利用语义引导生成对抗样本并进行擦除训练，确保对抗样本能更全面地拟合目标概念空间。

Result: 在多种扩散模型遗忘场景下，与7种最先进方法和3种对抗提示生成策略相比，S-GRACE显著提升擦除性能26%，更好保留非目标概念，减少90%训练时间。

Conclusion: S-GRACE通过语义引导的对抗样本生成有效解决了现有概念擦除方法的局限性，在扩散模型中实现了更高效和精确的概念擦除。
...
TL;DR: 提出SAGS框架，一种自适应的无锯齿高斯溅射方法，用于内窥镜视频中的动态组织重建，通过4D变形解码器和多尺度滤波技术解决组织移动引起的锯齿和伪影问题。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频中动态组织的重建在机器人辅助手术中至关重要，但现有方法在组织移动时会产生锯齿和伪影，严重影响可视化质量。
...
Conclusion: SAGS方法在动态内窥镜组织重建中实现了卓越的性能，有效解决了组织移动引起的锯齿和伪影问题，为机器人辅助手术提供了高质量的3D重建解决方案。
...
TL;DR: 本文提出了一种超低比特率视觉通信方法，通过结合文本生成和深度图像压缩技术，在极低带宽下实现准确的视觉场景重建和视觉分析。
...
Method: 将图像生成与深度图像压缩无缝集成，使用联合文本和编码潜在向量来指导修正流模型，实现视觉场景的精确生成。语义文本描述和编码潜在都以极低比特率进行编码和传输。

Result: 实验结果表明，该方法在使用更少带宽的情况下，能够达到与现有方法相同的图像重建质量和视觉分析精度。

Conclusion: 提出的方法在超低比特率下实现了准确的视觉场景重建，为极低带宽环境下的视觉通信提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺癌检测面临挑战，主要原因是高质量分割标签的可用性有限，需要开发更有效的分类方法以改善早期检测。

Method: 采用迭代开发过程，包括概念设计、实验、评估和优化，重点关注性能、鲁棒性和临床相关性。

Result: 提供了完整的实现方案并公开发布在GitHub上，展示了方法在乳腺癌MRI分析中的潜力。

Conclusion: 基于分类的方法对于乳腺癌早期检测具有重要价值，特别是在大规模筛查应用中，需要继续开发更鲁棒的解决方案。
...
Result: 在SmartEdit数据集上PSNR指标提升9.955 dB，在自建CIEBench基准测试中也优于先前方法，能更好地保持应一致的区域。

Conclusion: CIELR方法通过LLM推理有效处理复杂图像编辑指令，无需联合微调，在保持图像一致性方面表现优异，为复杂图像编辑提供了高效解决方案。
...
TL;DR: RzenEmbed是一个统一的多模态嵌入框架，支持文本、图像、视频和视觉文档等多种模态，通过两阶段训练策略和增强的InfoNCE损失函数，在MMEB基准测试中实现了最先进的性能。
...
Result: 在MMEB基准测试中实现了新的最先进水平，不仅在总体得分上表现最佳，还在具有挑战性的视频和视觉文档检索任务上超越了所有先前工作。

Conclusion: RzenEmbed成功构建了一个统一的多模态嵌入框架，有效解决了现有方法在视频和视觉文档等模态上的局限性，为跨模态检索任务提供了强大的解决方案。
...
TL;DR: 提出了Feedforward-based Parameter Selection (FPS)，一种无需梯度的参数选择方法，通过单次前向传播识别最优参数子集，解决了现有参数高效微调方法在推理延迟和内存使用方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法存在显著限制：基于加法的方法（如Adapters）引入推理延迟和工程复杂性，而基于选择的方法（如GPS）需要完整反向传播，导致与全微调相同的峰值内存使用。

Method: FPS通过参数幅度与对应输入激活的乘积来排名参数，利用预训练知识和下游数据，在单次前向传播中识别最优参数子集。
...
TL;DR: 提出了一个两阶段微调流程，用于视频扩散变换器，能够从小数据集生成电视和电影制作的电影场景。第一阶段使用LoRA模块在单GPU上快速适应视觉风格，第二阶段生成风格一致的关键帧并扩展为连贯的720p序列。


<details>
  <summary>Details</summary>
Motivation: 为电视和电影制作开发实用的电影场景合成管道，解决从小数据集高效学习视觉风格和运动生成的问题。

Method: 两阶段流程：第一阶段使用LoRA模块在交叉注意力层进行视觉风格适应，第二阶段生成风格一致的关键帧并通过视频解码器扩展为连贯序列。采用轻量级并行化和序列分区策略加速推理。

Result: 定量和定性评估（FVD、CLIP-SIM、LPIPS指标）以及专家用户研究表明，在电影保真度和时间稳定性方面相比基础模型有显著改进。

Conclusion: 提出的完整训练和推理管道支持跨电影领域的可重复性和适应性，为电影制作提供了高效的场景生成解决方案。
...
TL;DR: 提出Alignment across Trees方法，通过构建和对齐图像与文本的树状层次特征来解决视觉语言模型中的模态不对称问题，在双曲流形中进行跨模态对齐，在少样本和跨域分类任务中表现优异。
...
Method: 构建语义感知的视觉特征提取框架，使用交叉注意力机制从中间Transformer层提取粗到细的视觉特征；将两种模态的特征树嵌入具有不同曲率的双曲流形；通过最小化异构流形间的KL距离来学习中间流形进行对齐。

Result: 在多个图像数据集上的分类学开放集分类任务中，该方法在少样本和跨域设置下持续优于强基线方法。

Conclusion: 提出的跨树对齐方法有效解决了视觉语言模型中的模态不对称问题，通过层次特征提取和异构双曲流形对齐实现了更好的跨模态集成。
...
Method: 作者审计了乳腺癌肿瘤分割数据集MAMA-MIA中自动分割标签的公平性，评估了不同年龄、种族和数据源的分割质量。

Result: 分析揭示了针对年轻患者的内在年龄相关偏见，即使在控制数据源等混杂因素后仍然存在。此外，多数据源聚合会影响特定站点的种族偏见。
...
TL;DR: 本文提出了解决部分相关视频检索中语义坍缩问题的新方法，包括文本相关性保持学习和跨分支视频对齐，有效提升了检索准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将所有标注的文本-视频对视为正样本，其他为负样本，忽略了视频内部和跨视频的丰富语义变化，导致同一视频中不同事件的查询和视频片段嵌入坍缩在一起，而不同视频中语义相似的查询和片段却被推远，限制了检索性能。

Method: 1. 文本相关性保持学习：保持基础模型编码的文本查询间的语义关系；2. 跨分支视频对齐：通过对比对齐方法在不同时间尺度上解缠层次化视频表示；3. 保序令牌合并和自适应CBVA：生成内部一致但相互区分的视频片段以增强对齐。

Result: 在PRVR基准测试上的广泛实验表明，该框架有效防止了语义坍缩，显著提高了检索准确性。

Conclusion: 提出的方法成功解决了部分相关视频检索中的语义坍缩问题，通过保持文本语义关系和跨尺度视频对齐，大幅提升了检索性能。
...
### [41] [Towards Universal Video Retrieval: Generalizing Video Embedding via Synthesized Multimodal Pyramid Curriculum](https://arxiv.org/abs/2510.27571)
*Zhuoning Guo,Mingxin Li,Yanzhao Zhang,Dingkun Long,Pengjun Xie,Xiaowen Chu*

Main category: cs.CV

TL;DR: 提出了一个视频检索框架，通过评估、数据和建模的协同设计来解决现有方法结构不对齐的问题。包括建立通用视频检索基准(UVRB)、生成大规模合成数据、以及设计模态金字塔训练方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频检索方法在狭窄基准上训练，导致通用能力受限，缺乏诊断性评估来定义和需求多维度泛化能力。

Method: 1) 建立16个数据集的UVRB基准；2) 基于诊断生成155万高质量合成数据对；3) 设计模态金字塔课程训练通用视频嵌入器(GVE)。

Result: GVE在UVRB上实现了最先进的零样本泛化性能，分析显示流行基准对通用能力的预测性差，部分相关检索是重要但被忽视的场景。
...
TL;DR: CoMViT是一种紧凑且可泛化的Vision Transformer架构，专为资源受限的医学图像分析设计，通过集成卷积分词器、对角掩码、动态温度缩放和池化序列聚合等技术，在12个MedMNIST数据集上实现稳健性能，仅需约450万参数，性能优于或匹配更深层的CNN和ViT变体，参数量减少5-20倍。
...
Method: CoMViT集成了卷积分词器、对角掩码、动态温度缩放和池化序列聚合，通过系统架构优化实现轻量级设计。

Result: 在12个MedMNIST数据集上实现稳健性能，仅需约450万参数，匹配或优于更深层的CNN和ViT变体，参数量减少5-20倍而不牺牲准确性。

Conclusion: 这些结果突出了原则性ViT重新设计在低资源医学成像环境中开发高效可解释模型的潜力。
...
TL;DR: VisPainter是一个基于模型上下文协议的多智能体框架，用于科学插图生成，通过模块化设计实现元素级控制和后期编辑能力，并引入VisBench基准进行七维评估。
...
Method: VisPainter采用多智能体框架，包含三个专门模块：Manager、Designer和Toolbox，基于模型上下文协议协同工作，生成与标准矢量图形软件兼容的图表。模块化、基于角色的设计使每个元素都能被显式表示和操作。

Result: 通过广泛的消融实验验证了架构的合理性和评估方法的可靠性，评估了各种视觉语言模型，提供了公平可信的模型排名及其能力详细比较，并分离量化了角色划分、步骤控制和描述对插图质量的影响。

Conclusion: VisPainter成功填补了科学插图生成中的空白，提供了真正的元素级控制和后期编辑能力，同时通过VisBench基准建立了系统的科学插图质量评估体系。
...
TL;DR: 提出一个多层次的人机交互框架，用于改进发展中国家教育设施记录的准确性和完整性。该框架结合机器学习分析、卫星影像和深度学习模型来识别和验证学校位置，并通过交互界面让人工操作员参与结果验证。


<details>
  <summary>Details</summary>
Motivation: 在发展中国家，教育设施记录往往稀缺且更新不及时，这影响了教育规划和资源分配。需要一种可扩展且成本效益高的方法来准确识别和验证学校位置。

Method: 采用多层次方法：第一层使用机器学习分析人口密度、土地覆盖和现有基础设施来识别潜在学校位置和错误标注；第二层使用中分辨率卫星影像识别学校可能存在的区域；第三层使用超高分辨率影像和深度学习模型生成详细的候选学校位置。中分辨率方法因改进不明显后被移除。整个框架包含交互界面供人工操作员验证和优化结果。

Result: 初步评估表明，这种多层次策略为教育基础设施映射提供了一个可扩展且成本效益高的解决方案，能够支持教育规划和资源分配。

Conclusion: 该人机交互框架通过结合自动化分析和人工验证，有效提高了学校位置识别的准确性和完整性，特别适用于数据稀缺的发展中地区。
...
TL;DR: 提出了一种名为Referee的参考感知音视频深度伪造检测方法，利用单次示例中的说话人特定线索来检测超出时空伪影的操纵，通过匹配和对齐参考与目标内容中的身份相关查询到跨模态特征，联合推理音视频同步性和身份一致性。


<details>
  <summary>Details</summary>
Motivation: 由于先进生成模型生成的深度伪造品迅速构成严重威胁，现有的音视频深度伪造检测方法难以泛化到未见过的伪造品。

Method: 利用单次示例中的说话人特定线索，通过匹配和对齐身份相关查询从参考和目标内容到跨模态特征，联合推理音视频同步性和身份一致性。
...
TL;DR: 本文构建了NautData数据集（145万图像-文本对），支持8个水下场景理解任务，并提出可插拔的视觉特征增强模块（VFE）来提升水下图像质量，最终构建了水下大型多模态模型NAUTILUS。


<details>
  <summary>Details</summary>
Motivation: 水下探索对资源勘探和国家安全等领域具有重要意义，但缺乏大规模水下多任务指令调优数据集，且水下图像退化问题严重干扰任务性能。

Method: 构建NautData数据集，引入水下成像模型的物理先验，提出可插拔的视觉特征增强模块（VFE）来显式恢复清晰的水下信息，并将其集成到LLaVA-1.5和Qwen2.5-VL基线模型中。

Result: 在NautData和公共水下数据集上的实验表明，VFE模块能持续提升两个基线模型在大多数支持任务上的性能，确保NAUTILUS在水下场景理解领域的优越性。

Conclusion: NAUTILUS通过结合大规模数据集和视觉特征增强技术，有效解决了水下场景理解中的关键挑战，为自动化水下探索提供了有力工具。
...
TL;DR: ThinkMorph是一个统一的多模态推理模型，通过24K高质量交错推理轨迹进行微调，学习生成渐进式的文本-图像推理步骤，在视觉中心基准测试中表现优异，并展现出新兴的多模态智能能力。
...
Result: 在视觉中心基准测试中平均比基础模型提升34.7%，能够泛化到领域外任务，匹配或超越更大和专有的视觉语言模型。模型展现出新兴的多模态智能能力，包括未见过的视觉操作技能、自适应切换推理模式，以及通过多样化多模态思维实现更好的测试时扩展。
...
Method: 基于Visual Mamba架构，通过上下文门控跨模态感知模块自适应增强模态间特征交互，强调信息丰富区域同时抑制噪声。
...
TL;DR: 提出Spatial-SSRL自监督强化学习范式，通过从普通RGB或RGB-D图像中自动生成五个空间结构预训练任务来提升大型视觉语言模型的空间理解能力，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型在空间理解方面存在不足，而传统的监督微调和强化学习方法依赖昂贵的监督数据、专用工具或受限环境，限制了可扩展性。

Method: 自动制定五个空间结构预训练任务：打乱补丁重排序、翻转补丁识别、裁剪补丁修复、区域深度排序和相对3D位置预测，这些任务提供易于验证的ground-truth答案。

Result: 在七个空间理解基准测试中，Spatial-SSRL相比Qwen2.5-VL基线在图像和视频设置下分别获得4.63%(3B)和3.89%(7B)的平均准确率提升，同时保持通用视觉能力。

Conclusion: 简单的内在监督能够实现大规模强化学习，为增强LVLMs的空间智能提供了实用途径。
...
TL;DR: 本文提出了一种基于用户草图作为直观约束的图形布局生成方法，通过多模态transformer模型将草图和内容资源转换为高质量布局，解决了传统约束方法复杂难用的问题。
...
Method: 采用多模态transformer架构，以草图和内容资源作为输入生成布局。提出了一种新颖的合成方法大规模生成训练草图数据，避免昂贵的人工标注。

Result: 在PubLayNet、DocLayNet和SlidesVQA三个公开数据集上测试，模型性能优于现有基于约束的方法，同时提供更直观的设计体验。
...
TL;DR: 提出VessShape方法生成大规模2D合成数据集，通过形状偏置训练提高血管分割模型的鲁棒性和数据效率，在少样本和零样本场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 血管语义分割面临标注数据稀缺和模型跨模态泛化能力差的问题，CNN倾向于学习纹理特征限制了在新领域的性能。假设利用血管的几何先验（管状和分支特性）可以构建更鲁棒的模型。

Method: 引入VessShape方法生成大规模2D合成数据集，包含程序生成的管状几何结构与多样化前景背景纹理，强制模型学习形状线索而非纹理特征。

Result: 在VessShape上预训练的模型在两个真实世界数据集上表现出强大的少样本分割性能，仅需4-10个样本进行微调。模型还展现出显著的零样本能力，能在未见过的领域有效分割血管而无需目标特定训练。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学应用中主要局限于2D成像，而3D PET/CT领域面临大容量体积数据、小而分散的病灶以及冗长放射学报告的挑战，需要开发专门针对3D医学影像的视觉语言模型。

Method: 构建了包含11,000多个病灶级描述和5,000多个PET/CT检查的3D分割的大规模数据集，采用基于规则和大型语言模型的混合管道提取数据。提出了PETAR-4B模型，整合PET、CT和病灶轮廓，实现全局上下文推理与细粒度病灶感知的结合。

Result: 综合自动化和人工评估表明，PETAR显著提高了PET/CT报告生成质量，在3D医学视觉语言理解方面取得了实质性进展。

Conclusion: PETAR模型成功将视觉语言模型扩展到3D PET/CT领域，能够生成临床连贯且具有空间定位的发现，推动了3D医学视觉语言理解的发展。
...
TL;DR: DUST是一个双流扩散世界模型增强的视觉-语言-动作框架，通过分离模态流解决多模态冲突，在机器人策略学习中实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 增强视觉-语言-动作模型与世界建模结合，但面临不同模态联合预测的挑战，因为视觉观察和动作序列在本质上存在差异。

Method: 提出多模态扩散变换器架构，维护分离的模态流同时支持跨模态知识共享；引入独立噪声扰动和解耦流匹配损失；基于训练时的模态解耦，提出支持测试时缩放的联合采样方法。

Result: 在RoboCasa和GR-1模拟基准上比基线方法提升高达6%，测试时缩放额外提供2-5%提升；在Franka Research 3真实任务中成功率提高13%；在BridgeV2无动作视频预训练后在RoboCasa上获得显著迁移增益。

Conclusion: DUST框架有效解决了多模态冲突问题，在模拟和真实环境中均表现出优越性能，展示了大规模VLA预训练的潜力。
...
TL;DR: 提出了一种名为高斯组合距离（GCD）的新相似性度量方法，用于解决小目标检测中IoU和Wasserstein距离的局限性，在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有IoU度量对小目标检测敏感，Wasserstein距离缺乏尺度不变性且优化效率低，需要一种更好的相似性度量方法来提升小目标检测性能。

Method: 提出高斯组合距离（GCD），通过分析GCD及其梯度特性，证明其具有尺度不变性和联合优化能力，可作为边界框回归损失函数和标签分配度量。

Result: 在AI-TOD-v2小目标检测数据集上实现最先进性能，在MS-COCO-2017和Visdrone-2019数据集上也优于Wasserstein距离，验证了GCD的泛化能力。

Conclusion: GCD作为一种新的相似性度量方法，解决了现有方法的局限性，在多种检测器和数据集上表现出优越性能，特别适合小目标检测任务。
...
TL;DR: 提出了一种基于无监督深度学习的去噪框架，用于提高操作显微镜的定量分析能力，通过减少噪声提升分辨率并保持物理保真度。
...
Method: 开发了无监督深度学习去噪框架，结合偏微分方程约束优化，应用于多种显微镜模态和尺度。

Result: 在多种实验应用中，去噪方法显著提升了图像质量：在STXM中揭示了纳米级异质性，在光学显微镜中实现了自动颗粒分割，在中子射线照相中减少了80%的噪声变异性。

Conclusion: 深度去噪是一种强大的、模态无关的增强技术，能够推进定量操作成像并扩展先前受噪声限制技术的应用范围。
...
<details>
  <summary>Details</summary>
Motivation: 现有行人重识别方法在遮挡、视角失真和图像质量差等监控场景下表现不佳，需要复杂模块或仅适用于清晰正面图像。

Method: 基于ViT-Base构建，包含三个核心组件：最终Transformer层的Shuffle模块、场景自适应数据增强、DeiT知识蒸馏技术。
...
TL;DR: Phased DMD是一种多步蒸馏框架，结合了分阶段蒸馏和专家混合的思想，解决了DMD在复杂生成任务中模型容量不足的问题，同时保持了生成多样性。
...
Method: 采用分阶段分布匹配和子区间内分数匹配的方法，将SNR范围划分为子区间，逐步将模型细化到更高的SNR水平，以更好地捕捉复杂分布。

Result: 实验验证了Phased DMD在蒸馏最先进的图像和视频生成模型（包括Qwen-Image和Wan2.2）时，比DMD更好地保持了输出多样性，同时保留了关键的生成能力。

Conclusion: Phased DMD通过分阶段蒸馏和专家混合的方法，有效解决了DMD在复杂生成任务中的局限性，在保持生成多样性的同时提升了模型性能。
...
### [58] [CATArena: Evaluation of LLM Agents through Iterative Tournament Competitions](https://arxiv.org/abs/2510.26852)
*Lingyue Fu,Xin Ding,Yaoming Zhu,Shao Zhang,Lin Qiu,Weiwen Liu,Weinan Zhang,Xuezhi Cao,Xunliang Cai,Jiaxin Ding,Yong Yu*

Main category: cs.AI

TL;DR: 该论文提出了CATArena评估平台，通过四款棋牌游戏来评估LLM智能体的学习能力，解决现有基准测试中分数饱和和依赖专家标注的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体基准测试主要评估固定场景下的端到端性能，存在分数饱和问题，且随着智能体能力提升越来越依赖专家标注。论文强调学习能力是智能体向人类水平智能进化的核心驱动力。

Method: 提出迭代式竞争性同伴学习框架，让智能体通过重复交互和反馈来优化策略；引入CATArena锦标赛式评估平台，包含四款开放式计分的棋牌游戏。
...
### [59] [The Denario project: Deep knowledge AI agents for scientific discovery](https://arxiv.org/abs/2510.26887)
...
TL;DR: Denario是一个AI多智能体科学研究助手系统，能够执行从生成想法到撰写科学论文的全流程研究任务，支持跨学科研究，并在多个科学领域生成论文。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够自动化科学研究的AI助手，解决传统研究过程中的效率问题，促进跨学科研究的融合。

Method: 采用模块化架构设计，包含Cmbagent作为深度研究后端，能够处理特定任务和端到端科学分析，支持多学科交叉研究。

Result: 系统在多个科学领域成功生成论文，包括天体物理学、生物学、化学等，专家评估显示其具备实际应用价值，但也存在局限性。

Conclusion: Denario展示了AI驱动研究的潜力，但需要关注伦理问题和科学哲学影响，系统代码已公开，提供在线演示。
...
### [60] [SUSTAINABLE Platform: Seamless Smart Farming Integration Towards Agronomy Automation](https://arxiv.org/abs/2510.26989)
...
TL;DR: SUSTAINABLE是一个集成了物联网、人工智能、卫星成像和基于角色的任务编排的智能农业平台，旨在通过葡萄种植试点用例实现高效、可追溯和可持续的农业。
...
Method: 整合物联网、人工智能、卫星成像和基于角色的任务编排技术，特别针对地中海葡萄园开发了卫星指数集成、实时环境数据和角色感知任务管理功能。

Result: 提出了SUSTAINABLE平台的关键特性，包括卫星指数集成、实时环境数据监测和角色感知任务管理系统。

Conclusion: SUSTAINABLE平台通过技术创新为农业可持续发展提供了可行的解决方案，特别是在葡萄种植领域展示了其应用潜力。
...
### [61] [Causal Masking on Spatial Data: An Information-Theoretic Case for Learning Spatial Datasets with Unimodal Language Models](https://arxiv.org/abs/2510.27009)
*Jared Junkin,Samuel Nathanson*

Main category: cs.AI

TL;DR: 该研究在象棋领域比较了空间表示和序列表示对语言模型性能的影响，发现即使使用因果掩码，基于空间棋盘状态训练的模型比基于序列数据训练的模型表现更好。


<details>
  <summary>Details</summary>
Motivation: 传统语言模型使用因果掩码，但在具有空间或关系结构的领域，因果掩码常被认为不合适，通常采用序列线性化方法。然而，关于在非序列数据上接受因果掩码引入的信息损失是否可行的问题缺乏直接研究。
...
### [62] [e1: Learning Adaptive Control of Reasoning Effort](https://arxiv.org/abs/2510.27042)
*Michael Kleinman,Matthew Trager,Alessandro Achille,Wei Xia,Stefano Soatto*

Main category: cs.AI

TL;DR: 本文提出自适应努力控制方法，通过强化学习训练模型使用用户指定的相对于当前平均思维链长度的token比例，实现动态控制推理成本与准确性的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要用户预先指定绝对token数量，但用户难以事先知道问题的难度来合理设置token预算。用户需要在输出质量与延迟成本之间进行权衡，但缺乏细粒度控制推理努力的方法。

Method: 提出自适应努力控制方法，使用强化学习训练模型根据用户指定的努力参数（相对于当前平均思维链长度的比例）来动态调整推理token使用量。

Result: 该方法在1.5B到32B参数规模的模型上都有效，能够实现约3倍的思维链长度减少，同时保持或改善相对于RL训练基础模型的性能。
...
### [63] [CombiGraph-Vis: A Curated Multimodal Olympiad Benchmark for Discrete Mathematical Reasoning](https://arxiv.org/abs/2510.27094)
...
TL;DR: 该论文评估了SOTA LLMs在证明评分方面的能力，包括错误检测、严重性判断和分数分配，并提出了基于智能体工作流程的解决方案来改进部分学分分配的校准问题。


<details>
  <summary>Details</summary>
Motivation: 随着SOTA LLMs在奥林匹克数学问题上的表现显著提升，需要评估这些模型在证明评分方面的能力，特别是检测错误、判断严重性和分配公平分数，而不仅仅是二元正确性判断。

Method: 使用90个Gemini 2.5 Pro生成的解决方案作为语料库，采用1-4分制进行评分并详细标注错误；同时使用MathArena的IMO/USAMO 2025解决方案集进行0-7分制评分。引入智能体工作流程来提取和分析参考解决方案，自动推导问题特定的评分标准，实现多步骤评分过程。

Result: 模型能够可靠地标记错误（包括细微错误）的解决方案，但在部分学分分配方面存在校准差距。提出的智能体工作流程在人工评分一致性和部分学分处理一致性方面表现更好。

Conclusion: 智能体工作流程能够显著提高证明评分的准确性和一致性，特别是在部分学分分配方面，为未来研究提供了代码、数据和提示/日志资源。
...
### [64] [Reinforcement Learning for Long-Horizon Unordered Tasks: From Boolean to Coupled Reward Machines](https://arxiv.org/abs/2510.27329)
*Kristina Levina,Nikolaos Pappas,Athanasios Karapantelakis,Aneta Vulgarakis Feljan,Jendrik Seipp*

Main category: cs.AI

TL;DR: 本文提出了三种奖励机器（RM）的泛化形式：数值RM、议程RM和耦合RM，以及新的组合学习算法CoRM，用于解决长视野问题中无序子任务的学习效率问题。


<details>
  <summary>Details</summary>
Motivation: 传统奖励机器在处理长视野问题中无序子任务时，学习信息量会随子任务数量呈指数级增长，限制了其可扩展性。

Method: 提出了三种RM泛化形式：数值RM（紧凑表达复杂任务）、议程RM（跟踪剩余子任务）、耦合RM（子任务状态耦合），并开发了CoRM组合学习算法。

Result: 实验表明CoRM在具有无序子任务的长视野问题上，比现有最先进的RM算法具有更好的扩展性。
...
### [65] [Discriminative Rule Learning for Outcome-Guided Process Model Discovery](https://arxiv.org/abs/2510.27343)
*Ali Norouzifar,Wil van der Aalst*

Main category: cs.AI

TL;DR: 提出了一种基于结果感知的过程发现方法，通过区分期望和不可期望的过程执行，学习可解释的判别规则来分组轨迹，并为每组分别建立过程模型，从而揭示驱动不同结果的关键过程模式。


<details>
  <summary>Details</summary>
Motivation: 传统过程发现方法不考虑执行结果差异，导致模型不适合一致性检查和性能分析，无法捕捉期望与不可期望行为之间的关键差异。区分这两种执行结果可以为过程发现提供更有指导性的方向。

Method: 通过学习控制流特征上的可解释判别规则，将具有相似期望性特征的轨迹分组，然后在每个组内分别应用过程发现技术。

Result: 该方法已实现为公开可用工具，并在多个真实事件日志上进行了评估，证明其能够有效隔离和可视化关键过程模式。

Conclusion: 该方法能够产生聚焦且可解释的模型，揭示驱动期望和不可期望执行的关键因素，为过程改进提供有价值的见解。
...
### [66] [An In-depth Study of LLM Contributions to the Bin Packing Problem](https://arxiv.org/abs/2510.27353)
*Julien Herrmann,Guillaume Pallez*

Main category: cs.AI

TL;DR: 本文重新评估了LLMs在数学发现中的贡献，通过分析LLM生成的装箱问题启发式算法，发现这些算法虽然人类可读但难以理解。作者提出了更简单、高效、可解释的新算法，并质疑LLMs对此问题的实际贡献。


<details>
  <summary>Details</summary>
Motivation: 重新评估LLMs在数学发现中的贡献，特别是针对在线装箱问题的启发式算法，质疑先前关于LLMs提供新见解的论断。

Method: 详细分析LLM生成的启发式算法行为与可解释性，并针对特定装箱问题实例设计新的算法类别。

Result: 发现LLM生成的启发式算法虽然人类可读但难以理解，提出的新算法更简单、高效、可解释且泛化能力更强，表明原问题实例本身相对简单。

Conclusion: LLMs对此问题的贡献被高估，强调在评估LLM生成输出的科学价值时需要严格的验证和背景分析。
...
### [67] [Realistic pedestrian-driver interaction modelling using multi-agent RL with human perceptual-motor constraints](https://arxiv.org/abs/2510.27383)
*Yueyang Wang,Mehmet Dogar,Gustav Markkula*

Main category: cs.AI

TL;DR: 提出一个整合视觉和运动约束的多智能体强化学习框架，用于模拟行人-驾驶员交互，在真实世界无信号人行横道数据集上验证了约束模型的有效性。
...
Conclusion: 带有人类约束的多智能体强化学习是模拟真实道路用户交互的有前景的建模方法，能够在小数据集上有效工作并考虑个体差异。
...
### [68] [DeepCompress: A Dual Reward Strategy for Dynamically Exploring and Compressing Reasoning Chains](https://arxiv.org/abs/2510.27419)
*Tian Liang,Wenxiang Jiao,Zhiwei He,Jiahao Xu,Haitao Mi,Dong Yu*

Main category: cs.AI

TL;DR: DeepCompress是一个新颖框架，通过自适应长度奖励机制同时提升大型推理模型的准确性和效率。它动态地将问题分类为“简单”或“困难”，对简单问题鼓励更短的推理路径，对困难问题促进更长的探索性思维链。
...
Method: DeepCompress采用自适应长度奖励机制，基于模型不断发展的能力实时动态分类问题为“简单”或“困难”。它使用双重奖励策略：对简单问题鼓励更短、更高效的推理，对困难问题促进更长、更具探索性的思维链。

Result: 在具有挑战性的数学基准测试中，DeepCompress始终优于基线方法，实现了更高的准确性，同时显著提高了令牌效率。

Conclusion: DeepCompress框架能够使模型自主调整其思维链长度，压缩已掌握问题的推理过程，延长对具有挑战性问题的推理过程，从而同时提升准确性和效率。
...
### [69] [GeoFM: Enhancing Geometric Reasoning of MLLMs via Synthetic Data Generation through Formal Language](https://arxiv.org/abs/2510.27448)
...
Result: 使用GeoFM合成数据训练的模型在MathVista几何问题解决任务上比GPT-4o高出18.7%，在GeoQA上高出16.5%；相比领先开源模型，在MathVista上高出5.7%，在GeoQA上高出2.7%。
...
### [70] [Mechanics of Learned Reasoning 1: TempoBench, A Benchmark for Interpretable Deconstruction of Reasoning System Performance](https://arxiv.org/abs/2510.27544)
*Nikolaus Holzer,William Fishell,Baishakhi Ray,Mark Santolucito*

Main category: cs.AI

TL;DR: TempoBench是第一个基于形式化基础的诊断基准，通过参数化难度来系统分析LLM的推理能力，包含时间轨迹评估(TTE)和时间因果评估(TCE)两个基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖临时生成的数据集（可能编码偏见且无法验证），要么使用形式化证明系统（如Lean，不适合捕捉基于决策链的任务），这导致了LLM推理基准在推理结构和现实世界对齐方面的不足。

Method: 使用两个评估基准：时间轨迹评估(TTE)测试LLM理解和模拟多步推理系统执行的能力；时间因果评估(TCE)测试LLM执行多步因果推理和从复杂系统中提取因果关系的能力。

Result: 模型在TCE-normal上得分65.6%，在TCE-hard上仅得7.5%，表明最先进的LLM能理解TCE任务，但随着系统复杂性增加表现显著下降。

Conclusion: TempoBench填补了现有LLM推理基准的空白，提供了形式化基础和可验证的诊断工具，揭示了LLM在复杂推理任务中的局限性。
...
TL;DR: SIGMA是一个多智能体检索增强推理框架，通过协调专门智能体进行独立推理、针对性搜索和结果合成，在数学推理任务上显著优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强模型存在单视角、搜索策略僵化、多源信息整合困难等问题，无法有效解决需要多步推理的数学问题。

Method: 引入SIGMA框架，通过专门智能体独立推理并生成假设性段落优化检索，使用调节机制合成各智能体发现，实现上下文敏感且计算高效的知识整合。
...
TL;DR: VeriMoA是一个无需训练的多智能体框架，通过质量引导缓存和多路径生成策略，显著提升了硬件描述语言(HDL)生成的性能，在多个基准测试中实现了15-30%的Pass@1提升。
...
Method: 提出VeriMoA框架，包含两个创新：1) 质量引导缓存机制，维护所有中间HDL输出并进行质量排序选择；2) 多路径生成策略，利用C++和Python作为中间表示，将规范到HDL的转换分解为两阶段过程。

Result: 在VerilogEval 2.0和RTLLM 2.0基准测试中，VeriMoA实现了15-30%的Pass@1提升，使较小模型能够匹配较大模型和微调替代方案，无需昂贵训练。

Conclusion: VeriMoA通过协同创新有效解决了多智能体方法的局限性，为HDL生成提供了高效且无需训练的解决方案。
...
TL;DR: BEAT框架首次在多模态大语言模型（MLLM）驱动的具身智能体中植入视觉后门攻击，使用环境中的物体作为触发条件，当触发物体出现时，智能体会持续执行攻击者指定的多步策略。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型使具身智能体能够直接从视觉输入进行感知、推理和规划任务导向行动，但这种视觉驱动的智能体带来了新的攻击面——视觉后门攻击。

Method: BEAT通过（1）构建涵盖多样化场景、任务和触发位置放置的训练集来暴露智能体面对触发变化；（2）引入两阶段训练方案：先进行监督微调（SFT），然后使用新颖的对比触发学习（CTL），将触发识别建模为存在触发与无触发输入之间的偏好学习。

Result: 在各种具身智能体基准测试和MLLM上，BEAT实现了高达80%的攻击成功率，同时保持强大的良性任务性能，并能可靠地泛化到分布外触发位置。与朴素SFT相比，CTL在有限后门数据下将后门激活准确率提升了39%。
...
TL;DR: 本文提出了Agentic AI的新定义，将其视为一种软件交付机制，强调其成功取决于终端用户和主要利益相关者的验证，而非仅依赖大语言模型。
...
Result: 发现Agentic AI主要是应用程序而非基础模型，其成功依赖于有效的验证措施，在良好验证下，基础模型可被更简单、快速、可解释的模型替代。

Conclusion: Agentic AI的关键在于有效性验证，大语言模型只是实现有效性的一种可能选择，而非必需组件。
...
### [75] [Interaction as Intelligence Part II: Asynchronous Human-Agent Rollout for Long-Horizon Task Training](https://arxiv.org/abs/2510.27630)
...
TL;DR: Apollo是一个集成异步人类指导与动作级数据过滤的采样框架，用于训练LLM智能体处理长时域、领域专业化任务，相比传统方法显著降低了人工标注成本并提高了训练效果。


<details>
  <summary>Details</summary>
Motivation: 当前训练LLM智能体处理长时域、领域专业化任务的方法存在两个主要问题：基于行为克隆的方法需要密集人工标注，成本过高；基于结果驱动的采样方法由于有效正轨迹稀少而容易失败。

Method: Apollo框架结合异步人类指导和动作级数据过滤，允许人类仅在智能体偏离有希望轨迹时进行干预，提供先验知识、策略建议等，然后通过监督控制过滤次优动作以防止错误传播。

Result: 在InnovatorBench上的实验表明，使用Apollo训练的GLM-4.5模型相比未训练基线提升了50%以上，相比无人交互训练的变体提升了28%。

Conclusion: Apollo证明了人机交互采样在处理长时域、领域专业化任务中的关键作用，其设计具有鲁棒性，能够以较低成本产生有价值的轨迹数据。
...
### [76] [Frame Semantic Patterns for Identifying Underreporting of Notifiable Events in Healthcare: The Case of Gender-Based Violence](https://arxiv.org/abs/2510.26969)
...
TL;DR: 提出了一种基于语义框架的方法，用于从电子医疗记录中识别应报告事件，特别是针对性别暴力事件的漏报问题。该方法在巴西葡萄牙语语料库上验证，识别精度达到0.726。


<details>
  <summary>Details</summary>
Motivation: 解决医疗记录中性别暴力事件漏报的问题，通过自动化方法提高公共卫生监测效率。

Method: 使用语义框架定义细粒度模式，在电子医疗记录的非结构化文本字段中搜索这些模式，共定义了8个模式并在2100万句巴西葡萄牙语语料上进行测试。

Result: 方法有效识别暴力报告，精度达到0.726，验证了方法的鲁棒性。

Conclusion: 该方法作为透明、高效、低碳且语言无关的流程，可轻松适应其他健康监测场景，促进NLP在公共卫生系统中的更广泛、伦理和可解释应用。
...
### [77] [Overview of the MEDIQA-OE 2025 Shared Task on Medical Order Extraction from Doctor-Patient Consultations](https://arxiv.org/abs/2510.26974)
...
<details>
  <summary>Details</summary>
Motivation: 当前临床文档自动化主要使用语音识别和摘要技术，但将对话转换为可操作的电子健康记录医疗指令仍未被探索，这能显著减轻临床医生负担并直接影响患者护理。

Method: 引入了MEDIQA-OE 2025共享任务，6个团队参与并尝试了多种方法，包括闭源和开源的大型语言模型。

Result: 共享任务成功举办，参与者探索了广泛的技术方法，论文描述了任务设置、数据集、最终排名和参与者的解决方案。

Conclusion: MEDIQA-OE是医疗对话到指令转换领域的首个挑战，为减少临床文档负担提供了重要研究平台。
...
### [78] [Elastic Architecture Search for Efficient Language Models](https://arxiv.org/abs/2510.27037)
*Shang Wang*

Main category: cs.CL

TL;DR: ELM是一种新颖的神经架构搜索方法，专门为紧凑语言模型优化，通过引入灵活搜索空间和动态模块来提升搜索效率和灵活性，在语言建模任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 随着大型预训练语言模型在自然语言理解任务中变得越来越关键，其巨大的计算和内存需求引发了经济和环境方面的担忧，需要开发更高效的紧凑模型。

Method: ELM扩展了现有NAS方法，引入具有高效transformer块和动态维度及头数调整模块的灵活搜索空间，并提出新颖的知识蒸馏损失来保持每个块的独特特征。
...
### [79] [Dataset Creation and Baseline Models for Sexism Detection in Hausa](https://arxiv.org/abs/2510.27038)
*Fatima Adam Muhammad,Shamsuddeen Muhammad Hassan,Isa Inuwa-Dutse*

Main category: cs.CL

TL;DR: 该研究创建了首个豪萨语性别歧视检测数据集，通过社区参与和定性编码开发，并探索了传统机器学习模型与预训练多语言模型在豪萨语性别歧视检测中的效果。
...
Method: 通过两阶段用户研究（n=66）收集本地说话者数据，结合社区参与、定性编码和数据增强构建数据集，实验使用传统机器学习分类器和预训练多语言语言模型，评估少样本学习效果。

Result: 研究发现捕获文化细微差别存在挑战，特别是在寻求澄清和习语表达方面，这些情况下容易出现许多误报。

Conclusion: 该研究为豪萨语性别歧视检测提供了首个数据集，揭示了在低资源语言中检测性别歧视时文化细微差别的重要性及相关挑战。
...
### [80] [Quantitative Intertextuality from the Digital Humanities Perspective: A Survey](https://arxiv.org/abs/2510.27045)
*Siyu Duan*

Main category: cs.CL

TL;DR: 本文为定量互文性研究提供了路线图，总结了数据、方法和应用，涵盖多语言多主题数据，回顾了从统计到深度学习的方法，并展望了在AI与人文跨学科研究中的更广泛应用前景。


<details>
  <summary>Details</summary>
Motivation: 互文性作为文学理论中的重要概念，随着自然语言处理技术的发展，互文性研究已进入定量时代。本文旨在总结这一新兴领域的现状，为研究者提供系统的研究路线图。

Method: 本文采用综述研究方法，基于多语言多主题的数据，系统回顾了从传统统计方法到深度学习的各种定量互文性研究方法，并总结了相关平台工具。

Result: 研究总结了定量互文性研究的数据来源、方法体系和应用领域，展示了该领域在人文社科研究中的广泛应用价值。

Conclusion: 随着计算机技术的进步，可以预期更精确、多样化和大规模的互文性研究。互文性在连接AI与人文学的跨学科研究中具有广阔的应用前景。
...
### [81] [Recursive numeral systems are highly regular and easy to process](https://arxiv.org/abs/2510.27049)
*Ponrawee Prasertsom,Andrea Silvi,Jennifer Culbertson,Moa Johansson,Devdatt Dubhashi,Kenny Smith*

Main category: cs.CL

TL;DR: 本文认为递归数字系统在规律性和处理复杂度方面更高效，而非仅优化词汇量大小和形态句法复杂度的权衡。通过最小描述长度方法，作者提出了更好的规律性和处理复杂度度量，能够更好地区分自然系统和非自然系统。


<details>
  <summary>Details</summary>
Motivation: 先前研究认为递归数字系统优化了词汇量大小和平均形态句法复杂度之间的权衡，但需要依赖临时约束来排除非自然系统。作者认为这个问题源于忽略了规律性这一人类语法中的关键复杂性方面。

Method: 采用最小描述长度方法，提出了基于MDL的规律性和处理复杂度度量方法，分析递归数字系统的效率。

Result: 基于MDL的规律性和处理复杂度度量能够更好地区分已证实的自然系统与未证实但可能的系统，包括先前研究中的"最优"递归数字系统。先前文献中的临时约束自然地从规律性中推导出来。

Conclusion: 在试图测量和解释语言最优性的研究中，需要将形式集合的规律性纳入考量。
...
### [82] [VISTA Score: Verification In Sequential Turn-based Assessment](https://arxiv.org/abs/2510.27052)
...
Method: VISTA将每个助手回复分解为原子事实声明，根据可信来源和对话历史进行验证，并将不可验证语句分类（主观、矛盾、缺乏证据或弃权）。

Result: 在8个大型语言模型和4个对话事实性基准测试中，VISTA显著优于FACTSCORE和LLM-as-Judge基线方法。人工评估证实VISTA的分解提高了标注者一致性，并揭示了现有基准中的不一致性。
...
### [83] [LLM-Centric RAG with Multi-Granular Indexing and Confidence Constraints](https://arxiv.org/abs/2510.27054)
*Xiaofan Guo,Yaxuan Luan,Yue Kang,Xiangchen Song,Jinxu Guo*

Main category: cs.CL

TL;DR: 提出了一种结合多粒度内存索引和不确定性估计的置信度控制方法，解决复杂知识环境下检索增强生成的覆盖不足、结果不稳定和可靠性有限的问题。
...
### [84] [Detecting Data Contamination in LLMs via In-Context Learning](https://arxiv.org/abs/2510.27055)
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型中训练数据污染的检测和量化问题，特别是对于训练语料未公开的模型，需要一种实用且准确的方法来识别数据记忆情况。

Method: 利用上下文学习对模型性能的影响：上下文示例通常能提升对未见数据集的置信度，但对于训练过的数据集可能因记忆模式被破坏而降低置信度。

Result: CoDeC产生可解释的污染分数，能清晰区分已见和未见数据集，并在训练语料未公开的开源权重模型中揭示了强烈记忆证据。
...
### [85] [Contrastive Knowledge Transfer and Robust Optimization for Secure Alignment of Large Language Models](https://arxiv.org/abs/2510.27077)
*Jiasen Zheng,Huajun Zhang,Xu Yan,Ran Hao,Chong Peng*

Main category: cs.CL

TL;DR: 提出一种结合对比蒸馏和噪声鲁棒训练的微调方法，通过冻结主干模型并转移教师模型知识边界，提升语义一致性和对齐精度，同时引入噪声扰动和鲁棒优化约束确保模型在噪声输入下保持稳定预测。


<details>
  <summary>Details</summary>
Motivation: 解决大规模语言模型在安全对齐和鲁棒性方面的局限性，现有方法在知识转移和噪声抵抗能力上存在不足。
...
### [86] [Characterizing Selective Refusal Bias in Large Language Models](https://arxiv.org/abs/2510.27087)
...
<details>
  <summary>Details</summary>
Motivation: LLM安全护栏旨在防止恶意用户大规模生成有害内容，但这些措施可能无意中引入或反映新的偏见，因为LLM可能拒绝针对某些人口统计群体而非其他群体生成有害内容。

Method: 通过分析针对个人和交叉人口统计群体的拒绝率、LLM响应类型以及生成拒绝的长度来探索选择性拒绝偏见，并通过间接攻击调查额外的安全影响。

Result: 结果显示在性别、性取向、国籍和宗教属性上存在选择性拒绝偏见的证据，这导致了对先前被拒绝群体的间接攻击。

Conclusion: 研究强调了在安全护栏中需要更公平和稳健的性能，以确保对所有人口统计群体的平等保护。
...
### [87] [Rating Roulette: Self-Inconsistency in LLM-As-A-Judge Frameworks](https://arxiv.org/abs/2510.27106)
*Rajarshi Haldar,Julia Hockenmaier*

Main category: cs.CL

TL;DR: 研究发现使用大语言模型（LLM）作为自然语言生成评估工具时存在评分不一致性问题，LLM评判者在不同运行中给出的分数可靠性较低，这种不稳定性影响了评估质量。


<details>
  <summary>Details</summary>
Motivation: 随着自然语言生成技术的广泛应用，传统评估方法（如n-gram或基于嵌入的指标）与人类偏好的一致性较差，而LLM评估方法虽然更接近人类判断，但其评分可靠性尚未得到充分研究。

Method: 通过实验分析LLM评判者在不同NLG任务和基准测试中的评分一致性，量化其评分的不稳定性，并探索在遵循适当指导原则下如何有效使用LLM评判者。

Result: 实验表明LLM评判者在不同运行中给出的评分具有较低的内部评分者信度，这种不一致性使得其判断质量难以准确衡量，在最坏情况下几乎显得随意。

Conclusion: 尽管LLM评判者与人类偏好更一致，但其评分的不稳定性限制了其作为可靠评估工具的实用性，需要在遵循适当指导原则的前提下谨慎使用。
...
### [88] [Probability Distributions Computed by Hard-Attention Transformers](https://arxiv.org/abs/2510.27118)
*Andy Yang,Anej Svete,Jiaoda Li,Anthony Widjaja Lin,Jonathan Rawski,Ryan Cotterell,David Chiang*

Main category: cs.CL

TL;DR: 本文分析了Transformer语言模型在自回归和概率性生成设置下的表达能力，揭示了与作为语言识别器使用时不同的表达特性。


<details>
  <summary>Details</summary>
Motivation: 大多数Transformer表达能力研究将其视为语言识别器（接受或拒绝字符串），但实际应用中它们主要作为语言模型（自回归概率性生成字符串）。本文旨在分析Transformer在语言模型使用场景下的概率分布表达能力。

Method: 通过理论分析，研究将Transformer语言识别器转变为自回归语言模型时的表达能力变化，以及引入概率性后对表达能力的影响。

Result: 研究发现：1）使Transformer语言识别器自回归有时能增加其表达能力；2）引入概率性会打破在非概率情况下成立的等价关系。

Conclusion: 本文区分了Transformer在作为语言模型这一最常见使用场景下能够表达的函数类型，揭示了自回归和概率性设置对表达能力的独特影响。
...
### [89] [Simple Additions, Substantial Gains: Expanding Scripts, Languages, and Lineage Coverage in URIEL+](https://arxiv.org/abs/2510.27183)
*Mason Shipton,York Hay Ng,Aditya Khan,Phuong Hanh Hoang,Xiang Lu,A. Seza Doğruöz,En-Shiun Annie Lee*

Main category: cs.CL

TL;DR: 本文扩展了URIEL+语言知识库，通过引入脚本向量、整合Glottolog语言数据以及扩展谱系插补，显著减少了数据稀疏性，提高了语言覆盖率和插补质量，在跨语言迁移任务中展现出性能提升。
...
### [90] [Identifying the Periodicity of Information in Natural Language](https://arxiv.org/abs/2510.27241)
...
<details>
  <summary>Details</summary>
Motivation: 信息密度理论的最新进展引发了对自然语言中信息编码周期性程度的疑问，需要开发新方法来探索语言中是否存在周期性信息模式。
...
Conclusion: 语言中信息的周期性是结构化因素和长距离驱动因素共同作用的结果；该方法在LLM生成检测方面具有潜在应用价值。
...
TL;DR: 本文提出了BEAM基准测试和LIGHT框架，用于评估和改进大语言模型在长对话中的记忆能力。BEAM包含100个长对话和2000个问题，LIGHT通过三个互补的记忆系统提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏叙事连贯性、覆盖领域狭窄，只测试简单的回忆任务，无法充分评估大语言模型在需要长期记忆的长上下文推理任务中的能力。

Method: 1) 提出自动生成长达1000万token的连贯对话框架，构建BEAM基准测试；2) 提出LIGHT框架，为大语言模型配备三个记忆系统：长期情景记忆、短期工作记忆和重要事实记录板。

Result: 实验显示，即使具有100万token上下文窗口的大语言模型（无论是否使用检索增强）在对话变长时表现都会下降。相比之下，LIGHT在不同模型上都能持续提升性能，相比最强基线平均提升3.5%-12.69%。消融研究证实了每个记忆组件的贡献。

Conclusion: LIGHT框架通过模拟人类认知的多记忆系统，有效提升了大语言模型在长对话中的记忆能力，为长上下文推理任务提供了实用解决方案。
...
### [92] [Languages are Modalities: Cross-Lingual Alignment via Encoder Injection](https://arxiv.org/abs/2510.27254)
*Rajan Agarwal,Aarush Gupta*

Main category: cs.CL

TL;DR: LLINK是一种计算高效的方法，通过将多语言编码器的句子嵌入对齐到解码器的潜在嵌入空间，并使用最小适配器让冻结的解码器处理信号，显著提升了低资源非拉丁文字语言在指令调优大语言模型上的性能。


<details>
  <summary>Details</summary>
Motivation: 指令调优的大语言模型在低资源非拉丁文字语言上表现不佳，主要原因是分词器碎片化和弱跨语言耦合。

Method: 首先通过轻量级对比投影器将冻结多语言编码器的句子嵌入对齐到解码器潜在嵌入空间的保留位置；然后将该向量扩展为K个软槽，并使用最小适配器进行训练，使冻结的解码器能够处理信号。
...
### [93] [MedCalc-Eval and MedCalc-Env: Advancing Medical Calculation Capabilities of Large Language Models](https://arxiv.org/abs/2510.27267)
*Kangkun Mao,Jinru Ding,Jiayuan Chen,Mouxiao Bian,Ruiyao Chen,Xinwei Peng,Sijie Ren,Linyang Li,Jie Xu*

Main category: cs.CL

TL;DR: 该论文提出了MedCalc-Eval，这是评估大型语言模型医疗计算能力的最大基准，包含700多个任务，涵盖方程式和规则评分系统。同时开发了MedCalc-Env强化学习环境来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有医疗领域基准主要关注问答和描述性推理，忽视了临床决策中至关重要的定量推理能力。现有数据集如MedCalc-Bench覆盖的计算任务有限，无法反映真实世界的计算场景。

Method: 构建包含700多个任务的MedCalc-Eval基准，分为方程式计算和规则评分系统两类，涵盖多个医学专科。开发基于InternBootcamp框架的MedCalc-Env强化学习环境，支持多步骤临床推理和规划。
...
### [94] [Why Do Multilingual Reasoning Gaps Emerge in Reasoning Language Models?](https://arxiv.org/abs/2510.27269)
*Deokhyung Kang,Seonjeong Hwang,Daehui Kim,Hyounghun Kim,Gary Geunbae Lee*

Main category: cs.CL

TL;DR: 研究发现多语言推理差距主要由语言理解失败引起，提出了选择性翻译策略来检测和缓解这一问题，仅需翻译约20%的输入即可达到接近全翻译的性能。
...
Method: 通过评估多种检测方法来识别语言理解失败，提出选择性翻译策略，仅在检测到理解失败时将多语言输入翻译为英语。

Result: 理解失败确实可以被识别，监督方法表现最佳。选择性翻译策略显著缩小了多语言推理差距，仅需翻译约20%的输入就能达到接近全翻译的性能。
...
### [95] [A Unified Representation Underlying the Judgment of Large Language Models](https://arxiv.org/abs/2510.27328)
*Yi-Long Lu,Jiajun Song,Wei Wang*

Main category: cs.CL

TL;DR: 研究发现大型语言模型中的不同评价判断都沿着一个主导维度计算，称为价-同意轴(VAA)，该轴同时编码主观价值和事实主张的同意。这个统一表征导致推理过程从公正推理转向目标导向的合理化，解释了系统性偏见和幻觉的机制。


<details>
  <summary>Details</summary>
Motivation: 探索生物和人工智能中判断是依赖专门模块还是统一领域通用资源的问题，检验LLMs中可解码神经表征是否真正独立系统。

Method: 通过一系列LLMs实验，识别评价判断的主导维度，并进行直接干预来验证VAA作为控制信号的功能。

Result: 发现VAA作为统一表征创造了关键依赖：它作为控制信号引导生成过程构建与其评价状态一致的合理化，即使牺牲事实准确性。

Conclusion: 研究揭示了促进连贯判断的架构如何系统性地破坏忠实推理，为系统性偏见和幻觉提供了机制性解释。
...
### [96] [TransAlign: Machine Translation Encoders are Strong Word Aligners, Too](https://arxiv.org/abs/2510.27337)
...
<details>
  <summary>Details</summary>
Motivation: 大多数世界语言缺乏大规模训练数据，基于翻译的跨语言迁移策略需要有效的标签投影方法，而现有基于机器翻译模型的词对齐方法性能不佳。
...
Conclusion: TransAlign为跨语言迁移中的标签投影提供了更有效的解决方案，特别是在标记分类任务中表现优异。
...
### [97] [ThoughtProbe: Classifier-Guided LLM Thought Space Exploration via Probing Representations](https://arxiv.org/abs/2510.27355)
*Zijian Wang,Chang Xu*

Main category: cs.CL

TL;DR: ThoughtProbe是一个新的推理时框架，利用LLM的隐藏推理特征来提升推理性能，通过分类器评分和分支聚合方法在树状响应空间中高效探索。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要操纵隐藏表示来引导LLM生成，而本文利用这些表示作为判别信号来指导树状响应空间探索，以提高推理性能。

Method: 在节点扩展时使用分类器作为评分和排序机制，优先扩展高分候选；完成树扩展后收集所有分支答案形成候选池，通过聚合CoT分数来识别最优答案。
...
### [98] [Balancing Knowledge Updates: Toward Unified Modular Editing in LLMs](https://arxiv.org/abs/2510.27400)
...
<details>
  <summary>Details</summary>
Motivation: 现有知识编辑方法主要关注MLP模块权重，而忽略了注意力模块在事实知识存储和检索中的重要作用，导致残留过时知识和编辑效果受限。

Method: 基于知识定位实验发现，提出IntAttn-Edit方法，将关联记忆范式扩展到联合更新MLP和注意力模块，采用知识平衡策略按模块贡献比例分配更新幅度。

Result: 在标准基准测试中，IntAttn-Edit相比先前方法实现了更高的编辑成功率、更好的泛化能力和更强的知识保留能力。

Conclusion: 注意力模块在事实知识存储中发挥重要作用，联合编辑MLP和注意力模块能显著提升知识编辑效果，平衡策略能在不同设置下保持最佳编辑性能。
...
### [99] [Awal -- Community-Powered Language Technology for Tamazight](https://arxiv.org/abs/2510.27407)
*Alp Öktem,Farida Boudichat*

Main category: cs.CL

TL;DR: Awal是一个社区驱动的塔马齐特语语言技术资源开发项目，通过awaldigital.org平台收集翻译和语音数据，18个月内获得6421个翻译对和3小时语音数据，但参与度主要集中在语言学家和活动家群体。


<details>
  <summary>Details</summary>
Motivation: 解决塔马齐特语在数字空间中代表性不足的问题，应对数据稀缺挑战，通过社区协作方式开发语言技术资源。

Method: 建立协作平台awaldigital.org，让塔马齐特语使用者贡献翻译和语音数据，分析18个月的社区参与情况。

Result: 收集到6421个翻译对和3小时语音数据，发现参与障碍包括书面塔马齐特语信心不足和标准化挑战，实际贡献主要集中在语言学家和活动家群体。
...
### [100] [VCORE: Variance-Controlled Optimization-based Reweighting for Chain-of-Thought Supervision](https://arxiv.org/abs/2510.27462)
*Xuan Gong,Senmiao Wang,Hanbo Huang,Ruoyu Sun,Shiyu Liang*

Main category: cs.CL

TL;DR: VCORE是一种基于优化理论的重加权框架，通过方差控制优化方法对CoT轨迹中的token进行差异化监督，解决了标准交叉熵损失对所有token平等处理的问题，显著提升了LLMs在数学和编程任务中的推理性能。


<details>
  <summary>Details</summary>
Motivation: 标准交叉熵损失在长链式思维监督微调中平等对待所有token，忽略了它们在推理轨迹中的异质性贡献，导致监督分配不当和泛化能力弱，特别是在复杂的长形式推理任务中。

Method: 提出VCORE框架，将CoT监督重新表述为约束优化问题，采用优化理论视角实现跨token的原则性和自适应监督分配，使训练目标更贴近稳健推理泛化的目标。

Result: 在Qwen3系列和LLaMA-3.1-8B-Instruct模型上的评估表明，VCORE在数学和编程基准测试中显著优于现有token重加权方法，在域内和域外设置下均取得实质性性能提升。

Conclusion: VCORE不仅提升了推理性能，还为后续强化学习提供了更有效的初始化，为推进LLMs推理能力建立了更强的基础。
...
### [101] [Detecting Prefix Bias in LLM-based Reward Models](https://arxiv.org/abs/2505.13487)
*Ashwin Kumar,Yuzi He,Aram H. Markosyan,Bobbie Chern,Imanol Arrieta-Ibarra*

Main category: cs.CL

TL;DR: 本研究提出检测和评估LLM奖励模型中前缀偏见的新方法，揭示了基于人类反馈的强化学习在种族和性别维度上的显著偏见，并提出数据增强策略来缓解这些偏见。


<details>
  <summary>Details</summary>
Motivation: 虽然公开可用的偏好数据集提供了响应对的比较，但由此产生的奖励模型中的偏见潜力仍未得到充分探索。本研究旨在检测和评估由查询前缀微小变化引发的模型偏好系统性偏移。

Method: 引入检测和评估前缀偏见的新方法；利用这些指标揭示偏好模型在种族和性别维度上的显著偏见；提出数据增强策略来缓解偏见。

Result: 在多样开源偏好数据集和奖励模型架构上的综合评估显示，无论底层模型架构如何，都存在对这种偏见的易感性；提出的数据增强策略有效减少了前缀偏见的影响。

Conclusion: 研究结果强调了在开发公平可靠奖励模型时，需要进行偏见感知的数据集设计和评估，为AI公平性的广泛讨论做出贡献。
...
### [102] [Diffuse Thinking: Exploring Diffusion Language Models as Efficient Thought Proposers for Reasoning](https://arxiv.org/abs/2510.27469)
*Chenyang Shao,Sijian Ren,Fengli Xu,Yong Li*

Main category: cs.CL

TL;DR: 提出了一种高效的协作推理框架，利用扩散语言模型生成候选思维，大语言模型评估其质量，以解决传统方法计算开销大、性能提升有限的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的自回归生成范式导致推理性能随测试时间计算呈次优扩展，往往需要过多计算开销来提出思维，而性能提升有限。扩散语言模型能够通过单次前向传播并行去噪高效生成多样样本，这启发了我们利用它们来提出中间思维，从而减轻自回归生成的计算负担。

Method: 提出高效的协作推理框架，利用扩散语言模型生成候选思维，大语言模型评估其质量。通过这种分工协作的方式，在保持质量的同时显著降低计算开销。

Result: 在多样化基准测试上的实验表明，该框架在复杂推理任务中实现了强大的性能。

Conclusion: 该工作为未来研究提供了一个有前景的方向，展示了扩散语言模型和大语言模型协作推理的有效性。
...
### [103] [The aftermath of compounds: Investigating Compounds and their Semantic Representations](https://arxiv.org/abs/2510.27477)
*Swarang Joshi*

Main category: cs.CL

TL;DR: 本研究比较了GloVe和BERT嵌入在英语复合词处理中与人类语义判断的一致性，发现BERT在捕捉组合语义方面优于GloVe，且可预测性评分是语义透明度的强预测因子。


<details>
  <summary>Details</summary>
Motivation: 研究计算嵌入与人类语义判断在处理英语复合词时的对齐程度，旨在推进计算心理语言学发展。

Method: 使用静态词向量(GloVe)和上下文嵌入(BERT)与人类对词素意义支配性(LMD)和语义透明度(ST)的评分进行比较，通过关联强度、频率和可预测性指标计算嵌入衍生的LMD和ST指标，并通过Spearman相关和回归分析评估其与人类判断的关系。
...
### [104] [Effect of Domain Generalization Techniques in Low Resource Systems](https://arxiv.org/abs/2510.27512)
*Mahi Aminu,Chisom Chibuike,Fatimo Adebanjo,Omokolade Awosanya,Samuel Oyeneye*

Main category: cs.CL

TL;DR: 本文研究了两种因果域泛化方法在低资源自然语言任务中的应用：因果数据增强(CDA)通过生成反事实样本来提升对虚假相关性的鲁棒性；不变因果表示学习(ICRL)使用DINER框架学习跨域不变表示。两种方法都能有效提升模型在未见域上的性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中训练和测试数据分布往往不同，特别是在低资源设置下，数据稀缺和领域多样性有限严重阻碍了模型的鲁棒泛化能力。需要开发能够应对分布偏移的域泛化方法。

Method: 1. 因果数据增强(CDA)：自动生成反事实示例，在NaijaSenti Twitter语料库上进行情感分类，通过语义等价改写来模拟受控分布偏移
2. 不变因果表示学习(ICRL)：使用DINER框架，将其适应到多语言设置中学习跨域不变表示

Result: 两种方法都增强了模型对未见域的鲁棒性：反事实数据增强在情感分类中带来一致的跨域准确率提升；基于DINER的因果表示学习在多语言情感分析中改善了分布外性能，尽管在不同语言间收益有所差异。

Conclusion: 因果域泛化方法在低资源自然语言处理任务中具有实际价值，能够有效提升模型在分布偏移下的鲁棒性，为处理现实世界中的分布不匹配问题提供了有前景的解决方案。
...
### [105] [BiSparse-AAS: Bilinear Sparse Attention and Adaptive Spans Framework for Scalable and Efficient Text Summarization](https://arxiv.org/abs/2510.27516)
*Desta Haileselassie Hagos,Legand L. Burge,Anietie Andy,Anis Yazidi,Vladimir Vlassov*

Main category: cs.CL

TL;DR: BiSparse-AAS是一个结合稀疏注意力、自适应跨度和双线性注意力的新型Transformer框架，旨在解决长文档文本摘要中二次计算复杂度的问题，在多个数据集上显著提升性能。
...
Method: 提出BiSparse-AAS框架，结合稀疏注意力（降低计算成本）、自适应跨度（动态调整注意力范围）和双线性注意力（在精炼上下文中建模复杂token交互）。

Result: 在抽取式和生成式摘要任务中持续优于最先进基线，在CNN/DailyMail上平均ROUGE提升约68.1%，在XSum上提升约52.6%，在OpenWebText和Gigaword数据集上保持强劲性能。
...
### [106] [SQLSpace: A Representation Space for Text-to-SQL to Discover and Mitigate Robustness Gaps](https://arxiv.org/abs/2510.27532)
*Neha Srikanth,Victor Bursztyn,Puneet Mathur,Ani Nenkova*

Main category: cs.CL

TL;DR: SQLSpace是一种人类可解释、可泛化、紧凑的文本到SQL示例表示方法，通过最小人工干预获得，用于评估文本到SQL模型的性能分析。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到SQL基准测试评估通常只关注整体准确率，缺乏对模型性能的细粒度分析，难以识别不同基准测试的独特维度，也无法深入理解模型在特定类型查询上的表现。

Method: 开发SQLSpace表示方法，通过最小人工干预从文本到SQL示例中提取人类可解释的特征表示，支持三种应用场景：基准测试对比分析、细粒度性能评估和基于正确性估计的查询重写。

Result: SQLSpace能够揭示不同基准测试之间的组成差异，暴露仅靠准确率无法发现的性能模式，并支持查询成功率的建模，从而改进模型性能。

Conclusion: SQLSpace为文本到SQL任务提供了有效的分析工具，能够进行更深入的性能评估和模型改进，比单独使用原始示例更有效地支持模型分析和优化。
...
### [107] [Patient-Centered Summarization Framework for AI Clinical Summarization: A Mixed-Methods Design](https://arxiv.org/abs/2510.27535)
...
TL;DR: 该研究提出了患者中心摘要（PCS）作为AI临床摘要任务的新标准，旨在捕捉患者价值观并确保临床实用性。通过混合方法开发框架，评估开源LLM在此任务中是否能达到人类水平表现。


<details>
  <summary>Details</summary>
Motivation: 当前LLM生成的临床摘要往往关注患者生物学信息而非其偏好、价值观、愿望和关切，为实现以患者为中心的护理，需要开发能够捕捉患者价值观的临床摘要标准。

Method: 采用混合方法：通过患者和临床医生参与半结构化访谈确定摘要内容和结构；制定标注指南；使用8名临床医生为88个房颤咨询创建黄金标准PCS；使用5个开源LLM进行零样本和少样本提示生成摘要；通过ROUGE-L、BERTScore和定性指标评估。

Result: 患者强调生活方式、社会支持、近期压力和护理价值观；临床医生需要简洁的功能、心理社会和情感背景。最佳零样本表现由Mistral-8B（ROUGE-L 0.189）和Llama-3.1-8B（BERTScore 0.673）实现；最佳少样本由Llama-3.1-8B（ROUGE-L 0.206，BERTScore 0.683）实现。完整性和流畅性在专家和模型间相似，而正确性和患者中心性偏向人类PCS。

Conclusion: 开源LLM在生成患者中心摘要方面显示出潜力，但在正确性和患者中心性方面仍需改进以达到人类水平表现，为AI支持的以患者为中心的护理提供了基础。
...
### [108] [DialectalArabicMMLU: Benchmarking Dialectal Capabilities in Arabic and Multilingual Language Models](https://arxiv.org/abs/2510.27543)
...
<details>
  <summary>Details</summary>
Motivation: 现有的阿拉伯语和多语言基准测试主要关注现代标准阿拉伯语(MSA)，但日常交流中广泛使用的方言变体在评估中代表性不足。

Method: 基于MMLU-Redux框架，通过人工翻译和改编3K个多项选择题答案对到五种主要方言(叙利亚、埃及、阿联酋、沙特和摩洛哥)，总共生成15K个问答对。
...
### [109] [Multilingual BERT language model for medical tasks: Evaluation on domain-specific adaptation and cross-linguality](https://arxiv.org/abs/2510.27552)
...
TL;DR: 该研究探讨了在荷兰语、罗马尼亚语和西班牙语三种低资源语言中，通过领域特定语料进行进一步预训练对医疗NLP任务性能的影响。研究发现领域适应显著提升了任务表现，临床领域适应的模型优于一般生物医学领域适应的模型，并观察到跨语言可转移性。


<details>
  <summary>Details</summary>
Motivation: 在多语言医疗应用中，领域特定的自然语言处理工具有限，特别是对于低资源语言。虽然多语言BERT为缩小语言差距提供了有前景的方法，但低资源语言的医疗NLP任务仍未充分探索。

Method: 研究进行了四个实验来创建医疗领域模型，包括进一步预训练。然后将这些模型在三个下游任务上进行微调：荷兰语临床笔记中的自动患者筛查、罗马尼亚语和西班牙语临床笔记中的命名实体识别。

Result: 结果显示领域适应显著增强了任务性能。临床领域适应的模型表现优于更一般的生物医学领域适应的模型。此外，观察到了跨语言可转移性的证据。
...
### [110] [Data-Efficient Domain Adaptation for LLM-based MT using Contrastive Preference Optimization](https://arxiv.org/abs/2510.27556)
...
### [111] [MARAG-R1: Beyond Single Retriever via Reinforcement-Learned Multi-Tool Agentic Retrieval](https://arxiv.org/abs/2510.27569)
...
Method: 提出MARAG-R1框架，配备四种检索工具（语义搜索、关键词搜索、过滤和聚合），通过两阶段训练（监督微调+强化学习）学习如何和何时使用这些工具，实现推理和检索的交错进行。

Result: 在GlobalQA、HotpotQA和2WikiMultiHopQA上的实验表明，MARAG-R1显著优于强基线方法，在语料库级推理任务中达到了新的最先进水平。

Conclusion: MARAG-R1通过多工具动态协调机制有效解决了传统RAG系统的信息获取瓶颈，为语料库级推理任务提供了更全面和精确的信息访问能力。
...
### [112] [SpecAttn: Speculating Sparse Attention](https://arxiv.org/abs/2510.27641)
*Harsh Shah*

Main category: cs.CL

TL;DR: SpecAttn是一种无需训练的方法，通过利用推测解码中已计算的注意力权重来识别重要token，实现预训练transformer中的高效稀疏注意力，减少75%的KV缓存访问，仅增加15.29%的困惑度。
...
Conclusion: 推测执行可以增强为提供近似验证而不会显著降低性能，证明了利用推测解码中已有计算工作的有效性。
...
### [113] [Continuous Autoregressive Language Models](https://arxiv.org/abs/2510.27688)
*Chenze Shao,Darren Li,Fandong Meng,Jie Zhou*

Main category: cs.CL

TL;DR: CALM通过将语言建模从离散的下一词预测转变为连续的下一向量预测，使用高保真自动编码器压缩多个token为单个连续向量，显著减少生成步骤，提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的效率受限于其顺序、逐词生成的流程，需要新的扩展设计维度来提高每个生成步骤的语义带宽。

Method: 引入连续自回归语言模型（CALM），使用高保真自动编码器将K个token压缩为单个连续向量，并开发了无似然框架进行训练、评估和可控采样。
