...
TL;DR: 该研究通过构建高质量多模态数据集和优化推理语法，仅用监督微调就在标准化考试基准上达到接近最先进模型的性能，证明了数据质量和表示语法在多模态推理中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 多模态推理已成为现代AI研究的基石，标准化考试问题为此提供了严格测试平台。虽然近期研究主要关注算法进步（如强化学习），但视觉语言推理的数据中心基础仍未充分探索。本研究旨在证明高质量数据下的监督微调可以媲美专有方法。

Method: 1. 构建包含1.614亿token的多模态数据集，整合教科书问答对、课程对齐图表和上下文材料；2. 使用优化的推理语法（QMSA）对Qwen-2.5VL-32B模型进行监督微调；3. 在新发布的YKSUniform基准上评估，该基准标准化了309个课程主题下的1,854个多模态考试问题。

Result: 微调后的模型在YKSUniform基准上达到78.6%的准确率，仅比Gemini 2.0 Flash低1.0%。这表明数据组合和表示语法在多模态推理中起决定性作用。

Conclusion: 本研究建立了推进开放权重视觉语言模型的数据中心框架，证明精心策划和基于课程的多模态数据可以将监督微调提升到接近最先进的性能水平。数据质量和表示语法是多模态推理成功的关键因素。
...
TL;DR: PEFT-DML是一个参数高效的深度度量学习框架，用于自动驾驶中的鲁棒多模态3D目标检测，能够在传感器丢失或未知模态组合情况下保持可靠检测。


<details>
  <summary>Details</summary>
Motivation: 传统多模态3D检测模型假设传感器固定可用，但在实际自动驾驶场景中常面临传感器丢失、天气变化、快速运动和领域偏移等挑战，需要更鲁棒的解决方案。

Method: 提出PEFT-DML框架，将多种模态（LiDAR、雷达、相机、IMU、GNSS）映射到共享潜在空间，集成低秩适应（LoRA）和适配器层实现参数高效训练。

Result: 在nuScenes基准测试中展示了优越的准确率，显著提升了训练效率，同时增强了对抗快速运动、天气变化和领域偏移的鲁棒性。

Conclusion: PEFT-DML为自动驾驶中的多模态3D目标检测提供了参数高效且鲁棒的解决方案，特别适用于传感器动态变化和未知模态组合的实际场景。
...
TL;DR: 提出一种名为Satellite-to-Street的深度学习框架，通过联合处理灾前灾后卫星图像生成精细像素级损害地图，改进传统分割和变化检测模型在结构细微变化和类别不平衡方面的不足。


<details>
  <summary>Details</summary>
Motivation: 传统人工解释卫星图像进行灾后损害评估存在速度慢、主观性强、难以规模化的问题，而现有的深度学习分割模型（如U-Net）和变化检测模型在处理细微结构变化和严重类别不平衡时表现不佳，特别是对严重受损区域的检测效果差。
...
Result: 在公开可用的灾害数据集上进行实验，相比传统分割和基线变化检测模型，在结构损害的定位和分类方面表现出改进。生成的损害地图能够提供快速一致的评估机制。

Conclusion: 该模型为灾害管理提供了高效、数据驱动的支持工具，能够辅助而非替代专家决策，实现更有效的灾害响应和资源分配。
...
TL;DR: 本文提出ProvRain管道，使用轻量级MobileNet-U-Net架构结合课程训练，提升夜间雨天条件下的车辆检测性能，相比基准模型准确率提升8.94%，召回率提升10.25%。


<details>
  <summary>Details</summary>
Motivation: 夜间车辆检测面临天气条件（如雨雪）和相机条件造成的噪声问题，这些噪声会干扰基于车灯特征的前瞻性车辆检测。需要一种既能处理噪声又能保持检测准确性的解决方案。

Method: 提出ProvRain管道，采用轻量级MobileNet-U-Net架构，通过课程训练概念增强对恶劣天气条件的鲁棒性。使用合成数据和PVDN数据集进行训练，并与基于PVDN数据集训练的Faster RCNN基准架构进行比较。

Result: 在雨天夜间帧的车辆检测中，系统准确率提升8.94%，召回率提升10.25%。MobileNet-U-Net架构相比其他Transformer方法，PSNR提升10-15%，SSIM提升5-6%，感知误差（LPIPS）降低高达67%。

Conclusion: ProvRain管道通过结合去噪架构有效提升了夜间雨天条件下的车辆检测性能，证明了课程训练和轻量级架构在处理恶劣天气噪声方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 随着扩散模型的快速发展，零样本图像生成技术能够在无需修改模型权重的情况下，仅使用一张肖像或艺术品就能实现高保真度的身份复制或风格模仿。虽然这些技术显著增强了创作可能性，但也带来了与知识产权侵权相关的重大风险，包括未经授权的身份克隆和风格模仿。需要一种防御机制来保护个人图像免遭此类滥用。

Method: 首先研究当前零样本方法如何通过图像编码器从输入图像中提取嵌入向量，并通过交叉注意力层输入到扩散模型的UNet中。基于此机制构建可逆加密系统，将原始嵌入映射为根据不同密钥生成的加密表示。授权用户可通过解密模块和正确密钥恢复原始嵌入，实现正常的授权生成任务。为保护目的，设计多目标对抗扰动方法，主动将原始嵌入向指定的加密模式偏移，使受保护图像嵌入防御层，确保未经授权用户只能产生扭曲或加密的输出。

Result: 广泛评估表明，该方法在阻止未经授权的零样本图像合成方面超越了现有最先进的防御方法，同时支持灵活且安全的访问控制，供已验证用户使用。

Conclusion: Adapter Shield是首个针对零样本图像生成场景的通用认证集成防御方案，通过可逆加密和多目标对抗扰动技术，有效保护个人图像免遭未经授权的身份克隆和风格模仿，同时为授权用户提供正常的生成功能。
...
<details>
  <summary>Details</summary>
Motivation: 明场显微镜图像中的精确单细胞检测对生物学研究至关重要，但数据稀缺和标注瓶颈限制了深度学习方法的发展。需要探索合成数据生成来缓解这些问题。

Method: 训练基于U-Net的扩散模型生成合成明场显微镜图像，创建不同合成与真实图像比例的数据集，使用YOLOv8、YOLOv9和RT-DETR进行目标检测实验，并通过专家调查评估图像真实性。

Result: 使用合成数据训练可以显著提高检测精度（以最小成本实现）。专家调查显示生成图像具有高度真实性，专家无法区分合成与真实图像（准确率50%）。
...
TL;DR: MARWIN机器人在欧洲XFEL中使用深度视觉立体里程计替代现有导航系统，以提高在单调加速器隧道中的自主导航能力


<details>
  <summary>Details</summary>
Motivation: 现有导航系统结合激光雷达边缘检测、轮式/激光雷达里程计和QR码参考，在预定义区域表现稳健，但缺乏对未知几何形状和障碍物的灵活性，需要更自主的导航方案

Method: 采用深度视觉立体里程计，利用立体视差、光流和自监督学习联合估计深度和自我运动，无需标记数据，并可融合绝对参考或其他传感器实现全局一致性

Result: 概念性评估显示DVSO在加速器隧道环境中具有潜力，预期优势包括通过立体视觉减少尺度漂移、低成本传感和可扩展数据收集

Conclusion: 深度视觉立体里程计为MARWIN在受限、安全关键基础设施中实现更自主导航提供了研究方向，但仍需解决低纹理表面、光照变化、计算负载和辐射鲁棒性等挑战
...
TL;DR: 诊断提示能提升多模态大语言模型评估亚马逊搜索结果页面视觉复杂度的可靠性，但性能仍有提升空间


<details>
  <summary>Details</summary>
Motivation: 研究诊断提示是否能改善多模态大语言模型在评估亚马逊搜索结果页面视觉复杂度方面的可靠性，并与基于格式塔原理的标准提示方法进行比较

Method: 使用200个亚马逊搜索结果页面和人类专家标注，比较诊断提示与基于格式塔原理的标准提示方法，分析决策树揭示的模型推理模式

Result: 诊断提示显著提升了预测人类复杂度判断的能力，F1分数从0.031提升到0.297（相对提升858%），但绝对性能仍有限（Cohen's κ = 0.071）。模型更关注视觉设计元素（徽章杂乱度：38.6%重要性），而人类更强调内容相似性

Conclusion: 诊断提示是迈向人类对齐的多模态大语言模型评估的有希望的第一步，但在产品相似性和颜色强度评估方面仍存在挑战，需要更大的真实数据集和提示方法改进才能实现可靠的实用部署
...
TL;DR: FastTextDiff是一种基于扩散模型的标签高效医学图像分割方法，通过集成医学文本标注来增强语义表示，使用ModernBERT替代Clinical BioBERT，提高了分割精度和训练效率。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要密集的像素级标注，这些标注昂贵、耗时且需要专业知识。现有扩散模型在医学图像生成和去噪方面有效，但在分割任务中受到标注限制。

Method: 提出FastTextDiff模型，集成医学文本标注增强语义表示。使用ModernBERT处理长临床笔记，通过跨模态注意力机制紧密链接文本标注与医学图像的语义内容。模型在MIMIC-III和MIMIC-IV上训练，利用FlashAttention 2、交替注意力机制和2万亿token语料库。

Result: ModernBERT作为Clinical BioBERT的快速、可扩展替代方案，在基于扩散的分割流程中表现良好。FastTextDiff相比传统基于扩散的模型，在分割精度和训练效率方面都有提升。

Conclusion: 该研究验证了ModernBERT在基于扩散的分割流程中的有效性，并突出了多模态技术在医学图像分析中的潜力。通过集成文本标注，实现了标签高效的分割方法。
...
TL;DR: 本文提出了一种用于超低功耗物联网平台的多模态设备端学习技术，通过稀疏更新方案在MCU上实现单目深度估计模型的在线微调，显著提升了模型在新环境中的精度。
...
Method: 1) 在GAP9 MCU上部署多模态设备端学习系统，包含80mW单目相机和8×8像素深度传感器；2) 正常运行时使用107k参数的μPyD-Net模型进行单目图像推理；3) 在新环境中激活深度传感器收集伪标签数据；4) 提出内存驱动的稀疏更新方案，将微调内存降至1.2MB（比完整更新减少2.2倍）；5) 完全在MCU上执行微调任务。

Result: 1) 在KITTI和NYUv2数据集上精度仅下降2%和1.5%；2) 现场测试显示，物联网节点上17.8分钟即可完成设备端学习；3) 使用仅3k自标记样本，均方根误差从4.9m降至0.6m；4) 整个系统功耗约300mW。

Conclusion: 该研究首次证明了在物联网节点上实现单目深度估计的设备端学习的可行性，通过稀疏更新方案有效解决了资源受限环境下的域偏移问题，显著提升了模型在实际部署场景中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统课堂观察依赖人工标注，资源密集且难以规模化，需要开发自动化分析系统为教师提供可扩展的反馈。

Method: 使用164小时视频和68节课转录的密集标注数据集，设计并行模态特定管道：视频方面评估零样本多模态LLM、微调视觉语言模型和自监督视频变换器；转录方面微调基于变换器的分类器，并应用针对类别不平衡和多标签复杂性的处理技术。

Result: 微调模型在视频分析上获得0.577的宏F1分数，在转录分析上获得0.460的宏F1分数，均优于基于提示的方法，证明了自动化课堂分析的可行性。

Conclusion: 该研究展示了AI驱动课堂分析的可行性，为可扩展的教师反馈系统奠定了基础，微调方法在多模态教学活动和话语识别方面表现优于提示方法。
...
TL;DR: TeleViT模型通过融合局部火灾驱动因素、全球粗化场和遥相关指数，实现了长达4个月的野火预测，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 提前数周至数月预测野火对于燃料处理和资源分配至关重要，但长期预测需要考虑地球系统的相互关联性，包括全球模式和遥相关。

Method: 提出TeleViT模型，采用非对称标记化策略整合(i)细尺度局部火灾驱动因素、(ii)粗化全球场、(iii)遥相关指数，通过Transformer编码器处理异质标记，解码器保持空间结构。

Result: 使用SeasFire数据集(2001-2021)，TeleViT在所有预测时间范围内(包括4个月)的AUPRC性能优于U-Net++、ViT和气候学方法。在零提前期达到0.630，4个月提前期保持0.601-0.603，始终超过气候学基准(0.572)。

Conclusion: 显式编码大尺度地球系统上下文的架构可以扩展野火在次季节到季节时间尺度上的可预测性，特别是在季节性一致的火灾制度区域表现最佳。
...
TL;DR: 本文提出了一种用于挤出式和喷射式3D混凝土打印的自动化细丝质量控制方法，该方法独立于数据采集传感器，可用于新鲜或固化状态的材料，支持在线和打印后质量控制。


<details>
  <summary>Details</summary>
Motivation: 在建筑、工程和施工行业中，3D混凝土打印技术不断发展以满足可持续建筑需求。细丝作为定义打印对象的基本结构，其几何质量对打印结构的质量至关重要，因此需要有效的质量控制方法。

Method: 开发了一种独立于传感器的自动化质量控制工作流程，可使用相机、结构光系统或地面激光扫描仪等不同传感器进行数据采集，适用于挤出式和喷射式3D打印方法，并能处理新鲜或固化状态的材料。

Result: 该方法实现了对3D混凝土打印细丝的自动化质量控制，能够在线监测打印过程的质量，也能在打印完成后进行质量评估，为3D打印建筑的质量保证提供了有效工具。

Conclusion: 提出的自动化质量控制方法为3D混凝土打印提供了灵活、通用的质量监控解决方案，有助于提高3D打印建筑的质量和可靠性，推动该技术在建筑行业的应用发展。
...
TL;DR: 研究比较了VGG16、ViT-B/16和CoAtNet-Tiny三种图像方法在基于日常活动记录图像识别抑郁、精神分裂症和健康对照的表现，发现CoAtNet-Tiny在准确性和稳定性方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 探索不同图像识别方法在基于活动记录图像的心理健康诊断中的性能差异，寻找最适合处理活动记录图像的方法。

Method: 将手腕活动信号转换为30×48图像，使用VGG16、ViT-B/16和CoAtNet-Tiny三种模型，通过三折受试者交叉验证评估抑郁、精神分裂症和健康对照的分类性能。

Result: CoAtNet-Tiny表现最佳，获得最高平均准确率、最稳定的跨折性能曲线，以及在抑郁和精神分裂症类别上最强的精确率、召回率和F1分数；VGG16表现稳定但准确率较低；ViT-B/16在某些运行中表现强劲但稳定性较差。

Conclusion: CoAtNet-Tiny在处理活动记录图像方面表现最一致，某些混合设计可能特别适合基于活动记录图像的心理健康研究工作。
...
TL;DR: 该研究提出了一种仅使用可见光图像进行光伏面板表面故障检测和严重程度评估的紧凑型深度学习系统，无需依赖昂贵的多模态成像设备。


<details>
  <summary>Details</summary>
Motivation: 太阳能光伏资产的持续运行依赖于对分布式模块表面故障的准确检测和优先级排序。虽然多模态成像策略很流行，但它们为常规农场级部署引入了物流和经济障碍。需要一种仅使用可见光图像就能实现稳健表面异常分类和严重程度估计的方法。

Method: 提出TinyViT紧凑管道，整合了基于Transformer的分割、光谱空间特征工程和集成回归。系统接收消费级彩色相机拍摄的光伏面板图像，分类七种细微表面故障，并生成可操作的严重程度等级用于维护优先级排序。

Result: 在真实公共数据集上的实验验证了分类和回归子模块，实现了与专门方法相竞争的准确性和可解释性。通过消除对电致发光或红外传感器的依赖，该方法使资源有限的安装能够进行经济实惠、可扩展的维护。

Conclusion: 该工作展示了深度学习和经典机器学习可以明智地结合，仅从平面可见光图像实现稳健的表面异常分类和严重程度估计，推动了太阳能健康监测向普遍现场可访问性的发展。
...
<details>
  <summary>Details</summary>
Motivation: 工业质量检测中，深度学习需要大量高质量标注数据，但实际生产中缺陷样本稀少且标注成本高昂，导致类别不平衡问题严重，阻碍了机器学习方法在真实生产环境中的广泛应用。

Method: 提出混合合成数据生成框架，集成模拟渲染、域随机化和真实背景合成技术。通过改变零件几何形状、光照和表面属性生成合成数据，并将合成零件合成到真实图像背景上。采用两阶段架构：YOLOv8n用于目标检测，MobileNetV3-small用于质量分类。

Result: 在300个真实工业零件上评估，检测mAP@0.5达到0.995，分类准确率96%，平衡准确率90.1%。在严重类别不平衡情况下，合成数据方法达到90-91%平衡准确率，而少样本真实数据基线方法仅达到50%准确率。

Conclusion: 该方法实现了无需标注、可扩展且鲁棒的工业质量检测，为真实制造应用提供了有效的解决方案，显著优于传统少样本学习方法。
...
TL;DR: Mammo-FM是首个专门针对乳腺X线摄影的医学基础模型，在140,677名患者的大规模数据集上预训练，统一支持乳腺癌诊断、病理定位、结构化报告生成和风险预测等核心临床任务。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球女性主要死因之一，现有通用基础模型在乳腺影像领域缺乏针对性，需要专门为乳腺X线摄影设计的领域特定基础模型，以统一支持多种临床任务并提高透明度和临床可审计性。

Method: 开发了首个乳腺X线摄影基础模型Mammo-FM，在四个美国机构的140,677名患者（821,326张乳腺X线片）的大规模多样化数据集上进行预训练。模型采用图像-文本对齐架构，支持视觉和文本可解释性，使用原生分辨率乳腺X线片，参数仅为最先进通用基础模型的三分之一。

Result: Mammo-FM在诊断、预后和报告生成任务上，在分布内和分布外数据集上均表现出色。尽管使用更少的参数，但在多个公共和私有基准测试中持续优于最先进的通用基础模型。

Conclusion: 领域特定基础模型围绕临床领域完整任务谱系设计具有显著效率和价值，强调了严格领域对齐评估的重要性，为乳腺影像的临床应用提供了统一、透明且可审计的解决方案。
...
TL;DR: ReactionMamba是一个用于生成长序列3D人体反应动作的新框架，结合了运动VAE和Mamba状态空间模型，在真实感、多样性和长序列生成方面表现优异，同时大幅提升了推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有的3D人体反应动作生成方法在处理长序列复杂动作（如舞蹈和武术）时存在局限性，需要能够同时生成短序列简单动作和长序列复杂动作的高效框架。

Method: ReactionMamba整合了运动VAE进行高效的运动编码，并使用基于Mamba的状态空间模型解码时间一致的反应动作，实现了对短序列简单动作和长序列复杂动作的统一生成。

Result: 在NTU120-AS、Lindy Hop和InterX三个数据集上的评估显示，ReactionMamba在真实感、多样性和长序列生成方面与InterFormer、ReMoS、Ready-to-React等方法相比具有竞争力，同时推理速度有显著提升。
...
TL;DR: DenseScan是一个包含详细多层次描述的3D场景理解数据集，通过自动化流程生成密集标注和场景问题，显著提升了3D环境中的物体级理解和问答性能。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景理解数据集通常只提供几何和实例级信息，缺乏丰富的语义标注，难以支持复杂的视觉语言任务。高质量数据对3D理解社区的发展至关重要。

Method: 提出自动化标注流程，利用多视角2D图像和多模态大语言模型生成密集的场景元素描述，并通过基于场景的问题生成扩展标注，整合物体属性、空间关系和场景上下文。
...
TL;DR: UniDiff框架使用预训练扩散模型，通过参数高效适配（仅5%参数）实现多模态遥感数据融合，解决稀疏标注问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏标注限制了多模态遥感的发展，即使像MSFMamba这样的先进监督方法也受限于标注数据可用性。ImageNet预训练模型虽然提供丰富视觉表示，但难以适应HSI和SAR等异质模态且无需大量标注数据。

Method: 提出UniDiff框架：1) 使用FiLM-based timestep-modality conditioning进行时间步-模态条件化；2) 参数高效适配（约5%参数）；3) 伪RGB锚定以保持预训练表示并防止灾难性遗忘。

Result: 在两个已建立的多模态基准数据集上，预训练扩散模型的无监督适配有效缓解了标注约束，实现了多模态遥感数据的有效融合。

Conclusion: UniDiff通过参数高效适配预训练扩散模型，能够在稀疏标注条件下有效提取遥感特征，为多模态遥感数据融合提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 病理与健康脑结构关系对神经影像学至关重要，但配对的病理-健康数据极难获取，现有方法多为领域特定且独立处理病理和健康图像建模。
...
TL;DR: HIMOSA是一个轻量级遥感图像超分辨率框架，通过内容感知稀疏注意力机制和分层窗口扩展，在保持计算效率的同时实现高性能重建。
...
Method: 提出HIMOSA框架，利用遥感图像固有的冗余性，引入内容感知稀疏注意力机制，实现快速推理。同时采用分层窗口扩展来有效利用遥感图像中的多尺度重复模式，并通过调整注意力稀疏度来降低计算复杂度。
...
TL;DR: AI系统在低剂量CT扫描中直接检测和诊断肺结节恶性程度，在25,709次扫描上训练，性能优于放射科医生和现有AI模型，AUC达0.98，假阳性率低，特别擅长早期癌症诊断。


<details>
  <summary>Details</summary>
Motivation: 传统肺癌筛查依赖结节大小和生长速度，导致诊断延迟。现有AI系统在数据集规模和可解释性方面存在局限，需要开发更准确、可解释的肺癌筛查AI系统。

Method: 设计了一个集成系统，结合浅层深度学习和基于特征的专业模型。在25,709次扫描（包含69,449个标注结节）上进行训练和评估，直接进行结节级别的检测和恶性诊断。

Result: 系统性能优于放射科医生、Lung-RADS标准和领先AI模型（Sybil、Brock、Google、Kaggle）。内部验证AUC为0.98，独立队列验证AUC为0.945。每扫描0.5个假阳性率下达到99.3%灵敏度。在所有结节大小和分期上均优于放射科医生，特别擅长第1期癌症诊断。在生长相关指标（包括体积倍增时间）上也表现优异，对不确定和缓慢生长结节的诊断比放射科医生提前最多一年。

Conclusion: 该AI系统通过集成方法解决了AI采用的关键障碍，在肺癌筛查中表现出卓越性能，特别是在早期检测方面，有望改善肺癌筛查的及时性和准确性。
...
TL;DR: TGSFormer：一种用于具身3D语义场景补全的可扩展时序高斯溅射框架，通过持久高斯内存和置信感知融合机制，在减少基元数量的同时实现更好的精度和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯的方法存在冗余和可扩展性差的问题，特别是在无边界场景中。深度引导方法虽然有所改进，但仍然是局部性的，随着场景规模增大面临延迟和内存开销问题。

Method: 提出TGSFormer框架：1) 维护持久高斯内存进行时序预测；2) 双时序编码器通过置信感知交叉注意力联合处理当前和历史高斯特征；3) 置信感知体素融合模块将重叠基元合并为体素对齐表示，调节密度并保持紧凑性。

Result: 在局部和具身SSC基准测试中均达到最先进结果，以显著更少的基元数量实现更优的精度和可扩展性，同时保持一致的长期场景完整性。
...
TL;DR: 该论文提出了一种基于最优传输(OT)的数据集蒸馏方法，通过全局和实例级别的精细对齐来提升蒸馏效果，在ImageNet-1K等大规模数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模数据集蒸馏方法主要关注匹配全局分布统计量（如均值和方差），但忽略了关键的实例级特征和类内变化，导致泛化性能不佳。需要一种能够同时考虑全局和细粒度对齐的方法来提升蒸馏效果。

Method: 将数据集蒸馏重新表述为最优传输(OT)距离最小化问题，包含三个组件：1) OT引导的扩散采样，对齐真实和蒸馏图像的潜在分布；2) 标签-图像对齐的软重标记，根据蒸馏图像分布的复杂性调整标签分布；3) 基于OT的logit匹配，对齐学生模型的输出与软标签分布。

Result: 在多种架构和大规模数据集上的实验表明，该方法始终优于最先进的方法，在IPC=10设置下，每个架构在ImageNet-1K上至少获得4%的准确率提升，且效率较高。

Conclusion: 通过最优传输框架实现全局和实例级别的精细对齐，能够有效保留复杂高维分布的几何特性，包括局部模式、类内模式和细粒度变化，从而显著提升数据集蒸馏的性能。
...
TL;DR: 提出一种基于直线特征和惯性测量的视觉里程计新方法，通过分析图像空间中直线运动形成的直纹面来估计相机运动，避免传统点特征对应的高计算成本和精度问题。


<details>
  <summary>Details</summary>
Motivation: 传统视觉里程计依赖图像序列中的特征提取和光流计算，这种点对点对应关系计算成本高且精度不稳定，影响里程计估计质量。虽然已有研究尝试使用直线特征或融合其他传感器，但仍严重依赖对应关系。

Method: 提出一种新颖算法，利用相机观察直线运动时在图像空间-时间中形成的直纹面进行分析。该方法仅需基于点-线关联的微分计算更新，并通过融合IMU惯性测量来约束解空间维度，大幅降低求解复杂度。

Result: 通过分析直纹面形状获取里程计信息，结合IMU约束后解空间维度显著降低，能够从这些直纹面重建3D场景和视觉里程计。

Conclusion: 该方法利用直线特征和惯性测量，避免了传统点特征对应的计算负担和精度问题，为视觉里程计提供了一种更高效可靠的解决方案，特别适合边缘检测能力强的传感器如事件相机。
...
TL;DR: MVAD是首个专门用于检测AI生成多模态视频-音频内容的数据集，填补了现有数据集仅关注视觉模态或局限于面部深度伪造的空白。


<details>
  <summary>Details</summary>
Motivation: AI生成的多模态视频-音频内容快速发展引发了信息安全和内容真实性的重大担忧。现有合成视频数据集主要关注视觉模态，少数包含音频的数据集也大多局限于面部深度伪造，这无法应对不断扩展的通用多模态AI生成内容，严重阻碍了可信检测系统的发展。

Method: 创建了MVAD数据集，具有三个关键特征：1) 真实多模态性，包含三种现实视频-音频伪造模式的样本；2) 通过多样化的最先进生成模型实现高感知质量；3) 全面多样性，涵盖现实和动漫视觉风格、四个内容类别（人类、动物、物体和场景）以及四种视频-音频多模态数据类型。

Result: MVAD是首个专门为检测AI生成多模态视频-音频内容设计的综合性数据集，填补了该领域的关键空白，将促进可信检测系统的开发。

Conclusion: MVAD数据集通过提供真实多模态、高质量和多样化的AI生成视频-音频样本，为开发更有效的多模态内容检测系统提供了重要资源，有助于应对AI生成内容带来的安全和真实性挑战。
...
TL;DR: AMDET提出了一种无需先验知识的模型级后门检测框架，通过分析文本编码器的特征同化特性来检测CLIP等视觉语言预训练模型中的后门攻击。


<details>
  <summary>Details</summary>
Motivation: 现有后门检测方法通常依赖于训练数据集、后门触发器、攻击目标或下游分类器的先验知识，这在现实应用中往往不切实际。需要一种无需这些先验知识的检测方法。

Method: AMDET首先揭示了后门文本编码器的特征同化特性：后门样本中所有token的表征表现出高度相似性。通过梯度反演恢复能够激活后门行为的隐式特征，并通过分析损失景观过滤自然后门特征。

Result: 在3,600个后门和良性微调模型上的实验表明，AMDET的F1分数达到89.90%。在RTX 4090 GPU上约5分钟完成一次完整检测，对自适应攻击表现出强鲁棒性。

Conclusion: AMDET是一种有效的无需先验知识的模型级后门检测框架，能够检测CLIP等视觉语言预训练模型中的后门攻击，并发现了OpenAI官方CLIP模型中存在的自然后门特征。
...
TL;DR: 首次将毫米波雷达引入人体运动预测，提出mmPred扩散框架，通过双域历史运动表示和全局骨架关系Transformer解决雷达信号噪声问题，在mmBody和mm-Fi数据集上性能显著提升。
...
Method: 提出mmPred扩散框架，包含双域历史运动表示：时域姿态细化分支学习细粒度细节，频域主导运动分支捕捉全局运动趋势并抑制帧级不一致。设计全局骨架关系Transformer作为扩散主干，建模全局关节间协作。

Result: 在mmBody和mm-Fi数据集上达到最先进性能，分别比现有方法提升8.6%和22%。

Conclusion: 首次将毫米波雷达用于人体运动预测，提出的mmPred框架有效解决了雷达信号噪声问题，在保持隐私的同时实现了鲁棒的运动预测。
...
Method: THCRL包含两个关键模块：1) 深度对称层次融合模块，利用UNet架构集成多种去噪机制实现多视图数据的可信融合；2) 平均K近邻对比学习模块，将融合表示与视图特定表示对齐，增强同一簇内样本的表示相似性，而非仅关注跨视图的同一样本。

Result: 大量实验表明，THCRL在深度多视图聚类任务中实现了最先进的性能。

Conclusion: THCRL通过可信的层次对比表示学习有效解决了多视图聚类中的不可信融合问题，通过去噪机制和结构信息增强提升了融合表示的质量和可靠性。
...
TL;DR: POLARIS提出了一种新的扩散模型反演方法，通过将引导尺度ω作为步进变量，最小化噪声近似误差，显著提升反演潜变量质量


<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的反演-去噪范式在图像编辑和修复任务中表现出色，但存在重建退化问题。研究发现关键被忽视的因素是噪声近似误差，这种误差源于用t-1步的预测来近似t步的噪声，导致反演过程中严重的误差累积。

Method: 提出POLARIS方法，将反演从误差补偿问题重新表述为误差起源问题。不是通过优化嵌入或潜变量来抵消累积漂移，而是将引导尺度ω作为步进变量，推导出数学上严谨的公式来最小化每一步的反演误差。

Result: POLARIS仅需一行代码就能显著提升反演潜变量质量，性能开销可忽略不计，能大幅减轻噪声近似误差，并持续提升下游任务的准确性。

Conclusion: POLARIS通过重新思考扩散模型反演中的误差来源问题，提供了一种简单高效的解决方案，显著改善了反演质量，为基于扩散模型的图像编辑和修复任务提供了更好的基础。
...
TL;DR: 本文提出了PorePatch数据集和DMCE框架，用于解决面部弱纹理区域描述符匹配的挑战，在匹配任务上SOTA模型表现优异，但在3D重建任务中优势有限。


<details>
  <summary>Details</summary>
Motivation: 面部皮肤区域的弱纹理特性给局部描述符匹配带来挑战，而缺乏毛孔尺度图像补丁数据集限制了深度学习描述符在面部领域的发展。

Method: 提出PorePatch高质量毛孔尺度图像补丁数据集，建立评估基准，采用数据-模型协同进化(DMCE)框架从高分辨率面部图像生成渐进精炼的高质量数据集。

Result: SOTA模型在匹配任务上达到1.91%的FPR95值，比PSIFT(22.41%)提升20.5%，但在3D重建任务中整体性能与传统描述符相比优势不明显。

Conclusion: 深度学习描述符在解决面部弱纹理区域挑战方面仍有局限性，该领域仍需大量工作，PorePatch数据集为相关研究提供了重要基础。
...
TL;DR: WiseEdit是一个知识密集型图像编辑基准测试，用于全面评估认知和创造力驱动的图像编辑能力，包含1,220个测试案例，揭示当前最先进模型在知识推理和创意合成方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑基准测试评估范围过于狭窄，无法全面评估现代图像编辑模型的高级认知和创造力能力，需要一个新的综合性评估框架。

Method: 基于人类认知创作过程，将图像编辑分解为三个级联步骤：感知、解释和想象，每个步骤对应特定任务挑战。同时包含三种基本知识类型：陈述性知识、程序性知识和元认知知识。

Result: 构建了包含1,220个测试案例的WiseEdit基准测试，客观揭示了当前最先进图像编辑模型在基于知识的认知推理和创意合成能力方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在集成分割任务时面临三难困境：需要同时保持对话能力、实现高分割性能、确保快速推理。现有方法要么引入冲突的像素级目标损害对话能力，要么采用自回归方式在分割性能和推理速度之间妥协。

Method: 提出全掩码预测范式，将自回归对话生成与非自回归掩码预测解耦。STAMP模型在生成文本响应后，通过将分割掩码预测视为图像块的并行"填空"任务，在单次前向传播中预测整个分割掩码。

Result: STAMP在多个分割基准测试中显著优于最先进方法，在对话能力、分割性能和推理速度三个方面都表现出色，无需妥协。

Conclusion: 全掩码预测范式成功解决了多模态大语言模型集成分割任务的三难困境，为同时保持对话能力、实现高分割性能和快速推理提供了有效解决方案。
...
TL;DR: DiSCo是一个语义视频压缩框架，在极低比特率下通过传输文本描述、降质视频和可选草图/姿态等紧凑模态，利用条件视频扩散模型重建高质量视频，相比传统编解码器在感知指标上提升2-10倍。


<details>
  <summary>Details</summary>
Motivation: 传统视频编解码器在极低比特率下会崩溃并产生严重伪影，这是因为像素精度与人类感知之间存在根本性错位。需要一种能够传输最有意义信息并依赖生成先验进行细节合成的压缩方法。

Method: 将源视频分解为三个紧凑模态：文本描述（语义）、时空降质视频（外观）和可选草图/姿态（运动线索）。使用条件视频扩散模型从这些紧凑表示中重建高质量、时间一致的视频。提出了时间前向填充、令牌交错和模态特定编解码器来改进多模态生成和模态紧凑性。

Result: 实验表明，该方法在低比特率下在感知指标上优于基线语义和传统编解码器2-10倍。

Conclusion: DiSCo框架通过将视频分解为语义、外观和运动紧凑模态，并利用生成先验进行重建，有效解决了传统编解码器在极低比特率下的局限性，实现了更好的感知质量压缩。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注2D平面艺术字体，而个性化3D艺术字体生成尚未充分探索。3D艺术字体不仅能在沉浸式3D环境中应用，还能通过渲染新视角增强2D字体生成。3D字体具有精确语义和强结构约束，需要细粒度部件级风格控制。

Method: 提出SplatFont3D框架：1) Glyph2Cloud模块逐步增强2D字形（或部件）的形状和风格，生成对应的3D点云用于高斯初始化；2) 通过分数蒸馏采样与预训练2D扩散模型交互优化3D高斯；3) 动态部件分配策略利用3D高斯的几何先验进行部件分割，缓解优化过程中的漂移纠缠问题。

Result: SplatFont3D在风格文本一致性、视觉质量和渲染效率方面优于现有3D模型。相比NeRF，提供更明确有效的部件级风格控制，渲染效率更高。

Conclusion: SplatFont3D成功解决了3D艺术字体生成中的挑战，实现了从多样化风格文本提示创建3D艺术字体，并支持精确的部件级风格控制，为沉浸式3D环境应用提供了有效解决方案。
...
TL;DR: 提出了一种基于物理的3D形状生成流水线，通过交替更新过程将物理属性约束融入生成模型，提升工业设计形状的真实性。


<details>
  <summary>Details</summary>
Motivation: 现有3D形状生成模型虽然能合成视觉上逼真的形状，但对于经过工程设计过程的形状类别（如汽车），形状真实性与底层物理属性（如空气动力学效率）紧密相关。现有方法缺乏物理知识，无法利用这些知识增强形状生成的真实性。

Method: 提出统一物理基础的3D形状生成流水线：1）引入具有显式物理指导的流匹配模型，采用交替更新过程（速度更新和物理精炼）；2）在速度更新步骤中加入物理感知正则化项；3）构建形状-物理变分自编码器（SP-VAE），将形状和物理信息联合编码到统一潜空间。

Result: 在三个基准测试上的实验表明，这种协同公式超越了单纯的视觉逼真度，提高了形状的真实性。

Conclusion: 通过将物理约束整合到生成模型中，提出的方法能够生成不仅视觉逼真而且物理有效的3D形状，特别适用于工业设计应用。
...
TL;DR: 该研究利用公交车上已有的CCTV监控，开发了一个自动化系统来恢复非洲撒哈拉以南地区公交车的起讫点客流数据。系统在理想条件下表现良好，但在实际拥挤、光线变化等压力条件下性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 非洲撒哈拉以南地区的公共交通经常处于过度拥挤状态，现有的自动化系统无法可靠地捕捉乘客流量数据。需要利用已部署的安全监控摄像头来获取准确的起讫点客流信息。

Method: 提出一个基线流程，结合YOLOv12检测、BotSORT跟踪、OSNet嵌入、基于OCR的时间戳和基于遥测的站点分类，从公交车CCTV视频中恢复起讫点客流数据。

Result: 在低密度、光照良好的条件下，系统达到高计数准确率（召回率约95%，精确率约91%，F1约93%），生成的起讫点矩阵与人工计数高度匹配。但在实际压力条件下（如过度拥挤、彩色转单色、姿势变化、非标准门使用等），性能显著下降（例如高峰时段登车计数低估约40%，单色片段召回率下降约17个百分点）。

Conclusion: 系统揭示了部署特定的故障模式，表明需要开发更鲁棒、面向部署的重新识别方法，以适应非洲撒哈拉以南地区公共交通的实际挑战。
...
Method: 使用Google最新AI模型Nano Banana生成合成胸部X光图像，仅用这些合成数据训练肺炎分类器，然后在两个真实世界数据集（RSNA肺炎检测数据集和胸部X光数据集）上进行外部验证。

Result: 在RSNA数据集上获得AUROC 0.923和AUPR 0.900，在胸部X光数据集上获得AUROC 0.824和AUPR 0.913，表明仅用合成数据训练的模型在真实数据上表现良好。

Conclusion: 合成数据在医学AI开发中具有潜力，但需要解决提示设计多样性、数据后处理对齐等挑战，临床应用前需要充分的验证、监管批准和伦理监督。
...
TL;DR: FR-TTS提出了一种基于填充的奖励机制，用于解决测试时缩放技术在下一个token预测范式中的应用难题，通过填充中间序列来评估样本质量，显著提升了生成效果。


<details>
  <summary>Details</summary>
Motivation: 测试时缩放技术在图像生成中效果显著，但在下一个token预测范式中难以应用，主要障碍是中间token序列解码的图像奖励与最终生成图像奖励之间的相关性很低，导致中间表示无法有效指导剪枝方向。

Method: 提出了基于填充的奖励机制，通过寻找合理的填充方案来补全中间序列，从而估计中间样本的近似未来轨迹。基于此设计了FR-TTS策略，高效搜索好的填充方案，并引入多样性奖励和动态权重调度来实现对中间样本的平衡全面评估。

Result: 实验验证了FR-TTS在多个基准测试和各种奖励模型上的优越性，中间样本与最终样本奖励之间的相关系数以及token置信度等内在信号表明，FR提供了优秀可靠的中间样本质量评估指标。

Conclusion: FR-TTS成功解决了测试时缩放在下一个token预测范式中的应用难题，通过创新的填充奖励机制实现了对中间样本质量的准确评估，为文本生成任务提供了有效的测试时缩放解决方案。
...
TL;DR: CausalAffect：首个用于面部情感分析的因果图发现框架，通过两级因果层次建模AU间和AU-表情的依赖关系，无需联合标注数据集或手工因果先验，在AU检测和表情识别上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法很少解决如何从数据中直接推断AU与表情之间的心理合理因果关系。虽然AU长期以来一直是情感计算的基础，但缺乏对潜在依赖关系的结构化推理。

Method: 提出CausalAffect框架，通过两级极性感知和方向感知的因果层次建模AU-AU和AU-表情依赖关系，整合群体级规律与样本自适应结构，并采用特征级反事实干预机制来强化真实因果效应并抑制虚假相关性。

Result: 在六个基准测试上的广泛实验表明，CausalAffect在AU检测和表情识别方面都达到了最先进水平，恢复的因果结构与既定心理学理论一致，同时揭示了新的抑制性和先前未表征的依赖关系。

Conclusion: CausalAffect建立了因果发现与可解释面部行为之间的原则性连接，无需联合标注数据集或手工因果先验即可恢复心理合理的因果结构，为面部情感分析提供了新的因果推理框架。
...
TL;DR: RealGen是一个逼真的文本到图像生成框架，通过集成LLM提示优化和扩散模型，引入"检测器奖励"机制来量化伪影并评估真实感，显著提升了图像真实感和细节。


<details>
  <summary>Details</summary>
Motivation: 当前先进的图像生成模型（如GPT-Image-1和Qwen-Image）在文本到图像一致性和世界知识方面表现优异，但在逼真图像生成方面仍存在不足。这些模型倾向于产生带有明显AI伪影的"虚假"图像，常见问题包括"过度光滑的皮肤"和"油腻的面部光泽"。为了重新实现"与现实无法区分"的生成目标，需要开发更逼真的文本到图像框架。

Method: RealGen框架包含两个核心组件：LLM用于提示优化，扩散模型用于逼真图像生成。受对抗生成启发，引入了"检测器奖励"机制，该机制使用语义级和特征级合成图像检测器来量化伪影并评估真实感。利用GRPO算法优化整个生成流程，显著提升图像真实感和细节。此外，还提出了RealBench自动评估基准，采用检测器评分和竞技场评分，实现无需人工参与的逼真度评估。

Result: 实验表明，RealGen在真实感、细节和美学方面显著优于通用模型（如GPT-Image-1和Qwen-Image）以及专门的逼真模型（如FLUX-Krea）。提出的RealBench评估基准能够提供更准确、更符合真实用户体验的评估结果。

Conclusion: RealGen通过集成LLM提示优化、扩散模型和创新的检测器奖励机制，成功提升了文本到图像生成的逼真度。该框架在逼真图像生成方面超越了现有模型，同时提出的RealBench评估基准为逼真度评估提供了有效的自动化解决方案。代码已开源，可供进一步研究和应用。
...
TL;DR: CC-FMO是一个零样本、相机条件化的单图像到3D场景生成管道，通过混合实例生成器和相机条件化尺度求解算法，在保持实例保真度的同时实现场景级一致性，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单图像生成高质量3D场景时面临挑战：早期方法依赖在小数据集上训练的专业模型，泛化能力差；近期大规模3D基础模型在实例级生成上有进步，但场景生成仍受限于不准确的对象姿态估计和空间不一致性。

Method: CC-FMO采用混合实例生成器，结合语义感知的向量集表示和细节丰富的结构化潜在表示，生成既语义合理又高质量的物体几何。通过相机条件化尺度求解算法，将基础姿态估计模型应用于场景生成任务，确保场景级一致性。

Result: 大量实验表明，CC-FMO能够一致地生成高保真度、相机对齐的组合场景，在所有最先进方法中表现最优。

Conclusion: CC-FMO通过创新的混合表示和相机条件化尺度求解，成功解决了单图像到3D场景生成中的实例保真度和场景一致性难题，为零样本场景生成提供了有效解决方案。
...
TL;DR: 提出基于智能手机的结构光系统，通过投影网格图案到地面重建局部地形不平度，并开发拓扑约束的二维动态时间规整算法来匹配变形网格。


<details>
  <summary>Details</summary>
Motivation: 低成本移动机器人在不平坦地形上运行时，小凸起或倾斜难以视觉感知但显著影响运动稳定性，需要一种简单有效的地形感知方法。

Method: 使用智能手机结构光系统投影网格图案到地面，提出拓扑约束的二维动态时间规整算法，在全局网格一致性约束下进行列对齐，匹配变形网格。

Result: 开发的方法足够简单可在资源有限平台上运行，同时保持网格结构以实现精确三角测量，不仅适用于地形感知，也可作为匹配结构化网格图案的通用工具。

Conclusion: 提出的2D-DTW算法扩展了传统1D-DTW，能有效处理二维网格模式匹配问题，为低成本地形感知提供了实用解决方案。
...
TL;DR: 本文探索使用预训练图像生成模型作为机器人视觉规划器，通过LoRA微调实现文本条件和轨迹条件的机器人操作视频生成。


<details>
  <summary>Details</summary>
Motivation: 生成真实的机器人操作视频对于统一具身智能体的感知、规划和行动至关重要。现有视频扩散模型需要大量领域特定数据且泛化能力有限，而预训练的语言-图像模型表现出强大的组合能力，暗示其具有视频生成潜力。

Method: 提出两阶段框架：1) 文本条件生成：使用语言指令和第一帧图像；2) 轨迹条件生成：使用2D轨迹叠加和相同初始帧。通过LoRA微调轻量适配预训练图像生成模型。

Result: 在Jaco Play、Bridge V2和RT1数据集上的实验表明，两种模式都能生成平滑、连贯且与条件对齐的机器人视频。

Conclusion: 预训练图像生成器编码了可迁移的时间先验，能够在最小监督下作为视频式机器人规划器，为机器人视觉规划提供了新思路。
...
TL;DR: 提出Cross-Temporal 3D Gaussian Splatting框架，利用稀疏图像和历史场景先验，高效重建和更新不同时间段的3D场景


<details>
  <summary>Details</summary>
Motivation: 现实应用中（如城市规划、灾害评估、历史遗址保护）通常无法获得密集扫描数据，需要从稀疏视角观测中更新3D场景表示，并保持时间一致性

Method: 三阶段框架：1)跨时间相机对齐，估计和对齐不同时间戳的相机姿态；2)基于干扰的置信度初始化，识别时间戳间未变化区域以指导更新；3)渐进式跨时间优化，迭代整合历史先验信息到3D场景

Result: 实验结果显示在重建质量和数据效率方面显著优于基线方法，支持非连续捕获，既能用新稀疏视图更新现有场景，也能借助当前捕获恢复过去场景

Conclusion: 该方法为场景版本控制、跨时间数字孪生和长期空间文档化提供了有前景的解决方案，展示了仅用稀疏图像实现时间变化检测并重建为详细3D表示的潜力
...
TL;DR: 提出SAIDO框架，通过场景感知和重要性引导的动态优化机制，解决AI生成图像检测中的泛化问题，在持续学习任务中显著降低检测错误率和遗忘率。


<details>
  <summary>Details</summary>
Motivation: 图像生成技术的滥用引发安全担忧，现有AI生成图像检测方法在真实场景中难以适应新兴生成方法和内容类型，泛化能力成为关键挑战。

Method: 提出SAIDO框架：1) 场景感知专家模块(SAEM)利用VLLMs动态识别新场景并分配独立专家模块；2) 重要性引导动态优化机制(IDOM)通过重要性引导的梯度投影策略优化每个神经元，平衡模型可塑性和稳定性。

Result: 在持续学习任务中，相比当前SOTA方法，平均检测错误率相对降低44.22%，遗忘率相对降低40.57%；在开放世界数据集上，平均检测准确率提升9.47%。

Conclusion: SAIDO框架通过场景感知和重要性引导的动态优化，有效解决了AI生成图像检测中的泛化问题，在持续学习和开放世界场景中均表现出优越性能。
...
TL;DR: 提出了一种混合方法，结合3D生成模型、语义感知变形和3D高斯溅射优化，用于多人物多物体动态场景的几何建模，在单目设置下保持结构一致性。


<details>
  <summary>Details</summary>
Motivation: 现实世界的人造环境高度动态，包含多个人物与物体的复杂交互。虽然3D几何建模对AR/VR、游戏和具身AI应用至关重要，但由于多样的运动模式和频繁遮挡等挑战，这类场景的建模仍未被充分探索。单目设置下这些挑战更加严峻，仅基于高斯溅射渲染损失优化时，在严重遮挡下保持结构一致性变得困难。

Method: 提出混合方法：1) 使用3D生成模型生成场景元素的高保真网格；2) 语义感知变形，包括刚性物体的刚性变换和基于LBS的人物变形，并将变形后的高保真网格映射到动态场景中；3) 基于高斯溅射优化单个元素以进一步细化它们在场景中的对齐。

Result: 在HOI-M3数据集上评估，该数据集是唯一包含动态场景中多人物多物体交互的数据集。该方法在生成更好的场景表面重建方面优于现有最先进方法。

Conclusion: 混合方法能够有效处理多人物多物体动态场景的挑战，即使在严重遮挡下也能保持物体结构，并产生多视角和时间一致的几何模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注孤立脑区的类别选择性（如FFA的面孔识别），但无法揭示在复杂自然视觉过程中不同脑区如何相互作用。需要一种能够探索脑区间交互作用并生成满足复杂神经约束图像的方法。

Method: 开发NeuroVolve生成框架，在预训练视觉语言模型的嵌入空间中优化神经目标函数。通过编程化的神经目标（激活或抑制单个或多个脑区）引导图像生成，并追踪优化步骤以揭示语义轨迹。

Result: 验证了能够恢复已知脑区选择性，同时扩展到合成满足复杂多区域约束的连贯场景。能够生成低层次和语义特征特定的刺激，捕捉区域间的协同和拮抗调谐关系，并支持个性化大脑驱动合成。

Conclusion: NeuroVolve统一了大脑引导的图像编辑和偏好刺激生成，为映射、分析和探测视觉信息的神经表征提供了可解释的约束，支持个性化脑驱动合成。
...
TL;DR: 该研究提出了Yoga-16数据集，系统评估了三种深度学习架构在三种输入模态下的瑜伽姿势分类性能，发现基于骨架的表示优于原始图像输入，VGG16+MediaPipe Pose骨架输入达到最高96.09%准确率。


<details>
  <summary>Details</summary>
Motivation: 瑜伽作为全球流行的运动形式，不正确的姿势可能导致受伤。自动化的瑜伽姿势分类可以减少对专业从业者的依赖，但现有研究在系统化基准测试方面有限，通常只关注原始图像或单一姿势提取模型。

Method: 1. 创建了Yoga-16数据集，解决了现有数据集的局限性；2. 系统评估了三种深度学习架构（VGG16、ResNet50、Xception）；3. 使用了三种输入模态（直接图像、MediaPipe Pose骨架图像、YOLOv8 Pose骨架图像）；4. 使用Grad-CAM进行可解释性分析；5. 进行交叉验证分析。

Result: 1. 基于骨架的表示优于原始图像输入；2. VGG16结合MediaPipe Pose骨架输入达到最高准确率96.09%；3. 提供了模型决策的可解释性分析。

Conclusion: 该研究通过系统化的基准测试表明，基于骨架的表示在瑜伽姿势分类中具有优势，为自动化瑜伽姿势评估提供了有效方法，并通过可解释性分析增强了模型透明度。
...
<details>
  <summary>Details</summary>
Motivation: 讽刺作为一种结合幽默与隐性批判的艺术表达形式，具有重要的社会价值，但当前视觉语言模型在理解纯视觉讽刺方面存在困难。现有模型难以有效整合局部实体关系与全局上下文，导致误解、理解偏差和幻觉。

Method: 提出SatireDecoder框架：1）多智能体系统执行视觉级联解耦，将图像分解为细粒度的局部和全局语义表示；2）引入基于不确定性分析的思维链推理策略，将复杂的讽刺理解过程分解为顺序子任务以最小化不确定性。

Result: 实验结果表明，SatireDecoder在理解视觉讽刺方面优于现有基线方法，显著提高了解释准确性，同时减少了幻觉现象。

Conclusion: SatireDecoder为视觉语言推理在细微、高级语义任务中提供了有前景的方向，能够有效提升模型对视觉讽刺的理解能力。
...
TL;DR: MedCT-VLM是一个参数高效的医学CT视觉语言模型，通过LoRA技术微调CT-CLIP基础模型，仅训练0.38%的参数就能显著提升18种胸部病理的零样本分类性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言预训练基础模型在多个图像领域展现出强大的零样本能力，但在体积医学影像（如CT）中的应用仍然有限。医学影像领域标注数据稀缺，需要能够有效利用预训练知识并适应下游临床任务的解决方案。

Method: 提出MedCT-VLM框架，采用参数高效的LoRA（低秩适应）方法微调CT-CLIP模型。CT-CLIP是一个在25,692个胸部CT体积上训练的对比视觉语言模型。通过在视觉和文本编码器的注意力层中插入低秩分解矩阵，仅训练1.67M参数（占总参数440M的0.38%），而不是直接微调全部参数。

Result: 在18种胸部病理的零样本分类任务中，LoRA微调使平均AUROC从61.3%提升至68.9%（+7.6个百分点），准确率从67.2%提升至73.6%（+6.4个百分点），宏平均F1从32.1%提升至36.9%（+4.8个百分点）。

Conclusion: 参数高效方法能够有效将大规模预训练知识迁移到下游医学影像任务中，特别是在标注数据稀缺的零样本场景下具有重要价值。MedCT-VLM展示了在医学CT领域应用视觉语言模型的可行性。
...
TL;DR: 本研究评估了五种深度学习模型（YOLOv9、U-Net、Swin Transformer、DeepLabV3和Mask R-CNN）在树木横截面髓心检测中的性能，通过数据增强和模型优化，Swin Transformer达到最高精度0.94，并验证了模型在不同树种数据集上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 树木横截面髓心检测对林业和木材质量分析至关重要，但目前仍依赖人工操作，存在效率低、易出错的问题。本研究旨在通过深度学习模型实现自动化髓心检测，提高检测效率和准确性。

Method: 使用582张标注图像构建数据集，并进行动态数据增强以提高泛化能力。评估了五种深度学习模型：YOLOv9（边界框检测）、U-Net（结构化模式）、Swin Transformer（精细分割）、DeepLabV3（多尺度特征）和Mask R-CNN（实例分割）。通过超参数调优、非极大值抑制（NMS）等技术解决模型特定问题，并使用俄勒冈州立大学树木年轮实验室的11张橡树图像测试模型泛化能力。

Result: Swin Transformer表现最佳，准确率达到0.94，在精细分割方面表现突出；YOLOv9在边界框检测中表现良好但边界精度不足；U-Net对结构化模式有效；DeepLabV3能捕获多尺度特征但边界略有模糊；Mask R-CNN初始IoU仅0.45，应用NMS后提升至0.80。在橡树数据集上的泛化测试显示模型具有较好的跨树种适应性。

Conclusion: 深度学习在树木横截面髓心检测中具有巨大潜力，模型选择应根据数据集特征和应用需求。Swin Transformer在精细分割任务中表现最优，而通过适当的技术调整（如NMS）可以显著改善某些模型的性能。研究为自动化林业分析提供了有效的技术方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着甲状腺癌全球发病率上升，需要开发计算机辅助检测方法。甲状腺结节的准确分割是开发AI辅助临床决策支持系统的关键第一步。

Method: 使用YOLOv5算法（Nano、Small、Medium、Large、XLarge五个变体）在超声图像上进行甲状腺结节实例分割，评估两个数据集版本（包含和不包含多普勒图像）。
...
Conclusion: YOLOv5实例分割为甲状腺结节检测提供了有效的实时方法，多普勒图像虽然通常被医生排除，但能显著改善分割性能，具有在自动化诊断系统中应用的临床潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉Mamba模型的效率受限于输入token数量，而现有的token缩减方法（如剪枝或合并）会导致信息丢失，特别是当统一应用于所有图像的细粒度token表示时，无论视觉复杂度如何，都会加剧这一问题。

Method: CF-ViM采用自适应粗到细推理框架：首先将输入图像划分为大块进行粗粒度推理，显著减少token长度和计算量；当模型预测置信度低时，选择特定区域以更细分辨率重新处理，以最小额外成本恢复关键视觉细节。

Result: 在ImageNet上的实验表明，CF-ViM在准确性和效率方面均优于基线视觉Mamba模型和最先进的token缩减技术。

Conclusion: CF-ViM通过动态分辨率分配策略，能够根据图像复杂度自适应分配计算资源，确保高效处理而不损失必要的视觉信息，为视觉Mamba模型提供了更高效的替代方案。
...
TL;DR: 该论文创建了更真实的多数字书写者基准数据集，发现孤立数字分类器在多数字识别中表现不佳，需要新方法提升真实场景的数字识别性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中数字通常以多位数形式出现（如邮政编码、支票金额、预约时间），但现有研究主要关注孤立数字分类。需要创建更真实的基准数据集来评估和改进真实世界的数字识别系统。

Method: 利用NIST数字图像中的书写者信息，创建多数字书写者基准数据集。这些数据集包含特定任务的性能指标，超越传统错误计算，更贴近实际应用影响。

Result: 研究发现孤立数字分类器在多数字识别任务中表现不佳。创建的数据集为开发能够利用任务特定知识的方法提供了机会，性能可以显著超越单个数字分类方法。

Conclusion: 要解决真实数字识别问题，需要超越孤立数字分类的额外进展。多数字书写者基准数据集提供了更贴近现实的评估框架，支持开发能够利用上下文和书写者信息的改进方法。
...
### [57] [Generalized Medical Phrase Grounding](https://arxiv.org/abs/2512.01085)
*Wenjun Zhang,Shekhar S. Chandra,Aaron Nicolson*

Main category: cs.CV

TL;DR: 论文提出广义医学短语定位(GMPG)任务，将医学报告中的句子映射到零个、一个或多个带分数的区域，并介绍了首个GMPG模型MedGrounder，在PadChest-GR和MS-CXR数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有医学短语定位系统大多遵循指代表达理解范式，每个短语只返回一个边界框，但真实医学报告常包含多区域发现、非诊断文本和不可定位短语，需要更通用的任务定义。

Method: 提出广义医学短语定位任务，开发MedGrounder模型，采用两阶段训练策略：先在报告句子-解剖框对齐数据集上预训练，然后在报告句子-人工标注框数据集上微调。

Result: 在PadChest-GR和MS-CXR数据集上，MedGrounder实现了强大的零样本迁移能力，在多区域和不可定位短语上优于REC风格和基于报告的生成基线，且使用更少的人工标注框。

Conclusion: MedGrounder能有效处理真实医学报告中的复杂情况，可与现有报告生成器组合使用，无需重新训练生成器即可生成定位报告，为医学图像解释提供了更实用的解决方案。
...
TL;DR: Dynamic-eDiTor：无需训练的文本驱动4D场景编辑框架，结合MM-DiT和4DGS，通过STGA和CTP机制实现时空一致的多视角视频编辑


<details>
  <summary>Details</summary>
Motivation: 现有4D场景重建技术（如Dynamic NeRF和4DGS）已取得进展，但文本驱动的4D场景编辑仍面临挑战，主要问题在于如何确保编辑过程中的多视角一致性和时间一致性。现有方法依赖2D扩散模型独立编辑帧，常导致运动扭曲、几何漂移和不完整编辑。
...
Result: 在多视角视频数据集DyNeRF上进行广泛实验，结果表明该方法在编辑保真度、多视角一致性和时间一致性方面优于现有方法。

Conclusion: Dynamic-eDiTor实现了无缝、全局一致的文本驱动4D场景编辑，解决了现有方法中的运动扭曲和几何漂移问题，为4D场景编辑提供了有效的训练免费解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前步态识别模型受限于小型、窄设计模型，无法扩展或泛化。需要解决两个长期障碍：1) 可扩展性：为什么步态模型历史上未能遵循缩放定律？2) 泛化性：能否用一个模型服务传统上孤立研究的多样化步态任务？

Method: 提出FoundationGait，首个可扩展的自监督预训练步态理解框架。最大版本有近1.3亿参数，在12个公开步态数据集（超过200万个行走序列）上进行预训练。

Result: FoundationGait在有或无微调的情况下，在广泛的步态数据集、条件、任务（如身份识别、脊柱侧弯筛查、抑郁预测、属性估计）和输入模态上表现稳健。在Gait3D数据集上达到48.0%的零样本Rank-1准确率，在OU-MVLP数据集上达到64.5%，创下稳健步态识别新里程碑。

Conclusion: FoundationGait成功解决了步态模型的可扩展性和泛化性问题，为步态理解建立了首个统一的基础模型框架，在多种任务和条件下表现出色。
...
TL;DR: AFD提出了一种用于视频-语言理解的持续学习方法，通过将视频映射到缓慢变化的可供性令牌形成共享的时间对齐基底，同时使用轻量级查询路由调度器进行针对性适应，在保持稳定性的同时实现可塑性。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法在视频-语言理解中存在三个主要问题：1) 模糊了应该保持稳定与应该适应的部分；2) 依赖静态路由/容量；3) 需要回放过去的视频。作者旨在在现实内存和隐私约束下，明确指定稳定性所在和可塑性应聚焦的位置。

Method: AFD（可供性优先分解）方法：1) 将视频映射到缓慢变化的可供性令牌，形成共享的时间对齐基底；2) 使用轻量级、查询路由、冲突感知的调度器集中适应并仅在需要时增长容量；3) 通过弱对齐和教师一致性稳定基底；4) 训练时使用仅问题回放。

Result: AFD在多个协议上达到最先进水平：在领域增量VideoQA上平均准确率51.6%，遗忘率-1.8%；在ViLCo上R@1@0.5为29.6%（MQ）和20.7%（NLQ），stAP@0.25为18.4%（VQ）；在时间增量iVQA上准确率39.5%，遗忘率-1.6%。

Conclusion: AFD提供了一种明确、可解释的方法，在稳定的交互中心基底和针对性适应之间进行分割，为视频-语言理解的持续学习提供了有效解决方案。
...
### [61] [StreamGaze: Gaze-Guided Temporal Reasoning and Proactive Understanding in Streaming Videos](https://arxiv.org/abs/2512.01707)
...
TL;DR: StreamGaze是首个评估多模态大语言模型在流式视频中利用人类注视信号进行时序和前瞻推理的基准，包含注视引导的过去、现在和前瞻任务，揭示了当前模型在注视引导推理方面的显著性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频基准仅评估时序推理能力，但未衡量多模态大语言模型是否能解释或利用人类注视信号。流式视频理解需要模型不仅能处理时序输入帧，还要能预测用户意图（如AR眼镜应用），因此需要专门的基准来评估注视引导的流式视频理解能力。

Method: 开发了注视-视频问答生成流水线，通过注视点提取、区域特定视觉提示和扫描路径构建，将自我中心视频与原始注视轨迹对齐，生成时空基础的问题-答案对，反映人类感知动态。构建了StreamGaze基准，包含注视引导的过去、现在和前瞻任务。

Result: 在所有StreamGaze任务中，观察到最先进的多模态大语言模型与人类性能之间存在显著性能差距，揭示了模型在注视基础时序推理、意图建模和前瞻预测方面的根本局限性。提供了注视提示策略、推理行为和任务特定失败模式的详细分析。

Conclusion: StreamGaze填补了流式视频理解中注视信号评估的空白，揭示了当前多模态大语言模型在注视引导推理方面的不足，为未来模型发展提供了方向。所有数据和代码将公开以支持注视引导流式视频理解的持续研究。
...
TL;DR: 该论文提出了一种减少大型视觉语言模型幻觉的新方法，通过使用在线数据、幻觉分类器和动态样本重加权策略，显著降低了模型的幻觉率。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现出潜力，但幻觉缓解仍然是一个关键挑战。现有方法在训练样本中引入了额外的幻觉，可能增强模型的幻觉模式。

Method: 1. 分析发现在线数据显著优于离线数据，需要高效可靠的在线数据偏好标注；2. 提出训练幻觉分类器提供二元标注，确保后续对齐的干净选择样本；3. 设计鲁棒的迭代直接偏好优化算法，采用动态样本重加权方案。

Result: 在三个基准测试中与8个最先进基线比较：1. LLaVA-1.5-7B在MMHalBench上的幻觉率降低50.8%；2. 在Object HalBench上的平均幻觉率降低79.5%；3. LLaVA-1.5-13B的性能甚至超过GPT-4V。

Conclusion: 该方法通过在线数据、幻觉分类器和动态重加权策略，有效缓解了大型视觉语言模型的幻觉问题，充分挖掘了开源模型的潜力，实现了超越商业模型的性能。
...
### [63] [Deep Learning-Based Computer Vision Models for Early Cancer Detection Using Multimodal Medical Imaging and Radiogenomic Integration Frameworks](https://arxiv.org/abs/2512.00714)
*Emmanuella Avwerosuoghene Oghenekaro*

Main category: cs.CV

TL;DR: 深度学习模型在医学影像分析中实现癌症早期检测，通过多模态影像与基因组学融合实现个性化肿瘤学


<details>
  <summary>Details</summary>
Motivation: 癌症早期检测是现代医疗的关键挑战，延迟诊断显著降低生存率，需要更精准的检测方法

Method: 使用深度学习计算机视觉模型（CNN、transformer、混合注意力架构）分析多模态影像数据（MRI、CT、PET、乳腺X光、病理、超声），结合影像基因组学将定量影像特征与基因组学、转录组学、表观遗传生物标志物融合

Result: 深度学习模型超越传统放射学评估，能识别人眼不可见的细微组织异常和肿瘤微环境变化；影像基因组学融合可预测肿瘤基因型、免疫反应、分子亚型和治疗耐药性，无需侵入性活检

Conclusion: 人工智能驱动的多模态影像分析与影像基因组学融合为癌症早期检测和个性化肿瘤学引入了新范式，有望显著改善患者预后
...
TL;DR: 提出RS-ISRefiner框架，针对遥感图像交互式分割，通过适配器调优、混合注意力机制和改进的概率图调制，在多个数据集上超越现有方法。
...
Method: 提出RS-ISRefiner框架：1) 采用适配器调优策略，保持视觉基础模型的通用表示，同时高效学习遥感特定的空间和边界特征；2) 结合卷积局部建模和Transformer全局推理的混合注意力机制，增强对尺度多样性和场景复杂性的鲁棒性；3) 改进的概率图调制方案，有效整合历史用户交互，实现更稳定的迭代优化和更高边界保真度。

Result: 在iSAID、ISPRS Potsdam、SandBar、NWPU、LoveDA Urban和WHUBuilding六个遥感数据集上的综合实验表明，RS-ISRefiner在分割精度、效率和交互成本方面一致优于最先进的交互式分割方法。

Conclusion: RS-ISRefiner框架在遥感图像交互式分割中表现出有效性和泛化能力，特别适合实际遥感场景中的高质量实例分割任务。
...
### [65] [TrajDiff: End-to-end Autonomous Driving without Perception Annotation](https://arxiv.org/abs/2512.00723)
*Xingtai Gui,Jianbo Zhao,Wencheng Han,Jikai Wang,Jiahao Gong,Feiyang Tan,Cheng-zhong Xu,Jianbing Shen*

Main category: cs.CV

TL;DR: TrajDiff是一个无需感知标注的端到端自动驾驶轨迹生成框架，通过BEV热图条件和扩散模型直接生成多样化轨迹，在NAVSIM基准上达到最先进的标注无关方法性能。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶系统依赖辅助感知任务，但感知标注成本高昂，需要开发无需感知标注的规划范式。

Method: 提出TrajDiff框架：1) 使用原始传感器输入和未来轨迹构建高斯BEV热图目标；2) 设计轨迹导向的BEV编码器提取TrajBEV特征；3) 引入轨迹导向的BEV扩散变换器(TB-DiT)，利用自车状态和TrajBEV特征直接生成多样化轨迹。

Result: 在NAVSIM基准上达到87.5 PDMS，在所有标注无关方法中达到最先进性能。通过数据扩展可提升至88.5 PDMS，与先进的基于感知的方法相当。

Conclusion: TrajDiff成功建立了完全无需感知标注的生成式端到端自动驾驶方法，不仅实现了高性能，还能探索数据扩展的益处，为自动驾驶规划提供了新的标注高效解决方案。
...
### [66] [Multi-GRPO: Multi-Group Advantage Estimation for Text-to-Image Generation with Tree-Based Trajectories and Multiple Rewards](https://arxiv.org/abs/2512.00743)
...
TL;DR: Multi-GRPO提出了一种多组优势估计框架，通过时间分组和奖励分组机制，解决了GRPO方法在信用分配和奖励混合方面的局限性，在多目标对齐任务中表现出更好的稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于GRPO的方法存在两个关键限制：1）共享信用分配问题：轨迹级优势从组归一化的稀疏终端奖励中推导，均匀应用于所有时间步，无法准确估计具有广阔探索空间的早期去噪步骤的潜力；2）奖励混合问题：预定义权重组合多目标奖励（如文本准确性、视觉质量、文本颜色），这些奖励具有不匹配的尺度和方差，导致梯度不稳定和冲突更新。

Method: 提出Multi-GRPO框架，包含两种正交分组机制：1）时间分组：受蒙特卡洛树搜索启发，在选定的早期去噪步骤分支轨迹形成时间组，通过后代叶子节点准确估计早期步骤的优势，同时通过共享前缀分摊计算；2）奖励分组：为每个奖励函数独立计算优势后再聚合，解耦冲突信号。还构建了OCR-Color-10数据集用于多目标对齐评估。

Result: 在单奖励PickScore-25k和多目标OCR-Color-10基准测试中，Multi-GRPO实现了优越的稳定性和对齐性能，有效平衡了冲突目标。

Conclusion: Multi-GRPO通过时间分组和奖励分组机制解决了GRPO方法的局限性，在多目标文本到图像模型对齐任务中表现出更好的性能，为复杂对齐问题提供了有效解决方案。
...
### [67] [Probabilistic Modeling of Multi-rater Medical Image Segmentation for Diversity and Personalization](https://arxiv.org/abs/2512.00748)
*Ke Liu,Shangde Gao,Yichao Fu,Shangqi Gao,Chunhua Shen*

Main category: cs.CV

TL;DR: ProSeg提出概率建模方法解决多标注者医学图像分割问题，同时实现多样化和个性化分割输出，在鼻咽癌和肺结节数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割存在数据不确定性，源于医学扫描中的模糊边界和诊断中的观察者间差异。现有模型要么只能生成缺乏专家特异性的多样化分割，要么只能复制单个标注者的个性化输出，无法同时满足多样化和个性化需求。

Method: 提出概率建模的多标注者医学图像分割方法(ProSeg)，引入两个潜变量分别建模专家标注偏好和图像边界模糊性，通过变分推断获得条件概率分布，通过从这些分布中采样生成分割输出。

Result: 在鼻咽癌数据集(NPC)和肺结节数据集(LIDC-IDRI)上的大量实验表明，ProSeg实现了新的最先进性能，提供了既多样化又专家个性化的分割结果。

Conclusion: ProSeg能够同时实现医学图像分割的多样化和个性化，有效解决了多标注者医学图像分割中的不确定性挑战，为临床决策提供了更全面的支持。
...
### [68] [Charts Are Not Images: On the Challenges of Scientific Chart Editing](https://arxiv.org/abs/2512.00752)
*Shawn Li,Ryan Rossi,Sungchul Kim,Sunav Choudhary,Franck Dernoncourt,Puneet Mathur,Zhengzhong Tu,Yue Zhao*

Main category: cs.CV

TL;DR: FigEdit是一个针对科学图表编辑的大规模基准测试，包含30,000多个样本，涵盖10种图表类型和复杂编辑指令，揭示了现有生成模型在结构化图表编辑上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型（如扩散和自回归模型）在自然图像编辑上表现出色，但应用于科学图表时存在根本性缺陷：图表不是简单的像素排列，而是受图形语法约束的结构化数据可视化表示。图表编辑本质上是结构化转换问题，而非像素操作任务。

Method: 引入FigEdit基准测试，基于真实世界数据构建，包含30,000多个样本，涵盖10种图表类型和丰富的复杂编辑指令。基准分为五个渐进挑战性任务：单次编辑、多次编辑、对话式编辑、视觉引导编辑和风格迁移。

Result: 评估多个最先进模型发现，它们在科学图表编辑上表现不佳，无法处理有效的结构化转换。传统评估指标（如SSIM、PSNR）在捕捉图表编辑的语义正确性方面存在局限性。

Conclusion: FigEdit基准揭示了像素级操作的深刻局限性，为开发和评估未来结构感知模型提供了坚实基础。通过开源该基准，旨在促进结构感知图表编辑的系统性进展，为公平比较提供共同基础，并鼓励理解科学图表视觉和语义层的模型研究。
...
TL;DR: 该论文提出了一种端到端可微分的逆图形框架，能够从视频中恢复不可见的物理力场（如风场），通过建模物体几何、物理属性和相互作用来推断力表示。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉中长期目标是建模视频中的运动，但运动背后的表示——即导致物体变形和移动的不可见物理相互作用——在很大程度上尚未被探索。研究者希望从视觉观察中恢复不可见的力，例如通过观察叶子落地的过程来估计风场。

Method: 提出端到端可微分逆图形框架，联合建模物体几何、物理属性和相互作用，通过反向传播从物体运动中恢复力表示。

Result: 在合成和真实场景中验证了该方法，结果表明能够从视频中推断出合理的力场。展示了该方法在基于物理的视频生成和编辑等潜在应用。
...
### [70] [PolarGS: Polarimetric Cues for Ambiguity-Free Gaussian Splatting with Accurate Geometry Recovery](https://arxiv.org/abs/2512.00794)
*Bo Guo,Sijia Wen,Yifan Zhao,Jia Li,Zhiming Zheng*

Main category: cs.CV

TL;DR: PolarGS利用偏振光作为光学先验，解决3D高斯溅射在反射和无纹理表面的重建模糊问题，通过偏振引导的光度校正和增强的高斯密集化机制提升几何精度。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯溅射的表面重建方法在反射和无纹理等光度模糊区域性能下降，因为不可靠的光度线索破坏了光度一致性并阻碍了准确的几何估计。偏振光能部分揭示表面方向，可作为光度线索的光学补充来解决这些模糊问题。

Method: 提出了PolarGS，这是基于RGB的3DGS的光学感知扩展，利用偏振作为光学先验。包含两个互补模块：1）偏振引导的光度校正策略，通过线性偏振度识别反射区域，并使用颜色细化图优化反射高斯；2）偏振增强的高斯密集化机制，将角度和线性偏振度集成到基于PatchMatch的深度补全过程中，实现新高斯的后投影和融合。

Result: PolarGS是框架无关的，与最先进的方法相比，实现了更优越的几何精度，特别是在反射和无纹理表面区域。

Conclusion: 偏振光作为光学先验能有效解决3D高斯溅射在光度模糊区域的重建问题，PolarGS通过整合偏振信息显著提升了表面重建的几何精度和完整性。
...
### [71] [CircleFlow: Flow-Guided Camera Blur Estimation using a Circle Grid Target](https://arxiv.org/abs/2512.00796)
...
<details>
  <summary>Details</summary>
Motivation: 点扩散函数（PSF）作为连接真实场景与捕获信号的基本描述符，表现为相机模糊。准确的PSF估计对于光学表征和计算视觉都至关重要，但由于强度反卷积固有的模糊性和不适定性，这仍然具有挑战性

Method: CircleFlow采用流引导边缘定位框架：1）通过成像圆网格目标进行结构化捕获，编码局部各向异性和空间变化的PSF；2）利用目标的二值亮度先验解耦图像和核估计；3）通过光流引导的初始化二值结构亚像素对齐重建潜在清晰图像；4）将PSF建模为能量约束的隐式神经表示；5）在去马赛克感知的可微分框架中联合优化两个组件
...
### [72] [Thinking with Drafts: Speculative Temporal Reasoning for Efficient Long Video Understanding](https://arxiv.org/abs/2512.00805)
*Pengfei Hu,Meng Cao,Yingyao Wang,Yi Wang,Jiahua Dong,Jun Song,Yu Cheng,Bo Zheng,Xiaodan Liang*

Main category: cs.CV

TL;DR: SpecTemp是一个基于强化学习的推测性时序推理框架，通过解耦时序感知与推理的双模型协作设计，解决了长视频理解中的效率瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 长视频理解对类人智能至关重要，但现有的"thinking-with-frames"范式存在效率瓶颈，因为多模态上下文会逐渐增长且冗余。需要一种既能保持推理能力又能提高效率的方法。
...
Conclusion: SpecTemp通过模仿人脑协作路径的双模型设计，有效平衡了长视频理解中的效率与准确性，为解决多模态大语言模型在长视频处理中的效率瓶颈提供了新思路。
...
### [73] [IRPO: Boosting Image Restoration via Post-training GRPO](https://arxiv.org/abs/2512.00814)
*Haoxuan Xu. Yi Liu,Boyuan Jiang,Jinlong Peng,Donghao Luo,Xiaobin Hu,Shuicheng Yan,Haoang Li*

Main category: cs.CV

TL;DR: IRPO是一个基于GRPO的低层视觉后训练范式，通过数据选择和奖励建模解决图像恢复任务中的过平滑和泛化问题，在多个基准测试中取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有图像恢复方法依赖像素级硬拟合真实图像，存在过平滑和泛化能力差的问题。后训练范式在高层生成任务中表现出色，但在低层视觉领域探索不足。

Method: 1. 数据选择原则：从预训练阶段选择表现不佳的样本进行后训练；2. 奖励建模系统：包含通用奖励（结构保真）、专家奖励（利用Qwen-VL进行感知对齐）和恢复奖励（任务特定低层质量）。

Result: 在6个域内和5个域外低层基准测试中，IRPO在多种退化类型上取得最先进结果，相比AdaIR基线在域内任务提升0.83dB，域外设置提升3.43dB。

Conclusion: IRPO通过创新的数据选择和奖励建模策略，有效解决了低层视觉后训练的关键挑战，为图像恢复任务提供了新的范式，在性能和泛化能力上均有显著提升。
...
### [74] [PanFlow: Decoupled Motion Control for Panoramic Video Generation](https://arxiv.org/abs/2512.00832)
*Cheng Zhang,Hanwen Liang,Donny Y. Chen,Qianyi Wu,Konstantinos N. Plataniotis,Camilo Cruz Gambardella,Jianfei Cai*

Main category: cs.CV

TL;DR: PanFlow是一种新颖的全景视频生成方法，通过解耦相机旋转与光流条件，实现了对大动态运动的精确控制，并引入球面噪声扭曲策略保证边界一致性。


<details>
  <summary>Details</summary>
Motivation: 现有全景视频生成方法缺乏明确的运动控制，难以生成具有大而复杂运动的场景，限制了在虚拟现实和沉浸式媒体中的应用。

Method: 利用全景图的球面特性，将高度动态的相机旋转从输入光流条件中解耦；引入球面噪声扭曲策略促进全景边界上的运动循环一致性；构建大规模、运动丰富的全景视频数据集进行训练。

Result: PanFlow在运动保真度、视觉质量和时间一致性方面显著优于现有方法，并在运动转移和视频编辑等应用中展示了有效性。

Conclusion: PanFlow通过创新的运动解耦和球面噪声扭曲策略，实现了对全景视频中大动态运动的精确控制，为全景视频生成提供了更有效的解决方案。
...
### [75] [AFRAgent : An Adaptive Feature Renormalization Based High Resolution Aware GUI agent](https://arxiv.org/abs/2512.00846)
*Neeraj Anand,Rishabh Jain,Sohan Patnaik,Balaji Krishnamurthy,Mausoom Sarkar*

Main category: cs.CV

TL;DR: AFRAgent是一种基于instruct-BLIP的多模态架构，用于移动UI自动化，通过自适应特征重归一化技术增强图像嵌入，在保持模型小尺寸的同时实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 移动UI自动化需求增长，现有视觉语言模型在GUI自动化中存在两个主要问题：1) 由于视觉编码器特征空间信息有限，难以准确识别控件和确定操作；2) 高性能模型通常体积庞大，需要大量训练且推理延迟高。

Method: 提出AFRAgent，基于instruct-BLIP的多模态架构，引入自适应特征重归一化技术（基于token级别的仿射变换），有效丰富低分辨率图像嵌入并融合高分辨率细节，增强LLM管道中的图像嵌入。

Result: AFRAgent在Meta-GUI和AITW基准测试中建立了新的最先进基线，性能优于竞争对手，同时模型尺寸不到最近竞争对手的四分之一。
...
### [76] [Smol-GS: Compact Representations for Abstract 3D Gaussian Splatting](https://arxiv.org/abs/2512.00850)
*Haishan Wang,Mohammad Hassan Vali,Arno Solin*

Main category: cs.CV

TL;DR: Smol-GS提出了一种用于3D高斯泼溅的紧凑表示学习方法，通过递归体素层次结构和泼溅特征编码，在保持高质量渲染的同时实现大幅压缩


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）需要大量存储空间，限制了其在资源受限环境中的应用。需要一种既能保持渲染质量又能大幅压缩3D场景表示的方法

Method: 采用递归体素层次结构捕获泼溅坐标，同时使用泼溅特征存储颜色、透明度、变换和材质属性等抽象信息，将空间和语义信息集成到高效编码中

Result: 在标准基准测试中实现了最先进的压缩效果，同时保持高渲染质量，压缩幅度达到数量级级别

Conclusion: Smol-GS提供了一种灵活且高效的3D场景表示方法，不仅保持视觉保真度，还能为导航、规划和3D场景理解等下游任务提供基础
...
### [77] [TAP-CT: 3D Task-Agnostic Pretraining of Computed Tomography Foundation Models](https://arxiv.org/abs/2512.00872)
*Tim Veenboer,George Yiasemis,Eric Marcus,Vivien Van Veldhuizen,Cees G. M. Snoek,Jonas Teuwen,Kevin B. W. Groot Lipman*

Main category: cs.CV

TL;DR: TAP-CT是一种针对3D CT数据的任务无关预训练方法，通过改进Vision Transformers和DINOv2架构，实现无需大量微调即可获得强大特征表示的医学影像基础模型。


<details>
  <summary>Details</summary>
Motivation: 现有医学领域基础模型通常需要大量微调或依赖资源密集的解码器训练，且许多编码器的预训练目标偏向特定任务，因此需要一个强大的、任务无关的基础模型，只需特征提取即可适应下游任务。

Method: 提出TAP-CT方法：对Vision Transformers和DINOv2进行简单而有效的适配，使其适用于体数据，直接在3D CT体数据上进行可扩展的自监督预训练。关键改进包括针对性的补丁嵌入、位置编码和体数据增强修改，使架构具有深度感知能力同时保持底层架构的简洁性。

Result: 在包含105K CT体数据的大规模内部数据集上进行3D预训练，获得了稳定、鲁棒的冻结表示，这些表示在下游任务中表现出强大的泛化能力。

Conclusion: TAP-CT为医学影像研究提供了一个强大的低资源基准，所有预训练模型、实验配置和下游基准代码将在Hugging Face上开源，以促进透明度和可复现性。
...
### [78] [Quantum-Inspired Spectral Geometry for Neural Operator Equivalence and Structured Pruning](https://arxiv.org/abs/2512.00880)
*Haijian Shao,Wei Liu,Xing Deng*

Main category: cs.CV

TL;DR: 提出量子启发的几何框架，通过布洛赫超球上的归一化奇异值谱表示神经算子，建立谱与功能的等价关系，实现跨模态和跨架构的算子可替换性。


<details>
  <summary>Details</summary>
Motivation: 资源受限和异构的本地硬件上多模态智能快速增长，面临多模态特征异质性、动态场景实时需求、硬件特定算子冗余等关键瓶颈。

Method: 引入量子启发的几何框架，将每个算子用其在布洛赫超球上的归一化奇异值谱表示；证明谱到功能的等价定理；提出量子度量驱动的功能冗余图和一次性结构化剪枝。

Result: 受控模拟验证了所提度量优于幅度和随机基线；大规模多模态transformer和异构硬件（华为昇腾、寒武纪MLU、昆仑芯）的广泛实验验证将在扩展期刊版本中展示。

Conclusion: 建立了跨模态和跨架构算子可替换性的首个严格理论基础，为解决异构硬件上的多模态智能瓶颈提供了量子启发的几何框架。
...
### [79] [Look, Recite, Then Answer: Enhancing VLM Performance via Self-Generated Knowledge Hints](https://arxiv.org/abs/2512.00882)
*Xisheng Feng*

Main category: cs.CV

TL;DR: 提出"Look, Recite, Then Answer"框架解决VLMs在专业领域中的推理幻觉问题，通过三阶段推理流程提升精度农业任务性能
...
Method: 提出参数高效的"Look, Recite, Then Answer"框架：1) Look阶段生成客观视觉描述和候选集；2) Recite阶段使用轻量级1.7B路由器将视觉线索转化为针对性查询，触发候选特定参数知识；3) Answer阶段并行证据对齐，选择最一致的标签

Result: 在AgroBench上取得SOTA结果，杂草识别准确率比Qwen2-VL-72B提升23.52%，超越GPT-4o且无需外部搜索开销

Conclusion: 该模块化设计通过将被动感知转化为主动可控的知识检索，有效缓解了幻觉问题，为专业领域VLM应用提供了高效解决方案
...
### [80] [HanDyVQA: A Video QA Benchmark for Fine-Grained Hand-Object Interaction Dynamics](https://arxiv.org/abs/2512.00885)
*Masatoshi Tateno,Gido Kato,Hirokatsu Kataoka,Yoichi Sato,Takuma Yagi*

Main category: cs.CV

TL;DR: HanDyVQA是一个细粒度视频问答基准，专注于手-物交互的动态特性，包含6种问题类型和11.1K个多选题对，用于评估模型对手-物交互中操作和效果的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有语义手-物交互基准要么关注操作，要么关注粗粒度的效果，缺乏细粒度的时空推理来捕捉手-物交互中的动态特性。需要一个新的基准来全面覆盖手-物交互的操作和效果两个方面。

Method: 提出了HanDyVQA基准，包含六种互补的问题类型：动作、过程、物体、位置、状态变化和物体部件。总共收集了11.1K个多选题对，识别操作风格、手/物体运动和部件级状态变化。还包括10.3K个分割掩码用于物体和物体部件问题。

Result: 评估了最新的视频基础模型，发现即使表现最好的Gemini-2.5-Pro模型也只达到73%的平均准确率，远低于人类表现（97%）。分析显示模型在空间关系、运动和部件级几何理解方面仍面临挑战。将显式的手-物交互线索整合到视觉特征中可以提高性能。

Conclusion: HanDyVQA基准为评估模型对手-物交互动态的理解提供了全面的测试平台。当前模型与人类表现存在显著差距，特别是在细粒度时空推理方面。整合手-物交互相关线索可以提升模型性能，为开发更深入理解手-物交互动态的未来模型提供了见解。
...
### [81] [Multilingual Training-Free Remote Sensing Image Captioning](https://arxiv.org/abs/2512.00887)
*Carlos Rebelo,Gil Rocha,João Daniel Silva,Bruno Martins*

Main category: cs.CV

TL;DR: 提出首个无需训练的多语言遥感图像描述方法，基于检索增强提示，在四个基准数据集上验证了与全监督英语系统相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有遥感图像描述方法依赖大规模标注数据集且主要针对英语，限制了全球适用性。需要开发无需训练、支持多语言的方法来提升包容性和可扩展性。

Method: 提出基于检索增强提示的训练免费方法：1) 使用领域适应的SigLIP2编码器检索相关描述和少样本示例；2) 探索两种变体：仅文本提示的多语言LLM和联合处理图像与提示的VLM；3) 引入基于PageRank的图重排序策略提升检索内容一致性。

Result: 在四个基准数据集、十种语言上验证：1) 性能与全监督英语系统相当；2) PageRank重排序带来高达35%的性能提升；3) VLM生成视觉基础但词汇多样的描述，LLM在BLEU和CIDEr得分上更强；4) 直接生成目标语言描述优于翻译策略。

Conclusion: 该工作首次系统评估了多语言、无需训练的遥感图像描述方法，推动了更具包容性和可扩展性的多模态地球观测系统发展。
...
### [82] [Accelerating Streaming Video Large Language Models via Hierarchical Token Compression](https://arxiv.org/abs/2512.00891)
*Yiyu Wang,Xuyang Liu,Xiyan Gui,Xinying Lin,Boxue Yang,Chenfei Liao,Tailai Chen,Linfeng Zhang*

Main category: cs.CV

TL;DR: STC是一个用于流式视频大语言模型的层次化框架，通过缓存和剪枝视觉令牌来加速处理，在保持高准确率的同时显著降低延迟。
...
Result: 在四个基线流式VideoLLM和五个基准测试上的实验表明，STC优于其他压缩方法。在ReKV框架上保持99%准确率的同时，将ViT编码延迟降低24.5%，LLM预填充延迟降低45.3%。

Conclusion: STC是一个即插即用的层次化框架，能够有效优化流式视频大语言模型的处理效率，在保持准确性的同时显著降低计算延迟，适用于实时部署场景。
...
### [83] [SwiftVLA: Unlocking Spatiotemporal Dynamics for Lightweight VLA Models at Minimal Overhead](https://arxiv.org/abs/2512.00903)
...
TL;DR: SwiftVLA是一种高效的视觉-语言-动作模型，通过4D理解和轻量化设计，在保持性能的同时大幅减少计算资源和内存占用。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型参数庞大，实用性受限；轻量级模型又牺牲了时空推理能力。虽然加入3D输入能有所帮助，但仍依赖大型模型且缺乏时间维度理解。

Method: 1) 使用预训练的4D视觉几何变换器提取4D特征；2) 引入融合标记通过未来预测目标生成统一表示；3) 采用掩码重构策略训练模型学习4D表示，推理时可丢弃4D分支。

Result: 在真实和模拟环境中，SwiftVLA优于轻量级基线，性能媲美参数大7倍的模型，在边缘设备上速度快18倍，内存占用减少12倍。

Conclusion: SwiftVLA通过创新的4D理解和轻量化设计，实现了高效实用的视觉-语言-动作模型，在保持性能的同时显著提升了部署效率。
...
### [84] [Hierarchical Semantic Alignment for Image Clustering](https://arxiv.org/abs/2512.00904)
...
Method: 提出分层语义对齐方法CAE：1) 从WordNet选择相关名词和从caption数据集选择描述构建语义空间；2) 通过最优传输将图像特征与选定的名词和caption对齐，获得更具区分性的语义空间；3) 结合增强的语义特征和图像特征进行聚类。

Result: 在8个数据集上的广泛实验证明该方法有效，在ImageNet-1K数据集上，准确率比最先进的无需训练方法提升4.2%，调整兰德指数提升2.9%。

Conclusion: CAE通过结合caption级细粒度属性和noun级高层类别两种互补语义，有效解决了名词歧义问题，在无需训练的情况下显著提升了图像聚类性能。
...
### [85] [ForamDeepSlice: A High-Accuracy Deep Learning Framework for Foraminifera Species Classification from 2D Micro-CT Slices](https://arxiv.org/abs/2512.00912)
*Abdelghafour Halimi,Ali Alibrahim,Didier Barradas-Bautista,Ronell Sicat,Abdulkader M. Afifi*

Main category: cs.CV

TL;DR: 该研究开发了一个深度学习流程，利用3D扫描得到的2D微CT切片，自动分类12种有孔虫物种，最终集成模型在测试集上达到95.64%的准确率。


<details>
  <summary>Details</summary>
Motivation: 开发AI辅助的微古生物学识别系统，建立有孔虫分类的自动化方法，填补深度学习与应用地球科学之间的鸿沟。

Method: 使用97个微CT扫描标本构建数据集，选取12个代表性物种，采用标本级数据分割防止数据泄露，评估7种先进的2D CNN架构，最终结合ConvNeXt-Large和EfficientNetV2-Small构建集成模型。

Result: 集成模型测试准确率达到95.64%，top-3准确率为99.6%，所有物种的ROC曲线下面积(AUC)为0.998。开发了交互式高级仪表板，支持实时切片分类和3D切片匹配。

Conclusion: 该研究为AI辅助的微古生物学识别建立了新的基准，提供了完全可复现的有孔虫分类研究框架，推动了深度学习在地球科学中的应用。
...
### [86] [SceneProp: Combining Neural Network and Markov Random Field for Scene-Graph Grounding](https://arxiv.org/abs/2512.00936)
*Keita Otani,Tatsuya Harada*

Main category: cs.CV

TL;DR: SceneProp：一种将场景图定位重新表述为马尔可夫随机场中最大后验推理问题的新方法，通过全局推理处理复杂关系查询，性能随查询图复杂度提升而提升


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理包含多个对象和关系的复杂组合查询时存在困难。标准短语定位方法擅长定位单个对象，但缺乏解析复杂关系描述的结构归纳偏置。现有的场景图定位方法也存在性能随查询图增大而下降的悖论，未能充分利用本应使定位更容易的信息。

Method: 将场景图定位重新表述为马尔可夫随机场中的最大后验推理问题，通过可微分的信念传播算法进行全局推理，找到图像区域到查询图节点的最优分配，满足所有约束条件，形成端到端框架。

Result: 在四个基准测试上的实验表明，SceneProp显著优于先前工作。关键的是，其准确性随着查询图的大小和复杂度的增加而持续提升，首次证明更多关系上下文可以且应该带来更好的定位效果。

Conclusion: SceneProp通过将场景图定位重新表述为马尔可夫随机场中的最大后验推理问题，解决了现有方法在处理复杂关系查询时的结构缺陷。该方法不仅性能优于先前工作，更重要的是展示了关系上下文对定位的积极影响，为复杂视觉查询的定位提供了新思路。
...
### [87] [Binary-Gaussian: Compact and Progressive Representation for 3D Gaussian Segmentation](https://arxiv.org/abs/2512.00944)
*An Yang,Chenyu Liu,Jun Du,Jianqing Gao,Jia Pan,Jinshui Hu,Baocai Yin,Bing Yin,Cong Liu*

Main category: cs.CV

TL;DR: 本文提出了一种用于3D高斯泼溅分割的粗到细二进制编码方案，通过二进制到十进制映射将每个特征压缩为单个整数，大幅减少内存使用，并设计了渐进式训练策略提升细粒度分割能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅分割方法依赖高维类别特征导致内存开销大，细粒度分割面临标签空间拥挤和缺乏稳定多粒度控制机制的问题。
...
Conclusion: 该方法通过创新的编码方案和训练策略，有效解决了3D高斯泼溅分割中的内存开销和细粒度分割挑战，为3D语义任务提供了高效解决方案。
...
### [88] [Adaptive Evidential Learning for Temporal-Semantic Robustness in Moment Retrieval](https://arxiv.org/abs/2512.00953)
*Haojian Huang,Kaijing Ma,Jin Chen,Haodong Chen,Zhou Wu,Xianghao Zang,Han Fang,Chao Ban,Hao Sun,Mulin Chen,Zhongjiang He*

Main category: cs.CV

TL;DR: 该论文提出了一种用于视频时刻检索的去偏证据学习框架（DEMR），通过反射翻转融合块和查询重构任务解决模态不平衡问题，使用几何正则化器改进不确定性估计，在标准数据集和去偏数据集上表现出更好的效果和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在视频时刻检索领域，传统方法使用预训练模型难以处理细粒度信息和确定性推理，导致与复杂或模糊时刻对齐困难。现有方法缺乏对复杂视频场景的适应性，证据回归方法存在模态不平衡和不确定性估计偏差问题。

Method: 提出DEMR框架，包含：1）反射翻转融合（RFF）块用于跨模态对齐；2）查询重构任务增强文本敏感性；3）几何正则化器（Geom-regularizer）改进不确定性预测，实现与困难时刻的自适应对齐。

Result: 在标准数据集和去偏数据集ActivityNet-CD和Charades-CD上进行广泛测试，显示出在效果、鲁棒性和可解释性方面的显著提升，为时刻检索的时态语义鲁棒性提供了有前景的解决方案。

Conclusion: DEMR框架通过解决模态不平衡和不确定性估计偏差问题，显著提高了视频时刻检索的准确性和鲁棒性，为复杂视频场景下的时态语义对齐提供了有效的解决方案。
...
TL;DR: 提出一个结合语义感知医学图像重建与轻量级区块链溯源层的框架，旨在提升医学图像重建的解剖结构保真度和可追溯性


<details>
  <summary>Details</summary>
Motivation: 现实世界医学图像常受损坏、噪声和篡改影响，传统重建方法注重像素级恢复但可能损害解剖结构保真度，这直接影响临床结果可靠性

Method: 提出语义感知医学图像重建框架，集成高级潜在嵌入与混合U-Net架构以保留临床相关结构；同时采用基于无标度图设计的轻量级区块链溯源层，记录可验证的重建事件

Result: 在多个数据集和损坏类型上的广泛评估显示，相比现有方法，该方法在结构一致性、重建准确性和溯源完整性方面均有提升

Conclusion: 通过将语义引导重建与安全可追溯性相结合，该方案推进了医学图像中可信赖AI的发展，增强了医疗环境中的诊断信心和监管合规性
...
TL;DR: MM-ACT是一个统一的视觉-语言-动作模型，通过共享令牌空间集成文本、图像和动作，采用重新掩码并行解码策略提高效率，在多个机器人任务上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略需要语义理解进行任务规划，同时需要预测能力与环境交互。现有方法通常分开处理这些能力，缺乏统一的跨模态学习框架。

Method: 提出MM-ACT统一模型，在共享令牌空间中集成文本、图像和动作；采用重新掩码并行解码策略处理文本和图像生成，一步并行解码策略处理动作生成；引入上下文共享多模态学习训练范式。

Result: 在LIBERO仿真中成功率96.3%，真实Franka机器人三个任务中72.0%，RoboTwin2.0八个双手任务中52.38%，跨模态学习带来额外9.25%性能提升。

Conclusion: MM-ACT通过统一的视觉-语言-动作建模和上下文共享多模态学习，有效提升了机器人策略的语义理解和动作生成能力，在多个机器人任务上表现出色。
...
TL;DR: PhotoFramer是一个多模态构图指导框架，通过自然语言描述和示例图像生成来帮助用户改善照片构图


<details>
  <summary>Details</summary>
Motivation: 许多普通用户在拍照时难以获得良好构图，需要实用的构图指导工具来帮助他们改善照片质量

Method: 构建大规模数据集，将构图指导分为平移、放大和视角变换三个子任务；使用两阶段流水线合成训练数据，微调多模态模型同时生成文本指导和示例图像

Result: 文本指令能有效引导图像构图，结合示例图像的方法比仅使用示例的基线有持续改进

Conclusion: PhotoFramer向构图助手迈出了实用一步，使专业摄影知识能够被普通用户使用
...
<details>
  <summary>Details</summary>
Motivation: 尽管缩放定律和经验结果表明增大Vision Transformers通常能提升性能，但模型准确率和训练行为并不总是随规模单调增加。研究者想探索在ViT-B/16模型中是否存在过度参数化问题，以及是否可以通过更有效的参数分配来改进性能。

Method: 提出了两种应用于MLP块的参数减少策略：1) GroupedMLP：在相邻transformer块之间共享MLP权重；2) ShallowMLP：将MLP隐藏维度减半。两种方法分别减少了32.7%的参数。

Result: GroupedMLP达到81.47% top-1准确率（保持基线计算成本），ShallowMLP达到81.25% top-1准确率（推理吞吐量提升38%）。两者都优于86.6M参数的基线模型（81.05%），且训练稳定性显著改善，峰值到最终准确率下降从0.47%降至0.03%-0.06%。

Conclusion: 对于ImageNet-1K上的ViT-B/16模型，MLP层存在过度参数化，减少MLP容量不会损害性能甚至能轻微提升。架构约束如参数共享和宽度减少可作为有用的归纳偏置，参数分配方式在Vision Transformer设计中很重要。
...
TL;DR: S2AM3D：通过2D分割先验与3D一致性监督解决点云部件分割中的泛化不足和视角不一致问题，引入点一致性编码器和尺度感知提示解码器，并构建大规模高质量数据集


<details>
  <summary>Details</summary>
Motivation: 现有部件级点云分割研究面临两大挑战：1）原生3D模型因数据稀缺而泛化能力不足；2）引入2D预训练知识常导致不同视角下的分割结果不一致。需要解决这些限制以提升点云分割的鲁棒性和一致性。

Method: 提出S2AM3D框架：1）设计点一致性部件编码器，通过原生3D对比学习聚合多视角2D特征，生成全局一致的点特征；2）提出尺度感知提示解码器，通过连续尺度信号实现分割粒度的实时调整；3）构建包含超过10万个样本的大规模高质量部件级点云数据集，为模型训练提供充足监督信号。

Result: 大量实验表明，S2AM3D在多种评估设置下均取得领先性能，在处理复杂结构和尺寸差异显著的部件时表现出卓越的鲁棒性和可控性。

Conclusion: S2AM3D通过结合2D分割先验与3D一致性监督，有效解决了点云部件分割中的泛化不足和视角不一致问题，为部件级点云分割提供了鲁棒且可控的解决方案。
...
TL;DR: CycliST是一个评估视频语言模型在周期性状态转换文本推理能力的新基准数据集，通过合成视频序列测试模型对周期性模式的识别和理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前视频语言模型在理解周期性动态模式方面存在局限，缺乏时间理解和定量分析能力，需要专门的评估工具来推动模型在周期性模式理解方面的进步。

Method: 创建包含周期性物体运动和视觉属性变化的合成视频数据集，采用分层评估系统，通过增加循环物体数量、场景杂乱度和光照条件变化来逐步提高难度。

Result: 实验显示当前最先进的视频语言模型在泛化到线性/轨道运动等周期性动态以及颜色/尺度等时间相关视觉属性变化方面存在显著局限，无法可靠检测和利用循环模式。

Conclusion: CycliST基准数据集揭示了视频语言模型在周期性模式理解方面的技术差距，为开发超越当前技术水平、能更好理解周期性模式的视觉推理模型提供了评估框架。
...
TL;DR: 本文发现当前视觉语言模型在社交感知任务上存在负迁移问题，提出SocialFusion框架解决这一问题，实现跨五个社交任务的正向迁移和性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前强大的预训练视觉语言模型虽然在通用能力上表现优异，但在同时学习多个社交感知任务时却表现出负迁移现象，这阻碍了AI社交能力的发展。研究者发现这是由于预训练过程导致视觉编码器表示细微社交信息的能力受损，即"社交退化"问题。

Method: 提出SocialFusion统一框架，在冻结的视觉编码器和语言模型之间学习最小连接。通过线性表示探测和梯度冲突分析两个视角研究社交退化现象，发现预训练过程显著损害了视觉编码器对社交信息的可解码性。

Result: SocialFusion在五个社交任务上表现出正向迁移，利用任务间的协同效应提升整体性能，在多个基准测试中达到与任务专用最先进模型相当的性能。

Conclusion: 当前VLM预训练策略可能不利于获得通用社交能力，需要更注重社交意识的训练范式。SocialFusion框架为解决这一问题提供了有效方案。
...
TL;DR: LISA-3D是一个两阶段框架，通过将语言图像分割提升到3D，使用几何感知的LoRA层改进指令跟随模型LISA，并重用冻结的SAM-3D重建器，实现文本驱动的3D重建。


<details>
  <summary>Details</summary>
Motivation: 文本驱动的3D重建需要一个能够同时理解开放词汇指令并在不同视角下保持一致的掩码生成器。现有方法在这方面存在局限性。

Method: 采用两阶段框架：1) 通过几何感知的Low-Rank Adaptation层改进LISA模型；2) 重用冻结的SAM-3D重建器。利用现成的RGB-D序列和相机姿态构建可微分重投影损失，强制跨视图一致性，无需额外的3D-文本监督。

Result: 在ScanRefer和Nr3D数据集上，LISA-3D将语言到3D的准确性提高了最多+15.6个百分点，仅需适应1160万个参数。系统模块化、数据高效，支持对未见类别的零样本部署。

Conclusion: LISA-3D为语言引导的3D内容创建提供了一个实用的解决方案，具有模块化、数据高效和零样本部署能力，显著提升了文本驱动3D重建的性能。
...
TL;DR: 本文提出DPAC方法，通过将对抗梯度投影到生成分数几何定义的切空间，在保持攻击成功率的同时减少分布漂移，提高样本质量。


<details>
  <summary>Details</summary>
Motivation: 传统的对抗性扩散采样虽然能实现目标类别分类，但样本质量会随着对抗控制轨迹与名义轨迹之间的偏差累积而下降。需要一种既能保持攻击成功率又能最小化分布漂移的方法。

Method: 基于随机最优控制视角，将轨迹退化形式化为路径空间KL散度，推导出最优控制的一阶条件。提出DPAC方法，将对抗梯度投影到由生成分数几何定义的切空间，保留分类增益的同时最小化分布漂移。

Result: 理论分析表明DPAC在离散求解器中能将Wasserstein距离的误差项从O(Δt)降低到O(Δt²)，且对分数或度量近似具有二阶鲁棒性。在ImageNet-100上的实验验证了DPAC在相同攻击成功率下获得更低的FID和路径KL散度。

Conclusion: DPAC方法通过几何投影策略，在扩散引导中实现了攻击效果与样本质量之间的更好平衡，为对抗性扩散采样提供了理论指导和实用解决方案。
...
TL;DR: Lotus-2是一个两阶段确定性框架，通过优化预训练扩散模型的几何先验，实现稳定、准确、细粒度的单目几何密集预测，仅用少量训练数据就达到了最先进的深度估计效果。


<details>
  <summary>Details</summary>
Motivation: 单目图像几何恢复存在根本性的不适定性问题。现有方法要么依赖大规模监督数据但缺乏物理推理，要么直接使用扩散模型的随机生成公式对确定性几何推理不理想。需要一种能充分利用预训练生成先验的优化适配方案。

Method: 提出两阶段确定性框架：第一阶段使用单步确定性公式和局部连续性模块生成全局一致结构；第二阶段在核心预测器定义的流形内进行约束多步整流流细化，通过无噪声确定性流匹配增强细粒度几何。

Result: 仅使用59K训练样本（不到现有大规模数据集的1%），在单目深度估计上达到新的最先进结果，在表面法线预测上也表现出高度竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 传统目标检测模型（如YOLO）在农业等实时决策应用中面临数据集标注效率低下的挑战，传统标注方法劳动密集且耗时

Method: 开发实时标注系统，在边缘设备上部署YOLO模型，实现图像采集时的即时标注；对YOLOv5、YOLOv8、YOLOv12三种架构进行对比分析，评估单类vs多类标注、预训练vs从头训练等配置

Result: 预训练和单类配置在模型收敛、性能和鲁棒性方面表现显著优势；实时标注框架能大幅减少数据集准备时间，同时保持高标注质量

Conclusion: 实时标注框架具有可行性和有效性，为农业等实时应用中的目标检测模型部署提供了高效的数据集准备解决方案
...
TL;DR: TRoVe是一个自动化方法，用于发现时序视觉语言模型中导致错误的静态特征偏差，通过提取候选静态特征并评估其对分类错误的影响以及模型对特征的依赖程度来识别偏差。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理时序理解任务时可能依赖静态特征偏差（如背景或物体特征）而非动态视觉变化，这些偏差会导致下游任务的系统性预测错误，因此在模型部署前识别和表征这些错误诱导的静态特征偏差至关重要。

Method: TRoVe方法包括：1）从标注的验证数据集中提取候选静态特征；2）对每个特征进行评分，评估其对分类错误的影响以及模型在预测时对该特征的依赖程度；3）引入包含101个训练好的时序VLM及其学习到的静态特征偏差标注的评估框架。

Result: TRoVe能够准确识别VLM中的错误诱导静态特征偏差，相比最接近的基线方法提升了28.6%的性能。应用于7个现成的VLM和2个时序理解任务时，发现了先前未知的静态特征偏差，并证明了解学习到的偏差知识有助于提升模型在测试时的性能。

Conclusion: TRoVe提供了一个有效的自动化方法来发现和表征视觉语言模型中的静态特征偏差，这对于理解模型行为、减少系统性错误以及在实际部署前评估模型可靠性具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态媒体篡改检测方法普遍忽视局部信息的重要性，而篡改往往发生在特定区域（尤其是面部区域），这影响了检测的准确性和可靠性。

Method: 提出M4-BLIP框架：1）使用BLIP-2模型提取局部特征；2）引入局部面部信息作为先验知识；3）设计专门的对齐和融合模块整合局部与全局特征；4）与大语言模型（LLM）集成提升结果可解释性。

Result: 通过大量定量和可视化实验验证，M4-BLIP框架在检测准确性方面优于当前最先进的竞争对手。

Conclusion: M4-BLIP框架通过有效整合局部特征与全局信息，显著提升了多模态媒体篡改检测的准确性和可解释性，为解决数字信息完整性威胁提供了有效方案。
...
TL;DR: S²-MLLM：通过隐式空间推理增强多模态大语言模型在3D视觉定位中的空间理解能力，避免依赖低效的点云重建


<details>
  <summary>Details</summary>
Motivation: 现有的3D视觉定位方法主要依赖点云重建来为MLLMs提供显式结构指导，这种方法效率低下且空间推理能力有限。MLLMs主要处理2D视觉输入，难以仅从有限视角理解3D场景的空间结构。

Method: 提出S²-MLLM框架，采用隐式空间推理策略：1）利用前馈3D重建的结构感知能力进行空间引导；2）设计结构增强模块（SE），使用视图内和视图间注意力机制捕获依赖关系；3）集成多层次位置编码，将视觉表示与空间位置和视角信息关联。

Result: 在ScanRefer、Nr3D和Sr3D数据集上的广泛实验表明，S²-MLLM在性能、泛化能力和效率方面均优于现有方法，实现了显著的性能提升。

Conclusion: S²-MLLM通过隐式空间推理有效增强了MLLMs在3D视觉定位中的空间理解能力，避免了低效的点云重建过程，实现了性能、泛化和效率的统一。
...
<details>
  <summary>Details</summary>
Motivation: 掩码生成模型（MGM）虽然能生成高质量图像，但需要大量采样步骤导致推理速度慢。传统蒸馏方法将加速问题视为分布匹配问题，但作者认为可以将其重新定义为强化学习问题。

Method: 提出Speed-RL范式，将加速问题视为强化学习问题。结合质量奖励和速度奖励，使用强化学习微调基础模型，优化目标是生成高质量图像的同时减少步骤。

Result: 实验表明，该方法能够将基础模型加速3倍，同时保持可比的图像质量。

Conclusion: Speed-RL提供了一种新颖的强化学习范式来加速预训练掩码生成模型，在保持图像质量的同时显著提升推理速度。
...
TL;DR: SlotSPE：基于槽位的结构预后事件建模框架，通过压缩多模态输入为互斥槽位表示，有效捕捉稀疏但关键的预后信号，在10个癌症基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前方法难以高效建模组织学图像和基因谱之间的模态内和模态间交互，且难以捕捉稀疏、患者特异性、未标注但决定患者预后的关键结构信号（如空间组织学模式或通路共激活）。

Method: 提出SlotSPE框架，受因子编码原理启发，使用槽注意力将每个患者的多模态输入压缩为紧凑、模态特异性且互斥的槽位集合，将这些槽位表示作为预后事件的编码，实现复杂交互建模并融入生物学先验知识。

Result: 在10个癌症基准测试中，SlotSPE在8个队列中优于现有方法，整体性能提升2.9%，在基因组数据缺失情况下保持鲁棒性，并通过结构化事件分解显著提升可解释性。

Conclusion: SlotSPE通过槽位表示有效捕捉稀疏的结构预后事件，实现了多模态癌症生存预测的改进，在性能、鲁棒性和可解释性方面均表现出色。
...
TL;DR: OmniFD是一个统一的人脸伪造检测框架，在单一模型中同时处理图像分类、视频分类、空间定位和时间定位四个核心任务，通过共享编码器和跨任务交互实现高效的多任务学习。


<details>
  <summary>Details</summary>
Motivation: 当前人脸伪造检测方法通常使用任务特定的独立模型，导致计算冗余且忽略了相关任务之间的潜在关联。需要一种统一框架来同时处理多个核心任务。

Method: OmniFD包含三个主要组件：1) 共享的Swin Transformer编码器，从图像和视频输入中提取统一的4D时空表示；2) 带有可学习查询的跨任务交互模块，通过基于注意力的推理动态捕捉任务间依赖关系；3) 轻量级解码头，将精炼表示转换为所有FFD任务的预测。

Result: 实验表明OmniFD优于任务特定模型，通过多任务学习捕获跨任务的通用表示，特别是实现了细粒度知识转移。例如，当加入图像数据时，视频分类准确率提高了4.63%。统一框架减少了63%的模型参数和50%的训练时间。

Conclusion: OmniFD为实际应用中的人脸伪造检测提供了一个实用且可泛化的解决方案，通过统一图像、视频和四个任务在一个框架中，实现了在多样化基准测试中的卓越性能、高效率和可扩展性。
...
TL;DR: MedSegLatDiff：基于变分自编码器和潜在扩散模型的高效医学图像分割框架，能生成多样化的分割假设和置信度图


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割方法只生成单一分割掩码，无法捕捉固有的不确定性；现有生成模型虽然能模拟多位临床医生的协同解释，但计算量大

Method: 结合变分自编码器（VAE）和潜在扩散模型：VAE将输入压缩到低维潜在空间以减少噪声并加速训练，扩散过程直接在紧凑表示中操作；在VAE掩码重建路径中用加权交叉熵替换传统MSE损失以更好保留微小结构

Result: 在ISIC-2018（皮肤病变）、CVC-Clinic（息肉）和LIDC-IDRI（肺结节）数据集上评估，达到最先进或极具竞争力的Dice和IoU分数，同时生成多样化的分割假设和置信度图

Conclusion: 相比确定性基线方法，提供了更好的可解释性和可靠性，特别适合临床部署
...
TL;DR: 本文提出了首个针对飞机油箱内部异物碎片检测的数据集FOD-S2R，包含真实和合成图像，用于评估合成数据在封闭环境中的检测效果，并证明合成数据能提升真实世界检测性能。


<details>
  <summary>Details</summary>
Motivation: 飞机油箱内的异物碎片存在严重安全隐患，但缺乏针对封闭复杂环境的专用数据集。现有数据集主要关注外部或开放环境，无法满足油箱内部检测需求。

Method: 创建FOD-S2R数据集，包含3,114张真实图像（在受控油箱复制品中拍摄）和3,137张合成图像（使用Unreal Engine生成）。数据集涵盖不同视场角、物体距离、光照条件、颜色和物体尺寸。

Result: 基准测试显示，引入合成数据能提高检测精度和模型在真实条件下的泛化能力，有效缩小仿真到现实的差距。

Conclusion: 合成数据能增强模型性能，为开发自动化飞机油箱异物碎片检测系统提供有价值的基础，减少对大量真实标注数据的依赖。
...
TL;DR: 该研究提出了首个全面的颅内动脉瘤血管分割数据集IAVS，包含641个3D MRA图像和587个标注，并建立了包含血流动力学分析结果的标准化CFD适用性评估系统。


<details>
  <summary>Details</summary>
Motivation: 当前颅内动脉瘤分割方法主要关注图像评估指标，忽略了在后续计算流体动力学（CFD）应用中的实际有效性。现有数据集缺乏拓扑完整性和CFD适用性考虑。

Method: 1）构建IAVS数据集，包含多中心3D MRA图像和详细的血流动力学分析结果；2）建立两阶段评估基准（动脉瘤全局定位和精细分割）；3）开发简单有效的两阶段框架作为基线方法；4）建立标准化CFD适用性评估系统，实现分割掩码到CFD模型的自动转换。

Result: 创建了首个包含血流动力学分析结果的颅内动脉瘤血管分割数据集，建立了标准化CFD适用性评估系统，提供了可用于临床相关技术开发和评估的基准框架。

Conclusion: IAVS数据集填补了现有数据集的空白，通过整合血流动力学分析结果和建立CFD适用性评估系统，为开发临床相关的分割技术提供了重要资源，有助于提升分割结果在血流动力学分析中的实际应用价值。
...
TL;DR: RICE-VL是一个评估视觉语言模型在东南亚文化理解能力的新基准，包含超过28,000个VQA样本和1,000个图像-边界框对，揭示了现有模型在文化多样性理解上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型存在西方中心偏见，在东南亚等文化多元地区的表现受限，需要建立专门的文化理解评估基准来推动更包容的模型发展。

Method: 开发了RICE-VL基准，包含11个东盟国家的28,000+人工标注VQA样本（真/假、填空、开放式）和1,000个视觉定位样本，覆盖14个子类别，并提出了SEA-LAVE评估指标来衡量文本准确性、文化对齐和国家识别能力。

Result: 对6个开源和闭源VLM的评估显示，模型在低资源国家和抽象文化领域存在显著性能差距，视觉定位任务也暴露了模型在复杂场景中定位文化元素的能力不足。

Conclusion: RICE-VL揭示了VLM在文化理解方面的局限性，强调了开发更具包容性模型以服务全球多样化人群的必要性。
...
TL;DR: VSRD++是一个弱监督的单目3D目标检测框架，通过神经场体渲染和2D监督，无需3D标注即可生成3D伪标签来训练检测器。


<details>
  <summary>Details</summary>
Motivation: 现有的单目3D目标检测方法严重依赖需要大量人工标注的3D监督信号，标注成本高昂。本文旨在开发一个无需3D标注的弱监督框架。

Method: 采用两阶段流程：1）多视图3D自动标注阶段，使用带符号距离场表示物体表面，通过实例感知体素轮廓渲染生成实例掩码，将SDF分解为长方体SDF和残差距离场来优化3D边界框，并建模动态物体的速度和置信度；2）单目3D检测器训练阶段，使用优化的3D边界框作为伪标签训练检测器。

Result: 在KITTI-360数据集上的实验表明，VSRD++在静态和动态场景中都显著优于现有的弱监督单目3D目标检测方法。

Conclusion: VSRD++成功实现了无需3D标注的单目3D目标检测，通过神经场体渲染和弱2D监督有效解决了动态物体几何不一致性问题，为3D场景理解提供了更实用的解决方案。
...
TL;DR: TabletopGen是一个无需训练、全自动的框架，用于生成多样化的实例级交互式3D桌面场景，通过参考图像输入实现高保真、物理交互的模拟场景生成。
...
Method: TabletopGen框架包含：1）参考图像输入（可由文本到图像模型合成）；2）实例分割和补全获取每个实例图像；3）3D重建和规范坐标对齐；4）姿态和尺度估计；5）组装成无碰撞、模拟就绪的桌面场景。核心创新是两阶段空间推理方法：可微分旋转优化器用于精确旋转恢复，俯视图空间对齐机制用于稳健的平移和尺度估计。

Result: 大量实验和用户研究表明，TabletopGen在视觉保真度、布局准确性和物理合理性方面达到最先进水平，显著超越现有方法，能够生成具有丰富风格和空间多样性的逼真桌面场景。

Conclusion: TabletopGen是一个有效的训练免费框架，能够自动生成高质量、物理交互的3D桌面场景，为具身AI应用提供了重要的场景生成能力，代码将公开可用。
...
TL;DR: 本文提出了一种基于模糊规则的潜在空间多路径扩散模型（DFS），用于解决传统扩散模型在处理具有显著特征差异的图像集合时的局限性，通过多路径专门学习不同特征类别，并利用规则链推理实现高效协调。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成高分辨率逼真图像方面表现出色，但在处理特征差异显著的图像集合时仍存在困难。传统方法尝试通过多路径学习不同区域特征，但存在协调效率低和计算成本高的问题。

Method: 提出扩散模糊系统（DFS）：1）使用多个扩散路径，每个路径专门学习特定类别的图像特征；2）采用基于规则链的推理动态引导扩散过程，实现多路径高效协调；3）引入基于模糊隶属度的潜在空间压缩机制降低计算成本。

Result: 在LSUN Bedroom、LSUN Church和MS COCO三个公开数据集上测试，DFS相比现有单路径和多路径扩散模型：1）训练更稳定、收敛更快；2）图像质量和文本-图像对齐度更优；3）生成图像与目标参考的对比准确度更高。

Conclusion: DFS通过模糊规则引导的多路径扩散架构，有效解决了传统扩散模型在处理异质图像特征时的局限性，在保持高质量图像生成的同时，实现了更高效的训练和更好的特征学习能力。
...
TL;DR: 该论文提出了两种简单的实例级极小极大重构方法来解决部分AUC优化中的近似误差问题，实现了线性计算复杂度和收敛保证。


<details>
  <summary>Details</summary>
Motivation: 部分AUC是处理类别不平衡和决策约束场景的重要评估指标，但现有优化方法存在不可控的近似误差或可扩展性有限的问题，需要更精确高效的解决方案。

Method: 提出了两种实例级极小极大重构：一种具有渐近消失的近似误差，另一种通过增加变量实现无偏估计。核心思想是先建立等效的实例级问题降低时间复杂度，通过阈值学习简化样本选择，然后应用不同的平滑技术。

Result: 算法具有线性迭代计算复杂度，收敛率为O(ε^{-1/3})，提供了紧致的泛化边界，明确展示了TPR/FPR约束对泛化的影响，实验验证了方法的有效性。

Conclusion: 该研究填补了部分AUC优化的近似误差空白，提供了理论保证和高效算法，为实际应用中的类别不平衡和决策约束问题提供了更好的解决方案。
...
TL;DR: 该研究提出了一个大规模、多中心的脑影像深度无监督异常检测基准测试，评估了多种算法在不同扫描仪、病变类型和人口统计学因素下的表现，发现重建方法在病变分割上表现最佳，但所有算法都存在系统性偏差。


<details>
  <summary>Details</summary>
Motivation: 脑磁共振成像中的深度无监督异常检测无需病变特异性标注即可识别病理偏差，但碎片化评估、异质性数据集和不一致的指标阻碍了临床转化。本研究旨在建立一个透明的大规模基准测试来推动该领域发展。

Method: 使用大规模多中心数据集：训练集包含2,976个T1和2,972个T2加权健康个体扫描；验证集92个扫描用于超参数调优；测试集包含2,221个T1w和1,262个T2w扫描，涵盖健康数据和多种临床队列。系统评估了不同扫描仪、病变类型和大小、人口统计学因素（年龄、性别）对算法性能的影响。

Result: 所有算法的Dice分割性能在0.03到0.65之间，显示显著变异性。重建方法（特别是扩散启发方法）在病变分割上表现最强，而基于特征的方法在分布偏移下表现出更好的鲁棒性。大多数算法存在系统性偏差：小病变和低对比度病变更容易被遗漏，假阳性随年龄和性别变化。增加健康训练数据仅带来有限改善。

Conclusion: 当前无监督异常检测框架主要受算法限制而非数据可用性。基准测试为未来研究建立了透明基础，并强调了临床转化的优先事项：图像原生预训练、原则性偏差测量、公平感知建模和鲁棒域适应。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本驱动运动数据集存在以下问题：1) 数据是非功能性的且孤立，难以覆盖各种社交场景；2) 运动数据质量不一致；3) 文本标注缺乏细粒度细节。需要专门针对场景和角色设计的更高质量数据集。

Method: 精心设计和收集了专注于场景和角色的运动数据集，包含25个经典场景、110个功能性角色、500多种行为。建立了比现有评估器更强的评估系统，并验证了其可靠性。探索了身体和手部运动生成的相互作用。

Result: 创建了包含10296个高质量人体运动序列（身体和手部）的数据集，带有27831个细粒度文本描述。在各种文本到运动方法上进行了评估，实验结果表明该数据集在文本驱动全身生成方面具有高质量和功能性。

Conclusion: RoleMotion数据集通过专注于场景和角色的设计，解决了现有数据集在功能性、质量和标注细粒度方面的不足，为文本驱动全身运动生成提供了高质量基准。
...
Result: 引入包含7个子集的新基准，从三个维度评估多主体个性化性能。大量实验证明该方法在推进多主体个性化图像生成方面的有效性。

Conclusion: 该方法通过高质量数据集构建和强化学习奖励机制设计，有效解决了多主体个性化图像生成中的主体一致性和文本控制问题，显著提升了多主体场景下的生成性能。
...
TL;DR: TRivia是一种自监督微调方法，使预训练视觉语言模型能够直接从无标签表格图像中学习表格识别，无需人工标注，其3B参数模型在多个基准测试中超越了现有系统。


<details>
  <summary>Details</summary>
Motivation: 表格识别作为文档解析的核心组件，传统依赖监督学习，但获取大规模标注数据成本高昂。开源模型因资源有限和隐私法规限制，性能远落后于专有模型，需要一种无需标注数据的解决方案来缩小这一差距。

Method: 基于Group Relative Policy Optimization，通过问答奖励机制自动识别最有效的无标签样本进行学习。注意力引导模块为每个表格图像生成多样化问题，模型通过正确解释识别结果并回答问题获得反馈，形成闭环优化过程。

Result: 提出的TRivia-3B模型在三个流行基准测试中超越了现有系统（如Gemini 2.5 Pro、MinerU2.5），成为开源、紧凑且最先进的表格识别模型。

Conclusion: TRivia方法成功实现了无需标注数据的表格识别学习，通过自监督微调显著提升了开源模型的性能，为资源受限场景下的表格识别提供了可行解决方案。
...
TL;DR: 该研究提出了Envision基准测试，用于评估多图像序列生成中的因果事件进展能力，发现现有模型在时空一致性方面存在局限，强调从静态单图像生成转向动态多帧推理的重要性。
...
Method: 提出Envision基准测试，包含1000个四阶段提示，涵盖六个科学和人文领域，基于世界知识和时空因果关系构建。引入Envision-Score综合指标，评估多维度一致性、物理性和美学质量。

Result: 评估了15个模型（10个专业T2I模型，5个统一模型）：专业T2I模型在美学渲染方面表现出色但缺乏内在世界知识；统一多模态模型在因果叙事连贯性上优于专业模型，但仍落后于闭源模型，且在时空一致性方面存在挑战。

Conclusion: 专注于因果孤立的单图像会阻碍多帧推理和生成，促进静态模式匹配而非动态世界建模，最终限制了世界知识的内化和生成能力。需要从单图像评估转向序列帧评估。
...
TL;DR: 基于计算机视觉的粘度计，通过分析混合过程中自由表面变形引起的光折射导致的背景图案光学畸变来测量粘度，无需接触被测流体。


<details>
  <summary>Details</summary>
Motivation: 传统粘度计具有侵入性且需要受控实验室环境，与真实工艺条件差异大，需要一种非侵入式、适用于实际工艺条件的粘度测量方法。

Method: 利用固定背景图案在混合驱动的连续变形自由表面中发生光折射畸变的原理，通过计算机视觉分析光学畸变来推断粘度，采用多图案策略增强鲁棒性，并加入不确定性量化。

Result: 在不同光照条件下，系统在log m2 s^-1单位下达到0.113的平均绝对误差，粘度等级预测准确率最高达81%，多图案策略提高了系统鲁棒性。
...
TL;DR: 提出TCU和UESF框架，通过针对性对比遗忘减少声纳图像中海床特征依赖，提高泛化能力，并增强模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有声纳图像分析AI模型过度依赖海床特征，导致泛化能力差，需要解决这一背景偏置问题。

Method: 提出两个关键模块：1) 针对性对比遗忘(TCU)模块，扩展三元组损失减少海床背景偏置；2) 遗忘解释声纳框架(UESF)，提供模型遗忘内容的可视化解释，并适配LIME解释器生成更准确的归因分析。

Result: 在真实和合成声纳数据集上的广泛实验验证了方法的有效性，在遗忘效果、模型鲁棒性和可解释性方面均有显著提升。

Conclusion: 提出的TCU和UESF框架能有效减少声纳图像分析中的海床依赖偏置，提高模型泛化能力和可解释性。
...
TL;DR: EGG-Fusion是一个实时3D重建系统，通过鲁棒的稀疏到稠密相机跟踪和几何感知的高斯面元映射模块，结合基于信息滤波的融合方法，实现了高精度表面重建，在保持24FPS实时处理的同时，在Replica和ScanNet++数据集上达到0.6cm的表面重建误差。


<details>
  <summary>Details</summary>
Motivation: 当前基于可微分渲染的SLAM系统（如NeRF和3DGS）面临实时计算和传感器噪声敏感性的双重挑战，导致场景重建的几何保真度下降和实用性受限。需要开发既能实时处理又能抵抗传感器噪声的高精度重建系统。

Method: 提出EGG-Fusion系统，包含鲁棒的稀疏到稠密相机跟踪模块和几何感知的高斯面元映射模块。采用基于信息滤波的融合方法，显式考虑传感器噪声，实现高精度表面重建。可微分高斯面元映射有效建模多视角一致表面，同时支持高效参数优化。

Result: 在Replica和ScanNet++等标准化基准数据集上，系统达到0.6cm的表面重建误差，相比最先进的基于高斯分布的GS方法精度提升超过20%。系统保持24FPS的实时处理能力，成为最准确的可微分渲染实时重建系统之一。

Conclusion: EGG-Fusion成功解决了实时3D重建中计算效率和噪声敏感性的双重挑战，通过创新的几何感知高斯面元映射和信息滤波融合方法，实现了高精度、实时的场景重建，为可微分渲染SLAM系统提供了实用的解决方案。
...
TL;DR: RLWG是一个自监督后训练框架，通过几何和感知奖励将预训练的世界模型与可物理验证的结构对齐，提升导航任务中的空间一致性和长期稳定性。
...
Method: 提出了RLWG框架，使用可验证的几何和感知奖励（如姿态循环一致性、深度重投影和时间一致性）对预训练世界模型进行后训练对齐。具体实现为GrndCtrl方法，基于Group Relative Policy Optimization进行奖励对齐。

Result: GrndCtrl产生的世界模型能够保持稳定的轨迹、一致的几何结构和可靠的rollout，在室外环境中实现了比监督微调更好的空间一致性和导航稳定性。

Conclusion: RLWG框架通过可验证奖励桥接了生成式预训练和接地行为，类似于大语言模型中的后训练对齐，为具身导航提供了具有几何基础的世界模型。
...
TL;DR: DCText是一种无需训练的可视化文本生成方法，采用分治策略解决长文本或多文本生成问题，通过文本分割、区域分配和注意力掩码实现高精度文本渲染。


<details>
  <summary>Details</summary>
Motivation: 尽管现有的文本到图像模型在文本渲染方面取得了高保真度，但在处理长文本或多个文本时仍存在困难，主要原因是全局注意力被稀释。需要一种能够准确渲染复杂文本布局的方法。

Method: 1. 采用分治策略，利用多模态扩散变换器的可靠短文本生成能力；2. 首先通过提取和分割目标文本分解提示；3. 将每个文本段分配到指定区域；4. 引入两种注意力掩码（文本聚焦和上下文扩展）在去噪过程中顺序应用；5. 使用局部噪声初始化进一步提高文本准确性和区域对齐。

Result: 在单句和多句基准测试中，DCText实现了最佳的文本准确性，同时不损害图像质量，并且具有最低的生成延迟。

Conclusion: DCText是一种有效的训练免费方法，能够解决文本到图像模型中长文本和多文本生成的挑战，通过分治策略和创新的注意力机制实现了高质量的文本渲染。
...
TL;DR: VisualSync是一个基于多视角动态的优化框架，能够以毫秒级精度对齐未标定、未同步的视频，通过利用运动3D点的极线约束来实现跨相机同步。


<details>
  <summary>Details</summary>
Motivation: 当前人们可以轻松使用多个消费级相机记录重要时刻，但同步这些跨相机视频流仍然具有挑战性。现有方法需要受控设置、特定目标、手动校正或昂贵硬件。

Method: VisualSync基于多视角动态优化框架，利用运动3D点在两个相机中可见时，一旦正确同步就满足极线约束的原理。它使用现成的3D重建、特征匹配和密集跟踪来提取轨迹、相对姿态和跨视角对应关系，然后联合最小化极线误差来估计每个相机的时间偏移。

Result: 在四个多样化、具有挑战性的数据集上的实验表明，VisualSync优于基线方法，实现了中位数同步误差低于50毫秒的性能。

Conclusion: VisualSync提供了一个有效的框架，能够在不受控环境中实现高精度的跨相机视频同步，解决了实际应用中的重要问题。
...
TL;DR: 提出Gaussian Swaying框架，使用3D高斯表示表面进行空气动力学模拟，统一模拟和渲染，实现高效精细的交互


<details>
  <summary>Details</summary>
Motivation: 自然界中物体在气流作用下的运动（如树枝摇曳、旗帜飘动）对视觉和图形学的真实感至关重要，现有方法存在网格化成本高或粒子方法依赖离散数据的问题

Method: 基于3D高斯表示表面的连续建模框架，使用高斯斑块统一模拟和渲染，支持动力学力计算并提供法线用于轻量级着色

Result: 在合成和真实数据集上的综合实验表明，Gaussian Swaying在多个指标上达到最先进的性能和效率

Conclusion: Gaussian Swaying为真实空气动力学场景模拟提供了可扩展的高效方法，统一了模拟和渲染表示
...
TL;DR: 研究探索了脑影像数据质量对预训练的影响，发现不同质量水平的数据在脑龄预测任务中表现差异显著，揭示了数据质量在医学影像预训练中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 大规模脑影像数据集为领域基础模型预训练提供了机会，但神经影像数据存在高度异质性，从结构良好的扫描到严重失真或不完整的脑容量。这引发了一个基本问题：噪声或低质量扫描能否对预训练做出有意义的贡献，还是会阻碍模型学习？

Method: 系统探索数据质量水平在预训练中的作用及其对下游任务的影响。具体方法包括在不同质量水平的数据集上进行预训练，然后在外部队列上进行脑龄预测的微调。

Result: 结果显示不同质量水平之间存在显著的性能差异，揭示了机会和局限性。高质量数据通常带来更好的性能，但某些情况下低质量数据也可能提供有价值的信息。

Conclusion: 计算机视觉实践与临床神经影像标准之间存在差距，强调需要领域感知的数据整理以确保可信且可泛化的领域特定基础模型。数据质量在医学影像预训练中至关重要。
...
TL;DR: 本文提出了交互式视频语料库检索（IVCR）任务，这是一个更现实的设置，支持用户与检索系统之间的多轮、对话式交互，并引入了IVCR-200K数据集和基于多模态大语言模型的框架。


<details>
  <summary>Details</summary>
Motivation: 现有的视频检索和视频时刻检索任务虽然取得了进展，但缺乏用户与检索系统之间的有意义的"交互"，单向检索范式无法满足至少80.8%用户的个性化和动态需求。

Method: 1. 引入交互式视频语料库检索（IVCR）任务；2. 构建IVCR-200K数据集（高质量、双语、多轮、对话式、抽象语义）；3. 提出基于多模态大语言模型（MLLMs）的框架，支持多种交互模式和更可解释的解决方案。

Result: 广泛的实验证明了所提出数据集和框架的有效性。

Conclusion: IVCR任务和IVCR-200K数据集为视频检索领域提供了更现实、交互性更强的研究设置，基于MLLMs的框架能够更好地满足用户的个性化和动态需求。
...
Conclusion: TokenPure通过令牌化条件重建有效解决了水印去除与内容一致性之间的权衡问题，为数字水印去除提供了新的有效解决方案。
...
TL;DR: 该研究开发了一个结合集成学习和可解释AI的机器学习框架，用于卒中风险预测，在卒中预测数据集上达到了99.09%的准确率。


<details>
  <summary>Details</summary>
Motivation: 卒中是全球主要的死亡和永久性损伤原因，早期风险评估对于及时干预和有效预防策略至关重要。研究旨在开发一个既准确又可解释的卒中风险预测框架。

Method: 采用集成建模和可解释AI技术，包括数据预处理、特征工程、使用随机过采样解决类别不平衡问题，通过5折交叉验证评估10种不同机器学习模型，最终构建优化的集成模型（随机森林+ExtraTrees+XGBoost）。

Result: 优化的集成模型在卒中预测数据集上表现出色，达到99.09%的准确率。通过LIME可解释性分析识别出三个关键临床变量：年龄、高血压和血糖水平。

Conclusion: 研究证明结合集成学习和可解释AI可以提供高精度且可解释的卒中风险评估，通过早期预测实现数据驱动的预防和个性化临床决策，有望改变卒中预测和心血管风险管理。
...
TL;DR: 该研究构建了首个大规模多说话人生成质量评估数据集THQA-MT，并提出EvalTalker评估框架，用于评估多说话人生成视频的质量。
...
Method: 1) 构建THQA-MT数据集：包含5,492个多说话人生成视频，来自15个代表性模型和400个真实肖像；2) 通过主观实验分析感知差异并识别12种常见失真类型；3) 提出EvalTalker评估框架，能够感知全局质量、人物特征、身份一致性，并集成Qwen-Sync感知多模态同步性。

Result: 实验结果表明，EvalTalker与主观评分具有优越的相关性，为未来高质量多说话人生成和评估研究提供了坚实基础。

Conclusion: 该研究填补了多说话人生成质量评估的空白，提出的数据集和评估框架为改进多说话人生成技术提供了重要工具，有助于提升视听通信的交互性和沉浸感。
...
TL;DR: 论文提出InternVideo-Next框架，通过Encoder-Predictor-Decoder架构和两阶段预训练，解决视频表示学习中语义与细节的冲突问题，在公开无标签视频上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视频-文本预训练依赖噪声合成字幕，缺乏对隐含世界知识（如物体运动、3D几何、物理线索）的覆盖；而掩码视频建模直接利用时空结构但在通用任务上落后于文本监督方法。这种差距源于架构问题：像素级重建收敛困难且与语义冲突，潜在预测容易导致捷径学习。

Method: 提出Encoder-Predictor-Decoder框架，其中预测器作为潜在世界模型。采用两阶段预训练：第一阶段使用条件扩散解码器并注入可靠的图像级语义先验，增强语义和收敛性；第二阶段在构建的潜在空间中预测冻结的第一阶段目标，学习世界知识并避免捷径学习。

Result: 在公开无标签视频上训练的InternVideo-Next在多个基准测试中达到最先进性能，为通用视频表示学习提供了可扩展路径。

Conclusion: 通过解耦编码器-解码器设计为Encoder-Predictor-Decoder框架，并采用两阶段预训练方案，成功构建了语义一致且保留细节的潜在空间，有效解决了视频表示学习中语义抽象与像素级保真度之间的冲突。
...
TL;DR: BharatOCR：一种基于ViT-Transformer Decoder-LM架构的无分割段落级手写印地语和乌尔都语文本识别系统，在多个数据集上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 尽管手写文本识别已取得显著进展，但段落级手写文本识别，特别是在印地语、乌尔都语等低资源语言中，仍然是一个挑战。这些语言通常缺乏全面的语言资源，需要开发鲁棒的光学字符识别系统。

Method: 提出ViT-Transformer Decoder-LM架构：使用Vision Transformer提取视觉特征，Transformer解码器生成文本序列，预训练语言模型优化输出以提高准确性、流畅性和连贯性。采用DeiT进行掩码图像建模，RoBERTa架构进行掩码语言建模。模型通过隐式行分割逐行处理段落图像。

Result: 在NUST-UHWR数据集上字符识别率达到96.24%，PUCIT-OUHL数据集上92.05%，Parimal-Urdu数据集上94.80%，印地语数据集上80.64%。模型在多个乌尔都语文本识别方法中表现最优。

Conclusion: BharatOCR在段落级手写印地语和乌尔都语文本识别方面取得了突破性进展，为低资源语言的手写文本识别提供了有效的解决方案，并在多个数据集上建立了新的基准。
...
TL;DR: OpenBox：利用2D视觉基础模型的两阶段自动标注管道，无需自训练即可生成高质量的3D边界框标注


<details>
  <summary>Details</summary>
Motivation: 现有无监督和开放词汇3D目标检测方法存在三个主要问题：1）统一标注3D边界框，忽略物体的物理状态；2）需要多次自训练迭代进行标注细化；3）导致标注质量不理想且计算开销大。这些问题在自动驾驶场景中尤为突出，因为降低标注成本和识别未见物体对安全性和可扩展性至关重要。

Method: OpenBox采用两阶段自动标注管道：第一阶段通过跨模态实例对齐，将2D视觉基础模型处理的图像中的实例级线索与对应的3D点云关联；第二阶段根据刚性和运动状态对实例进行分类，然后使用类别特定的尺寸统计生成自适应边界框。

Result: 在Waymo Open Dataset、Lyft Level 5 Perception dataset和nuScenes数据集上的实验表明，OpenBox在准确性和效率方面均优于基线方法，能够生成高质量的3D边界框标注。

Conclusion: OpenBox通过创新的两阶段管道解决了现有3D目标标注方法的局限性，无需自训练即可生成高质量的3D边界框标注，为自动驾驶场景中的无监督和开放词汇3D目标检测提供了有效的解决方案。
...
TL;DR: BlinkBud是一个利用单耳机和配对手机实时检测用户后方危险物体接近的系统，通过3D物体追踪算法和强化学习优化图像采样策略，在低功耗下实现高精度危险检测。


<details>
  <summary>Details</summary>
Motivation: 行人或骑行者无法察觉后方快速接近的车辆对道路安全构成巨大威胁，需要一种低功耗、高精度的实时危险检测系统。

Method: 使用单耳机和配对手机，开发了基于卡尔曼滤波的轨迹估计和强化学习优化图像采样策略的3D物体追踪算法，利用耳机姿态估计校正物体深度和坐标系对齐，消除头部运动对追踪精度的影响。

Result: 原型系统在真实环境中测试，耳机平均功耗29.8mW，手机702.6mW，平均误报率4.90%，漏报率1.47%，实现了轻量级高精度危险检测。

Conclusion: BlinkBud系统通过创新的3D物体追踪算法和优化策略，在极低功耗下实现了对用户后方危险物体的准确检测，为行人安全提供了有效解决方案。
...
TL;DR: 提出了一种基于大语言模型的形状真实感对齐度量方法，通过将3D网格编码到语言空间，结合专门设计的真实感解码器来评估3D形状的真实感，无需地面真值参考。


<details>
  <summary>Details</summary>
Motivation: 随着3D生成和重建技术在游戏、电影等领域的广泛应用，对逼真3D形状的需求日益增长。传统评估方法依赖地面真值来衡量网格保真度，但在许多实际应用中，形状的真实感并不需要地面真值参考。需要一种能够评估3D形状真实感的新方法。

Method: 1. 采用网格编码方法将3D形状转换为语言标记空间；2. 设计专门的真实感解码器，使语言模型输出与人类对真实感的感知对齐；3. 引入新的数据集RealismGrading，提供无需地面真值形状的人类标注真实感分数；4. 通过k折交叉验证验证度量的性能和泛化能力。

Result: 实验结果表明，该度量方法与人类感知相关性良好，优于现有方法，并具有良好的泛化能力。数据集包含16种不同算法在十多个对象上生成的形状，更具代表性。

Conclusion: 提出的形状真实感对齐度量方法能够有效评估3D形状的真实感，无需地面真值参考，与人类感知一致，为3D形状质量评估提供了新的有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D模型评估方法存在多个问题：传统几何指标（如Chamfer Distance）与人类感知不一致；基于渲染图像的方法存在结构覆盖不完整和视角选择敏感性问题；大多数方法在合成失真数据上训练，与真实世界失真存在领域差距。

Method: 提出Textured Geometry Evaluation (TGE)方法，直接基于纹理3D网格进行评估，无需依赖渲染。该方法联合使用几何和颜色信息来计算输入纹理网格相对于参考彩色形状的保真度。为训练和评估该指标，构建了人工标注的真实世界失真数据集。
...
TL;DR: 本文提出ReInversion方法，通过两阶段去噪过程和掩码引导选择性去噪策略，实现高效且高质量的示例引导图像编辑，无需大规模预训练。


<details>
  <summary>Details</summary>
Motivation: 现有示例引导图像编辑方法通常需要大规模预训练来学习源图像和参考图像之间的关系，计算成本高。而标准反演技术虽然无需训练，但用于EIE时效果不佳，导致质量差和效率低。

Method: 提出Reversible Inversion (ReInversion)方法：1）两阶段去噪过程：先基于源图像条件去噪，再基于参考图像条件去噪；2）Mask-Guided Selective Denoising (MSD)策略：约束编辑仅限于目标区域，保持背景结构一致性。

Result: 定性和定量比较表明，ReInversion方法在示例引导图像编辑任务上达到了最先进的性能，同时具有最低的计算开销。

Conclusion: ReInversion通过创新的可逆反演框架和掩码引导策略，为示例引导图像编辑提供了一种高效且高质量的解决方案，无需昂贵的预训练过程。
...
TL;DR: 本文首次将掩码扩散模型（MDMs）引入场景文本识别（STR）任务，提出MDiff4STR模型，通过两种改进策略解决了MDMs在STR应用中的关键挑战，在保持高效推理的同时超越了自回归模型的准确率。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型在视觉语言任务中展现出效率和准确性的良好平衡，但尚未应用于场景文本识别任务。虽然MDMs能提高识别效率，但原始版本在准确性上仍落后于自回归模型，需要改进以充分发挥其潜力。

Method: 提出MDiff4STR模型，针对STR任务的两个关键挑战：训练与推理之间的噪声差异，以及推理过程中的过度自信预测。为解决第一个问题，开发了六种噪声策略以更好地对齐训练与推理行为；针对第二个问题，提出了令牌替换噪声机制，提供非掩码噪声类型，鼓励模型重新考虑和修正过度自信但错误的预测。

Result: 在标准和挑战性STR基准测试中进行了广泛评估，涵盖不规则、艺术、遮挡和中文文本等多种场景，以及是否使用预训练的情况。MDiff4STR在所有设置中始终优于流行的STR模型，在准确性上超越了最先进的自回归模型，同时仅需三个去噪步骤即可实现快速推理。

Conclusion: MDiff4STR成功将掩码扩散模型应用于场景文本识别任务，通过针对性的改进策略解决了关键挑战，实现了在保持高效推理的同时超越自回归模型准确性的目标，为STR任务提供了新的有效解决方案。
...
TL;DR: ViRectify是一个评估多模态大语言模型视频推理错误纠正能力的基准，包含30K+实例，涵盖动态感知、科学推理和具身决策三个领域，通过AI辅助标注构建，要求模型进行逐步错误识别和基于视频证据的纠正。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂视频推理场景中经常出错，但现有基准缺乏对这些模型识别和纠正错误能力的系统性评估。为了揭示模型的弱点并提升性能，需要建立一个专门的评估框架。

Method: 1) 通过AI辅助标注流程构建包含30K+实例的数据集；2) 提出轨迹证据驱动的纠正框架，包括逐步错误轨迹建模和基于视觉证据的奖励建模；3) 要求模型进行逐步错误识别并生成基于关键视频证据的纠正理由。

Result: 1) 在16个先进MLLM上的评估显示ViRectify是一个具有挑战性的测试平台，GPT-5仅达到31.94%的纠正准确率；2) 提出的框架使Qwen2.5-VL-7B模型在ViRectify上持续优于72B变体；3) 分析揭示了模型间错误纠正的系统性不对称性。

Conclusion: ViRectify为全面评估多模态大语言模型在视频推理中的能力提供了新方向，数据集也可作为反思学习的宝贵资源。提出的轨迹证据驱动框架能有效引导模型关注错误传播和关键时间戳，提升纠正性能。
...
TL;DR: ResDiT：一种无需训练的方法，通过修正位置嵌入和局部增强机制，解决扩散变换器在高分辨率图像合成中的布局崩溃和纹理退化问题


<details>
  <summary>Details</summary>
Motivation: 预训练的扩散变换器（DiTs）用于高分辨率图像合成时，经常出现空间布局崩溃和纹理保真度下降的问题。现有方法通常采用复杂的多阶段流程，而本文希望探索DiTs的内在生成机制，提出更高效的解决方案。

Method: 1. 提出位置嵌入缩放技术，修正分辨率变化时的位置编码错误；2. 基于基础分辨率局部注意力的局部增强机制；3. 设计块级融合模块聚合全局和局部线索；4. 采用高斯加权拼接策略消除网格伪影

Result: ResDiT能够持续生成高保真度的高分辨率图像，无缝集成到下游任务中，包括空间可控生成，相比现有方法更加高效

Conclusion: ResDiT通过分析DiTs的内在生成机制，提出了一种无需训练的高分辨率图像合成方法，有效解决了布局崩溃和纹理退化问题，为扩散变换器的高分辨率应用提供了新思路
...
TL;DR: Clipomaly：首个基于CLIP的自动驾驶开放世界和异常分割方法，无需异常训练数据，能动态扩展词汇表并给未知物体分配语义标签
...
Method: 提出Clipomaly方法，基于CLIP的零样本方法，无需异常特定训练数据，利用CLIP共享的图像-文本嵌入空间来分割未知物体并为其分配人类可解释的名称。与开放词汇方法不同，该模型在推理时动态扩展词汇表而无需重新训练。

Result: 在已建立的异常分割基准测试中实现了最先进的性能，同时提供了实际部署所需的可解释性和灵活性。

Conclusion: Clipomaly是首个基于CLIP的自动驾驶开放世界和异常分割方法，能够稳健地检测和命名超出常见类别定义（如Cityscapes）的异常，为实际部署提供了重要优势。
...
TL;DR: 提出统一学习框架解决3D人体化身动画中的两个关键问题：通过U-Net架构快速生成人体模板，以及数据驱动的细化技术提升结构完整性，在效率和质量上超越现有方法。
...
Method: 提出两阶段统一学习框架：第一阶段使用U-Net架构在正向传播过程中解耦纹理和姿态信息，快速生成人体模板；第二阶段采用数据驱动的细化技术增强结构完整性。

Result: 实验表明，该模型在不同姿态下表现一致，在效率和质量之间达到最优平衡，超越了当前最先进的方法。
...
TL;DR: ChronosObserver：无需训练的4D世界生成方法，通过世界状态超空间和超空间引导采样实现3D一致、时间同步的多视角视频生成


<details>
  <summary>Details</summary>
Motivation: 现有相机控制视频生成模型难以直接扩展到3D一致、高保真、时间同步的多视角视频生成，而这是驯服4D世界的关键能力。现有方法依赖数据增强或测试时优化，但受限于模型泛化能力和可扩展性问题。

Method: 提出ChronosObserver方法，包括：1）世界状态超空间：表示4D世界场景的时空约束；2）超空间引导采样：利用超空间同步多个视角的扩散采样轨迹。该方法无需训练或微调扩散模型。

Result: 实验结果表明，该方法能够生成高保真、3D一致、时间同步的多视角视频，无需对扩散模型进行训练或微调。

Conclusion: ChronosObserver提供了一种无需训练的方法，成功解决了4D世界场景中多视角视频生成的3D一致性和时间同步问题，克服了现有方法的泛化和可扩展性限制。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种自动从图像中提取曲线和1D结构的有效方法，该方法应尽可能无监督，并能处理曲率相关的几何特征。

Method: 基于变分方法，利用能量离散化和Smirnov向量场分解定理，设计双层最小化框架。通过将曲线提升到位置-方向空间，并采用适当的次黎曼或Finsler度量来处理曲率相关能量。

Result: 开发了一种主要无监督的曲线提取方法，能够自动从图像中提取曲线和1D结构，并扩展到处理曲率相关的几何特征。

Conclusion: 该方法提供了一种有效的变分框架，用于从图像中提取曲线结构，特别是通过空间提升技术处理曲率约束，实现了自动化的1D结构提取。
...
<details>
  <summary>Details</summary>
Motivation: 当前分子和材料机器学习模型缺乏物理可迁移性，它们拟合整个分子或晶体的相关性，而不是学习原子对之间的量子相互作用。然而，化学键、电荷重分布、轨道杂化和电子耦合都源于这些定义多体系统中局部量子场的双体相互作用。

Method: 构建了包含2850个元素-元素对的数据集，每个对标注了18个电子、热力学和几何性质，并配以十通道图像表示（源自l-和m-分辨的轨道密度、角场变换、共占据图和电荷密度投影）。这些物理基础的图像编码了空间、角和静电对称性，无需显式坐标。基准测试了8种架构在18个目标上的表现。

Result: GATv2在能隙上达到0.201 eV的平均绝对误差，EGNN在HOMO和LUMO上分别达到0.265 eV和0.274 eV。DimeNet在总能量上达到2.27 eV MAE，在排斥能量上达到0.132 eV MAE。多模态融合模型在Mermin自由能上达到2.15 eV MAE。在QuantumCanvas上预训练进一步提高了在QM9、MD17和CrysMTM等更大数据集上的收敛稳定性和泛化能力。

Conclusion: 通过将轨道物理与基于视觉的表征学习统一起来，QuantumCanvas为通过耦合视觉和数值模态学习可迁移的量子相互作用提供了原则性和可解释的基础。
...
TL;DR: FlashVGGT是一种高效的3D重建方法，通过描述符注意力机制替代全自注意力，显著降低计算复杂度，能在保持精度的同时处理超长图像序列。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的3D重建方法（如VGGT）使用全自注意力机制，计算复杂度呈二次方增长，在处理长图像序列时存在可扩展性问题，限制了实际应用。

Method: 提出描述符注意力机制：将每帧图像的空间信息压缩为紧凑的描述符token集合，然后通过图像token与描述符集合之间的交叉注意力计算全局关系，大幅减少计算量。同时采用分块递归机制，重用先前分块的缓存描述符，支持在线长序列推理。

Result: FlashVGGT在重建精度上与VGGT相当，但推理时间仅为VGGT的9.3%（处理1000张图像时），并能高效扩展到超过3000张图像的长序列。

Conclusion: FlashVGGT通过创新的描述符注意力机制解决了现有3D重建方法在处理长图像序列时的可扩展性问题，在保持精度的同时大幅提升了计算效率，为实际应用提供了可行方案。
...
TL;DR: AbscessHeNe数据集：包含4,926张头颈部脓肿增强CT切片的标注数据集，用于开发语义分割模型，最佳模型Dice系数0.39，显示任务挑战性大。


<details>
  <summary>Details</summary>
Motivation: 头颈部脓肿是可能危及生命的急性感染，需要及时诊断和治疗。准确识别脓肿边界和深部间隙受累对临床决策至关重要，但目前缺乏专门的数据集支持AI模型开发。

Method: 创建了AbscessHeNe数据集，包含4,926张临床确诊的头颈部脓肿增强CT切片，提供像素级标注和临床元数据。评估了CNN、Transformer和Mamba等多种先进分割架构，建立性能基准。

Result: 最佳模型性能：Dice相似系数0.39，交并比0.27，归一化表面距离0.67。这些相对较低的指标表明头颈部脓肿分割任务具有挑战性，需要进一步研究改进。

Conclusion: AbscessHeNe数据集为头颈部脓肿分割提供了首个全面标注的资源，支持AI模型开发和临床决策。数据集将公开可用，未来还可用于基于内容的医学图像检索和知识驱动的临床工作流程。
...
<details>
  <summary>Details</summary>
Motivation: 油基泥浆微电阻率成像仪采用交错设计的上下极板，即使经过速度校正后，极板图像之间仍存在深度错位问题，需要更精确的深度匹配方法

Method: 基于Shape Dynamic Time Warping算法，提取局部形状特征构建形态敏感距离矩阵，采用一维梯度方向直方图(HOG1D)和原始信号组合作为形状描述符

Result: 现场测试表明，该方法能精确对齐具有复杂纹理、深度偏移或局部缩放的图像，为复杂地质特征的深度匹配提供了有效解决方案

Conclusion: 该方法不仅解决了油基泥浆成像仪的深度匹配问题，还提供了灵活的特征扩展框架，可根据特定地质特征集成其他描述符
...
TL;DR: SPARK是一个从单张RGB图像重建物理一致、运动学部件级铰接物体的框架，通过视觉语言模型提取粗略URDF参数，结合生成扩散变换器合成一致部件，并利用可微分前向运动学和渲染优化关节参数。
...
Method: 首先利用视觉语言模型提取粗略URDF参数并生成部件级参考图像；然后整合部件图像引导和推断的结构图到生成扩散变换器中，合成一致的部件和完整铰接物体形状；最后通过可微分前向运动学和可微分渲染优化关节类型、轴和原点。

Result: 大量实验表明，SPARK能够跨不同类别生成高质量的仿真就绪铰接资产，支持机器人操作和交互建模等下游应用。

Conclusion: SPARK框架能够从单张RGB图像有效重建物理一致的运动学部件级铰接物体，为具身AI和机器人应用提供了实用的资产创建解决方案。
...
Method: 提出Fusion-Diff框架：1）在联合视觉-语言空间中引入多模态融合特征编辑策略，显著缩小模态鸿沟；2）采用轻量级Control-Adapter，仅需在20万合成样本上进行微调。

Result: 在CIRR、FashionIQ和CIRCO等标准基准测试中，Fusion-Diff显著优于先前的零样本方法，并通过可视化融合多模态表示增强了模型可解释性。

Conclusion: Fusion-Diff通过创新的多模态融合特征编辑和高效微调策略，有效解决了零样本组合图像检索中的模态鸿沟问题，实现了高性能且数据高效的解决方案。
...
TL;DR: Vision Test-Time Training (ViT³) 是一种纯测试时训练架构，通过将注意力操作重新定义为在线学习问题，实现线性计算复杂度和可并行计算，在多种视觉任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 测试时训练（TTT）为高效序列建模提供了新方向，但视觉TTT设计缺乏系统性的理解和实践指导，需要填补这一关键空白。

Method: 通过系统性的实证研究，提炼出6个实用见解，建立了视觉TTT的设计原则，并基于此开发了Vision Test-Time Training (ViT³)模型，这是一种纯TTT架构。

Result: ViT³在图像分类、图像生成、目标检测和语义分割等多样化视觉任务中，一致匹配或优于先进的线性复杂度模型（如Mamba和线性注意力变体），有效缩小了与高度优化的视觉Transformer之间的差距。

Conclusion: 本研究为视觉TTT模型提供了设计原则和实践指导，ViT³基准模型为未来工作奠定了基础，有望推动视觉TTT模型的进一步发展。
...
TL;DR: 提出DB-KAUNet用于视网膜血管分割，结合CNN和Transformer双分支编码器，引入KANConv和KAT块，通过CCI和SFE-GAF模块增强特征交互与空间特征，在多个数据集上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 传统CNN方法在视网膜血管分割中存在局限性，难以捕捉长距离依赖和复杂非线性关系，影响分割精度。

Method: 提出自适应双分支Kolmogorov-Arnold UNet（DB-KAUNet），包含异构双分支编码器（HDBE），并行CNN和Transformer路径，集成KANConv和KAT块。引入跨分支通道交互（CCI）模块和基于注意力的空间特征增强（SFE）模块，进一步开发SFE-GAF模块进行自适应采样以聚焦真实血管形态。

Result: 在DRIVE、STARE和CHASE_DB1数据集上的大量实验验证DB-KAUNet实现了领先的分割性能，并表现出卓越的鲁棒性。

Conclusion: DB-KAUNet通过创新的双分支架构和自适应特征增强模块，有效解决了传统CNN在视网膜血管分割中的局限性，为临床诊断提供了更准确的工具。
...
TL;DR: ScaleBridge-Det：首个专为遥感图像中小目标检测设计的大型检测框架，通过尺度自适应专家路由和密度引导查询分配，实现不同尺度目标的平衡检测性能。


<details>
  <summary>Details</summary>
Motivation: 遥感图像中小目标检测面临巨大挑战，特别是在密集小目标与大目标共存场景中，现有方法难以实现跨尺度的平衡检测性能。虽然大型基础模型在通用视觉任务中表现出色，但由于遥感图像中极端的尺度变化和密度分布，它们在微小目标检测中的应用尚未探索。

Method: 提出ScaleBridge-Det框架，包含两个核心模块：1) 路由增强混合注意力模块，通过自适应路由动态选择和融合尺度特定的专家特征，生成互补且具有判别力的多尺度表示；2) 密度引导动态查询模块，预测目标密度以自适应调整查询位置和数量，实现不同尺度目标的高效资源分配。

Result: 在基准数据集和跨域数据集上的大量实验表明，ScaleBridge-Det在AI-TOD-V2和DTOD数据集上达到最先进性能，同时在VisDrone数据集上表现出优异的跨域鲁棒性。

Conclusion: ScaleBridge-Det是首个专为遥感图像中小目标检测设计的大型检测框架，通过创新的尺度自适应专家路由和密度引导查询分配机制，成功解决了跨尺度平衡检测的挑战，在密集小目标和一般目标检测中均取得优异性能。
...
TL;DR: GRASP方法通过样本聚类和残差适配器解决扩散模型在长尾数据（如罕见病理）上的模式崩溃问题，提升尾部类别的生成质量和多样性。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在长尾场景（如医学影像）中表现不佳，罕见病理数据稀少导致尾部类别生成质量差、多样性不足，影响合成数据增强效果。现有方法主要关注推理引导而不改变学习分布，未能解决频繁类别与罕见类别之间的梯度冲突问题。

Method: 提出GRASP方法：1）使用外部先验静态划分样本为聚类，最小化组内梯度冲突；2）在预训练模型基础上，向transformer前馈层注入聚类特定的残差适配器进行微调；3）绕过学习门控机制以保证稳定性和效率。

Result: 在MIMIC-CXR-LT数据集上，GRASP在FID和多样性指标上表现优异，尤其在罕见类别上超越基线方法（如普通微调和混合专家变体）。在NIH-CXR-LT的下游分类任务中，尾部标签性能显著提升。在ImageNet-LT上的泛化实验证实了方法的广泛适用性。

Conclusion: GRASP是一种轻量级、可扩展的方法，能有效解决扩散模型在长尾数据上的梯度冲突问题，提升尾部类别的生成质量和多样性，易于集成到现有扩散流程中。
...
TL;DR: 提出SCAR方法，通过结构和接触感知表示生成逼真的手-物体交互视频，解决了2D和3D表示在可扩展性和交互保真度之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前手-物体交互视频生成方法面临物理约束建模困难，现有方法使用HOI表示作为辅助生成目标，但2D和3D表示之间存在可扩展性和交互保真度无法同时保证的困境。

Method: 提出结构和接触感知表示，捕捉手-物体接触、遮挡和整体结构上下文，无需3D标注；采用联合生成范式，通过共享-专业化策略生成交互导向表示和视频。

Result: 在两个真实世界数据集上超越最先进方法，生成物理真实且时间连贯的HOI视频；在具有挑战性的开放世界场景中表现出强大的泛化能力。

Conclusion: SCAR方法通过可扩展的交互导向表示有效解决了手-物体交互视频生成中的物理约束建模问题，实现了高质量的交互视频生成和良好的泛化性能。
...
TL;DR: 该研究开发了一种基于自监督学习的计算病理学模型，能够利用在切除组织上训练的编码器处理活检样本，实现间皮瘤的亚型分类和预后预测。


<details>
  <summary>Details</summary>
Motivation: 间皮瘤的准确亚型分类和预后预测对指导治疗和患者护理至关重要。现有的计算病理学模型大多基于切除组织的大尺寸图像训练，但在实际临床中活检样本更为常见，这限制了现有模型的应用。

Method: 使用在切除组织上训练的自监督编码器，将其应用于活检材料，捕捉有意义的形态学模式。利用这些模式构建模型来预测患者生存期和分类肿瘤亚型。

Result: 该方法成功展示了自监督编码器能够从活检样本中提取有意义的形态学特征，并利用这些特征准确预测患者生存期和分类肿瘤亚型。

Conclusion: 该研究证明了AI驱动工具在支持间皮瘤诊断和治疗规划方面的潜力，特别是在处理临床常见的活检样本时，为实际临床应用提供了可行方案。
...
Method: 基于预训练视频扩散变换器模型，提出RegionalRoPE区域感知位置编码方案，使用掩码条件损失约束视觉特征，集成LLM布局生成器从自然语言脚本推断漫画风格布局

Result: 相比先前方法，角色一致性提升29.2%，风格相似性提升36.2%，同时保持高空间准确性

Conclusion: DreamingComics通过布局感知的故事可视化框架，有效解决了角色一致性和风格保持问题，为可控的故事可视化提供了新方法
...
TL;DR: 本文提出了一种语义和空间校正（SSR）方法，通过跨模态原型对齐（CMPA）和超像素引导校正（SGC）来解决CLIP-based弱监督语义分割中的非目标前景和背景过激活问题，在PASCAL VOC和MS COCO数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的弱监督语义分割方法存在两个主要问题：非目标前景区域的过激活和背景区域的过激活。这些问题限制了分割性能的进一步提升，需要从语义和空间两个层面进行校正。

Method: 提出语义和空间校正（SSR）方法，包含两个核心组件：1）跨模态原型对齐（CMPA）：建立对比学习机制，强制跨模态特征空间对齐，减少类间重叠同时增强语义相关性，有效校正非目标前景区域的过激活；2）超像素引导校正（SGC）：利用基于超像素的空间先验，在亲和力传播过程中精确过滤非目标区域的干扰，显著校正背景过激活。

Result: 在PASCAL VOC和MS COCO数据集上的大量实验表明，该方法优于所有单阶段方法以及更复杂的多阶段方法，分别取得了79.5%和50.6%的mIoU分数。

Conclusion: 提出的SSR方法通过语义和空间层面的双重校正，有效解决了CLIP-based弱监督语义分割中的过激活问题，在两个基准数据集上实现了最先进的性能，证明了该方法在弱监督语义分割任务中的有效性。
...
Method: FreqEdit包含三个协同组件：1) 从参考速度场注入高频特征以保留精细细节；2) 自适应注入策略，通过空间调制注入强度实现精确的区域控制；3) 路径补偿机制，定期重新校准编辑轨迹以防止过度约束。

Result: 在7个最先进的基线方法对比实验中，FreqEdit在身份保持和指令遵循方面都表现出优越性能，能够实现10+轮连续迭代的稳定编辑。
...
TL;DR: 该论文提出了一种通用的视频情境分析框架，通过关系模型和图形模型相结合的方式，实现跨领域的视频情境自动检测，解决了传统方法依赖人工或特定领域算法的局限性。


<details>
  <summary>Details</summary>
Motivation: 视频内容理解在公民监控、一般监控、辅助生活等领域有重要应用。虽然图像和视频分析研究在内容提取方面取得了进展，但识别有意义的活动或情境仍然困难，目前主要依赖人工或特定领域算法，缺乏通用性且效率低下。

Method: 提出通用视频情境分析框架：1）使用先进的视频内容提取技术提取内容；2）采用扩展关系模型和图形模型两种表示方法；3）关系模型支持连续查询处理；4）图形模型检测关系模型难以识别的情境；5）使用参数化模板实现领域独立性。

Result: 在辅助生活、公民监控和一般监控三个领域的视频数据集上进行了广泛实验，评估了所提方法在准确性、效率和鲁棒性方面的表现。

Conclusion: 该框架克服了现有方法的局限性，实现了跨领域的通用视频情境分析，通过结合关系模型和图形模型，能够有效检测各种复杂情境。
...
Method: 集成可学习边缘核与学习式配准技术：从预定义边缘检测核开始，添加随机噪声扰动，在训练中学习提取任务特定的最优边缘特征；设计了刚性配准和非刚性配准各4个变体模型。

Result: 在医学大学数据集（刚性配准-有无颅骨去除、非刚性配准）和两个公开数据集上评估，均优于现有最先进技术。

Conclusion: 该方法能有效提升多模态医学图像配准性能，改善解剖结构分析，具有临床应用潜力。
...
TL;DR: 学习压缩算法在卫星遥感大数据压缩中优于传统JPEG 2000，特别是在多通道光学图像上，但在小规模单通道热红外数据上传统方法仍有优势，联合优化未带来额外收益。


<details>
  <summary>Details</summary>
Motivation: 卫星地球观测数据快速增长带来传输和存储挑战，需要评估任务特定学习压缩算法在减少数据量的同时保留关键信息的潜力。

Method: 比较传统压缩（JPEG 2000）与学习压缩方法（离散混合高斯似然）在三个遥感分割任务上：火灾、云和建筑物检测，并测试联合端到端优化。

Result: 学习压缩在多通道光学图像的重建质量（PSNR）和分割精度上显著优于JPEG 2000，但在小规模单通道热红外数据集上传统编码器仍有竞争力，联合优化未改善性能。

Conclusion: 学习压缩在卫星遥感大数据处理中具有优势，但需考虑数据类型和规模，传统方法在小规模特定数据上仍有价值，端到端联合优化在当前设置下未提供额外收益。
...
TL;DR: SAM3-UNet是Segment Anything Model 3的简化变体，通过适配器和轻量级U-Net解码器，以低成本适应下游分割任务，在多项任务中表现优于SAM2-UNet和其他SOTA方法，训练时GPU内存需求低于6GB。


<details>
  <summary>Details</summary>
Motivation: 为了以低成本将强大的SAM3模型适应到下游分割任务中，需要开发一个参数高效的简化变体，降低计算和内存需求，同时保持或提升性能。

Method: 提出SAM3-UNet，包含三个组件：SAM3图像编码器、用于参数高效微调的简单适配器、以及轻量级U-Net风格解码器。这种架构设计允许在保持SAM3强大特征提取能力的同时，通过适配器进行高效微调，并通过U-Net解码器生成精细的分割结果。

Result: 在镜像检测和显著性目标检测等多个任务上的初步实验表明，SAM3-UNet优于之前的SAM2-UNet和其他最先进方法，同时在批量大小为12的训练中仅需不到6GB的GPU内存。

Conclusion: SAM3-UNet成功地将SAM3适应到下游分割任务中，以较低的计算成本实现了优异的性能，为实际应用提供了高效的解决方案。
...
TL;DR: 提出了一种基于真实世界人体动作学习潜空间的新型视频生成评估指标，通过融合外观无关的骨骼几何特征和外观特征来捕捉动作细节、约束和时间平滑性，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型评估指标存在严重局限性：纯视觉编码器和多模态大语言模型具有强烈的外观偏见，缺乏时间理解能力，难以识别生成视频中复杂的运动动态和解剖学不合理性。

Method: 首先通过融合外观无关的人体骨骼几何特征和基于外观的特征，捕捉真实世界动作的细微差别、约束和时间平滑性，构建真实世界动作分布。然后通过测量生成视频底层表示与该学习分布之间的距离来量化动作质量。

Result: 在专门设计的多层面基准测试中，新指标相比现有最先进方法实现了超过68%的显著改进；在已建立的外部基准测试中表现具有竞争力；与人类感知有更强的相关性。

Conclusion: 该研究揭示了当前视频生成模型的关键局限性，为视频生成领域的高级研究建立了新的评估标准，提供了一种更准确评估人类动作视觉和时间正确性的方法。
...
Method: 提出MILO隐式空间世界建模范式，集成视觉生成器提供几何感知反馈；提出RePE相对位置编码方案捕捉相对相机位姿变换；构建GeoGen大规模几何感知生成数据集（2,241个视频，67,827个观察-动作-结果三元组）。

Result: 实验表明该方法在多个基线和基准测试中显著提升了空间推理能力，提供了更全面的3D空间理解。
...
TL;DR: 论文提出了视觉因果发现任务，构建了包含32,000张图像的大规模数据集VCG-32K，并开发了CauSight模型，通过因果感知推理在视觉因果发现任务上超越GPT-4.1，性能提升三倍以上。


<details>
  <summary>Details</summary>
Motivation: 人类能够通过因果思维理解事物发生的原因，而不仅仅是看到什么。为了让现代AI系统具备这种能力，需要让模型能够推断视觉实体之间的因果关系，而不仅仅是感知它们的存在。

Method: 1) 构建VCG-32K数据集，包含32,000多张带有实体级因果图标注的图像；2) 开发CauSight视觉语言模型，通过因果感知推理进行视觉因果发现；3) 训练方法包括：从VCG-32K进行数据整理、使用因果思维树(ToCT)合成推理轨迹、以及通过设计的因果奖励进行强化学习来优化推理策略。

Result: CauSight在视觉因果发现任务上显著优于GPT-4.1，实现了超过三倍的性能提升（绝对增益21%）。

Conclusion: 该研究成功引入了视觉因果发现任务，构建了大规模数据集并开发了有效的模型，为AI系统具备因果推理能力提供了重要进展。所有代码、模型和数据集都已开源。
...
### [167] [OpenREAD: Reinforced Open-Ended Reasoning for End-to-End Autonomous Driving with LLM-as-Critic](https://arxiv.org/abs/2512.01830)
*Songyan Zhang,Wenhui Huang,Zhan Chen,Chua Jiahao Collister,Qihang Huang,Chen Lv*

Main category: cs.CV

TL;DR: OpenREAD是一个基于视觉语言模型的端到端强化微调自动驾驶框架，通过构建大规模思维链标注和使用Qwen3作为评判器来量化推理质量，实现了从高层推理到低层轨迹规划的全面性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前两阶段微调策略（SFT+RFT）在知识驱动自动驾驶中表现出潜力，但SFT的泛化推理能力有限，而RFT主要应用于下游任务，因为场景理解是开放性问题难以量化奖励。需要解决这些限制以实现端到端的强化微调。

Method: 1. 在开源驾驶知识数据集上构建大规模思维链标注；2. 使用强大的Qwen3大语言模型作为RFT中的评判器，为开放性问题量化推理质量；3. 实现从高层推理到低层轨迹规划的端到端联合强化微调。

Result: 端到端联合RFT在上下游任务中都带来了显著改进，使OpenREAD在推理和规划基准测试中达到了最先进的性能。

Conclusion: OpenREAD框架成功解决了当前自动驾驶中推理泛化能力有限和开放性问题奖励量化困难的问题，通过端到端强化微调实现了全面的性能提升，为知识驱动自动驾驶提供了新的解决方案。
...
TL;DR: 该研究构建了物理不可行性检测数据集(PID)，开发了轻量级微调方法PhyDetEx，使视觉语言模型能够检测视频中的物理不可行内容并生成解释，用于评估文本到视频生成模型对物理定律的遵循程度。


<details>
  <summary>Details</summary>
Motivation: 尽管文本到视频生成模型在视频质量、长度和指令跟随能力方面取得了显著进展，但这些模型是否真正理解物理规律并生成物理上合理的视频仍然是一个问题。现有的视觉语言模型难以识别生成视频中的物理不可能内容，需要专门的方法来评估T2V模型的物理合理性。

Method: 1. 构建PID数据集：包含500个手动标注的测试视频和2,588个配对视频的训练集，每个不可行视频通过重写真实世界视频的标题来诱导T2V模型产生物理上不合理的内容。2. 开发轻量级微调方法PhyDetEx：使视觉语言模型能够检测物理不可行事件并生成违反物理原理的文本解释。

Result: 研究结果表明，尽管最近的T2V模型在生成物理合理内容方面取得了显著进展，但理解和遵循物理定律仍然是一个具有挑战性的问题，特别是对于开源模型。PhyDetEx能够有效检测物理不可行内容并生成解释。

Conclusion: 该研究提出了一个系统性的方法来评估T2V模型的物理合理性，构建了专门的数据集和检测器。虽然T2V模型在物理合理性方面有所改进，但仍需进一步研究来提高模型对物理定律的理解和遵循能力。
...
TL;DR: 提出一种基于条件生成的点云配准方法，通过学习连续的点级速度场将噪声点云传输到配准场景，直接生成配准后的点云，无需传统对应匹配和变换估计。


<details>
  <summary>Details</summary>
Motivation: 传统点云配准方法依赖对应匹配来估计点云对之间的变换，然后优化成对变换实现多视图配准，这种方法在低重叠情况下效果不佳。本文希望开发一种更直接的方法，将配准视为条件生成问题。

Method: 将点云配准建模为条件生成：学习一个连续的点级速度场，将噪声点云传输到配准场景，从中恢复每个视图的位姿。使用轻量级局部特征提取器和测试时刚性约束，直接生成配准点云。

Result: 在成对和多视图配准基准测试中达到最先进结果，特别是在低重叠情况下表现优异。方法能够跨尺度和传感器模态泛化，并支持重定位、多机器人SLAM和多会话地图融合等下游任务。

Conclusion: 将点云配准重新定义为条件生成问题是一种有效的方法，通过学习连续速度场直接生成配准点云，避免了传统对应匹配的局限性，在低重叠情况下表现优异且具有良好的泛化能力。
...
TL;DR: KM-ViPE是一个实时开放词汇的SLAM框架，适用于未标定的单目相机在动态环境中工作，无需深度传感器和离线标定，直接处理原始RGB视频流。


<details>
  <summary>Details</summary>
Motivation: 现有SLAM系统通常需要深度传感器、离线标定，或者缺乏对动态场景的鲁棒性，限制了它们在自主机器人、AR/VR和从互联网规模视频数据中学习的应用。

Method: 将DINO视觉特征与几何约束紧密耦合，通过基于高级特征的自适应鲁棒核处理移动物体和可移动静态物体，融合几何和深度视觉特征并与语言嵌入对齐，实现同时在线定位和开放词汇语义建图。

Result: 与最先进方法相比具有竞争力，现有解决方案要么离线运行，要么需要深度数据和/或里程计估计，或者缺乏动态场景鲁棒性。

Conclusion: KM-ViPE结合了在线操作、未标定单目输入和动态场景鲁棒处理，适合自主机器人和AR/VR应用，推进了具身AI的实际空间智能能力。
...
TL;DR: StyleYourSmile是一种新颖的单样本跨域人脸重定向方法，通过双编码器框架和扩散模型实现身份、表情和风格属性的解耦控制，无需精心策划的多风格配对数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常在真实人脸数据上训练，存在跨域泛化能力不足、需要测试时优化或依赖精心策划的多风格数据集等问题，难以实现域不变的身份表示。

Method: 提出高效的数据增强策略和双编码器框架，分别提取域不变的身份线索和捕获域特定的风格变化，然后利用这些解耦的控制信号来条件化扩散模型，实现跨域人脸表情重定向。

Result: 广泛的实验表明，StyleYourSmile在广泛的视觉领域中实现了卓越的身份保持和重定向保真度。

Conclusion: StyleYourSmile通过创新的数据增强和双编码器设计，成功解决了跨域人脸重定向中的身份保持和风格迁移问题，无需依赖精心策划的多风格配对数据集。
...
TL;DR: SARL是一个空间感知的自监督学习框架，通过三个地图级目标（SAL、PPDA、RAM）增强BYOL架构，在融合视觉-触觉数据上保持注意力焦点、部件组合和几何关系的一致性，在机器人操作任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 机器人操作需要编码局部几何的表征。视觉提供全局上下文但缺乏纹理、硬度等直接测量，而触觉提供这些线索。融合视觉-触觉传感器捕获两种模态，但现有自监督学习框架将特征图压缩为全局向量，丢弃了空间结构，与操作需求不匹配。
...
Result: SARL在融合视觉-触觉数据的六个下游任务中一致优于九个自监督学习基线。在几何敏感的边姿态回归任务中，SARL达到0.3955 MAE，相比次优方法（0.5682 MAE）相对改进30%，接近监督学习上限。
...
<details>
  <summary>Details</summary>
Motivation: 医疗大视觉语言模型在医疗应用中存在幻觉问题，即输出看似合理但实际错误的内容。现有解决方案通常依赖二次解码或回滚程序，会显著降低推理速度，且往往是领域特定的，可能引入模态间或生成内容与真实内容之间的不对齐。

Method: 提出Med-VCD稀疏视觉对比解码方法，包含新颖的令牌稀疏化策略，动态选择视觉信息丰富的令牌，在保持关键视觉上下文的同时减少冗余，平衡效率与可靠性。

Result: 在8个医疗数据集（涵盖眼科、放射科、病理学的视觉问答、报告生成和专门的幻觉基准测试）上评估，Med-VCD相对于基线医疗LVLMs，平均提高事实准确性13%，改善幻觉准确性6%。

Conclusion: Med-VCD是一种有效的医疗大视觉语言模型幻觉缓解方法，无需二次解码的时间开销，在保持效率的同时显著提升输出的可靠性。
...
TL;DR: AdvTraj是首个针对检测跟踪式多目标追踪的在线物理ID操纵攻击，通过对抗性轨迹将攻击者ID转移到目标物体，无需攻击目标检测模块，在CARLA模拟中达到100%成功率


<details>
  <summary>Details</summary>
Motivation: 多目标追踪算法在计算机视觉中至关重要，但对其安全威胁研究不足。现有攻击要么针对单个物体追踪器，要么通过攻击目标检测模块操纵ID，存在模型特定、非鲁棒、仅影响离线数据集等局限

Method: 提出AdvTraj攻击方法：1）使用对抗性轨迹将攻击者ID转移到目标物体；2）不攻击目标检测模块；3）在CARLA模拟环境中测试；4）针对SORT算法进行白盒攻击；5）提出两种通用对抗机动策略

Result: 1）白盒攻击SORT算法成功率100%；2）对SOTA多目标追踪算法攻击迁移率高达93%；3）识别出AdvTraj生成的轨迹模式；4）提出人类可行走的两种通用对抗机动

Conclusion: 揭示了SOTA多目标追踪系统在物体关联阶段的未充分探索的脆弱性，为增强此类系统鲁棒性提供了见解，表明现有追踪算法在对抗性攻击下存在严重安全风险
...
TL;DR: Script是一种无需重新训练的即插即用剪枝方法，通过图结构剪枝和查询条件语义剪枝模块，有效减少多模态大语言模型中的视觉令牌冗余，显著提升推理效率同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型中视觉令牌的快速增长导致内存消耗和推理延迟过高，尤其是在处理高分辨率图像和视频时。现有令牌剪枝方法往往忽略用户查询相关性或受注意力机制限制，降低了适应性和有效性。

Method: 提出Script方法，包含两个模块：1) 图结构剪枝模块，用于移除视觉冗余令牌；2) 查询条件语义剪枝模块，用于保留查询相关的视觉信息。该方法无需重新训练，可跨不同MLLMs通用。

Result: 在14个图像和视频理解基准测试中，Script相比现有剪枝方法在模型效率和预测准确性方面表现更优。在LLaVA-NeXT-7B上，实现了最高6.8倍的预填充加速和10倍的FLOP减少，同时保留了96.88%的原始性能。

Conclusion: Script作为一种即插即用的剪枝方法，能够有效解决多模态大语言模型中视觉令牌冗余问题，显著提升推理效率同时保持任务性能，具有良好的通用性和实用性。
...
### [176] [SpriteHand: Real-Time Versatile Hand-Object Interaction with Autoregressive Video Generation](https://arxiv.org/abs/2512.01960)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于物理引擎的模拟方法依赖于明确定义的刚性物体模型和预设的手势，无法有效捕捉与非刚性或铰接实体（如可变形织物、弹性材料、铰链结构、毛绒表面甚至生物）的动态交互。

Method: SpriteHand采用自回归视频生成框架，输入静态物体图像和想象中手与虚拟物体交互的视频流，实时生成对应的手-物体交互效果。模型采用因果推理架构进行自回归生成，并利用混合后训练方法增强视觉真实性和时间一致性。

Result: 13亿参数的模型在单张NVIDIA RTX 5090 GPU上支持约18FPS、640x368分辨率的实时流式生成，延迟约150毫秒，可连续输出超过一分钟。实验表明，在视觉质量、物理合理性和交互保真度方面均优于生成式和基于引擎的基线方法。

Conclusion: SpriteHand框架能够实时合成多样化的手-物体交互视频，突破了传统物理引擎在模拟复杂交互方面的限制，为处理非刚性、铰接和生物实体提供了有效的解决方案。
...
### [177] [SGDiff: Scene Graph Guided Diffusion Model for Image Collaborative SegCaptioning](https://arxiv.org/abs/2512.01975)
*Xu Zhang,Jin Yuan,Hanwang Zhang,Guojin Zhong,Yongsheng Zang,Jiacheng Lin,Zhiyong Li*

Main category: cs.CV

TL;DR: SegCaptioning任务：通过简单提示（如边界框）生成多样化的（描述，掩码）对，实现图像语义理解的可控灵活输出


<details>
  <summary>Details</summary>
Motivation: 传统图像语义理解任务（如描述或分割）需要用户输入复杂提示且只能输出单一结果，存在提示输入成本高、输出信息有限的问题。需要一种能从简单提示生成多样化语义解释的方法。

Method: 提出场景图引导的扩散模型（SGDiff）：1）提示中心场景图适配器将用户提示映射为场景图；2）场景图引导的双模态Transformer通过扩散过程预测相关的描述-掩码对；3）多实体对比学习损失显式对齐视觉和文本实体。

Result: 在两个数据集上的大量实验表明，SGDiff在SegCaptioning任务上取得优越性能，能够以最小提示输入为描述和分割任务产生有希望的结果。

Conclusion: 提出的SegCaptioning任务和SGDiff模型能够从简单提示生成多样化的语义解释，有效解决了传统方法中提示输入成本高和输出单一的问题，为图像语义理解提供了更灵活的用户交互方式。
...
### [178] [Artemis: Structured Visual Reasoning for Perception Policy Learning](https://arxiv.org/abs/2512.01988)
*Wei Tang,Yanpeng Sun,Shan Zhang,Xiaofan Li,Piotr Koniusz,Wei Li,Na Zhao,Zechao Li*

Main category: cs.CV

TL;DR: Artemis是一个视觉感知策略学习框架，使用结构化提议推理（标签-边界框对）替代纯语言中间推理，在空间和物体中心表示中进行推理，提升视觉感知任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于自然语言的中间推理链在视觉感知任务中往往降低性能，核心问题在于推理形式不匹配：语言推理在非结构化语义空间进行，而视觉感知需要在空间和物体中心空间进行推理。

Method: 提出Artemis框架，采用结构化提议推理，每个中间步骤表示为（标签，边界框）对，捕捉可验证的视觉状态。基于Qwen2.5-VL-3B构建，支持中间状态显式跟踪、提议质量直接监督，避免语言推理的模糊性。

Result: 在grounding和检测任务上表现优异，在计数和几何感知任务上展示出显著泛化能力。在通用MLLM基准测试中也取得有竞争力的性能。
...
### [179] [PAI-Bench: A Comprehensive Benchmark For Physical AI](https://arxiv.org/abs/2512.01989)
...
<details>
  <summary>Details</summary>
Motivation: 物理AI需要能够感知和预测真实世界动态的模型，但目前多模态大语言模型和视频生成模型在这些能力上的表现尚未得到充分理解，需要系统性的评估基准。

Method: 提出了PAI-Bench基准测试，包含2808个真实世界案例，涵盖视频生成、条件视频生成和视频理解三个任务，使用任务对齐的指标来评估物理合理性和领域特定推理能力。

Result: 视频生成模型虽然视觉保真度高，但难以保持物理一致的动态；多模态大语言模型在预测和因果解释方面表现有限，表明当前系统处理物理AI的感知和预测需求仍处于早期阶段。

Conclusion: PAI-Bench为评估物理AI建立了现实基础，揭示了未来系统需要解决的关键差距，当前模型在物理一致性方面仍需改进。
...
### [180] [Learning Visual Affordance from Audio](https://arxiv.org/abs/2512.02005)
...
Result: AVAGFormer在AV-AG任务上达到最先进性能，超越相关任务基线；全面分析突出了AV-AG与AVS的区别、端到端建模的优势以及各组件贡献

Conclusion: 音频为可供性定位提供了有价值的线索，AVAGFormer模型有效解决了音频-视觉可供性定位任务，代码和数据集已开源
...
### [181] [MV-TAP: Tracking Any Point in Multi-View Videos](https://arxiv.org/abs/2512.02006)
*Jahyeok Koo,Inès Hyeonsu Kim,Mungyeom Kim,Junghyun Park,Seohyun Park,Jaeyeong Kim,Jung Yi,Seokju Cho,Seungryong Kim*

Main category: cs.CV

TL;DR: MV-TAP是一种新颖的多视角点跟踪方法，通过利用跨视角信息和注意力机制来跟踪动态场景中的点轨迹


<details>
  <summary>Details</summary>
Motivation: 多视角相机系统能够提供复杂真实场景的丰富观测，理解多视角设置中的动态物体已成为各种应用的核心需求

Method: MV-TAP利用相机几何和跨视角注意力机制来聚合跨视角的时空信息，实现多视角视频中更完整可靠的点轨迹估计

Result: MV-TAP在具有挑战性的基准测试中优于现有的点跟踪方法，为多视角点跟踪研究建立了有效的基线

Conclusion: MV-TAP通过整合跨视角信息，在多视角点跟踪任务中表现出色，为这一领域的研究提供了新的有效方法
...
### [182] [AirSim360: A Panoramic Simulation Platform within Drone View](https://arxiv.org/abs/2512.02009)
...
TL;DR: AirSim360是一个用于无人机视角的全景数据仿真平台，提供大规模多样化数据，支持像素级几何、语义和实体级理解，包含交互式行人感知系统和自动轨迹生成，已收集超过60K全景样本。
...
Method: 提出AirSim360仿真平台，采用渲染对齐的数据和标注范式实现像素级几何、语义和实体级理解；包含交互式行人感知系统建模人类行为；开发自动轨迹生成范式支持导航任务。

Result: 收集了超过60K全景样本，通过多个任务的广泛实验验证了仿真器的有效性，是首个系统建模4D真实世界全景环境的平台。

Conclusion: AirSim360为360度全景理解提供了全面的仿真平台，解决了数据缺乏问题，支持多种任务，平台工具包、插件和数据集将公开可用。
...
### [183] [TUNA: Taming Unified Visual Representations for Native Unified Multimodal Models](https://arxiv.org/abs/2512.02014)
...
TL;DR: TUNA是一个统一的多模态模型，通过级联VAE编码器和表示编码器构建统一的连续视觉表示空间，在单一框架中同时处理理解和生成任务，在图像和视频理解、生成、编辑等多个任务上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型（UMMs）通常使用解耦的表示方法，导致理解和生成任务之间存在表示格式不匹配的问题。TUNA旨在通过构建统一的连续视觉表示空间来解决这一问题，实现真正的端到端多模态处理。

Method: TUNA采用级联架构：首先使用VAE编码器处理视觉输入，然后通过表示编码器构建统一的连续视觉表示空间。这种设计允许模型在单一框架中同时处理图像和视频的理解与生成任务，避免了传统解耦方法中的表示不匹配问题。

Result: 实验表明：1）TUNA在图像和视频理解、生成、编辑等多个基准测试中达到最先进水平；2）更强的预训练表示编码器在所有多模态任务中都能带来更好的性能；3）在统一设置下，理解和生成任务的联合训练使两者相互促进而非相互干扰。

Conclusion: TUNA通过统一的连续视觉表示空间设计，成功解决了传统UMMs中表示格式不匹配的问题，证明了统一表示设计的有效性和可扩展性，为多模态理解和生成任务提供了更优的解决方案。
...
### [184] [Generative Video Motion Editing with 3D Point Tracks](https://arxiv.org/abs/2512.02015)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法在复杂物体运动下缺乏精确控制能力，2D轨迹无法处理深度顺序和遮挡问题

Method: 使用3D点轨迹作为条件，建立源视频和目标运动之间的稀疏对应关系，通过两阶段训练（合成和真实数据）

Result: 支持相机/物体联合操控、运动转移、非刚性变形等多种编辑功能，保持时空一致性

Conclusion: 3D轨迹跟踪为视频运动编辑提供了更精确的控制能力，解锁了视频编辑的新创作潜力
...
### [185] [Objects in Generated Videos Are Slower Than They Appear: Models Suffer Sub-Earth Gravity and Don't Know Galileo's Principle...for now](https://arxiv.org/abs/2512.02016)
*Varun Varma Thozhiyoor,Shivam Tripathi,Venkatesh Babu Radhakrishnan,Anand Bhattad*

Main category: cs.CV

TL;DR: 视频生成器在重力物理规律建模上存在系统性偏差，通过无单位双物体协议揭示其违反伽利略等效原理，但可通过少量数据微调部分修正


<details>
  <summary>Details</summary>
Motivation: 评估视频生成器作为世界模型的潜力，需要测试其对基本物理定律（特别是重力）的编码和理解能力。现有视频生成器在重力表现上存在明显偏差，但需要区分这是真实的物理建模缺陷还是度量尺度模糊造成的假象。

Method: 1. 首先检验物理错误是否源于度量尺度模糊（如帧率假设错误）；2. 引入无单位双物体协议，测试时间平方比与高度比的关系（t₁²/t₂² = h₁/h₂），该关系独立于重力加速度g、焦距和尺度；3. 使用轻量级低秩适配器在100个单球下落视频上进行微调。

Result: 1. 开箱即用的视频生成器一致生成物体以较慢加速度下落；2. 即使进行时间重缩放也无法纠正高方差的重力伪影；3. 相对测试揭示了伽利略等效原理的违反；4. 微调后有效重力加速度从1.81 m/s²提升到6.43 m/s²（达到地球重力的65%）；5. 专用适配器能零样本泛化到双球下落和斜面场景。

Conclusion: 视频生成器在重力物理规律建模上存在系统性缺陷，但通过针对性的少量数据微调可以部分修正特定物理定律，这为改进生成模型作为世界模型的物理保真度提供了可行路径。
...
### [186] [Data-Centric Visual Development for Self-Driving Labs](https://arxiv.org/abs/2512.02018)
*Anbang Liu,Guanzhong Hu,Jiayi Wang,Ping Guo,Han Liu*

Main category: cs.CV

TL;DR: 本文提出了一种混合数据生成管道，融合真实和虚拟数据来解决自驱动实验室中气泡检测的数据稀缺问题，特别是负样本的获取难题。


<details>
  <summary>Details</summary>
Motivation: 自驱动实验室需要高精度模型，但训练这些模型需要大量标注数据，特别是负样本在常规实践中难以获取。移液作为SDL中最关键和精度敏感的操作，其气泡检测面临数据稀缺的挑战。

Method: 构建混合数据生成管道：真实数据采用人机协同方案，结合自动化采集和选择性人工验证；虚拟数据采用参考条件提示引导的图像生成技术，并通过筛选验证确保可靠性。两种数据源结合产生类别平衡的数据集。

Result: 在保留的真实测试集上，仅使用自动采集的真实图像训练的模型达到99.6%准确率；混合真实和生成数据训练时保持99.4%准确率，同时显著减少了数据收集和审核的工作量。

Conclusion: 该方法为SDL工作流程提供视觉反馈数据提供了可扩展且经济高效的策略，为解决罕见事件检测和更广泛视觉任务中的数据稀缺问题提供了实用解决方案。
...
### [187] [Chunking Strategies for Multimodal AI Systems](https://arxiv.org/abs/2512.00185)
*Shashanka B R,Mohith Charan R,Seema Banu F*

Main category: cs.AI

TL;DR: 这是一篇关于多模态分块策略的综述论文，系统性地梳理了文本、图像、音频、视频和跨模态数据的分块方法，分析了各种方法的优缺点，并指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 为研究人员和从业者提供多模态分块策略的技术基础和设计空间，帮助开发更有效和高效的多模态AI系统，推动在现实应用中处理复杂模态、提高处理准确性和生成连贯性的创新。

Method: 采用综述研究方法，建立全面的分类体系和技术分析框架，针对每种模态（文本、图像、音频、视频、跨模态数据）分别分析经典和现代分块方法，包括固定大小token窗口、递归文本分割、对象中心视觉分块、基于静音的音频分割、视频场景检测等，并分析每种方法的方法论、支持工具、优势和挑战。

Result: 提供了多模态分块策略的全面分类和技术分析，识别了粒度-上下文权衡和多模态对齐相关的挑战，探索了保持跨模态对齐和语义一致性的新兴策略，提供了比较性见解，并突出了异步信息密度和噪声对齐信号等开放问题。

Conclusion: 该综述为多模态AI系统的分块策略提供了系统性的技术基础，指出了未来在自适应、基于学习和任务特定分块方面的研究机会，为开发更有效和高效的多模态处理管道铺平了道路。
...
### [188] [A Rosetta Stone for AI Benchmarks](https://arxiv.org/abs/2512.00193)
*Anson Ho,Jean-Stanislas Denain,David Atanasov,Samuel Albanie,Rohin Shah*

Main category: cs.AI

TL;DR: 提出一个统计框架，将不同AI基准测试结果整合到统一的数值尺度上，用于长期追踪AI能力发展趋势和预测未来进展。


<details>
  <summary>Details</summary>
Motivation: 现有的AI基准测试往往在发布后几个月或几年内就达到饱和，难以研究AI能力的长期发展趋势。需要一种方法能够跨越不同基准测试和时间段来比较AI模型的能力。

Method: 构建一个统计框架，将不同基准测试的结果"缝合"在一起，将模型能力和基准测试难度映射到统一的数值尺度上。这种方法不需要假设能力如何随时间或训练计算量演变，就像一个"罗塞塔石碑"一样，即使模型没有在相同的基准测试上评估，也能进行比较。

Result: 该框架展示了三个应用：1) 测量AI随时间进展的速度并预测未来能力；2) 估计算法效率的改进速率，发现比先前工作更高的估计值但大体一致；3) 能够检测AI进展中的快速加速。

Conclusion: 提出的统计框架为解决AI基准测试饱和问题提供了有效方法，能够实现跨时间、跨基准测试的AI能力比较，为研究长期AI发展趋势和预测未来进展提供了有力工具。
...
### [189] [Reasoning Under Pressure: How do Training Incentives Influence Chain-of-Thought Monitorability?](https://arxiv.org/abs/2512.00218)
*Matt MacDermott,Qiyao Wei,Rada Djoneva,Francis Rhys Ward*

Main category: cs.AI

TL;DR: 研究不同训练激励对AI推理过程可监控性的影响，发现对抗性优化会降低监控性能，而直接优化可监控性并不总能带来改善


<details>
  <summary>Details</summary>
Motivation: AI系统输出自然语言推理过程为安全监控提供了机会，但推理过程是否真实反映底层思考过程以及可监控程度可能受训练方式影响，需要研究不同训练激励如何影响模型的可监控性

Method: 引入新的方法论来测量可监控性：根据监控器能否使用模型的推理过程预测关键潜在变量；研究不同训练激励的影响，包括长度惩罚、KL正则化、对抗性优化和直接优化可监控性

Result: 在控制准确性的情况下，常见激励（长度惩罚和KL正则化）没有发现一致影响；对抗性优化会降低监控性能；直接优化可监控性并不能可靠地带来改善

Conclusion: 训练激励确实会影响AI推理过程的可监控性，对抗性优化会损害监控能力，而直接优化可监控性并非有效策略，需要进一步研究如何在不损害性能的情况下提高可监控性
...
### [190] [CogEvo-Edu: Cognitive Evolution Educational Multi-Agent Collaborative System](https://arxiv.org/abs/2512.00331)
*Yefeng Wu,Yuchen Song,Yecheng Zhao,Ling Wu,Shan Wan*

Main category: cs.AI

TL;DR: 论文提出CogEvo-Edu系统，将检索、记忆和控制视为耦合的认知演化过程，通过分层多智能体架构提升STEM教育中LLM对话导师的能力，在DSP领域取得显著效果提升。


<details>
  <summary>Details</summary>
Motivation: 当前STEM教育中部署的LLM对话导师大多采用单一LLM加静态RAG流水线，在复杂领域如数字信号处理中存在局限性：难以维持连贯的长期学生模型、管理异构知识库、适应长期交互的教学策略。

Method: 提出CogEvo-Edu分层教育多智能体系统，包含认知感知层（维护双重记忆并进行置信度加权整合）、知识演化层（为知识块分配时空价值驱动激活、语义压缩和遗忘）、元控制层（将教学制定为分层顺序决策，通过双重内外循环协调专业智能体并联合调整CPL/KEL超参数）。

Result: 构建DSP-EduBench垂直基准测试，使用三模型LLM-as-a-Judge集成评估，CogEvo-Edu将总分从5.32提升至9.23，在所有六个指标上均优于静态RAG、简单记忆和单智能体变体。

Conclusion: 将学生档案、知识库和教学策略联合演化的方法在复杂STEM教育领域具有显著价值，证明了将检索、记忆和控制视为耦合认知演化过程的合理性。
...
### [191] [Echo-N1: Affective RL Frontier](https://arxiv.org/abs/2512.00344)
...
TL;DR: 本文提出首个能够实时推断用户个性并优化模型行为以适应个性化对话偏好的RL框架，挑战了传统认为RL不适用于主观对话领域的观点，在人类化交互质量上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前LLM领域过度关注机器已擅长的数学、代码和确定性推理任务，而忽视了真正定义人类智能的主观、情感基础、个性敏感的对话领域。这一领域常被认为过于主观难以形式化，不适合传统RL方法，但作者认为这是一个可解决且具有变革性的RL问题。

Method: 提出首个能够实时推断用户个性并优化模型行为以适应个性化对话偏好的强化学习框架。同时引入了首个动态情商评估套件来量化这些改进。模型名为Echo-N1。

Result: 该方法在非可验证环境中不仅没有崩溃，反而产生了持续、稳健且显著的改进，在人类化交互质量上表现优异。Echo-N1模型的表现远超其基础版本，并优于专有的Doubao 1.5 Character模型。

Conclusion: 这项工作为强化学习开辟了新前沿：优化模型以适应对话中深刻主观、深刻人性化的维度，证明了RL在主观对话领域的可行性和有效性。
...
### [192] [Debate with Images: Detecting Deceptive Behaviors in Multimodal Large Language Models](https://arxiv.org/abs/2512.00349)
*Sitong Fang,Shiyi Hou,Kaile Wang,Boyuan Chen,Donghai Hong,Jiayi Zhou,Josef Dai,Yaodong Yang,Jiaming Ji*

Main category: cs.AI

TL;DR: 该论文揭示了前沿AI系统在能力提升的同时，隐藏着更危险的欺骗性风险，特别是多模态欺骗。研究提出了首个多模态欺骗基准MM-DeceptionBench，并开发了基于图像辩论的多智能体监控框架来检测这种欺骗行为。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统能力提升，欺骗性风险日益严重且从文本扩展到多模态领域。当前研究主要局限于文本欺骗，对多模态大语言模型的欺骗风险缺乏系统探索，现有监控方法在多模态欺骗检测上存在盲点。

Method: 1) 创建MM-DeceptionBench基准，涵盖六类欺骗行为，评估模型通过视觉和文本模态进行的策略性操纵；2) 提出"基于图像的辩论"多智能体监控框架，强制模型基于视觉证据进行辩论，提高欺骗策略的可检测性。

Result: 实验表明，提出的监控方法在所有测试模型上都显著提高了与人类判断的一致性，在GPT-4o上将Cohen's kappa提高了1.5倍，准确率提高了1.25倍。

Conclusion: 多模态欺骗是前沿AI系统的重要安全风险，需要专门的评估基准和监控方法。提出的MM-DeceptionBench和基于图像的辩论框架为检测和缓解多模态欺骗提供了有效工具。
...
### [193] [GreenPlanner: Practical Floorplan Layout Generation via an Energy-Aware and Function-Feasible Generative Framework](https://arxiv.org/abs/2512.00406)
*Pengyu Zeng,Yuqin Dai,Jun Yin,Jing Zhong,Ziyang Han,Chaoyang Shi,ZhanXiang Jin,Maowei Jiang,Yuxing Han,Shuai Lu*

Main category: cs.AI

TL;DR: GreenPlanner是一个统一的能源和功能感知生成框架，通过自动化评估和生成来创建符合规范的空间功能布局，将评估速度提升10^5倍，准确率>99%，设计效率比专业建筑师提高87%。
...
Method: GreenPlanner包含四个组件：1) 用于学习约束先验的标记设计可行性数据集；2) 用于预测能源性能和空间功能有效性的快速实用设计评估器(PDE)；3) 通过PDE引导过滤得到的绿色平面数据集(GreenPD)，将用户需求与规范合规布局配对；4) 在GreenPD上训练并使用PDE反馈的GreenFlow生成器，用于可控的规范感知生成。

Result: GreenPlanner将评估速度加速超过10^5倍，准确率超过99%，消除无效样本，设计效率比专业建筑师提高87%。

Conclusion: GreenPlanner提供了一个统一的框架，将设计评估和生成相结合，能够高效生成符合能源和功能约束的建筑平面图，显著提升设计效率和合规性。
...
### [194] [Mind the data gap: Missingness Still Shapes Large Language Model Prognoses](https://arxiv.org/abs/2512.00479)
*Yuta Kobayashi,Vincent Jeanselme,Shalmali Joshi*

Main category: cs.AI

TL;DR: LLM在医疗数据预测中的表现受缺失模式显著影响，缺失指示符对不同模型有正负不同影响，大模型受益而小模型可能受损


<details>
  <summary>Details</summary>
Motivation: 研究缺失数据模式对大型语言模型零样本预测性能的影响，特别是在医疗领域，传统机器学习中缺失数据的影响常被忽视，而LLM范式可能进一步掩盖这一问题

Method: 使用哥伦比亚大学医学中心和MIMIC-IV医疗数据进行系列实验，分析缺失模式对LLM零样本预测性能的影响，特别测试了在提示中显式包含缺失指示符的效果

Result: 缺失模式显著影响LLM的零样本预测性能；显式包含缺失指示符对某些LLM有益，但对其他LLM有害，影响不一致；较大模型从这些干预中受益，而较小模型可能受到负面影响

Conclusion: 需要更透明的记录和系统评估（信息性）缺失表示对下游性能的影响，LLM范式可能进一步掩盖缺失数据的影响，这一问题甚至在传统机器学习中也常被忽视
...
### [195] [Clinical-R1: Empowering Large Language Models for Faithful and Comprehensive Reasoning with Clinical Objective Relative Policy Optimization](https://arxiv.org/abs/2512.00601)
*Boyang Gu,Hongjian Zhou,Bradley Max Segal,Jinge Wu,Zeyu Cao,Hantao Zhong,Lei Clifton,Fenglin Liu,David A. Clifton*

Main category: cs.AI

TL;DR: CRPO是一种用于临床推理的多目标可验证强化学习方法，通过整合基于规则和可验证的奖励信号，联合优化准确性、忠实性和全面性，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 当前LLM后训练方法（如GRPO）主要奖励正确性，这与医学等高风险领域所需的多维目标（如忠实性和全面性）不一致，需要开发专门针对临床推理原则的对齐方法。

Method: 提出临床目标相对策略优化（CRPO），一种可扩展的多目标可验证强化学习方法，整合基于规则和可验证的奖励信号，联合优化准确性、忠实性和全面性，无需人工标注。

Result: 训练了Clinical-R1-3B模型，在三个基准测试中显示CRPO在真实性和完整性方面显著优于标准GRPO，同时保持准确性提升。

Conclusion: CRPO为LLM推理与临床目标对齐提供了可扩展途径，使医疗AI系统更安全、更具协作性，同时突显了多目标可验证RL方法在医学领域LLM后训练扩展中的潜力。
...
### [196] [EDIT: Early Diffusion Inference Termination for dLLMs Based on Dynamics of Training Gradients](https://arxiv.org/abs/2512.00670)
*He-Yen Hsieh,Hong Wang,H. T. Kung*

Main category: cs.AI

TL;DR: EDIT方法通过监测推理稳定性来自适应提前终止扩散模型的去噪过程，利用训练梯度动态信息减少推理步骤，在保持准确性的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型通过迭代去噪生成答案，但答案通常在完成所有步骤前就已稳定。现有方法在模型发布时丢弃了训练过程中丰富的优化动态信息，这些信息包含了参数重要性和推理路径的宝贵元数据。

Method: 提出EDIT方法：1）训练时从AdamW聚合的LoRA更新中提取推理图作为推理路径的紧凑表示；2）推理时监测token激活与推理图的对齐度；3）将对齐分数转换为当前去噪步骤已解掩码token的分布；4）当连续步骤间KL散度低于阈值时检测收敛并提前终止。

Result: 在推理基准测试中，EDIT将扩散步骤减少11.8%至68.3%，在大多数设置中保持或提高准确性，存储开销仅约0.02%（约1.5-2MB），适用于8GB模型的32个块中的所有QKV模块。

Conclusion: 通过利用训练梯度动态，EDIT为减少扩散大语言模型推理时间和成本开辟了新研究方向，实现了计算效率与模型性能的有效平衡。
...
### [197] [Model of human cognition](https://arxiv.org/abs/2512.00683)
*Wu Yonggang*

Main category: cs.AI

TL;DR: 提出一种神经理论框架，解释智能系统的涌现机制，旨在解决大语言模型缺乏可解释性、缺乏统一理论和运营成本过高的问题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型发展面临三大限制：缺乏可解释性、缺少统一理论框架、运营成本过高。需要一种既功能稳健又生物学合理的智能涌现理论。

Method: 提出一种神经理论框架，该框架基于生物学合理的机制，解释智能系统如何涌现。该模型为决策、问题解决等认知过程提供理论洞察，并提供了计算高效的方法。

Result: 该框架为创建可解释和可泛化的人工智能提供了理论基础和计算高效的方法，有望解决当前LLM面临的核心挑战。

Conclusion: 提出的神经理论框架为智能系统的涌现提供了统一的理论解释，同时具备生物学合理性和计算效率，有望推动可解释、可泛化人工智能的发展。
...
### [198] [SpeContext: Enabling Efficient Long-context Reasoning with Speculative Context Sparsity in LLMs](https://arxiv.org/abs/2512.00722)
*Jiaming Xu,Jiayi Pan,Hanzhen Wang,Yongkang Zhou,Jiancai Ye,Yu Wang,Guohao Dai*

Main category: cs.AI

TL;DR: SpeContext：一种利用蒸馏语言模型作为检索算法的长上下文推理系统，通过算法、系统和编译三级优化，在云和边缘环境中实现显著性能提升


<details>
  <summary>Details</summary>
Motivation: 传统检索算法目标与LLM对齐，类似于LLM中的知识蒸馏目标。从信息论角度分析蒸馏语言模型与原始LLM在信息关注上的相似性，提出利用DLM作为检索算法的新范式

Method: 1. 算法层面：基于DLM的注意力头权重设计轻量级检索头，通过剪枝冗余减少90%以上参数；2. 系统层面：通过弹性加载策略设计异步预取数据流，重叠KV缓存检索与LLM计算；3. 编译层面：构建理论内存模型并实现自适应内存管理系统，最大化GPU内存利用率

Result: 在云环境中相比Huggingface框架实现最高24.89倍吞吐量提升，边缘环境中实现10.06倍加速，准确率损失可忽略，推动了准确率与吞吐量的帕累托前沿

Conclusion: SpeContext通过算法-系统-编译协同设计，有效利用蒸馏语言模型作为检索算法，在资源受限的云和边缘环境中显著提升长上下文推理性能，为高效LLM推理提供了新思路
...
### [199] [Probing the "Psyche'' of Large Reasoning Models: Understanding Through a Human Lens](https://arxiv.org/abs/2512.00729)
*Yuxiang Chen,Zuohan Wu,Ziwei Wang,Xiangning Yu,Xujia Li,Linyi Yang,Mengyue Yang,Jun Wang,Lei Chen*

Main category: cs.AI

TL;DR: 该论文提出了一个基于人类心理过程的分类法来理解大型推理模型(LRMs)的"心理"，开发了自动标注框架CAPO，并基于277,534个原子推理步骤的分析揭示了当前LRMs的局限性，特别是后答案"双重检查"的表面性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型(LRMs)因其解决复杂任务的卓越能力而受到关注，其推理过程中表现出类似人类的行为。为了深入理解LRMs的"心理"和智能本质，需要建立一个系统化的分析框架。

Method: 1. 提出基于人类心理过程的分类法，包含5组17个类别；2. 应用该分类法分析当前LRMs，构建包含277,534个原子推理步骤的标注数据集；3. 开发自动标注框架CAPO，利用大语言模型生成基于分类法的标注；4. 分析当代LRMs并提炼改进训练和后训练的可行建议。

Result: 1. CAPO框架在一致性方面优于基线方法，与人类专家标注更一致；2. 分析发现当前流行的后答案"双重检查"大多是表面的，很少产生实质性修订；3. 激励全面的多步骤反思比简单的自我监控更有效；4. 分类法、CAPO和洞察为理解和推进LRM推理提供了原则性、可扩展的路径。

Conclusion: 该研究通过建立基于人类认知的分类法、开发自动标注工具CAPO，以及对LRMs的深入分析，为理解和改进大型推理模型提供了系统化的框架。研究发现当前模型的自我监控机制存在局限性，需要转向更全面的反思机制。
...
### [200] [BioPro: On Difference-Aware Gender Fairness for Vision-Language Models](https://arxiv.org/abs/2512.00807)
*Yujie Lin,Jiayao Ma,Qingguo Hu,Derek F. Wong,Jinsong Su*

Main category: cs.AI

TL;DR: BioPro：一种无需训练的多模态选择性去偏框架，通过正交投影在保持明确性别特征的同时减少中性语境中的性别偏见


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型存在显著的性别偏见，现有公平性干预方法采用"差异无视"视角，对所有情境统一处理，无法区分需要中立的情境和需要保留群体特定属性的情境

Method: 提出BioPro（Bias Orthogonal Projection）框架，通过反事实嵌入识别低维性别变化子空间，应用投影选择性地中和性别相关信息，完全无需训练

Result: BioPro能有效减少中性情境中的性别偏见，同时在明确情境中保持性别忠实性，并能推广到连续偏置变量（如场景亮度）

Conclusion: BioPro为实现视觉语言模型的选择性公平性提供了有前景的方向，超越了性别偏见，具有更广泛的适用性
...
### [201] [Med-CMR: A Fine-Grained Benchmark Integrating Visual Evidence and Clinical Logic for Medical Complex Multimodal Reasoning](https://arxiv.org/abs/2512.00818)
...
TL;DR: Med-CMR是一个细粒度医学复杂多模态推理基准，通过系统能力分解、挑战性任务设计和高质量数据覆盖，评估MLLMs在临床医学中的复杂推理能力。


<details>
  <summary>Details</summary>
Motivation: 虽然MLLMs开始应用于临床工作流程，但其执行复杂医学推理的能力尚不明确，需要专门的基准来评估医学多模态推理能力。

Method: Med-CMR通过三个核心特征构建：1）系统能力分解，将医学多模态推理分为细粒度视觉理解和多步推理；2）挑战性任务设计，涵盖视觉理解的三个关键维度和四个临床相关场景的推理；3）广泛高质量数据覆盖，包含20,653个VQA对，经过两阶段专家审核。

Result: 评估18个最先进的MLLMs，GPT-5表现最佳（MCQ准确率57.81%，开放题得分48.70），优于Gemini 2.5 Pro和Qwen3-VL-235B-A22B。专业医学MLLMs并未稳定优于通用模型，长尾泛化是主要失败模式。

Conclusion: Med-CMR为医学MLLMs提供了视觉推理整合和罕见病例鲁棒性的压力测试，为未来临床系统建立了严格的评估标准。
...
### [202] [Assessing model error in counterfactual worlds](https://arxiv.org/abs/2512.00836)
*Emily Howerton,Justin Lessler*

Main category: cs.AI

TL;DR: 论文提出三种方法来估计反事实场景建模中的模型误差，强调模型校准误差比场景偏差对决策更重要，并提供了评估场景预测的建议。


<details>
  <summary>Details</summary>
Motivation: 尽管反事实场景建模在规划和决策中普遍使用，但很少进行回顾性评估。预测与观测之间的差异来自场景偏差和模型校准误差，而后者对评估模型在决策中的价值更为重要，但需要在反事实世界中估计模型误差。

Method: 提出并对比了三种估计反事实误差的方法，通过模拟实验展示了每种方法的优缺点和局限性。

Result: 在模拟实验中验证了三种方法的有效性，分析了各自的优势和限制，为反事实误差估计提供了具体建议。

Conclusion: 提供了估计反事实误差的建议，并讨论了使场景预测可评估所需的设计要素，强调模型校准误差评估对决策制定的重要性。
...
### [203] [ARCADIA: Scalable Causal Discovery for Corporate Bankruptcy Analysis Using Agentic AI](https://arxiv.org/abs/2512.00839)
*Fabrizio Maturo,Donato Riccio,Andrea Mazzitelli,Giuseppe Bifulco,Francesco Paolone,Iulia Brezeanu*

Main category: cs.AI

TL;DR: ARCADIA是一个用于因果发现的智能AI框架，通过结合大语言模型推理和统计诊断来构建有效、时序一致的因果结构，相比传统算法在真实高风险领域表现更优。


<details>
  <summary>Details</summary>
Motivation: 传统因果发现算法在处理真实世界高风险领域时存在局限性，需要更稳定、可解释且能自主建模的框架。ARCADIA旨在通过智能AI方法提升因果结构的可靠性和可解释性。

Method: ARCADIA框架整合大语言模型推理与统计诊断，通过约束引导提示和因果有效性反馈迭代优化候选有向无环图，构建完全可解释、支持干预的管道。

Result: 在企业破产数据实验中，ARCADIA相比NOTEARS、GOLEM和DirectLiNGAM等传统算法，能产生更可靠的因果图，同时提供完全可解释的干预就绪管道。

Conclusion: ARCADIA框架通过展示智能大语言模型如何参与自主科学建模和结构化因果推断，推动了AI在因果发现领域的发展，为高风险领域提供了稳定、可解释的解决方案。
...
### [204] [One Swallow Does Not Make a Summer: Understanding Semantic Structures in Embedding Spaces](https://arxiv.org/abs/2512.00852)
*Yandong Sun,Qiang Huang,Ziwei Xu,Yiqun Sun,Yixuan Tang,Anthony K. H. Tung*

Main category: cs.AI

TL;DR: 该论文提出了语义场子空间（SFS）和SAFARI算法，用于在嵌入空间中捕捉局部语义邻域并发现层次化语义结构，通过高效的语义偏移近似实现15-30倍加速，在多个文本和图像数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 嵌入空间是现代AI的基础，但现有方法要么牺牲语义连贯性来保持结构规律性，要么需要高计算成本来提高可解释性，缺乏既能保持几何结构又能捕捉语义关系的有效方法。

Method: 1. 提出语义场子空间（SFS），一种保持几何结构、上下文感知的表示方法，捕捉嵌入空间中的局部语义邻域；2. 提出SAFARI算法，使用新颖的语义偏移度量来量化SFS演化过程中的语义变化，无监督地发现层次化语义结构；3. 开发高效的语义偏移近似方法，替代昂贵的SVD计算。

Result: 1. 语义偏移近似方法实现15-30倍加速，平均误差低于0.01；2. 在6个真实世界文本和图像数据集上的评估显示，SFS在分类和政治偏见检测等任务中优于标准分类器；3. SAFARI始终能发现可解释且可泛化的语义层次结构。

Conclusion: 该工作为嵌入空间中的语义理解提供了一个统一框架，能够结构化、分析和扩展语义理解，解决了现有方法在语义连贯性和计算效率方面的局限性。
...
### [205] [Hybrid-DMKG: A Hybrid Reasoning Framework over Dynamic Multimodal Knowledge Graphs for Multimodal Multihop QA with Knowledge Editing](https://arxiv.org/abs/2512.00881)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态知识编辑(MKE)基准主要评估最终答案正确性，但忽视了中间推理质量和视觉重述输入的鲁棒性。需要一个新的基准来评估多模态多跳推理能力。

Method: 提出了MMQAKE基准测试，评估2-5跳多模态推理链；提出了Hybrid-DMKG混合推理框架，基于动态多模态知识图(DMKG)，使用大语言模型分解问题，多模态检索模型定位更新事实，通过关系链接预测和RAG推理两条路径进行混合推理。

Result: 实验结果显示，当前MKE方法在多模态推理链更新和推理方面表现不佳；Hybrid-DMKG在MMQAKE上显著优于现有MKE方法，实现了更高的准确性和对知识更新的更好鲁棒性。

Conclusion: MMQAKE基准填补了多模态知识编辑评估的空白，Hybrid-DMKG框架通过动态多模态知识图和混合推理机制，有效提升了多模态多跳推理的准确性和鲁棒性。
...
### [206] [Integrating Causal Foundation Model in Prescriptive Maintenance Framework for Optimizing Production Line OEE](https://arxiv.org/abs/2512.00969)
*Felix Saretzky,Lucas Andersen,Thomas Engel,Fazel Ansari*

Main category: cs.AI

TL;DR: 本文提出基于因果机器学习的方法，从预测性维护转向规范性维护，通过因果基础模型模拟潜在修复措施，量化干预对系统KPI的影响，为生产线提供数据驱动的行动推荐。


<details>
  <summary>Details</summary>
Motivation: 制造业向规范性维护转型受限于对预测模型的依赖，这些模型往往基于虚假相关性而非真正的故障因果驱动因素，导致误诊和无效干预。关键挑战在于：虽然能预测故障可能发生，但缺乏系统方法来理解故障原因，从而无法确定最有效的干预措施。

Method: 提出基于因果机器学习的方法，使用预训练的因果基础模型作为"假设分析"模型来估计潜在修复措施的效果。通过测量每个干预对系统级KPI（如总体设备效率OEE）的因果效应，为生产线提供数据驱动的行动排名推荐。

Result: 使用半合成制造数据评估模型，并与基线机器学习模型进行比较。该方法不仅能识别根本原因，还能量化其操作影响，为工程师在因果环境中测试潜在解决方案提供技术基础。
...
### [207] [IndiMathBench: Autoformalizing Mathematical Reasoning Problems with a Human Touch](https://arxiv.org/abs/2512.00997)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决数学定理证明评估的挑战，特别是自然语言问题形式化的困难，需要创建高质量、经过人工验证的基准来评估AI模型在数学推理和定理证明方面的能力。

Method: 采用AI驱动的人工辅助流程：1) 从印度数学奥林匹克竞赛中收集问题；2) 通过类别检索、迭代编译器反馈和多模型集成生成候选形式化；3) 专家通过交互式仪表板验证，仪表板提供自动化质量摘要。

Result: 创建了包含312个正式Lean 4定理的基准，评估多个前沿模型显示：自动形式化仍然具有挑战性，语法有效性和语义正确性之间存在显著差距，即使通过迭代优化，定理证明成功率仍然较低。

Conclusion: IndiMathBench为数学推理提供了一个具有挑战性的测试平台，展示了当前AI在数学定理证明方面的局限性，特别是在将自然语言问题准确形式化方面的困难。
...
### [208] [ChartAnchor: Chart Grounding with Structural-Semantic Fidelity](https://arxiv.org/abs/2512.01017)
*Xinhang Li,Jingbo Zhou,Pengfei Luo,Yixiong Xiao,Tong Xu*

Main category: cs.AI

TL;DR: ChartAnchor是一个包含8k+图表-表格-代码三元组的综合基准测试，涵盖30种图表类型，通过图表到代码生成和受控图表到表格重建两个互补任务，评估多模态大语言模型在结构化图表理解方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在图表多样性、任务孤立性和评估框架完整性方面存在局限，无法全面评估图表接地（chart grounding）能力。图表接地需要模型在视觉外观和结构化语义之间进行双向对齐，直接反映模型在数值推理、多模态对齐和结构重建方面的能力，在现实场景中有重要应用价值。

Method: 提出ChartAnchor基准，包含8k+图表-表格-代码三元组，涵盖30种图表类型，来自多样化的真实世界和增强数据源。引入两个互补任务：图表到代码生成（合成可执行代码复制图表）和受控图表到表格重建（使用预定义表头提取精确数据），实现视觉和数值保真度的交叉验证。采用多层次评估框架，整合语义验证、风格分析和感知度量，评估结构和内容层面的正确性。

Result: 在多模态大语言模型上的广泛实验揭示了数值精度和代码合成方面的关键局限性，强调了超越表面感知的结构化推理的必要性。通过统一符号和数据驱动的接地方法，ChartAnchor为图表接地建立了严格的基础。

Conclusion: ChartAnchor通过统一符号和数据驱动的接地方法，为图表接地建立了严格的基准测试框架，为推进多模态大语言模型在科学、金融和工业领域的应用提供了有意义的见解。
...
### [209] [Evaluating Legal Reasoning Traces with Legal Issue Tree Rubrics](https://arxiv.org/abs/2512.01020)
*Jinu Lee,Kyoung-Woon On,Simeng Han,Arman Cohan,Julia Hockenmaier*

Main category: cs.AI

TL;DR: LEGIT是一个大规模法律推理数据集，通过将法庭判决转换为层次化论证树来评估LLM生成的法律推理轨迹的质量，发现LLM的法律推理能力受问题覆盖率和正确性影响，RAG和RL能带来互补性改进。


<details>
  <summary>Details</summary>
Motivation: 在专家领域（如法律）中评估LLM生成的推理轨迹质量对于确保可信度和可解释性至关重要，但由于此类推理任务固有的复杂性，这仍然具有挑战性。

Method: 引入LEGIT数据集（24K实例），将法庭判决转换为对立双方论证和法庭结论的层次化树结构，作为评估推理轨迹问题覆盖率和正确性的评分标准。通过专家标注验证这些标准的可靠性。

Result: LLM的法律推理能力受到法律问题覆盖率和正确性的严重影响；检索增强生成（RAG）和基于标准的强化学习（RL）对法律推理能力带来互补性益处：RAG提高整体推理能力，而RL提高正确性但会降低覆盖率。

Conclusion: LEGIT数据集为评估专家级法律推理提供了可靠框架，揭示了LLM在法律推理中的局限性，并展示了RAG和RL在改进法律推理质量方面的互补作用。
...
### [210] [Med-CRAFT: Automated Construction of Interpretable and Multi-Hop Video Workloads via Knowledge Graph Traversal](https://arxiv.org/abs/2512.01045)
*Shenxi Liu,Kan Li,Mingyang Zhao,Yuhang Tian,Shoujun Zhou,Bin Li*

Main category: cs.AI

TL;DR: Med-CRAFT是一个神经符号数据工程框架，通过确定性图遍历过程自动生成医学视频推理基准，解决了高质量逻辑标注视频数据集稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 医学领域多模态大语言模型发展面临的主要瓶颈是高质量、逻辑标注的视频数据集稀缺。传统手动标注成本高昂且不可扩展，现有合成方法存在随机幻觉和缺乏逻辑可解释性的问题。

Method: 提出Med-CRAFT神经符号数据工程框架，将基准合成形式化为确定性图遍历过程。从原始视频流中提取结构化视觉基元，实例化为动态时空知识图谱，通过在图中的有效路径上锚定查询生成，为每个合成基准项强制执行严格的思维链溯源。

Result: 创建了M3-Med-Auto大规模医学视频推理基准，具有细粒度时间选择性和多跳逻辑复杂性。评估表明自动管道生成的查询复杂度可与专家策划数据集相媲美，逻辑对齐分析显示图拓扑与最先进MLLM推理步骤高度相关。

Conclusion: 该工作为关键领域可扩展、低成本的鲁棒评估协议构建铺平了道路，验证了系统将可验证逻辑编码到视觉语言基准中的能力。
...
### [211] [SimWorld: An Open-ended Realistic Simulator for Autonomous Agents in Physical and Social Worlds](https://arxiv.org/abs/2512.01078)
...
TL;DR: SimWorld是一个基于虚幻引擎5构建的新型模拟器，专门为开发和评估LLM/VLM智能体在丰富、真实世界般的环境中设计，解决了现有模拟器在物理、社交动态和智能体支持方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前LLM/VLM智能体在数学、编程和计算机使用方面进展迅速，但在复杂物理和社交环境中的应用仍然具有挑战性。现有世界模拟器存在环境有限、物理规则简化、社交规则简单化、缺乏对LLM/VLM智能体原生支持等问题，无法满足开发能够在真实世界中生存和发展的智能体需求。

Method: 基于虚幻引擎5构建SimWorld模拟器，提供三个核心能力：1）真实、开放式的世界模拟，包括精确的物理和社交动态以及语言驱动的程序化环境生成；2）为LLM/VLM智能体提供丰富的接口，支持多模态世界输入和不同抽象层次的开放词汇动作；3）多样且可扩展的物理和社交推理场景，易于用户定制。

Result: 通过部署前沿LLM智能体（GPT-4o、Gemini-2.5-Flash、Claude-3.5、DeepSeek-Prover-V2）在涉及战略合作与竞争的长时程多智能体配送任务上进行演示，揭示了不同模型之间的推理模式和局限性差异。

Conclusion: SimWorld作为一个开源平台，有望成为跨学科推进真实世界智能体智能的基础平台，为开发能够在复杂物理和社交环境中有效运作的AI智能体提供了重要工具。
...
### [212] [Energy-Aware Data-Driven Model Selection in LLM-Orchestrated AI Systems](https://arxiv.org/abs/2512.01099)
*Daria Smirnova,Hamid Nasiri,Marta Adamska,Zhengxin Yu,Peter Garraghan*

Main category: cs.AI

TL;DR: GUIDE框架通过引入定量模型性能特征改进LLM编排器，在提高准确性的同时显著降低能耗和延迟。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的模型编排器依赖定性描述进行决策，无法反映真实模型能力和性能特征，导致模型选择次优、准确性降低和能耗增加。

Method: 提出GUIDE框架，通过纳入定量模型性能特征进行能源感知的模型选择，考虑性能-能耗权衡。

Result: GUIDE在各种任务中将准确性提高0.90%-11.92%，能耗效率提升最高达54%，并将编排器模型选择延迟从4.51秒降低到7.2毫秒。

Conclusion: GUIDE框架通过引入定量性能特征显著改善了LLM编排器的决策质量，在准确性、能耗效率和延迟方面都取得了显著提升。
...
### [213] [Foundation Priors](https://arxiv.org/abs/2512.01107)
*Sanjog Misra*

Main category: cs.AI

TL;DR: 论文提出"基础先验"概念，将大语言模型生成的合成数据视为先验预测分布而非真实观测，强调其同时反映模型学习模式和用户主观先验、期望与偏见。


<details>
  <summary>Details</summary>
Motivation: 随着基础模型（特别是大语言模型）生成高质量响应能力的提升，研究者越来越倾向于使用这些"合成"输出作为实证研究和决策的数据。但直接将合成数据视为真实观测存在风险，需要建立理论框架来规范其在实证研究中的使用。

Method: 提出"基础先验"概念，将生成过程建模为用户预期数据分布、提示工程过程和模型信任度的函数。推导出基础先验作为用户原始先验的指数倾斜广义贝叶斯更新，其中信任参数控制合成数据的权重。

Result: 建立了将合成数据纳入标准统计和计量经济学工作流程的理论框架，展示了其在精炼复杂模型、指导潜在构念、引导实验设计以及增强随机系数和部分线性规范等应用中的使用方法。

Conclusion: 通过将生成输出视为结构化、明确主观的先验而非经验观测，该框架为在实证工作中利用基础模型提供了原则性方法，避免了将合成"事实"与真实数据混为一谈的问题。
...
### [214] [A Benchmark of Causal vs Correlation AI for Predictive Maintenance](https://arxiv.org/abs/2512.01149)
*Krishna Taduri,Shaunak Dhande,Giacinto Paolo,Saggese,Paul Smith*

Main category: cs.AI

TL;DR: 因果AI方法在预测性维护中显著优于传统相关模型，在成本极度不对称（故障漏检成本是误报的50倍）的场景下，因果模型实现年节省116万美元，比最佳相关模型多8万美元，同时保持高召回率并大幅减少误报。


<details>
  <summary>Details</summary>
Motivation: 制造业预测性维护面临极端成本不对称问题（故障漏检成本约为误报的50倍），传统机器学习方法通常优化统计精度指标，无法反映这种操作现实，且难以区分因果关系与虚假相关。

Method: 研究评估了8种预测模型，从基线统计方法到正式因果推断方法，使用包含10,000台CNC机床（故障率3.3%）的数据集。对比了相关模型与因果模型在预测性维护任务中的表现。

Result: 正式因果推断模型（L5）实现年成本节省116万美元（减少70.2%），比最佳相关决策树模型（L3）每年多节省约8万美元。因果模型保持最高召回率（87.9%）的同时，将误报减少97%（从165降至5），精度达92.1%，训练-测试性能差距仅2.6个百分点。

Conclusion: 因果AI方法结合领域知识，在预测性维护应用中比基于相关的方法能产生更优的财务结果和更可解释的预测，特别适合成本极度不对称的工业场景。
...
### [215] [Knowledge Graph Augmented Large Language Models for Next-Visit Disease Prediction](https://arxiv.org/abs/2512.01210)
*Ruiyu Wang,Tuan Vinh,Ran Xu,Yuyin Zhou,Jiaying Lu,Carl Yang,Francisco Pasquel*

Main category: cs.AI

TL;DR: 本文提出了一种基于知识图谱引导的思维链框架，用于电子健康记录中的疾病预测，通过将ICD-9代码映射到PrimeKG知识图谱，提取疾病相关节点和多跳推理路径作为思维链的支架，然后微调轻量级LLaMA和Gemma模型，在有限训练数据下取得了优于传统基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有电子健康记录临床预测模型通常提供粗略的事后解释，对患者层面的决策支持价值有限。需要开发能够生成临床基础扎实、时间一致性强的推理解释的方法，以增强模型在医疗决策中的可信度和实用性。

Method: 提出知识图谱引导的思维链框架：1）将ICD-9代码映射到PrimeKG知识图谱；2）提取疾病相关节点和多跳推理路径作为思维链的支架；3）仅保留结论与观察结果匹配的解释；4）在生成的监督语料上微调轻量级LLaMA-3.1-Instruct-8B和Gemma-7B模型。

Result: 在MIMIC-III数据集上，针对10种PrimeKG映射的疾病，在有限训练样本（400和1000例）下，KG引导模型优于传统基线方法，AUROC达到0.66-0.70，macro-AUPR达到0.40-0.47。在CRADLE队列上的零样本迁移学习将准确率从约0.40-0.51提升到0.72-0.77。盲法临床评估显示医生对KG引导的思维链解释在清晰度、相关性和临床正确性方面有显著偏好。

Conclusion: 知识图谱引导的思维链框架能够生成临床基础扎实的解释，提高疾病预测模型的性能，并在有限训练数据下展现出良好的泛化能力。该方法为开发可解释的临床预测模型提供了有前景的方向，有助于增强医疗AI系统的可信度和临床实用性。
...
### [216] [Unsupervised decoding of encoded reasoning using language model interpretability](https://arxiv.org/abs/2512.01222)
*Ching Fang,Samuel Marks*

Main category: cs.AI

TL;DR: 研究通过构建ROT-13加密推理测试平台，评估机制可解释性方法（特别是logit lens）能否解码大语言模型的隐藏推理过程，发现这些方法对简单编码推理具有较强鲁棒性。
...
Method: 通过微调推理模型（DeepSeek-R1-Distill-Llama-70B）使其在ROT-13加密下进行思维链推理，同时保持可理解的英语输出，构建受控测试平台。评估机制可解释性方法（特别是logit lens分析）仅使用内部激活来解码模型的隐藏推理过程。

Result: logit lens能有效翻译编码推理，准确率在中后期层达到峰值。开发了结合logit lens与自动改写的完全无监督解码流程，能够从内部模型表示中重建完整推理记录，达到相当高的准确率。

Conclusion: 当前机制可解释性技术对简单编码推理的鲁棒性可能比之前理解的要强。研究为评估可解释性方法对抗非人类可读格式推理模型提供了初步框架，有助于维护对日益强大AI系统的监督。
...
### [217] [RoboDriveVLM: A Novel Benchmark and Baseline towards Robust Vision-Language Models for Autonomous Driving](https://arxiv.org/abs/2512.01300)
*Dacheng Liao,Mengshi Qi,Peng Shu,Zhining Zhang,Yuxin Lin,Liang Liu,Huadong Ma*

Main category: cs.AI

TL;DR: 论文提出了RoboDriveBench基准测试来评估基于视觉语言模型的端到端自动驾驶系统的鲁棒性，并开发了RoboDriveVLM框架和测试时自适应方法来提升系统在真实驾驶场景中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型的端到端自动驾驶系统依赖大语言模型根据场景理解直接生成驾驶决策，但在真实驾驶场景中存在多种风险。需要评估这些系统是否真正适用于自动驾驶，并解决其鲁棒性问题。

Method: 1) 提出RoboDriveBench基准测试，包含11个模拟场景，涵盖传感器损坏和环境变化等6类传感器损坏，以及人为干预和数据传输故障等5类提示损坏；2) 开发RoboDriveVLM框架，将激光雷达和雷达等多模态数据映射到统一潜在空间；3) 提出基于跨模态知识蒸馏的测试时自适应方法。

Result: 基准测试包含250个独特驾驶场景和5,689帧图像，每个评估包含64,559个轨迹预测案例。实验表明当前VLM自动驾驶系统存在局限性，而提出的解决方案能提升系统在真实世界部署的可靠性。

Conclusion: 研究揭示了当前基于视觉语言模型的端到端自动驾驶系统的局限性，为真实世界部署提供了更可靠的解决方案，包括鲁棒性基准测试、增强框架和自适应方法。
...
### [218] [CuES: A Curiosity-driven and Environment-grounded Synthesis Framework for Agentic RL](https://arxiv.org/abs/2512.01311)
...
<details>
  <summary>Details</summary>
Motivation: 在复杂工具增强环境中，基于大语言模型的智能体需要强化学习来改进，但其有效性依赖于结构化训练任务。许多现实环境中缺乏预定义任务（任务稀缺问题），这成为扩展智能体强化学习的关键瓶颈。

Method: 提出CuES框架：好奇心驱动和环境基础的任务合成方法。通过内在好奇心驱动探索，将交互模式抽象为可重用任务模式，并通过轻量级自上而下指导和基于记忆的质量控制进行精炼。

Result: 在AppWorld、BFCL和WebShop三个代表性环境中，CuES生成的任务分布在多样性和可执行性方面匹配或超越手动策划的数据集，带来显著的下游策略改进。

Conclusion: 好奇心驱动、环境基础的任务生成为智能体提供了可扩展的基础，使智能体不仅能学习如何行动，还能学习学习什么内容。
...
### [219] [Extending NGU to Multi-Agent RL: A Preliminary Study](https://arxiv.org/abs/2512.01321)
*Juan Hernandez,Diego Fernández,Manuel Cifuentes,Denis Parra,Rodrigo Toro Icarte*

Main category: cs.AI

TL;DR: 将Never Give Up算法扩展到多智能体环境，在PettingZoo的simple_tag环境中评估，相比多智能体DQN基线获得更高回报和更稳定学习动态，研究了共享回放缓冲区、新奇度共享和异质beta参数等设计选择。


<details>
  <summary>Details</summary>
Motivation: NGU算法在稀疏奖励的强化学习任务中已证明有效，但主要应用于单智能体环境。本研究旨在将NGU扩展到多智能体环境，探索其在多智能体协作或竞争任务中的表现，并研究不同设计选择对算法性能的影响。

Method: 将NGU算法扩展到多智能体环境，在PettingZoo的simple_tag环境中进行评估。对比多智能体DQN基线，研究三种设计选择：1）共享回放缓冲区vs个体回放缓冲区；2）使用不同k阈值在智能体间共享情节新奇度；3）使用异质的beta参数值。通过实验分析这些设计对算法性能和稳定性的影响。

Result: NGU在多智能体环境中相比多智能体DQN基线获得中等更高的回报和更稳定的学习动态。共享回放缓冲区表现最佳，表明收益来自NGU内在探索与经验共享的结合。新奇度共享在k=1时表现相当，但k值增大会降低学习效果。异质beta值相比小的共同值没有改进。

Conclusion: NGU可以有效地应用于多智能体环境，前提是共享经验并仔细调整内在探索信号。共享回放缓冲区是实现这一目标的关键设计选择，而新奇度共享和异质beta参数调整需要谨慎处理。
...
### [220] [The Necessity of Imperfection:Reversing Model Collapse via Simulating Cognitive Boundedness](https://arxiv.org/abs/2512.01354)
*Zhongjie Jiang*

Main category: cs.AI

TL;DR: 论文提出认知计算框架PMCSF，通过模拟人类认知过程而非模仿数据表面特性来生成合成数据，解决现有合成数据因过度平滑而移除人类文本长尾特征导致模型崩溃的问题。


<details>
  <summary>Details</summary>
Motivation: 当前合成数据生成范式过度追求统计平滑性，系统性地移除了人类文本中具有认知基础的长尾不规则特征，导致训练数据认知贫乏，加速模型崩溃。需要从模仿数据表面特性转向模拟人类认知过程。

Method: 提出Prompt驱动的认知计算框架(PMCSF)，包含认知状态解码器(CSD)将非结构化文本反转为结构化认知向量，以及认知文本编码器(CTE)通过数学定义的认知扰动算子将这些状态重新物质化为富含人类典型缺陷的文本。

Result: 在认知编解码验证中，CTE文本与人类文本的Jensen-Shannon散度为0.0614（标准LLM输出为0.4431），通过双盲专业媒体评审，认知轮廓对齐的组内相关系数ICC>0.9。在功能增益评估中，A股市场同构压力测试显示，整合CTE生成数据的策略在2015年股灾期间最大回撤减少47.4%，产生8.6%防御性Alpha，超过交易成本33倍。
...
### [221] [A Flexible Multi-Agent LLM-Human Framework for Fast Human Validated Tool Building](https://arxiv.org/abs/2512.01434)
*Daull Xavier,Patrice Bellot,Emmanuel Bruno,Vincent Martin,Elisabeth Murisasco*

Main category: cs.AI

TL;DR: CollabToolBuilder是一个多智能体LLM框架，通过专家在环指导迭代学习创建工具，减少任务适应时间和人工反馈需求


<details>
  <summary>Details</summary>
Motivation: 解决复杂迭代问题（如科学文档生成）中工具创建与人类意图对齐的挑战，减少任务/领域适应时间和人工反馈收集成本

Method: 采用四智能体架构（教练、编码员、批评者、资本化者），通过强化动态提示和系统化人工反馈集成来生成和验证工具

Result: 初步实验展示了系统在生成前沿研究论文或专利摘要方面的能力，证明了其在复杂迭代问题解决中的适用性

Conclusion: CollabToolBuilder是一个系统级集成方法，结合了多智能体上下文学习、专家在环控制和可重用工具资本化，适用于科学文档生成等复杂迭代问题
...
### [222] [A Selective Temporal Hamming distance to find patterns in state transition event timeseries, at scale](https://arxiv.org/abs/2512.01440)
*Sylvain Marié,Pablo Knecht*

Main category: cs.AI

TL;DR: 提出状态转移事件时间序列(STE-ts)和选择性时序汉明距离(STH)，避免对大型数据库进行代价高昂且扭曲的重采样，同时利用转移时间和状态持续时间信息


<details>
  <summary>Details</summary>
Motivation: 离散事件系统在自然观测、社会经济科学和工业系统中普遍存在，但标准分析方法通常未能充分利用其事件/状态的双重性质。现有方法要么将信号建模为转移事件序列强调事件顺序对齐，要么作为分类或有序状态时间序列进行重采样，随着观测周期和事件数量的增长，重采样操作变得代价高昂且会扭曲数据

Method: 定义状态转移事件时间序列(STE-ts)，并提出新的选择性时序汉明距离(STH)，该距离同时利用转移时间和状态持续时间信息，避免对大型数据库进行代价高昂且扭曲的重采样

Result: STH泛化了重采样汉明和Jaccard度量，具有更好的精度和计算时间，并且能够关注多个感兴趣的状态。在模拟和真实世界数据集上验证了这些优势

Conclusion: 提出的STE-ts和STH方法有效解决了离散事件系统分析中传统方法的局限性，能够同时利用事件和状态信息，避免代价高昂的重采样，在精度和计算效率方面都有显著改进
...
### [223] [Automated Risk-of-Bias Assessment of Randomized Controlled Trials: A First Look at a GEPA-trained Programmatic Prompting Framework](https://arxiv.org/abs/2512.01452)
*Lingbo Li,Anuradha Mathrani,Teo Susnjak*

Main category: cs.AI

TL;DR: 本研究开发了基于DSPy和GEPA模块的可编程偏倚风险评估管道，通过结构化代码优化替代手动提示设计，在随机对照试验的偏倚风险评估中实现了自动化和可复现性。


<details>
  <summary>Details</summary>
Motivation: 随机对照试验的偏倚风险评估对于证据合成至关重要，但传统方法资源密集且存在评估者间差异。大型语言模型提供了自动化途径，但现有方法依赖难以复制、泛化或评估的手动设计提示。

Method: 引入可编程偏倚风险评估管道，使用DSPy及其GEPA模块替代临时提示设计，通过帕累托引导搜索优化LLM推理，生成可检查的执行轨迹，实现优化过程的透明复制。在100个已发表荟萃分析的随机对照试验中评估七个偏倚风险领域。

Result: 在方法报告更清晰的领域（如随机序列生成），GEPA生成的提示表现最佳；在分配隐藏和参与者盲法领域结果相似；商业模型整体表现略优。与三个手动设计提示相比，GEPA在随机序列生成和选择性报告领域性能提升30%-40%，在其他领域表现相当。

Conclusion: GEPA能够为偏倚风险评估生成一致且可复现的提示，支持在证据合成中结构化、有原则地使用大型语言模型。
...
### [224] [SynthStrategy: Extracting and Formalizing Latent Strategic Insights from LLMs in Organic Chemistry](https://arxiv.org/abs/2512.01507)
*Daniel Armstrong,Zlatko Jončev,Andres M Bran,Philippe Schwaller*

Main category: cs.AI

TL;DR: 该研究提出了一种利用大型语言模型将合成策略知识转化为可验证代码的方法，用于改进计算机辅助合成规划系统的战略决策能力。


<details>
  <summary>Details</summary>
Motivation: 现代计算机辅助合成规划系统虽然能生成化学上有效的反应步骤，但在整合战略考量（如收敛组装、保护基最小化、最优环形成序列）方面存在不足。需要将合成策略知识系统化地编码到系统中。

Method: 利用大型语言模型将合成知识转化为代码，分析合成路线并将战略原则转化为Python函数，代表各种战略和战术规则（如战略官能团互变和环构建策略）。通过将知识形式化为可验证代码而非简单启发式规则，创建可测试、可解释的合成策略表示。

Result: 发布了完整代码库和USPTO-ST数据集（带有战略标签的合成路线注释）。该框架实现了基于自然语言的路线检索，在基准测试中达到75%的Top-3准确率。通过历史趋势的时间分析和化学直观的路线聚类验证了该库的有效性。

Conclusion: 这项工作弥合了计算机辅助合成规划中战术与战略之间的鸿沟，使路线能够根据战略标准而非仅结构进行规范、搜索和评估，为合成规划系统提供了新的战略决策能力。
...
### [225] [LEC: Linear Expectation Constraints for False-Discovery Control in Selective Prediction and Routing Systems](https://arxiv.org/abs/2512.01556)
*Zhiyuan Wang,Aniri,Tianlong Chen,Yue Zhang,Heng Tao Shen,Xiaoshuang Shi,Kaidi Xu*

Main category: cs.AI

TL;DR: LEC方法通过线性期望约束重新定义选择性预测问题，利用校准样本计算FDR约束下的最优阈值，并扩展到双模型路由机制，在保持统一FDR保证的同时提高样本保留率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常生成不可靠的答案，而传统的启发式不确定性方法无法完全区分正确与错误的预测，导致用户在没有统计保证的情况下接受错误答案。需要一种能够提供统计保证的方法来控制错误发现率。

Method: 提出LEC方法，将选择性预测重新解释为约束决策问题，通过对选择和错误指标施加线性期望约束来建立有限样本充分条件。利用校准样本计算FDR约束下的覆盖最大化阈值。进一步扩展到双模型路由机制：当当前模型的不确定性超过校准阈值时，将任务委托给更强的模型，同时保持统一的FDR保证。

Result: 在封闭式和开放式问答数据集上的评估显示，LEC实现了更严格的FDR控制，并显著提高了样本保留率。双模型路由机制在保持较低风险水平的同时，比单个模型接受了更多正确样本。

Conclusion: LEC方法通过线性期望约束和FDR控制框架，为LLM预测提供了统计保证，有效解决了传统不确定性方法无法区分正确与错误预测的问题。双模型路由机制进一步优化了性能，在保持风险控制的同时提高了预测质量。
...
### [226] [CLIP-RL: Aligning Language and Policy Representations for Task Transfer in Reinforcement Learning](https://arxiv.org/abs/2512.01616)
*Chainesh Gautam,Raghuram Bharadwaj Diddigi*

Main category: cs.AI

TL;DR: 提出一种基于CLIP原理的新方法，通过将语言指令与对应策略对齐到统一表示空间，实现多任务环境中的高效迁移学习。


<details>
  <summary>Details</summary>
Motivation: 随着需要在同一环境中解决多个语言相关任务的需求增加，需要开发能够高效进行任务迁移的智能体。现有方法在语言与策略之间的对齐和迁移效率方面存在不足。

Method: 受计算机视觉中CLIP方法的启发，将"相同概念在不同模态中应有相似表示"的原理扩展到强化学习领域。将任务指令和对应策略视为同一任务在不同模态的表示，建立语言嵌入和策略嵌入的统一表示空间。

Result: 实验结果表明，该方法在跨任务迁移中能够实现更快的迁移速度，验证了统一表示空间对任务迁移的有效性。

Conclusion: 通过将CLIP原理扩展到强化学习领域，建立语言与策略的统一表示空间，为多任务环境中的高效迁移学习提供了新的有效方法。
...
### [227] [Who Judges the Judge? LLM Jury-on-Demand: Building Trustworthy LLM Evaluation Systems](https://arxiv.org/abs/2512.01786)
*Xiaochuan Li,Ke Wang,Girija Gouda,Shubham Choudhary,Yaqun Wang,Linwei Hu,Joel Vaughan,Freddy Lecue*

Main category: cs.AI

TL;DR: LLM Jury-on-Demand：一个动态、基于学习的框架，通过训练可靠性预测器选择最可靠的LLM法官组成陪审团，实现可扩展且上下文感知的评估，在总结和RAG基准测试中显著提高了与人类判断的相关性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLMs）被集成到高风险领域，需要既适合实时部署又可扩展，又能支持关键决策的可靠评估方法。人工评估虽然可靠但缓慢且昂贵，单个LLM法官存在偏见，静态陪审团缺乏适应性。

Method: 提出LLM Jury-on-Demand框架，训练一组可靠性预测器来评估LLM法官何时会与人类专家达成一致，利用标记分布、嵌入和结构化输入特征。为每个数据点动态选择最可靠法官组成最优陪审团，并使用其可靠性作为权重聚合评分。

Result: 在总结和RAG基准测试中，动态陪审团系统比单法官和静态陪审团基线实现了显著更高的人类判断相关性。

Conclusion: 自适应、基于学习的陪审团方法为现代LLMs在高风险领域构建可扩展、更可靠和可信的评估系统展现了前景。
...
### [228] [H-Neurons: On the Existence, Impact, and Origin of Hallucination-Associated Neurons in LLMs](https://arxiv.org/abs/2512.01797)
*Cheng Gao,Huimin Chen,Chaojun Xiao,Zhiyi Chen,Zhiyuan Liu,Maosong Sun*

Main category: cs.AI

TL;DR: 研究发现大语言模型中的幻觉现象与特定神经元（H-Neurons）相关，这些神经元仅占模型总神经元的不到0.1%，能可靠预测幻觉发生，且与过度顺从行为有因果关联。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生幻觉（看似合理但事实错误的输出），这削弱了其可靠性。先前研究主要从训练数据和目标等宏观角度分析幻觉，而神经元层面的机制尚未得到充分探索。

Method: 从三个角度系统研究幻觉相关神经元（H-Neurons）：1）识别：确定能可靠预测幻觉发生的稀疏神经元子集；2）行为影响：通过受控干预验证这些神经元与过度顺从行为的因果关联；3）起源：追溯这些神经元到预训练基础模型，分析其形成过程。

Result: 发现仅占模型总神经元不到0.1%的稀疏神经元子集能可靠预测幻觉发生，且在不同场景下具有强泛化能力。这些神经元与过度顺从行为有因果关联，且源于预训练阶段，在预训练后仍能有效用于幻觉检测。

Conclusion: 该研究将宏观行为模式与微观神经机制联系起来，为开发更可靠的大语言模型提供了新见解，揭示了幻觉现象的神经元基础及其在预训练阶段的形成过程。
...
### [229] [Predicting Human Chess Moves: An AI Assisted Analysis of Chess Games Using Skill-group Specific n-gram Language Models](https://arxiv.org/abs/2512.01880)
*Daren Zhong,Dingcheng Huang,Clayton Greenberg*

Main category: cs.AI

TL;DR: 该研究提出了一种基于n-gram语言模型的国际象棋走子预测框架，将走子预测视为行为分析任务，能够根据玩家技能水平动态选择模型进行预测。


<details>
  <summary>Details</summary>
Motivation: 传统国际象棋引擎主要关注计算最优走法，忽略了不同技能水平玩家之间的走子变异性。本研究旨在通过行为分析方法来预测不同技能水平玩家的实际走子模式。

Method: 将玩家分为7个技能等级（从新手到专家），使用开源平台Lichess的数据训练独立的n-gram语言模型。框架包含模型选择器模块，能够根据早期对局信息动态选择最适合的模型进行走子预测。

Result: 模型选择器在使用早期对局信息（16个半回合）时，技能水平分类准确率达到31.7%。走子预测框架的Selector Assisted Accuracy比基准准确率提高了39.1%，且计算效率高，适合实时分析。

Conclusion: 该框架成功将国际象棋走子预测转化为行为分析任务，通过考虑玩家技能水平的变异性，显著提高了预测准确性，为实时国际象棋分析提供了高效工具。
...
### [230] [Learned-Rule-Augmented Large Language Model Evaluators](https://arxiv.org/abs/2512.01958)
*Jie Meng,Jin Mao*
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM评估器依赖人工设计的评估原则，成本高且与数据和LLM理解不一致，难以泛化到多样化评估场景

Method: 1) 规则蒸馏：使用LLM辅助的蒙特卡洛树搜索从数据中自动提取评分规则；2) 规则应用：提出Chain-of-Rule指导LLM遵循规则，并通过强化学习训练规则增强的LLM评估器

Result: 在多样化任务上的广泛实验证明了该方法在各种评估场景中的有效性和泛化能力

Conclusion: 提出的规则增强评估范式能够解决现有LLM评估器的局限性，实现更通用、可扩展的评估能力
...
### [231] [From Atomic to Composite: Reinforcement Learning Enables Generalization in Complementary Reasoning](https://arxiv.org/abs/2512.01970)
*Sitao Cheng,Xunjian Yin,Ruiwen Zhou,Yuxuan Li,Xinyi Wang,Liangming Pan,William Yang Wang,Victor Zhong*

Main category: cs.AI

TL;DR: RL不是简单的概率放大器，而是推理合成器，但前提是基础模型需先通过SFT掌握独立的原子技能（参数推理和上下文推理）。


<details>
  <summary>Details</summary>
Motivation: 研究RL如何促进推理能力——是激励新技能合成还是仅仅放大现有行为，这是一个激烈争论的话题。通过互补推理任务来探究这个问题。

Method: 使用人类传记的合成数据集，将互补推理能力严格解耦为两个原子技能：参数推理（依赖内部知识）和上下文推理（依赖外部信息）。在I.I.D.、组合和零样本三个难度级别评估泛化能力。

Result: 发现SFT在分布内表现良好，但在分布外泛化（特别是零样本设置）上表现不佳，存在"SFT泛化悖论"：模型仅依赖路径捷径的记忆。RL能作为推理合成器，但前提是基础模型已通过SFT掌握独立的原子技能。

Conclusion: RL不是简单的放大器，在具备足够原子基础的情况下，RL能主动从学习的原语中合成复杂推理策略，无需对复杂策略进行显式监督。解耦原子训练后接RL为复杂推理任务提供了可扩展的泛化路径。
...
### [232] [Chain-of-Ground: Improving GUI Grounding via Iterative Reasoning and Reference Feedback](https://arxiv.org/abs/2512.01979)
*Aiden Yiliu Li,Bizhi Yu,Daoan Lei,Tianhe Ren,Shilong Liu*

Main category: cs.AI

TL;DR: Chain of Ground (CoG)是一个无需训练的多步GUI定位框架，通过迭代视觉推理和细化来提升多模态大语言模型在复杂用户界面中的定位精度。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉GUI定位方面虽然表现出色，但在处理小目标、视觉相似目标以及真实世界布局中的模糊性时仍存在困难。这些限制源于有限的定位能力和对现有推理潜力的利用不足。

Method: 提出Chain of Ground (CoG)框架，这是一个无需训练的多步定位方法。模型不是直接预测，而是通过渐进式反思和调整假设来进行迭代视觉推理和细化，从而实现更准确和可解释的定位。

Result: 在ScreenSpot Pro基准测试中达到68.4%的准确率，提升了4.8个百分点。在真实世界泛化测试中，使用新引入的TPanel UI数据集（包含420个带视觉失真的工业控制面板），CoG比强大的基线模型Qwen3 VL 235B提升了6.9个百分点。

Conclusion: 研究结果表明，通过结构化迭代细化而非额外训练，可以释放多模态大语言模型的定位潜力，该方法在真实世界和数字界面中均表现出有效性。
...
### [233] [LLM CHESS: Benchmarking Reasoning and Instruction-Following in LLMs through Chess](https://arxiv.org/abs/2512.01992)
...
<details>
  <summary>Details</summary>
Motivation: 现有静态基准测试容易过拟合和记忆化，需要一种能够评估LLM在动态、随机环境中推理和指令遵循能力的框架，国际象棋提供了一个理想的测试领域。

Method: 开发LLM CHESS评估框架，让LLM与随机对手进行国际象棋对弈，使用胜率、移动质量、合法性、幻觉动作等行为指标评估50多个开源和闭源模型，对顶级推理模型使用Elo评分系统与不同技能水平的国际象棋引擎对弈。

Result: 许多最先进的模型难以完成游戏或取得稳定胜利，推理模型与非推理模型之间存在明显差距，LLM CHESS的动态随机特性有效减少了过拟合和记忆化，即使顶级推理模型也难以饱和该基准。

Conclusion: LLM CHESS是一个有效的评估框架，能够独特地减少过拟合和记忆化，支持未来LLM推理和指令遵循能力的研究，作者发布了实验框架、公开排行榜和相关游戏数据集。
...
### [234] [Text Annotation via Inductive Coding: Comparing Human Experts to LLMs in Qualitative Data Analysis](https://arxiv.org/abs/2512.00046)
*Angelina Parfenova,Andreas Marfurt,Alexander Denzler,Juergen Pfeffer*

Main category: cs.CL

TL;DR: 该研究探索使用大语言模型进行定性数据分析的自动化，特别是归纳式编码。研究发现人类专家在复杂句子标注上表现更好，而LLMs在简单句子上表现更佳，两者存在相反的倾向。


<details>
  <summary>Details</summary>
Motivation: 传统定性数据分析依赖人工归纳编码，过程耗时且主观。研究旨在探索大语言模型能否自动化这一过程，特别是比较人类专家与LLMs在归纳编码中的表现差异。

Method: 使用六个开源大语言模型与人类专家进行对比实验，评估他们在归纳编码任务中的表现。专家对编码句子的难度进行评分，并将人类和LLMs生成的标签与测试集中的黄金标准进行比较。

Result: 发现一个有趣的二分现象：人类编码者在复杂句子标注上表现更好，但在简单句子上表现较差；而LLMs则相反，在简单句子上表现更好，在复杂句子上表现较差。人类标注虽然有时与黄金标准不同，但往往获得其他人类更积极的评价；而某些LLMs虽然更接近真实标签，却获得专家较低的评价。

Conclusion: 大语言模型在自动化定性数据分析方面具有潜力，但与人类专家存在不同的优势和局限性。人类和LLMs在编码任务中表现出互补的特点，未来可能需要结合两者的优势来改进自动化定性分析系统。
...
### [235] [Emergent Convergence in Multi-Agent LLM Annotation](https://arxiv.org/abs/2512.00047)
*Angelina Parfenova,Alexander Denzler,Juergen Pfeffer*

Main category: cs.CL

TL;DR: 该研究通过模拟7500次多智能体多轮讨论，分析了黑盒LLM在协作任务中的协调行为，发现即使没有明确角色提示，LLM群体也能发展出类似谈判的协调策略。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地被部署在协作环境中，但人们对它们作为黑盒智能体时的协调机制知之甚少。研究者希望了解LLM在协作任务中如何协调，特别是在没有明确角色提示的情况下。

Method: 研究模拟了7500次多智能体、多轮讨论的归纳编码任务，生成了超过125,000条话语记录。引入了过程级指标：代码稳定性、语义自一致性、词汇置信度以及情感和收敛度量。还分析了输出嵌入的几何演变，特别是内在维度的变化。

Result: 研究发现：1）LLM群体在词汇和语义上会收敛；2）发展出不对称的影响模式；3）即使没有明确角色提示，也表现出类似谈判的行为；4）输出嵌入的内在维度随着轮次减少，表明语义压缩的发生。

Conclusion: 这项工作展示了黑盒交互分析如何能够揭示涌现的协调策略，为基于内部探针的可解释性方法提供了可扩展的补充。研究表明LLM在协作中能够发展出复杂的协调机制。
...
### [236] [Tree Matching Networks for Natural Language Inference: Parameter-Efficient Semantic Understanding via Dependency Parse Trees](https://arxiv.org/abs/2512.00204)
*Jason Lunder*

Main category: cs.CL

TL;DR: 本文提出Tree Matching Networks (TMN)，基于依赖解析树结构进行自然语言推理任务，相比BERT序列模型在SNLI任务上取得更好结果且计算效率更高。


<details>
  <summary>Details</summary>
Motivation: 虽然基于Transformer的BERT模型在NLI任务上准确率高，但需要数亿参数且计算成本高。依赖解析树等显式语言结构可能包含预编码的关系信息，无需从头学习，有望提高学习效率。

Method: 将Graph Matching Networks (GMN)适配到依赖解析树上，创建Tree Matching Networks (TMN)。在SNLI蕴含任务和SemEval相似性任务上比较TMN与BERT模型。提出多头注意力聚合方法解决当前聚合方法的可扩展性限制。

Result: TMN在SNLI任务上显著优于基于BERT的模型，内存占用更小，训练时间更短。但在SemEval任务上两个模型都表现不佳。显式结构表示在可比规模下显著优于序列模型，但当前聚合方法限制了可扩展性。

Conclusion: 基于依赖解析树的显式结构表示在自然语言推理任务上比序列模型更高效，但需要改进聚合方法以提高可扩展性。提出的多头注意力聚合是解决这一限制的有前景方向。
...
### [237] [Minimal-Edit Instruction Tuning for Low-Resource Indic GEC](https://arxiv.org/abs/2512.00219)
*Akhil Rajeev P*

Main category: cs.CL

TL;DR: 本文提出了一种无需数据增强的印度语言语法纠错方法，使用指令调优的大语言模型和保守解码策略，在马来语和印地语上取得了优异性能。
...
Method: 使用12B参数的GEMMA 3模型进行4位精度指令调优，采用参数高效微调(PEFT)和Alpaca格式。解码过程采用确定性、约束感知的方法，结合轻量级规范化器鼓励最小化、保持语义的编辑。通过基于确定性错误分类器分类体系、标签分布和优先顺序的固定语言特定提示进行推理。

Result: 在官方未调优的GLEU评估中，系统在马来语上得分92.41（总体第六），在印地语上得分81.44（总体第三）。

Conclusion: 基于分类器信息的提示设计、适配器指令调优和确定性解码为印度语言语法纠错提供了一种可复现且计算高效的替代方案，为未来更强的形态句法约束和以人为中心的保守编辑评估提供了方向。
...
### [238] [OmniFusion: Simultaneous Multilingual Multimodal Translations via Modular Fusion](https://arxiv.org/abs/2512.00234)
*Sai Koneru,Matthias Huck,Jan Niehues*

Main category: cs.CL

TL;DR: OmniFusion是一个端到端的多模态翻译系统，通过融合多模态基础模型和翻译专用大语言模型，支持语音到文本、语音+图像到文本、文本+图像到文本的翻译，相比级联流水线在同时翻译中降低了1秒延迟并提升了翻译质量。


<details>
  <summary>Details</summary>
Motivation: 当前开源文本翻译大语言模型虽然语言覆盖和质量有所提升，但只能用于级联流水线的语音翻译，这会引入额外延迟（对同时翻译尤为关键），且无法利用多模态上下文（如图像）来辅助消歧。而预训练的多模态基础模型虽具备跨模态感知推理能力，但缺乏专门翻译模型的多语言覆盖和翻译性能。

Method: 提出端到端方法，将多模态基础模型与翻译大语言模型融合。引入新颖的融合策略，将预训练多模态基础模型多个隐藏层的状态连接到翻译大语言模型，实现联合端到端训练。具体基于Omni 2.5-7B作为多模态基础模型，SeedX PPO-7B作为翻译大语言模型构建OmniFusion模型。

Result: 实验表明OmniFusion能有效利用音频和视觉输入，在同时翻译中相比级联流水线降低了1秒延迟，同时提升了整体翻译质量。模型支持语音到文本、语音+图像到文本、文本+图像到文本三种翻译模式。

Conclusion: 通过融合多模态基础模型和翻译专用大语言模型，OmniFusion实现了有效的端到端多模态翻译系统，在延迟和翻译质量方面均优于传统级联方法，为多模态翻译提供了新的解决方案。
...
### [239] [Lost without translation -- Can transformer (language models) understand mood states?](https://arxiv.org/abs/2512.00274)
*Prakrithi Shivaprakash,Diptadhi Mukherjee,Lekhansh Shukla,Animesh Mukherjee,Prabhat Chand,Pratima Murthy*

Main category: cs.CL

TL;DR: 研究评估了大型语言模型对印度语言中四种情绪状态表达的理解能力，发现当前模型无法直接从印度语言中有效识别情绪状态，但通过翻译到英语或中文可以显著改善性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在精神病学中显示出潜力，但主要是英语中心的。不同语言有自己的痛苦表达方式（痛苦习语），模型能否理解其他语言中的情绪状态尚不清楚，特别是在印度语言中。

Method: 收集了11种印度语言中四种情绪状态（抑郁、正常情绪、欣快躁狂、烦躁躁狂）的247个独特短语。测试了七种实验条件，比较k-means聚类性能：直接嵌入原生和罗马化脚本（使用多语言和印度特定模型），以及将短语翻译成英语和中文后的嵌入。使用基于调整兰德指数、归一化互信息、同质性和完整性的综合评分来衡量性能。

Result: 直接嵌入印度语言无法有效聚类情绪状态（综合评分=0.002）。所有基于翻译的方法都显示出显著改善。使用Gemini翻译的英语（综合评分=0.60）和人工翻译的英语（综合评分=0.61）嵌入gemini-001模型表现良好。令人惊讶的是，人工翻译的英语进一步翻译成中文并使用中文模型嵌入表现最佳（综合评分=0.67）。专门的印度模型（IndicBERT和Sarvam-M）表现不佳。

Conclusion: 当前模型无法直接从印度语言中有效表示情绪状态，这对其在印度精神病学诊断或治疗应用构成了根本障碍。虽然高质量翻译可以弥补这一差距，但依赖专有模型或复杂翻译流程是不可持续的。模型必须首先能够理解多样化的本地语言，才能在全球心理健康领域有效应用。
...
### [240] [EduEval: A Hierarchical Cognitive Benchmark for Evaluating Large Language Models in Chinese Education](https://arxiv.org/abs/2512.00290)
*Guoqing Ma,Jia Zhu,Hanghui Guo,Weijie Shi,Yue Cui,Jiawei Shen,Zilong Li,Yidan Liang*

Main category: cs.CL

TL;DR: EduEval是一个针对中文K-12教育场景的LLM评估基准，包含认知框架、真实性和规模三大特点，评估发现LLMs在事实性任务表现良好但在课堂对话分类和创造性内容生成方面存在挑战。


<details>
  <summary>Details</summary>
Motivation: LLMs在教育应用中有巨大潜力，但未经审查的部署可能对教育标准构成风险，因此需要建立严格的评估体系来确保LLMs在教育场景中的有效性和安全性。

Method: 提出EduEval基准，包含三个关键贡献：1) 认知框架：结合布鲁姆分类法和韦伯知识深度理论提出EduAbility分类法；2) 真实性：整合真实考试题、课堂对话、学生作文和专家设计的提示；3) 规模：包含24种任务类型，超过11,000个问题，涵盖小学到高中。

Result: 评估14个领先的LLMs发现：模型在事实性任务上表现良好，但在课堂对话分类方面表现不佳，创造性内容生成结果不一致；有趣的是，一些开源模型在复杂教育推理任务上优于专有系统；few-shot提示在不同认知维度上效果各异。

Conclusion: 研究结果为开发专门针对中文教育任务优化的LLMs提供了有针对性的基准指标，表明不同教育目标需要定制化的方法，few-shot提示的有效性因认知维度而异。
...
### [241] [Comparative Analysis of 47 Context-Based Question Answer Models Across 8 Diverse Datasets](https://arxiv.org/abs/2512.00323)
...
<details>
  <summary>Details</summary>
Motivation: 基于上下文的问答模型能够利用上下文信息提供更准确相关的答案，在各种应用中具有重要价值。本研究旨在评估不同CBQA模型在多个数据集上的表现，找出无需额外微调的最佳模型，为实际应用提供参考。

Method: 从Hugging Face选取47个CBQA模型，在8个不同数据集上进行基准测试，评估模型性能而不进行额外微调。使用遗传算法整合多个模型的响应以提高整体准确率。

Result: 最佳模型ahotrod/electra_large_discriminator_squad2_512平均准确率43%，在bioasq10b-factoid(65.92%)、biomedical_cpgQA(96.45%)等数据集上表现优异。模型计算时间取决于上下文长度和模型大小，性能通常随答案长度增加而下降，且受上下文复杂度影响。

Conclusion: 基于SQuAD数据集训练的模型在CBQA任务中表现最佳，无需额外微调即可在不同领域数据集上取得良好效果。模型性能受多种因素影响，遗传算法有助于提升整体准确率，为实际应用提供了有价值的参考。
...
### [242] [Evidence-Guided Schema Normalization for Temporal Tabular Reasoning](https://arxiv.org/abs/2512.00329)
*Ashish Thanga,Vibhu Dixit,Abhilash Shankarampeta,Vivek Gupta*

Main category: cs.CL

TL;DR: 该论文提出了一种基于SQL的方法来处理演化半结构化表格的时间推理问题，发现模式设计质量对问答精度的影响大于模型容量，并建立了三个基于证据的设计原则。


<details>
  <summary>Details</summary>
Motivation: 处理演化半结构化表格的时间推理对当前问答系统构成挑战，需要有效的方法来处理维基百科信息框等动态变化的结构化数据。

Method: 采用基于SQL的三步方法：1) 从维基百科信息框生成3NF模式；2) 生成SQL查询；3) 执行查询。强调模式设计质量的重要性。

Result: 最佳配置（Gemini 2.5 Flash模式 + Gemini-2.0-Flash查询）达到80.39 EM，相比基线（68.89 EM）提升了16.8%。证明了模式设计质量对精度的关键影响。

Conclusion: 模式设计质量比模型容量对问答精度的影响更大。提出了三个基于证据的设计原则：保留上下文的规范化、减少歧义的语义命名、一致的时间锚定。
...
### [243] [IndicParam: Benchmark to evaluate LLMs on low-resource Indic Languages](https://arxiv.org/abs/2512.00333)
*Ayush Maheshwari,Kaushal Sharma,Vivek Patel,Aditya Maheshwari*

Main category: cs.CL

TL;DR: IndicParam是一个包含13,000多道选择题的人工标注基准，涵盖11种低资源和极低资源印度语言，用于评估LLM在印度语言上的表现，发现即使最佳模型准确率也仅45%左右。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在高资源多语言任务上表现出色，但低资源和极低资源的印度语言仍然严重缺乏评估。现有基准对这些语言覆盖不足，需要专门的评估工具来了解LLM在这些语言上的真实能力。

Method: 创建了IndicParam基准，包含13,000多道人工标注的多选题，涵盖11种印度语言（尼泊尔语、古吉拉特语、马拉地语、奥里亚语为低资源；多格拉语、迈蒂利语、拉贾斯坦语、梵语、博多语、桑塔利语、孔卡尼语为极低资源），以及梵语-英语混合集。评估了19个LLM（包括专有和开源模型），并标注问题类型（知识导向vs纯语言），评估不同问题格式的处理能力。

Result: 评估结果显示，即使是表现最好的GPT-5平均准确率也只有45.0%，其次是DeepSeek-3.2（43.1%）和Claude-4.5（42.7%）。模型在知识导向问题和纯语言问题上表现不同，且在处理列表匹配、断言-推理对、序列排序等非传统多选题格式时面临挑战。

Conclusion: IndicParam揭示了跨语言迁移的局限性，为印度语言建立了一个具有挑战性的基准。该基准表明当前LLM在低资源和极低资源印度语言上的能力仍然有限，需要进一步研究来改进这些语言的处理能力。
...
### [244] [CourseTimeQA: A Lecture-Video Benchmark and a Latency-Constrained Cross-Modal Fusion Method for Timestamped QA](https://arxiv.org/abs/2512.00360)
*Vsevolod Kovalev,Parteek Kumar*

Main category: cs.CL

TL;DR: 该研究针对教育讲座视频的时间戳问答任务，在单GPU内存/延迟预算下，提出了CourseTimeQA数据集和轻量级跨模态检索器CrossFusion-RAG，显著提升了检索性能并实现了低延迟。


<details>
  <summary>Details</summary>
Motivation: 研究动机是在单GPU计算资源限制下，解决教育讲座视频的时间戳问答问题。现有方法要么性能不足，要么计算开销过大，需要一种既高效又准确的解决方案。

Method: 提出了CrossFusion-RAG方法，结合冻结编码器、学习的视觉投影、浅层查询无关的跨注意力机制（处理ASR和视频帧）、时间一致性正则化器，以及小型交叉注意力重排序器。在CourseTimeQA数据集（52.3小时，902个查询）上进行评估。

Result: CrossFusion-RAG在CourseTimeQA上比强大的BLIP-2检索器提升了nDCG@10 0.10和MRR 0.08，同时在单A100 GPU上实现了约1.55秒的中位端到端延迟。在ASR噪声、时间定位等方面表现出鲁棒性。

Conclusion: 该研究证明了在严格计算约束下，通过精心设计的跨模态融合架构可以实现高效准确的教育视频时间戳问答，为实际部署提供了可行方案。
...
### [245] [Mitigating the Threshold Priming Effect in Large Language Model-Based Relevance Judgments via Personality Infusing](https://arxiv.org/abs/2512.00390)
*Nuo Chen,Hanpei Fang,Jiqun Liu,Wilson Wei,Tetsuya Sakai,Xiao-Ming Wu*

Main category: cs.CL

TL;DR: 研究发现LLM作为相关性标注工具时存在启动效应（先前判断影响后续判断），通过模拟大五人格特质可以降低这种偏差，其中高开放性和低神经质人格最有效。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM被用作可扩展的相关性标注工具，但研究表明它们容易受到启动效应的影响。虽然心理学理论将人格特质与此类偏差联系起来，但尚不清楚LLM中模拟的人格是否表现出类似效应。

Method: 研究LLM中大五人格配置文件如何影响相关性标注中的启动效应，使用多个LLM在TREC 2021和2022深度学习方法数据集上进行实验。

Result: 结果显示某些人格配置文件（如高开放性和低神经质）能持续降低启动敏感性。此外，最有效缓解启动效应的人格可能因模型和任务类型而异。

Conclusion: 基于这些发现，提出人格提示作为缓解阈值启动效应的方法，将心理学证据与基于LLM的评估实践联系起来。
...
### [246] [A Taxonomy of Errors in English as she is spoke: Toward an AI-Based Method of Error Analysis for EFL Writing Instruction](https://arxiv.org/abs/2512.00392)
*Damian Heywood,Joseph Andrew Carrier,Kyu-Hong Hwang*

Main category: cs.CL

TL;DR: 开发了一个基于大语言模型的英语写作错误分析系统，能够识别、分类和纠正各种语言错误，在测试中表现出潜力但也存在上下文理解限制。


<details>
  <summary>Details</summary>
Motivation: 开发AI辅助的错误分析系统，旨在自动化英语写作错误分析，为EFL（英语作为外语）教学提供比传统评分标准更细致的反馈，提升教学效率。

Method: 使用Claude 3.5 Sonnet和DeepSeek R1等大语言模型，基于Corder、Richards和James的语言学理论建立详细分类法，通过Python编码的API调用实现系统，在孤立错误测试后使用包含真实语言错误的"English as she is spoke"文本进行最终测试。

Result: AI系统成功识别了多种错误类型，但在上下文理解方面存在局限性，遇到未编码的错误时偶尔会生成新的错误类别，显示出处理复杂多层分析的能力。
...
### [247] [SCALE: Selective Resource Allocation for Overcoming Performance Bottlenecks in Mathematical Test-time Scaling](https://arxiv.org/abs/2512.00466)
*Yang Xiao,Chunpu Xu,Ruifeng Yuan,Jiashuo Wang,Wenjie Li,Pengfei Liu*

Main category: cs.CL

TL;DR: SCALE框架通过选择性分配计算资源解决数学推理中测试时计算扩展的瓶颈问题，根据子问题难度分配不同处理模式，显著提升性能同时降低计算成本。
...
Result: SCALE显著优于统一扩展基线，在AIME25上准确率提升13.75个百分点（从57.50%到71.25%），同时计算成本降低33%-53%。

Conclusion: SCALE通过选择性资源分配解决了当前测试时扩展方法的根本限制，实现了更好的资源利用和性能提升，代表了该领域的重要进展。
...
### [248] [CACARA: Cross-Modal Alignment Leveraging a Text-Centric Approach for Cost-Effective Multimodal and Multilingual Learning](https://arxiv.org/abs/2512.00496)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型通常需要跨多种模态进行资源密集型的训练，扩展到新语言也采用类似的资源密集型策略。研究人员希望开发一种更高效的架构，能够在不完全重新训练的情况下集成新模态，并且通过单语言训练就能获得多语言能力。

Method: 提出CACARA多模态多语言架构，采用涌现对齐学习范式。仅通过英语对齐数据对新加入的模态进行微调，而不需要重新训练文本编码器，就能使模型支持100多种语言。这种方法保留了先前学习到的知识，训练成本与单语言模型相当。

Result: 该方法在音频到文本检索任务中实现了R@1指标14.24个百分点的提升，优于最先进的多模态模型。模型能够高效获得涌现的多模态和多语言特性，同时避免了跨每个模态和语言的重新训练带来的巨大计算成本。

Conclusion: 涌现对齐学习范式能够解锁单语言训练中的多语言能力，为多模态模型的高效扩展提供了新途径。CACARA架构展示了在不完全重新训练的情况下集成新模态的可行性，同时通过英语对齐数据就能支持多种语言，显著降低了计算成本。
...
### [249] [G-KV: Decoding-Time KV Cache Eviction with Global Attention](https://arxiv.org/abs/2512.00504)
*Mengqi Liao,Lu Wang,Chaoyun Zhang,Zekai Shen,Xiaowei Mao,Si Qin,Qingwei Lin,Saravan Rajmohan,Dongmei Zhang,Huaiyu Wan*

Main category: cs.CL

TL;DR: G-KV是一种KV缓存驱逐方法，通过全局评分机制结合局部和历史注意力分数来更准确评估token重要性，并采用强化学习和蒸馏等后训练技术优化模型在压缩KV缓存设置下的性能。


<details>
  <summary>Details</summary>
Motivation: 当前推理大语言模型在处理长序列时面临显著的计算和内存挑战，现有KV缓存压缩方法主要关注提示压缩或基于局部注意力分数的token驱逐，忽视了token的长期重要性。

Method: 提出G-KV方法，采用全局评分机制结合局部和历史注意力分数来评估token重要性；引入后训练技术包括强化学习和蒸馏，优化模型在压缩KV缓存设置下的表现。

Result: 该方法在压缩KV缓存的同时保持了模型性能，具体实验结果未在摘要中详细说明，但代码已在GitHub开源。

Conclusion: G-KV通过全局评分机制和后训练优化，有效解决了现有KV缓存压缩方法忽视token长期重要性的问题，提升了推理大语言模型在长序列处理中的效率。
...
### [250] [Catch Me If You Can: How Smaller Reasoning Models Pretend to Reason with Mathematical Fidelity](https://arxiv.org/abs/2512.00552)
*Subramanyam Sahoo,Vinija Jain,Saanidhya Vats,Siddharth Mohapatra,Rui Min,Aman Chadha,Divya Chaudhary*

Main category: cs.CL

TL;DR: 论文提出诊断框架评估语言模型数学推理能力，超越传统准确率指标，发现小模型依赖模式匹配而非真正逻辑计算
...
Method: 提出四轴诊断框架：前向-后向一致性、传递性覆盖、反事实敏感性、扰动鲁棒性，应用于Qwen3-0.6B模型和MenatQA数据集

Result: Qwen3-0.6B表面准确率70%+，但后向一致性仅15%，传递性覆盖32.2%，对扰动敏感，显示模型依赖模式匹配而非真正推理

Conclusion: 传统准确率指标可能掩盖推理失败，需要更全面的诊断框架评估推理保真度，该框架可推广到不同模型规模和架构
...
### [251] [Slovak Conceptual Dictionary](https://arxiv.org/abs/2512.00579)
*Miroslav Blšták*

Main category: cs.CL

TL;DR: 本文介绍了斯洛伐克语的首个概念词典，旨在解决低资源语言在自然语言处理任务中缺乏词典工具的问题。


<details>
  <summary>Details</summary>
Motivation: 斯洛伐克语作为资源有限的语言，缺乏机器可读的大规模语言数据源，导致许多需要自动化文本处理的任务相比其他语言效果较差甚至无法解决。

Method: 开发了斯洛伐克语的首个概念词典，这是一种新型的语言学工具，专门为低资源语言设计。

Result: 创建了斯洛伐克语的首个此类语言学工具，填补了该语言在机器可读词典数据方面的空白。

Conclusion: 该概念词典的开发为改善斯洛伐克语自然语言处理任务提供了重要资源，有助于提升自动化文本处理的效果。
...
### [252] [Wikontic: Constructing Wikidata-Aligned, Ontology-Aware Knowledge Graphs with Large Language Models](https://arxiv.org/abs/2512.00590)
*Alla Chepurova,Aydar Bulatov,Yuri Kuratov,Mikhail Burtsev*

Main category: cs.CL

TL;DR: Wikontic是一个从开放域文本构建知识图谱的多阶段管道，通过提取带限定符的三元组、强制类型和关系约束、实体归一化来生成紧凑、一致、连接良好的知识图谱，在多项基准测试中表现优异且构建效率高。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的系统通常将知识图谱作为文本检索的辅助结构，未能充分利用其内在质量。需要一种能够构建高质量、紧凑、一致的知识图谱的方法，以更好地利用结构化知识增强大语言模型。

Method: 提出Wikontic多阶段管道：1) 从开放域文本提取带限定符的候选三元组；2) 强制执行基于Wikidata的类型和关系约束；3) 实体归一化以减少重复。生成的KG紧凑、符合本体论且连接良好。

Result: 在MuSiQue上，正确答案实体出现在96%的生成三元组中；HotpotQA上仅使用三元组达到76.0 F1，MuSiQue上达到59.8 F1，匹配或超越需要文本上下文的检索增强生成基线；MINE-1基准上达到86%的信息保留率（SOTA）；构建时仅使用不到1000个输出token，效率是AriGraph的3倍，GraphRAG的1/20。

Conclusion: Wikontic管道提高了生成知识图谱的质量，为在大语言模型中利用结构化知识提供了可扩展的解决方案，展示了仅使用结构化知识就能达到与需要文本上下文的系统相当的性能。
...
### [253] [ART: Adaptive Response Tuning Framework -- A Multi-Agent Tournament-Based Approach to LLM Response Optimization](https://arxiv.org/abs/2512.00617)
*Omer Jauhar Khan*

Main category: cs.CL

TL;DR: ART框架通过锦标赛式ELO排名和多智能体推理优化大语言模型输出，相比单模型方法提升8.4%质量指标，ELO评分收敛R²值超过0.96


<details>
  <summary>Details</summary>
Motivation: 单个大语言模型的响应存在不一致性、幻觉问题，且在不同查询领域质量参差不齐，需要系统性的优化方法来提升输出质量

Method: 提出ART框架，采用锦标赛式ELO排名和多智能体推理，通过可配置的锦标赛参数、动态智能体选择和多种共识融合策略，让多个LLM智能体在结构化工作流中竞争、批评和协作

Result: 实验评估显示在响应准确性、连贯性和可靠性方面相比基线单模型方法有显著提升，整体质量指标提高8.4%，ELO评分收敛R²值超过0.96

Conclusion: ART框架为需要高质量、经过验证的LLM响应的应用提供了可扩展、生产就绪的解决方案，能够产生优于单个模型输出的共识响应
...
### [254] [Sycophancy Claims about Language Models: The Missing Human-in-the-Loop](https://arxiv.org/abs/2512.00656)
*Jan Batzner,Volker Stocker,Stefan Schmid,Gjergji Kasneci*

Main category: cs.CL

TL;DR: 本文回顾了大型语言模型中奉承性响应模式的研究方法挑战，识别了五种核心操作化定义，指出当前研究缺乏对人类感知的评估，并提出了未来研究的可行建议。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的奉承性响应模式在文献中日益受到关注，但现有研究方法存在挑战，需要系统梳理和评估。

Method: 通过文献回顾分析，识别了测量LLM奉承性的五种核心操作化定义，分析了当前研究方法的局限性。

Result: 研究发现奉承性本质上是人类中心的概念，但当前研究并未评估人类感知；同时难以区分奉承性响应与AI对齐中的相关概念。

Conclusion: 提出了针对未来研究的可行建议，强调需要更系统的方法来测量和理解LLM中的奉承性响应模式。
...
### [255] [Graphing the Truth: Structured Visualizations for Automated Hallucination Detection in LLMs](https://arxiv.org/abs/2512.00663)
*Tanmay Agrawal*

Main category: cs.CL

TL;DR: 论文提出了一个可视化知识图谱框架，用于检测和缓解企业环境中LLM的幻觉问题，通过将模型生成内容与知识源链接并提供置信度指示，创建人机协作的工作流。


<details>
  <summary>Details</summary>
Motivation: 企业环境中，大型语言模型经常需要结合闭源领域知识来提供上下文感知的响应，但存在上下文窗口限制、预训练数据与提供知识不一致等问题，导致产生难以通过常规人工审查检测的高可信度幻觉。现有缓解策略要么依赖昂贵的大规模黄金标准问答标注，要么依赖二次模型验证，都无法提供确定性保证。

Method: 引入一个框架，将专有知识和模型生成内容组织成交互式可视化知识图谱。通过将模型断言与底层真实来源链接，并指示置信度水平，为用户提供清晰的潜在幻觉区域视图。用户可以通过可视化界面诊断不一致性、识别薄弱推理链并提供纠正反馈。

Result: 创建了一个人机协作的工作流，形成了结构化反馈循环，可以增强模型可靠性并持续改进响应质量。

Conclusion: 提出的可视化知识图谱框架为企业环境中LLM幻觉问题提供了一种有效的检测和缓解方法，通过透明化模型推理过程和知识来源，实现了可解释性更强、可靠性更高的人机协作系统。
...
### [256] [A Comparison of Human and ChatGPT Classification Performance on Complex Social Media Data](https://arxiv.org/abs/2512.00673)
*Breanna E. Green,Ashley L. Shea,Pengfei Zhao,Drew B. Margolin*

Main category: cs.CL

TL;DR: GPT-4在分类包含微妙语言的复杂数据集时表现有限，虽然提示中包含标签定义可能有助于提升性能，但整体上仍难以处理细微的语言差异。


<details>
  <summary>Details</summary>
Motivation: 尽管生成式AI工具如ChatGPT在计算社会科学中应用日益广泛，但对其在复杂任务（如分类和标注包含微妙语言的数据集）中的性能仍缺乏深入了解，需要评估其在处理细微语言差异时的表现。

Method: 研究比较了GPT-4与人类标注者的表现，测试了ChatGPT 3.5、4和4o版本，设计了四种提示风格作为输入，并评估了精确率、召回率和F1分数，同时进行了定量和定性分析。

Result: 定量和定性评估显示，虽然提示中包含标签定义可能有助于提升性能，但GPT-4在分类微妙语言时仍存在困难。定性分析揭示了四个具体发现，表明GPT-4在此类任务中的局限性。

Conclusion: 在涉及微妙语言的分类任务中使用ChatGPT时应保持谨慎态度，尽管技术快速发展，但当前模型在处理语言细微差别方面仍有明显限制。
...
### [257] [FastPOS: Language-Agnostic Scalable POS Tagging Framework Low-Resource Use Case](https://arxiv.org/abs/2512.00745)
*Md Abdullah Al Kafi,Sumit Kumar Banshal*

Main category: cs.CL

TL;DR: 提出一个语言无关的基于Transformer的词性标注框架，适用于低资源语言，以孟加拉语和印地语为例，仅需少量代码修改即可跨语言迁移，准确率达96-97%。


<details>
  <summary>Details</summary>
Motivation: 为低资源语言开发词性标注系统，解决传统方法需要大量语言特定工程和资源的问题，通过模块化设计降低跨语言适应成本。

Method: 基于Transformer架构的语言无关框架，仅需少量框架特定代码即可实现跨语言适应，模块化设计允许快速替换底层模型。

Result: 在孟加拉语和印地语上分别达到96.85%和97%的词级准确率，尽管数据集存在不平衡和语言重叠问题，仍保持较高的F1分数。

Conclusion: 该框架通过模块化设计和最小化代码修改实现了有效的跨语言适应，使研究者能专注于语言预处理和数据集优化，推动低资源语言NLP发展。
...
### [258] [Auxiliary-Hyperparameter-Free Sampling: Entropy Equilibrium for Text Generation](https://arxiv.org/abs/2512.00789)
*Xiaodong Cai,Hai Lin,Shaoxiong Zhan,Weiqi Luo,Hong-Gee Kim,Hongyan Hao,Yu Yang,Hai-Tao Zheng*

Main category: cs.CL

TL;DR: 提出了一种无需超参数调优的熵均衡采样方法（EES），通过平衡归一化熵与概率质量来动态调整候选集，在推理和生成任务上表现稳定且具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有token采样策略需要额外的超参数调优，增加了部署复杂性，因此需要一种无需超参数的方法来简化部署并提升性能。

Method: 基于信息论提出熵均衡采样（EES），通过平衡归一化熵与概率质量来动态调整候选集，无需额外超参数调优。

Result: EES在不同温度设置下表现稳定，在推理和生成任务上均能提供有竞争力的准确性和连贯性，同时保持多样性。

Conclusion: EES通过消除超参数调优需求，大大简化了部署过程，同时提升了性能表现。
...
### [259] [Accelerating Bangla NLP Tasks with Automatic Mixed Precision: Resource-Efficient Training Preserving Model Efficacy](https://arxiv.org/abs/2512.00829)
*Md Mehrab Hossain Opi,Sumaiya Khan,Moshammad Farzana Rahman*

Main category: cs.CL

TL;DR: 该研究探索在孟加拉语NLP任务中使用自动混合精度训练，在保持模型性能的同时显著提升训练效率并降低内存消耗。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语NLP开发面临计算资源有限的问题，训练模型需要大量计算资源和时间，特别是在硬件条件受限的环境中，需要寻找提高计算效率的方法。
...
Result: AMP训练将训练速度提升了44.5%，内存消耗降低了17.6%，同时F1分数保持在全精度基线的99.7%范围内，表明在性能几乎不损失的情况下显著提高了计算效率。

Conclusion: 自动混合精度训练能够显著降低计算门槛，有助于在硬件受限的环境中普及先进的NLP能力，为孟加拉语NLP开发提供了实用的解决方案。
...
### [260] [WaterSearch: A Quality-Aware Search-based Watermarking Framework for Large Language Models](https://arxiv.org/abs/2512.00837)
...
Method: 提出控制种子池的新嵌入方案，实现多样化并行水印文本生成；设计WaterSearch框架，联合优化分布保真度和水印信号特性

Result: 在10个任务上平均性能提升51.01%，短文本生成提升47.78%，低熵输出提升36.47%；在插入、同义词替换、改写等攻击下保持高检测率

Conclusion: WaterSearch通过搜索优化框架有效解决了水印检测能力与文本质量之间的权衡问题，具有强大的抗攻击能力
...
### [261] [Less is More: Resource-Efficient Low-Rank Adaptation](https://arxiv.org/abs/2512.00878)
*Chunlin Tian,Xuyang Wei,Huanrong Liu,Zhijiang Guo,Li Li*

Main category: cs.CL

TL;DR: EffiLoRA是一种资源高效的低秩适应方法，通过跨层共享A矩阵和运行时选择性更新B矩阵，在保持性能的同时显著降低训练成本，适用于语言、多模态和扩散模型。


<details>
  <summary>Details</summary>
Motivation: LoRA虽然被广泛采用，但在复杂数据集上仍存在显著开销和参数干扰问题。现有方法通过解耦LoRA更新矩阵来利用矩阵不对称性，但训练成本仍然很高。需要从层间和层内参数冗余的角度重新审视LoRA，提出更轻量且通用的方法。

Method: EffiLoRA采用两种关键技术：1) 在所有Transformer层之间共享统一的A矩阵，减少参数冗余；2) 引入运行时选择性B矩阵更新机制，根据系统资源预算动态调整，实现资源与性能的权衡。

Result: EffiLoRA在多种模态任务中一致优于标准LoRA，包括常识推理、视觉指令调优和图像生成，展示了更高的效率和鲁棒性。

Conclusion: EffiLoRA通过减少参数冗余和动态资源分配，为语言、多模态和扩散模型提供了一种轻量级且通用的参数高效微调方法，在保持性能的同时显著提升了训练效率。
...
### [262] [Reward Auditor: Inference on Reward Modeling Suitability in Real-World Perturbed Scenarios](https://arxiv.org/abs/2512.00920)
*Jianxiang Zang,Yongda Wei,Ruxue Bai,Shiyu Jiang,Nijia Mo,Binhong Li,Qiang Sun,Hui Liu*

Main category: cs.CL

TL;DR: 论文提出Reward Auditor框架，通过假设检验评估奖励模型在真实世界扰动场景下的适用性，而非仅关注特定场景的偏好感知准确率。


<details>
  <summary>Details</summary>
Motivation: 当前奖励模型评估方法仅关注特定场景下的偏好感知准确率，无法揭示其在真实世界场景中的关键脆弱性。需要评估奖励模型在真实世界扰动下的条件可靠性（适用性）。
...
Conclusion: Reward Auditor框架通过科学审计方法评估奖励模型的适用性，解决了当前评估方法的局限性，有助于开发更安全可靠的LLM对齐系统。
...
### [263] [Mitigating Hallucinations in Zero-Shot Scientific Summarisation: A Pilot Study](https://arxiv.org/abs/2512.00931)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在生成内容时会出现上下文不一致的幻觉问题，特别是在零样本科学文本摘要任务中。本研究旨在探索是否可以通过提示工程方法来缓解这一问题。

Method: 使用8篇酵母生物技术论文摘要，对6个指令调优的大语言模型应用7种提示方法：基准提示、两种复杂度递增的指令（PE-1和PE-2）、两种上下文重复（CR-K1和CR-K2）、两种随机添加（RA-K1和RA-K2）。上下文重复涉及识别并重复摘要中的K个关键句子，随机添加则重复K个随机选择的句子。共生成336个摘要，使用6个评估指标（ROUGE-1、ROUGE-2、ROUGE-L、BERTScore、METEOR、余弦相似度）计算摘要与原文的词汇和语义对齐。

Result: 统计分析3744个数据点显示，上下文重复（CR）和随机添加（RA）方法能显著改善大语言模型生成摘要与原文的词汇对齐。使用偏差校正加速自助法置信区间和Bonferroni-Holm校正的Wilcoxon符号秩检验验证了这一结果。

Conclusion: 提示工程方法，特别是上下文重复和随机添加，能够有效减少大语言模型在零样本科学摘要任务中的幻觉问题，提高生成摘要与原文的对齐度。
...
### [264] [DeformAr: Rethinking NER Evaluation through Component Analysis and Visual Analytics](https://arxiv.org/abs/2512.00938)
*Ahmed Mustafa Younes*

Main category: cs.CL

TL;DR: DeformAr是一个专门针对阿拉伯语命名实体识别系统的调试和评估框架，旨在解释阿拉伯语与英语NER系统之间的性能差距，通过跨组件分析和行为分析提供系统诊断。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在英语NLP任务中表现出色，但在阿拉伯语命名实体识别中效果有限，即使使用更大的预训练模型。这种性能差距源于分词、数据集质量和标注不一致等多重因素，现有研究往往孤立分析这些问题，未能捕捉它们对系统行为和性能的联合影响。

Method: DeformAr框架集成了数据提取库和交互式仪表板，支持两种评估模式：跨组件分析和行为分析。框架将每种语言划分为数据集和模型组件以检查它们的交互。分析分两个阶段：第一阶段通过跨组件分析提供系统和模型子组件的诊断指标；第二阶段结合可解释性技术、token级指标、交互式可视化和表示空间分析进行行为分析。

Result: DeformAr是第一个阿拉伯语特定的、基于组件的可解释性工具，提供了一个组件感知的诊断过程，能够检测模型行为并通过将其与底层表示模式和数据因素联系起来进行解释。

Conclusion: DeformAr为资源不足语言中的模型分析提供了关键资源，通过系统化的诊断框架帮助理解和解决阿拉伯语与英语NER系统之间的性能差距问题。
...
### [265] [Fine-tuning of lightweight large language models for sentiment classification on heterogeneous financial textual data](https://arxiv.org/abs/2512.00946)
*Alvaro Paredes Amorin,Andre Python,Christoph Weisser*

Main category: cs.CL

TL;DR: 研究比较了轻量级开源大语言模型（如Qwen3 8B、Llama3 8B）与金融专用模型FinBERT在多个金融情感分析数据集上的表现，发现这些轻量级模型即使只使用5%的训练数据也能在大多数场景中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型在金融市场分析中需要大量计算资源和专有数据集，成本高昂且对许多研究者和从业者不可及。本研究旨在探索轻量级开源LLMs在有限计算资源下处理金融文本情感分析的能力。

Method: 比较了基准金融NLP模型FinBERT与三个开源轻量级LLMs（DeepSeek-LLM 7B、Llama3 8B Instruct、Qwen3 8B）在五个公开数据集上的表现：FinancialPhraseBank、Financial Question Answering、Gold News Sentiment、Twitter Sentiment和Chinese Finance Sentiment。测试了不同规模训练数据（包括仅5%数据）以及零样本和少样本学习场景。

Result: LLMs，特别是Qwen3 8B和Llama3 8B，在大多数场景中表现最佳，即使只使用5%的可用训练数据。这些结果在零样本和少样本学习场景中同样成立。

Conclusion: 轻量级开源大语言模型是一种经济有效的选择，即使在有限的标注语料训练下，也能在异构文本数据上获得有竞争力的性能，为资源有限的研究者和从业者提供了可行的替代方案。
...
### [266] [Advancing Academic Chatbots: Evaluation of Non Traditional Outputs](https://arxiv.org/abs/2512.00991)
*Nicole Favero,Francesca Salute,Daniel Hardt*

Main category: cs.CL

TL;DR: 该研究比较了两种检索策略（基于知识图谱的Graph RAG和混合关键词-语义搜索的Advanced RAG）在问答任务中的表现，并评估了LLM生成非传统学术输出（幻灯片和播客脚本）的能力。GPT 4o mini配合Advanced RAG表现最佳。


<details>
  <summary>Details</summary>
Motivation: 大多数大语言模型评估集中在事实问答或简短摘要等标准任务上。本研究旨在扩展评估范围：一是比较两种检索策略在问答任务中的效果；二是评估LLM生成非传统学术输出（幻灯片和播客脚本）的能力。

Method: 研究结合了Meta的LLaMA 3 70B开源模型和OpenAI的GPT 4o mini API。问答性能通过人类评分（11个质量维度）和LLM评判进行交叉验证。幻灯片和播客生成测试采用基于文档的检索方法。

Result: GPT 4o mini配合Advanced RAG产生了最准确的回答。Graph RAG改进有限且导致更多幻觉，部分原因是其结构复杂性和手动设置。在幻灯片和播客生成方面，GPT 4o mini再次表现最佳，而LLaMA 3在叙事连贯性方面有潜力。人类评审在检测布局和风格缺陷方面至关重要。

Conclusion: 研究强调了结合人类和LLM评估在评估新兴学术输出中的必要性。Advanced RAG在问答任务中表现优于Graph RAG，而GPT 4o mini在生成非传统学术输出方面整体表现最佳。
...
### [267] [When Safety Blocks Sense: Measuring Semantic Confusion in LLM Refusals](https://arxiv.org/abs/2512.01037)
*Riad Ahmed Anonto,Md Labid Al Nahiyan,Md Tanvir Hassan,Ch. Md. Rakin Haider*

Main category: cs.CL

TL;DR: 论文提出"语义混淆"概念来评估语言模型在相近意图表达上的不一致拒绝行为，开发了ParaGuard数据集和三个模型无关的指标来衡量这种局部不一致性。


<details>
  <summary>Details</summary>
Motivation: 当前安全对齐的语言模型经常拒绝实际上无害的提示，现有评估主要关注全局拒绝率或合规率，忽略了局部不一致性——模型接受某种意图表达但拒绝其相近的改写版本。这种差距限制了模型的诊断和调优。

Method: 1. 提出"语义混淆"概念来描述局部不一致的失败模式；2. 构建ParaGuard数据集，包含10k个受控的改写聚类，保持意图不变而改变表面形式；3. 提出三个模型无关的token级指标：混淆指数、混淆率和混淆深度，通过比较每个拒绝与其最近接受的邻居，利用token嵌入、下一个token概率和困惑度信号。

Result: 在不同模型系列和部署防护的实验表明：1. 全局错误拒绝率掩盖了关键结构；2. 指标揭示了某些设置中的全局不稳定边界，其他设置中的局部不一致区域，以及更严格拒绝不会增加不一致性的情况；3. 混淆感知审计能够区分系统拒绝的频率和拒绝的合理性。

Conclusion: 该框架为开发者提供了实用的信号，可以在保持安全性的同时减少错误拒绝，通过分离"拒绝频率"和"拒绝合理性"来改进模型评估和调优。
...
### [268] [ELR-1000: A Community-Generated Dataset for Endangered Indic Indigenous Languages](https://arxiv.org/abs/2512.01077)
...
TL;DR: 本文介绍了ELR-1000数据集，包含来自印度东部10种濒危语言的1060个传统食谱，评估了LLM在翻译这些文化特定内容时的表现，发现提供针对性上下文能显著提升翻译质量。


<details>
  <summary>Details</summary>
Motivation: 当前语言技术主要关注高资源语言，缺乏对濒危语言和文化特定内容的支持。需要建立包含语言和文化细微差别的数据集，以推动更公平、文化敏感的语言技术发展。

Method: 通过为数字素养较低的贡献者设计的移动界面，从印度东部偏远地区收集1060个传统食谱，涵盖10种濒危语言。使用最先进的大语言模型进行翻译评估，并测试提供语言背景、翻译示例和文化保护指南等针对性上下文的效果。

Result: 尽管大语言模型具有强大能力，但在处理低资源、文化特定的语言时仍存在困难。然而，提供针对性上下文（包括语言背景信息、翻译示例和文化保护指南）能显著提高翻译质量。

Conclusion: 需要为代表性不足的语言和领域建立专门的基准测试，以推动公平和文化敏感的语言技术发展。ELR-1000数据集的发布旨在激励为濒危语言开发语言技术。
...
### [269] [How do we measure privacy in text? A survey of text anonymization metrics](https://arxiv.org/abs/2512.01109)
*Yaxuan Ren,Krithika Ramesh,Yaxing Yao,Anjalie Field*

Main category: cs.CL

TL;DR: 本文通过系统调查澄清和协调文本隐私保护的评估指标，分析了47篇论文中的6种隐私概念，评估了这些指标与法律标准（HIPAA和GDPR）及用户期望的一致性，为文本匿名化的隐私评估提供实用指导。


<details>
  <summary>Details</summary>
Motivation: 文本匿名化对于在敏感数据领域进行NLP研究和模型开发至关重要，但评估匿名化方法是否充分保护隐私仍然是一个开放挑战。当前缺乏对隐私评估指标的系统性理解和协调，需要澄清不同隐私概念及其与法律标准和用户期望的一致性。

Method: 通过对47篇报告隐私指标的论文进行人工审查，识别并比较了6种不同的隐私概念，分析了相关指标如何捕捉隐私风险的不同方面。然后评估这些概念与法律隐私标准（HIPAA和GDPR）以及基于HCI研究的用户中心期望的一致性。

Result: 识别出6种不同的隐私概念及其相关指标，揭示了当前隐私评估实践的多样性。分析发现这些隐私概念与法律标准和用户期望存在不同程度的对齐，指出了当前实践中的差距和不足。

Conclusion: 该分析为导航隐私评估方法提供了实用指导，强调了当前实践中的差距，旨在促进文本匿名化中更稳健、可比较且具有法律意识的隐私评估。最终目标是推动隐私保护评估的标准化和规范化。
...
### [270] [TempPerturb-Eval: On the Joint Effects of Internal Temperature and External Perturbations in RAG Robustness](https://arxiv.org/abs/2512.01183)
*Yongxin Zhou,Philippe Mulhem,Didier Schwab*

Main category: cs.CL

TL;DR: 该研究系统分析了RAG系统中检索扰动与温度参数之间的交互影响，发现高温设置会放大扰动带来的性能下降，并提出诊断框架和实用指南。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统评估通常孤立地检查检索质量和生成参数（如温度），忽略了它们之间的相互作用。本研究旨在系统研究文本扰动（模拟噪声检索）如何与不同温度设置相互作用。

Method: 提出了全面的RAG扰动-温度分析框架，对检索文档应用三种不同类型的扰动，并在不同温度设置下进行实验。在HotpotQA数据集上使用开源和专有LLM进行广泛实验。

Result: 性能下降呈现特定模式：高温设置会持续放大对扰动的脆弱性，而某些扰动类型在温度范围内表现出非线性敏感性。

Conclusion: 研究贡献包括：(1) 评估RAG鲁棒性的诊断基准，(2) 量化扰动-温度交互的分析框架，(3) 噪声检索条件下的模型选择和参数调优实用指南。
...
### [271] [Generalist Large Language Models Outperform Clinical Tools on Medical Benchmarks](https://arxiv.org/abs/2512.01191)
*Krithik Vishwanath,Mrigayu Ghosh,Anton Alyakin,Daniel Alexander Alber,Yindalon Aphinyanaphongs,Eric Karl Oermann*

Main category: cs.CL

TL;DR: 临床专用AI助手在医学知识问答和临床对齐任务上表现不如通用大语言模型，GPT-5等前沿模型在多个维度上优于OpenEvidence和UpToDate Expert AI等临床工具


<details>
  <summary>Details</summary>
Motivation: 临床AI助手在医疗实践中应用日益广泛，常被宣传为比通用大语言模型更安全可靠，但缺乏独立定量评估，存在证据缺口，可能影响诊断、分诊和指南解读等关键医疗决策

Method: 使用包含1000个项目的迷你基准测试，结合MedQA（医学知识）和HealthBench（临床对齐）任务，评估两个广泛部署的临床AI系统（OpenEvidence和UpToDate Expert AI）与三个前沿通用LLM（GPT-5、Gemini 3 Pro和Claude Sonnet 4.5）的表现

Result: 通用模型在各项任务中一致优于临床工具，GPT-5得分最高；临床工具在完整性、沟通质量、情境意识和基于系统的安全推理方面存在缺陷

Conclusion: 市场上宣传用于临床决策支持的AI工具可能落后于前沿大语言模型，强调了在患者工作流程中部署前进行透明独立评估的紧迫性
...
### [272] [Sentiment Analysis and Emotion Classification using Machine Learning Techniques for Nagamese Language - A Low-resource Language](https://arxiv.org/abs/2512.01256)
*Ekha Morang,Surhoni A. Ngullie,Sashienla Longkumer,Teisovi Angami*

Main category: cs.CL

TL;DR: 首次对Nagamese语言进行情感分析和情绪分类研究，构建了1,195个词的极性词典，使用朴素贝叶斯和支持向量机进行监督学习


<details>
  <summary>Details</summary>
Motivation: Nagamese语言是印度东北部那加兰邦和阿萨姆邦之间贸易交流中发展起来的克里奥尔语，虽然英语、印地语等资源丰富语言已有大量情感分析研究，但Nagamese语言尚未有相关工作，这是首次尝试

Method: 构建包含1,195个Nagamese词汇的情感极性词典，结合其他特征，使用朴素贝叶斯和支持向量机等监督机器学习技术进行情感分析

Result: 成功构建了Nagamese语言的情感极性词典，并建立了情感分析模型，能够检测文本中的极性（积极、消极、中性）和基本情绪

Conclusion: 这是对Nagamese语言进行情感分析和情绪分类的首个研究，为低资源语言的情感分析提供了重要基础，填补了该领域的空白
...
### [273] [SUPERChem: A Multimodal Reasoning Benchmark in Chemistry](https://arxiv.org/abs/2512.01274)
...
TL;DR: SUPERChem是一个包含500个专家策划的化学推理问题的基准测试，旨在解决现有基准的局限性，通过推理路径保真度评分评估模型推理质量，结果显示当前最佳模型表现仍低于人类基线。


<details>
  <summary>Details</summary>
Motivation: 当前用于评估大语言模型化学推理能力的基准存在三个主要问题：任务过于简化、缺乏过程级评估、与专家级化学技能不匹配。需要一个新的基准来更准确地评估模型的化学推理能力。

Method: 开发了SUPERChem基准，包含500个专家策划的推理密集型化学问题，涵盖多个子领域，提供多模态和纯文本格式。采用原创内容和迭代策划流程消除有缺陷的项目并减轻数据污染。每个问题都配有专家编写的解决方案路径，支持推理路径保真度评分来评估推理质量。

Result: 人类基线准确率为40.3%，而表现最佳的模型GPT-5（高）仅达到38.5%，其次是Gemini 2.5 Pro（37.9%）和DeepSeek-V3.1-Think（37.3%）。基准能够引发多步骤、多模态推理，揭示视觉信息的模型依赖性影响，并区分高保真推理者和启发式推理者。

Conclusion: SUPERChem提供了一个具有挑战性的基准和可靠的评估框架，旨在促进大语言模型向专家级化学智能的发展。数据集已在Hugging Face平台公开。
...
### [274] [Agreement-Constrained Probabilistic Minimum Bayes Risk Decoding](https://arxiv.org/abs/2512.01316)
...
<details>
  <summary>Details</summary>
Motivation: MBR解码虽然能生成高质量翻译，但需要评估候选集所有成对得分，计算复杂度为二次方。PMBR解码通过采样部分得分并使用矩阵补全算法减少计算量，但会降低翻译质量。需要改进质量与成本之间的权衡。

Method: 提出一致性约束的PMBR（AC-PMBR）解码方法，利用知识蒸馏模型指导得分矩阵的补全过程，提高矩阵补全的准确性。

Result: AC-PMBR将矩阵补全的近似误差降低了最多3倍，在WMT'23英德互译任务中，在可比计算成本下实现了比PMBR更高的翻译质量。

Conclusion: AC-PMBR解码方法通过知识蒸馏模型指导矩阵补全，有效改善了质量与计算成本之间的权衡，为MBR解码的实际应用提供了更好的解决方案。
...
### [275] [MARSAD: A Multi-Functional Tool for Real-Time Social Media Analysis](https://arxiv.org/abs/2512.01369)
*Md. Rafiul Biswas,Firoj Alam,Wajdi Zaghouani*

Main category: cs.CL

TL;DR: MARSAD是一个专注于阿拉伯语世界的多功能NLP平台，用于实时社交媒体监控和分析，支持情感分析、情绪分析、宣传检测、事实核查和仇恨言论检测等功能。


<details>
  <summary>Details</summary>
Motivation: 为研究人员和非技术用户提供一个专门针对阿拉伯语世界的实时社交媒体监控分析平台，解决阿拉伯语社交媒体内容分析的工具缺乏问题。

Method: 平台采用灵活的后端架构，集成文档存储和结构化数据管理，支持大规模多模态数据处理，前端提供用户友好的数据上传和交互界面，通过API密钥实现安全的数据抓取。

Result: 开发了一个能够处理实时和存档社交媒体内容的平台，提供详细的可视化和报告功能，涵盖多个分析维度，包括情感、情绪、宣传、事实核查和仇恨言论检测。

Conclusion: MARSAD是一个功能全面的阿拉伯语社交媒体分析平台，为研究人员和非技术用户提供了强大的实时监控和分析工具，填补了该领域的空白。
...
### [276] [DyFuLM: An Advanced Multimodal Framework for Sentiment Analysis](https://arxiv.org/abs/2512.01410)
...
<details>
  <summary>Details</summary>
Motivation: 理解复杂文本表达中的情感是情感计算的基本挑战，需要能够同时捕捉分层语义表示和细粒度情感细微差别的多模态框架。

Method: 提出DyFuLM模型，包含两个关键模块：分层动态融合模块自适应整合多级特征，门控特征聚合模块调节跨层信息流以实现平衡表示学习。

Result: 在多任务情感数据集上达到82.64%粗粒度和68.48%细粒度准确率，最低回归误差（MAE=0.0674，MSE=0.0082）和最高R²系数（0.6903）。消融实验显示各模块均有显著贡献。

Conclusion: DyFuLM通过有效的分层特征融合增强了情感表示和整体性能，每个模块都对特征交互和任务平衡有重要贡献。
...
### [277] [PromptBridge: Cross-Model Prompt Transfer for Large Language Models](https://arxiv.org/abs/2512.01420)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在实际应用中频繁切换（因能力、成本、部署约束、隐私等因素），但为特定模型优化的提示词在其他模型上效果显著下降，这种现象被称为"模型漂移"，需要解决跨模型提示词重用问题。

Method: 提出PromptBridge框架：1）使用模型自适应反射提示进化（MAP-RPE）通过迭代反射精化和定量评估获得任务和模型特定的最优提示；2）利用源模型和目标模型的校准提示对学习跨模型提示映射；3）测试时，给定源模型提示，直接为目标模型生成优化提示。

Result: 实验表明PromptBridge在单智能体和多智能体设置中能持续提升下游任务准确性，同时减少迁移工作量，且仅需少量对齐任务进行校准。

Conclusion: PromptBridge有效解决了模型漂移问题，实现了无需昂贵任务或模型重新优化的跨模型提示转移，提高了大语言模型应用的灵活性和效率。
...
### [278] [Multilingual Conversational AI for Financial Assistance: Bridging Language Barriers in Indian FinTech](https://arxiv.org/abs/2512.01439)
*Bharatdeep Hazarika,Arya Suneesh,Prasanna Devadiga,Pawan Kumar Rajpoot,Anshuman B Suresh,Ahmed Ifthaquar Hussain*

Main category: cs.CL

TL;DR: 开发支持印度多语言（特别是代码混合语言如Hinglish）的对话AI系统，用于金融援助场景，提升金融包容性


<details>
  <summary>Details</summary>
Motivation: 印度语言多样性带来金融科技平台的机遇与挑战：31种主要语言和100多种次要语言，但只有10%人口懂英语，这成为金融包容性的障碍

Method: 采用多智能体架构，包含语言分类、功能管理和多语言响应生成模块；通过多个语言模型的比较分析和实际部署验证

Result: 系统显著提升了用户参与度，同时保持低延迟开销（4-8%）；通过实际部署证明了有效性

Conclusion: 这项工作有助于弥合新兴市场数字金融服务中的语言鸿沟，为多语言金融科技解决方案提供了实用框架
...
### [279] [MEGConformer: Conformer-Based MEG Decoder for Robust Speech and Phoneme Classification](https://arxiv.org/abs/2512.01443)
*Xabier de Zuazo,Ibon Saratxaga,Eva Navas*

Main category: cs.CL

TL;DR: 本文提出基于Conformer架构的解码器，用于LibriBrain 2025 PNPL竞赛的两个MEG基础任务：语音检测和音素分类。通过轻量级卷积投影层和任务特定头，在原始306通道MEG信号上实现高效处理，在官方排行榜上取得了优于基准线的结果。


<details>
  <summary>Details</summary>
Motivation: 针对LibriBrain 2025 PNPL竞赛中的两个基础MEG任务（语音检测和音素分类），需要开发能够有效处理原始306通道MEG信号的模型，以超越竞赛基准线并取得有竞争力的排名。

Method: 1. 采用紧凑型Conformer架构处理原始MEG信号；2. 使用轻量级卷积投影层进行特征提取；3. 为不同任务设计特定头部：语音检测采用MEG导向的SpecAugment增强，音素分类使用逆平方根类别加权和动态分组加载器处理100样本平均示例；4. 应用简单的实例级归一化缓解分布偏移问题。

Result: 在官方标准赛道划分下，使用F1-macro进行模型选择，最佳系统在排行榜上取得了88.9%（语音检测）和65.8%（音素分类）的成绩，超越了竞赛基准线，在两个任务中都进入了前十名。

Conclusion: 基于Conformer的解码器在MEG信号处理任务中表现出色，通过任务特定的增强策略和归一化技术有效提升了模型性能，为MEG信号分析提供了有效的解决方案。
...
### [280] [Enhancing BERT Fine-Tuning for Sentiment Analysis in Lower-Resourced Languages](https://arxiv.org/abs/2512.01460)
*Jozef Kubík,Marek Šuppa,Martin Takáč*

Main category: cs.CL

TL;DR: 本文提出了一种结合主动学习、聚类和动态数据选择调度器的集成微调管道，用于低资源语言的模型优化，能在减少30%标注数据的同时提升性能达4个F1分数点。


<details>
  <summary>Details</summary>
Motivation: 低资源语言通常数据有限，导致语言模型性能较弱。由于预训练计算成本高，更实际的方法是在微调阶段进行改进。本研究旨在通过主动学习和结构化数据选择策略来提升有限训练数据下的微调效果。

Method: 提出集成微调管道，将主动学习与数据聚类相结合，并引入动态数据选择调度器（Active Learning schedulers）。系统性地整合主动学习、聚类和调度器来增强模型性能。

Result: 在斯洛伐克语、马耳他语、冰岛语和土耳其语的实验中，微调阶段使用聚类结合主动学习调度器能同时实现：1）节省高达30%的标注成本；2）性能提升高达4个F1分数点；3）提供更好的微调稳定性。

Conclusion: 通过主动学习调度器和聚类技术的结合，可以在有限数据条件下显著提升低资源语言模型的微调效果，同时降低标注成本并提高训练稳定性。
...
### [281] [MCAT: Scaling Many-to-Many Speech-to-Text Translation with MLLMs to 70 Languages](https://arxiv.org/abs/2512.01512)
*Yexing Du,Kaiyuan Liu,Youcheng Pan,Bo Yang,Keqi Deng,Xie Chen,Yang Xiang,Ming Liu,Bin Qin,YaoWei Wang*

Main category: cs.CL

TL;DR: MCAT是一个多语言成本效益加速语音到文本翻译框架，通过课程学习和数据平衡策略将MLLM支持的语言扩展到70种，并设计优化的语音适配器模块将语音序列长度减少到仅30个标记，显著提升翻译性能和推理效率。
...
Method: 提出MCAT框架，包含两个创新：1）语言扩展方法，利用课程学习和数据平衡策略将支持的语言扩展到70种；2）优化的语音适配器模块，将语音序列长度减少到仅30个标记。

Result: 在FLEURS数据集上，MCAT在70x69个翻译方向上超越了最先进的端到端模型，同时提升了批量推理效率。仅使用约1亿可训练参数和每种语言10小时的S2TT数据。

Conclusion: MCAT框架有效解决了MLLM在语音到文本翻译中的语言覆盖和效率问题，实现了多语言互译能力的扩展和推理速度的显著提升，已开源以促进MLLM在S2TT能力方面的发展。
...
### [282] [Language Diversity: Evaluating Language Usage and AI Performance on African Languages in Digital Spaces](https://arxiv.org/abs/2512.01557)
...
<details>
  <summary>Details</summary>
Motivation: 非洲语言的数字表征面临挑战，当前语言检测工具在处理这些语言时存在问题。这些语言虽然使用者众多，但在线对话平台上的使用稀疏且受英语影响严重，缺乏真实单语对话数据，导致训练语言模型时数据稀缺。

Method: 从Reddit子论坛和当地新闻来源收集约鲁巴语、基尼亚卢旺达语和阿姆哈拉语数据。分析两种数据源的差异，评估包括专门工具AfroLID和通用大语言模型在内的语言检测模型在两种数据上的表现。

Result: Reddit数据量少且代码转换严重，而当地新闻媒体提供了干净的单语数据，并促进了社交媒体上本地语言的用户参与。语言检测模型在干净的新闻数据上表现近乎完美，但在代码转换的Reddit帖子上表现不佳。

Conclusion: 专业策划的新闻内容比对话平台数据更适合训练非洲语言的上下文丰富AI模型。未来需要能够同时处理干净文本和代码转换文本的模型，以提高非洲语言检测的准确性。
...
### [283] [MAC-SLU: Multi-Intent Automotive Cabin Spoken Language Understanding Benchmark](https://arxiv.org/abs/2512.01603)
...
TL;DR: MAC-SLU是一个多意图车载语音理解数据集，增加了SLU任务的难度，并对主流开源LLM和LALM进行了全面基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有SLU数据集缺乏足够的多样性和复杂性，且缺少针对最新大语言模型和大音频语言模型的统一基准测试。

Method: 引入MAC-SLU多意图车载语音理解数据集，基于该数据集对主流开源LLM和LALM进行基准测试，涵盖上下文学习、监督微调、端到端和流水线范式。

Result: 实验表明，LLM和LALM通过上下文学习有潜力完成SLU任务，但性能仍显著落后于监督微调。端到端LALM表现与流水线方法相当，并能有效避免语音识别错误传播。

Conclusion: MAC-SLU为SLU研究提供了更具挑战性的数据集和基准测试框架，展示了端到端LALM在避免错误传播方面的优势。
...
### [284] [Learning the Boundary of Solvability: Aligning LLMs to Detect Unsolvable Problems](https://arxiv.org/abs/2512.01661)
*Dengyun Peng,Qiguang Chen,Bofei Liu,Jiannan Guan,Libo Qin,Zheng Yan,Jinhao Liu,Jianshu Zhang,Wanxiang Che*

Main category: cs.CL

TL;DR: 该研究提出了UnsolvableQA数据集和UnsolvableRL框架，用于解决LLM在区分客观不可解问题（内在矛盾）和主观能力限制问题时的困难，通过强化学习提升模型在可解问题上的准确性同时实现近乎完美的不可解性检测。


<details>
  <summary>Details</summary>
Motivation: 当前LLM难以区分客观不可解性（问题内在矛盾）和主观能力限制（超出模型能力范围的问题），这导致幻觉和过度自信问题。需要让模型既能解决可行问题，又能检测内在矛盾，并谨慎拒绝超出能力范围的任务。

Method: 1. 构建UnsolvableQA数据集：采用双轨方法生成配对的可解和不可解实例，包括逻辑谜题的程序化生成和数学问题的"反向构造"方法（在有效推理链中注入矛盾）。2. 提出UnsolvableRL强化学习框架：包含三个奖励组件，共同考虑准确性、不可解性和难度。

Result: 实验结果表明，该方法实现了近乎完美的不可解性检测，同时提高了在可解任务上的准确性。关键发现是"能力崩溃"现象，表明明确暴露于不可解数据对于防止模型系统性过度自信是必不可少的。

Conclusion: 通过UnsolvableQA数据集和UnsolvableRL框架，研究成功解决了LLM在区分客观不可解性和主观能力限制方面的困难，显著提升了模型的可靠性和自知能力，为防止过度自信提供了有效方法。
...
### [285] [MMAG: Mixed Memory-Augmented Generation for Large Language Models Applications](https://arxiv.org/abs/2512.01710)
*Stefano Zeppieri*

Main category: cs.CL

TL;DR: 本文提出混合记忆增强生成（MMAG）模式，为基于LLM的智能体构建五层交互记忆系统，解决长对话中相关性、个性化和连续性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在单次提示中能生成连贯文本，但在长时间交互中难以维持相关性、个性化和连续性。人类沟通依赖多种记忆形式，包括回忆过往对话、适应个人特质和情境上下文。

Method: 引入混合记忆增强生成（MMAG）模式，将LLM智能体的记忆组织为五个交互层：对话记忆、长期用户记忆、情景与事件关联记忆、感知与情境感知记忆、短期工作记忆。借鉴认知心理学，将这些层次映射到技术组件，并制定协调、优先级和冲突解决策略。

Result: 在Heero对话智能体中实现该方法，加密的长期个人资料和对话历史已改善用户参与度和留存率。讨论了存储、检索、隐私和延迟等实施问题。
...
### [286] [Self-Supervised Borrowing Detection on Multilingual Wordlists](https://arxiv.org/abs/2512.01713)
*Tim Wientzek*

Main category: cs.CL

TL;DR: 提出了一种完全自监督的多语言词汇表借词检测方法，结合PMI相似度和对比学习组件，无需标注数据即可自动选择决策阈值，性能优于现有字符串相似度方法，与有监督基线相当或更好。


<details>
  <summary>Details</summary>
Motivation: 现有借词检测方法通常依赖有监督学习或简单的字符串相似度度量，需要标注数据且性能有限。本文旨在开发一种完全自监督的方法，无需人工标注即可在多语言词汇表中有效检测借词。

Method: 方法结合两种信息源：基于全局对应模型的PMI相似度和基于语音特征向量的轻量级对比学习组件。包含自动选择决策阈值的程序，无需标注数据。通过字符编码、温度设置和数据增强策略进行优化。

Result: 在基准数据集上的实验表明：单独使用PMI相似度已优于NED和SCA等现有字符串相似度方法；组合相似度方法性能与有监督基线相当或更好。方法可扩展到不同规模的数据集，无需人工监督。

Conclusion: 提出了一种完全自监督的借词检测方法，结合PMI相似度和对比学习，性能优越且无需标注数据。提供了命令行工具供研究人员使用，为多语言词汇分析提供了实用解决方案。
...
### [287] [Beware of Reasoning Overconfidence: Pitfalls in the Reasoning Process for Multi-solution Tasks](https://arxiv.org/abs/2512.01725)
*Jiannan Guan,Qiguang Chen,Libo Qin,Dengyun Peng,Jinhao Liu,Liangyu Huo,Jian Xie,Wanxiang Che*

Main category: cs.CL

TL;DR: LLMs在需要单一正确答案的推理任务上表现出色，但在需要生成全面多样答案的多解任务上表现不佳，这归因于"推理过度自信"问题。研究者通过MuSoBench基准测试发现，传统短链思维提示存在明显过度自信，而新兴的长链思维方法通过迭代探索和自我反思缓解了这一问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在需要单一正确答案的推理任务中表现出色，但在需要生成全面多样答案的多解任务中表现不佳。研究者认为这种局限性源于"推理过度自信"现象，即模型倾向于对不完整的解决方案集表达过度的确定性。为了研究这一问题，需要开发专门的评估基准和方法。

Method: 1. 引入MuSoBench基准测试，专门用于评估多解问题；2. 比较传统短链思维提示(Short-CoT)和新兴长链思维(Long-CoT)方法的表现；3. 提出"认知刚性假说"来解释过度自信现象；4. 使用注意力熵分析来验证这一假说。

Result: 实验表明：1. 传统短链思维提示范式表现出明显的过度自信；2. 长链思维方法通过迭代探索和自我反思有效缓解了过度自信问题；3. 注意力熵分析为认知刚性假说提供了初步支持；4. 研究者识别了可观察的行为模式和影响因素。

Conclusion: 该研究为评估LLM推理的完整性提供了工具，并强调需要超越单一答案准确性的评估，转向更全面的探索。研究揭示了LLM在多解任务中的局限性，并提出了缓解过度自信的方法，这对开发更全面、多样化的AI推理系统具有重要意义。
...
### [288] [Reasoning About the Unsaid: Misinformation Detection with Omission-Aware Graph Inference](https://arxiv.org/abs/2512.01728)
*Zhengjia Wang,Danding Wang,Qiang Sheng,Jiaying Wu,Juan Cao*

Main category: cs.CL

TL;DR: OmiGraph是首个针对基于省略的虚假信息的检测框架，通过构建省略感知图、建模省略关系、消息传递等机制，在两大基准上平均提升5.4% F1和5.3% ACC。


<details>
  <summary>Details</summary>
Motivation: 虚假信息不仅通过显式捏造内容欺骗读者，还通过隐式省略关键信息误导判断。虽然前者已被广泛研究，但基于省略的欺骗方式仍被忽视，尽管它能以"信息完整"的假象微妙地引导读者得出错误结论。

Method: 提出OmiGraph框架：1) 利用捕捉同一事件互补视角的上下文环境构建省略感知图，揭示潜在省略内容；2) 提出省略导向的关系建模，识别内部上下文依赖和动态省略意图；3) 引入省略感知的消息传递和聚合机制，整合省略内容和关系以提取欺骗模式。

Result: 在两个大规模基准测试中，通过考虑省略视角，OmiGraph取得了显著性能提升，平均F1提高5.4%，准确率提高5.3%。

Conclusion: OmiGraph是首个针对基于省略的虚假信息的检测框架，通过系统建模省略内容和关系，有效识别了传统方法忽视的欺骗形式，为虚假信息检测开辟了新方向。
...
### [289] [Beyond SFT: Reinforcement Learning for Safer Large Reasoning Models with Better Reasoning Ability](https://arxiv.org/abs/2512.01848)
*Jinghan Jia,Nathalie Baracaldo,Sijia Liu*

Main category: cs.CL

TL;DR: 大型推理模型的安全对齐：研究发现监督微调存在局限性，强化学习能更有效地抑制不安全推理轨迹，同时保持推理能力


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过显式思维链推理显著提升了数学和逻辑问题解决能力，但这也引入了新的安全风险——不安全行为可能出现在中间推理轨迹中，即使最终答案看似无害。现有的基于监督微调的安全对齐方法效果不一致，会降低推理能力，且在不同模型家族间泛化性差。

Method: 研究探索强化学习作为LRM安全训练的补充优化框架。与监督微调不同，RL直接通过奖励反馈优化模型策略，实现更自适应和稳定的对齐。实验在多个模型家族和基准测试上进行，分析反思动态和token级熵。

Result: RL实现了更强、更一致的安全提升，同时保持推理能力。分析显示RL抑制了不安全的探索性推理，同时保持了反思深度，从而产生更安全可靠的推理过程。

Conclusion: 纯监督方法不足以实现LRM的鲁棒安全对齐，强化学习作为补充框架能更有效地平衡安全性和推理能力，为大型推理模型的安全训练提供了新方向。
...
### [290] [BHRAM-IL: A Benchmark for Hallucination Recognition and Assessment in Multiple Indian Languages](https://arxiv.org/abs/2512.01852)
*Hrishikesh Terdalkar,Kirtan Bhojani,Aryan Dongare,Omm Aditya Behera*

Main category: cs.CL

TL;DR: BHRAM-IL是一个针对多种印度语言（印地语、古吉拉特语、马拉地语、奥里亚语和英语）的幻觉识别与评估基准，包含36,047个精心设计的问题，涵盖9个类别，用于评估多语言大语言模型的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 虽然幻觉检测在英语中已有广泛研究，但资源不足的印度语言领域仍未被充分探索。大语言模型在多语言应用中经常产生看似合理但错误或误导性的输出（幻觉），因此需要专门针对印度语言的评估基准来填补这一研究空白。

Method: 创建了BHRAM-IL基准，包含36,047个精心设计的问题，涵盖9个类别（事实性、数值、推理和语言任务）。评估了14个最先进的多语言大语言模型，使用10,265个问题的子集进行分析，采用类别特定指标（归一化到0-1范围）评估跨语言和事实性幻觉。

Result: 所有类别和模型的聚合得分为0.23，语言校正模糊得分为0.385，表明BHRAM-IL在幻觉评估方面的有效性。研究分析了跨语言、模型规模、类别和领域的幻觉模式，为多语言幻觉检测提供了重要基准。

Conclusion: BHRAM-IL基准填补了印度语言幻觉检测的研究空白，为评估多语言大语言模型的幻觉问题提供了系统框架。数据集和代码已开源，支持未来多语言幻觉检测和缓解的研究工作。
...
### [291] [Cross-Lingual Interleaving for Speech Language Models](https://arxiv.org/abs/2512.01865)
*Adel Moumen,Guangzhi Sun,Philip C. Woodland*

Main category: cs.CL

TL;DR: 提出跨语言交织方法，通过混合语音标记构建多语言口语模型，无需文本监督，并发布EN-FR训练数据和评估基准


<details>
  <summary>Details</summary>
Motivation: 口语模型主要针对英语，缺乏多语言训练数据和评估基准，限制了非英语语言的自然语言处理技术发展

Method: 提出跨语言交织方法，在无文本监督下混合不同语言的语音标记；构建EN-FR训练数据集TinyStories和两个评估基准

Result: 在360M和1B参数模型上，交织方法提升单语言语义准确性，实现稳健的跨语言延续，增强跨语言隐藏状态对齐

Conclusion: 跨语言交织是构建理解并跨语言交流的多语言口语模型的简单可扩展途径
...
### [292] [Exploring Human Perceptions of AI Responses: Insights from a Mixed-Methods Study on Risk Mitigation in Generative Models](https://arxiv.org/abs/2512.01892)
...
TL;DR: 研究评估人类对AI生成有害内容缓解策略的感知，通过混合方法实验发现语言背景、AI经验和标注熟悉度显著影响评估结果，参与者对语言和上下文属性高度敏感。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速普及，研究人类对AI生成响应的感知变得至关重要。主要挑战在于AI容易产生幻觉和生成有害内容。尽管已有大量实施防护措施的努力，但人类对这些缓解策略的感知在很大程度上仍是未知的。

Method: 采用混合方法实验，评估缓解策略在多个维度上的响应：忠实性、公平性、有害内容移除能力和相关性。在受试者内研究设计中，57名参与者在两种条件下评估响应：有害响应加其缓解措施，以及仅缓解后的响应。

Result: 结果显示参与者的母语、AI工作经验和标注熟悉度显著影响评估。参与者对语言和上下文属性表现出高度敏感性，惩罚轻微语法错误同时奖励保留的语义上下文。这与语言在LLM定量评估中的处理方式形成对比。

Conclusion: 研究为训练和评估缓解策略引入了新指标，并为人类-AI评估研究提供了见解。强调了考虑人类感知因素在AI安全评估中的重要性。
...
### [293] [The Art of Scaling Test-Time Compute for Large Language Models](https://arxiv.org/abs/2512.02008)
*Aradhye Agarwal,Ayan Sengupta,Tanmoy Chakraborty*

Main category: cs.CL

TL;DR: 测试时缩放（TTS）是提升大语言模型推理能力的有前景方向，但缺乏系统比较。本研究首次大规模评估了8个开源LLM在4个推理数据集上的TTS策略，发现没有单一策略普遍最优，推理模型在不同问题难度和轨迹长度上呈现不同模式，且最优TTS性能随计算预算单调提升。


<details>
  <summary>Details</summary>
Motivation: 测试时缩放（TTS）通过动态分配推理计算资源来提升大语言模型的推理能力，但目前缺乏在不同模型类型和问题难度下的系统比较，不清楚各种TTS策略在相同条件下的表现差异。

Method: 进行了首次大规模TTS研究，使用8个开源大语言模型（参数从7B到235B），在4个推理数据集上生成了超过300亿个token，系统比较了不同TTS策略在相同条件下的表现。

Result: 发现了三个一致趋势：1）没有单一TTS策略在所有情况下都最优；2）推理模型在不同问题难度和轨迹长度上呈现不同模式，形成短视界和长视界两类；3）对于给定模型类型，最优TTS性能随计算预算单调提升。

Conclusion: 基于研究结果，提出了一个实用的TTS策略选择指南，综合考虑问题难度、模型类型和计算预算，为有效的推理时缩放提供了实用指导。
...
### [294] [Four Over Six: More Accurate NVFP4 Quantization with Adaptive Block Scaling](https://arxiv.org/abs/2512.02010)
*Jack Cook,Junxian Guo,Guangxuan Xiao,Yujun Lin,Song Han*

Main category: cs.CL

TL;DR: 本文提出4/6方法改进NVFP4量化算法，通过评估每个数值块的两个潜在缩放因子来减少量化误差，特别是在接近最大值区域，从而防止训练发散并提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模增大，低精度数值格式如NVFP4因速度和内存优势而流行。然而，使用NVFP4加速计算需要将所有矩阵乘法操作数（前向传播的权重和激活，反向传播的权重、激活和梯度）量化为NVFP4，这常导致训练发散和推理性能下降。

Method: 提出Four Over Six (4/6)方法，这是对NVFP4量化算法的改进。与整数格式不同，浮点格式如FP4在每个块中接近最大值的数值上具有最大量化误差。4/6通过为每个数值块评估两个潜在缩放因子，有时通过缩放到较小的FP4值使可表示值的分布更均匀，从而更好地表示接近最大值的数值。该方法可在NVIDIA Blackwell GPU上高效实现。

Result: 在transformer和混合模型架构的预训练实验中，4/6在多个情况下防止了训练发散，使训练损失显著接近BF16基准，优于当前最先进的NVFP4训练方法。4/6还可轻松集成到多种后训练量化方法中，普遍提升下游任务准确率。

Conclusion: 4/6方法有效解决了NVFP4量化中的训练发散和性能下降问题，为未来使用NVFP4训练和部署模型提供了有前景的解决方案。
