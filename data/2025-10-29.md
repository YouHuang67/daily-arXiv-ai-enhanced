...
Method: 使用轻量级卷积分类器（Faster-Than-Lies）与视觉语言模型（Qwen2-VL-7B）相结合，通过自编码器重建误差图生成伪影定位热图，并将70种视觉伪影类型分为8个语义组。

Result: 在扩展的CiFAKE数据集上达到96.5%的准确率，在8核CPU上推理时间为175ms，能够生成可解释的文本描述检测到的异常。

Conclusion: 这项工作证明了结合视觉和语言推理在低分辨率图像中进行可解释真实性检测的可行性，并概述了在取证、工业检测和社交媒体审核等领域的潜在跨领域应用。
...
<details>
  <summary>Details</summary>
Motivation: 人类能够通过感知视觉重复性和结构关系来轻松计数，而现有计数模型在复杂形状、内部对称或重叠组件的情况下经常出错。

Method: 基于CounTR架构，用自监督基础模型DINOv2替换视觉编码器以生成更丰富和空间一致的特征表示，并加入位置嵌入融合来保持几何关系，通过轻量卷积解码器将特征解码为密度图。
...
<details>
  <summary>Details</summary>
Motivation: 河流中漂浮的人为垃圾已成为紧迫的环境问题，对生物多样性、水质以及航运和娱乐等人类活动产生不利影响，需要有效的监测方法。

Method: 使用固定原位摄像头采集数据，采用深度学习模型进行漂浮垃圾检测和量化，测试了多种环境条件和学习配置，包括数据泄漏偏差实验，并实现了基于投影几何和回归校正的物体尺寸估计模型。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像超分辨率在分布外条件下（如不同传感器捕获的稀有地貌特征）经常失败，产生视觉上合理但物理上不准确的结果。需要开发能够在严重域偏移下保持物理准确性的鲁棒方法。

Method: RareFlow采用双条件架构：门控ControlNet保持低分辨率输入的几何保真度，文本提示为复杂特征合成提供语义指导。引入多面损失函数确保光谱和辐射一致性，并使用随机前向传播方法量化预测不确定性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D场景生成方法通常局限于单对象生成、需要领域特定训练或缺乏完整的360度可视性支持。本文旨在克服这些限制，实现大规模、连贯的场景合成。

Method: 将场景生成重新表述为多瓦片去噪问题，独立生成重叠的3D区域并通过加权平均进行无缝融合。该方法无需场景级数据集或重新训练，仅依赖最少启发式方法。

Result: 该方法支持多样化的场景布局、高效生成和灵活编辑，能够合成大规模、连贯的场景，同时保持局部语义控制。

Conclusion: 该方法为通用语言驱动的3D场景构建建立了一个简单而强大的基础，继承了对象级先验的泛化能力。
...
Method: 使用YouCookII数据集的场景标注，DynaStride执行自适应帧采样和多模态窗口化来捕捉每个场景中的关键转换，然后采用多模态思维链过程生成多个动作-对象对，通过动态步长窗口选择算法进行精炼和融合。

Result: 与VLLaMA3和GPT-4o等强基线相比，在N-gram指标（BLEU、METEOR）和语义相似度度量（BERTScore、CLIPScore）上均显示出持续增益。

Conclusion: DynaStride生成的字幕在时间上更加连贯和信息丰富，为改进AI驱动的教学内容生成提供了有前景的方向。
...
Method: 采用前馈图像到虚拟形象生成管道获取初始3D表示和噪声渲染，然后使用单步扩散模型在输入图像条件下进行多视角一致的精炼，并通过预训练合成多视角数据和微调真实图像的有效训练策略。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在低纹理室内场景中，由于光度损失的模糊性，难以恢复高质量的3D表面几何。

Method: 设计语言提示平面先验(LP3)流程，使用预训练视觉语言分割模型，并通过交叉视图融合和几何先验检查优化区域提议。增加平面先验监督和几何先验监督项来优化3D高斯。

Result: 在标准室内基准测试中，PlanarGS重建出准确详细的3D表面，大幅优于现有最先进方法。

Conclusion: PlanarGS通过引入平面和几何先验，有效解决了3DGS在室内场景重建中的几何模糊问题，显著提升了重建质量。
...
Method: 借鉴计算机图形学中的通用场景描述符标准，引入Neural USD框架，采用结构化分层表示场景和对象，并应用微调方法确保控制信号解耦。

Result: 评估了框架的多种设计考虑，证明Neural USD能够支持迭代和增量式工作流程。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像防护模型受限于预定义类别，缺乏语义推理能力，难以适应新兴威胁且需要昂贵的重新训练。
...
Conclusion: SafeVision建立了一个全面、策略遵循且可解释的图像防护系统，能够动态适应新兴威胁。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在医学图像分析中缺乏透明推理过程，无法提供临床医生依赖的逐步推理，限制了在医疗环境中的可信度和实用性。

Method: 结合高保真视觉编码和两阶段训练：首先进行推理风格的监督微调，然后使用基于X光异常可验证奖励的强化学习，生成模仿放射科医生系统思维过程的推理。

Result: 在分布外评估中实现竞争性多标签分类同时提高可解释性；在专家放射科医生阅读研究中，完整推理轨迹提高了信心，支持错误审计，并减少了报告最终化时间。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常孤立评估AI助手能力，缺乏真实流式场景或仅支持短期任务，无法满足现实世界中AI助手需要处理多模态输入、实时响应并保留长期记忆的需求。

Method: 创建包含同步自我中心视频、音频和文本的长时数据集，数据在统一全局时间线上对齐，包含高质量视觉叙述和语音转录，通过人工精炼。定义12个子任务和3个核心能力（记忆、理解、跨记忆推理），在流式设置下严格评估。

Result: 数据集包含超过14小时/参与者的多模态数据，涵盖工作学习、生活方式、社交活动、外出文化4个领域，包含3291个人工验证的QA项目，支持多种问题格式。

Conclusion: TeleEgo提供了一个现实且全面的评估框架，通过实时准确性和记忆持久时间两个关键指标共同评估正确性、时间响应性和长期记忆保留能力，推动了实用AI助手的发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型在糖尿病视网膜病变检测中面临分布变化（如设备差异、人口差异、成像条件）导致的鲁棒性挑战，需要提高模型在未见数据上的泛化能力。
...
Conclusion: AdvBlur方法通过对抗性模糊和双损失函数有效解决了糖尿病视网膜病变分类中的领域泛化问题，提高了模型在现实世界应用中的鲁棒性。
...
Method: 通过SEG.A挑战赛引入大型多机构公开数据集，使用隐藏测试集评估自动分割算法，后续可选任务包括计算模拟的表面网格生成。

Result: 深度学习特别是3D U-Net架构主导了最佳提交方案，集成模型显著优于单个模型。性能与算法设计（特别是定制后处理步骤）和训练数据特征密切相关。

Conclusion: 该研究不仅建立了新的性能基准，还为推动未来创新提供了持久资源，有助于开发稳健且可临床转化的工具。
...
Method: 提供标准化的数据集，使用在自然图像、地球卫星数据和最先进的视觉语言模型上预训练的模型进行基线评估。
...
Method: 提出交替优化-微调流程，在对抗性后缀优化和利用优化后缀微调LLM之间交替进行；集成双重规避策略，通过辅助LLM困惑度评分和禁止词惩罚来绕过基于困惑度的过滤器和黑名单词过滤器。

Result: 广泛的实验表明，该方法在红队测试中表现优异，生成的人类可读、抗过滤的对抗性提示具有出色的零样本迁移能力，能够即时适应未见过的提示，并在商业API中暴露关键漏洞。

Conclusion: APT框架有效解决了现有方法的局限性，提供了一种高效、可迁移的黑盒红队测试方法，能够评估文本到图像模型的安全漏洞。
...
<details>
  <summary>Details</summary>
Motivation: 现有稀疏调优方法存在两个问题：1) 两阶段方法忽略了微调过程中的参数调整，限制性能；2) 稀疏掩码梯度更新导致高内存使用。

Method: 提出SNELLA方法：1) 通过两个低秩可学习矩阵的稀疏矩阵选择性更新权重；2) 引入非线性核函数扩展低秩分解，防止权重更新相互依赖；3) 自适应双层稀疏分配机制，基于重要性分数端到端竞争权重。

Result: 在分类、分割和生成任务上，SNELLA取得SOTA性能：FGVC基准上Top-1准确率比SPT-LoRA高1.8%(91.9% vs 90.1%)；内存使用减少31.1%-39.9%(模型参数规模86M-632M)。

Conclusion: SNELLA通过单阶段设计和自适应稀疏分配，在保持低内存使用的同时显著提升下游任务性能，是参数高效微调的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的对抗防御方法主要关注对抗微调或提示优化，忽视了CLIP编码特征中存在的文本和图像特征不匹配问题，这种不对齐在对抗扰动下会被显著放大，导致分类性能严重下降。

Method: COLA采用基于最优传输的框架：(1)将对抗图像嵌入投影到类别文本特征张成的子空间，过滤非语义失真；(2)将图像和文本建模为多个增强视图上的离散分布，通过OT优化对齐，子空间投影无缝集成到成本计算中。

Result: 在14个零样本分类基准上的广泛评估显示，COLA在PGD对抗攻击下在ImageNet及其变体上平均提升6.7%，同时在干净样本上保持高准确率。

Conclusion: COLA是一种无需训练且与现有微调模型兼容的有效方法，能够显著提升CLIP模型在对抗攻击下的鲁棒性，同时保持对干净样本的良好性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的文本修复方法存在三个主要限制：无法移除文本、缺乏对渲染文本样式的控制、以及倾向于生成重复字母。这些限制阻碍了它们在更广泛的文本图像操作任务中的应用。

Method: 通过研究交叉注意力和自注意力机制的两个关键属性来实现文本移除和样式控制。使用自注意力反转来减少文本幻觉，重新分配交叉注意力以减少文本幻觉。在潜在优化框架中引入新的损失函数：交叉注意力内容损失来提高文本渲染准确性，自注意力样式损失来促进样式定制。

Result: OmniText在多个任务和指标上实现了最先进的性能，与其他文本修复方法相比表现优异，与专门方法相当。

Conclusion: OmniText是第一个能够执行多样化文本图像操作任务的通才方法，通过创新的注意力机制和损失函数设计，有效解决了文本移除、样式控制和内容编辑等挑战。
...
Method: 提出IIS评分，通过衡量可解释语义在表示中的比例来量化可解释性，并基于信息损失进行评估。

Result: 发现可解释性与分类能力呈正相关，高分类能力的表示提供更多可解释语义；通过可解释性最大化微调可进一步提升分类能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有训练自由方法在加速扩散过程时忽略去噪趋势，缺乏模型特定容忍度的误差控制，导致多步重用下的轨迹偏差和生成结果不一致性加剧。

Method: 引入一致性趋势预测器利用扩散轨迹的平滑连续性，将历史去噪模式投影到稳定未来方向；提出模型特定误差容忍度搜索机制，通过识别从波动语义规划到稳定质量精炼的过渡点来推导修正阈值。

Result: ETC在FLUX上实现2.65倍加速，一致性退化极小（-0.074 SSIM分数）。
...
Method: 利用大型语言模型从输入提示中合成显式布局，并将这些布局注入图像生成过程，通过目标中心视觉语言模型对多个候选结果进行重排，迭代选择最符合提示的结果。

Result: 通过统一显式布局基础与基于自精炼的推理时扩展，该框架在场景与提示对齐方面比最近的文本到图像模型表现更好。

Conclusion: 该训练免费框架能够有效提高文本到图像生成的布局忠实度，同时保持图像质量。
...
Method: 提出VC4VG框架，从T2V角度分析字幕内容，将视频重建所需元素分解为多个维度，并设计原则性的字幕设计方法；构建VC4VG-Bench基准，包含细粒度、多维度和必要性分级的评估指标。

Result: 广泛的T2V微调实验显示，改进的字幕质量与视频生成性能之间存在强相关性，验证了该方法的有效性。

Conclusion: VC4VG框架通过优化视频字幕显著提升了文本到视频生成模型的性能，为相关研究提供了实用的基准工具和代码支持。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在自动驾驶场景理解任务中面临的跨任务干扰、空间推理不足和输出质量不稳定的问题。

Method: 使用混合提示路由分类问题并分发到专家提示；任务特定提示包含坐标系统、空间推理规则、角色扮演和思维链推理；视觉组装模块组合多视角图像和对象裁剪；优化模型推理参数。
...
<details>
  <summary>Details</summary>
Motivation: SAM2作为SAM的继任者，在视频分割方面表现出强大的泛化能力，但其鲁棒性尚未被探索。现有针对SAM的攻击方法是否能直接迁移到SAM2尚不清楚，需要研究SAM2特有的架构挑战。

Method: 提出UAP-SAM2方法：1）使用目标扫描策略将每帧分成k个区域并随机分配提示，减少提示依赖性；2）设计双语义偏差框架，通过扭曲当前帧语义和破坏连续帧间语义一致性来优化UAP。

Result: 在六个数据集上的广泛实验表明，该方法对SAM2有效。比较结果显示UAP-SAM2显著优于最先进的攻击方法，优势明显。

Conclusion: UAP-SAM2成功解决了SAM2特有的架构挑战，证明了其在跨提示通用对抗攻击方面的有效性，为SAM2的鲁棒性评估提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统CNN模型在息肉和心脏分割中存在泛化性差、鲁棒性不足以及无法处理不确定性的问题，影响分割性能。

Method: 提出基于编码器-解码器的CLFSeg框架，集成模糊卷积模块，结合卷积层和模糊逻辑，并使用二元交叉熵与dice损失来处理类别不平衡问题。

Result: 在四个公开数据集上的实验表明，CLFSeg超越了现有SOTA性能，能够有效关注解剖结构中的相关感兴趣区域。

Conclusion: CLFSeg在确保计算效率的同时提升了分割性能，有望成为现实医疗诊断场景的潜在解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自回归建模在视觉生成中表现出色，但其逐令牌生成的推理速度过慢，单个样本需要数千步生成，严重限制了实际应用。

Method: 提出MC-SJD框架，基于耦合理论的信息论方法，通过最大化连续迭代中采样相同草稿令牌的概率来加速标准SJD，同时保持无损特性，仅需对现有算法进行单行修改。

Result: 该方法实现了显著的性能提升，在图像生成中达到约4.2倍加速，在视频生成中达到约13.3倍加速，且输出质量无任何下降。

Conclusion: MC-SJD通过简单的算法修改有效解决了自回归视觉生成的推理速度瓶颈，为实际应用提供了可行的加速方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有扩散模型在推理时通过负引导或能量优化进行身份抑制时，引起的分布偏移和身份与非身份属性纠缠问题，从而提升视觉质量和数据效用。

Method: 设计条件扩散模型，包含身份掩码学习方案：身份解耦潜在重组器使用身份变分自编码器建模身份特征，非身份属性通过同身份对提取和双向潜在对齐；身份引导潜在协调器通过软门控融合表示；采用基于重组的重建损失强制解耦。推理时从学习到的身份空间采样随机身份向量实现匿名化。

Result: 实验表明ID²Face在视觉质量、身份抑制和效用保持方面优于现有方法。

Conclusion: 提出的训练中心化匿名化框架能够有效解耦身份和非身份信息，实现高质量的人脸匿名化，无需推理时优化干预。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉token剪枝方法主要基于注意力分数选择最显著的token，导致所选token的语义不完整。MLLMs处理大量视觉token会产生显著计算开销，而许多token是冗余的。

Method: 提出SCOPE方法，引入集合覆盖率概念，计算基于token关系的覆盖度。定义token覆盖增益，量化包含未选token能获得的额外覆盖。将显著性分数整合到token覆盖增益中，提出SCOPE分数并迭代选择得分最高的token。

Result: 在多个视觉语言理解基准测试中使用LLaVA-1.5和LLaVA-Next模型进行广泛实验，结果表明该方法持续优于先前方法。

Conclusion: SCOPE方法通过联合考虑显著性和覆盖率，有效解决了现有视觉token剪枝方法的语义不完整问题，显著提升了MLLMs的效率。
...
Method: 1. 从序列建模角度重新审视阴影去除，采用选择性状态空间模型Mamba；2. 提出CrossGate方向调制机制，在Mamba输入门中注入阴影感知相似性；3. 引入ColorShift正则化，通过全局颜色统计驱动的对比学习目标来抑制颜色污染。
...
Method: 使用图像重合成方法：生成图像描述提示词，用所有候选源模型重合成图像，在特征空间中选择最接近原始图像的重合成结果进行归因。

Result: 在仅少量样本可用于训练或微调的情况下，所提出的重合成方法优于现有技术。新数据集被证明具有挑战性，是开发评估未来少样本和零样本方法的宝贵基准。
...
Method: 提出两阶段任务，将视觉感知学习构建为从粗到细的渐进过程，开发ViPER自引导框架，通过自我批评和自我预测实现迭代进化，结合图像级和实例级重建与两阶段强化学习策略。

Result: 在Qwen2.5-VL系列上应用ViPER产生Qwen-Viper系列，在七个综合基准测试中平均提升1.7%，在细粒度感知任务上提升高达6.0%，在不同视觉语言场景中保持卓越性能。

Conclusion: ViPER不仅实现了感知能力的自我提升，还为生成与理解之间的互惠关系提供了具体证据，是开发更自主、更强大视觉语言模型的突破。
...
<details>
  <summary>Details</summary>
Motivation: 遥感应用依赖深度学习进行场景分类，但面临标注数据稀缺和跨地理传感器域标注成本高的问题。现有视觉语言模型如CLIP存在领域差距和任务特定语义适应需求，需要轻量高效的适应策略。

Method: 评估了多种代表性提示学习方法：上下文优化、条件上下文优化、多模态提示学习、以及带自约束的提示学习。这些方法体现了从静态上下文优化到条件提示增强泛化、多模态提示联合视觉语言适应、以及语义正则化提示稳定学习的设计理念。

Result: 在多个基准遥感数据集上的实验表明，提示学习方法在少样本场景中始终优于零样本CLIP和线性探测基线。特别是带自约束的提示学习方法实现了最稳健的跨域性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习框架如nnU-Net在脑卒中病灶分割中性能优异但难以临床部署，存在依赖繁重和单体设计问题，需要开发轻量可部署的解决方案。
...
Conclusion: 高性能研究管道可以成功转化为便携、临床可用的工具，StrokeSeg框架实现了研究到临床的无缝转换。
...
TL;DR: 本研究系统探讨了在医学图像分类中如何利用放射学报告，发现在预训练阶段使用报告对标签与文本高度相关的任务有益，但对关联性弱的任务可能有害；在微调阶段使用报告能显著提升性能，在某些情况下比预训练方法影响更大。


<details>
  <summary>Details</summary>
Motivation: 医学图像训练通常伴随包含丰富专家注释的放射学报告，但依赖这些报告进行临床预测需要放射科医生的及时人工工作。本研究旨在探索何时以及如何在训练中利用放射学报告来改进仅基于图像的分类。

Method: 系统研究放射学报告在预训练和微调阶段的使用方法，涵盖诊断和预后任务（如12个月再入院），并在不同训练集规模下进行评估。

Result: 研究发现：(1) 当标签在文本中有良好表示时，预训练阶段利用报告对下游分类任务有益；但当标签与文本关联性弱时，通过显式图像-文本对齐进行预训练可能有害；(2) 微调阶段使用报告能带来显著改进，在某些情况下比预训练方法影响更大。
...
Method: 使用基于流的生成模型REFLECT进行无监督异常检测。在ATLAS数据上使用双专家中央切片标注，通过Free-Response ROC分析评估异常图的对象级性能。训练了两个模型：一个使用卒中患者无病灶切片（ATLAS），另一个使用健康对照（IXI）数据。

Result: 在ATLAS测试对象上，IXI训练的模型实现了更高的病灶分割（Dice = 0.37 vs 0.27）和更好的非病灶异常敏感性（FROC = 0.62 vs 0.43）。
...
<details>
  <summary>Details</summary>
Motivation: 解决多目标跟踪中目标数量未知和时变的问题，特别是在保持目标身份一致性和处理非线性动态方面，以及应对弱检测器和噪声环境下的挑战。

Method: 采用混合跟踪方法结合随机和确定性策略，利用粒子群优化（PSO）和提出的适应度度量引导粒子，整合目标间的社交交互来增强粒子引导和轨迹更新，建立基于空间一致性、外观、检测置信度、轨迹惩罚和社交分数的综合状态和观测模型。

Result: 实验结果表明GenTrack在标准基准测试和实际场景中优于最先进的跟踪器，减少了身份切换和轨迹丢失，特别是在遮挡期间表现更好。

Conclusion: GenTrack提供了一种有效的多目标跟踪解决方案，通过混合跟踪策略和社交交互整合显著提升了跟踪性能，为未来研究提供了可扩展的基础框架。
...
<details>
  <summary>Details</summary>
Motivation: 解决未知和时变目标数量下的标识一致性问题，处理非线性动态和非高斯噪声，在目标交互和长时间遮挡时保持跟踪稳定性。

Method: 使用随机粒子滤波处理非线性动态，结合粒子群优化引导粒子；采用确定性关联机制保持标识一致性；提出状态平滑更新方案和速度回归技术。

Result: 实验结果显示相比现有最先进跟踪器具有优越性能，适用于预录制视频和实时摄像头流。

Conclusion: 提出的方法在复杂场景下能够有效保持目标标识一致性，在目标交互和遮挡情况下表现良好，具有实际应用价值。
...
Conclusion: 该方法为水库存储量的连续、无传感器监测提供了实用解决方案，可复制应用于其他水体，生成的50年时间序列数据对气候变化和环境模式研究具有重要价值。
...
TL;DR: 本文提出了一个专门用于评估语义分割中可解释AI（XAI）方法的系统性框架，考虑了空间和上下文任务复杂性，使用像素级评估策略和精心设计的指标。


<details>
  <summary>Details</summary>
Motivation: 随着AI模型在安全关键和高风险领域的应用增加，确保透明度和可信度至关重要。虽然XAI方法在分类任务中的评估已有进展，但针对语义分割的评估策略相对较少探索。

Method: 开发了一个全面的系统性评估框架，专门用于评估语义分割中的XAI方法，采用像素级评估策略和精心设计的指标，考虑空间和上下文任务复杂性。

Result: 使用最近适应的基于类激活映射（CAM）的XAI方案进行模拟，证明了所提出方法的高效性、鲁棒性和可靠性。
...
TL;DR: 提出一种基于功能分解的深度条件图像压缩框架，通过自生成先验来分离图像的结构骨架和纹理细节，有效解决了传统方法在低码率下的几何变形问题。


<details>
  <summary>Details</summary>
Motivation: 当前学习型图像压缩方法难以建模自然图像中复杂的相关结构，特别是全局不变结构与局部瞬态纹理的纠缠问题，导致低码率下出现严重几何变形。

Method: 采用功能分解框架，首先生成自编码先验来捕捉图像结构骨架，然后利用该先验深度调节整个压缩流程，特别是分析变换，使其专注于剩余的高熵细节。

Result: 在Kodak、CLIC和Tecnick数据集上，相比VVC测试模型VTM-12.1实现了显著的BD-rate降低，分别为14.4%、15.7%和15.1%。视觉分析显示大幅减轻了几何变形伪影。

Conclusion: 所提出的深度条件图像压缩框架通过分层依赖驱动的方法有效分离信息流，在保持高压缩性能的同时显著改善了低码率下的视觉质量。
...
TL;DR: 视频扩散模型（VDMs）在视觉任务中表现出比大型语言模型（LLMs）更高的数据效率，视频预训练为视觉基础模型提供了有利的归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语言领域表现出强大的少样本适应能力，但这种成功未能有效迁移到视觉领域，视觉模型仍在组合理解、样本效率和通用问题解决方面存在困难。

Method: 通过对比预训练LLM和预训练VDM在各自自然模态任务上的表现，使用轻量级适配器，在ARC-AGI、ConceptARC、视觉游戏、路径规划和元胞自动机等多个基准上进行评估。

Result: VDM在所有基准测试中都表现出比语言对应模型更高的数据效率，表明视频预训练提供了支持任务适应的强归纳偏置。

Conclusion: 视频预训练为视觉基础模型的发展提供了有前景的方向，其时空数据预训练赋予了模型对结构和动态的强归纳偏置，支持广泛的视觉任务适应性。
...
TL;DR: 该论文提出使用深度学习技术检测三种茶树病害（红锈病、Helopeltis和红蜘蛛螨），并评估了SSD MobileNet V2和Faster R-CNN ResNet50 V1两种目标检测模型，其中Faster R-CNN表现更好（mAP 25%），同时使用Mask R-CNN进行实例分割以计算叶片受损面积。


<details>
  <summary>Details</summary>
Motivation: 开发能够自动识别茶树病害并量化叶片受损面积的深度学习系统，以帮助茶农及时监测和管理茶树健康状况。

Method: 使用SSD MobileNet V2和Faster R-CNN ResNet50 V1进行目标检测，Mask R-CNN进行实例分割，并开发了自定义方法来计算叶片病害受损区域。

Result: Faster R-CNN ResNet50 V1在IOU 0.50:0.95范围内获得25%的mAP，优于SSD MobileNet V2的20.9% mAP。同时成功实现了病害区域的实例分割和面积计算。

Conclusion: Faster R-CNN ResNet50 V1在茶树病害检测中表现优于SSD MobileNet V2，结合Mask R-CNN的实例分割能够有效识别病害并量化受损面积，为茶树病害监测提供了可行的技术方案。
...
TL;DR: Kineo是一个完全自动化的无标定多视角运动捕捉系统，使用未同步、未标定的消费级RGB相机，通过2D关键点同时标定相机和重建3D关键点，在计算效率和精度上显著优于现有方法。
...
Method: 利用现成的2D关键点检测器，通过置信度驱动的时空关键点采样策略和图优化方法，同时标定相机（包括畸变系数）并重建3D关键点和密集场景点云，引入成对重投影一致性评分量化重建可靠性。

Result: 在EgoHumans和Human3.6M数据集上的评估显示，相比现有方法，相机平移误差降低83-85%，相机角度误差降低86-92%，世界平均关节误差降低83-91%。

Conclusion: Kineo在保持高精度的同时显著提升了计算效率，在特定配置下处理速度超过实时，为无标定运动捕捉提供了实用解决方案。
...
TL;DR: 提出Decoupled MeanFlow方法，将流模型转换为流图模型，无需架构修改，实现1-4步高质量生成，推理速度提升100倍以上。


<details>
  <summary>Details</summary>
Motivation: 去噪生成模型（如扩散模型和流模型）需要大量去噪步骤，存在离散化误差问题。流图模型可以缓解此误差并加速采样，但传统训练方法需要架构修改，限制了与预训练流模型的兼容性。

Method: 引入Decoupled MeanFlow解码策略，通过将扩散变换器的最后块条件化到后续时间步，直接将预训练流模型重新用作流图。结合增强的训练技术，实现1-4步高质量生成。

Result: 在ImageNet 256x256和512x256上，1步FID分别达到2.16和2.12，大幅超越现有技术；4步FID达到1.51和1.68，接近流模型性能，推理速度提升100倍以上。

Conclusion: 训练流模型后转换为流图比从头训练流图更高效有效，该方法实现了高质量快速生成，在保持性能的同时显著提升推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉理解方面表现出色，但在需要视觉规划和想象的复杂场景中表现不佳。受人类使用草图进行视觉思考的启发，研究者希望为模型提供类似的视觉思考能力。

Method: 在现有MLLMs基础上集成视觉生成到自回归推理过程中，包含两个组件：上下文感知视觉头自回归生成视觉表示，预训练草图解码器将这些表示渲染为可解释的图像。

Result: 在MazePlanning数据集上的实验表明，Latent Sketchpad在保持主干模型推理性能的同时，提供了可比较甚至更优的推理表现，并在Gemma3和Qwen2.5-VL等不同MLLMs上具有良好泛化性。

Conclusion: 通过将模型的文本推理扩展到视觉思考，该框架为人机交互和更广泛的应用开辟了新机会。
...
<details>
  <summary>Details</summary>
Motivation: 病理学家对千兆像素全切片图像（WSIs）的解读准确率平均约为70%，且增加第二位病理学家并不能显著提高决策一致性。该领域缺乏足够的行为数据来解释诊断错误和不一致性。

Method: 通过名为PTAH的应用基础测试平台收集数据，强调生态效度。数据集包含19位病理学家解读397张WSIs的18.69小时数据，记录了171,909个注视点、263,320个扫视和1,867,362个鼠标交互事件。

Result: 创建了PathoGaze1.0数据集，包含眼动追踪、鼠标交互、刺激追踪、视口导航和诊断决策数据（EMSVD），为研究病理学家的诊断行为提供了全面资源。

Conclusion: 该数据集不仅有助于解释病理学家的诊断错误和一致性，还可用于改进病理学家和AI系统的训练，所有实验已预注册，完整数据集和分析代码已公开。
...
Method: 分析DiT模型中的MM-Attention机制，发现Query和Key token共享仅层依赖的偏置向量，将其解释为模型固有编辑行为，而token与对应偏置的delta编码内容特定编辑信号。提出GRAG方法重新加权不同token的delta值。

Result: 在现有图像编辑框架上的广泛实验表明，GRAG仅需四行代码即可集成，持续提升编辑质量。相比常用的Classifier-Free Guidance，GRAG实现更平滑精确的编辑程度控制。

Conclusion: GRAG是一种简单有效的方法，能够实现连续细粒度的编辑强度控制，无需任何调优即可提升编辑质量。
...
TL;DR: SAGE是一种零样本视频过渡方法，通过结合结构引导（线条图和运动流）与生成合成，在多样视频片段之间产生平滑、语义一致的过渡，无需微调。


<details>
  <summary>Details</summary>
Motivation: 传统视频过渡方法（如线性混合、交叉淡入淡出、变形）和最近的生成方法在处理具有大时间间隔或显著语义差异的多样化视频片段时，难以保持内容感知和视觉连贯性。

Method: 基于艺术工作流程，提出SAGE方法，结合结构引导（线条图和运动流）与生成合成，在零样本设置下实现视频过渡。

Result: 与现有方法（FILM、TVG、DiffMorpher、VACE、GI）相比，SAGE在定量指标和用户研究中均表现出色，能够为多样化视频片段生成更好的过渡效果。

Conclusion: SAGE通过结构感知的生成方法，成功解决了多样化视频片段之间的过渡挑战，在保持语义一致性和视觉连贯性方面优于现有方法。
...
TL;DR: MIC-BEV是一个基于Transformer的鸟瞰图感知框架，用于基础设施多摄像头3D目标检测，支持可变数量摄像头和异构参数，在传感器退化下保持强鲁棒性。
...
Conclusion: MIC-BEV展示了在实际部署中的潜力，为基础设施感知提供了有效的解决方案。
...
TL;DR: 研究发现预训练的视觉变换器（ViTs）能够自然涌现出物体绑定能力，即识别哪些图像块属于同一物体的能力。这种能力在自监督模型（DINO、MAE、CLIP）中表现突出，但在ImageNet监督模型中较弱，表明这是通过特定预训练目标习得的能力。


<details>
  <summary>Details</summary>
Motivation: 探索视觉变换器是否能够自然涌现出物体绑定能力，即识别哪些图像块属于同一物体的能力，这对于理解ViTs的内部表示机制具有重要意义。

Method: 使用相似性探针从ViT各层的图像块嵌入中解码"IsSameObject"属性，分析不同预训练模型（自监督和监督）的表现，并进行消融实验验证该信号对注意力的指导作用。

Result: IsSameObject解码准确率超过90%，在自监督ViTs中可靠涌现，但在ImageNet监督模型中明显较弱。该信号编码在物体特征之上的低维子空间中，并主动指导注意力。

Conclusion: 视觉变换器能够自然涌现物体绑定能力，挑战了ViTs缺乏物体绑定的观点，展示了连接主义系统中"哪些部分属于一起"的符号知识如何自然产生。
...
TL;DR: ProMoE是一个针对扩散变换器（DiT）的混合专家框架，通过两阶段路由器和显式路由指导来解决视觉令牌与语言令牌的差异问题，提升专家专业化，在ImageNet基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有将MoE应用于扩散变换器的尝试效果有限，作者认为这是由于语言令牌和视觉令牌的根本差异：语言令牌语义密集且变化明显，而视觉令牌存在空间冗余和功能异质性，阻碍了视觉MoE中的专家专业化。

Method: 提出ProMoE框架，包含两阶段路由器：1）条件路由根据功能角色将图像令牌划分为条件集和无条件集；2）原型路由通过可学习原型基于语义内容细化条件图像令牌的分配。还提出路由对比损失来增强原型路由过程。

Result: 在ImageNet基准测试中，ProMoE在Rectified Flow和DDPM两种训练目标下均超越了最先进的方法。

Conclusion: ProMoE通过显式路由指导和原型路由机制有效解决了视觉MoE中的专家专业化问题，验证了语义指导对视觉MoE的重要性，为扩散变换器的MoE应用提供了有效解决方案。
...
TL;DR: URSA是一个离散视频生成框架，通过全局迭代优化离散时空token，结合线性化度量路径和分辨率相关时间步长偏移机制，实现了高效的高分辨率图像合成和长视频生成，性能媲美连续扩散方法。


<details>
  <summary>Details</summary>
Motivation: 连续空间视频生成方法发展迅速，而离散方法由于误差累积和长上下文不一致性问题而落后。本文旨在重新审视离散生成建模，弥合与连续方法在可扩展视频生成方面的差距。

Method: URSA将视频生成任务建模为离散时空token的迭代全局优化，核心设计包括线性化度量路径和分辨率相关时间步长偏移机制，还引入了异步时间微调策略来统一多种任务。

Result: 在具有挑战性的视频和图像生成基准测试中，URSA持续优于现有离散方法，并实现了与最先进连续扩散方法相当的性能。

Conclusion: URSA是一个简单而强大的离散生成建模框架，能够有效扩展到高分辨率图像合成和长视频生成，同时需要更少的推理步骤，为离散方法在视频生成领域提供了有竞争力的解决方案。
...
TL;DR: 提出Generative View Stitching (GVS)方法，通过并行采样整个序列，解决自回归视频扩散模型无法利用未来条件指导当前生成的问题，实现与预定义相机轨迹一致的稳定、无碰撞视频生成。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型虽然能够进行长序列生成且保持历史一致性，但无法利用未来条件指导当前生成。在相机引导的视频生成中，这会导致与生成场景的碰撞，进而导致自回归快速崩溃。

Method: 提出GVS采样算法，将扩散缝合技术从机器人规划扩展到视频生成。该方法与任何经过Diffusion Forcing训练的现成视频模型兼容，并引入Omni Guidance技术增强时间一致性，以及循环闭合机制实现长程连贯性。

Result: GVS实现了稳定、无碰撞、帧间一致的相机引导视频生成，能够处理各种预定义相机路径，包括Oscar Reutersvärd的不可能楼梯。

Conclusion: GVS方法成功解决了自回归视频扩散模型在相机引导生成中的局限性，通过并行采样和扩散缝合技术实现了与预定义相机轨迹完全一致的视频生成。
...
TL;DR: Game-TARS是一个通用游戏智能体，使用统一、可扩展的键盘鼠标动作空间进行训练，在跨域大规模预训练中表现出色，在多个游戏基准测试中超越现有最先进模型。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够跨操作系统、网页和模拟游戏等异构域进行大规模持续预训练的通用游戏智能体，避免API或GUI方法的限制。

Method: 采用基于人类对齐的键盘鼠标输入的统一动作空间，使用衰减持续损失减少因果混淆，以及高效的稀疏思维策略平衡推理深度和推理成本，在500B+ token的多模态数据上进行预训练。

Result: 在开放世界Minecraft任务中成功率是之前SOTA模型的2倍，在未见过的网页3D游戏中接近人类新手水平，在FPS基准测试中超越GPT-5、Gemini-2.5-Pro和Claude-4-Sonnet。
...
Method: 引入学科创造力的概念，通过两个数学案例进行分析，比较计算和AI在学科创造力中的作用差异。

Result: 研究发现计算可以扩展学科创造力，但某些AI方法可能取代学科创造力，从而改变科学追求的价值。

Conclusion: AI在科学问题解决中的应用可能对学科创造力产生消极影响，需要谨慎考虑其对科学价值的影响。
...
TL;DR: 本文提出了多环境POMDP（ME-POMDP）模型，用于处理具有离散模型不确定性的POMDP问题，并进一步推广为对抗性信念POMDP（AB-POMDP）。作者开发了精确和近似算法来计算鲁棒策略。


<details>
  <summary>Details</summary>
Motivation: 当多个领域专家对问题建模存在分歧时，需要一种能够处理模型不确定性的框架。ME-POMDP扩展了标准POMDP，能够表示一组共享状态、动作和观测空间但可能在不同转换、观测和奖励模型上存在差异的POMDP集合。

Method: 1）将ME-POMDP推广为具有初始信念集合的AB-POMDP；2）证明任意ME-POMDP可以简化为仅在转换和奖励函数或仅在观测和奖励函数上变化的ME-POMDP；3）设计精确和近似（基于点）算法来计算AB-POMDP的鲁棒策略。

Result: 成功为扩展到多环境设置的标准POMDP基准测试计算了鲁棒策略，验证了所提方法的有效性。

Conclusion: ME-POMDP和AB-POMDP为处理模型不确定性提供了有效的理论框架，所开发的算法能够计算针对最坏情况的鲁棒策略，适用于多个领域专家建模存在分歧的实际场景。
...
<details>
  <summary>Details</summary>
Motivation: 当前串联质谱分析方法依赖于数据库匹配或多步骤流程，难以识别参考数据库中不存在的化合物，需要开发能够直接从质谱数据生成分子结构的新方法。
...
TL;DR: 该研究探讨了生成式AI在象棋谜题创作中的创造力，开发了一个能生成具有美学吸引力、新颖性和反直觉解法的AI系统，并邀请三位国际象棋专家评估其创造性。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速发展，研究其能否产生真正具有创造性和新颖性的输出，特别是在象棋谜题创作领域。

Method: 开发专门用于生成象棋谜题的AI系统，该系统能够产生具有美学吸引力、新颖性和反直觉解法的谜题，并邀请三位国际象棋专家进行评估。

Result: 三位世界知名象棋专家（国际象棋排局大师Amatzia Avni、特级大师Jonathan Levitt和Matthew Sadler）对AI生成的谜题进行了评估，选择了他们最喜欢的谜题并解释了其吸引力所在。

Conclusion: 研究表明生成式AI在象棋谜题创作中展现出创造性潜力，能够产生被专家认可的具有美学价值和挑战性的作品。
...
TL;DR: 病理学基础模型在癌症诊断和预后方面存在根本性弱点，包括诊断准确性低、鲁棒性差、计算需求大等，这些源于通用AI基础模型假设与人体组织内在复杂性之间的概念不匹配。
...
Method: 通过系统评估分析当前病理学基础模型的局限性，识别出七个相互关联的根本原因：生物复杂性、无效的自监督学习、过度泛化、过度复杂的架构、缺乏领域特定创新、数据不足以及与组织块大小相关的根本设计缺陷。

Result: 研究发现当前病理学基础模型在诊断准确性、鲁棒性、几何稳定性、计算效率和安全性方面存在严重不足，无法有效处理人体组织的复杂形态特征。

Conclusion: 当前病理学基础模型在概念上与组织形态学本质存在错位，需要对整个范式进行根本性的重新思考，以解决模型假设与生物复杂性之间的不匹配问题。
...
TL;DR: ReCAP是一个用于大语言模型的递归上下文感知推理和规划框架，通过计划分解、父计划结构化重注入和内存高效执行机制，显著提升了长时程任务的子目标对齐和成功率。
...
Method: 结合三个关键机制：(i) 计划提前分解，生成完整子任务列表并执行第一个项目；(ii) 结构化重注入父计划，保持多级上下文一致性；(iii) 内存高效执行，限制活跃提示使成本随任务深度线性增长。

Result: 在各种长时程推理基准测试中显著提升了子目标对齐和成功率，在同步和异步Robotouille任务上分别实现了32%和29%的改进。

Conclusion: ReCAP框架通过将高层目标与低层行动对齐、减少冗余提示和保持连贯的上下文更新，有效解决了长时程推理中的关键挑战。
...
TL;DR: 该论文研究了多智能体路径规划中的分散式目标分配问题，比较了贪婪启发式、最优分配和基于大语言模型（LLM）的智能体在完全可观测网格环境中的表现。


<details>
  <summary>Details</summary>
Motivation: 解决在分散条件下协调多个自主智能体在共享环境中的长期挑战，特别是在多智能体路径规划中的目标分配问题。
...
Conclusion: 这些发现强调了语言模型在分散式多智能体路径规划目标分配中的潜力，并突出了信息结构在此类系统中的重要性。
...
Result: 强化学习方法将反直觉谜题生成率从0.22%（监督学习）大幅提升至2.5%，超过了现有数据集比率（2.1%）和最佳Lichess训练模型（0.4%）。生成的谜题满足新颖性和多样性基准，保留了美学主题，并被人类专家评为比传统书籍谜题更具创造性、趣味性和反直觉性，甚至接近经典作品水平。

Conclusion: 最终成果是一本经过精心策划的AI生成谜题手册，该手册的创造性得到了三位世界知名专家的认可，证明了该方法在生成高质量创造性内容方面的有效性。
...
TL;DR: 该研究比较了数字孪生在动态系统建模和控制中的应用，包括物理模型、数据驱动模型和混合模型，以及传统和AI驱动的控制器。在温室测试平台上，HAM模型表现最均衡，LSTM精度高但资源消耗大；MPC控制器稳健，RL适应性强，LLM控制器提供灵活的人机交互。


<details>
  <summary>Details</summary>
Motivation: 研究数字孪生在动态系统建模和控制中的集成应用，比较不同建模方法和控制策略在精度、适应性和实施复杂度方面的权衡。

Method: 使用微型温室作为测试平台，开发四种预测模型（线性、PBM、LSTM、HAM）和三种控制策略（MPC、RL、LLM），在插值和外推场景下进行比较评估。

Result: HAM模型在准确性、泛化能力和计算效率方面表现最均衡；LSTM精度高但资源消耗大；MPC控制器性能稳健可预测；RL控制器适应性强；LLM控制器结合预测工具提供灵活的人机交互。

Conclusion: 数字孪生系统中，HAM方法在建模方面提供最佳平衡，而不同控制器各有优势：MPC适合稳健控制，RL适合自适应场景，LLM适合人机协作控制。
...
TL;DR: 本文提出了一种基于变分推理的链式思维训练方法，通过稀疏奖励函数和贝叶斯推理缩放策略，提升大型视觉语言模型的推理能力、泛化性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有训练算法（如SFT、PPO、GRPO）在未见推理任务上泛化能力不足，且严重依赖有偏的奖励模型，限制了链式思维推理的可靠性。

Method: 将LVLMs中的推理重新表述为后验推断，基于摊销变分推理提出可扩展训练算法，使用多样性寻求强化学习算法引入稀疏奖励函数，并采用贝叶斯推理缩放策略替代昂贵的Best-of-N和Beam Search。

Result: 在七个推理基准测试中，该方法显著提升了最先进LVLMs的性能，在有效性、泛化性和可解释性方面均有改善。

Conclusion: 提出的变分推理方法能够有效克服确定性采样限制和奖励攻击问题，为LVLMs的链式思维推理提供了更可靠和可扩展的训练框架。
...
TL;DR: 提出直觉主义去中心化因果发现框架——柔道演算，使用j-稳定因果推断和j-do-演算在层拓扑中形式化处理现实应用中因果效应的情境依赖性。


<details>
  <summary>Details</summary>
Motivation: 现实应用（从生物学到医学和社会科学）中因果效应依赖于情境（年龄、国家、剂量、基因型或实验室协议），需要形式化处理这种情境依赖性。

Method: 柔道演算，结合标准基于分数、约束和梯度的因果发现方法，使用Lawvere-Tierney模态算子j选择相关情境，j-稳定性确保因果主张在相关情境族中构造性和一致性地成立。

Result: 在合成和真实世界数据集（生物学和经济学）上的实验结果显示，层理论因果发现的去中心化性质带来了计算效率提升，且性能优于经典因果发现方法。

Conclusion: 柔道演算为处理因果效应的情境依赖性提供了形式化框架，通过去中心化方法提高了计算效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统LLM对齐方法对人类偏好的异质性很敏感，拟合简单的概率模型到成对比较数据会产生不一致的总体平均效用估计，这是社会福利的规范衡量标准。

Method: 提出符号估计器方法，在聚合步骤中用二元分类损失替换交叉熵损失，在温和假设下恢复一致的有序对齐，并在此设置中首次实现多项式有限样本误差界限。

Result: 在基于数字孪生的LLM对齐现实模拟中，符号估计器显著减少了模拟人物面板的偏好失真，将（角度）估计误差降低了近35%，与真实总体偏好的不一致性从12%降至8%。
...
<details>
  <summary>Details</summary>
Motivation: 预测破坏性事件前个体移动模式的变化具有挑战性，因为缺乏衡量个体异质性社会基础设施韧性的方法，且个体移动与空间上下文的复杂关系未被充分捕捉，同时个体级移动数据通常稀疏且不适合传统预测方法。

Method: 将个体的社会基础设施韧性(SIR)整合到条件深度学习模型中，利用大规模稀疏的个体级数据捕捉个体移动模式与局部空间上下文之间的复杂关系。

Result: 实验表明，结合个体的SIR和空间上下文可以增强模型预测事件后个体移动模式的能力。条件模型能够捕捉到具有相似事件前移动模式但SIR不同的个体之间移动模式的差异变化。

Conclusion: 该研究证明了将个体社会基础设施韧性纳入预测模型的重要性，能够更准确地预测破坏性事件后个体移动模式的变化，为社区资源需求预测提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决跨域时间序列预测中面临的领域特定趋势变化和不一致周期性模式的挑战，现有方法将时间序列视为未分化的序列，未能显式解耦其固有结构组件。

Method: 提出OneCast框架，将时间序列分解为季节性和趋势组件：季节性组件通过轻量级投影模块使用可解释基函数重建周期性模式；趋势组件通过语义感知tokenizer编码为离散token，并使用掩码离散扩散机制进行推断。

Result: 在八个领域的广泛实验中，OneCast大多优于最先进的基线方法。

Conclusion: OneCast通过结构化分解和组件化建模，有效解决了跨域时间序列预测中的泛化问题，能够同时捕捉季节性模式和跟踪领域特定趋势。
...
TL;DR: 本研究比较了经典模型和机器学习模型在电动汽车跟驰行为建模中的表现，发现随机森林模型在所有场景下都优于物理模型，特别是在不同跟车距离下都表现出更高的预测精度。
...
Method: 使用经典模型（IDM、OVM、OVRV、简化CACC）和随机森林回归器，基于真实世界电动汽车跟随内燃机车辆的数据集进行建模，通过最小化RMSE来校准经典模型参数。

Result: 随机森林模型在所有跟车距离场景下表现最佳，RMSE分别为0.0046（中等距离）、0.0016（长距离）和0.0025（超长距离）；在物理模型中，CACC模型表现最好，长距离RMSE为2.67。

Conclusion: 机器学习模型在电动汽车跟驰行为建模中表现优于经典物理模型，这些模型对于模拟电动汽车行为和分析混合自动驾驶交通动态具有重要价值。
...
TL;DR: HistoLens是一个透明的AI助手，让病理学家能用自然语言提问组织切片问题，提供结构化报告和可视化证据，增强医生对AI的信任。


<details>
  <summary>Details</summary>
Motivation: 为了让医生真正信任人工智能，需要让AI变得透明可解释，就像咨询同事一样理解其推理过程。

Method: 开发了HistoLens系统，允许病理学家用简单英语提问，系统将问题转换为精确查询，提供结构化报告和热图可视化证据，并训练AI专注于患者组织而忽略背景噪音。

Result: 创建了一个工作流程，病理学家保持专家主导地位，使用可信赖的AI助手验证见解，做出更快、更自信的诊断。

Conclusion: HistoLens通过透明性和协作性设计，成功建立了医生对AI的信任，使AI成为病理学家的可靠合作伙伴。
...
TL;DR: BLM₁是一个多模态空间基础模型，通过两阶段训练实现跨空间传输、跨任务学习和跨具身泛化能力，在数字和物理任务中均优于现有模型家族。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在数字-物理空间和具身智能体之间泛化能力差，VLAs缺乏高级具身推理能力，ELLMs局限于数字空间，因此需要能够无缝操作于数字和物理空间并跨具身和任务泛化的统一模型。

Method: 采用两阶段训练范式：第一阶段通过精选数字语料库将具身知识注入MLLM同时保持语言能力；第二阶段通过意图桥接接口训练策略模块，从MLLM提取高级语义指导控制，无需微调MLLM主干。

Result: 在数字和物理基准测试中，单个BLM₁实例优于四个模型家族（MLLMs、ELLMs、VLAs和GMLMs），在数字任务中提升约6%，在物理任务中提升约3%。

Conclusion: BLM₁通过创新的两阶段训练方法成功实现了跨空间、跨任务和跨具身的统一建模，为具身智能的发展提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: MCTS的样本效率问题可以通过状态和动作抽象来解决，但现有抽象算法（如pruned OGA）在处理同一抽象节点中多个动作时使用随机平局决胜规则，存在改进空间。

Method: 提出并实证评估了多种替代的内部抽象策略，这些策略旨在更有效地处理同一抽象节点中多个动作的UCB值相同的情况。

Result: 多个提出的内部抽象策略在大多数环境和参数设置中表现优于随机策略。

Conclusion: 通过改进抽象节点内部的策略选择，可以显著提升MCTS在抽象环境中的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具有强大的推理能力，但容易产生错误和幻觉。现有的验证方法严重依赖外部资源，导致计算开销大且仅适用于特定领域。

Method: 研究发现输入问题与输出推理路径之间的相关矩阵秩是推理正确性的稳健指标。基于此设计了简单、即插即用的Self-Indicator方法，通过重新加权候选推理路径来验证正确性。

Result: 该方法在多个不同规模和模型家族的LLM上实验有效，区分正确与错误推理路径的准确率超过75%，并在三个推理基准上将准确率提高了8%以上。

Conclusion: Self-Indicator方法仅依赖LLM自身即可有效验证推理路径可信度，计算开销小且性能显著优于其他投票和验证方法。
...
TL;DR: 本文提出了一种新颖的多智能体框架，用于基于判断预测的声明验证，通过不同智能体生成正反证据并构建定量双极论证框架，实验表明多智能体组合能提高预测准确性并提供可解释的证据组合。


<details>
  <summary>Details</summary>
Motivation: 将判断预测视为声明验证任务，需要评估未来事件的可能性，但现有方法在证据收集和解释性方面存在不足。

Method: 提出了一个多智能体框架，包含三种类型的LLM智能体：ArgLLM、RbAM和RAG-ArgLLM，分别采用不同的证据生成方法，并构建定量双极论证框架来表示证据。

Result: 在两个标准判断预测数据集上的实验表明，组合多个智能体的证据可以改善预测准确性，特别是在使用三个智能体时，同时提供了可解释的证据组合。

Conclusion: 多智能体框架能够有效支持声明验证任务，通过整合不同来源的证据提高预测性能，并为决策过程提供透明度。
...
<details>
  <summary>Details</summary>
Motivation: 尽管生成式大语言模型在传播研究内容分析中展现出巨大潜力，能够超越众包工作者和训练有素的编码员，但其在传播研究方法论工具包中的整合仍不充分，需要解决影响结果质量的七个关键挑战。

Method: 综合新兴研究，提出应对七个关键挑战的全面最佳实践指南：代码本开发、提示工程、模型选择、参数调优、迭代优化、模型可靠性验证以及性能提升。

Result: 研究表明gLLMs在传播科学相关的各种编码任务中表现出色，能以更低的时间和成本解码隐含意义和上下文信息，仅需基本编程技能和少量标注数据。

Conclusion: 本文旨在使基于gLLM的内容分析对更广泛的传播研究者更加易用，并确保符合有效性、可靠性、可重复性和研究伦理等既定的学科质量标准。
...
Result: 在9个具有不同特征的数据集上评估，使用DeepSeek-V3和GPT-4o作为后端，VDSAgents在性能上持续优于AutoKaggle和DataInterpreter等最先进的端到端数据科学系统。

Conclusion: 验证了将PCS原则嵌入LLM驱动的数据科学自动化的可行性，为构建更可信和鲁棒的自动化数据科学系统提供了有效方法。
...
TL;DR: 本文提出了一种多智能体生态系统，用于N-of-1决策支持，旨在解决传统医疗AI系统在边缘病例（罕见变异、多发病、代表性不足人群）上的表现不佳问题。


<details>
  <summary>Details</summary>
Motivation: 传统医疗AI系统为平均患者设计，在大型数据集上最小化错误，但在边缘病例上表现不佳，这损害了公平性和信任度。

Method: 构建多智能体生态系统，智能体按器官系统、患者群体和分析模式聚类，共享模型库和证据合成工具，通过协调层整合结果并评估可靠性、不确定性和数据密度。

Result: 系统为临床医生提供决策支持包，包括风险估计（带置信区间）、异常值标记和关联证据，验证重点从群体平均转向个体可靠性。
...
Method: Orion将推理过程分解为两个协同阶段：关键点生成（通过检索增强的少样本提示提炼逻辑结构化的关键点）和内容并行扩展（基于依赖图并发阐述关键点以确保逻辑一致性），并引入跨查询的流水线调度机制。

Result: 实验表明，Orion相比基线方法实现了最高4.33倍的token生成速度和3.42倍的答案延迟降低，同时通过显式建模点间依赖关系将推理质量提升了18.75%。

Conclusion: Orion框架成功平衡了推理效率与质量，为LLM在实时Web应用中的部署提供了可行解决方案。
...
Conclusion: APTBench填补了预训练阶段智能体能力评估的空白，为更有效地指导模型训练提供了新的基准框架。
...
TL;DR: 本研究比较了多个大型语言模型（GPT、Claude、DeepSeek等）的逻辑和抽象推理能力，通过8个定制推理问题评估，并与人类表现进行基准测试，揭示了LLMs在演绎推理方面的显著差异和困难。
...
Result: 研究结果显示LLMs之间存在显著差异，并表明LLMs在演绎推理方面存在困难。

Conclusion: 大型语言模型在逻辑和抽象推理能力上仍有明显不足，特别是在演绎推理方面，需要进一步改进和发展。
...
### [83] [Law in Silico: Simulating Legal Society with LLM-Based Agents](https://arxiv.org/abs/2510.24442)
*Yiding Wang,Yuxuan Chen,Fanxu Meng,Xifan Chen,Xiaolei Yang,Muhan Zhang*

Main category: cs.AI

TL;DR: 本文提出Law in Silico框架，使用基于大语言模型的智能体来模拟法律社会，验证了该框架能够复现宏观犯罪趋势并揭示法律系统对弱势群体权利保护的重要性。


<details>
  <summary>Details</summary>
Motivation: 由于现实世界中的法律实验成本高昂或难以实施，利用人工智能系统模拟法律社会成为验证和发展法律理论的有效替代方案。大语言模型具有世界知识和角色扮演能力，适合作为法律社会模拟的基础。

Method: 引入Law in Silico框架，这是一个基于大语言模型的智能体框架，用于模拟包含个体决策以及立法、裁决和执行等制度机制的法律场景。

Result: 实验表明，基于大语言模型的智能体能够很大程度上复现宏观层面的犯罪趋势，并提供与现实世界观察相符的见解。微观层面模拟显示，功能良好、透明且适应性强的法律系统能更好地保护弱势个体的权利。

Conclusion: 基于大语言模型的智能体框架能够有效模拟法律系统，不仅复现宏观犯罪趋势，还揭示了良好法律系统对弱势群体权利保护的重要性，为法律理论验证和系统优化提供了新途径。
...
### [84] [From Cross-Task Examples to In-Task Prompts: A Graph-Based Pseudo-Labeling Framework for In-context Learning](https://arxiv.org/abs/2510.24528)
*Zihan Chen,Song Wang,Xingbo Fu,Chengshuai Shi,Zhenyu Lei,Cong Shen,Jundong Li*

Main category: cs.AI

TL;DR: 提出一种成本效益高的两阶段管道，通过跨任务示例伪标记少量目标实例，然后使用图标签传播方法扩展标签，减少对LLM标注的依赖。


<details>
  <summary>Details</summary>
Motivation: 为ICL收集高质量示例成本高且劳动密集，需要减少对LLM标注的依赖。

Method: 两阶段管道：1) 利用跨任务示例提示LLM伪标记少量目标实例；2) 图标签传播方法扩展标签信息，无需额外LLM查询。
...
### [85] [Generative AI for Healthcare: Fundamentals, Challenges, and Perspectives](https://arxiv.org/abs/2510.24551)
...
TL;DR: 本文提出了一种以数据为中心的方法来设计和部署医疗领域的生成式AI系统，通过重新定位数据生命周期，将医疗数据生态系统作为生成式医疗系统的基础支撑。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在医疗领域具有巨大潜力，能够减轻临床医生的认知负担并改善医疗服务。然而，医疗领域部署生成式AI需要深入理解医疗任务及其可行性。

Method: 将医疗数据生态系统作为基础支撑，通过语义向量搜索和上下文查询等高效数据处理管道，支持上游模型组件和下游临床应用。

Result: 该生态系统不仅为基础模型提供高质量多模态数据用于大规模预训练和领域特定微调，还作为知识检索后端支持任务特定推理。

Conclusion: 这种以数据为中心的范式能够实现高质量、有效的医疗生成式AI系统部署。
...
### [86] [FunReason-MT Technical Report: Overcoming the Complexity Barrier in Multi-Turn Function Calling](https://arxiv.org/abs/2510.24645)
...
TL;DR: FunReason-MT是一个用于多轮工具调用的数据合成框架，通过环境-API图交互、高级工具查询合成和引导迭代链来解决现有方法在真实世界环境中的局限性，在BFCL基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有数据合成方法（如随机环境采样或多智能体角色扮演）在真实世界环境中生成高质量多轮工具调用数据的能力不足，存在目标模型训练、工具架构隔离和多轮逻辑依赖三大挑战。

Method: FunReason-MT采用三种核心技术：1）环境-API图交互收集多样化高质量轨迹；2）高级工具查询合成简化复杂查询构建；3）引导迭代链生成复杂思维链。

Result: 在Berkeley Function-Calling Leaderboard (BFCLv3)上，基于FunReason-MT生成的4B模型在同类规模模型中达到最先进性能，优于大多数闭源模型。在BFCLv4上的进一步性能提升证实了其可靠性。

Conclusion: FunReason-MT为智能体学习提供了可靠且鲁棒的数据源，有效解决了多轮工具调用数据合成的复杂性障碍。
...
### [87] [Advancing site-specific disease and pest management in precision agriculture: From reasoning-driven foundation models to adaptive, feedback-based learning](https://arxiv.org/abs/2510.24650)
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习和深度学习在实时计算机视觉中的快速发展，作物定点病害管理技术从手工特征提取发展到大规模自动特征学习。基础模型的出现为作物病害数据集处理提供了全新方式，能够整合视觉和文本数据，解释症状文本，推理症状与管理的关系，并为种植者和教育者提供交互式问答支持。

Method: 通过文献综述方法，筛选了约40篇相关论文，重点分析基础模型（特别是大语言模型和视觉语言模型）在作物定点病害管理中的应用，包括它们在自适应学习、强化学习和数字孪生框架中的角色。

Result: 主要发现：(a) 基础模型在2023-24年文献激增；(b) 视觉语言模型发展快于大语言模型，发表量增加5-10倍；(c) 强化学习和自适应学习在智能喷洒中仍处于早期阶段；(d) 结合强化学习的数字孪生可以虚拟模拟定点喷洒；(e) 解决模拟到现实的差距对实际部署至关重要；(f) 人机协作仍然有限；(g) 具有实时反馈的多模态基础模型将推动下一代定点病害管理。

Conclusion: 基础模型正在改变作物定点病害管理的方式，特别是视觉语言模型展现出巨大潜力。虽然强化学习和自适应学习应用仍处于早期阶段，但结合数字孪生技术有望实现更精准的病害管理。未来需要重点解决模拟到现实的差距，并加强人机协作，特别是人类在环方法的发展。
...
### [88] [OrchDAG: Complex Tool Orchestration in Multi-Turn Interactions with Plan DAGs](https://arxiv.org/abs/2510.24663)
*Yifu Lu,Shengjie Liu,Li Dong*

Main category: cs.AI

TL;DR: OrchDAG是一个合成数据生成管道，将工具执行建模为具有可控复杂度的有向无环图（DAGs），用于评估多轮工具交互性能并提出基于图的奖励来增强RLVR训练。


<details>
  <summary>Details</summary>
Motivation: 现有工作大多忽视多轮工具交互的复杂性，需要更好的基准和训练方法来处理复杂的工具使用场景。

Method: 引入OrchDAG合成数据生成管道，将工具执行建模为DAGs，并提出基于图的奖励机制结合GRPO风格算法进行训练。

Result: 实验表明该数据集提供了具有挑战性但可解决的基准，所提出的奖励机制在结合GRPO风格算法时表现有效。

Conclusion: 在多轮工具使用中，利用拓扑结构和数据复杂性至关重要，图基奖励与GRPO算法的结合能有效提升性能。
...
### [89] [Bridging Tool Dependencies and Domain Knowledge: A Graph-Based Framework for In-Context Planning](https://arxiv.org/abs/2510.24690)
*Shengjie Liu,Li Dong,Zhenyu Zhang*

Main category: cs.AI

TL;DR: 提出了一个通过构建工具和文档知识图谱来增强示例工件生成的框架，通过深度-稀疏集成策略将工具依赖关系与程序知识对齐，有效提升计划生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在工具增强推理和规划中未能充分利用工具与文档之间的依赖关系，限制了示例工件的生成质量。

Method: 构建工具知识图谱和文档知识图谱，采用深度-稀疏集成策略融合两者，将结构化的工具依赖关系与程序知识对齐。

Result: 实验表明该统一框架能有效建模工具交互，显著改善计划生成效果。

Conclusion: 将工具图谱与领域知识图谱连接对于工具增强推理和规划具有重要价值。
...
### [90] [BitSkip: An Empirical Analysis of Quantization and Early Exit Composition](https://arxiv.org/abs/2510.23766)
...
Method: 引入BitSkip混合架构框架，系统探索量化与Hadamard变换的相互作用，包括8位量化模型和Hadamard增强版本。

Result: BitSkip-V1（8位量化无Hadamard变换）在困惑度上表现最佳（1.13 vs 1.19），优于4位和Hadamard增强版本；Hadamard变换即使在8位精度下也会导致性能灾难性下降37,000%；BitSkip-V1在第18层提供最佳早期退出特性，实现32.5%速度增益和仅4%质量损失。

Conclusion: 简单的8位量化模型在效率和质量平衡方面优于更复杂的方法，Hadamard变换在量化环境中存在根本性的训练不稳定性问题。
...
### [91] [Beyond Understanding: Evaluating the Pragmatic Gap in LLMs' Cultural Processing of Figurative Language](https://arxiv.org/abs/2510.23828)
*Mena Attia,Aashiq Muhamed,Mai Alkhamissi,Thamar Solorio,Mona Diab*

Main category: cs.CL

TL;DR: 本文评估了大型语言模型处理文化背景语言的能力，特别是理解和实用化使用体现当地知识和文化细微差别的比喻表达。研究发现LLMs在阿拉伯语谚语、埃及阿拉伯语习语和英语谚语的理解上存在显著差异，模型在实用化使用和内涵解释方面面临挑战。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型处理文化背景语言的能力，特别是理解和实用化使用体现当地知识和文化细微差别的比喻表达，以比喻语言作为文化推理的诊断工具。

Method: 设计了针对上下文理解、实用化使用和内涵解释的评估任务，在阿拉伯语和英语中评估了22个开源和闭源LLMs，使用埃及阿拉伯语习语、多方言阿拉伯语谚语和英语谚语作为测试材料。

Result: 结果显示一致性的层次结构：阿拉伯语谚语的平均准确率比英语谚语低4.29%，埃及习语的表现比阿拉伯语谚语低10.28%。在实用化使用任务中，准确率相对于理解下降了14.07%，但提供上下文习语句子可将准确率提高10.66%。模型在内涵意义理解上也存在困难，最高仅达到85.58%的人类标注者一致性。

Conclusion: 比喻语言是文化推理的有效诊断工具：虽然LLMs通常能够解释比喻意义，但在适当使用方面面临挑战。为支持未来研究，发布了首个用于比喻理解和实用化使用评估的埃及阿拉伯语习语数据集Kinayat。
...
### [92] [How Pragmatics Shape Articulation: A Computational Case Study in STEM ASL Discourse](https://arxiv.org/abs/2510.23842)
*Saki Imai,Lee Kezar,Laurel Aichler,Mert Inan,Erin Walker,Alicia Wooten,Lorna Quandt,Malihe Alikhani*

Main category: cs.CL

TL;DR: 该研究收集了美国手语STEM对话的动作捕捉数据集，分析了对话中手语的时空变化特征，发现对话手语比孤立手语持续时间短24.6%-44.6%，并评估了手语嵌入模型识别STEM手语的能力。


<details>
  <summary>Details</summary>
Motivation: 现有手语模型主要基于翻译者或孤立词汇数据训练，忽略了自然对话中的变异性。人类交流会根据情境和对话者动态调整，特别是在教育环境中，教师和学生会使用新颖词汇。

Method: 收集ASL STEM对话的动作捕捉数据集，使用连续运动学特征分析对话特定的趋同效应与个体努力减少，比较对话、独白和翻译文章中的手语差异。

Result: 对话手语比孤立手语持续时间显著缩短24.6%-44.6%，这种减少在独白环境中不存在。通过评估手语嵌入模型，能够识别STEM手语并近似参与者随时间趋同的程度。

Conclusion: 该研究通过连接语言分析和计算建模，理解语用学如何塑造手语发音及其在手语技术中的表示，为开发更自然的手语技术提供了基础。
...
### [93] [CRADLE Bench: A Clinician-Annotated Benchmark for Multi-Faceted Mental Health Crisis and Safety Risk Detection](https://arxiv.org/abs/2510.23845)
*Grace Byun,Rebecca Lipschutz,Sean T. Minton,Abigail Lott,Jinho D. Choi*

Main category: cs.CL

TL;DR: CRADLE BENCH是一个用于心理健康危机检测的基准测试，涵盖7种临床标准定义的危险类型，包含600个临床医生标注的评估样本和420个开发样本，以及约4K个自动标注的训练样本。
...
Method: 使用多数投票集成多个语言模型自动标注约4K个训练样本，在共识和一致同意标准下微调六个危机检测模型。

Result: 多数投票集成方法显著优于单模型标注，提供了在不同一致性标准下训练的互补模型。

Conclusion: CRADLE BENCH是首个包含时间标签的多方面危机检测基准，为心理健康危机检测提供了全面的评估框架。
...
### [94] [Temporal Blindness in Multi-Turn LLM Agents: Misaligned Tool Use vs. Human Time Perception](https://arxiv.org/abs/2510.23853)
*Yize Cheng,Arshia Soltani Moakhar,Chenrui Fan,Kazem Faghih,Parsa Hosseini,Wenxiao Wang,Soheil Feizi*

Main category: cs.CL

TL;DR: 该论文研究了大型语言模型在动态环境中的时间盲区问题，提出了TicToc-v1测试集来评估模型的时间感知能力，发现当前模型在时间敏感任务中表现不佳，需要专门的后训练对齐。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多轮对话环境中存在时间盲区，无法感知消息之间的实际时间间隔，导致工具调用决策失误——要么过度依赖旧上下文而跳过必要工具调用，要么不必要地重复工具调用。

Method: 引入TicToc-v1测试集，包含34个不同时间敏感度的场景，在对话消息中增加时间戳信息，收集人类偏好数据，评估LLM工具调用决策与人类偏好的对齐程度。

Result: 无时间信息时，大多数模型表现仅略优于随机猜测，最佳对齐率约60%；添加时间戳后略有改善，特别是对较大模型，但改善有限，峰值约65%；基于提示的对齐方法效果有限。

Conclusion: 需要专门的后训练对齐来使多轮LLM工具使用与人类时间感知保持一致，当前模型在时间敏感任务中的表现仍有待提升。
...
### [95] [Language Models for Longitudinal Clinical Prediction](https://arxiv.org/abs/2510.23884)
*Tananun Songdechakraiwut,Michael Lutz*

Main category: cs.CL

TL;DR: 提出一种轻量级框架，无需微调即可将冻结的大型语言模型应用于纵向临床数据分析，在神经心理学评估中实现准确预测，对阿尔茨海默病早期监测具有潜力。


<details>
  <summary>Details</summary>
Motivation: 探索如何利用冻结的大型语言模型分析纵向临床数据，解决传统方法需要大量训练数据和模型微调的问题，为阿尔茨海默病等疾病的早期监测提供更高效解决方案。

Method: 开发轻量级框架，在语言模型空间中整合患者历史记录和上下文信息，直接生成预测结果，无需对预训练模型进行微调。

Result: 在神经心理学评估任务中，即使使用极少的训练数据，该方法也能实现准确可靠的性能表现。

Conclusion: 该框架展示了利用冻结语言模型分析纵向临床数据的可行性，为阿尔茨海默病早期监测提供了有前景的工具，具有数据效率高、无需微调的优势。
...
### [96] [AfriMTEB and AfriE5: Benchmarking and Adapting Text Embedding Models for African Languages](https://arxiv.org/abs/2510.23896)
*Kosei Uemura,Miaoran Zhang,David Ifeoluwa Adelani*

Main category: cs.CL

TL;DR: 本文介绍了AfriMTEB基准测试和AfriE5嵌入模型，旨在解决非洲语言在文本嵌入评估中的代表性不足问题。AfriMTEB覆盖59种语言、14个任务和38个数据集，AfriE5通过跨语言对比蒸馏在非洲语言上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 非洲语言在现有的多语言文本嵌入基准测试（如MMTEB）中代表性严重不足，现有任务往往是从翻译基准（如FLORES聚类或SIB-200）重新调整用途而来，无法充分评估非洲语言的文本嵌入能力。

Method: 1) 构建AfriMTEB基准测试，扩展MMTEB覆盖59种非洲语言、14个任务和38个数据集，包括6个新数据集；2) 开发AfriE5模型，通过跨语言对比蒸馏方法将指令调优的mE5模型适配到非洲语言。

Result: AfriE5在非洲语言文本嵌入任务上实现了最先进的性能，超越了Gemini-Embeddings和mE5等强基线模型。新添加的数据集覆盖14到56种非洲语言，并引入了仇恨言论检测、意图检测和情感分类等全新任务类型。

Conclusion: AfriMTEB基准测试和AfriE5模型有效解决了非洲语言在文本嵌入评估中的代表性不足问题，为非洲语言的NLP研究提供了重要的基础设施，同时证明了跨语言对比蒸馏方法在适配多语言嵌入模型方面的有效性。
...
### [97] [Breaking the Benchmark: Revealing LLM Bias via Minimal Contextual Augmentation](https://arxiv.org/abs/2510.23921)
*Kaveh Eskandari Miandoab,Mahammed Kamruzzaman,Arshia Gharooni,Gene Louis Kim,Vasanth Sarathy,Ninareh Mehrabi*

Main category: cs.CL

TL;DR: 该论文提出了一个新颖的增强框架，通过三个即插即用步骤来评估大型语言模型在公平性基准测试中的表现。研究发现LLMs对输入扰动很敏感，更容易表现出刻板印象偏见，特别是对于文献研究较少的社区。


<details>
  <summary>Details</summary>
Motivation: 尽管已有方法试图避免在决策中使用刻板印象信息，但现有的偏见对齐方法存在脆弱性。需要开发更稳健的框架来评估LLMs的公平性表现。

Method: 提出了一个包含三个即插即用步骤的增强框架，适用于多个公平性评估基准。通过在偏见问答基准数据集(BBQ)上应用增强技术来测试LLMs。

Result: 研究发现包括最先进的开源和闭源模型在内的LLMs都对输入扰动很敏感，更可能表现出刻板印象行为。模型对于文献研究较少的社区更容易产生偏见行为。

Conclusion: 需要将公平性和安全性研究扩展到更多样化的社区，以解决LLMs在公平性评估中的脆弱性问题。
...
### [98] [Agent-based Automated Claim Matching with Instruction-following LLMs](https://arxiv.org/abs/2510.23924)
*Dina Pisarevskaya,Arkaitz Zubiaga*

Main category: cs.CL

TL;DR: 提出了一种基于智能体的自动声明匹配方法，使用指令跟随LLMs构建两步流水线：首先生成提示，然后进行二元分类的声明匹配。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在声明匹配任务中的潜力，特别是如何通过LLM生成的提示超越人工提示的性能，并研究计算资源优化策略。

Method: 采用两步流水线方法：1）使用LLMs生成提示；2）使用LLMs进行二元分类的声明匹配任务。支持不同LLMs分别用于提示生成和声明匹配步骤。

Result: LLM生成的提示可以超越最先进的人工生成提示性能；较小的LLMs在生成过程中可以达到与较大LLMs相当的效果，节省计算资源；使用不同LLMs分别处理流水线步骤是有效的。

Conclusion: 该方法不仅提升了声明匹配的性能，还揭示了LLMs对声明匹配任务的理解，为计算资源优化提供了可行方案。
...
### [99] [Auto prompting without training labels: An LLM cascade for product quality assessment in e-commerce catalogs](https://arxiv.org/abs/2510.23941)
...
Method: 使用基于人工编写提示种子的级联方法，自动生成和优化指令以满足目录特定要求，无需训练标签或模型微调。

Result: 相比传统思维链提示，精度和召回率提升8-10%，专家工作量从每属性5.1小时减少到3分钟（减少99%），并在五种语言和多个质量评估任务中保持性能增益。

Conclusion: 该自动提示级联系统有效连接了通用语言理解和领域特定知识，在复杂工业目录中实现了高效、可扩展的产品质量评估。
...
### [100] [Leveraging LLMs for Early Alzheimer's Prediction](https://arxiv.org/abs/2510.23946)
*Tananun Songdechakraiwut*

Main category: cs.CL

TL;DR: 提出了一个基于连接组信息的LLM框架，将动态fMRI连接性编码为时间序列，应用鲁棒归一化，并将这些数据映射到适合冻结预训练LLM的表示形式，用于临床预测。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够敏感检测早期阿尔茨海默病的方法，实现低于临床公认误差率的预测，为及时干预提供可能。

Method: 使用连接组信息化的LLM框架，将动态fMRI连接性编码为时间序列，进行鲁棒归一化处理，然后映射到适合冻结预训练LLM的表示形式。

Result: 在早期阿尔茨海默病检测中实现了敏感预测，错误率显著低于临床公认的误差范围。
...
### [101] [Uncovering the Potential Risks in Unlearning: Danger of English-only Unlearning in Multilingual LLMs](https://arxiv.org/abs/2510.23949)
*Kyomin Hwang,Hyeonjin Kim,Seungyeon Kim,Sunghyun Wee,Nojun Kwak*

Main category: cs.CL

TL;DR: 本文研究了多语言大语言模型在知识遗忘过程中的语言混淆现象，发现仅使用英语数据进行遗忘在多语言LLMs中不足，并提出N-Mix评分来量化这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注性能导向的分析，而忽视了多语言LLMs在完全微调后使用平行多语言数据集进行遗忘时出现的语言混淆问题，这导致基于参考的评估指标失效。

Method: 通过三个步骤：(1)引入基于N-gram的语言混合评分(N-Mix)来量化语言混淆；(2)证明当N-Mix评分高时，基于参考的指标会产生假阴性；(3)提出需要能够直接评估生成句子内容的语义型评估指标。

Result: 研究发现语言混淆在多语言LLMs中普遍且一致存在，基于参考的评估指标在语言混淆严重时无法准确评估遗忘效果。

Conclusion: 需要开发新的语义型评估指标来直接评估生成内容，以解决多语言LLMs知识遗忘中的语言混淆问题。
...
### [102] [M-Eval: A Heterogeneity-Based Framework for Multi-evidence Validation in Medical RAG Systems](https://arxiv.org/abs/2510.23995)
*Mengzhou Sun,Sendong Zhao,Jianyu Chen,Haochun Wang,Bin Qin*

Main category: cs.CL

TL;DR: 提出M-Eval方法，基于循证医学的异质性分析来检测RAG系统在医疗问答中的事实错误，通过多源证据验证回答准确性，提升LLM可靠性。
...
### [103] [PICOs-RAG: PICO-supported Query Rewriting for Retrieval-Augmented Generation in Evidence-Based Medicine](https://arxiv.org/abs/2510.23998)
*Mengzhou Sun,Sendong Zhao,Jianyu Chen,Bin Qin*

Main category: cs.CL

TL;DR: 本文提出了PICOs-RAG方法，通过将用户查询扩展和规范化为专业格式，并使用EBM中的PICO搜索策略提取关键信息，显著提升了检索效率和相关性，相比基线方法有8.8%的改进。


<details>
  <summary>Details</summary>
Motivation: 当前基于检索增强生成的EBM研究在处理复杂临床查询时存在不足，当查询缺乏信息或使用不精确语言时，模型可能检索到不相关证据并生成无用的回答。

Method: 提出PICOs-RAG方法，将用户查询扩展和规范化为专业格式，使用EBM中的PICO搜索策略提取最重要的检索信息。

Result: 该方法显著提升了检索效率和相关性，相比基线方法有8.8%的改进。
...
### [104] [META-RAG: Meta-Analysis-Inspired Evidence-Re-Ranking Method for Retrieval-Augmented Generation in Evidence-Based Medicine](https://arxiv.org/abs/2510.24003)
*Mengzhou Sun,Sendong Zhao,Jianyu Chen,Haochun Wang,Bin Qin*

Main category: cs.CL

TL;DR: 本文提出了一种基于元分析启发的医学证据重排序和过滤方法，通过可靠性分析、异质性分析和外推分析来筛选高质量医学证据，使RAG在循证医学任务中提取更可靠的证据，实验显示准确率提升达11.4%。
...
Method: 采用元分析思想，结合可靠性分析、异质性分析和外推分析等多种EBM方法，对医学证据进行重排序和过滤，筛选出最佳证据供LLMs诊断使用。

Result: 实验结果显示该方法能显著提升准确率（最高达11.4%），成功从PubMed数据集中提取更高质量和更可靠的医学证据。

Conclusion: 该方法能减少错误知识注入到响应中，帮助用户获得更有效的回复，提高了RAG在循证医学应用中的实用性。
...
### [105] [Teaching LLMs to Abstain via Fine-Grained Semantic Confidence Reward](https://arxiv.org/abs/2510.24020)
*Hao An,Yang Xu*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用粗粒度信号（如整体置信度或不确定性分数）来指导LLMs拒绝回答超出知识范围的问题，这可能导致模型对自身知识边界认识不精确。

Method: 采用强化学习框架，通过采样多个候选答案并进行语义聚类，训练LLMs保留高置信度聚类中的答案，丢弃低置信度聚类中的答案，从而实现准确的事后拒绝。

Result: 该方法在领域内和领域外基准测试中显著提高了可靠性。

Conclusion: 提出的细粒度语义置信度奖励方法能够更精确地引导LLMs识别知识边界，有效缓解幻觉问题，提升模型部署的可靠性。
...
### [106] [SpecKD: Speculative Decoding for Effective Knowledge Distillation of LLMs](https://arxiv.org/abs/2510.24021)
*Haiduo Huang,Jiangcheng Song,Yadong Zhang,Pengju Ren*

Main category: cs.CL

TL;DR: 提出Speculative Knowledge Distillation (SpecKD)框架，通过动态token级门控机制选择性应用蒸馏损失，避免学生模型学习教师模型的不确定预测，显著提升知识蒸馏效果。
...
Method: 提出SpecKD框架，采用受推测解码'提议-验证'范式启发的动态token级门控机制：在每个步骤中验证学生token提议与教师分布的匹配度，仅对'接受'的token应用蒸馏损失，'拒绝'的token被屏蔽。

Result: 在多样化文本生成任务上的广泛实验表明，SpecKD持续且显著优于强基线知识蒸馏方法，带来更稳定的训练和更强大的学生模型，并达到最先进的结果。

Conclusion: SpecKD通过选择性应用蒸馏损失有效解决了传统知识蒸馏中的噪声问题，是提升知识蒸馏效果的有效框架。
...
### [107] [Success and Cost Elicit Convention Formation for Efficient Communication](https://arxiv.org/abs/2510.24023)
...
<details>
  <summary>Details</summary>
Motivation: 人类通过共享对话上下文能够形成临时语言约定，从而实现更高效、成功的沟通。研究旨在让大型多模态模型也能形成这种约定，实现高效通信。

Method: 使用模拟参考游戏在模型之间进行训练，无需额外人工数据。在涉及照片和七巧板图像的重复参考游戏中实施该方法。

Result: 模型能够与人进行高效沟通：在交互过程中消息长度减少高达41%，成功率提高15%。人类听众与形成约定的模型交互时响应更快。

Conclusion: 仅基于成功或成本进行训练是不够的，两者都是引发约定形成的必要条件。该方法为模型形成约定提供了有效途径。
...
### [108] [Pie: A Programmable Serving System for Emerging LLM Applications](https://arxiv.org/abs/2510.24051)
*In Gim,Zhiyao Ma,Seung-seob Lee,Lin Zhong*

Main category: cs.CL

TL;DR: Pie是一个可编程的LLM服务系统，通过将传统生成循环分解为细粒度服务处理器，让用户程序控制生成过程，在保持标准任务性能的同时显著提升智能体工作流的效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM服务系统基于单一令牌生成循环构建，难以适应多样化推理策略和智能体工作流的需求，限制了应用的灵活性和优化空间。

Method: 将传统生成循环分解为细粒度服务处理器，通过用户提供的inferlet程序控制生成过程，使用WebAssembly执行inferlet以提供轻量级沙箱环境。

Result: 在标准任务上性能与最先进系统相当（延迟开销3-12%），在智能体工作流上通过应用特定优化实现了1.3-3.4倍的延迟和吞吐量提升。

Conclusion: Pie通过可编程架构实现了LLM服务系统的灵活性和效率平衡，为复杂应用场景提供了更好的支持。
...
### [109] [Challenging Multilingual LLMs: A New Taxonomy and Benchmark for Unraveling Hallucination in Translation](https://arxiv.org/abs/2510.24073)
*Xinwei Wu,Heng Liu,Jiang Zhou,Xiaohu Zhao,Linlong Xu,Longyue Wang,Weihua Luo,Kaifu Zhang*

Main category: cs.CL

TL;DR: 本文提出了HalloMTBench，一个多语言幻觉检测基准，用于诊断大型语言模型在机器翻译中的幻觉问题，涵盖11种英语到其他语言的方向，包含5,435个高质量实例。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在机器翻译方面取得进展，但仍容易产生幻觉。现有的机器翻译基准无法充分暴露多语言LLM中的失败情况，因此需要专门的诊断框架来揭示幻觉问题。

Method: 提出了一个诊断框架，将幻觉分为指令脱离和源文脱离两类。通过4个前沿LLM生成候选翻译，使用LLM评委集合和专家验证进行筛选，最终构建包含5,435个实例的多语言基准。

Result: 评估了17个LLM，发现了独特的"幻觉触发器"，包括模型规模、源文长度敏感性、语言偏见以及强化学习放大的语言混合等失败模式。

Conclusion: HalloMTBench为诊断LLM翻译失败提供了一个前瞻性的测试平台，能够有效揭示多语言LLM中的幻觉问题。
...
### [110] [Global PIQA: Evaluating Physical Commonsense Reasoning Across 100+ Languages and Cultures](https://arxiv.org/abs/2510.24081)
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏覆盖多种语言和文化的特定文化评估基准，需要构建一个能够评估LLM在不同文化背景下常识推理能力的基准。

Method: 采用参与式方法，由来自65个国家的335名研究人员手工构建，涵盖116种语言变体、14个语系和23种文字系统，包含平行和非平行数据集。
...
### [111] [RegSpeech12: A Regional Corpus of Bengali Spontaneous Speech Across Dialects](https://arxiv.org/abs/2510.24096)
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉语具有丰富的方言多样性，但对其计算处理的研究有限。本研究旨在记录和分析这些方言的特性，开发适合地区变体的计算模型，以促进方言多样性的保护和包容性数字工具的发展。

Method: 研究通过文档化和分析孟加拉语方言的语音和形态学特性，探索构建针对地区变体的自动语音识别系统的可行性，并创建了公开可用的数据集。

Result: 研究记录了孟加拉语方言的语音和形态学特性，构建了计算模型特别是自动语音识别系统的可行性得到验证，并发布了用于公共使用的数据集。

Conclusion: 该研究为孟加拉语方言的计算处理提供了系统研究基础，开发的自动语音识别系统在虚拟助手等语言技术中具有应用潜力，有助于保护方言多样性和推进包容性数字工具的发展。
...
### [112] [Squrve: A Unified and Modular Framework for Complex Real-World Text-to-SQL Tasks](https://arxiv.org/abs/2510.24102)
...
<details>
  <summary>Details</summary>
Motivation: 尽管Text-to-SQL技术在学术上取得了显著进展，但在实际系统中部署仍面临挑战，主要由于缺乏集成工具。
...
### [113] [Reinforcement Learning for Long-Horizon Multi-Turn Search Agents](https://arxiv.org/abs/2510.24126)
*Vivek Kalyan,Martin Andrews*

Main category: cs.CL

TL;DR: 使用强化学习训练的140亿参数模型在法律文档搜索基准测试中表现优于前沿模型（85% vs 78%准确率），研究表明多轮交互能显著提升智能体性能。


<details>
  <summary>Details</summary>
Motivation: 探索强化学习如何通过从经验中学习来显著提升大型语言模型智能体在复杂任务中的能力，超越基于提示的方法。

Method: 采用强化学习训练140亿参数模型，并在法律文档搜索基准上进行实验，同时研究训练和测试时的轮次限制对性能的影响。

Result: RL训练的模型在准确率上显著优于前沿模型（85% vs 78%），且多轮交互能带来更好的性能表现。

Conclusion: 强化学习能够有效提升LLM智能体的能力，多轮交互对任务解决至关重要，RL方法在复杂任务中具有显著优势。
...
### [114] [Beyond Line-Level Filtering for the Pretraining Corpora of LLMs](https://arxiv.org/abs/2510.24139)
...
### [115] [Ko-MuSR: A Multistep Soft Reasoning Benchmark for LLMs Capable of Understanding Korean](https://arxiv.org/abs/2510.24150)
*Chanwoo Park,Suyoung Park,JiA Kang,Jongyeon Park,Sangho Kim,Hyunji M. Park,Sumin Bae,Mingyu Kang,Jaejin Lee*

Main category: cs.CL

TL;DR: Ko-MuSR是首个全面评估韩语长叙事中多步软推理能力的基准，通过人类标注验证逻辑一致性和可回答性，评估显示多语言模型在韩语推理任务中优于韩语专用模型，提示策略能显著提升准确率接近人类水平。


<details>
  <summary>Details</summary>
Motivation: 构建首个韩语多步软推理基准，最小化数据污染，为韩语NLP提供系统评估长上下文推理和提示策略的基础。

Method: 基于MuSR构建，包含完全韩语的叙事、推理链和多选题，由人类标注验证逻辑一致性和可回答性，评估了四种大语言模型（两种多语言、两种韩语专用），并设计了结合少样本示例、推理轨迹和任务特定提示的提示策略。

Result: 多语言模型在韩语推理任务中表现优于韩语专用模型，表明推理能力具有跨语言泛化性；精心设计的提示策略显著提升准确率，接近人类水平性能。

Conclusion: Ko-MuSR为推进韩语NLP提供了坚实基础，支持系统评估长上下文推理和提示策略，揭示了推理能力的跨语言泛化特性。
...
### [116] [MuSaG: A Multimodal German Sarcasm Dataset with Full-Modal Annotations](https://arxiv.org/abs/2510.24178)
*Aaron Scott,Maike Züfle,Jan Niehues*

Main category: cs.CL

TL;DR: 本文介绍了MuSaG，这是首个德语多模态讽刺检测数据集，包含来自德国电视节目的33分钟人工选择和标注的语句，涵盖文本、音频和视频三种模态。研究评估了9种开源和商业模型，发现人类在对话环境中主要依赖音频线索，而模型在文本上表现最佳，揭示了当前多模态模型与现实场景的差距。
...
Method: 创建了MuSaG数据集，包含33分钟来自德国电视节目的手动选择和人工标注的语句，每个实例提供对齐的文本、音频和视频模态，分别由人类标注。评估了9种开源和商业模型，涵盖文本、音频、视觉和多模态架构，并将其性能与人类标注进行比较。

Result: 研究结果显示，人类在对话环境中主要依赖音频线索，而模型在文本上表现最佳。这突显了当前多模态模型与现实场景之间的差距。

Conclusion: MuSaG数据集可用于开发更适合现实场景的模型，并支持未来关于多模态讽刺检测和人机对齐的研究。数据集已公开发布以促进相关研究。
...
### [117] [Exploring the Influence of Relevant Knowledge for Natural Language Generation Interpretability](https://arxiv.org/abs/2510.24179)
*Iván Martínez-Murillo,Paloma Moreda,Elena Lloret*

Main category: cs.CL

TL;DR: 该研究通过创建KITGI基准测试，探索外部知识整合对自然语言生成的影响，发现在常识生成任务中，相关外部知识对保持生成句子的连贯性和概念覆盖率至关重要。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解外部知识在自然语言生成中的作用，特别是常识生成任务中知识整合的重要性。

Method: 采用三阶段方法：(1)识别并移除关键知识，(2)重新生成句子，(3)人工评估输出的常识合理性和概念覆盖率。使用T5-Large模型在CommonGen数据集基础上创建KITGI基准。

Result: 完整知识条件下生成的句子在正确性方面达到91%，而过滤掉高度相关关系后性能急剧下降至6%。

Conclusion: 相关外部知识对于保持NLG的连贯性和概念覆盖率至关重要，需要设计可解释的知识增强NLG系统，并开发超越表面指标的评价框架。
...
### [118] [Beyond Neural Incompatibility: Easing Cross-Scale Knowledge Transfer in Large Language Models through Latent Semantic Alignment](https://arxiv.org/abs/2510.24208)
*Jian Gu,Aldeida Aleti,Chunyang Chen,Hongyu Zhang*

Main category: cs.CL

TL;DR: 本文提出了一种基于潜在空间语义对齐的LLM跨规模知识迁移方法，通过激活值而非层参数作为知识迁移媒介，解决了不同规模LLM之间的神经不兼容问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法由于LLM不同规模间的架构和参数差异（神经不兼容），直接重用层参数进行知识迁移效果受限，需要探索更有效的跨规模知识迁移方法。

Method: 采用激活值作为层间知识迁移媒介，利用潜在空间中的语义对齐来实现跨规模知识迁移，而非直接使用层参数。

Result: 在四个基准测试上验证了方法的有效性，相比先前工作表现更好，能更好地在不同规模模型间对齐模型行为。

Conclusion: 潜在空间语义对齐是实现LLM跨规模知识迁移的关键前提，该方法为理解潜在语义对齐的本质提供了新见解。
...
### [119] [HACK: Hallucinations Along Certainty and Knowledge Axes](https://arxiv.org/abs/2510.24222)
...
TL;DR: 该论文提出了一个基于知识和确定性两个维度的幻觉分类框架，通过模型特定的数据集构建过程来区分不同类型的幻觉，并验证了基于知识轴的分类有效性。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常根据幻觉的外部特性而非LLMs的内部特性进行分类，这种外部关注忽视了幻觉可能需要基于其底层机制量身定制的缓解策略。

Method: 提出了一个沿知识和确定性两个轴分类幻觉的框架，包括模型特定的数据集构建过程，并应用转向缓解来验证知识轴分类。

Result: 验证了两种幻觉类型之间存在显著差异，分析了模型间不同的知识和幻觉模式，并发现某些缓解方法在关键案例上表现不佳。

Conclusion: 研究强调了在幻觉分析中同时考虑知识和确定性的重要性，呼吁采用考虑幻觉底层因素的针对性缓解方法。
...
### [120] [Towards Transparent Reasoning: What Drives Faithfulness in Large Language Models?](https://arxiv.org/abs/2510.24236)
*Teague McMillan,Gabriele Dominici,Martin Gjoreski,Marc Langheinrich*

Main category: cs.CL

TL;DR: 该研究探讨了大型语言模型在医疗等敏感领域中解释的忠实性问题，分析了推理和训练阶段的选择如何影响解释的忠实性，重点关注部署时可控制的因素。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生不能忠实反映其预测驱动因素的解释，这在医疗环境中尤其成问题，因为省略关键临床线索或掩盖虚假捷径的解释会破坏临床医生的信任并导致不安全的决策支持。

Method: 评估了三种LLM（GPT-4.1-mini、LLaMA 70B、LLaMA 8B）在两个数据集（BBQ和MedQA）上的表现，并操纵了少样本示例的数量和类型、提示策略以及训练过程。

Result: 结果显示：(i) 少样本示例的数量和质量显著影响模型的忠实性；(ii) 忠实性对提示设计敏感；(iii) 指令调优阶段提高了MedQA上的忠实性测量。

Conclusion: 这些发现为在敏感领域中增强LLM的可解释性和可信度提供了策略见解。
...
### [121] [Abjad AI at NADI 2025: CATT-Whisper: Multimodal Diacritic Restoration Using Text and Speech Representations](https://arxiv.org/abs/2510.24247)
*Ahmad Ghannam,Naif Alharthi,Faris Alasmary,Kholood Al Tabash,Shouq Sadah,Lahouari Ghouti*

Main category: cs.CL

TL;DR: 本文提出了一种结合文本和语音信息的多模态方法来解决阿拉伯语方言句子的变音符号恢复任务，通过两种融合策略和随机禁用语音输入的训练方法，在开发集和测试集上取得了较低的词错误率和字符错误率。


<details>
  <summary>Details</summary>
Motivation: 解决阿拉伯语方言句子中的变音符号恢复问题，传统方法主要依赖文本信息，本文探索结合语音信息的多模态方法来提高恢复准确性。

Method: 使用CATT预训练模型的编码器处理文本模态，OpenAI Whisper基础模型的编码器处理语音模态。采用两种融合策略：早期融合（将语音token与文本token合并）和交叉注意力融合。在训练时随机禁用语音输入以提高模型鲁棒性。

Result: 在开发集上获得词错误率0.25和字符错误率0.9；在测试集上获得词错误率0.55和字符错误率0.13。

Conclusion: 提出的多模态方法在阿拉伯语方言变音符号恢复任务中表现良好，证明了结合语音信息可以有效提高变音符号恢复的准确性。
...
### [122] [Evaluating LLMs on Generating Age-Appropriate Child-Like Conversations](https://arxiv.org/abs/2510.24250)
*Syed Zohaib Hassan,Pål Halvorsen,Miriam S. Johnson,Pierre Lison*

Main category: cs.CL

TL;DR: 本研究比较了5种大型语言模型生成适合挪威5岁和9岁儿童对话的能力，发现大多数模型生成的语言比目标年龄组更复杂，GPT-4和NorBloom-7b表现相对较好。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型主要基于成人对话数据训练，在生成真实、儿童化对话方面面临挑战，特别是在挪威语等低资源语言中，缺乏全面的年龄适宜词汇资源。

Method: 通过盲评实验，由11位教育专业人士评估真实儿童访谈数据和LLM生成文本样本的真实性和发展适宜性，评估模型包括GPT-4、RUTER-LLAMA-2-13b、GPTSW、NorMistral-7b和NorBloom-7b。

Result: 评估者达到了强评分者间信度(ICC=0.75)，对5岁儿童的年龄预测准确率高于9岁儿童。大多数模型生成的语言被认为比目标年龄组更复杂，GPT-4和NorBloom-7b表现相对较好。

Conclusion: 研究结果凸显了为涉及儿童的专业应用开发LLM系统时面临的关键数据相关挑战，特别是在低资源语言环境中。
...
### [123] [MERGE: Minimal Expression-Replacement GEneralization Test for Natural Language Inference](https://arxiv.org/abs/2510.24295)
*Mădălina Zgreabăn,Tejaswini Deoskar,Lasha Abzianidze*

Main category: cs.CL

TL;DR: 提出MERGE方法自动生成自然语言推理问题的变体，通过替换开放类词汇但保持推理逻辑，评估模型在最小修改问题上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有NLI基准测试显示语言模型在推理任务中缺乏鲁棒性，但手动创建新基准成本高，自动生成高质量变体困难。

Method: 通过替换开放类词汇生成原始NLI问题的变体，同时保持底层推理逻辑不变，创建MERGE泛化测试。

Result: NLI模型在变体问题上的性能下降4-20%，表明即使在最小修改的问题上泛化能力也很低。分析了替换词类别、词概率和合理性对性能的影响。

Conclusion: 提出的MERGE方法能有效评估NLI模型的泛化能力，揭示了模型在推理任务中的脆弱性。
...
### [124] [Lookahead Tree-Based Rollouts for Enhanced Trajectory-Level Exploration in Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2510.24302)
*Shangyu Xing,Siyuan Wang,Chenyuan Yang,Xinyu Dai,Xiang Ren*

Main category: cs.CL

TL;DR: 本文提出了一种名为LATR的新策略，通过在不确定性高的生成步骤进行分支、前瞻模拟和剪枝，显著提升强化学习中轨迹多样性，从而加速策略学习并提高性能。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习管道中，组rollout期间采样的轨迹多样性有限，导致回报信号减弱，阻碍有效的策略学习。这种多样性不足主要源于token级随机采样，容易产生相似的推理路径。

Method: 提出LATR策略，包含三个迭代阶段：在高不确定性生成步骤进行分支、对每个新分支执行前瞻模拟、剪除模拟过程中表现出持续相似性的分支。
...
### [125] [Critique-RL: Training Language Models for Critiquing through Two-Stage Reinforcement Learning](https://arxiv.org/abs/2510.24320)
...
Method: 采用双玩家范式：演员生成响应，评论家提供反馈，演员根据反馈优化响应。使用两阶段优化策略：第一阶段通过基于规则的直接奖励信号增强评论家的判别能力；第二阶段引入基于演员优化的间接奖励来提升评论家的帮助性，同时通过正则化保持判别能力。

Result: 在各种任务和模型上的广泛实验表明，Critique-RRL带来显著的性能提升。例如，Qwen2.5-7B在域内任务上获得9.02%的提升，在域外任务上获得5.70%的提升。

Conclusion: Critique-RL是一种有效的无需强监督的批判语言模型训练方法，能够显著提升模型性能，具有广阔的应用前景。
...
### [126] [Beyond MCQ: An Open-Ended Arabic Cultural QA Benchmark with Dialect Variants](https://arxiv.org/abs/2510.24328)
*Hunzalah Hassan Bhatti,Firoj Alam*

Main category: cs.CL

TL;DR: 该研究提出了一种综合方法，将现代标准阿拉伯语多选题翻译成英语和多种阿拉伯语方言，转换为开放式问题，并评估多种LLM在两种设置下的表现，同时生成思维链推理来微调模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在回答日常问题时的表现存在语言差异，特别是在文化基础和方言内容方面，需要评估和改进模型在阿拉伯语方言上的表现。

Method: 将现代标准阿拉伯语多选题翻译成英语和多种阿拉伯语方言，转换为开放式问题，评估零样本和微调LLM在两种设置下的表现，生成思维链推理来微调模型进行逐步推理。

Result: 模型在阿拉伯语方言上表现不佳，阿拉伯语中心模型在多选题上表现良好但在开放式问题上表现不佳，思维链推理提高了判断正确性但n-gram指标表现不一。

Conclusion: 研究揭示了模型在文化基础和方言特定知识方面的持续差距，开发的公开数据集将支持文化和语言包容性评估的进一步研究。
...
### [127] [LongWeave: A Long-Form Generation Benchmark Bridging Real-World Relevance and Verifiability](https://arxiv.org/abs/2510.24345)
*Zikai Xiao,Fei Huang,Jianhong Tu,Jianhui Wei,Wen Ma,Yuxuan Zhou,Jian Wu,Bowen Yu,Zuozhu Liu,Junyang Lin*

Main category: cs.CL

TL;DR: LongWeave是一个新的长文本生成基准，通过约束验证评估(CoV-Eval)方法平衡真实世界场景和可验证性评估，涵盖7个任务，支持最长64K输入/8K输出，评估显示即使最先进的LLM在长文本生成中仍面临显著挑战。


<details>
  <summary>Details</summary>
Motivation: 现有长文本生成基准要么使用难以验证指标评估真实世界查询，要么使用忽略真实世界复杂性的合成设置，需要平衡真实性和可验证性的评估方法。

Method: 提出约束验证评估(CoV-Eval)方法：首先在真实场景中定义可验证目标，然后系统生成对应查询、文本材料和约束，确保任务既真实又可客观评估。

Result: 对23个LLM的评估表明，随着真实世界复杂性和输出长度的增加，即使是SOTA模型在长文本生成中也面临显著挑战。

Conclusion: LongWeave为长文本生成提供了更全面和严格的评估基准，揭示了当前LLM在复杂长文本生成任务中的局限性。
...
### [128] [Fine-tuning Large Language Models with Limited Data: A Survey and Practical Guide](https://arxiv.org/abs/2411.09539)
*Marton Szep,Daniel Rueckert,Rüdiger von Eisenhart-Rothe,Florian Hinterwimmer*

Main category: cs.CL

TL;DR: 本文系统综述了在数据稀缺场景下微调大语言模型的方法，包括参数高效微调、领域和跨语言适应、模型专业化策略以及偏好对齐方法，旨在为研究者和实践者提供在有限数据和资源条件下有效微调LLM的实用指南。


<details>
  <summary>Details</summary>
Motivation: 在低资源语言、专业领域和受限部署环境中，使用有限数据微调大语言模型面临实际挑战。虽然预训练LLM提供了坚实基础，但在数据稀缺条件下进行有效适应需要专注且高效的微调技术。

Method: 系统回顾参数高效微调技术以降低训练和部署成本，涵盖编码器和解码器模型的领域和跨语言适应方法，以及模型专业化策略。同时研究使用有限人类或合成反馈指导模型行为的偏好对齐方法，强调样本和计算效率。

Result: 通过分析揭示了经验权衡、选择标准和最佳实践，为根据任务约束（包括模型扩展、数据扩展和灾难性遗忘缓解）选择合适技术提供了指导。

Conclusion: 该综述旨在为研究者和实践者提供可操作的见解，帮助他们在数据和资源有限的情况下有效微调大语言模型。
...
### [129] [Comprehensive and Efficient Distillation for Lightweight Sentiment Analysis Models](https://arxiv.org/abs/2510.24425)
*Guangyu Xie,Yice Zhang,Jianzhu Bao,Qianlong Wang,Yang Sun,Bingbing Wang,Ruifeng Xu*

Main category: cs.CL

TL;DR: COMPEFFDIST是一个用于情感分析的综合高效蒸馏框架，通过属性自动指令构建和难度数据过滤模块，解决了手动指令多样性不足和大规模文本计算成本高的问题，使3B学生模型在大多数任务上达到20倍大教师模型的性能，数据效率提升10倍。
...
Method: 提出COMPEFFDIST框架，包含两个关键模块：属性自动指令构建和难度数据过滤，分别解决指令多样性不足和数据计算成本高的问题。

Result: 在多个模型系列（Llama-3、Qwen-3和Gemma-3）上应用该方法，3B学生模型在大多数任务上能够匹配20倍大教师模型的性能，数据效率显著提升，仅需10%的数据即可达到相同性能水平。

Conclusion: COMPEFFDIST框架有效解决了情感分析知识蒸馏中的指令多样性和计算效率问题，实现了高效的知识迁移和显著的数据效率提升。
...
### [130] [SynthWorlds: Controlled Parallel Worlds for Disentangling Reasoning and Knowledge in Language Models](https://arxiv.org/abs/2510.24427)
*Ken Gu,Advait Bhat,Mike A Merrill,Robert West,Xin Liu,Daniel McDuff,Tim Althoff*

Main category: cs.CL

TL;DR: SynthWorlds框架通过构建结构相同但知识内容不同的真实映射世界和合成映射世界，来分离语言模型的任务推理能力和事实知识记忆，从而更准确地评估模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以清晰区分语言模型的推理能力和事实知识记忆，因为基准测试表现往往反映的是事实回忆而非真正的推理。需要一种能够分离这两种能力的方法来准确评估模型的推理能力。

Method: 构建两个结构相同但知识内容不同的平行语料库：真实映射世界（模型可利用参数知识）和合成映射世界（参数知识无意义）。在这两个语料库上设计镜像任务（如多跳问答和页面导航），保持跨世界的推理难度一致。

Result: 实验发现存在持续的知识优势差距，即模型从记忆的参数世界知识中获得的性能提升。知识获取和整合机制可以减少但无法消除这一差距。

Conclusion: SynthWorlds提供了一个受控环境，能够以之前难以实现的方式评估语言模型，实现推理和记忆能力的精确可测试比较，为系统改进指明了方向。
...
### [131] [LuxIT: A Luxembourgish Instruction Tuning Dataset from Monolingual Seed Data](https://arxiv.org/abs/2510.24434)
*Julian Valline,Cedric Lothritz,Jordi Cabot*

Main category: cs.CL

TL;DR: 本文介绍了LuxIT，一个针对卢森堡语的新型单语指令调优数据集，旨在解决低资源语言环境下高质量训练数据不足的问题。通过使用DeepSeek-R1-0528从原生卢森堡语文本合成数据集，并采用LLM作为评判的质量保证流程。实验结果显示，在卢森堡语能力测试中，基于LuxIT微调的小型LLMs表现参差不齐。
...
Method: 1. 从原生卢森堡语文本语料库合成LuxIT数据集；2. 使用DeepSeek-R1-0528（因其在卢森堡语方面的熟练表现而被选择）；3. 采用LLM作为评判的质量保证流程；4. 在LuxIT上微调多个小型LLMs。
...
### [132] [SPARTA: Evaluating Reasoning Segmentation Robustness through Black-Box Adversarial Paraphrasing in Text Autoencoder Latent Space](https://arxiv.org/abs/2510.24446)
...
TL;DR: 本文提出了一种新的对抗性转述任务，旨在生成语法正确但会降低多模态大语言模型分割性能的语义等价转述，并开发了SPARTA方法来有效实现这一目标。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注图像输入的扰动，而语义等价的文本转述在真实应用中至关重要，但尚未得到充分探索。用户可能以不同方式表达相同意图，因此需要评估模型对此类转述的鲁棒性。

Method: 提出了SPARTA方法——一种在黑盒、句子级别的优化方法，在文本自编码器的低维语义潜在空间中操作，通过强化学习进行引导，生成对抗性转述。

Result: SPARTA在ReasonSeg和LLMSeg-40k数据集上取得了显著更高的成功率，比先前方法高出最多2倍，成功揭示了先进推理分割模型对对抗性转述的脆弱性。

Conclusion: 即使受到严格的语义和语法约束，先进的推理分割模型仍然容易受到对抗性转述攻击，这凸显了开发更鲁棒的多模态模型的重要性。
...
### [133] [Charting the European LLM Benchmarking Landscape: A New Taxonomy and a Set of Best Practices](https://arxiv.org/abs/2510.24450)
*Špela Vintar,Taja Kuzman Pungeršek,Mojca Brglez,Nikola Ljubešić*

Main category: cs.CL

TL;DR: 本文提出了针对多语言或非英语使用场景的基准测试分类法，并为欧洲语言基准测试的协调发展制定了一套最佳实践和质量标准，强调评估方法需要更高的语言和文化敏感性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型能力的不断增强，现有的基准测试主要针对英语，而在非英语语言中使用和评估LLMs仍然是一个研究较少的领域，需要专门针对多语言场景的评估框架。

Method: 首先回顾了LLM基准测试的最新发展，然后提出了专门针对多语言或非英语使用场景的基准测试分类法，并制定了一套最佳实践和质量标准。

Result: 提出了新的基准测试分类法，强调评估方法需要更高的语言和文化敏感性，为欧洲语言基准测试的协调发展提供了具体建议。

Conclusion: 需要更加协调地开发针对多语言环境的基准测试，特别是要提高评估方法的语言和文化敏感性，以更好地适应非英语语言的使用场景。
...
### [134] [Iterative Critique-Refine Framework for Enhancing LLM Personalization](https://arxiv.org/abs/2510.24469)
...
<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法在生成个性化文本时，往往在语调、主题或风格上产生偏离，需要更好的方法来确保生成的文本与目标用户的风格、语调和主题焦点保持一致。

Method: 提出PerFine框架，包含生成器和批评器两个LLM组件，通过迭代的配置文件基础反馈机制，结合淘汰策略保留更强的草稿版本，并研究Best-of-N和主题提取等推理时策略来平衡质量和效率。

Result: 在Yelp、Goodreads和Amazon数据集上，PerFine相比PGraphRAG在GEval指标上提升了7-13%，经过3-5次细化迭代后效果稳定提升，且随着批评器规模的增大具有良好的可扩展性。

Conclusion: 基于配置文件的后期反馈为个性化LLM生成提供了一个强大且无需训练、模型无关的范式。
...
### [135] [Mitigating Hallucination in Large Language Models (LLMs): An Application-Oriented Survey on RAG, Reasoning, and Agentic Systems](https://arxiv.org/abs/2510.24476)
*Yihan Li,Xiyuan Fu,Ghanshyam Verma,Paul Buitelaar,Mingming Liu*

Main category: cs.CL

TL;DR: 该论文系统分析了RAG和推理增强在缓解大语言模型幻觉方面的协同潜力，提出了基于知识和逻辑的幻觉分类，并建立了统一框架。


<details>
  <summary>Details</summary>
Motivation: 幻觉是LLMs可靠部署的主要障碍，RAG和推理增强是最有效的缓解方法，但它们的协同机制尚未被系统研究。

Method: 采用应用导向的能力增强视角，提出知识型和逻辑型幻觉分类，系统分析RAG和推理如何分别解决这些问题，并建立统一框架。

Result: 提供了包含实际应用、评估和基准测试的统一框架，系统展示了RAG和推理增强在缓解幻觉方面的协同作用。

Conclusion: RAG和推理增强的集成在Agentic Systems中能有效缓解LLMs的幻觉问题，为平衡创造性和可靠性提供了系统解决方案。
...
### [136] [Talk2Ref: A Dataset for Reference Prediction from Scientific Talks](https://arxiv.org/abs/2510.24478)
*Frederik Broy,Maike Züfle,Jan Niehues*

Main category: cs.CL

TL;DR: 提出了从科学演讲中预测参考文献（RPT）的新任务，并创建了Talk2Ref数据集（6,279个演讲和43,429篇引用论文），通过双编码器架构和领域自适应训练显著提升了引用预测性能。
...
Method: 构建Talk2Ref数据集，评估最先进的文本嵌入模型在零样本检索场景中的表现，提出双编码器架构，探索处理长文本转录本的策略和领域自适应训练方法。

Result: 在Talk2Ref数据集上的微调显著提高了引用预测性能，证明了任务的挑战性以及从口语科学内容中学习语义表示的有效性。
...
### [137] [CritiCal: Can Critique Help LLM Uncertainty or Confidence Calibration?](https://arxiv.org/abs/2510.24505)
...
Method: 提出自然语言批评作为解决方案，包括：1）分析批评内容：不确定性（问题焦点）或置信度（答案特定）；2）提出自我批评方法让LLM批评并优化置信度；3）提出CritiCal批评校准训练方法，利用自然语言批评改进置信度校准。

Result: 实验显示CritiCal显著优于自我批评和其他竞争基线，在复杂推理任务中甚至超过其教师模型GPT-4o，并在分布外设置中展现出稳健的泛化能力。
...
### [138] [Levée d'ambiguïtés par grammaires locales](https://arxiv.org/abs/2510.24530)
*Eric G. C. Laporte*

Main category: cs.CL

TL;DR: 本文提出了一种词性消歧方法，旨在实现零静默率（正确词性标签从不被丢弃），并展示了在INTEX系统中实现该方法的正式描述。


<details>
  <summary>Details</summary>
Motivation: 解决词性标注中的歧义问题，确保在自然语言处理系统中正确词性标签从不被丢弃，实现零静默率目标。

Method: 在Silberztein的INTEX系统中实现词性消歧方法，使用局部消歧语法和转换器组合，强调需要验证转换器路径的交互作用。

Result: 发现单独考虑转换器路径不足以验证局部消歧语法，需要检查它们的交互作用；组合多个转换器的结果无法通过单独考虑来预测。

Conclusion: 为实现零静默率，局部语法必须仔细测试，需要详细规范语法在文本应用中的行为，因为语法直觉可能因未预见的构造或歧义而不准确。
...
### [139] [Dark & Stormy: Modeling Humor in the Worst Sentences Ever Written](https://arxiv.org/abs/2510.24538)
...
### [140] [Open Korean Historical Corpus: A Millennia-Scale Diachronic Collection of Public Domain Texts](https://arxiv.org/abs/2510.24541)
...
Method: 构建了开放韩语历史语料库，包含1800万份文档和50亿个标记，涵盖从7世纪到2025年的19个来源，包括韩式汉文（Idu）和韩汉混合文字等书写系统。

Result: 量化分析发现：(1) Idu使用在1860年代达到顶峰后急剧下降；(2) 从汉字到韩文的转变始于1890年左右，是一个快速转型；(3) 朝鲜的词汇差异导致现代分词器产生高达51倍的未登录词率。

Conclusion: 该语料库为韩语历史的定量历时分析提供了基础资源，可作为大型语言模型的预训练语料，提高其对现代韩文中的汉字词和古代书写系统的理解能力。
...
### [141] [BEST-RQ-Based Self-Supervised Learning for Whisper Domain Adaptation](https://arxiv.org/abs/2510.24570)
...
Method: 提出BEARD框架，结合BEST-RQ目标和冻结教师编码器的知识蒸馏，使用未标记数据适配Whisper编码器，确保编码器与预训练解码器的互补性。
...
### [142] [ReForm: Reflective Autoformalization with Prospective Bounded Sequence Optimization](https://arxiv.org/abs/2510.24592)
*Guoxin Chen,Jing Wu,Xinjie Chen,Wayne Xin Zhao,Ruihua Song,Chengxi Li,Kai Fan,Dayiheng Liu,Minpeng Liao*

Main category: cs.CL

TL;DR: ReForm提出了一种反射式自动形式化方法，通过将语义一致性评估整合到自动形式化过程中，使模型能够迭代生成形式化语句、评估语义保真度并通过渐进细化自我纠正错误。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在自动形式化任务中虽然能生成语法正确的形式化语句，但经常无法保持原始问题语义意图的问题。现有方法将自动形式化视为简单的翻译任务，缺乏人类专家自然使用的自我反思和迭代优化机制。

Method: 提出ReForm反射式自动形式化方法，整合语义一致性评估；引入前瞻有界序列优化(PBSO)，在不同序列位置使用不同奖励，确保模型既发展准确的自动形式化能力，又发展正确的语义验证能力。

Result: 在四个自动形式化基准测试中，ReForm比最强基线平均提升了17.2个百分点；引入ConsistencyCheck基准（859个专家标注项目），验证了LLMs作为评判者的可靠性，并揭示自动形式化本质上的困难性——人类专家在高达38.5%的情况下也会产生语义错误。

Conclusion: ReForm通过反射式自动形式化方法显著提升了自动形式化的性能，证明了整合语义一致性评估和迭代优化的有效性。同时发现自动形式化任务本身具有挑战性，即使是人类专家也难以完全避免语义错误。
...
### [143] [Diffusion LLM with Native Variable Generation Lengths: Let [EOS] Lead the Way](https://arxiv.org/abs/2510.24605)
*Yicun Yang,Cong Wang,Shaobo Wang,Zichen Wen,Biqing Qi,Hanlin Xu,Linfeng Zhang*

Main category: cs.CL

TL;DR: 本文提出了一种具有原生可变生成长度的扩散大语言模型dLLM-Var，通过训练模型准确预测[EOS]标记，解决了传统dLLM固定生成长度的问题，实现了更高的并行性和推理效率。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的大语言模型(dLLMs)虽然具有并行文本生成的潜力，但存在固定生成长度的问题，需要在解码前确定生成长度作为超参数，这影响了效率和灵活性。

Method: 提出dLLM-Var方法，训练模型准确预测[EOS]标记，使dLLM能够以块扩散方式原生推断，同时保持全局双向注意力和高并行性。

Result: 在标准基准测试中，该方法相比传统dLLM推理范式实现了30.1倍加速，相比自回归模型如Qwen和Llama实现了2.4倍加速，同时达到更高的准确率。

Conclusion: 该方法实现了更高的准确率和更快的推理速度，使dLLM超越了纯粹的学术新颖性，支持其在现实世界应用中的实际使用。
...
### [144] [Long-Context Modeling with Dynamic Hierarchical Sparse Attention for On-Device LLMs](https://arxiv.org/abs/2510.24606)
*Siheng Xiong,Joe Zou,Faramarz Fekri,Yae Jee Cho*

Main category: cs.CL

TL;DR: DHSA是一种动态分层稀疏注意力框架，通过数据驱动方式在线预测注意力稀疏性，无需重新训练，在保持密集注意力准确性的同时显著降低计算成本和内存使用。


<details>
  <summary>Details</summary>
Motivation: 现有静态稀疏方法（如滑动窗口或全局标记）无法适应注意力中内容依赖的变化，而动态方法依赖预定义模板或启发式机制，限制了通用性并可能修剪掉上下文重要的标记。

Method: DHSA自适应地将序列分割为可变长度的块，通过长度归一化聚合计算块表示，然后将块级相似性分数上采样到标记级相似性，以确定应保留哪些标记级交互。

Result: 在Gemma2上的实验表明，DHSA在准确性与密集注意力相当的同时，将预填充延迟降低20-60%，峰值内存使用降低35%，相比其他基线方法获得6-18%的相对准确率提升。
...
### [145] [Zero-Shot Cross-Lingual Transfer using Prefix-Based Adaptation](https://arxiv.org/abs/2510.24619)
*Snegha A,Sayambhu Sen,Piyush Singh Pasi,Abhishek Singhania,Preethi Jyothi*

Main category: cs.CL

TL;DR: 本文系统研究了三种基于前缀的方法在零样本跨语言迁移中的应用，从英语迁移到35+种高低资源语言，发现前缀方法在Llama 3.1 8B和Mistral v0.3 7B上均优于LoRA基准，在Belebele基准上提升达6%。


<details>
  <summary>Details</summary>
Motivation: 随着Llama和Mistral等大型语言模型的发布，零样本跨语言迁移因多语言预训练和强泛化能力而日益可行，但将这些仅解码器LLMs适应到跨语言新任务仍具挑战性。

Method: 使用三种基于前缀的方法（软提示调优、前缀调优和Llama Adapter）进行零样本跨语言迁移研究，涵盖从1B到24B的模型规模扩展，并与参数高效微调技术LoRA进行比较。

Result: 在Llama 3.1 8B上，前缀方法在Belebele基准上比LoRA基线提升达6%；在Mistral v0.3 7B上也观察到类似改进；尽管前缀调优仅使用1.23M学习参数，但在多样化基准上实现了持续改进。
...
### [146] [Relative Scaling Laws for LLMs](https://arxiv.org/abs/2510.24626)
...
<details>
  <summary>Details</summary>
Motivation: 传统的缩放定律在聚合测试集上评估，平均了异质子群体的表现，掩盖了性能差异。需要跟踪不同测试分布间性能差距随规模的变化。

Method: 使用255个解码器Transformer模型，在匹配计算预算（10^18-10^20 FLOPs）下训练，分析相对缩放定律而非绝对误差。

Result: 发现多样化轨迹：学术领域趋于均衡；区域英语方言表现取决于人口规模；AI风险行为分裂，能力和影响力相关风险增加而对抗性风险不变。

Conclusion: 缩放改进整体性能但不是普遍均衡器，建议同时测量相对和传统缩放定律以更好地优先处理鲁棒性挑战。
...
### [147] [OpenReward: Learning to Reward Long-form Agentic Tasks via Reinforcement Learning](https://arxiv.org/abs/2510.24636)
...
Conclusion: 将OpenRM集成到推理时响应选择和训练时数据选择中，在下游LLM对齐任务中取得一致增益，突显了工具增强奖励模型在扩展可靠长文本评估方面的潜力。
...
### [148] [Quantifying the Effects of Word Length, Frequency, and Predictability on Dyslexia](https://arxiv.org/abs/2510.24647)
*Hugo Rydel-Johnston,Alex Kafkas*

Main category: cs.CL

TL;DR: 本研究通过眼动追踪分析发现，阅读障碍者在单词长度、频率和可预测性三个特征上都表现出更强的敏感性，其中可预测性的影响最大。通过反事实操作这些特征，阅读障碍与正常读者的差距可缩小约三分之一。


<details>
  <summary>Details</summary>
Motivation: 探究在自然阅读情境下，阅读障碍者的阅读成本何时以及如何产生，旨在量化这些额外成本的大小和来源。

Method: 使用大规模自然阅读数据集，结合眼动追踪技术，分析单词长度、频率和可预测性三个特征对阅读时间的影响，并进行反事实特征操作。

Result: 所有三个特征都显著影响典型读者和阅读障碍者的阅读时间，阅读障碍者对每个特征都表现出更强的敏感性，特别是对可预测性。反事实操作可将阅读障碍-控制组差距缩小约三分之一。

Conclusion: 研究结果支持阅读障碍理论中关于语言工作记忆和语音编码需求增加的假设，为干预措施和计算模型提供了可操作的指导，并指出需要进一步研究词汇复杂性和副中央凹预览效益来解释剩余的差距。
...
### [149] [Optimizing Retrieval for RAG via Reinforced Contrastive Learning](https://arxiv.org/abs/2510.24652)
*Jiawei Zhou,Lei Chen*

Main category: cs.CL

TL;DR: R3是一个专为RAG优化的检索框架，通过基于试错反馈的强化对比学习，使检索器能够在RAG环境中动态探索和优化相关性，无需依赖标注或合成数据进行监督微调。
...
Method: R3框架采用强化对比学习方法，在训练过程中让检索结果与环境交互产生对比信号，自动指导检索器的自我改进，无需标注数据。

Result: 在多样化任务上的广泛实验表明，R3将RAG性能比原始检索器提升5.2%，比最先进的检索器提升4.9%，达到与基于后训练或指令调优LLM的LLM增强检索和RAG系统相当的结果。

Conclusion: R3是一种高效实用的检索框架，仅需4个GPU即可在一天内完成训练，为RAG系统提供了无需标注数据的有效检索优化方案。
...
### [150] [MQM Re-Annotation: A Technique for Collaborative Evaluation of Machine Translation](https://arxiv.org/abs/2510.24664)
*Parker Riley,Daniel Deutsch,Mara Finkelstein,Colten DiIanni,Juraj Juraska,Markus Freitag*

Main category: cs.CL

TL;DR: 本文提出了一种改进的机器翻译评估方法——MQM重标注，通过让标注者重新审查和编辑已有的MQM标注来提高评估质量。
...
Method: 采用两阶段的MQM评估范式，让标注者重新审查和编辑已有的MQM标注（可能来自自己、其他人类标注者或自动系统）。

Result: 重标注行为符合预期目标，且能产生更高质量的标注，主要原因是发现了第一轮评估中遗漏的错误。

Conclusion: MQM重标注是一种有效的机器翻译评估改进方法，能够提高评估的准确性和可靠性。
...
### [151] [Dissecting Role Cognition in Medical LLMs via Neuronal Ablation](https://arxiv.org/abs/2510.24677)
*Xun Liang,Huayi Lai,Hanyu Wang,Wentao Zhang,Linfeng Zhang,Yanfang Chen,Feiyu Xiong,Zhiyu Li*

Main category: cs.CL

TL;DR: 该研究评估了基于提示的角色扮演对大型语言模型医学推理能力的影响，发现角色提示主要影响表面语言特征而非推理能力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究基于提示的角色扮演是否能在大型语言模型中诱导出特定的、角色相关的认知过程，还是仅仅修改语言风格。

Method: 引入了RP-Neuron-Activated评估框架，在三个医学问答数据集上使用神经元消融和表示分析技术来评估推理路径的变化。

Result: 结果显示角色提示并未显著增强大型语言模型的医学推理能力，主要影响表面语言特征，没有证据表明存在不同的推理路径或跨临床角色的认知差异。

Conclusion: 当前基于提示的角色扮演方法无法复制真实世界医学实践中的认知复杂性，强调了需要模拟真正认知过程而非语言模仿的模型。
...
### [152] [SPICE: Self-Play In Corpus Environments Improves Reasoning](https://arxiv.org/abs/2510.24684)
...
TL;DR: SPICE是一个强化学习框架，通过单一模型扮演挑战者和推理者两个角色，从大型语料库中挖掘文档生成多样化推理任务，通过对抗动态创建自动课程，在数学和通用推理基准上实现持续改进。


<details>
  <summary>Details</summary>
Motivation: 自改进系统需要环境交互来实现持续适应。现有无基础的自学习方法收益有限，需要一种能够持续生成挑战性目标并实现的方法。

Method: 引入SPICE框架，单一模型同时作为挑战者（从大型语料库挖掘文档生成多样化推理任务）和推理者（解决这些任务），通过对抗动态在推理者能力边界创建自动课程。

Result: 在多个模型系列上，SPICE在数学推理基准上实现+8.9%的提升，在通用推理基准上实现+9.8%的提升。

Conclusion: 文档基础是SPICE持续生成日益挑战性目标并实现的关键要素，能够实现持续的自改进。
...
### [153] [Repurposing Synthetic Data for Fine-grained Search Agent Supervision](https://arxiv.org/abs/2510.24694)
...
### [154] [AgentFrontier: Expanding the Capability Frontier of LLM Agents with ZPD-Guided Data Synthesis](https://arxiv.org/abs/2510.24695)
*Xuanzhong Chen,Zile Qiao,Guoxin Chen,Liangcai Su,Zhen Zhang,Xinyu Wang,Pengjun Xie,Fei Huang,Jingren Zhou,Yong Jiang*

Main category: cs.CL

TL;DR: 本文提出了一种基于最近发展区(ZPD)理论的数据合成方法，开发了AgentFrontier引擎来自动生成位于LLM能力前沿的高质量多学科数据，用于预训练和后训练，并创建了ZPD Exam基准来评估智能体能力。


<details>
  <summary>Details</summary>
Motivation: 在大型语言模型智能体的能力前沿进行训练是解锁高级推理的关键。受最近发展区(ZPD)教育理论的启发，该理论定义了智能体无法独立解决但能在指导下掌握的任务边界。

Method: 提出了AgentFrontier引擎，这是一个自动化流水线，能够合成精确位于LLM最近发展区内的高质量多学科数据。该引擎支持知识密集型数据的持续预训练和复杂推理任务的针对性后训练。

Result: 训练了AgentFrontier-30B-A3B模型，在Humanity's Last Exam等要求严格的基准测试中取得了最先进的结果，甚至超越了一些领先的专有智能体。

Conclusion: 基于ZPD的数据合成方法为构建更强大的LLM智能体提供了一条可扩展且有效的路径。
...
### [155] [WebLeaper: Empowering Efficiency and Efficacy in WebAgent via Enabling Info-Rich Seeking](https://arxiv.org/abs/2510.24697)
...
TL;DR: WebLeaper框架通过构建高覆盖度的信息搜索任务和生成高效解决方案轨迹，解决了当前LLM智能体搜索效率低的问题，在多个基准测试中实现了效果和效率的双重提升。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的信息搜索智能体存在搜索效率低的问题，这限制了整体性能。主要原因是训练任务中目标实体的稀疏性，限制了智能体学习和泛化高效搜索行为的机会。

Method: 将信息搜索建模为树状结构推理问题，在受限上下文中嵌入更多目标实体；利用精选的维基百科表格，提出Basic、Union和Reverse-Union三种任务合成变体；通过保留同时准确且高效的训练轨迹来优化模型。

Result: 在五个信息搜索基准测试（BrowserComp、GAIA、xbench-DeepSearch、WideSearch和Seal-0）上的广泛实验表明，该方法在基本和综合设置下均优于强基线，在效果和效率上均取得一致改进。

Conclusion: WebLeaper框架通过系统性地提高信息搜索任务的覆盖度和效率，有效解决了LLM智能体搜索效率低的问题，为开放性问题解决提供了更高效的解决方案。
...
### [156] [Tongyi DeepResearch Technical Report](https://arxiv.org/abs/2510.24701)
...
TL;DR: Tongyi DeepResearch是一个专门为长期深度信息检索任务设计的智能大语言模型，通过端到端训练框架结合智能中期训练和后期训练，在多个深度研究基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 开发专门针对长期深度信息检索任务的智能大语言模型，激励自主深度研究能力。

Method: 采用端到端训练框架，结合智能中期训练和后期训练；设计高度可扩展的自动数据合成流水线；为每个阶段构建定制化环境以确保稳定交互。

Result: Tongyi DeepResearch拥有305亿总参数，每token仅激活33亿参数，在多个智能深度研究基准测试中达到最先进性能，包括Humanity's Last Exam、BrowseComp等。
...
### [157] [ComboBench: Can LLMs Manipulate Physical Devices to Play Virtual Reality Games?](https://arxiv.org/abs/2510.24706)
*Shuqing Li,Jiayi Yan,Chenyu Niu,Jen-tse Huang,Yun Peng,Wenxuan Wang,Yepang Liu,Michael R. Lyu*

Main category: cs.CL

TL;DR: ComboBench是一个评估大语言模型将语义动作转换为VR设备操作序列能力的基准测试，涵盖4款流行VR游戏的262个场景。研究发现顶级模型在任务分解方面表现良好，但在程序推理和空间理解方面仍落后于人类。
...
Method: 创建ComboBench基准测试，评估7个LLM在4款VR游戏（Half-Life: Alyx, Into the Radius, Moss: Book II, Vivecraft）262个场景中的表现，与人工标注的真实数据和人类表现进行比较。

Result: Gemini-1.5-Pro等顶级模型展现出强大的任务分解能力，但在程序推理和空间理解方面仍不及人类。不同游戏间的性能差异显著，表明对交互复杂度的敏感性。少量示例能显著提升性能。

Conclusion: 大语言模型在VR设备操作序列转换方面具有潜力，但需要进一步改进程序推理和空间理解能力。基准测试的发布为未来研究提供了基础。
...
### [158] [MetricX-25 and GemSpanEval: Google Translate Submissions to the WMT25 Evaluation Shared Task](https://arxiv.org/abs/2510.24707)
...
<details>
  <summary>Details</summary>
Motivation: 参与WMT25翻译评估共享任务，改进翻译质量评估和错误检测方法，提升评估系统的性能。

Method: 使用Gemma 3多语言开放权重模型，在公开WMT数据上进行微调。MetricX-25采用编码器架构加回归头预测质量分数；GemSpanEval采用解码器架构，将错误跨度检测作为生成任务，输出错误跨度及其上下文。

Result: MetricX-25在MQM和ESA质量分数预测上显著优于前代模型；GemSpanEval在错误跨度检测方面与强基线xCOMET竞争，并能明确识别错误跨度。

Conclusion: 基于Gemma 3的MetricX-25和GemSpanEval系统在翻译质量评估和错误检测任务中表现出色，证明了先进模型架构和训练策略的有效性。
