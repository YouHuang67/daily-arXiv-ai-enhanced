...
TL;DR: UniFit是一个基于多模态大语言模型的通用虚拟试穿框架，通过MLLM引导的语义对齐模块解决文本指令与参考图像之间的语义鸿沟问题，并采用两阶段渐进训练策略从有限数据中学习复杂任务。
...
Method: 提出UniFit框架，包含MLLM引导的语义对齐模块(MGSA)来整合多模态输入，并通过语义对齐损失捕获跨模态语义关系。采用两阶段渐进训练策略和自合成流程。

Result: 实验表明UniFit不仅支持多种VTON任务（包括多服装和模型到模型试穿），而且实现了最先进的性能。

Conclusion: UniFit通过MLLM驱动的语义对齐和渐进训练策略，有效解决了VTON中的语义鸿沟和数据稀缺问题，构建了通用的虚拟试穿框架。
...
TL;DR: 本文针对遮挡条件下的6D物体姿态估计问题，提出了四项创新扩展：动态非均匀密集采样、多假设推理机制、迭代精炼和遮挡增强训练，显著提升了在遮挡场景下的精度和速度。


<details>
  <summary>Details</summary>
Motivation: 现有基于CAD模型的6D姿态估计方法在遮挡条件下容易出错，因为多阶段流水线的早期错误会传播到后续处理，导致性能下降。需要解决遮挡带来的挑战。

Method: 提出四项扩展：1) 动态非均匀密集采样策略，专注于可见区域；2) 多假设推理机制，保留多个置信度排序的姿态候选；3) 迭代精炼逐步提高精度；4) 遮挡增强训练提升鲁棒性。还提出了新的加权可见性评估指标。

Result: 在ICBIN数据集上精度提升超过5%，在BOP数据集基准上提升超过2%，同时推理速度提升约3倍。

Conclusion: 所提出的方法有效解决了遮挡条件下的6D姿态估计问题，显著提高了精度和效率，为机器人、增强现实等应用提供了更可靠的解决方案。
...
TL;DR: Box6D是一种针对仓库场景中存储箱的类别级6D姿态估计方法，通过单次RGB-D观测快速推断箱子尺寸并使用类别CAD模板估计姿态，显著降低推理时间约76%


<details>
  <summary>Details</summary>
Motivation: 解决仓库自动化、拣选等场景中在杂乱和遮挡条件下准确高效估计物体6D姿态的问题，现有方法在灵活性、准确性和实用性方面存在不足

Method: 使用快速二分搜索推断箱子尺寸，基于类别CAD模板而非实例特定模型估计姿态，采用深度合理性过滤器和早停策略拒绝不合理假设

Result: 在真实仓库场景和公共基准测试中表现出竞争性或更优的6D姿态精度，同时大幅降低推理时间

Conclusion: Box6D方法在保持高精度的同时显著提升了计算效率，适用于工业环境中的实际应用
...
TL;DR: 本文提出了一种两阶段自改进范式，通过让视觉语言模型生成详细文本推理，然后基于这些自生成的推理进行微调，有效解决了视觉语言模型在领域特定视频分类任务中的推理鸿沟问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在领域特定视频分类任务中表现不佳，特别是在数据有限的情况下。这源于关键的推理鸿沟——稀疏的领域数据不足以弥合复杂时空内容与抽象分类标签之间的语义距离。

Method: 采用两阶段自改进范式：第一阶段提示视觉语言模型为每个视频生成详细文本推理，强制其阐述领域特定逻辑；第二阶段在自生成推理上进行微调，利用中间监督使模型表示与目标领域细节对齐，然后进行传统的监督微调。

Result: 在多样化数据集上的广泛实验表明，该方法显著优于直接监督微调，验证了自生成推理作为适应视觉语言模型到领域特定视频分析的有效、注释高效范式。

Conclusion: 自生成推理是一种有效的、注释高效的范式，能够成功地将视觉语言模型适应到领域特定的视频分析任务中，显著提升了模型性能。
...
TL;DR: MGLL是一个多粒度语言学习框架，通过对比学习改进多标签和跨粒度对齐，在医学影像等多标签复杂领域表现优异。


<details>
  <summary>Details</summary>
Motivation: CLIP等现有方法专注于单标签、单粒度对齐，但在医学影像等复杂领域，图像通常对应多个高层标签和不同注释粒度，需要更有效的多标签跨粒度对齐方法。

Method: 提出MGLL对比学习框架，利用结构化多标签监督，整合跨粒度文本描述，引入点式约束的软标签监督来增强对齐，使用平滑KL散度确保跨粒度一致性。

Result: 在构建的大规模多粒度数据集上预训练并在多个数据集上评估，MGLL在下游任务中优于其他最先进方法。

Conclusion: MGLL作为一个即插即用模块，能有效提升视觉语言模型在多标签跨粒度对齐任务中的性能，特别适用于医学影像等复杂领域。
...
TL;DR: 提出一种从3D心脏超声体积中自动选择标准2D视图的方法，通过深度学习视图分类器和基于解剖标志的启发式算法重建标准超声心动图视图，准确率达96%，并验证了提取的2D视频在检测心脏异常和生成临床级测量方面的有效性。
...
Result: 在来自2家医院的1600个视频中，三位心脏病专家的盲评准确率达到96%。提取的2D视频能够使用AI超声心动图模型检测心脏异常，并生成临床级的心脏解剖测量。

Conclusion: 从3D体积中提取的2D视频保留了空间校准和诊断特征，使临床医生能够从3D体积中获得准确的真实世界解释。该方法成功连接了3D扫描的优势与医生熟悉的2D工作流程。
...
TL;DR: Click2Graph是首个用于全景视频场景图生成的交互式框架，将视觉提示与空间、时间和语义理解相结合，通过用户点击或边界框等单一提示，实现目标分割跟踪、交互对象发现和场景图生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频场景图生成系统是封闭的前馈管道，无法融入人类指导；而可提示分割模型缺乏语义或关系推理。Click2Graph旨在统一视觉提示与空间、时间、语义理解，实现用户引导的视频场景理解。

Method: 引入动态交互发现模块生成主体条件对象提示，以及语义分类头执行联合实体和谓词推理。通过单一用户提示实现目标分割跟踪、自主发现交互对象并预测三元组。

Result: 在OpenPVSG基准测试中，Click2Graph为用户引导的全景视频场景图生成建立了坚实基础，展示了人类提示如何与全景定位和关系推理相结合。

Conclusion: Click2Graph实现了可控和可解释的视频场景理解，通过结合人类提示与全景定位和关系推理，为交互式视频场景图生成开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在有限类别上微调CLIP进行分割会导致过拟合，并破坏预训练的视觉-语言对齐。需要稳定模态对齐的迁移过程。

Method: 提出InfoCLIP，基于互信息设计两个新目标：压缩像素-文本模态对齐以减少噪声；最大化预训练CLIP与微调模型间对齐知识的互信息，以传递适合分割任务的局部语义关系。
...
TL;DR: 提出一种新颖的一致性正则化方法，通过可微分的BI-RADS形态特征来缓解乳腺超声肿瘤分割与分类任务之间的破坏性干扰，显著提升多任务学习在外部验证数据集上的泛化性能。
...
Method: 提出基于一致性正则化的多任务学习方法，使用可微分的BI-RADS启发形态特征来缓解分割与分类任务间的破坏性干扰。在BrEaST数据集上训练，在三个外部数据集上进行验证。

Result: 在三个外部数据集上均显示出统计学显著改进（p<0.001）：UDIAT（0.81 vs 0.59）、BUSI（0.66 vs 0.56）、BUS-UCLM（0.69 vs 0.49）。在UDIAT数据集上达到了最先进的分割性能。

Conclusion: 所提出的一致性正则化方法有效缓解了多任务学习中的破坏性干扰，显著提升了乳腺超声肿瘤分割任务在外部验证中的泛化性能，实现了最先进的性能表现。
...
TL;DR: 提出了一种检测引导的生成式框架，用于统一处理目标检测、类别预测和属性识别，通过分层类别和属性token预测，在电商场景中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖全局相似性，难以捕捉细粒度类别差异和类别特定属性的多样性，特别是在大规模电商场景中。

Method: 检测引导的生成式框架，提取精炼的ROI级特征，使用BART生成器按从粗到细的顺序生成语义token，涵盖类别层次结构和属性值对，支持属性条件识别。
...
Method: 探索上下文学习作为改进公平性的轻量级、免调优替代方案。提出公平感知演示选择（FADS），通过基于聚类的采样构建人口统计学平衡且语义相关的演示。

Result: 在多个医学成像基准测试上的实验表明，FADS持续减少性别、种族和民族相关的差异，同时保持强大的准确性。

Conclusion: 公平感知的上下文学习为公平医学图像推理提供了一条高效且可扩展的路径，突出了其作为可扩展和数据高效解决方案的潜力。
...
TL;DR: 本文提出了一种用于单步图像超分辨率的混合秩（MoR）架构，将稀疏门控的混合专家（MoE）思想引入真实世界图像超分辨率任务，通过细粒度专家划分策略和退化感知路由机制，实现了对复杂退化样本的自适应处理。


<details>
  <summary>Details</summary>
Motivation: 现有的真实世界图像超分辨率方法主要依赖通过LoRA模块微调预训练扩散模型，但这些密集模型难以自适应捕捉复杂真实世界退化样本的异质特征，也无法在同等计算预算下实现输入间的知识共享。

Method: 1. 提出混合秩架构，将LoRA中的每个秩视为独立专家；2. 引入细粒度专家划分策略，隔离固定位置秩作为共享专家；3. 开发基于CLIP嵌入的退化估计模块；4. 结合零专家槽和退化感知负载均衡损失。

Result: 综合实验验证了该框架的有效性和最先进的性能表现。

Conclusion: MoR架构成功将稀疏MoE思想应用于真实世界图像超分辨率，通过灵活的专家组合和退化感知路由机制，实现了对复杂退化样本的自适应处理，同时保持了计算效率。
...
TL;DR: CuriGS是一个基于课程学习的稀疏视图3D重建框架，通过引入学生视图来增强3D高斯泼溅在稀疏视图设置下的训练效果。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在稀疏视图设置下面临监督稀缺和过拟合的挑战，需要解决有限视角覆盖带来的问题。

Method: 提出课程引导框架，为每个真实姿态生成多组不同扰动水平的学生视图，通过深度相关和协同正则化进行训练，并采用多信号评估指标选择最佳学生视图。

Result: 实验结果表明CuriGS在渲染保真度和几何一致性方面优于现有最先进方法，适用于各种合成和真实稀疏视图场景。

Conclusion: CuriGS通过课程学习策略有效解决了稀疏视图3D重建中的监督稀缺问题，显著提升了3D高斯泼溅在稀疏视图设置下的性能。
...
TL;DR: 提出一个基于大语言模型的框架来解决食物识别中的领域偏移、长尾分布和细粒度分类问题，通过生成食物标题和成分，将文本和图像投影到共享嵌入空间进行对齐，在食物数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决食物识别中训练数据与真实环境图片的领域偏移问题，以及真实世界食物数据集的长尾分布和细粒度分类挑战。

Method: 利用大语言模型解析食物图像生成标题和成分，将生成的文本和不同领域的食物图像投影到共享嵌入空间以最大化配对相似度，使用对齐后的多模态特征进行识别。

Result: 在两个食物数据集上，该方法在长尾数据分布、领域适应和细粒度分类方面均优于现有专门方法。

Conclusion: 基于大语言模型的简单框架能够有效解决食物识别中的多个挑战性问题，包括领域偏移、长尾分布和细粒度分类。
...
TL;DR: AMS-KV是一种用于视觉自回归模型中下一尺度预测的自适应KV缓存策略，通过优先存储压缩尺度和局部尺度的KV，减少KV缓存使用达84.83%，降低自注意力延迟60.48%，并提高模型的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 下一尺度预测在视觉自回归模型中面临KV内存过度增长的瓶颈，限制了模型的可扩展性，而现有的KV缓存设计对此问题研究不足。

Method: 基于系统研究发现，提出AMS-KV策略：优先存储压缩尺度和局部尺度的KV；通过跨尺度相似性分析识别缓存需求层；优化KV缓存利用和计算效率。

Result: 相比基线VAR模型，AMS-KV减少KV缓存使用达84.83%，自注意力延迟降低60.48%；在基线模型内存不足的情况下，AMS-KV能稳定扩展到批量大小256并提高吞吐量。
...
TL;DR: LiSTAR是一个直接在传感器原生几何上操作的生成世界模型，通过混合圆柱-球面表示和时空注意力机制，解决了4D LiDAR数据合成中的几何保真度、时间稀疏性和复杂动态场景挑战，在重建、预测和条件生成方面实现了最先进的性能。
...
Method: 提出LiSTAR模型，采用混合圆柱-球面表示来保持数据保真度，利用时空注意力与射线中心变换器来捕获复杂动态，并提出4D点云对齐体素布局和离散掩码生成框架进行可控合成。

Result: 在4D LiDAR重建、预测和条件生成方面实现了最先进的性能，生成MMD降低了76%，重建IoU提高了32%，预测L1 Med降低了50%。

Conclusion: LiSTAR为创建真实可控的自动驾驶系统仿真提供了强大的新基础，在多个关键指标上取得了显著提升。
...
Method: 提出三阶段框架：1)分层文本引导帧采样模拟人类注意力；2)推理模型生成空间线索和显式推理链；3)使用SAM2和XMem进行分割传播，并引入任务难度感知机制自适应控制推理长度。
...
TL;DR: SpectralTrain是一个通用的HSI分类训练框架，通过结合课程学习和PCA降维来提升学习效率，显著降低计算成本，同时保持良好性能。
...
Method: 集成课程学习与基于PCA的光谱降采样，逐步引入光谱复杂度同时保留关键信息，实现高效的光谱-空间模式学习。

Result: 在三个基准数据集上的实验显示训练时间减少2-7倍，精度损失较小，在云分类等气候遥感应用中展现出潜力。

Conclusion: 训练策略优化是HSI模型架构设计的有效补充，SpectralTrain框架具有强泛化能力和实际部署价值。
...
TL;DR: Rad-GS是一个基于4D雷达-相机融合的SLAM系统，使用3D高斯作为可微分空间表示，专为千米级户外环境设计。该系统结合雷达点云和多普勒信息，通过动态物体掩码提升定位精度，并利用非同步图像帧优化3D高斯表示，在大规模环境中显著降低内存消耗。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够在千米级户外环境中稳定运行的SLAM系统，利用4D毫米波雷达的优势来替代传统相机或激光雷达输入，实现大规模场景的鲁棒重建。

Method: 结合原始雷达点云与多普勒信息，通过几何增强的点云指导同步图像中的动态物体掩码；利用非同步图像帧全局优化3D高斯表示；采用全局八叉树结构和目标高斯基元管理策略来抑制噪声并减少内存消耗。

Result: 实验表明Rad-GS在性能上与传统基于相机或激光雷达的3D高斯方法相当，成功实现了千米级真实世界重建，验证了使用4D毫米波雷达进行鲁棒户外建图的可行性。

Conclusion: Rad-GS证明了4D毫米波雷达在大规模户外场景重建中的潜力，为雷达在SLAM系统中的应用提供了新的可能性，特别是在需要大规模环境建图的场景中。
...
TL;DR: 本文提出了T2T-VICL框架，研究视觉语言模型在不同视觉任务间的跨任务上下文学习能力，通过文本提示生成和推理机制实现跨任务视觉上下文学习。
...
Conclusion: T2T-VICL框架有效解锁了视觉语言模型在跨任务视觉上下文学习方面的潜力，为统一视觉语言模型解决多样化下游任务提供了新途径。
...
TL;DR: DCS-LDM是一种新的视觉生成范式，通过构建分层、尺度无关的潜在空间，将信息复杂度与尺度解耦，支持在固定潜在表示下解码到任意分辨率和帧率。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型通常将尺度与内容复杂度耦合，使用更多潜在标记来表示更高分辨率图像或更高帧率视频。但表示视觉数据所需的潜在容量主要取决于内容复杂度，尺度仅作为上限。

Method: 构建分层、尺度无关的潜在空间，通过多级标记建模样本复杂度，支持在固定潜在表示下解码到任意分辨率和帧率，实现灵活的计算-质量权衡。
...
TL;DR: 本文提出了多模态交互欺骗评估（MIDA）任务和数据集，评估了12个最先进的多模态大语言模型在识别欺骗方面的表现，发现即使GPT-4o等强大模型也难以可靠区分真假。作者设计了社交思维链推理和动态社交认知记忆模块来改进性能。
...
Method: 引入MIDA任务和包含同步视频文本的多模态数据集，建立基准评估12个模型，分析失败模式，并设计社交思维链推理管道和动态社交认知记忆模块。

Result: 最先进模型在区分真假方面存在显著性能差距，无法有效将语言与多模态社交线索关联，缺乏建模他人知识、信念或意图的能力。提出的框架在该挑战性任务上实现了性能提升。

Conclusion: 需要新方法来构建更具感知力和可信度的AI系统，提出的方法为构建能够进行真正类人社交推理的多模态大语言模型展示了有前景的新路径。
...
TL;DR: 该论文提出了一种用于压缩视频质量增强的预训练退化表示学习模块，能够提取高维多尺度退化表示来指导伪影去除，同时还引入了分层终止机制来动态调整计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有压缩视频质量增强方法主要依赖已知量化参数，但在实际场景中量化参数可能部分或完全未知，限制了这些方法的适用性。此外，现有盲方法只能捕获全局退化信息而缺乏空间细节，且所有方法都采用统一架构而忽略了不同压缩级别下的计算需求差异。

Method: 提出预训练退化表示学习模块，从视频内容中解耦并提取高维多尺度退化表示来指导伪影去除；引入分层终止机制，根据压缩级别动态调整伪影减少阶段的数量。

Result: 所提方法显著提升了性能，在QP=22时相比现有最先进盲方法实现了110%的PSNR提升（从0.31 dB到0.65 dB）；分层终止机制在QP=22时将平均推理时间相比QP=42减少了一半。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D目标检测器存在训练与推理之间的差距，这种差距源于训练时缺乏空间可靠性和排名感知，与推理时基于排名的预测选择相冲突，阻碍了模型学习与推理行为对齐的表征。

Method: SR3D包含两个组件：1）空间优先最优传输分配，动态强调位置良好且空间可靠的样本；2）排名感知自适应自蒸馏方案，通过自蒸馏范式自适应注入排名感知。

Result: 在ScanNet V2和SUN RGB-D数据集上的广泛实验表明，SR3D有效弥合了训练-推理差距，在保持实时速度的同时显著优于现有方法的准确性。
...
TL;DR: 本文提出了一个高空间分辨率的水体变化检测数据集HSRW-CD，并设计了一个SSCP注意力模块来充分利用深度特征中的空间语义和结构信息，显著提高了水体变化检测的性能。
...
Method: 提出了HSRW-CD数据集（空间分辨率高于3米），设计了SSCP注意力模块，包含多语义空间注意力(MSA)、结构关系感知全局注意力(SRGA)和通道自注意力(CSA)三个组件，可即插即用到现有WBCD模型中。

Result: 在提出的HSRW-CD和Water-CD数据集上的大量实验验证了SSCP的有效性和泛化能力。

Conclusion: SSCP模块能够有效利用深度特征的空间语义和结构信息，显著提升水体变化检测的判别能力，且具有良好的泛化性能。
...
Method: 1) WSI-基因CoT增强病例库构建：通过低倍镜筛查、跨模态相似性感知补丁挖掘和置信度感知补丁挖掘分析病理图像，基因分层分析处理六类功能基因；2) 二分法多专家智能体推理：通过RAG检索相似病例，整合多模态报告并通过渐进区间细化进行专家预测。
...
TL;DR: LEGO-SLAM是首个在3D高斯泼溅SLAM系统中实现实时开放词汇语义映射的框架，通过场景自适应编码器将语言特征压缩到16维，减少内存占用并加速渲染，同时支持语言引导的修剪和闭环检测。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS SLAM系统缺乏开放词汇语义理解能力，无法满足高级机器人交互需求。集成语言特征面临内存占用大、渲染开销高的问题，而静态模型方法缺乏对新环境的适应性。

Method: 提出场景自适应编码器-解码器，将高维语言嵌入蒸馏到紧凑的16维特征空间；采用语言引导的修剪策略识别语义冗余；利用映射特征进行基于语言的闭环检测。

Result: 减少每个高斯的内存占用并加速渲染，实现实时性能（15 FPS）；高斯数量减少超过60%同时保持渲染质量；在映射质量和跟踪精度方面达到竞争性水平。

Conclusion: LEGO-SLAM成功解决了3DGS SLAM中的语义集成挑战，实现了实时开放词汇语义映射，为高级机器人交互提供了有效解决方案。
...
TL;DR: 本文提出了Thinking-while-Generating (TwiG)框架，这是第一个在视觉生成过程中交织文本推理的框架，通过动态交互产生更具上下文感知和语义丰富的视觉输出。


<details>
  <summary>Details</summary>
Motivation: 当前视觉生成方法在生成前或生成后整合文本推理，但缺乏在生成过程中的多模态交互。本文旨在探索在视觉生成过程中实时交织文本推理的潜力。

Method: 提出了TwiG框架，在视觉内容逐步生成时交织文本推理，既指导即将生成的局部区域，又反思已合成的区域。研究了三种策略：零样本提示、在TwiG-50K数据集上的监督微调，以及通过定制TwiG-GRPO策略的强化学习。

Result: 该框架能够产生更具上下文感知和语义丰富的视觉输出，三种策略各自为交织推理的动态性提供了独特的见解。

Conclusion: 这项工作为在视觉生成中交织文本推理以增强生成效果的研究提供了启发，代码将在GitHub上发布。
...
TL;DR: 提出RGE方法，通过结合MLLMs的生成推理能力和对比训练来增强多模态嵌入质量，在MMEB基准上比非推理基线提升4.9%


<details>
  <summary>Details</summary>
Motivation: 现有方法将嵌入提取视为直接编码步骤，忽略了MLLMs具有的生成推理能力可以提升表示质量

Method: 提出RGE方法：首先让模型基于指令执行结构化推理生成，然后在推理展开后提取表示，结合对比训练

Result: 在MMEB基准上，推理引导的条件化使多模态检索性能比非推理基线提升4.9%

Conclusion: 显式推理能有效增强嵌入质量，推理引导的嵌入方法在多模态表示学习中具有优势
...
TL;DR: 提出PPCL框架，通过可插拔剪枝和连续层蒸馏技术，在保持图像生成质量的同时，将DiT模型的参数量减少50%，性能下降小于3%。
...
Method: 使用线性探测机制和相似度度量的一阶微分趋势分析识别冗余层区间，提出可插拔的师生交替蒸馏方案，在单次训练中集成深度和宽度剪枝。

Result: 在多个多模态DiT模型上，PPCL实现参数量减少50%，关键客观指标下降小于3%，保持高质量图像生成能力。

Conclusion: PPCL框架实现了更高的压缩比，适合资源受限环境，代码和检查点已开源。
...
TL;DR: 提出了Video2Layout框架，使用连续边界坐标替代离散网格表示，从视频中重建度量空间布局，提升多模态大语言模型的空间推理能力。
...
Conclusion: 连续边界坐标表示能够有效提升多模态大语言模型的空间推理能力，缓解自然语言描述空间关系时的固有模糊性。
...
TL;DR: Simba是一个新颖的点云补全框架，将点级变换回归重新表述为分布学习问题，结合对称先验和扩散模型的生成能力，避免实例特定记忆化，同时捕获鲁棒的几何结构。


<details>
  <summary>Details</summary>
Motivation: 点云补全中的主要挑战是同时保留输入中的细粒度细节并确保完成形状的全局结构完整性。现有基于回归的方法存在过拟合问题，倾向于记忆实例特定变换而非学习可泛化的几何先验，且对输入噪声高度敏感。

Method: 引入Simba框架，将点级变换回归重新表述为分布学习问题，结合对称先验和扩散模型的生成能力。采用基于Mamba的分层架构实现高保真上采样。

Result: 在PCN、ShapeNet和KITTI基准测试上的广泛实验验证了该方法的最先进性能。

Conclusion: Simba通过将变换回归重新表述为分布学习问题，有效解决了现有方法的过拟合和噪声敏感性问题，在点云补全任务中实现了最先进的性能。
...
TL;DR: LNG-SWR是一种用于医学图像分割的鲁棒性增强方法，通过在训练时注入层间噪声学习频率偏置先验，并在推理时进行选择性小波重建，实现频率自适应，提升模型对分布偏移和扰动的稳定性。


<details>
  <summary>Details</summary>
Motivation: 临床部署需要分割模型在分布偏移和扰动下保持稳定。传统对抗训练存在清洁-鲁棒性权衡和高训练成本问题，限制了医学影像中的可扩展性和可维护性。

Method: 提出层间噪声引导的选择性小波重建（LNG-SWR）：训练时在多个层注入小噪声学习频率偏置先验；推理时应用先验引导的选择性小波重建，抑制噪声敏感频带，增强方向结构和形状线索，稳定边界响应。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在多模态任务上的显著成功，其部署效率问题日益突出。现有方法仅延迟EOS令牌的出现来隐式延长输出，缺乏稳定性和可控性，无法直接最大化输出令牌长度作为明确优化目标。
...
Conclusion: 提出的VTIA方法能够有效诱导视觉语言模型产生冗长输出，解决了现有方法缺乏稳定性和可控性的问题，为模型部署效率评估提供了新视角。
...
TL;DR: EvoVLA是一个自监督的视觉-语言-动作框架，通过阶段对齐奖励、基于姿态的对象探索和长视野记忆三个组件，解决了长视野机器人操作中的阶段幻觉问题，在模拟和真实环境中都显著提升了任务成功率。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型存在阶段幻觉问题，即智能体利用粗略评估信号来走捷径完成多步任务，报告高进展但未真正完成任务。需要解决长视野机器人操作中的这一挑战。
...
Result: 在Discoverse-L长视野操作基准测试中，EvoVLA比最强基线(OpenVLA-OFT)平均任务成功率提高10.2个百分点，达到69.2%。样本效率提高1.5倍，阶段幻觉从38.5%降至14.8%。真实世界部署在物理机器人上达到54.6%的平均成功率，比OpenVLA-OFT高11个百分点。

Conclusion: EvoVLA通过其三个互补组件有效解决了VLA模型中的阶段幻觉问题，在模拟和真实环境中都表现出优异的性能，实现了有效的模拟到真实迁移和强泛化能力。
...
TL;DR: 本文提出RF-CLIP方法，通过模拟人类注意力分散-重聚焦行为，解决CLIP在密集预测中注意力资源被无关token分散的问题，从而提升开放词汇语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然利用CLIP的视觉-语言对齐能力，但很少从可解释性机制角度研究CLIP在密集预测中的性能边界。作者发现CLIP存在类似人类注意力分散的现象，将大量注意力资源从目标区域转移到无关token上。

Method: 提出RF-CLIP方法，这是一种无需训练的方法，通过过滤维度特异性过度激活产生的无关token，将注意力从分散token重新聚焦到目标区域，从而改进CLIP的多模态对齐粒度。

Result: 在八个基准测试中实现了最先进的性能，同时保持高推理效率。

Conclusion: 通过系统分析CLIP内部机制并解决注意力分散问题，RF-CLIP有效提升了开放词汇语义分割的性能，为多模态密集预测提供了新的解决方案。
...
TL;DR: 本文提出了一种新颖的两阶段模型DSLGA，用于解决无监督域自适应可见光-红外行人重识别中的跨模态差异问题，通过域共享学习和渐进对齐策略显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 由于公共数据集与现实世界数据之间存在差异，现有的VI-ReID算法在实际应用中表现不佳。本文旨在研究无监督域自适应VI-ReID，将公共数据的知识迁移到现实世界数据中，同时不降低准确性且无需新样本标注。

Method: 提出DSLGA两阶段模型：1）预训练阶段使用域共享学习策略（DSLS）缓解域间模态差异；2）微调阶段使用渐进对齐策略（GAS）通过聚类到整体的方式处理域内跨模态对齐挑战。还构建了新的测试方法CMDA-XD。

Result: 大量实验表明，该方法在各种设置下显著优于现有的VI-ReID域自适应方法，甚至优于一些监督学习方法。

Conclusion: DSLGA模型通过有效处理域间和域内模态差异，在无监督域自适应VI-ReID任务中取得了卓越性能，为解决现实世界应用中的跨模态行人重识别问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法通常独立处理器官的子结构，导致解剖学上不合理的重建结果，需要一种能够保持器官内部连接关系的统一重建方法。

Method: 从连接的模板开始，PrIntMesh联合变形所有子结构以匹配患者特定解剖结构，明确保持内部边界并强制平滑、无伪影的表面。

Result: 在心脏、海马体和肺部测试中，PrIntMesh实现了高几何精度、正确拓扑结构，即使在有限或噪声训练数据下也能保持鲁棒性能。
...
TL;DR: 本文提出了VLA-Fool框架，系统研究视觉-语言-动作模型在具身环境中的多模态对抗鲁棒性，涵盖白盒和黑盒设置下的文本、视觉及跨模态攻击。
...
Method: VLA-Fool统一了三种多模态对抗攻击：基于梯度和提示的文本扰动、补丁和噪声的视觉扰动、以及故意破坏感知与指令语义对应的跨模态错位攻击。还引入了VLA感知语义空间到语言提示中，开发了首个自动构建的语义引导提示框架。

Result: 在LIBERO基准测试中使用微调的OpenVLA模型进行实验，结果表明即使轻微的多模态扰动也会导致显著的行为偏差，揭示了具身多模态对齐的脆弱性。

Conclusion: VLA模型在多模态对抗攻击下表现出明显的脆弱性，跨模态语义对齐是影响其鲁棒性的关键因素，需要进一步研究提升其对抗鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 开发高效的实时场景理解系统，支持辅助导航等应用，克服VGGT高内存需求，实现连续3D场景表示更新。

Method: 使用滑动窗口处理图像流对齐子图，利用VGGT跟踪头聚合2D语义实例掩码为3D对象，存储时间戳和实例级身份以实现时间一致性和环境变化检测。
...
TL;DR: 本文提出将稀疏自编码器(SAE)重新理解为主题模型，并开发了SAE-TM框架，将SAE特征作为可重用的主题原子，在文本和图像数据集上生成更连贯的主题，支持跨模态的大规模主题分析。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器在嵌入分析中的作用和实际价值存在争议，作者希望通过新的视角证明SAE可以自然地理解为主题模型，从而提升其实际应用价值。

Method: 将潜在狄利克雷分配扩展到嵌入空间，推导出SAE目标作为该模型下的最大后验估计器；提出SAE-TM框架：训练SAE学习可重用主题原子，将其解释为下游数据上的词分布，无需重新训练即可合并成任意数量的主题。

Result: SAE-TM在文本和图像数据集上比强基线方法产生更连贯的主题，同时保持多样性；成功分析了图像数据集中的主题结构，并追踪了日本木版画中主题随时间的变化。

Conclusion: 该研究将SAE定位为跨模态大规模主题分析的有效工具，为SAE的实际应用提供了新的理论基础和框架。
...
TL;DR: NaTex是一个原生纹理生成框架，直接在3D空间中预测纹理颜色，避免了传统基于多视图扩散模型的局限性，通过将纹理视为密集颜色点云的新范式实现更好的纹理一致性和对齐效果。


<details>
  <summary>Details</summary>
Motivation: 解决传统基于多视图扩散模型（MVDs）方法在纹理生成中的固有局限，包括处理遮挡区域困难、难以实现精确的网格-纹理边界对齐、以及保持跨视图一致性和颜色强度连贯性等问题。

Method: 提出潜在颜色扩散方法，包含几何感知的颜色点云VAE和多控制扩散变换器（DiT），完全使用3D数据从头训练；引入原生几何控制，通过位置嵌入和几何潜在变量为DiT提供直接3D空间信息；设计VAE-DiT架构，几何分支与颜色VAE紧密耦合。

Result: NaTex表现出强大的性能，在纹理一致性和对齐方面显著优于先前方法；同时展现出强大的泛化能力，无需训练或仅需简单调优即可应用于材质生成、纹理优化、部件分割和纹理化等各种下游应用。

Conclusion: NaTex通过原生3D纹理生成方法有效解决了传统2D多视图方法的局限性，在纹理质量、对齐精度和跨视图一致性方面取得了显著改进，并具有良好的泛化能力。
...
TL;DR: 该研究提出了一种用于无人机遥感地形分类的弱监督学习方法，通过教师-学生架构结合关键帧选择和更新算法，仅使用30%标注数据即可同时提升mIoU和时序一致性。
...
Result: 实验结果表明，仅使用30%标注数据，该方法能同时提高mIoU和时序一致性，确保地形目标的稳定定位。

Conclusion: 该方法有效克服了传统时序一致性训练在航空任务中的缺陷，为无人机遥感地形分类提供了一种高效的弱监督学习解决方案。
...
TL;DR: 提出了一种基于预捕获彩色LiDAR点云的实时相机定位方法，通过神经渲染技术缩小合成与真实图像之间的域差距，实现无漂移、正确度量尺度的相机跟踪。
...
Method: 从预捕获的高精度彩色LiDAR点云渲染合成视图，建立实时帧与点云的2D-3D对应关系，使用神经渲染技术减少合成与真实图像之间的域差距，提出两种实时变体：在线渲染匹配和预构建定位。

Result: 在ScanNet++数据集上展示改进结果，优于现有SLAM流程，实现无漂移的相机跟踪和正确的度量尺度。
...
TL;DR: 提出MOMNet框架解决RGB-D图像不对齐问题，通过多阶匹配机制和聚合策略实现无需对齐的高质量深度超分辨率重建。


<details>
  <summary>Details</summary>
Motivation: 现实场景中RGB-D图像难以严格对齐，现有方法在不对齐情况下性能下降严重，需要解决不对齐RGB-D的超分辨率问题。

Method: 使用多阶匹配机制（零阶、一阶、二阶匹配）全面识别与深度一致的RGB信息，并通过多阶聚合结构检测器选择性融合RGB和深度特征。

Result: 实验证明MOMNet在不对齐场景下达到最先进性能，并表现出优异的鲁棒性。

Conclusion: MOMNet能够有效处理不对齐RGB-D图像，为现实应用提供实用的深度超分辨率解决方案。
...
Method: 设计了门控交叉注意力机制，从CLIP的高层图像编码块中通过一组潜在单元捕获细粒度语义特征，同时自适应抑制背景和其他无关信息；然后进行多空间解耦，实现属性和对象语义的解耦。

Result: 在三个流行基准测试（MIT-States、UT-Zappos和C-GQA）上的实验表明，CAMS在封闭世界和开放世界设置下都达到了最先进的性能。

Conclusion: CAMS通过提取视觉特征中的语义特征并在多维空间中进行语义解耦，有效提升了组合零样本学习的泛化能力。
...
TL;DR: 提出一种几何引导的自监督环视深度估计方法，通过将多相机图像投影到共享圆柱体上建立跨视图邻域关系，使用非学习的空间注意力机制聚合特征，实现跨视图一致的深度估计。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督环视深度估计方法存在重叠图像间深度估计不一致的问题，需要解决多相机系统下的跨视图一致性深度预测挑战。

Method: 基于相机内参和相对位姿参数，首先为每张图像预测初始深度图，然后将所有图像的3D点投影到共享单位圆柱体上建立跨图像邻域关系，生成位置图，最后应用非学习的空间注意力机制根据圆柱体上的距离聚合跨图像特征来预测最终深度图。

Result: 在DDAD和nuScenes数据集上的评估表明，该方法相比现有最优方法提高了跨图像深度估计的一致性和整体深度质量。

Conclusion: 所提出的几何引导方法能够有效实现跨视图一致的密集度量深度估计，解决了多相机系统下深度预测的一致性问题。
...
TL;DR: 本文提出两种结合Vision Transformer和Graph Neural Networks的图基分割方法，用于提高腹腔镜胆囊切除术中肝胆囊解剖结构的识别精度，在遮挡、长距离依赖和细尺度几何结构等挑战上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 准确识别肝胆囊解剖结构对于预防腹腔镜胆囊切除术中的手术并发症至关重要。深度学习模型在处理遮挡、长距离依赖和罕见结构的细尺度几何特征方面存在困难，需要增强空间和语义理解能力。

Method: 提出两种分割模型：1）静态k最近邻图结合GCNII，实现稳定的长距离信息传播；2）动态可微分图生成器结合GAT，支持自适应拓扑学习。两种模型均集成ViT特征编码器和GNN，在Endoscapes-Seg50和CholecSeg8k基准上评估。

Result: 所提方法在mIoU和mDice指标上分别比现有最优方法提升7-8%和6%，特别是在薄、罕见和安全关键结构上产生解剖学一致的预测结果。

Conclusion: 图基分割方法通过结合ViT的全局上下文和基于图的关系推理，提高了手术场景分割的性能和解剖一致性，增强了模型的可解释性和可靠性，为更安全的腹腔镜和机器人辅助手术铺平了道路。
...
TL;DR: 本文提出了一种语言驱动的属性泛化（LDAG）架构，通过利用目标类别的固有属性语言描述来构建鲁棒的支持策略，以解决小样本分割中支持图像提取的元信息由于类内变化而无法为未训练类别提供准确指导的问题。


<details>
  <summary>Details</summary>
Motivation: 现有小样本分割方法主要从支持图像中挖掘参考信息作为元指导，但由于视觉表示的类内变化，从支持图像提取的元信息无法为未训练类别提供准确指导。作者认为支持图像提供的参考可能不是必需的，支持角色的关键是为训练和未训练类别提供无偏的元指导。

Method: 提出语言驱动的属性泛化（LDAG）架构，包含两个核心模块：1）多属性增强（MaE）模块，通过大语言模型生成目标类别的多个详细属性描述，并利用多模态匹配构建精细的视觉-文本先验指导；2）多模态属性对齐（MaA）模块，实现属性文本与视觉特征之间的跨模态交互，解决文本-视觉模态偏移问题。

Result: 实验表明，所提出的方法明显优于现有方法，实现了新的最先进性能。

Conclusion: 通过利用语言描述构建无偏支持表示，可以有效解决小样本分割中支持图像指导的局限性，该方法在性能上显著超越了现有方法。
...
TL;DR: 提出了StreetView-Waste数据集，专注于城市垃圾容器检测、跟踪和溢流分割任务，并通过启发式方法和几何先验策略提升了基线模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有垃圾检测数据集缺乏对垃圾容器跟踪和溢流监控的专门标注，且在静态环境中采集，限制了在真实城市物流场景中的应用价值。

Method: 构建包含垃圾容器检测、跟踪和溢流分割三个任务的StreetView-Waste数据集；提出启发式方法改进容器跟踪，以及利用几何先验的模型无关框架优化垃圾分割。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLM专用令牌剪枝方法仅基于语义显著性选择令牌，忽视了VLA模型的双系统特性（高级语义理解和低级动作执行），导致动作生成所需的关键信息被丢弃，性能显著下降。

Method: 提出VLA-Pruner方法，采用双层级重要性标准：视觉-语言预填充注意力用于语义级相关性，通过时间平滑估计的动作解码注意力用于动作级重要性。基于此标准设计新颖的双层级令牌选择策略。

Result: 实验表明VLA-Pruner在多种VLA架构和多样化机器人任务上实现了最先进的性能。

Conclusion: VLA-Pruner是一种通用即插即用的VLA专用令牌剪枝方法，能够有效平衡语义理解和动作执行需求，显著提升VLA模型的实时部署效率。
...
TL;DR: LLaVA³是一种无需微调、仅使用多视角2D图像来提升视觉语言模型3D场景理解能力的新方法，通过全方位视觉表示描述3D场景，在3D视觉问答和3D语言定位任务上优于现有2D方案。
...
Method: 受立体派画家启发，通过从中间多视角3D重建中提取每个对象的全方位视觉表示，为VLM描述3D场景。

Result: 在3D视觉问答和3D语言定位任务上的广泛实验表明，该方法优于之前基于2D的VLM解决方案。

Conclusion: LLaVA³仅使用多视角2D图像即可有效提升VLM的3D场景理解能力，无需任何微调。
...
<details>
  <summary>Details</summary>
Motivation: 胼胝体是大脑中最大的连合结构，在衰老和神经系统疾病研究中是重要焦点，也是深部脑刺激等干预措施的关键目标。尽管已有大量胼胝体分割研究，但缺乏提供全面自动化分析流程的公开工具。

Method: 开发了FastSurfer-CC框架，自动识别矢状面切片、分割胼胝体和穹窿，定位前后连合以标准化头部位置，生成厚度剖面和细分，提取8个形状指标。

Result: FastSurfer-CC在各项任务中优于现有专门工具，并揭示了亨廷顿病患者与健康对照组之间现有最先进方法未检测到的统计学显著差异。

Conclusion: FastSurfer-CC提供了一个高效、全自动的胼胝体形态测量解决方案，在性能和检测疾病相关变化方面优于现有工具。
...
Method: 使用特征级线性调制（FiLM）层将采集时间信息融入模型特征，允许利用每个成像研究中采集的完整可变数量图像。
...
TL;DR: 本文提出了一个专门用于拳击击打检测和分类的综合视频数据集，包含6,915个高质量击打片段，涵盖6种击打类型，来自20个YouTube训练视频，涉及18名运动员。


<details>
  <summary>Details</summary>
Motivation: 由于动态、非结构化的动作特性和录制环境的变化，开发鲁棒的数据集是计算机视觉在格斗运动分析中的主要瓶颈。

Method: 从20个公开的YouTube训练视频中提取击打片段，手动分割和标注，确保精确的时间边界和类别一致性，涵盖各种运动风格、摄像机角度和运动员体型。

Result: 创建了一个包含6,915个高质量击打片段的数据集，分为6种不同的击打类型，为实时视觉动作识别研究提供支持。

Conclusion: 通过提供具有多样化击打示例的丰富基准，该贡献旨在加速拳击及相关领域运动分析、自动化教练和性能评估的进展。
...
TL;DR: 本文提出了SemCLIP方法，通过结合释义和否定处理来改进CLIP模型，使用LLM生成的原始、释义和否定文本三元组进行训练，显著提高了对否定文本的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP模型在处理否定和释义文本时表现不一致，因为否定会以最小的词汇变化彻底改变含义，而释义可能产生完全不同但含义相同的文本表达，这对视觉语言模型的评估和对齐提出了挑战。

Method: 提出SemCLIP方法，评估释义和否定的组合，设计新的CLIP对比损失函数，应用LLM生成的包含原始、释义和否定文本描述的三元组来训练CLIP类模型。

Result: SemCLIP能将释义文本推向原始图像嵌入，同时将否定文本推离嵌入空间。在CC-Neg基准测试中，图像检索准确率从68.1%提升到78.1%，在下游零样本分类任务中表现优于CLIP。

Conclusion: SemCLIP能够实现对语义变换的显著鲁棒性，在保持CLIP性能的同时显著提高对否定文本的处理能力。
...
TL;DR: 提出了一种新颖的两阶段检测框架，用于解决合成图像检测中的泛化挑战，通过监督对比学习提取特征嵌入，结合k-NN分类器在少样本学习范式下实现跨生成器的泛化检测。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展使得合成图像越来越难以与真实内容区分，传统检测方法因需要定期重新训练而计算不可行且操作不实用，需要能够适应不断发展的生成AI景观的鲁棒检测系统。

Method: 两阶段检测框架：第一阶段使用监督对比学习训练的视觉深度学习模型提取判别性嵌入；第二阶段在学习的嵌入空间上使用k-NN分类器，采用少样本学习范式整合来自未见测试生成器的有限样本。

Result: 在少样本学习机制下仅使用每类150张图像，平均检测准确率达到91.3%，比现有方法提高5.2个百分点；在源归属任务中，AUC和OSCR分别提高了14.70%和4.27%。

Conclusion: 该框架在无需大量重新训练的情况下，显著推进了能够适应不断发展的生成AI景观的鲁棒、可扩展取证归属系统的开发。
...
TL;DR: 提出了一种鲁棒的多摄像机体操运动员跟踪方法，通过结合体操领域知识（运动员3D中心位于预设垂直平面）和级联数据关联策略，在交叉视角检测不足时使用射线-平面相交生成共面3D轨迹候选，解决了体操场馆摄像机数量有限和检测失败导致的跟踪挑战。


<details>
  <summary>Details</summary>
Motivation: 体操运动员跟踪面临独特挑战：场馆空间限制导致摄像机数量有限，光照变化、背景、服装和遮挡等因素可能导致某些视角检测失败，传统多摄像机三角测量难以准确确定3D轨迹。

Method: 提出级联数据关联范式：当交叉视角检测充足时使用三角测量生成3D轨迹候选，当检测不足时使用射线-平面相交方法生成共面3D轨迹候选，利用体操运动员3D中心位于预设垂直平面的领域知识。

Result: 通过广泛实验验证了方法的鲁棒性，在挑战性场景中优于现有方法。该跟踪方法已成功应用于最近的体操世锦赛，获得国际体操联合会的高度认可。

Conclusion: 结合体操领域知识和级联数据关联策略的多摄像机跟踪方法能够有效应对体操运动员跟踪的特殊挑战，提高了跟踪的准确性和鲁棒性，在实际应用中取得了显著成效。
...
Method: 实现了Horn-Schunck算法的多分辨率版本，使用双线性插值和延拓技术来提高精度和收敛性，同时比较了局部方法（如Lucas-Kanade）和全局方法（如Horn-Schunck）。

Result: 通过多分辨率策略改进了Horn-Schunck算法的准确性和收敛性能，研究了这些组合策略在不同图像条件下的运动估计效果。

Conclusion: 多分辨率版本的Horn-Schunck算法结合局部和全局方法的优势，能够有效提高光流计算的精度和鲁棒性。
...
TL;DR: EOGS++是一种针对卫星图像的新型3D高斯泼溅方法，可直接处理原始高分辨率全色数据，无需外部预处理，通过集成束调整和改进技术实现更高质量的重建。


<details>
  <summary>Details</summary>
Motivation: 扩展地球观测高斯泼溅(EOGS)框架，解决卫星图像处理中的预处理依赖问题，提高相机姿态估计精度和重建质量。

Method: 提出EOGS++方法，直接在原始高分辨率全色数据上操作，利用光流技术将束调整嵌入训练过程，引入早期停止和TSDF后处理等改进。
...
Method: 采用两阶段Faster R-CNN架构，改进平衡组Softmax框架缓解类别不平衡；探索度量学习生成特征嵌入，使用k-NN方法提升分类性能。

Result: 在LVISv1数据集上达到24.5%的mAP，超越之前24.0%的基准，特别在稀有类别上表现提升显著。

Conclusion: 提出的方法有效提升了长尾目标检测性能，度量学习和k-NN分类器对改善稀有类别识别效果明显。
...
<details>
  <summary>Details</summary>
Motivation: 解决在自然图像中进行3D物体重建的挑战，特别是在存在遮挡和场景杂乱的复杂环境中，利用上下文视觉线索提升重建效果。
...
Conclusion: 突破了3D数据的限制，将发布代码、模型权重、在线演示以及一个新的野外3D物体重建基准测试。
...
Method: 在重要性分数计算中利用教师模型的梯度信号，识别对任务性能和知识转移都至关重要的参数，然后进行一次性全局剪枝，最后使用稀疏感知的再训练来恢复精度。
...
TL;DR: 本文提出了SAM2S模型，用于增强SAM2在手术视频分割中的性能，通过构建SA-SV基准数据集和引入DiveMem、时间语义学习等技术，显著提升了长期跟踪能力和零样本泛化性能。
...
Method: 构建SA-SV基准数据集，提出SAM2S模型包含：DiveMem多样化记忆机制、时间语义学习、抗模糊学习技术，并在多源数据集上进行微调。

Result: SAM2S达到80.42平均J&F分数，比原始SAM2提升17.10分，比微调SAM2提升4.11分，同时保持68 FPS实时推理速度和强零样本泛化能力。

Conclusion: SAM2S通过专门设计的机制有效解决了手术视频分割的挑战，在保持实时性能的同时显著提升了分割精度和泛化能力。
...
TL;DR: AGU是一种用于低光照图像上采样的高效方法，能够同时优化多个图像质量特性，如降噪和增强锐度。
...
Method: 采用多参数优化方法，从少量样本图像对中学习低光照和明亮图像特征之间的关联，基于引导图像方法将特征从引导图像转移到目标图像。

Result: 实验表明AGU在低光照场景下优于现有最先进方法，能够使用低质量、低分辨率输入实时渲染高质量图像。

Conclusion: AGU通过多参数优化和机器学习方法有效解决了低光照图像上采样的挑战，实现了实时高质量图像渲染。
...
TL;DR: TRIM是一种后训练方法，通过时间和空间修剪策略加速3D高斯扩散模型的推理，无需重新训练即可提高效率和质量。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯扩散模型由于高斯基元数量庞大，导致去噪和后处理耗时，生成速度慢且采样轨迹扩展性有限。

Method: 提出TRIM方法，包含两个策略：1）轻量级选择器模型评估潜在高斯基元，实现早期轨迹缩减；2）实例掩码去噪，过滤冗余背景区域的高斯基元。

Result: 大量实验和分析表明，TRIM显著提高了3D生成的效率和质量。

Conclusion: TRIM在不影响输出质量的情况下加速了3D高斯扩散模型的推理，并支持推理时的缩放。
...
TL;DR: 本文对Cambrian-S视频世界模型进行了批判性分析，发现其基准测试VSR和VSC存在缺陷：VSR可通过简单的词袋模型近乎完美解决，而VSC的推理方法利用了基准中的捷径启发式而非真正的空间超感知能力。
...
Method: 提出了两个简单基准：NoSense（使用词袋SigLIP模型）和VSC-Repeat（通过视频重复拼接来测试系统稳定性），对Cambrian-S进行系统性分析。

Result: NoSense在VSR上达到95%准确率，表明该基准无需空间认知即可解决；VSC-Repeat测试使Cambrian-S准确率从42%降至0%，证明其依赖基准中的捷径而非真正的空间超感知。
...
Method: 基于PartField学习部件分解方法，结合高层语义部件分解和新颖几何启发式方法，采用自上而下的递归框架，确保每个图表的失真低于用户指定阈值，同时最小化图表总数。
...
TL;DR: TriDiff-4D是一个创新的4D生成管道，通过扩散模型的三平面重定位技术生成高质量、时间一致的4D虚拟形象，支持任意长度的4D序列生成，显著提升了生成效率和质量。


<details>
  <summary>Details</summary>
Motivation: 解决现有4D生成方法在时间几何一致性、感知伪影、运动不规则性、计算成本和动态控制方面的局限性，满足对高保真、可控4D虚拟形象的日益增长需求。

Method: 采用自回归策略和扩散模型，首先生成规范3D虚拟形象和运动序列，然后使用第二个扩散模型根据运动序列为虚拟形象添加动画，支持骨架驱动的4D生成。

Result: 实验结果表明TriDiff-4D显著优于现有方法，将生成时间从数小时减少到数秒，同时大幅改善了复杂运动的生成质量，具有高保真外观和准确的3D几何结构。

Conclusion: TriDiff-4D通过结合3D结构和运动先验知识，实现了高效、高质量的4D虚拟形象生成，在时间一致性、运动准确性、计算效率和视觉保真度方面表现出色。
...
TL;DR: V-ReasonBench是一个评估视频推理能力的基准测试，涵盖结构化问题解决、空间认知、模式推理和物理动力学四个维度，通过合成和真实图像序列构建，为视频模型提供可重复、可扩展的评估框架。
...
Method: 构建V-ReasonBench基准测试，包含合成和真实图像序列，设计可验证答案的任务，评估六个最先进的视频模型，并与强图像模型进行比较，分析幻觉行为和视频时长对推理的影响。

Result: 评估显示不同视频模型在结构化、空间、模式和物理推理维度上存在明显差异，发现了常见的幻觉行为，并研究了视频时长对链式帧推理的影响。

Conclusion: V-ReasonBench提供了一个统一可重复的框架来测量视频推理能力，旨在支持开发具有更可靠、与人类对齐的推理技能的模型。
...
TL;DR: 本文提出了Video-Next-Event Prediction (VNEP)任务，将视频作为下一个事件预测的答案模态，并开发了VANS模型，通过强化学习将视觉语言模型与视频扩散模型对齐，实现从文本预测转向视频展示的直观回答。


<details>
  <summary>Details</summary>
Motivation: 视频具有展示物理世界信息的能力，这些信息难以仅通过语言传达（如系领带的教学）。现有的事件预测任务主要使用文本回答，而视频作为答案模态尚未充分利用。

Method: 提出VANS模型，使用强化学习（Joint-GRPO）将视觉语言模型(VLM)与视频扩散模型(VDM)对齐。VLM生成准确且易于可视化的描述，VDM根据描述和输入视觉上下文生成视频。构建了VANS-Data-100K数据集用于训练。

Result: 在程序和预测基准测试中，VANS在视频事件预测和可视化方面达到了最先进的性能。

Conclusion: VNEP任务将事件预测从文本描述扩展到视频展示，VANS模型通过多模态对齐实现了更直观和定制化的回答，为程序性学习和创意探索提供了新途径。
...
TL;DR: 提出一种双模式思维视觉语言模型DualMindVLM，通过强化学习让模型根据任务难度自动切换快速思维和慢速思维模式，在保持高性能的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要追求冗长的推理链，导致计算成本过高。受人类双系统思维机制启发，希望让模型能够根据任务复杂度自动选择不同的思考模式。

Method: 采用两阶段方法：第一阶段根据模型输出长度标注数据为快速或慢速思维模式；第二阶段使用GRPO强化学习训练模型发展双模式思维能力。

Result: DualMindVLM显著超越基础模型，性能与最先进的视觉推理模型相当，同时保持极高的token效率。

Conclusion: 提出的简单RL方法成功实现了视觉语言模型的双模式思维，在保持高性能的同时大幅提升计算效率，验证了该方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖准确的相机姿态和人体姿态作为输入，但在实际应用中姿态估计往往存在噪声，导致重建质量下降。

Method: 提出NoPo-Avatar方法，仅从图像重建化身，完全消除对测试时姿态输入的依赖。

Result: 在THuman2.0、XHuman和HuGe100K数据集上的实验表明，NoPo-Avatar在实际设置（无真实姿态）中优于现有基线，在实验室设置（有真实姿态）中表现相当。

Conclusion: NoPo-Avatar通过消除对姿态输入的依赖，提高了重建方法的鲁棒性和实用性。
...
### [78] [Majority Rules: LLM Ensemble is a Winning Approach for Content Categorization](https://arxiv.org/abs/2511.15714)
...
Method: 提出eLLM集成框架，基于集体决策的数学模型建立原则性聚合标准，在IAB分层分类法上评估10个最先进的LLM，使用8,660个人工标注样本进行零样本测试。
...
### [79] [Graph-Memoized Reasoning: Foundations Structured Workflow Reuse in Intelligent Systems](https://arxiv.org/abs/2511.15715)
*Yash Raj Singh*

Main category: cs.AI

TL;DR: Graph-Memoized Reasoning框架通过图结构记忆来存储和重用推理工作流，减少重复计算，提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 现代大型语言模型系统经常在不同任务中重复计算相似的推理步骤，浪费计算资源、增加推理延迟并限制可重现性，需要持久化推理机制来回忆和重用先前的计算轨迹。

Method: 引入Graph-Memoized Reasoning框架，将推理工作流表示为图结构记忆，通过结构和语义相似性检索过去的决策图，实现跨新推理任务的子图组合重用。

Result: 提出了最小化总推理成本并正则化存储与生成工作流之间不一致性的优化目标，为智能系统中的效率-一致性权衡提供了理论基础。
...
### [80] [MACIE: Multi-Agent Causal Intelligence Explainer for Collective Behavior Understanding](https://arxiv.org/abs/2511.15716)
...
Result: 在四个多智能体强化学习场景（合作、竞争和混合动机）中评估显示：准确的结果归因（平均phi_i=5.07，标准差<0.05）、合作任务中检测到正向涌现（协同指数高达0.461）、高效计算（CPU上每个数据集0.79秒）。
...
### [81] [How Modality Shapes Perception and Reasoning: A Study of Error Propagation in ARC-AGI](https://arxiv.org/abs/2511.15717)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于指令的系统将网格转换为自然语言或DSL规则执行，但缺乏对编码如何影响模型感知的原则性理解，以及如何区分指令错误和执行错误。研究假设不同模态会施加感知瓶颈，从而影响网格特征的可靠感知。

Method: 使用加权集合分歧度量和两阶段推理流程，在九种文本和图像模态中分离感知与推理，比较不同表示方式对模型感知的影响。

Result: 结构化文本在稀疏特征上提供精确坐标，图像能捕捉2D形状但对分辨率敏感，结合文本和图像能提高执行准确性（约8个感知点；约0.20中位相似度）。
...
### [82] [Automated Hazard Detection in Construction Sites Using Large Language and Vision-Language Models](https://arxiv.org/abs/2511.15720)
*Islem Sahraoui*
...
Result: 尽管模型规模较小，但Molmo 7B和Qwen2 VL 2B在某些提示配置下表现出有竞争力的性能，证明了低资源多模态系统在规则感知安全监控中的可行性。
...
### [83] [Spatial Reasoning in Multimodal Large Language Models: A Survey of Tasks, Benchmarks and Methods](https://arxiv.org/abs/2511.15722)
*Weichen Liu,Qiyao Xue,Haoming Wang,Xiangyu Yin,Boyuan Yang,Wei Gao*

Main category: cs.AI

TL;DR: 本文从认知角度提出空间智能分类法，按推理复杂度组织任务，将现有基准映射到该分类法中，分析评估方法和改进策略。


<details>
  <summary>Details</summary>
Motivation: 现有调查通常基于输入模态分类，但空间能力不仅取决于输入格式。本文从认知角度组织空间智能，以进行更原则性的跨任务比较，揭示当前模型能力与人类推理之间的关键差距。

Method: 引入基于认知方面的空间智能分类法，按推理复杂度划分任务并关联认知功能；将文本、视觉语言和具身环境中的现有基准映射到该分类法；分析评估指标和方法论。

Result: 通过认知视角实现更原则性的跨任务比较，揭示了当前模型能力与人类推理之间的关键差距；分析了训练型和推理型改进方法的互补机制。

Conclusion: 该认知视角为研究人员提供了对该领域的全面理解，并为未来研究指明了可操作的方向，强调了空间推理作为人类智能基本方面的重要性。
...
### [84] [Uncertainty-Resilient Multimodal Learning via Consistency-Guided Cross-Modal Transfer](https://arxiv.org/abs/2511.15741)
*Hyo-Jeong Jang*
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习系统面临数据噪声、低质量标签和模态异质性带来的不确定性挑战，特别是在人机交互场景中，这些问题尤为突出。需要开发能够在不依赖大规模高质量标注的情况下提高语义鲁棒性和数据效率的方法。

Method: 采用一致性引导的跨模态迁移框架，利用跨模态语义一致性进行鲁棒表示学习。通过将异构模态投影到共享潜在空间来减小模态差距，并发现支持不确定性估计和稳定特征学习的结构关系。
...
### [85] [Multi-Agent LLM Orchestration Achieves Deterministic, High-Quality Decision Support for Incident Response](https://arxiv.org/abs/2511.15755)
*Philip Drammeh*

Main category: cs.AI

TL;DR: 多智能体编排相比单智能体方法在事件响应中实现了100%可操作建议率，比单智能体的1.7%提升了80倍行动特异性和140倍解决方案正确性，且质量零方差，使生产SLA承诺成为可能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在事件响应中承诺加速响应，但单智能体方法生成模糊、不可用的建议，需要提升建议质量和可靠性。

Method: 提出MyAntFarm.ai框架，通过348次对照试验比较单智能体副驾驶与多智能体系统在相同事件场景下的表现，引入决策质量新指标。

Result: 多智能体编排实现100%可操作建议率，单智能体仅1.7%；多智能体在行动特异性提升80倍，解决方案正确性提升140倍，且质量零方差。

Conclusion: 多智能体编排从性能优化转变为基于LLM的事件响应的生产就绪要求，所有代码和试验数据公开可用。
...
TL;DR: 本研究比较了基于规则的NLP方法和大型语言模型在从波兰医院电子健康记录中提取医疗信息的性能，发现规则方法在年龄和性别提取上更准确，而LLM在药物识别方面表现更好，并探讨了文本规范化和翻译的影响。
...
Method: 使用基于规则的低计算NLP方法和大型语言模型，从波兰医院的电子健康记录中提取患者人口统计信息、临床发现和处方药物，并评估文本规范化和翻译对信息提取的影响。
...
### [87] [Mini Amusement Parks (MAPs): A Testbed for Modelling Business Decisions](https://arxiv.org/abs/2511.15830)
...
TL;DR: 该论文提出了Mini Amusement Parks (MAPs)游乐园模拟器，用于评估智能体在复杂商业环境中的决策能力，发现人类在长期优化、空间推理等方面显著优于现有AI系统。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统在现实世界决策中面临多个相互关联的挑战，如开放多目标优化、稀疏经验学习、随机环境长期规划等，但现有基准测试无法全面评估这些能力。

Method: 开发了MAPs游乐园模拟器，统一了环境建模、长期不确定性规划、复杂业务运营等挑战，并提供了人类基准和最先进LLM智能体的全面评估。
...
### [88] [Step-Audio-R1 Technical Report](https://arxiv.org/abs/2511.15848)
...
TL;DR: Step-Audio-R1是首个成功的音频推理模型，通过模态基础推理蒸馏框架解锁音频领域的推理能力，在音频理解基准测试中超越Gemini 2.5 Pro，达到与Gemini 3 Pro相当的性能。


<details>
  <summary>Details</summary>
Motivation: 解决音频语言模型中存在的困惑现象：推理越少性能越好，探索音频智能是否能真正受益于深思熟虑的推理过程。

Method: 提出模态基础推理蒸馏框架，使模型学习生成与音频相关的推理链，真正基于声学特征而非产生不相关的推理过程。
...
### [89] [Decomposing Theory of Mind: How Emotional Processing Mediates ToM Abilities in LLMs](https://arxiv.org/abs/2511.15895)
*Ivan Chulo,Ananya Joshi*

Main category: cs.AI

TL;DR: 本文通过分析激活引导对语言模型心理理论能力的影响，发现情感处理而非分析推理是提升心理理论表现的关键机制。
...
Method: 使用线性探针分析45种认知行为，对比引导与基线模型在1000个BigToM前向信念场景中的激活差异，应用对比激活加法引导Gemma-3-4B模型。

Result: 信念归因任务准确率从32.5%提升至46.7%，这种改善主要由情感内容处理（情感感知+2.23，情感评价+2.20）驱动，同时抑制分析过程（质疑-0.78，收敛思维-1.59）。

Conclusion: 语言模型成功的心理论能力主要由情感理解而非分析推理所介导，情感处理在心理理论中扮演关键角色。
...
### [90] [Thinking, Faithful and Stable: Mitigating Hallucinations in LLMs](https://arxiv.org/abs/2511.15921)
...
<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在多步推理中出现的幻觉问题，不仅关注最终答案正确性，更强调中间推理步骤的可靠性和忠实性。

Method: 设计复合奖励函数，惩罚不合理的高置信度和熵峰值，鼓励稳定准确的推理轨迹；通过强化学习策略使模型更具自省能力，利用置信度感知的奖励反馈塑造生成行为。

Result: 实验表明该方法提高了最终答案准确性和推理校准度，消融实验验证了各信号组件的独立贡献。

Conclusion: 所提出的自校正框架能够有效改善大型语言模型在多步推理中的准确性和可靠性，提升中间推理步骤的连贯性和忠实性。
...
### [91] [JudgeBoard: Benchmarking and Enhancing Small Language Models for Reasoning Evaluation](https://arxiv.org/abs/2511.15958)
*Zhenyu Bi,Gaurav Srivastava,Yang Li,Meng Lu,Swastik Roy,Morteza Ziyadi,Xuan Wang*

Main category: cs.AI

TL;DR: JudgeBoard是一个新颖的评估流水线，直接查询模型来评估候选答案的正确性，无需额外答案比较。通过MAJ多智能体框架，使用多个交互的小型语言模型来近似大型语言模型的判断准确性。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型在判断答案正确性方面的能力与大型语言模型相比尚不明确，现有基于预定义指标的评估方法间接且难以完全自动化，限制了推理输出的细粒度和可扩展评估。

Method: 提出JudgeBoard评估流水线，在数学推理和科学/常识推理两个核心领域构建任务特定的评估排行榜，使用基于准确性的排名和Elo评分系统。提出MAJ多智能体判断框架，利用多个具有不同推理特征的小型语言模型通过协作审议来近似大型语言模型的判断准确性。

Result: 实验结果显示小型语言模型与大型语言模型在独立判断任务中存在显著性能差距。但MAJ框架显著提高了小型语言模型的可靠性和一致性。在MATH数据集上，使用较小模型作为骨干的MAJ表现与较大模型相当甚至更好。

Conclusion: 多智能体小型语言模型系统在判断任务中可能匹配或超越大型语言模型的性能，对可扩展和高效评估具有重要意义。
...
### [92] [CARE-RAG - Clinical Assessment and Reasoning in RAG](https://arxiv.org/abs/2511.15994)
*Deepthi Potluri,Aby Mammen Mathew,Jeffrey B DeWitt,Alexander L. Rasgon,Yide Hao,Junyuan Hong,Ying Ding*

Main category: cs.AI

TL;DR: 本文研究了大型语言模型在获取正确证据后仍无法正确推理的问题，特别是在临床环境中。使用书面暴露疗法指南作为测试平台，发现即使提供权威段落，模型错误仍然存在。提出了评估准确性、一致性和推理保真度的框架，强调需要像严格评估检索一样严格评估推理。


<details>
  <summary>Details</summary>
Motivation: 在临床环境中，大型语言模型获取正确证据后仍无法正确推理的问题尤为严重，因为输出必须符合结构化协议。这种检索与推理之间的差距需要被解决。

Method: 使用书面暴露疗法指南作为测试平台，评估模型对经过临床医生审查的问题的响应。提出一个评估框架来衡量准确性、一致性和推理保真度。

Result: 研究发现即使提供权威段落，模型错误仍然存在。检索增强生成可以约束输出，但安全部署需要像严格评估检索一样严格评估推理。

Conclusion: 检索增强生成有潜力约束模型输出，但在临床等关键环境中安全部署需要同等重视检索和推理的评估，确保模型能够正确使用提供的证据进行推理。
...
### [93] [SpellForger: Prompting Custom Spell Properties In-Game using BERT supervised-trained model](https://arxiv.org/abs/2511.16018)
*Emanuel C. Silva,Emily S. M. Salum,Gabriel M. Arantes,Matheus P. Pereira,Vinicius F. Oliveira,Alessandro L. Bicho*

Main category: cs.AI

TL;DR: SpellForger是一个游戏，玩家通过自然语言提示创建自定义法术，使用BERT模型解释提示并映射到法术模板，在Unity中开发，旨在验证AI作为直接游戏机制的使用。


<details>
  <summary>Details</summary>
Motivation: 探索AI作为核心游戏共创工具的应用，目前这方面研究不足，旨在提供个性化和创造性的独特游戏体验。
...
Conclusion: 该研究验证了AI作为直接游戏机制的可行性，为玩家提供了通过自然语言创造个性化游戏内容的创新方式。
...
### [94] [An Aligned Constraint Programming Model For Serial Batch Scheduling With Minimum Batch Size](https://arxiv.org/abs/2511.16045)
*Jorge A. Huertas,Pascal Van Hentenryck*

Main category: cs.AI

TL;DR: 提出了一种新的约束规划模型用于串行批处理调度问题，该模型无需预定义虚拟批次集合，通过关键对齐参数直接在机器上推理同家族作业序列，实现了更紧凑的建模。


<details>
  <summary>Details</summary>
Motivation: 现有CP模型依赖预定义虚拟批次集合，存在维度灾难问题并增加问题复杂性。在半导体制造等实际应用中需要考虑最小批次大小要求。

Method: 使用关键对齐参数直接推理机器上同家族作业序列，避免虚拟批次集合；通过定制搜索阶段和增强约束传播器推理级别来利用问题结构。

Result: 在近5000个实例上的计算实验表明，新模型在最多100个作业的小到中型实例上表现优越，在最多500个作业、10个家族、10台机器的大规模实例上能找到比现有方法好25%的解决方案。

Conclusion: 提出的新型CP模型在串行批处理调度问题中显著优于现有方法，特别是在处理最小批次大小约束时表现出色。
...
TL;DR: 提出了一种结合CNN LSTM时间序列预测和多智能体深度强化学习的混合架构，用于云边缘工作负载的主动资源管理，通过将预测信息嵌入DRL智能体状态空间实现更智能的长期决策。


<details>
  <summary>Details</summary>
Motivation: 传统云边缘工作负载资源管理过于被动，依赖静态阈值会导致资源过度配置或性能下降，需要转向主动解决方案。
...
Conclusion: 通过赋予AI管理器预见能力，系统能够制定长期计划，在节约成本与保持系统健康、应用快速之间找到最佳平衡点，实现平滑的资源管理路径。
...
### [96] [OpenMMReasoner: Pushing the Frontiers for Multimodal Reasoning with an Open and General Recipe](https://arxiv.org/abs/2511.16334)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态推理领域缺乏透明和可复现的数据整理和训练策略，这阻碍了可扩展的研究。

Method: 采用两阶段训练方法：SFT阶段使用874K样本的冷启动数据集进行逐步验证；RL阶段使用74K样本数据集进一步优化和稳定推理能力。
...
### [97] [SkyRL-Agent: Efficient RL Training for Multi-turn LLM Agent](https://arxiv.org/abs/2511.16108)
...
TL;DR: SkyRL-Agent是一个用于高效多轮长视野智能体训练和评估的框架，通过异步调度、轻量级工具集成和灵活后端互操作性，显著提升了训练效率。
...
Method: 采用优化的异步流水线调度器（1.55倍加速比），结合基于AST的代码搜索工具增强训练配方，支持与SkyRL-train、VeRL和Tinker等RL框架的无缝集成。

Result: 训练出的SA-SWE-32B模型在SWE-Bench Verified上达到39.4% Pass@1，相比达到类似性能的先前模型成本降低2倍以上，并能有效泛化到其他智能体任务。

Conclusion: SkyRL-Agent框架通过其高效调度和工具增强能力，显著提升了智能体训练效率和性能，同时具有良好的可扩展性和泛化能力。
...
### [98] [TOFA: Training-Free One-Shot Federated Adaptation for Vision-Language Models](https://arxiv.org/abs/2511.16423)
*Li Zhang,Zhongxuan Han,XiaoHua Feng,Jiaming Zhang,Yuyuan Li,Linbo Jiang,Jianan Lin,Chaochao Chen*

Main category: cs.AI

TL;DR: 提出了一种名为TOFA的无训练一次性联邦视觉语言模型适应框架，通过视觉和文本双管道提取任务相关表示，无需客户端或服务器端额外训练资源，有效应对数据异构性。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习中的视觉语言模型适应方法存在通信成本高、易受攻击的问题，而现有一次性方法在利用多模态信息、处理数据异构性和资源需求方面存在不足。

Method: TOFA采用视觉和文本双管道：视觉管道使用分层贝叶斯模型学习个性化的类特定原型分布；文本管道评估并全局对齐生成的本地文本提示。引入自适应权重校准机制结合两个模态的预测。

Result: 在9个数据集的各种联邦设置下进行广泛实验，证明了TOFA方法的有效性。

Conclusion: TOFA是一种高效、轻量级的无训练一次性联邦视觉语言模型适应框架，能够充分利用预训练模型的多模态特征，有效处理数据异构性，且无需额外训练资源。
...
### [99] [Multidimensional Rubric-oriented Reward Model Learning via Geometric Projection Reference Constraints](https://arxiv.org/abs/2511.16139)
*Yongnan Jin,Xurui Li,Feng Cao,Liucun Gao,Juanjuan Yao*

Main category: cs.AI

TL;DR: 本文提出了MR-RML框架，通过GPRC方法解决大语言模型在医疗实践中的对齐挑战，在权威医疗基准Healthbench上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在医疗实践中的关键对齐挑战：静态评估基准与动态临床认知需求脱节、难以适应多源医疗标准演变、传统奖励模型无法捕捉多维医疗质量标准。

Method: 提出MR-RML框架，包含三个核心创新：1）"维度-场景-学科"医疗标准系统；2）独立多维奖励模型；3）几何投影参考约束，将医疗认知逻辑转化为数学正则化。

Result: 在Healthbench基准测试中，相比基础模型Qwen-32B，在完整子集上提升45%，在困难子集上提升85%，在开源大语言模型中达到最先进水平。

Conclusion: MR-RML框架有效解决了医疗大语言模型的对齐问题，显著提升了模型在权威医疗基准上的表现，为医疗AI的实际应用提供了可行方案。
...
### [100] [FOOTPASS: A Multi-Modal Multi-Agent Tactical Context Dataset for Play-by-Play Action Spotting in Soccer Broadcast Videos](https://arxiv.org/abs/2511.16183)
*Jeremie Ochin,Raphael Chekroun,Bogdan Stanciulescu,Sotiris Manitsaris*

Main category: cs.AI

TL;DR: 该论文介绍了FOOTPASS数据集，这是首个用于足球比赛全程多模态、多智能体战术背景下play-by-play动作识别的基准数据集，旨在通过结合计算机视觉输出和足球战术知识来生成可靠的play-by-play数据流。


<details>
  <summary>Details</summary>
Motivation: 当前足球视频理解方法在构建可靠的play-by-play数据方面仍不足够，通常只能辅助而非完全自动化标注。而战术建模、轨迹预测和性能分析等研究都需要基于比赛状态和play-by-play数据，因此需要利用战术知识作为先验来支持基于计算机视觉的预测。

Method: 引入FOOTPASS数据集，支持开发利用计算机视觉任务输出（如跟踪、识别）和足球战术知识（包括长期战术规律）的球员中心动作识别方法。

Result: 创建了首个用于足球比赛全程play-by-play动作识别的多模态、多智能体战术基准数据集，为数据驱动的体育分析提供基础。

Conclusion: FOOTPASS数据集能够促进更自动化和可靠的play-by-play数据提取，这对于数据驱动的体育分析至关重要，通过整合计算机视觉和战术知识来改进足球视频理解。
...
### [101] [From Performance to Understanding: A Vision for Explainable Automated Algorithm Design](https://arxiv.org/abs/2511.16201)
*Niki van Stein,Anna V. Kononova,Thomas Bäck*

Main category: cs.AI

TL;DR: 本文提出可解释的自动化算法设计愿景，通过结合LLM驱动的算法发现、可解释基准测试和问题类别描述符，形成闭环知识循环，从盲目搜索转向可解释的类特定算法设计。
...
Result: 形成发现、解释和泛化相互加强的闭环知识循环，产生关于优化策略何时及为何成功的可重用科学见解。

Conclusion: 将自动化与系统性基准测试的理解相结合，将使领域从盲目搜索转向可解释的类特定算法设计，加速进展同时产生科学洞察。
...
### [102] [ChemLabs on ChemO: A Multi-Agent System for Multimodal Reasoning on IChO 2025](https://arxiv.org/abs/2511.16205)
*Xu Qiang,Shengyuan Bai,Leqing Chen,Zijing Liu,Yu Li*

Main category: cs.AI

TL;DR: ChemO是一个基于国际化学奥林匹克竞赛的新基准，通过评估等效重构和结构化视觉增强解决化学问题自动化评估的挑战。ChemLabs多智能体框架显著提升性能，达到93.6分，超过人类金牌水平。


<details>
  <summary>Details</summary>
Motivation: 数学和物理的奥林匹克级基准已用于测试AI推理能力，但化学因其独特的多模态符号语言一直是个挑战。需要开发专门基准来评估AI在复杂化学问题解决中的能力。

Method: 1. ChemO基准：采用评估等效重构将需要视觉输出的问题转换为可计算格式，结构化视觉增强分离视觉感知与化学推理能力。2. ChemLabs多智能体框架：模拟人类专家协作，包含问题分解、感知、推理和审计等专门智能体。

Result: 在先进多模态模型上的实验表明，结构化视觉增强与多智能体系统结合带来显著性能提升。最佳配置获得93.6分（满分100），超过估计的人类金牌阈值，建立了化学问题自动解决的新最先进水平。
...
### [103] [FlipVQA-Miner: Cross-Page Visual Question-Answer Mining from Textbooks](https://arxiv.org/abs/2511.16216)
*Zhen Hao Wong,Jingwen Deng,Hao Liang,Runming He,Chengyu Shen,Wentao Zhang*

Main category: cs.AI

TL;DR: 提出自动化流水线从教育文档中提取高质量问答对，结合布局感知OCR和基于LLM的语义解析，为LLM训练提供真实教育内容的可扩展替代方案。


<details>
  <summary>Details</summary>
Motivation: 现有指令调优和RL数据集成本高昂且依赖合成样本，存在幻觉和多样性有限问题，而教材和练习材料包含大量高质量人类编写的问答内容未被充分利用。

Method: 结合布局感知OCR和基于大语言模型的语义解析，构建自动化流水线从教育文档PDF中提取格式良好的QA和视觉QA对。
...
### [104] [MuISQA: Multi-Intent Retrieval-Augmented Generation for Scientific Question Answering](https://arxiv.org/abs/2511.16283)
*Zhiyuan Li,Haisheng Yu,Guangchuan Guo,Nan Zhou,Jiajun Zhang*

Main category: cs.AI

TL;DR: 本文提出了Multi-Intent Scientific Question Answering (MuISQA)基准，用于评估RAG系统在多意图科学问答中的异构证据覆盖能力，并提出了一种意图感知检索框架来提升多意图问题的证据覆盖和检索准确性。


<details>
  <summary>Details</summary>
Motivation: 复杂科学问题通常包含多个意图，需要从不同来源获取证据并进行多跳推理，而传统的RAG系统通常是单意图导向的，导致证据覆盖不完整。

Method: 提出意图感知检索框架：利用LLM假设潜在答案，将其分解为意图特定查询，为每个底层意图检索支持段落，然后通过互惠排名融合(RRF)聚合和重新排序检索到的片段，以平衡不同意图的覆盖范围并减少冗余。

Result: 在MuISQA基准和其他通用RAG数据集上的实验表明，该方法在检索准确性和证据覆盖方面持续优于传统方法。

Conclusion: 该方法能有效解决多意图科学问答中的证据覆盖问题，显著提升RAG系统在复杂科学问题上的性能。
...
### [105] [Reducing Instability in Synthetic Data Evaluation with a Super-Metric in MalDataGen](https://arxiv.org/abs/2511.16373)
*Anna Luiza Gomes da Silva,Diego Kreutz,Angelo Diniz,Rodrigo Mansilha,Celso Nobre da Fonseca*

Main category: cs.AI

TL;DR: 提出了一种名为Super-Metric的综合评估指标，用于解决Android恶意软件合成数据质量评估的不稳定性和缺乏标准化问题。


<details>
  <summary>Details</summary>
Motivation: Android恶意软件领域合成数据质量评估存在不稳定性和缺乏标准化的问题，现有指标无法提供一致的评估结果。

Method: 在MalDataGen中集成了Super-Metric，该指标聚合了四个保真度维度的八个指标，生成单一加权分数。
...
### [106] [Pharos-ESG: A Framework for Multimodal Parsing, Contextual Narration, and Hierarchical Labeling of ESG Report](https://arxiv.org/abs/2511.16417)
*Yan Chen,Yu Zou,Jialei Zeng,Haoran You,Xiaorui Zhou,Aixi Zhong*

Main category: cs.AI

TL;DR: Pharos-ESG是一个统一框架，通过多模态解析、上下文叙述和分层标注将ESG报告转化为结构化表示，解决了ESG报告阅读顺序混乱和层次结构不明确的问题。
...
Conclusion: Pharos-ESG框架能够有效解决ESG报告的结构化处理问题，为金融治理和决策中的ESG整合提供更好的支持。
...
### [107] [From generative AI to the brain: five takeaways](https://arxiv.org/abs/2511.16432)
*Claudius Gros*
...
<details>
  <summary>Details</summary>
Motivation: 研究生成式AI的生成原则在大脑中的潜在应用，以及机器学习研究对神经信息处理系统的启示，推动认知神经科学的发展。

Method: 通过讨论五个具体例子（世界建模的缺陷、思维过程生成、注意力机制、神经缩放定律和量化）来阐述机器学习研究对神经科学的潜在贡献。

Result: 识别出机器学习研究中的多个关键概念（如注意力、缩放定律等）可能对理解大脑信息处理机制有重要启示。

Conclusion: 神经科学可以从机器学习研究中学习很多，特别是生成原则和神经信息处理系统的特征化，这有助于深化对大脑认知功能的理解。
...
### [108] [PersonaDrift: A Benchmark for Temporal Anomaly Detection in Language-Based Dementia Monitoring](https://arxiv.org/abs/2511.16445)
*Joy Lai,Alex Mihailidis*

Main category: cs.AI

TL;DR: PersonaDrift是一个用于评估机器学习方法检测痴呆症患者日常沟通行为渐进变化的合成基准，模拟了基于真实患者访谈的60天交互日志，重点关注情感扁平化和语义漂移两种变化模式。


<details>
  <summary>Details</summary>
Motivation: 痴呆症患者的沟通行为会随时间发生渐进变化，但现有计算工具大多无法跟踪这种长期行为漂移。需要开发能够检测这些细微变化的方法来帮助护理人员。

Method: 基于护理人员访谈创建合成用户模型，模拟60天交互日志，注入渐进的情感扁平化和语义漂移变化。评估了异常检测方法、统计方法、序列模型和监督分类器。

Result: 情感扁平化在基线变异性低的用户中可用简单统计模型检测，而语义漂移需要时序建模和个性化基线。个性化分类器在所有任务中均优于通用分类器。

Conclusion: 个性化方法对于检测痴呆症患者沟通行为变化至关重要，不同变化模式需要不同的检测策略，PersonaDrift为这一新兴应用领域提供了可扩展的评估框架。
...
### [109] [Utilizing Large Language Models for Zero-Shot Medical Ontology Extension from Clinical Notes](https://arxiv.org/abs/2511.16548)
...
<details>
  <summary>Details</summary>
Motivation: 临床笔记作为富含详细患者观察的非结构化文档，提供了有价值的上下文特定见解，是本体扩展的有前景但未充分利用的来源。直接利用临床笔记进行本体扩展仍然很大程度上未被探索。
...
Conclusion: CLOZE框架成功展示了利用大型语言模型从临床笔记中自动扩展医学本体的可行性，为生物医学研究和临床应用提供了高效、隐私保护的解决方案。
...
### [110] [Consciousness in Artificial Intelligence? A Framework for Classifying Objections and Constraints](https://arxiv.org/abs/2511.16582)
*Andres Campero,Derek Shiller,Jaan Aru,Jonathan Simon*

Main category: cs.AI

TL;DR: 本文开发了一个分类框架，用于对数字人工智能系统意识可能性的挑战进行分类，区分挑战的粒度层级（基于Marr层级）和力度程度（从质疑计算功能主义到声称数字意识严格不可能）。


<details>
  <summary>Details</summary>
Motivation: 为数字人工智能系统意识可能性的辩论提供结构和工具，澄清对计算功能主义的挑战与对数字意识的挑战之间的区别，以及解析这些挑战的不同方式。

Method: 开发了一个分类学框架，包含粒度层级（基于Marr层级）和力度程度（三个级别），并将此框架应用于科学和哲学文献中的14个典型案例。

Result: 成功创建了一个系统性的分类框架，能够清晰区分不同挑战的意图层级和论证力度，为相关辩论提供了结构化的分析工具。

Conclusion: 该框架有助于在数字意识辩论中提供结构，澄清不同类型的挑战，但不旨在对辩论本身采取立场，而是为理解和分类相关论证提供方法论工具。
...
### [111] [You Only Forward Once: An Efficient Compositional Judging Paradigm](https://arxiv.org/abs/2511.16600)
*Tianlong Zhang,Hongwei Xue,Shilin Yan,Di Wu,Chen Xu,Yunyun Yang*

Main category: cs.AI

TL;DR: YOFO是一种基于模板的多模态大语言模型判断方法，通过单次前向传播同时验证所有结构化要求，实现数量级的速度提升，同时保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM判断方法面临基本权衡：适配MLLM输出单一分数与其生成性质不一致且限制细粒度要求理解，而自回归生成判断分析在高吞吐量场景下速度过慢。

Method: 提出YOFO方法，基于自回归模型接受结构化需求模板，在单次推理步骤中通过读取与每个要求相关的最终标记的logits，为每个要求生成二元是/否决策。

Result: 大量实验表明，YOFO不仅在标准推荐数据集上达到最先进结果，还支持依赖感知分析（后续判断基于先前判断），并能从后验思维链中获益。

Conclusion: YOFO通过将判断简化为验证输入是否满足结构化要求集，在保持可解释性的同时实现了数量级的速度提升，解决了MLLM判断方法的基本权衡问题。
...
### [112] [Bridging VLMs and Embodied Intelligence with Deliberate Practice Policy Optimization](https://arxiv.org/abs/2511.16602)
...
TL;DR: DPPO是一个元认知训练框架，通过动态交替监督微调（能力扩展）和强化学习（技能精炼）来解决具身智能中的数据瓶颈和算法效率问题，在有限数据下实现高效学习。
...
Method: 引入Deliberate Practice Policy Optimization (DPPO)框架，采用元认知"Metaloop"训练方法，动态交替监督微调和强化学习，自动识别弱点并针对性分配资源。
...
### [113] [MedBayes-Lite: Bayesian Uncertainty Quantification for Safe Clinical Decision Support](https://arxiv.org/abs/2511.16625)
*Elias Hossain,Md Mehedi Hasan Nipu,Maleeha Sheikh,Rajib Rana,Subash Neupane,Niloofar Yousefi*

Main category: cs.AI

TL;DR: MedBayes-Lite是一个轻量级的贝叶斯增强框架，用于基于transformer的临床语言模型，旨在产生可靠、不确定性感知的预测，无需重新训练即可集成不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 尽管transformer在临床决策支持中表现出强大潜力，但在模糊的医疗案例中容易过度自信，而校准的不确定性至关重要。

Method: 框架包含三个组件：贝叶斯嵌入校准（使用蒙特卡洛dropout）、不确定性加权注意力（对token可靠性进行边际化）和置信度引导决策塑造（受临床风险最小化启发）。

Result: 在生物医学QA和临床预测基准测试中，MedBayes-Lite持续改善校准和可信度，将过度自信减少32%至48%。在模拟临床环境中，通过标记不确定预测供人工审查，可预防高达41%的诊断错误。
...
### [114] [Enhancing Forex Forecasting Accuracy: The Impact of Hybrid Variable Sets in Cognitive Algorithmic Trading Systems](https://arxiv.org/abs/2511.16657)
*Juan C. King,Jose M. Amigo*

Main category: cs.AI

TL;DR: 本文实现了一个基于人工智能的算法交易系统，专门用于EUR-USD货币对的高频外汇交易，整合了基本面和技术面特征，并比较了两类特征的预测能力。


<details>
  <summary>Details</summary>
Motivation: 开发一个结合基本面和技术面分析的AI交易系统，以确定在EUR-USD货币对的高频交易中，哪类输入特征能提供更可靠和盈利的交易信号。

Method: 整合欧元区和美国的关键宏观经济变量（如GDP、失业率）作为基本面特征，以及技术指标（如振荡器、斐波那契水平、价格背离）作为技术面特征，构建AI算法交易系统，并通过标准机器学习指标和回测模拟评估性能。

Result: 使用标准机器学习指标量化预测准确性，并通过历史数据回测模拟评估交易盈利能力和风险。
...
### [115] [Cognitive Foundations for Reasoning and Their Manifestation in LLMs](https://arxiv.org/abs/2511.16660)
...
TL;DR: 该论文提出了一个包含28个认知元素的分类法，分析了17个模型和人类在推理过程中的行为差异，发现人类使用层次嵌套和元认知监控，而模型依赖浅层前向链式推理。研究开发了测试时推理指导方法，将复杂问题性能提升高达60%。
...
Method: 综合认知科学研究构建28个认知元素的分类法，分析17个模型17万条推理轨迹和54条人类思考轨迹，进行大规模行为分析，并开发测试时推理指导方法。

Result: 发现系统性结构差异：人类使用层次嵌套和元认知监控，模型依赖浅层前向链式推理。元分析显示研究社区关注可量化行为而忽视元认知控制。开发的推理指导方法将复杂问题性能提升高达60%。

Conclusion: 通过连接认知科学和LLM研究，为开发基于原则性认知机制的模型奠定基础，为改进模型能力和大规模测试人类认知理论开辟新方向。
...
### [116] [Mind the Motions: Benchmarking Theory-of-Mind in Everyday Body Language](https://arxiv.org/abs/2511.15887)
...
<details>
  <summary>Details</summary>
Motivation: 现有心智理论基准主要关注错误信念任务和不对称信息推理，忽视了信念之外的其他心理状态以及丰富的非语言交流。

Method: 利用专家策划的身体语言参考作为知识库，构建包含精细非语言线索标注和人工验证心理解释的视频数据集。

Result: 当前AI系统在非语言线索解释方面表现显著不足，在检测方面存在巨大性能差距，在解释方面相比人类标注者表现出过度解释模式。

Conclusion: Motion2Mind框架揭示了AI在理解非语言线索心智状态方面的局限性，为评估机器心智理论能力提供了新基准。
...
### [117] [Liars' Bench: Evaluating Lie Detectors for Language Models](https://arxiv.org/abs/2511.16035)
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型谎言检测技术通常在狭窄的环境中验证，无法捕捉模型可能生成的各种谎言类型，因此需要更全面的测试平台。

Method: 创建LIARS' BENCH测试平台，包含72,863个谎言和诚实回答示例，涵盖四种开源模型和七个数据集，从两个维度区分谎言类型：模型说谎的原因和谎言针对的信念对象。

Result: 评估三种黑白盒谎言检测技术后发现，现有技术系统性地无法识别某些类型的谎言，特别是在仅凭对话记录无法判断模型是否说谎的情况下。
...
### [118] [Early science acceleration experiments with GPT-5](https://arxiv.org/abs/2511.16072)
...
TL;DR: GPT-5在多个科学领域（数学、物理、天文学、计算机科学、生物学、材料科学）中展示了加速研究进程的能力，提供了新的研究思路并帮助解决了四个数学问题，但人类专家的参与仍然是关键。


<details>
  <summary>Details</summary>
Motivation: 展示前沿AI（如GPT-5）在科学研究中的实际应用价值，帮助科学家了解AI的能力，并促进人机协作的研究模式。

Method: 通过多个跨学科案例研究，记录科学家与GPT-5的互动过程，分析AI在哪些方面加速了研究，哪些方面仍需人类输入。

Result: GPT-5在多个科学领域产生了具体的研究进展，特别是在数学领域帮助解决了四个先前未解决的问题，验证了AI在科学研究中的实用性。

Conclusion: 前沿AI已成为科学研究的有价值工具，能够加速研究进程并产生新发现，但人类专家的指导和验证仍然是不可或缺的，人机协作模式具有广阔前景。
...
### [119] [ELPO: Ensemble Learning Based Prompt Optimization for Large Language Models](https://arxiv.org/abs/2511.16122)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的性能高度依赖精心设计的提示，但手动提示工程耗时费力。现有自动提示优化方法多采用单一模型或算法，在处理复杂任务时性能受限。

Method: 提出ELPO框架，采用集成学习思想，结合投票机制、共享生成策略和不同搜索方法来搜索更优提示，并设计了更高效的提示生成和搜索算法。

Result: 实验结果表明ELPO在不同任务上优于最先进的提示优化方法，如在ArSarcasm数据集上F1分数提高了7.6分。

Conclusion: ELPO框架通过集成学习方法有效提升了自动提示优化的性能，为复杂任务提供了更准确和鲁棒的解决方案。
...
TL;DR: 本文提出了一种新的参数高效微调范式TS-PEFT，通过选择性对部分位置索引应用PEFT修改，挑战了传统对所有索引应用修改的方法。


<details>
  <summary>Details</summary>
Motivation: 质疑传统PEFT方法对所有位置索引应用修改的必要性，认为这种不加选择的应用可能多余甚至适得其反。

Method: 引入Token-Selective PEFT (TS-PEFT)范式，使用函数S选择性对位置索引子集应用PEFT修改。
...
### [121] [SemanticCite: Citation Verification with AI-Powered Full-Text Analysis and Evidence-Based Reasoning](https://arxiv.org/abs/2511.16198)
*Sebastian Haan*

Main category: cs.CL

TL;DR: SemanticCite是一个AI驱动的系统，通过全文分析验证引用准确性，提供详细推理和文本片段，解决语义引用错误、AI生成幻觉引用等学术文献挑战。


<details>
  <summary>Details</summary>
Motivation: 学术文献面临语义引用错误、AI生成幻觉引用以及传统引用格式无法指向具体支持段落的问题，需要准确验证引用以支持科学交流。

Method: 结合多种检索方法和四分类系统（支持、部分支持、不支持、不确定），使用微调的轻量级语言模型进行全文源分析。

Result: 实验显示微调的轻量级模型性能与大型商业系统相当，但计算需求显著降低，使大规模引用验证实际可行。系统提供透明、基于证据的解释。
...
### [122] [SeSE: A Structural Information-Guided Uncertainty Quantification Framework for Hallucination Detection in LLMs](https://arxiv.org/abs/2511.16275)
*Xingtao Zhao,Hao Peng,Dingli Su,Xianghua Zeng,Chunyang Liu,Jinzhi Liao,Philip S. Yu*

Main category: cs.CL

TL;DR: 本文提出了语义结构熵(SeSE)框架，从结构信息角度量化大语言模型的语义不确定性，用于幻觉检测。该方法通过构建自适应稀疏有向语义图，利用层次抽象挖掘潜在语义结构信息，在29个模型-数据集组合上显著优于现有不确定性量化方法。
...
Method: 开发自适应稀疏有向语义图构建算法，捕捉方向性语义依赖关系并自动剪除引入负干扰的不必要连接。通过层次抽象利用潜在语义结构信息，将SeSE定义为最优语义编码树的结构熵，形式化语义空间在最优压缩后的内在不确定性。

Result: 在29个模型-数据集组合上的广泛实验表明，SeSE显著优于先进的不确定性量化基线方法，包括强监督方法和最近提出的KLE方法。

Conclusion: SeSE框架通过挖掘语义结构信息，为大语言模型提供了可靠的不确定性量化方法，能够有效检测幻觉，在安全关键应用中具有重要价值。
...
### [123] [SDA: Steering-Driven Distribution Alignment for Open LLMs without Fine-Tuning](https://arxiv.org/abs/2511.16324)
...
Method: SDA框架基于用户定义的对齐指令动态重新分配模型输出概率，是轻量级、资源高效的，兼容各种开源LLMs，可独立运行或与基于训练的对齐策略集成。

Result: 在8个不同规模和来源的开源LLMs上评估，SDA在帮助性、无害性和诚实性三个关键对齐维度上表现一致提升，平均增益分别为64.4%、30%和11.5%。

Conclusion: SDA是一种有效且通用的对齐方法，能够跨不同模型和应用场景提升LLMs的对齐性能，支持个性化偏好对齐。
...
TL;DR: 本文提出了一种自重写框架，通过让模型重写自己的推理文本并从中学习，来改善大型推理模型的内部推理过程质量，解决了传统强化学习中仅关注最终正确性的奖励机制导致的推理质量问题。
...
Method: 提出选择性重写方法，仅对模型持续正确的"简单"样本进行重写，保持GRPO的所有原始奖励信号；在单一批次中同时进行重写和原始生成，保持强化学习算法的可扩展性，仅增加约10%的开销。

Result: 在多种任务和不同模型规模上的实验表明，自重写方法在准确率-长度权衡方面，准确率提高0.6%，推理长度减少46%；在内部推理质量方面，LLM-as-a-judge指标得分显著提高7.2分，成功缓解了内部推理缺陷。

Conclusion: 自重写框架有效提升了大型推理模型的内部推理过程质量，在保持强化学习可扩展性的同时显著改善了推理准确性和效率。
...
### [125] [NLP Datasets for Idiom and Figurative Language Tasks](https://arxiv.org/abs/2511.16345)
*Blake Matheny,Phuong Minh Nguyen,Minh Le Nguyen,Stephanie Reynolds*

Main category: cs.CL

TL;DR: 本文创建了大规模习语和比喻语言数据集，用于评估预训练语言模型在习语识别任务中的表现，并通过槽位标注和序列标注任务进行训练和评估。
...
Method: 使用现有习语数据集获取习语列表，从大型语料库中检索上下文序列，创建了一个大规模潜在习语数据集和两个人工标注的确切习语数据集，并进行后处理以适应模型无关训练。

Result: 创建了用于评估预训练语言模型在习语识别任务中表现的数据集，并在槽位标注和序列标注任务中进行了训练和评估。

Conclusion: 提出的数据集为解决习语和比喻语言处理问题提供了一个答案，同时为构建新模型和开发新方法提供了多样化的类别基础。
...
### [126] [Learning from Sufficient Rationales: Analysing the Relationship Between Explanation Faithfulness and Token-level Regularisation Strategies](https://arxiv.org/abs/2511.16353)
*Jonathan Kamp,Lisa Beinborn,Antske Fokkens*

Main category: cs.CL

TL;DR: 本文探讨了自然语言解释（rationales）的充分性指标局限性，通过将其与两种建模范式（token分类和注意力正则化）关联，发现高信息量rationales不一定有助于正确分类，充分性实际上反映了非rationalized上下文的分类影响。


<details>
  <summary>Details</summary>
Motivation: 现有rationales的充分性指标对模型性能影响提供有限洞察，需要更深入理解rationales信息如何影响模型表现。

Method: 将充分性指标与token分类能力和通过注意力正则化在输入中融入rationales的方法进行关联分析。

Result: 发现高信息量rationales不一定帮助正确分类；充分性反映非rationalized上下文的干扰作用；融入rationales信息能提升跨域分类但结果不一致；充分性与token分类无关。

Conclusion: rationales的复杂性表明需要开发能系统捕捉此类信息的指标进行进一步研究。
...
### [127] [AICC: Parse HTML Finer, Make Models Better -- A 7.3T AI-Ready Corpus Built by a Model-Based HTML Parser](https://arxiv.org/abs/2511.16397)
...
TL;DR: MinerU-HTML是一种基于语言模型的HTML到文本提取管道，通过序列标注方法显著提升了网页内容提取质量，特别是在结构化元素（如代码、公式、表格）的保留方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有网页语料库主要依赖基于启发式的提取器（如Trafilatura），这些方法难以保留文档结构并经常损坏结构化元素。作者假设提升提取质量对下游性能的影响可能与积极过滤策略同样重要。

Method: 将内容提取重新定义为序列标注问题，使用0.6B参数的语言模型解决。采用两阶段格式化管道，先对语义元素进行显式分类，再转换为Markdown格式。

Result: 在包含7,887个标注网页的MainWebBench基准测试中，MinerU-HTML达到81.8% ROUGE-N F1，显著优于Trafilatura的63.6%。在结构化元素保留方面表现优异（代码块90.9%，公式94.0%）。使用MinerU-HTML构建的AICC语料库在13个基准测试中平均准确率达到50.8%，优于Trafilatura提取的语料库。

Conclusion: HTML提取是网页语料库构建中一个关键但常被低估的组成部分，基于模型的提取方法相比启发式方法具有更好的可扩展性和性能提升空间。
...
### [128] [Classification of worldwide news articles by perceived quality, 2018-2024](https://arxiv.org/abs/2511.16416)
*Connor McElroy,Thiago E. A. de Oliveira,Chris Brogly*

Main category: cs.CL

TL;DR: 本研究探索了监督机器学习和深度学习模型是否能有效区分感知低质量新闻文章与感知高质量新闻文章，使用141万+新闻文章数据集，发现传统机器学习分类器和深度学习模型都能有效区分新闻质量。


<details>
  <summary>Details</summary>
Motivation: 探索机器学习和深度学习模型在区分新闻文章质量方面的有效性，以帮助自动识别低质量新闻内容。

Method: 使用3种机器学习分类器和3种深度学习模型，基于新创建的1,412,272篇英文新闻文章数据集，通过专家共识评级将新闻源网站分为低质量和高质量两类，每篇文章提取194个语言特征。

Result: 随机森林等传统机器学习分类器表现良好（准确率0.7355，ROC AUC 0.8131），而深度学习模型中ModernBERT-large表现最佳（准确率0.8744，ROC-AUC 0.9593，F1 0.8739），其他深度学习模型也有优异表现。

Conclusion: 研究结果表明，传统CPU机器学习分类器和深度学习分类器都能有效区分全球新闻文章的感知质量，为自动新闻质量评估提供了可行方案。
...
### [129] [ESGBench: A Benchmark for Explainable ESG Question Answering in Corporate Sustainability Reports](https://arxiv.org/abs/2511.16438)
*Sherine George,Nithish Saji*

Main category: cs.CL

TL;DR: ESGBench是一个用于评估可解释ESG问答系统的基准数据集和评估框架，包含基于企业可持续发展报告的多主题问题、人工标注答案和证据支持，用于精细评估模型推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门评估ESG（环境、社会和治理）领域可解释问答系统的基准，需要促进透明和负责任的ESG导向AI系统研究。

Method: 构建包含多ESG主题的领域基础问题集，配以人工标注的答案和证据支持，建立评估框架分析最先进LLM在事实一致性、可追溯性和领域对齐方面的表现。

Result: 分析了最先进大语言模型在ESGBench上的表现，揭示了在事实一致性、可追溯性和领域对齐方面的关键挑战。

Conclusion: ESGBench旨在加速透明和负责任ESG导向AI系统的研究，为评估可解释ESG问答系统提供了标准化基准。
...
Method: 采用轻量级模型设计，仅占用6.6 GiB存储空间，可在A10 GPU（24GB内存）等资源受限硬件上部署，支持处理长达125页的A4文档。

Result: 模型在文档理解任务中表现出强大的性能，能够在资源受限环境下高效处理长文档，实现结构化数据提取。

Conclusion: Arctic-Extract证明了在保持最先进性能的同时，可以在资源受限硬件上部署轻量级文档理解模型的可行性，为实际应用提供了实用解决方案。
...
### [132] [TurkColBERT: A Benchmark of Dense and Late-Interaction Models for Turkish Information Retrieval](https://arxiv.org/abs/2511.16528)
*Özay Ezerceli,Mahmoud El Hussieni,Selva Taş,Reyhan Bayraktar,Fatma Betül Terzioğlu,Yusuf Çelebi,Yağız Asker*

Main category: cs.CL

TL;DR: 本文介绍了TurkColBERT，这是首个针对土耳其语检索的密集编码器和延迟交互模型的综合基准测试。研究发现延迟交互模型在参数效率上显著优于密集编码器，其中ColmmBERT-base-TR在特定领域任务上实现了+13.8%的mAP提升。
...
Result: 结果显示强参数效率：1.0M参数的colbert-hash-nano-tr比600M参数的turkish-e5-large密集编码器小600倍，同时保留了超过71%的平均mAP。延迟交互模型比密集编码器小3-5倍但显著优于它们。MUVERA+Rerank比PLAID快3.33倍，并提供+1.7%的相对mAP增益。

Conclusion: 延迟交互模型在土耳其语检索中表现出优异的参数效率和性能，为低延迟检索提供了可行方案。但存在依赖中等规模数据集和翻译基准的局限性，可能需要更大规模的MUVERA评估。
...
### [133] [Beyond Tokens in Language Models: Interpreting Activations through Text Genre Chunks](https://arxiv.org/abs/2511.16540)
...
Method: 使用Mistral-7B模型，基于其激活状态预测输入文本的体裁，采用scikit-learn分类器进行训练和评估。
...
### [134] [WER is Unaware: Assessing How ASR Errors Distort Clinical Understanding in Patient Facing Dialogue](https://arxiv.org/abs/2511.16544)
...
<details>
  <summary>Details</summary>
Motivation: 自动语音识别在临床对话中部署日益增多，但标准评估仍严重依赖词错误率(WER)，需要验证WER是否与转录错误的临床影响相关。

Method: 建立专家临床医生标注的黄金标准基准，比较真实话语与ASR生成版本；引入LLM-as-a-Judge，使用GEPA进行程序化优化以复制专家临床评估。

Result: 分析显示WER和现有指标与临床风险标签(无影响、最小影响、显著影响)相关性差；优化的评估器(Gemini-2.5-Pro)达到与人类相当的性能，准确率90%，Cohen's κ为0.816。
...
### [135] [Integrating Symbolic Natural Language Understanding and Language Models for Word Sense Disambiguation](https://arxiv.org/abs/2511.16577)
*Kexin Zhao,Ken Forbus*

Main category: cs.CL

TL;DR: 提出一种基于统计语言模型的词义消歧方法，无需人工标注训练数据，通过将符号NLU系统生成的候选含义转换为可区分的自然语言替代项，利用LLM根据上下文选择合适解释。
...
### [136] [Comparison of Text-Based and Image-Based Retrieval in Multimodal Retrieval Augmented Generation Large Language Model Systems](https://arxiv.org/abs/2511.16654)
...
TL;DR: 本文比较了多模态RAG系统中的两种检索方法：基于文本分块检索和直接多模态嵌入检索，发现直接多模态嵌入检索在金融收益电话基准测试中显著优于基于LLM摘要的方法。
...
Method: 对两种检索方法进行综合比较分析：文本分块检索（图像先被摘要成文本）和直接多模态嵌入检索（图像以原生方式存储在向量空间中），在6个LLM模型和2个多模态嵌入模型上进行评估。

Result: 直接多模态嵌入检索显著优于基于LLM摘要的方法，在mAP@5上实现13%的绝对提升（相对提升32%），在nDCG@5上实现11%的绝对提升（相对提升20%），并能产生更准确和事实一致的答案。

Conclusion: LLM摘要会在预处理阶段引入信息损失，而直接多模态嵌入能够保留视觉上下文用于检索和推理，在多模态RAG系统中具有更好的实际应用价值。
