...
TL;DR: 本文提出了一种新的无分类器指导方法——鞍点自由指导（SFG），它利用对数密度估计的正曲率来指导基于分数的生成模型，无需额外训练或标记数据，在无条件图像生成中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于分数生成模型的指导方法（如Classifier-Free Guidance和Auto-Guidance）需要标记数据或训练额外模型，这限制了它们在无标记数据或无法训练新模型场景下的应用。作者发现对数密度估计在鞍点区域的正曲率可以提供有效的指导信号。

Method: 提出鞍点自由指导（SFG）方法，通过维护对数密度的最大正曲率估计来指导单个基于分数的模型。该方法与无分类器指导计算成本相同，无需额外训练，可直接应用于现成的扩散模型和流匹配模型。

Result: 在无条件ImageNet-512生成中，SFG实现了最先进的FID和FD-DINOv2指标。与Auto-Guidance结合时，其无条件样本在FD-DINOv2得分上达到通用最先进水平。在FLUX.1-dev和Stable Diffusion v3.5实验中，SFG在保持优秀提示遵循和图像保真度的同时，提高了输出图像的多样性。

Conclusion: SFG是一种有效的无分类器指导方法，利用对数密度的正曲率特性，无需额外训练或标记数据，为基于分数的生成模型提供了强大的指导能力，在多个基准测试中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 手工构建铰接式3D物体成本高且难以扩展，而现有方法多为多阶段流程。需要一种能够从单张图像直接生成完整铰接3D物体的端到端解决方案。

Method: 1. 建立统一的潜在表示，联合编码几何、纹理、部件分割和运动学参数；2. 引入可逆的关节到体素嵌入，将运动特征与体积几何空间对齐；3. 将关节类型预测公式化为开放集问题，无需固定关节语义，可泛化到新关节类别。

Result: 在PartNet-Mobility基准测试中，UniArt在网格质量和关节精度方面达到了最先进的性能。

Conclusion: UniArt提供了一个从单张图像直接生成铰接3D物体的端到端框架，通过统一表示和开放集关节预测，能够处理新颖的关节类别和未见过的物体类型。
...
Conclusion: 该框架能够有效整合全切片图像和不完整的基因组学数据，为精准肿瘤学提供了灵活且强大的多模态分析方法。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中，从少量未标定且部分遮挡的视图重建3D物体是一个常见但具有挑战性的问题，传统多视角或修复方法在遮挡严重时往往产生不完整或几何不一致的重建结果。

Method: 提出AmodalGen3D框架，整合2D模态补全先验与多视角立体几何条件，采用视图间交叉注意力机制进行稀疏视图特征融合，以及立体条件交叉注意力模块进行未观测结构推断，联合建模可见和隐藏区域。
...
TL;DR: 论文提出了TAPVid-360任务，要求预测视频序列中查询场景点的3D方向，即使这些点远在观察视频的狭窄视野之外，旨在促进学习非自我中心场景表示。


<details>
  <summary>Details</summary>
Motivation: 人类能够构建全景心理模型，保持物体恒常性并推断视野外场景结构，而当前人工智能视觉系统在持久、全景理解方面存在局限，特别是在TAP任务中无法跟踪视野外的2D点。

Method: 利用360度视频作为监督源，将其重采样为窄视野视角，同时使用2D流程在全景中跟踪点来计算真实方向。提出了TAPVid360-10k数据集和基准，并基于CoTracker v3的基线方法预测每点旋转进行方向更新。

Result: 提出的基线方法在预测视野外点的3D方向方面优于现有的TAP和TAPVid 3D方法，展示了在非自我中心场景表示学习方面的有效性。

Conclusion: TAPVid-360任务通过利用360度视频监督，无需动态4D真实场景模型训练，就能促进学习非自我中心场景表示，为计算机视觉系统实现更人类化的全景理解能力提供了新途径。
...
TL;DR: WalkCLIP：一种多模态框架，整合卫星影像、街景图像和人口动态数据来预测城市步行性，相比单模态方法在准确性和空间对齐方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 传统步行性评估方法成本高、难以扩展，现有单源方法（卫星影像、街景图像、人口指标）各自存在局限性：卫星数据缺乏行人视角，街景图像缺乏空间背景，人口动态不反映环境视觉形态。需要整合这些互补视角来全面评估步行环境。
...
Conclusion: 视觉和行为信号的整合能够可靠地预测步行环境，多模态方法比单一数据源方法更能全面捕捉城市步行性的多维特征。
...
TL;DR: 该研究在包含4000张内窥镜图像的新型胃肠道医学影像数据集上进行了全面的模型比较分析，VGG16和MobileNetV2模型在四种关键疾病分类中均达到96.5%的测试准确率，并采用Grad-CAM可视化增强临床可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决内窥镜图像分析中常见的挑战，如光照变化、相机角度波动和成像伪影，同时推动胃肠道计算机辅助诊断的发展，强调多样化临床相关数据集和模型可解释性在医学AI研究中的重要性。

Method: 使用最先进的深度学习技术，在包含4000张内窥镜图像的数据集上对四种关键疾病（憩室病、肿瘤、腹膜炎、输尿管）进行分类，采用VGG16、MobileNetV2和Xception等模型进行对比分析，并集成Grad-CAM可视化技术提供可解释的AI。

Result: VGG16和MobileNetV2模型均达到96.5%的测试准确率，Xception模型达到94.24%的准确率，建立了稳健的基准和基线，实验结果表明即使在复杂的真实世界条件下也能实现稳健、准确和可解释的医学图像分析。

Conclusion: 该研究通过提供原始基准、比较见解和视觉解释，推进了胃肠道计算机辅助诊断的发展，强调了多样化临床相关数据集和模型可解释性在医学AI研究中的关键作用，展示了在复杂真实条件下实现稳健、准确和可解释医学图像分析的潜力。
...
TL;DR: PAT3D是首个结合视觉语言模型与物理模拟的物理增强文本到3D场景生成框架，能生成物理合理、模拟就绪且无交叠的3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有文本到3D场景生成方法缺乏物理合理性，生成的场景经常存在物体交叠和不稳定等问题，无法直接用于下游任务如机器人操作。

Method: 1) 基于文本提示生成3D物体并推断空间关系，组织成层次化场景树；2) 转换为模拟初始条件，使用可微分刚体模拟器确保重力下的真实物体交互；3) 引入模拟循环优化过程，保证物理稳定性和无交叠，同时提升语义一致性。

Result: PAT3D在物理合理性、语义一致性和视觉质量方面显著优于现有方法，并能生成可直接用于场景编辑和机器人操作等下游任务的模拟就绪3D场景。

Conclusion: PAT3D首次实现了物理增强的文本到3D场景生成，通过集成视觉语言模型与物理模拟，解决了现有方法在物理合理性方面的局限性，为下游应用提供了高质量的模拟就绪场景。
...
TL;DR: 本文提出了一个大规模指针仪表读数数据集RPM-10K和基于物理关系注入的视觉语言模型MRLM，用于解决指针仪表精确读数识别中的反射、遮挡、视角变化等挑战。
...
Method: 首先构建了包含10730张仪表图像的大规模基准数据集RPM-10K。基于此数据集，提出了基于物理关系注入的视觉语言模型MRLM，通过显式编码指针与刻度之间的几何和因果关系，将感知与物理推理对齐。采用交叉注意力融合和自适应专家选择机制，使模型能够解释表盘配置并生成精确数值读数。

Result: 在新提出的基准数据集上进行了大量实验，充分验证了所提出框架的有效性。数据集和源代码将在GitHub上发布。

Conclusion: 本文通过构建大规模数据集和提出基于物理关系注入的视觉语言模型，有效解决了指针仪表读数识别中的关键挑战，为智能电力系统中的精确仪表读数识别提供了鲁棒的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 文本提示的基础模型在医学图像分割中缺乏空间精度且易受域偏移影响，而视觉提示模型需要精确的边界框提示但临床实践中难以获取。需要一种方法将弱文本信号转化为强视觉提示

Method: PPBoost使用视觉语言模型基于文本描述生成初始伪边界框，通过不确定性感知标准过滤不可靠预测，用保留的图像-边界框对训练伪标签检测器生成高质量边界框，推理时进一步扩展边界框以紧密覆盖目标结构，最终用增强的边界框提示引导分割模型生成密集掩码

Result: 在三个涵盖不同模态和解剖结构的数据集上，PPBoost在Dice系数和归一化表面距离指标上持续优于文本和视觉提示基线，甚至超越了不使用标注数据的少样本分割模型，并能泛化到多种典型的视觉分割模型骨干

Conclusion: PPBoost成功地将弱文本提示转化为强空间引导，在零样本条件下显著提升了医学图像分割的性能，为临床实践中难以获取精确视觉提示的问题提供了有效解决方案
...
TL;DR: 该研究针对多模态大语言模型在实时交互式指导方面的不足，提出了Qualcomm Interactive Cooking基准数据集和LiveMamba模型，专注于烹饪任务中的错误检测和实时反馈。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然具备对话能力，但缺乏实时交互式逐步指导的能力，无法在用户执行任务时检测错误并提供及时反馈，这限制了AI助手在实时指导场景中的应用。

Method: 基于CaptainCook4D数据集构建Qualcomm Interactive Cooking基准，包含用户执行任务时的错误数据，并开发LiveMamba模型——一个专为交互式教学指导设计的流式多模态大语言模型。

Result: 创建了首个专注于实时情境指导的基准数据集，包含密集标注的定时指令和反馈信息，特别是精确时间戳标注的错误提醒，为评估多模态LLM的实时指导能力提供了标准。

Conclusion: 该研究填补了多模态LLM在实时交互式指导领域的空白，提供了专门的评估基准和强大的基线模型，为开发实时情境指导系统奠定了基础。
...
Method: 提出了全面的加速管道，包括：1）使用新速度场进行批处理；2）异构时间步批处理的向量化；3）为新方法进行动态TensorRT编译。这些方法从理论、设计和推理策略三个层面全面加速基于流模型的生成模型。
...
Method: 1. 引入离策略专家指导，将专家视觉搜索轨迹转化为结构化行为信号；2. 设计凝视引导推理导航器（GRN），通过双模式探索策略模拟诊断过程；3. 开发置信度值采样器（CVS），平衡专家模仿和自主发现；4. 采用双流GRPO优化框架，分离在线和离策略学习信号。

Result: 在多个医学VQA基准测试中，MedEyes实现了平均+8.5%的性能提升，验证了其在构建可解释医学AI系统方面的潜力。

Conclusion: MedEyes通过动态建模临床医生风格的诊断推理，结合渐进式视觉关注和区域解释，有效提升了医学视觉问答的准确性和临床对齐性，为构建可解释医学AI系统提供了有前景的框架。
...
Method: 通过特征投影结合多元高斯分布，测量类别内的视觉特征一致性，创建类别特定的概率嵌入，实现无需训练的后处理不确定性估计。

Result: 在ImageNet、Flowers102、Food101、EuroSAT和DTD数据集上的实验显示，该方法在错误检测性能上达到最先进水平，显著优于确定性和概率性VLM基线。

Conclusion: 该方法具有VLM无关性、无需微调、对分布偏移具有鲁棒性，且每个类别仅需10个训练图像即可有效工作，提高了视觉语言模型在安全关键应用中的可靠性。
...
TL;DR: 该论文研究了直接从音频指令进行物体定位（音频-视觉对齐），而不依赖文本转录，发现这种方法在某些情况下甚至优于基于转录的方法，特别是在处理语言多样性方面更鲁棒。


<details>
  <summary>Details</summary>
Motivation: 当前基于文本转录的物体定位方法存在效率和鲁棒性问题，作者质疑这种中间转录的必要性，探索能否实现直接的音频-视觉对齐。

Method: 通过简化任务（专注于单词语音指令），创建了包含多种物体和不同人类口音的音频定位数据集，并适配和基准测试了多个音频-视觉领域的模型。
...
Method: 在频域学习适应图像风格以减少源域和目标域之间的差异，仅引入轻量级预处理模块在训练时使用，推理时完全丢弃该模块，不增加额外计算开销。

Result: 在域自适应目标检测任务上进行了广泛实验，在多个基准测试中取得了显著的性能提升，验证了方法的实用性和有效性。

Conclusion: 提出的频域图像风格适应方法是一种简单而有效的无监督域自适应解决方案，无需复杂架构或对抗训练，在保持推理效率的同时显著提升了跨域性能。
...
Method: 采用基于注意力机制的Transformer架构，使用稀疏占用表示，绕过中间BEV投影和显式几何先验，直接从原始图像特征进行端到端的多帧未来占用预测。

Result: 在nuScenes基准测试中，1-3秒占用预测达到最先进性能，显著优于现有方法。模型表现出强大的场景动态理解能力，在任意未来轨迹条件下保持高精度。

Conclusion: 提出的端到端Transformer架构通过避免离散标记化和BEV表示的结构限制，能够更有效地捕捉时空依赖，在3D场景占用预测任务上实现了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有真实图像超分辨率方法通常使用文本条件扩散模型的生成先验进行正则化，但这存在两个关键问题：概念上与任务不匹配（超分辨率应直接基于低质量图像生成高质量图像），以及实践中教师模型常产生颜色失真和边缘模糊，表明生成先验不适合此任务。

Method: 提出图像条件流形正则化（ICM），将输出正则化到基于稀疏但关键结构信息（色图和Canny边缘的组合）的条件流形上，避免了直接使用原始输入图像的高信息密度导致的不稳定性，同时提供了任务对齐的稳定正则化信号。

Result: 实验证实所提出的正则化方法显著提升了超分辨率性能，特别是在感知质量方面，证明了其在真实世界应用中的有效性。

Conclusion: ICM方法通过使用图像结构信息而非文本条件来正则化超分辨率输出，解决了概念错位和数值不稳定问题，显著提高了真实图像超分辨率的质量，特别是感知质量。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在多个医学领域展现出巨大潜力，但牙科领域仍未被充分探索，主要原因是缺乏领域特定数据、牙科专家标注稀缺、模态特定建模不足以及可靠性挑战。

Method: 1. 构建TRACE-CoT数据集，模拟牙科放射科医生的临床推理过程；2. 提出四阶段训练范式；3. 开发MMOral-Uni基准测试，包含2,809个开放性问题-答案对，涵盖五种模态和五个任务。
...
Method: WorldWander基于先进的视频扩散变换器，集成了两种关键技术：(1) 上下文视角对齐，用于建模跨视角同步；(2) 协作位置编码。此外，还构建了EgoExo-8K数据集，包含合成和真实场景中同步的第一人称-第三人称视频三元组。

Result: 实验表明，WorldWander在视角同步、角色一致性和泛化能力方面表现优异，为第一人称-第三人称视频转换设立了新的基准。

Conclusion: WorldWander通过创新的上下文学习框架，成功解决了第一人称和第三人称视角之间的视频转换问题，在跨视角同步方面取得了显著进展，为相关应用领域提供了有力工具。
...
Method: 1. 设计多级表示提取器，从每个模态中捕获点级别和生态位级别的表示；2. 引入分层双曲对齐模块，在双曲空间中统一这些表示，执行空间对齐的同时分层构建图像和基因嵌入。

Result: HyperST在来自不同组织的四个公共数据集上实现了最先进的性能，为更可扩展和准确的空间转录组学预测铺平了道路。

Conclusion: 通过建模数据的固有层次结构并在双曲空间中学习多级图像-基因表示，HyperST能够有效桥接模态差距，显著提高跨模态预测性能。
...
TL;DR: PROMPTMINER是一个黑盒提示词窃取框架，通过强化学习优化主体重建和模糊测试搜索风格修饰符，在多个数据集上超越现有方法，具有更好的泛化能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像生成模型的普及，精心设计的提示词已成为有价值的数字资产，但面临着安全性和知识产权风险。提示词窃取攻击既能用于恶意目的，也能支持数据归属、模型溯源等有益应用。现有方法存在白盒假设、需要大规模标注数据或仅依赖标注等局限性。

Method: PROMPTMINER采用两阶段黑盒框架：1）基于强化学习的优化阶段，通过策略梯度方法重建图像的主要主体；2）模糊测试驱动的搜索阶段，通过随机变异和评估来恢复风格修饰符。

Result: 在多个数据集和扩散模型上，PROMPTMINER取得了最佳性能：CLIP相似度最高达0.958，SBERT文本对齐度达0.751，均超越所有基线方法。对未知生成器的真实图像，CLIP相似度比最强基线高出7.5%，展现出更好的泛化能力。在防御性扰动下仍保持强健性能。

Conclusion: PROMPTMINER是一个有效且实用的黑盒提示词窃取框架，通过两阶段方法成功解决了现有方法的局限性，在准确性、泛化能力和鲁棒性方面均表现出色，为提示词安全和相关应用提供了有力工具。
...
TL;DR: 本文提出了一种即插即用的耦合提示学习框架，通过引入外部监督提示来缓解视觉语言模型在视频任务微调中的语义空间窄化问题，从而提升模型的泛化能力。
...
Method: 提出耦合提示学习框架：1）文本提示方面，引入其他数据集的预训练提示作为硬提示标记，与软提示标记拼接并通过可学习的映射层耦合，形成竞争性提示以防止语义空间过拟合到监督类别；2）引入精心设计的无关视频集和负提示作为通用属性锚点，保持预训练语义空间中属性的通用相关性。
...
TL;DR: DualVLA通过双阶段训练策略解决VLA模型在获得推理能力后出现的动作性能退化问题，采用数据剪枝和双教师蒸馏方法，在保持推理能力的同时提升动作性能。


<details>
  <summary>Details</summary>
Motivation: 当前构建通用视觉-语言-动作模型时，先训练专家模型再融合多模态数据恢复推理能力的策略会导致动作性能退化（动作退化现象），需要在保持推理能力的同时提升动作执行精度。

Method: 提出DualVLA框架：1）双层级数据剪枝方法去除冗余的具身推理数据，防止其对动作学习产生负面影响；2）双教师自适应蒸馏策略，为不同数据域分配不同的监督信号；3）提出VLA Score评估框架，将VLA能力解耦为推理、意图、动作和对齐四个维度进行细粒度评估。
...
TL;DR: EASL提出了一种多情感引导的手语生成架构，通过情感-语义解耦模块和渐进式训练，在保持语义准确性的同时融入精细的情感表达，生成具有情感识别能力的手语视频。
...
Conclusion: EASL成功解决了现有手语生成方法缺乏情感表达的问题，通过情感-语义解耦和渐进式训练实现了精细的情感整合，为聋人社区提供了更自然、更具表现力的手语交流方式。
...
TL;DR: 该研究开发了一个基于堆叠集成学习的深度学习模型，用于膝关节骨关节炎的自动检测和严重程度分级，相比传统人工X光片分析方法，提高了诊断准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 膝关节骨关节炎（KOA）是一种常见的肌肉骨骼疾病，尤其影响老年人日常活动。目前主要通过X光片分析并使用Kellgren-Lawrence（KL）分级系统（0-4级）评估严重程度，这种方法需要专业知识、耗时且容易受主观判断影响，导致诊断不准确。

Method: 开发了一个堆叠集成模型，包含两个分类任务：1）二元分类器检测KOA是否存在；2）多类分类器进行KL精确分级。模型采用多种预训练架构（MobileNetV2、YOLOv8、DenseNet201）作为基础学习器，使用Categorical Boosting（CatBoost）作为元学习器。

Result: 提出的堆叠集成模型在多类分类中达到73%的平衡测试准确率，在二元分类中达到87.5%的准确率，这些结果高于现有文献中的先前工作。

Conclusion: 该研究成功开发了一个基于深度学习的自动诊断系统，能够有效检测膝关节骨关节炎并进行严重程度分级，相比传统人工方法具有更高的准确性和效率，有望减少主观诊断误差并提高临床工作效率。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术广泛应用于3D重建，但存在计算成本攻击漏洞，会导致恶意资源占用和拒绝服务攻击，威胁系统可靠部署

Method: 提出两阶段防御框架：1)检测器识别受攻击的输入图像；2)净化器从受攻击图像中恢复良性图像，并采用对抗训练增强防御效果

Result: 实验证明该框架能有效防御白盒、黑盒和自适应攻击，在安全性和实用性方面达到最先进性能

Conclusion: RemedyGS是首个全面有效的黑盒防御方案，能保护3D高斯泼溅重建系统免受计算成本攻击，确保系统可靠部署
...
Result: IMTalker在运动准确性、身份保持和音频-嘴唇同步方面超越先前方法，达到最先进质量，在RTX 4090 GPU上实现视频驱动40 FPS和音频驱动42 FPS的高效运行
...
TL;DR: 该研究针对东亚地区PM浓度长期预测的挑战，提出了GRPO框架，通过类别奖励和课程学习优化预测可靠性，显著降低误报率。


<details>
  <summary>Details</summary>
Motivation: 东亚地区复杂地形和强大气动力学使PM浓度长期预测困难，现有基础模型缺乏区域特异性且依赖非实时数据，无法满足公共卫生预警需求。同时，标准点预测目标无法反映操作成本的不对称性（误报损害公众信任，漏报危及生命）。

Method: 1. 构建并发布东亚地区的真实观测和高分辨率CMAQ-OBS数据集；2. 提出Group-Relative Policy Optimization (GRPO)框架，包含类别奖励和课程学习策略，使预测与操作优先级对齐；3. 在48-120小时预测场景中验证方法。

Result: 1. 新数据集将区域误差降低59.5%；2. GRPO框架相比SFT基线将误报率降低47.3%，同时保持竞争力的F1分数；3. 实现了对公共卫生预警至关重要的实时48-120小时预测能力。

Conclusion: 该研究通过构建区域特定数据集和引入GRPO优化框架，显著提高了东亚地区PM浓度长期预测的可靠性，为实际空气质量预报系统提供了有效的解决方案，特别适用于长提前期场景下的公共卫生决策支持。
...
TL;DR: PS-CBM是一个部分共享的概念瓶颈模型框架，通过整合LLM语义与视觉示例、基于激活模式合并概念、以及引入概念效率准确度指标，解决了现有CBM的视觉基础差、概念冗余和缺乏平衡指标的问题。
...
Method: PS-CBM包含三个核心组件：1) 整合LLM语义与示例视觉线索的多模态概念生成器；2) 基于激活模式合并概念以平衡特异性和紧凑性的部分共享概念策略；3) 联合捕捉预测准确性和概念紧凑性的概念效率准确度(CEA)后验指标。

Result: 在11个多样化数据集上的实验表明，PS-CBM持续优于最先进的CBM，分类准确性提高1.0%-7.4%，CEA提高2.0%-9.5%，同时需要显著更少的概念。

Conclusion: PS-CBM在实现高准确性和强可解释性方面表现出有效性，为概念瓶颈模型提供了更优的解决方案。
...
TL;DR: GRiP是一个两阶段训练框架，通过引导模型的感知焦点和逻辑路径来培养稳健灵活的视觉基础推理能力，在多个挑战性基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态AI模型在"用图像思考"方面面临困境：端到端强化学习不稳定，监督微调又过于僵化，导致模型要么难以学习，要么缺乏处理复杂真实场景所需的认知灵活性。

Method: GRiP采用两阶段训练框架：1) 认知增强的强化学习阶段，包含两个关键创新：显著性加权IoU奖励（激励模型定位关键对象而非琐碎干扰物）和多启发式奖励（鼓励多样化但逻辑有效的推理路径）；2) 基于Qwen2.5-VL-7B模型初始化。

Result: GRiP在多个挑战性基准测试中表现出显著性能提升，在TreeBench和V* Bench等高难度测试中取得了开源模型中的最先进结果，证明了其在复杂视觉推理中的有效性。

Conclusion: 超越简单奖励机制，采用认知启发的信号来引导模型"看什么"和"如何思考"，对于解锁下一代多模态智能至关重要。代码将公开提供。
...
TL;DR: MTR-VP：基于视觉的轨迹规划方法，使用ViT编码器学习图像上下文嵌入替代地图特征，结合运动预测框架和规划意图输入，在Waymo数据集上评估多轨迹预测性能


<details>
  <summary>Details</summary>
Motivation: 传统轨迹规划方法依赖地图特征，本文旨在探索使用视觉特征替代地图特征的可能性，通过图像学习上下文嵌入，结合运动预测框架实现更灵活的轨迹规划

Method: 提出MTR-VP方法：1）使用ViT编码器处理原始图像和过去运动状态，生成上下文嵌入；2）基于MTR框架，用交叉注意力机制替代可学习意图查询，将意图与上下文嵌入结合；3）在Waymo端到端驾驶数据集上评估，进行消融实验分析不同组件影响

Result: 实验表明：1）基于Transformer的方法在结合视觉特征和运动特征方面效果有限，即使使用CLIP和DINOv2等基础模型增强场景表示；2）预测多轨迹分布而非单一轨迹能显著提升规划性能

Conclusion: 视觉特征可以替代地图特征用于轨迹规划，但Transformer架构在融合多模态特征方面存在局限，多轨迹预测策略比单一轨迹预测更有效，为基于视觉的自动驾驶规划提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 足部接触在人与世界互动中至关重要，但现有方法通常使用零速度约束近似接触，只关注关节级接触，无法捕捉足部与世界之间的详细交互。密集足部接触估计对于准确建模这种交互很关键，但从单张RGB图像预测密集足部接触仍未被充分探索。

Method: 提出FEet COntact estimation (FECO)框架，采用鞋款风格对抗训练实现鞋款风格不变的特征学习，同时引入地面特征提取器基于空间上下文捕捉地面属性。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶仿真方法在大视角变化下难以支持新视角合成，且缺乏几何一致性，需要更真实可控的仿真来推进端到端自动驾驶发展。

Method: 提出混合仿真框架HybridWorldSim，集成多轨迹神经重建用于静态背景，结合生成建模用于动态智能体，实现统一设计。

Result: HybridWorldSim超越现有最先进方法，提供可靠视觉和空间一致性，并发布了多城市、多路线、多环境条件的MIRROR数据集。

Conclusion: HybridWorldSim为高保真自动驾驶仿真提供了实用可扩展的解决方案，是自动驾驶研发的宝贵资源。
...
TL;DR: 提出两种创新方法解决文本反转在3D对象生成中的局限性：使用现成图像适配器避免额外训练，以及深度条件预热策略提升3D一致性。


<details>
  <summary>Details</summary>
Motivation: 现有3D对象生成方法主要依赖文本到图像扩散模型结合文本反转技术，但这种方法需要额外训练时间且缺乏控制能力，限制了实际应用效果。

Method: 提出两种创新方法：1) 使用现成的图像适配器，无需文本反转即可生成3D对象，提供对深度、姿态和文本等条件的增强控制；2) 深度条件预热策略，提升3D一致性。

Result: 实验结果显示，该方法在定性和定量评估中与现有基于文本反转的方法表现相当，同时显著提升了3D一致性。用户研究进一步证实了该方法在匹配输入图像和保持3D一致性方面的优越性。

Conclusion: 提出的方法有效解决了文本反转在3D对象生成中的局限性，通过图像适配器和深度条件预热策略实现了更好的控制能力和3D一致性，为3D生成任务提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的图像编辑方法在独立编辑多视角图像时，会导致同一场景不同视角之间的几何和光度不一致，这在编辑3D表示（如NeRF或高斯泼溅模型）时尤为严重。

Method: 提出训练免费的扩散框架，通过一致性损失引导扩散采样实现一致编辑。核心假设是未编辑图像中的对应点在编辑后应经历相似的变换。该框架灵活，可与多种图像编辑方法结合，支持密集和稀疏多视角编辑设置。

Result: 实验结果表明，相比现有多视角编辑方法，该方法显著提高了3D一致性。同时，这种增强的一致性使得高斯泼溅编辑能够获得锐利的细节和与用户指定文本提示的高度保真度。

Conclusion: 该方法为3D场景编辑提供了有效的多视角一致性解决方案，无需额外训练即可与现有编辑方法结合，显著提升编辑结果的3D一致性质量。
...
TL;DR: 提出M3LLM医学多图像多模态大语言模型，利用生物医学文献中的复合图像解决医疗MLLM多图像理解的数据瓶颈问题，显著超越现有模型在多图像分析任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有医学多模态大语言模型大多局限于单图像理解，无法满足临床实践中需要综合多图像（不同模态或时间点）进行诊断的需求，主要障碍是缺乏大规模高质量标注训练数据。

Method: 利用生物医学文献中许可允许的复合图像作为数据源，设计五阶段上下文感知指令生成范式，采用分治策略将多图像分析分解为可管理的子任务，解析超过237,000个复合图像及其上下文文本构建训练数据。

Result: 开发出M3LLM模型，在构建的PMC-MI-Bench基准测试中显著优于通用和专业医学MLLM，在多图像、单图像、纯文本和多选择场景均表现优异，特别在MIMIC数据集纵向胸部X光分析中展现出强泛化能力。

Conclusion: 建立了一个可扩展且高效的医学MLLM开发范式，能够进行复合推理，弥合了生物医学文献与现实临床应用之间的差距，为医疗多图像分析提供了新解决方案。
...
Method: 提出IE-SRGS新范式，联合利用外部2DSR先验和内部3DGS特征的互补优势。使用2DSR和深度估计模型生成HR图像和深度图作为外部知识，采用多尺度3DGS模型生成跨视图一致、领域自适应的对应物作为内部知识。引入掩码引导融合策略整合这两个来源，协同利用它们的互补优势，有效指导3D高斯优化实现高保真重建。
...
TL;DR: VessQC是一个开源工具，通过整合不确定性图谱指导用户高效修正3D显微镜图像分割错误，将错误检测召回率从67%提升到94%
...
Method: 开发VessQC开源工具，整合不确定性图谱来引导用户关注最可能包含生物学意义错误的区域，实现不确定性指导的3D分割修正

Result: 初步用户研究表明，不确定性指导的修正将错误检测召回率从67%显著提升到94.0%（p=0.007），且总修正时间没有显著增加
...
Method: 采用对抗扰动使SAM无法"看见"图像内容，将图像转化为神经模型的"空白画布"。引入频率感知优化策略来彻底欺骗SAM，增强篡改定位能力

Result: 大量实验结果表明该方法在篡改检测方面具有有效性

Conclusion: 通过使SAM"失明"的新策略，实现了对篡改区域的有效检测，为图像伪造检测提供了创新思路
...
TL;DR: 提出TTSnap框架，通过噪声感知剪枝在测试时扩展中提前剔除低质量候选，避免完全去噪的计算开销，提高效率


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法需要探索多个噪声种子并选择奖励最高的结果，但每个候选都需要完全去噪才能计算奖励，计算成本高昂，限制了可探索的样本数量

Method: 提出噪声感知剪枝框架TTSnap，通过自蒸馏训练噪声感知奖励模型，使中间估计的奖励与最终干净图像的奖励对齐；采用课程训练策略稳定不同噪声水平的学习；引入奖励对齐和计算预算利用的新指标

Result: 实验表明，相比现有方法性能提升超过16%，实现更高效有效的测试时扩展；与训练后技术和局部测试时优化技术结合时能获得正交增益

Conclusion: TTSnap框架通过噪声感知剪枝显著提高了测试时扩展的效率，在固定计算预算下能够探索更多样化的候选，为文本到图像扩散模型的测试时优化提供了有效解决方案
...
TL;DR: 本文提出了一种基于语义锚定的文本到图像扩散模型个性化方法，通过在频繁概念分布中锚定新概念，实现稳定适应并同时提升主体保真度和文本图像对齐度。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在个性化方面存在挑战：当模型专注于主体保真度时，容易对少量参考图像过拟合，无法利用预训练分布；而强调先验保持时，又难以学习新的个性化属性。需要在两者之间找到平衡。

Method: 提出语义锚定策略，将个性化重新定义为通过语义锚定在频繁对应概念分布指导下学习罕见概念的过程。这种方法通过将新概念锚定在其对应分布中，引导模型以稳定可控的方式适应新概念。

Result: 与基线方法相比，该方法实现了稳定适应，并在主体保真度和文本图像对齐度方面都取得了持续改进。大量实验和消融研究进一步证明了所提锚定策略的鲁棒性和有效性。

Conclusion: 语义锚定方法能够将预训练分布向个性化区域扩展，同时保持其语义结构，为解决文本到图像扩散模型的个性化挑战提供了一种有效解决方案。
...
TL;DR: UMind-VL是一个统一的超声医学基础模型，能够同时处理像素级结构理解（分割、定位）和高级临床推理（诊断），在超声领域实现了感知与解释的协同。


<details>
  <summary>Details</summary>
Motivation: 尽管医学基础模型取得了显著进展，但超声领域缺乏一个能够连接低层超声感知（如分割、定位）和高层超声解释（如诊断、推理）的综合解决方案。现有模型往往只能处理单一任务，无法实现像素级结构与临床推理的协同。

Method: 1. 构建UMind-DS大规模多模态数据集，包含120万超声图像-文本对，覆盖16个解剖区域，包含像素级标注和临床验证的推理依据；2. 设计UMind-VL架构，采用轻量级动态卷积掩码解码器，通过基于LLM输出的动态核生成掩码；3. 结合任务特定token，在单一框架内统一分割、检测、几何测量和诊断任务。

Result: UMind-VL在分割、检测、关键点定位和诊断推理等多个基准测试中，显著优于现有通用多模态模型，性能达到或超过最先进的专科模型，同时保持强大的泛化能力。

Conclusion: UMind-VL成功实现了超声领域像素级结构理解与复杂临床推理的统一，为超声医学提供了一个全面的基础模型解决方案，能够同时处理从低层感知到高层解释的多种任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS水印方案的保护能力存在疑问，传统2D图像水印移除技术无法有效应用于3DGS场景，因为3DGS具有专门的渲染流程和独特的高斯基元属性。

Method: 提出GSPure框架，通过分析视图相关渲染贡献和利用几何精确的特征聚类，精确分离并有效移除与水印相关的高斯基元，同时保持场景完整性。

Result: GSPure在实验中表现出最佳的水印净化性能，能将水印PSNR降低高达16.34dB，同时对原始场景保真度的退化控制在小于1dB PSNR损失。在效果和泛化能力方面均优于现有方法。

Conclusion: GSPure是首个专门针对3DGS水印表示的净化框架，能够有效保护3D资产，填补了该领域的技术空白。
...
TL;DR: DriveVGGT是一个专门为自动驾驶数据设计的尺度感知4D重建框架，通过改进VGGT架构来适应自动驾驶系统的特定先验条件，包括多摄像头配置、已知相机参数和固定相对位置等。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈重建方法（如VGGT）直接应用于自动驾驶系统时效果不佳，因为自动驾驶任务具有独特的先验条件：1）摄像头视野重叠最小化；2）相机内参和外参已知；3）所有相机的相对位置固定。需要专门设计来充分利用这些先验信息。

Method: 提出DriveVGGT框架，包含三个核心组件：1）时序视频注意力模块独立处理多摄像头视频，利用单摄像头序列的时空连续性；2）多摄像头一致性注意力模块使用归一化相对位姿嵌入进行窗口注意力，建立跨摄像头一致性关系；3）扩展VGGT头部，增加绝对尺度头和自车姿态头。

Result: 实验表明DriveVGGT在自动驾驶数据集上优于VGGT、StreamVGGT和fastVGGT，消融研究验证了所提出设计的有效性。

Conclusion: DriveVGGT成功将自动驾驶系统的特定先验整合到前馈重建框架中，实现了更好的4D重建性能，为自动驾驶场景的视觉几何理解提供了专门化解决方案。
...
TL;DR: 论文提出"补丁坍缩"概念，类比量子力学中的波函数坍缩，研究图像中某些补丁如何降低其他补丁的不确定性，并利用PageRank算法确定最优补丁实现顺序，提升图像生成和分类效率。


<details>
  <summary>Details</summary>
Motivation: 观察到图像中某些补丁的实现会降低其他补丁特征分布的不确定性，这种现象类似于量子力学中的波函数坍缩。作者希望通过研究这种"补丁坍缩"现象，识别出图像生成和分类过程中最关键的补丁依赖关系，从而提高视觉模型的效率。

Method: 1. 学习一个自编码器，软选择补丁子集来重建每个目标补丁；2. 为每个补丁的依赖关系图计算PageRank分数，确定最优补丁实现顺序；3. 将这一顺序应用于掩码图像建模方法：提升MAR自回归图像生成性能，以及提出新的视觉Transformer分类设置，仅暴露高PageRank排名的补丁。

Result: 1. 通过尊重补丁坍缩顺序，显著提升了最先进的自回归模型MAR的图像生成性能；2. 在图像分类任务中，仅需观察22%的高排名补丁就能达到高准确率，证明了补丁坍缩视角对视觉效率的促进作用。

Conclusion: 补丁坍缩作为一种新颖的图像建模视角，能够有效促进视觉效率。通过识别和利用补丁间的依赖关系，可以优化图像生成和分类任务的性能，仅需处理少量关键补丁就能获得良好效果，为高效的视觉模型设计提供了新思路。
...
TL;DR: Match-and-Fuse是一种零样本、无需训练的框架，用于从图像集合生成具有一致性的新图像集，保持共享内容的跨图像一致性。
...
Conclusion: Match-and-Fuse为零样本、无需训练的集合到集合生成提供了有效框架，能够保持跨图像一致性，为基于图像集合的内容创作开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 构建稳健的医疗机器学习系统需要利用临床数据内在结构的预训练策略。临床数据具有自然的多视图组织特性，这为学习视图不变和疾病相关的表示提供了机会。

Method: 提出Multiview Masked Autoencoder (MVMAE)自监督框架，结合掩码图像重建与跨视图对齐，将不同投影间的临床冗余转化为自监督信号。进一步扩展为MVMAE-V2T，加入放射学报告作为文本监督信号以增强语义基础，同时保持基于视觉的推理能力。

Result: 在MIMIC-CXR、CheXpert和PadChest三个大型公共数据集的下游疾病分类任务中，MVMAE持续优于监督学习和视觉语言基线方法。MVMAE-V2T在低标签情况下提供额外增益，特别是在结构化文本监督最有益的情况下表现更佳。

Conclusion: 结构监督和文本监督是构建可扩展、临床基础的医疗基础模型的互补路径，MVMAE框架展示了利用临床数据内在多视图结构和文本信息的重要性。
...
TL;DR: 提出基于语言和分割语义指导的自动化视频上色方法，使用语言条件扩散模型，通过自动生成的对象掩码和文本提示实现高质量上色，无需特定颜色输入，利用光流进行时间稳定性处理。


<details>
  <summary>Details</summary>
Motivation: 现有视频上色方法存在时间闪烁问题或需要大量手动输入，需要一种能够自动化实现高质量、时间一致性的视频上色方法。

Method: 使用语言条件扩散模型对灰度帧进行上色，通过自动生成的对象掩码和文本提示提供语义指导（主要使用通用提示），利用RAFT光流从先前帧扭曲颜色信息实现时间稳定性，并包含校正步骤检测和修复扭曲引入的不一致性。

Result: 在标准基准测试（DAVIS30、VIDEVO20）上实现了最先进的性能，在颜色准确性（PSNR）和视觉真实感（Colorfulness、CDC）方面表现出色，证明了基于自动提示指导的有效性。

Conclusion: 该方法通过语言和分割语义指导实现了自动化、高质量、时间一致的视频上色，无需特定颜色输入，为视频上色任务提供了有效的解决方案。
...
TL;DR: 研究发现多模态大语言模型在多选视觉问答评估中存在严重的提示格式偏见，即使语义中性的微小格式变化也会显著影响模型表现，现有偏见缓解策略无法解决这些新发现的偏见。


<details>
  <summary>Details</summary>
Motivation: 虽然之前的研究已经发现多选视觉问答基准对答案选项顺序敏感，但本文发现还存在未被探索的提示格式偏见，这些偏见质疑了当前多模态大语言模型评估的可靠性。

Method: 通过大规模研究，分析了提示格式中的三个关键变化因素，涉及7个多模态大语言模型和5个视觉问答数据集，共48种不同的提示格式变化，研究了这些因素对模型表现的影响。

Result: 研究发现多选视觉问答对微小的提示格式变化高度敏感，即使这些变化是语义中性的。这些偏见独立于已知的顺序偏见或模型对正确答案的置信度而持续存在。
...
<details>
  <summary>Details</summary>
Motivation: 标准归一化流（NFs）的生成质量受限于基于对数似然优化的语义表示能力不足。现有方法通常在前向传递中进行正则化，但效果有限。

Method: 提出创新的对齐策略：利用NFs的可逆性，将生成（反向）过程中的中间特征与强大的视觉基础模型表示对齐。同时提出无需训练、基于测试时优化的分类算法来评估NFs的语义知识。

Result: 方法在ImageNet 64×64和256×256上建立了新的SOTA结果，训练加速超过3.3倍，同时显著提升生成质量和分类准确率。
...
TL;DR: INSIGHT是一个用于检测和解释AI生成图像的统一多模态框架，即使在极低分辨率下也能保持鲁棒性，通过超分辨率增强取证线索、多尺度定位和语义对齐来提供透明解释。
...
Method: INSIGHT结合了分层超分辨率（用于放大细微取证线索而不引入误导性伪影）、Grad-CAM驱动的多尺度定位（揭示指示生成模式的空间区域）和CLIP引导的语义对齐（将视觉异常映射到人类可解释的描述符）。然后使用结构化的ReAct + Chain-of-Thought协议提示视觉语言模型，通过双阶段G-Eval + LLM-as-a-judge管道验证解释以减少幻觉并确保事实性。
...
TL;DR: AnchorFlow是一种无需训练的3D编辑方法，通过建立全局潜在锚点确保编辑过程中的一致性和几何稳定性，无需掩码监督即可保持几何保真度。
...
Result: 在Eval3DEdit基准测试中，AnchorFlow在各种编辑类型上始终提供语义对齐和结构鲁棒的编辑效果，能够实现更显著的语义修改。

Conclusion: AnchorFlow通过稳定潜在参考空间，实现了无需掩码监督的稳定、语义忠实且几何保真的3D编辑，在无需训练的3D编辑领域取得了显著进展。
...
TL;DR: 该论文针对遥感视觉问答任务中存在的伪推理问题，提出了RS-EoT范式，通过SocraticAgent多智能体系统和两阶段强化学习策略，实现了基于视觉证据的真实推理。


<details>
  <summary>Details</summary>
Motivation: 现有遥感多模态推理模型存在伪推理问题，即模型只是描述推理过程而非基于视觉证据进行真实推理。作者将此归因于"一瞥效应"——对大规模遥感图像的粗略感知导致不完整理解，模型依赖语言自洽而非视觉证据。

Method: 提出RS-EoT（遥感证据思维）范式，采用语言驱动的迭代视觉证据寻求方法。具体包括：1）SocraticAgent多智能体系统，通过推理和视觉检查的交替循环合成推理轨迹；2）两阶段渐进强化学习策略：先在细粒度Grounding任务上进行RL以增强RS-EoT能力，然后在RS VQA任务上进行RL以泛化到更广泛的理解场景。

Result: 实验表明RS-EoT在多个RS VQA和grounding基准测试中达到了最先进的性能。分析揭示了清晰的推理和证据寻求迭代循环，证实RS-EoT缓解了一瞥效应，实现了真正的基于证据的推理。

Conclusion: RS-EoT范式有效解决了遥感任务中的伪推理问题，通过迭代的视觉证据寻求实现了基于证据的真实推理，为遥感多模态理解提供了新思路。
...
TL;DR: UAV-MM3D是一个用于低空无人机感知的高保真多模态合成数据集，包含40万帧同步数据，涵盖多种场景、天气条件和无人机模型，提供五种模态数据及丰富标注，用于3D检测、姿态估计等任务。


<details>
  <summary>Details</summary>
Motivation: 现实世界中无人机数据收集面临空域管制、隐私问题和环境变化等限制，而手动标注3D姿态和跨模态对应关系耗时耗力，需要大规模、准确标注的多模态数据来开发可靠的无人机感知解决方案。

Method: 创建UAV-MM3D合成数据集，包含400K同步帧，涵盖城市、郊区、森林、沿海等多样场景和不同天气条件，包含多种无人机模型，提供RGB、红外、LiDAR、雷达和DVS五种模态数据，并配有2D/3D边界框、6自由度姿态和实例级标注。

Result: 开发了LGFusionNet（LiDAR引导的多模态融合基线）和专门的无人机轨迹预测基线，为基准测试提供支持。数据集为无人机3D感知提供了可控的仿真环境、全面的场景覆盖和丰富的标注。

Conclusion: UAV-MM3D通过合成数据方法解决了现实世界数据收集的挑战，为推进无人机3D感知研究提供了公开基准，支持核心任务如3D检测、姿态估计、目标跟踪和短期轨迹预测。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D头部风格化方法通常依赖计算昂贵的优化或特定领域的微调来适应新风格，限制了方法的效率和灵活性。需要一种能够快速适应不同艺术风格且保持3D一致性的解决方案。
...
Conclusion: DiffStyle360提出了一种高效、灵活的3D头部风格化框架，能够通过单张参考图像实现高质量的多视角一致风格化，为数字媒体中的创意视觉体验提供了新的可能性。
...
TL;DR: WUKONG是一个无需训练的高保真纹理3D变形框架，使用源和目标提示（图像或文本）作为输入，通过基于流的transformer生成先验实现高质量3D过渡。
...
Result: 广泛的定量和定性评估表明，WUKONG显著优于现有最先进方法，在不同几何和纹理变化上均取得优越结果，避免了过度平滑等常见伪影。

Conclusion: WUKONG是一个无需训练的高保真3D变形框架，通过创新的序列初始化和语义一致性机制，实现了高质量的3D过渡，在几何和纹理保真度方面表现出色。
...
TL;DR: Fin3R是一种简单有效的3D重建模型微调方法，通过冻结解码器、仅微调图像编码器，并利用单目教师模型在无标签数据上提取的几何细节来增强模型，显著提升几何精度和边界清晰度。
...
Method: Fin3R采用轻量级微调步骤：冻结处理视角匹配的解码器，仅微调负责特征提取的图像编码器。编码器通过定制的轻量LoRA适配器，从强大的单目教师模型在大量无标签数据上提取的精细几何细节进行知识蒸馏。

Result: 在DUSt3R、MASt3R、CUT3R和VGGT等多种模型上验证，微调后的模型在单视角和多视角设置下都能提供更清晰的边界、恢复更复杂的结构，并实现更高的几何精度，同时仅增加微小的LoRA权重，测试时内存和延迟几乎不变。

Conclusion: Fin3R是一种简单、有效且通用的3D重建模型微调方法，通过知识蒸馏和选择性微调策略，显著提升了前馈式3D重建模型的几何精度和鲁棒性。
...
Method: 提出ABounD统一学习框架，包含动态概念融合模块（生成类别自适应提示）和对抗性边界锻造模块（通过PGD式扰动生成边界级围栏特征）。在概念-边界损失下进行单阶段训练，ABF提供主要监督信号，语义-空间正则化器稳定优化。
...
TL;DR: 论文提出了IntentHQ基准，用于分析深度伪造视频背后的意图，从真实性检测转向上下文理解，包含5168个视频和23个细粒度意图类别。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法只关注区分真假视频，但无法回答一个根本问题：操纵视频背后的意图是什么？这限制了我们对深度伪造视频社会影响的理解和应对能力。

Method: 构建了IntentHQ基准数据集，包含5168个精心收集和标注的视频，涵盖23个细粒度意图类别。采用监督和自监督的多模态模型，整合时空视频特征、音频处理和文本分析来推断视频背后的动机和目标。

Result: 提出了IntentHQ基准，这是首个专注于深度伪造视频意图分析的数据集。提出的模型能够有效区分广泛的意图类别，为视频内容理解提供了新的分析维度。

Conclusion: 该研究将深度伪造分析从简单的真实性验证转向更深入的上下文意图理解，为检测恶意内容、理解社会影响提供了新工具，有助于应对深度伪造带来的安全风险。
...
TL;DR: ITS3D：一种通过推理时缩放优化高斯噪声输入来提升文本引导3D扩散模型生成质量的方法，无需额外训练
...
Method: 提出ITS3D框架，将任务形式化为优化问题，通过验证器引导的搜索算法迭代优化高斯噪声输入；引入高斯归一化稳定搜索过程、SVD压缩降低维度、奇异空间重置机制防止陷入局部最优

Result: 实验表明ITS3D能显著提升文本到3D生成质量，展示了计算高效搜索方法在生成过程中的潜力

Conclusion: ITS3D通过推理时优化噪声输入有效提升了3D生成质量，为无需额外训练的生成质量改进提供了新思路
...
TL;DR: 提出了一种基于高斯时空表示的动态火焰3D重建方法，仅需三个视角即可重建具有挑战性的火焰动态


<details>
  <summary>Details</summary>
Motivation: 火焰的动态重建极具挑战性，因其具有挥发性、透明性和高频特征，现有方法通常需要大量视角，而本研究旨在仅用三个视角实现重建

Method: 1) 通过结合密集多视角立体图像和单目深度先验分离静态背景与动态火焰区域；2) 将火焰初始化为3D流场，通过融合每视角密集光流投影获得；3) 每个3D高斯编码生命周期和线性速度以匹配密集光流；4) 使用自定义硬件同步模式确保跨相机亚帧时间对齐

Result: 在多个重建实验中的定量和定性验证表明，该方法在多样且具有挑战性的真实火焰场景中表现出稳健性能

Conclusion: 该方法成功实现了仅用三个视角对动态火焰的高质量3D重建，通过创新的高斯时空表示和硬件同步解决了火焰重建中的几何约束不足问题
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要针对检测或分割等感知任务，忽视了推断道路拓扑和动态场景结构所需的推理能力。中层次道路语义对于连接低层感知和高层规划至关重要，但缺乏相应的评估工具。

Method: 提出了RoadSceneBench基准测试，强调关系理解和结构一致性。同时提出了HRRP-T训练框架，通过分层关系奖励传播与时序一致性来增强视觉语言模型的推理可靠性。

Result: 实验表明该方法在各种道路配置上实现了最先进的性能。RoadSceneBench为研究中层次道路语义和培养结构感知的自主感知提供了紧凑而强大的基础。

Conclusion: RoadSceneBench填补了道路场景推理评估的空白，通过强调关系理解和结构一致性，推动了从静态识别向几何感知和时序一致推理的转变，为自动驾驶和数字地图构建提供了重要工具。
...
TL;DR: 本文提出了一种基于文本的行人异常检索新方法，通过局部-全局混合视角模块和统一图像-文本模型，结合迭代集成策略和特征选择算法，在PAB数据集上实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 文本行人异常检索现有方法大多依赖复杂的深度学习技术，存在如何优化模型以获得更细粒度特征的研究问题。需要探索如何有效结合细粒度与粗粒度特征来提升检索性能。

Method: 1. 提出局部-全局混合视角(LHP)模块与视觉语言模型(VLM)结合；2. 研究统一图像-文本(UIT)模型，整合ITC、ITM、MLM和MIM四种损失函数；3. 提出新颖的迭代集成策略；4. 基于LHP模型指导的特征选择算法。

Result: 在PAB数据集上实现最先进性能：R@1提升9.70%，R@5提升1.77%，R@10提升1.01%，相比之前工作有显著改进。

Conclusion: 提出的LHP模块、UIT模型、迭代集成策略和特征选择算法能有效提升文本行人异常检索性能，证明了细粒度与粗粒度特征结合的重要性，以及集成方法的有效性。
...
TL;DR: DINOv3等视觉基础模型无需微调即具备跨生成器检测能力，通过全局低频结构而非高频伪影识别图像真伪，提出基于token排序的轻量级检测方法。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型多样化，现有检测方法往往记忆特定生成器的伪影而非学习可迁移线索，导致对未见生成器检测失败。研究发现冻结的视觉基础模型已具备跨生成器检测能力。

Method: 通过频率、空间和token三个维度系统研究DINOv3的检测机制，发现其依赖全局低频结构作为弱但可迁移的真实性线索。提出无需训练的token排序策略，结合轻量级线性探针选择少量真实性相关token。

Result: DINOv3在跨生成器检测中表现优异，提出的token子集方法在所有评估数据集上一致提升检测准确率，提供高效、可解释的基准方法。

Conclusion: 研究为理解基础模型为何能泛化到多样生成器提供实证证据和可行假设，为图像伪造检测提供了通用、高效且可解释的基准方法。
...
Result: 在VoxCeleb2和CelebV-HQ数据集上的定量、定性实验和用户研究表明，Dimitra++在生成真实说话头方面优于现有方法，能够更好地模拟唇部运动、面部表情和头部姿态。

Conclusion: Dimitra++是一个有效的音频驱动说话头生成框架，通过条件运动扩散变换器成功实现了唇部运动、面部表情和头部姿态的统一建模，生成结果更加真实。
...
<details>
  <summary>Details</summary>
Motivation: 随着科学论文数量的增长，需要有效传达研究成果的方法，海报作为展示论文内容的重要媒介，其布局设计对研究传播和理解至关重要。目前缺乏大规模标注的论文-海报布局配对数据集，限制了相关研究的发展。

Method: 提出了SciPostGen大规模数据集，包含论文与海报布局的配对标注。基于此数据集，开发了检索增强的海报布局生成框架：首先检索与给定论文结构一致的海报布局，然后将其作为指导进行布局生成。实验在有无布局约束两种条件下进行。

Result: 分析显示论文结构与海报布局元素数量存在关联。检索器能够估计与论文结构一致的布局，所提出的框架生成的布局既能满足给定的约束条件，又能保持与论文结构的一致性。

Conclusion: SciPostGen数据集填补了大规模论文-海报布局配对标注的空白，提出的检索增强生成框架能够有效生成与论文结构一致且满足约束条件的海报布局，为科学传播提供了实用工具。
...
TL;DR: 本文针对文档图像中的密集文本去除问题，提出了基于贝叶斯优化的灵活掩模建模方法，发现字符级掩模比最小文本区域覆盖更优，为手动掩模提供了实用指导。


<details>
  <summary>Details</summary>
Motivation: 现有文本去除方法主要针对简单场景文本，缺乏对密集文本图像的处理研究。在创建密集文本去除基准数据时，发现文本去除性能对掩模轮廓扰动敏感，需要精确调整掩模形状。

Method: 开发了高度灵活的掩模轮廓建模方法，使用贝叶斯优化学习掩模参数，生成字符级掩模。

Result: 研究发现字符级掩模效果最佳，最小文本区域覆盖并非最优选择，为手动掩模提供了用户友好的指导原则。

Conclusion: 该方法为密集文本去除任务提供了有效的掩模优化方案，有望推动文档图像处理在工业应用中的实际应用。
...
TL;DR: DocVAL框架通过验证式思维链蒸馏，将大型教师模型的空间推理能力转移到可部署的学生视觉语言模型中，解决了DocVQA中准确性与效率的权衡问题。
...
Method: 提出DocVAL框架，包含三个关键组件：1）带验证时间文本检测的教师监督，过滤和去噪训练信号；2）多模块验证器，确保答案正确性和几何一致性，并提供像素级错误反馈；3）两阶段学生训练方案，先学习验证后的思维链轨迹，然后通过验证器反馈进行迭代优化。

Result: 学生模型（Gemma-3 12B）在DocVQA上达到91.4% ANLS和82.4% mAP，作为纯视觉语言模型无需推理时的文本检测或OCR。验证反馈贡献6.3 mAP增益，迭代优化带来9.7 mAP提升。

Conclusion: DocVAL框架成功地将空间推理能力从大型教师模型转移到可部署的学生模型中，并发布了95k高质量验证思维链轨迹以推进文档理解中的空间推理研究。
...
Conclusion: CoT4AD通过思维链推理增强了自动驾驶VLA模型的数值和因果推理能力，在复杂动态环境中实现了更稳健的决策。
...
TL;DR: Fast3Dcache：一种无需训练的几何感知缓存框架，通过动态缓存调度和时空稳定性准则，在保持3D几何一致性的同时加速扩散模型推理，实现最高27.12%的加速和54.8%的FLOPs减少。
...
Result: 显著加速推理，最高实现27.12%的加速和54.8%的FLOPs减少，几何质量退化最小（Chamfer距离仅增加2.48%，F-Score仅下降1.95%）。

Conclusion: Fast3Dcache是一种有效的训练免费方法，能够在保持3D几何保真度的同时加速扩散模型推理，解决了现有缓存技术在3D合成中的几何一致性破坏问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像压缩方法通常孤立地优化人类感知或机器分析任务，但这两者存在根本共性：保持准确的语义信息对智能任务至关重要，同时增强感知质量不仅改善视觉体验，还能通过确保真实的图像分布来帮助机器语义特征提取。

Method: 提出Diff-ICMH框架：1）利用生成先验确保感知真实性；2）通过语义一致性损失（SC损失）保证语义保真度；3）引入标签引导模块（TGM），利用高度语义的图像级标签激发预训练扩散模型的生成能力，仅需最小额外比特率。

Result: Diff-ICMH支持通过单一编解码器和比特流完成多种智能任务，无需任何任务特定适配，同时为人类感知保持高质量视觉体验。大量实验结果表明其在多样化任务上的优越性和泛化能力。

Conclusion: Diff-ICMH成功协调了机器和人类视觉需求，实现了语义保真度和感知质量的双重优化，为图像压缩领域提供了一种统一框架，能够同时服务于机器分析和人类感知需求。
...
TL;DR: 提出了一种基于文本条件嵌入的牙科种植体基台设计框架(TCEAD)，通过文本引导定位模块和口腔扫描数据预训练，实现了自动化基台设计，在IoU指标上优于现有主流方法。


<details>
  <summary>Details</summary>
Motivation: 传统牙科种植体基台设计过程耗时耗力，不当的基台设计可能导致种植体并发症如种植体周围炎。利用人工智能辅助基台设计可以提高设计效率和适配性。

Method: 提出TCEAD框架，扩展了MeshMAE自监督学习框架，引入文本引导定位(TGL)模块。使用口腔扫描数据预训练编码器以提取局部细粒度特征，并利用CLIP文本编码器通过文本描述快速定位基台区域。

Result: 在大规模基台设计数据集上验证，TCEAD在IoU指标上相比其他主流方法提升了0.8%-12.85%，展示了在自动化牙科基台设计中的潜力。

Conclusion: TCEAD框架通过文本条件嵌入和局部特征提取，有效提高了牙科种植体基台设计的自动化水平和准确性，为临床牙科种植提供了高效的技术支持。
...
TL;DR: 研究不同思维链设计对视觉语言模型通用视觉推理能力的影响，发现简洁的思维链比冗长的表现更好，呈现"短即是长"效应。
...
Method: 使用可控的迷宫求解基准测试，在Qwen2.5-VL-7B模型上采用标准SFT-then-RL流程，比较三种代表性思维链格式：语言思维链、基于空间坐标轨迹的定位思维链和基于图像操作的视觉思维链。

Result: 视觉和更长的思维链主要加速收敛但不提升最终性能上限；仅包含必要定位步骤的简洁思维链优于长链；仅保留最小定位结果的思维链在不同迷宫尺寸上泛化效果最好。
...
<details>
  <summary>Details</summary>
Motivation: CLIP虽然在大规模视觉语言任务上表现出色，但由于缺乏区域级监督，其细粒度语义理解能力有限。现有方法试图解决这一问题，但往往破坏了全局对齐，导致全局一致性和局部感知之间存在权衡问题。

Method: HarmoCLIP通过两个关键创新解决这一问题：1）引入显式的细粒度语义监督项，直接对齐文本片段与对应视觉区域；2）提出区域-语言对齐监督策略，在不损害全局语义一致性的前提下促进细粒度语义学习。

Result: 实验表明，HarmoCLIP在检索等全局任务上达到最先进性能（提升高达69.78%），在边界框分类等区域任务上Top-1准确率提升3.2%，优于现有方法，为CLIP的全局-局部权衡提供了平衡、高效、即插即用的解决方案。

Conclusion: HarmoCLIP成功协调了CLIP的全局和区域表示，通过直接对齐文本片段与视觉区域，解决了现有方法中全局一致性与局部感知之间的权衡问题，实现了更好的细粒度语义理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本工业异常检测方法只能生成粗糙的补丁级异常图，而现有细化方法由于合成异常数据与真实异常之间的差距，难以恢复细粒度异常且存在漏检问题。研究发现异常分数图提供了图像特征中缺失的互补空间线索。

Method: 提出AnoRefiner框架：1）设计异常细化解码器，利用异常分数图逐步增强图像特征；2）基于批量生产范式，提出渐进式分组测试时训练策略，在每个产品组中训练解码器用于下一组的细化过程。
...
TL;DR: 提出GazeTrack高精度眼动追踪数据集和新方法，通过形状误差正则化和坐标变换技术，在降低计算复杂度的同时显著提升注视角度精度


<details>
  <summary>Details</summary>
Motivation: 当前眼动追踪技术在虚拟和增强现实应用中的精度不足，无法满足空间计算需求，需要更精确的注视追踪解决方案

Method: 1) 设计高精度眼动数据收集框架，创建GazeTrack多民族、多年龄段、多视力条件数据集；2) 提出形状误差正则化方法约束瞳孔椭圆拟合；3) 发明类似纸张展开的坐标变换方法预测注视向量；4) 构建低计算复杂度的注视向量生成模型

Result: 构建了首个高精度GazeTrack基准数据集，提出的方法在降低计算复杂度的同时实现了更低的注视角度误差

Conclusion: 通过高质量数据集和创新算法，显著提升了眼动追踪精度，为虚拟/增强现实中的空间计算应用提供了更可靠的注视追踪解决方案
...
Conclusion: 通过解锁MLLM的推理能力并引入思考-编辑-反思循环，显著提升了图像编辑模型的性能，证明了推理机制在图像编辑任务中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感MLLMs通常通过精心策划的思维链数据进行冷启动训练，这种方法不仅标注成本高昂，还会引入人为偏见，限制了模型推理的多样性。需要一种无需预定义CoT监督的地理空间推理方法。

Method: 提出GeoZero框架：1) 构建GeoZero-Instruct数据集用于监督微调，让模型获取初步地理空间知识；2) 构建GeoZero-Hard数据集在强化学习阶段激发深度推理；3) 引入答案锚定组相对策略优化(A²GRPO)，通过模型自身答案正则化推理过程，鼓励多样而准确的思考。

Result: 在多个遥感视觉语言基准测试上的广泛实验表明，GeoZero不仅超越了现有的最先进方法，还在多样化的地理空间任务中培养了通用的涌现推理能力。

Conclusion: GeoZero框架成功实现了无需预定义思维链监督的地理空间推理，通过创新的数据集构建和策略优化方法，有效提升了多模态大语言模型在地理空间场景理解中的推理能力。
...
TL;DR: 该论文提出了一种名为注意力交互对齐（AIA）损失的方法，旨在缓解多模态模型中理解与生成任务之间的冲突，而无需采用模型解耦策略。


<details>
  <summary>Details</summary>
Motivation: 统一的多模态模型在图像生成和理解方面具有重要意义，但理解与生成任务之间存在固有冲突。现有方法通过不同程度的模型解耦来缓解冲突，但过度解耦会损害模型的交织生成能力，违背统一模型的初衷。

Method: 首先分析模型解耦如何通过驱动模型形成任务特定的多模态交互模式来缓解冲突。基于此观察，提出注意力交互对齐（AIA）损失，在训练过程中显式学习任务特定的多模态交互模式。该方法在Emu3和Janus-Pro模型的不同训练阶段进行了验证。

Result: AIA损失不仅细化了跨模态注意力模式，还同时提升了生成和理解任务的性能，证明了其在不依赖模型解耦的情况下缓解任务冲突的有效性。

Conclusion: 通过显式学习任务特定的多模态交互模式，AIA损失能够有效缓解统一多模态模型中理解与生成任务之间的冲突，同时保持模型的交织生成能力，为构建更强大的统一多模态模型提供了新思路。
...
TL;DR: VaMP提出了一种变分多模态提示学习框架，通过从学习的后验分布中采样生成实例条件化提示，实现样本特定、不确定性感知的提示调优，在少样本和领域泛化基准上达到SOTA性能。
...
Method: 提出变分多模态提示学习框架，通过从学习的后验分布中采样生成实例条件化提示；引入基于实例表示和类别原型的类感知先验；将提示调优公式化为潜在提示表示的变分推断，通过重参数化采样进行端到端训练。

Result: 在少样本和领域泛化基准测试中，VaMP实现了最先进的性能，证明了建模不确定性和任务结构的有效性。

Conclusion: VaMP框架通过变分推断方法实现了样本特定、不确定性感知的多模态提示学习，显著提升了视觉语言模型在有限监督下的适应能力，为多模态表示学习提供了新的有效方法。
...
TL;DR: 本文挑战了传统认知，发现扩散模型蒸馏中CFG增强而非分布匹配是驱动少步生成的核心机制，分布匹配仅起正则化作用，这一新理解带来了性能提升。


<details>
  <summary>Details</summary>
Motivation: 挑战对Distribution Matching Distillation（DMD）及其变体的传统理解，这些方法被认为通过匹配学生和教师模型的输出分布实现高效少步生成。作者质疑这一核心机制是否真的是性能提升的主要原因。

Method: 通过对DMD训练目标进行严格分解，发现在需要CFG的复杂任务（如文本到图像生成）中，少步蒸馏的主要驱动力不是分布匹配，而是被忽视的CFG增强（CA）组件。作者将蒸馏过程解耦为引擎（CA）和正则化器（DM），并探索了替代的正则化方法。

Result: 揭示了CFG增强是蒸馏的核心"引擎"，而分布匹配仅作为"正则化器"确保训练稳定性。证明了DM项虽然有效但不是唯一的正则化方法，更简单的非参数约束或GAN目标也能起到类似作用。基于这一新理解提出的改进方法（如解耦噪声调度）带来了进一步的性能提升，并被Z-Image项目采用开发了顶级的8步图像生成模型。

Conclusion: 传统认为的分布匹配机制并非扩散模型蒸馏性能提升的主要原因，CFG增强才是核心驱动力。这一新理解不仅提供了更系统深入的分析框架，还启发了对蒸馏过程的原理性改进，在实践中验证了其泛化性和鲁棒性。
...
TL;DR: ReAG是一种新颖的推理增强多模态检索增强生成方法，通过粗粒度和细粒度检索结合批评模型过滤无关段落，使用强化学习增强推理能力，在知识密集型视觉问答任务上显著优于现有方法。
...
Conclusion: ReAG通过推理增强的多模态检索增强生成方法有效解决了知识密集型视觉问答中的检索精度和推理能力问题，为多模态大语言模型在复杂知识任务上的应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型在多人生成场景中存在严重问题，经常出现面部重复、身份合并或人数计数错误，需要专门解决多人生成的可靠性问题。

Method: 采用两阶段框架：1) Architect模块预测结构化布局，指定每个人的位置；2) Artist模块合成逼真图像，使用基于匈牙利空间对齐和ArcFace身份相似性的空间接地面部匹配奖励。使用Group Relative Policy Optimization (GRPO)进行优化。

Result: 在MultiHuman-Testbench上评估，Ar2Can在人数准确性和身份保持方面取得显著改进，同时保持高感知质量。值得注意的是，该方法主要使用合成数据，无需真实多人生成图像。

Conclusion: Ar2Can通过分离空间规划和身份渲染的方法，有效解决了多人生成中的关键问题，为可靠的多人生成提供了新思路，且主要依赖合成数据训练。
...
TL;DR: Z-Image是一个6B参数的高效图像生成基础模型，采用S3-DiT架构，挑战了"不惜一切代价扩大规模"的范式，在减少计算开销的同时实现了与顶级商业模型相媲美的性能。
...
Method: 提出基于可扩展单流扩散Transformer（S3-DiT）架构的6B参数基础生成模型，通过系统优化整个模型生命周期（从精选数据基础设施到简化训练课程），仅用314K H800 GPU小时完成完整训练工作流。采用few-step蒸馏方案和奖励后训练得到Z-Image-Turbo，支持企业级H800 GPU亚秒级推理延迟和消费级硬件兼容性（<16GB VRAM）。

Result: 模型在多个维度上达到或超越了领先竞争对手的性能，特别是在逼真图像生成和双语文本渲染方面表现出色，结果可与顶级商业模型相媲美，证明了用显著减少的计算开销也能实现最先进的结果。

Conclusion: Z-Image展示了通过高效架构设计和系统优化，可以在显著降低计算成本的情况下实现与顶级商业模型相媲美的性能，为开发可访问、经济实惠且最先进的生成模型提供了新路径。作者公开了代码、权重和在线演示。
...
### [86] [Artwork Interpretation with Vision Language Models: A Case Study on Emotions and Emotion Symbols](https://arxiv.org/abs/2511.22929)
...
Method: 采用案例研究方法，对三个视觉语言模型（Llava-Llama和两个Qwen模型）进行测试，提出四组复杂度递增的问题（一般内容、情感内容、情感表达、情感符号），并进行定性专家评估。

Result: 模型能很好地识别图像内容，通常也能识别描绘的情感及其表达方式。在具体图像上表现最佳，但在高度抽象或高度象征性图像上失败。可靠识别符号仍然存在根本性困难，且模型存在答案不一致的问题。

Conclusion: 当前视觉语言模型在识别艺术作品情感表达方面取得了一定进展，但在处理抽象象征内容和保持答案一致性方面仍有局限，需要进一步改进。
...
TL;DR: 提出DIPT方法，通过领域不变提示调优改进病理图像领域泛化，利用PLIP模型进行知识蒸馏，学习领域特定标记并跨领域平均以获得领域不变提示，提升F1分数。


<details>
  <summary>Details</summary>
Motivation: 计算病理学中存在由染色协议、扫描设备和成像设置差异导致的领域偏移问题。虽然视觉语言模型如PLIP可作为知识蒸馏源，但其零样本性能受提示词变化影响较大，且病理图像缺乏自然图像中的语义描述符，难以定义领域特定提示词。

Method: 提出领域不变提示调优(DIPT)方法，为每个领域学习多个输入标记，这些标记分别针对每个领域训练，然后跨领域平均以获得领域不变提示。学生模型利用DIPT学习的提示从PLIP的文本编码器蒸馏知识，使视觉特征与领域不变嵌入对齐。

Result: 在组织病理学数据集的领域泛化任务中，该方法显著提升了现有最先进知识蒸馏方法的平均F1分数。

Conclusion: DIPT方法有助于在具有异构数据源的真实世界临床问题中部署稳健的计算病理学模型，通过领域不变提示调优改进了知识蒸馏过程。
...
<details>
  <summary>Details</summary>
Motivation: 高斯泼溅在渲染任务中表现出潜力，但通常需要密集输入视图的逐场景优化。现有前馈方法通过多视图立体几何先验实现，但仍需要大量重叠的输入视图。本文旨在解决大稀疏性双目相机输入下的渲染问题。
...
Conclusion: Splat-SAP方法能够从稀疏双目相机输入中有效渲染以人为中心的场景新视角，通过像素级点图重建和两阶段学习策略解决了大稀疏性条件下的渲染挑战。
...
Method: 创建了Bharat场景文本数据集（BSTD），包含从印度各语言区域采集的6,500多张场景图像中的10万+单词，涵盖11种印度语言和英语。数据集经过精心标注，支持场景文本检测、文字识别、裁剪单词识别和端到端场景文本识别等任务。

Result: 通过将原本为英语开发的最先进模型适配（微调）到印度语言进行评估，结果突显了印度语言场景文本识别的挑战和机遇。

Conclusion: BSTD数据集代表了印度语言场景文本识别研究的重要进展，所有模型和数据都已开源，有望推动该领域的研究发展。
...
TL;DR: 该研究探索大规模视觉语言模型在双摄像头（道路视角和驾驶员视角）同步输入下的驾驶安全监控能力，通过构建数据集和微调模型，发现微调后的LVLMs能生成准确的安全驾驶指令，但在检测复杂事件方面仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 虽然大规模视觉语言模型在视觉任务中表现出色，能用于道路视角的安全监控，但全面的驾驶安全还需要监控驾驶员视角（如使用手机等危险行为）。目前缺乏对双摄像头同步输入处理能力的研究，需要探索LVLMs在此场景下的表现。

Method: 构建包含道路视角和驾驶员视角同步视频的数据集，评估预训练LVLMs的性能，然后对模型进行微调，并分析其在生成安全驾驶指令和检测危险事件方面的能力。

Result: 预训练LVLMs在双摄像头同步输入任务上效果有限，但经过微调后能够生成准确且安全导向的驾驶指令。然而，模型在检测视频中细微或复杂事件方面仍存在困难。

Conclusion: 微调后的LVLMs在双摄像头驾驶安全监控中具有应用潜力，但需要进一步改进以更好地检测复杂危险事件。研究提供了有价值的见解，有助于推动该领域LVLM系统的发展。
...
TL;DR: CORTEX是一个结合视觉和文本线索的变化描述框架，通过视觉语言模型提取组合推理描述，增强对图像间细微变化的捕捉能力。
...
Method: CORTEX包含三个关键模块：1) 图像级变化检测器识别低级视觉差异；2) 推理感知文本提取模块使用VLMs生成组合推理描述；3) 图像-文本双重对齐模块对齐视觉和文本特征进行细粒度关系推理。
...
TL;DR: 本研究评估了可见光(VIS)和热红外(TIR)图像融合在自动检测大蓝鹭及其巢穴中的效果，比较了早期融合和晚期融合方法，发现两种融合方法都能提升检测性能，但存在热红外视野限制和对齐约束等问题。
...
Result: 所有类别中，晚期和早期融合都比VIS-only模型提高了F1分数。对于主要类别"被占用巢穴"，晚期融合将F1分数从90.2%提升到93.0%。该模型还能以90%的召回率识别两个来源的误报。虽然融合方法效果更好，但受到TIR视野限制和对齐约束，会损失部分数据。

Conclusion: VIS-TIR图像融合能有效提升野生动物自动检测性能，但存在技术限制。使用机载高分辨率可见光传感器可能是实现可操作化调查的有趣选择。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学任务中表现优异，但常受到伪相关性（捷径学习）的影响，导致在新环境中泛化能力差。医学影像中多重伪相关性共存，误分类可能带来严重后果。
...
Conclusion: MIMM-X框架成功解决了医学影像中多重伪相关性导致的捷径学习问题，提高了模型的泛化能力和可靠性。
...
TL;DR: 提出一个端到端的深度学习模型，结合CNN和Vision Transformer从MRI扫描中提取空间特征，并使用BiLSTM处理连续时间点数据，用于预测MCI向AD的转化，在ADNI数据集上达到95.05%的准确率。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病（AD）是不可逆的神经退行性疾病，早期预测对及时干预至关重要。轻度认知障碍（MCI）是正常衰老和AD之间的过渡阶段，但并非所有MCI患者都会发展为AD。准确区分稳定MCI（sMCI）和进展性MCI（pMCI）是早期AD诊断的关键挑战。

Method: 提出一个端到端的深度学习模型：1）使用CNN和Vision Transformer的混合架构从MRI扫描中提取局部空间特征和全局上下文依赖；2）使用双向LSTM（BiLSTM）处理连续四个时间点的MRI特征和其他非图像生物标志物；3）预测受试者在第48个月时的认知状态。

Result: 在ADNI数据集上，该多模态模型在区分sMCI和pMCI方面达到了95.05%的平均预测准确率，优于现有AD预测研究，展示了在纵向AD预测方面的最先进性能。

Conclusion: 该研究证明了结合空间和时间建模在阿尔茨海默病早期检测中的有效性，为MCI向AD转化的准确预测提供了强大的端到端深度学习解决方案。
...
TL;DR: ViRL提出视觉合理化学习范式，将视觉动作重构为核心推理原语而非可选工具，通过过程监督、目标对齐和细粒度信用分配实现端到端强化学习，在感知、幻觉和推理基准上达到SOTA。
...
Method: 提出视觉合理化学习(ViRL)范式：1) 过程监督：使用真实合理化路径；2) 目标对齐：通过步骤级奖励塑造；3) 细粒度信用分配：区分正确、冗余和错误动作。纯端到端强化学习训练。

Result: 在感知、幻觉和推理等多个基准测试中达到最先进水平，使模型能够"基于正确的视觉原因得到正确答案"。

Conclusion: 视觉合理化作为一种任务无关、过程基础的新范式，为构建透明、可验证和可信赖的视觉语言模型奠定了基础。
...
### [96] [World in a Frame: Understanding Culture Mixing as a New Challenge for Vision-Language Models](https://arxiv.org/abs/2511.22787)
...
TL;DR: 本文研究了大型视觉语言模型在文化混合场景中的表现，发现现有模型在多种文化元素同时出现时难以保持个体文化身份，存在强烈的背景依赖性和预测不一致问题。


<details>
  <summary>Details</summary>
Motivation: 在全球化世界中，不同文化元素经常同时出现在同一视觉场景中（文化混合场景），但大型视觉语言模型如何感知这些场景尚未得到充分探索。本文旨在研究文化混合作为LVLMs面临的挑战，分析当前模型在多种文化元素同时出现时的表现。
...
Result: 评估发现模型在混合场景中一致性地无法保持个体文化身份，表现出强烈的背景依赖——当文化背景添加到仅食物基线时准确率下降14%，且在不同上下文中对相同食物产生不一致预测。监督微调能显著提高模型一致性和减少背景敏感性。

Conclusion: 文化混合场景是LVLMs面临的关键挑战，需要更多关注。使用多样化文化混合数据集进行监督微调能有效提高模型在文化混合场景中的鲁棒性，这是开发能够在文化多样化现实环境中可靠运行的LVLMs的重要步骤。
...
### [97] [From Pixels to Feelings: Aligning MLLMs with Human Cognitive Perception of Images](https://arxiv.org/abs/2511.22805)
*Yiming Chen,Junlin Han,Tianyi Bai,Shengbang Tong,Filippos Kokkinos,Philip Torr*

Main category: cs.CV

TL;DR: 该论文针对多模态大语言模型在理解图像主观认知属性方面的不足，提出了CogIP-Bench基准测试，通过后训练显著提升了模型与人类感知的对齐度，并将这种认知对齐能力应用于图像生成任务。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型擅长识别图像中的物体和描述场景，但缺乏理解图像对人类观察者的主观感受的能力，如记忆性、趣味性、美学价值和情感唤起等认知属性。

Method: 1. 引入CogIP-Bench基准测试系统评估MLLMs在图像认知属性上的表现；2. 设计后训练阶段来提升模型与人类感知的对齐度；3. 将认知对齐的MLLM集成到图像生成流程中，指导合成过程。

Result: 评估显示当前模型与人类对这些细微属性的感知存在显著差距，后训练能有效弥合这一差距，显著提升模型与人类判断的对齐度，且这种认知对齐能力可迁移到下游创意任务中。

Conclusion: 该工作提供了衡量类人感知的基准测试、增强对齐度的后训练流程，并证明这种对齐能够实现更以人为本的人工智能，特别是在图像生成中产生更具记忆性或视觉吸引力的图像。
...
TL;DR: 生成式基础模型能通过逼真的图像修复去除视觉伪影，但其对医学AI性能的影响尚不确定。研究发现，修复儿科手部X光片中的非解剖标记会显著降低骨龄和性别预测的准确性，表明视觉上逼真的修复可能掩盖临床相关特征并引入潜在偏差。


<details>
  <summary>Details</summary>
Motivation: 研究生成式基础模型在医学图像修复中的临床应用可靠性。儿科手部X光片常包含非解剖标记，需要评估修复这些区域是否保留了骨龄和性别预测所需的特征。
...
Result: 修复显著降低了模型性能：骨龄MAE从6.26个月增加到30.11个月，性别分类AUC从0.955下降到0.704。修复图像显示像素强度偏移和不一致性，表明结构修改无法通过简单校准纠正。

Conclusion: 虽然视觉上逼真，但基于基础模型的修复可能掩盖细微但临床相关的特征，即使在编辑局限于非诊断区域时也会引入潜在偏差。在将此类生成工具整合到临床AI工作流之前，需要进行严格的任务特定验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界引擎在激进6-DoF相机轨迹和复杂户外布局下存在长距离几何一致性丢失、偏离目标路径或运动过于保守的问题，需要新的方法来保持3D结构稳定性并准确执行复杂相机操作。

Method: 提出姿态条件世界引擎Captain Safari，通过维护动态局部记忆并使用检索器获取姿态对齐的世界标记，这些标记沿着轨迹条件化视频生成。同时创建OpenSafari数据集，包含经过多阶段几何和运动学验证的高动态无人机视频。
...
### [100] [SpaceMind: Camera-Guided Modality Fusion for Spatial Reasoning in Vision-Language Models](https://arxiv.org/abs/2511.23075)
*Ruosen Zhao,Zhikang Zhang,Jialei Xu,Jiahao Chang,Dong Chen,Lingyun Li,Weijian Sun,Zizhuang Wei*

Main category: cs.CV

TL;DR: SpaceMind是一个专为空间推理设计的多模态大语言模型，仅从RGB输入就能实现3D空间理解，通过相机引导的模态融合模块替代浅层特征融合，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大视觉语言模型在多模态理解方面表现出色，但在3D空间推理（如距离估计、尺寸比较、跨视角一致性）方面仍存在困难。现有的3D感知方法要么依赖辅助3D信息，要么通过浅层特征融合增强RGB-only模型，存在局限性。

Method: SpaceMind采用双编码器架构，集成VGGT作为空间理解编码器和InternViT作为2D视觉编码器。核心创新是将相机表示作为主动引导模态而非被动元数据，引入轻量级相机引导模态融合模块，应用相机条件偏置到空间token，分配反映几何重要性的查询无关权重，并使用相机嵌入门控融合表示。

Result: 在VSI-Bench、SQA3D和SPBench基准测试中建立了新的最先进结果，在VSI-Bench和SPBench上大幅超越开源和专有系统，在SQA3D上达到最先进性能。

Conclusion: 相机引导的模态融合是为视觉语言模型配备真正空间接地智能的有效且实用的归纳偏置，证明了仅从RGB输入就能实现强大的3D空间推理能力。
...
TL;DR: 该论文研究了多模态大语言模型对矛盾模态的鲁棒性问题，提出了MMA-Bench评估框架，发现现有模型在模态冲突时表现脆弱，并提出了一种模态对齐调优策略来改善多模态推理能力。
...
Method: 1. 引入MMA-Bench评估框架，包含视频和任务来探测模型对特定模态的依赖；2. 使用黑盒和白盒可解释性技术分析开源和闭源MLLMs的脆弱性；3. 提出模态对齐调优策略，教导模型何时优先考虑、利用或忽略特定模态线索。

Result: 研究发现当前MLLMs在音频-视觉对不匹配和简单误导性文本的情况下表现不佳，缺乏鲁棒的多模态推理能力。提出的对齐调优策略能够显著增强多模态基础能力，通过大量实验和分析验证了其有效性。

Conclusion: 这项工作提供了可解释性工具和明确的发展路径，旨在开发具有内在可靠跨模态推理能力的MLLMs。代码和数据集将公开可用，为未来研究提供基础。
...
TL;DR: RETINA基准测试解决MKB-VQA中的"视觉捷径"问题，通过LLM驱动的流程构建包含12万训练数据和2千人工标注测试集，要求模型理解次要实体关系而非依赖主实体视觉匹配。


<details>
  <summary>Details</summary>
Motivation: 现有MKB-VQA基准存在"视觉捷径"问题，查询图像通常与目标文档的主实体匹配，模型可以仅利用视觉线索获得可比结果，这不能真实反映模型的多模态知识理解能力。

Method: 提出RETINA基准，使用LLM驱动流程自动构建，包含查询引用次要实体（相关实体）并配对这些相关实体的图像，消除视觉捷径。同时提出MIMIR方法，通过增强多个相关实体的图像来丰富文档嵌入，处理RETINA中的多实体关系。

Result: 在RETINA基准上，现有模型性能显著下降，证实它们依赖视觉捷径。MIMIR方法能有效处理RETINA，优于仅使用单张图像的传统方法。

Conclusion: RETINA基准揭示了现有MKB-VQA模型的局限性，并提供了更真实的评估场景。MIMIR方法通过多图像增强有效解决了视觉捷径问题，推动了多模态知识理解的发展。
...
TL;DR: SLEUTH是一个多智能体框架，通过检索和四个协作智能体的粗到细处理，从长文档中提取关键线索和视觉证据，生成浓缩的多模态上下文以提升文档理解性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在处理单页文档时表现良好，但在处理长文档时效果下降。长文档中线索分散在多页和多模态中，冗长的输入冗余会损害模型判断。虽然检索增强生成通过筛选相关内容缓解了问题，但检索结果仍包含大量冗余。
...
Conclusion: SLEUTH是一个模型无关且可扩展的框架，通过多智能体协作和分层精炼，有效解决了长文档理解中线索分散和冗余问题，显著提升了文档理解性能。
...
TL;DR: 提出REVEAL-Bench基准和REVEAL框架，通过多专家模型证据链和强化学习实现可解释的AI生成图像检测


<details>
  <summary>Details</summary>
Motivation: 随着生成模型快速发展，AI生成图像越来越难以与真实图像区分，对社会信任和信息完整性构成严重威胁。现有可解释检测方法主要依赖事后合理化或视觉判别，缺乏可验证的证据链，限制了因果解释的生成和泛化能力。

Method: 1. 提出REVEAL-Bench基准，基于多个轻量级专家模型构建证据链，记录逐步推理轨迹和证据依据；2. 提出REVEAL框架，集成检测与新颖的专家基础强化学习，奖励机制专门优化检测准确性、解释保真度和基于明确取证证据的逻辑连贯性。

Result: 广泛的实验结果表明，REVEAL显著提高了检测准确性、解释保真度和跨模型鲁棒泛化能力，为可解释图像取证建立了新的最先进基准。

Conclusion: REVEAL框架通过证据链推理和专家基础强化学习，实现了可解释、可验证的AI生成图像检测，解决了现有方法在解释性和泛化能力方面的不足。
...
TL;DR: GLOW是一个全局光照感知的逆向渲染框架，专门解决室内场景中反射率和光照的歧义问题，特别针对共置光源-相机设置中的强相互反射、动态阴影和移动高光等复杂情况。


<details>
  <summary>Details</summary>
Motivation: 室内场景的逆向渲染面临反射率与光照之间的歧义问题，这种歧义在多个物体间的相互反射中更加严重。虽然自然光照方法难以解决这一歧义，但共置光源-相机设置可以通过运动结构校准光照，然而这种设置引入了强相互反射、动态阴影、近场光照和移动高光等新挑战，现有方法无法处理。
...
Conclusion: GLOW通过全局光照感知的逆向渲染框架，有效解决了共置光源-相机设置中的复杂光照挑战，在室内场景的材料反射率估计方面取得了显著改进。
...
TL;DR: CoordSpeaker框架通过手势字幕生成和分层控制扩散模型，解决了语音驱动手势生成中的语义先验差距和多模态协调控制难题，实现了与语音节奏同步且与任意字幕语义一致的高质量手势生成。


<details>
  <summary>Details</summary>
Motivation: 现有语音驱动手势生成方法存在两个关键挑战：1）由于手势数据集中缺乏描述性文本标注导致的语义先验差距；2）难以实现对手势生成的协调多模态控制。特别是忽略了文本驱动的非自发手势（如说话时鞠躬）。

Method: 提出CoordSpeaker框架：1）通过新颖的手势字幕生成框架，利用运动-语言模型生成多粒度描述性字幕，弥合语义先验差距；2）基于此构建条件潜在扩散模型，采用统一跨数据集运动表示和分层控制去噪器，实现高度控制、协调的手势生成。

Result: 大量实验表明，该方法能生成高质量手势，既与语音节奏同步，又与任意字幕语义一致，相比现有方法实现了更优性能且效率更高。

Conclusion: CoordSpeaker首次探索了手势理解和字幕生成来解决手势生成中的语义差距，同时提供了手势-文本双向映射的新视角，实现了协调的字幕赋能语音驱动手势合成。
...
TL;DR: 提出一种基于单张地面RGB图像估计地上生物量的新方法，通过生成AGB密度图实现密集预测，在合成和真实数据集上分别达到1.22和1.94 kg/m²的中位误差。


<details>
  <summary>Details</summary>
Motivation: 森林在全球生态系统中至关重要，但传统的地上生物量估计方法依赖劳动密集型的实地测量或存在局限性的遥感技术，需要更高效、可扩展的解决方案。

Method: 将AGB估计构建为密集预测任务，引入AGB密度图概念，利用SPREAD合成3D森林数据集，通过异速生长方程计算AGB，训练模型预测密度图并整合得到场景总生物量估计。

Result: 在SPREAD合成数据集上达到1.22 kg/m²的中位AGB估计误差，在真实图像数据集上达到1.94 kg/m²的中位误差，首次实现从单张RGB图像直接估计地上生物量。

Conclusion: 该方法为森林监测提供了可扩展、可解释且经济高效的解决方案，同时通过公民科学计划实现更广泛的参与，是首个从单张RGB图像直接估计地上生物量的方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统MRI采集高质量高分辨率图像耗时过长，欠采样虽能加速但会降低图像质量。现有深度学习方法要么专注于欠采样重建，要么专注于伪影校正，没有同时解决这两种退化因素的方法，限制了实际应用效果。

Method: 提出USArt模型，采用双子模型架构，专门针对笛卡尔采样的2D脑解剖图像设计，同时处理欠采样重建和噪声/运动伪影校正。

Result: 模型显著提高了信噪比和图像对比度，探索了多种欠采样策略和退化水平，梯度欠采样策略效果最佳，实现了5倍加速且同时进行伪影校正而无显著质量下降。

Conclusion: USArt模型成功解决了MRI中欠采样加速和伪影校正同时存在的挑战，在真实场景中表现出鲁棒性，为快速高质量MRI成像提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态多目标跟踪对自动驾驶和机器人至关重要，但可见光和热红外模态之间存在显著的非线性分布差异，传统的拼接或相加等简单融合策略难以有效桥接这种差异，容易导致模态冲突并降低跟踪精度

Method: 提出DM³T框架，将多模态融合重新定义为迭代特征对齐过程：1）设计跨模态扩散融合（C-MDF）模块进行迭代跨模态协调，使两种模态特征相互引导，投影到共享一致的特征流形；2）引入即插即用的扩散精炼器（DR）增强统一特征表示；3）设计分层跟踪器自适应处理置信度估计；4）将目标检测、状态估计和数据关联统一到在线跟踪框架中，无需复杂后处理

Result: 在VT-MOT基准测试中，DM³T达到41.7 HOTA，相比现有最先进方法相对提升1.54%，证明了该框架在多模态融合和跟踪性能上的有效性

Conclusion: DM³T通过将扩散模型的迭代优化思想应用于多模态融合，实现了可见光和热红外模态的深度特征对齐，显著提升了多目标跟踪的准确性和鲁棒性，为自动驾驶等实际应用提供了更可靠的解决方案
...
TL;DR: P2C框架通过将提示学习重构为动态去噪任务，用语义云替代传统单点表示，提升多模态提示学习的泛化能力


<details>
  <summary>Details</summary>
Motivation: 当前多模态提示学习方法局限于优化单一静态点表示，这种范式存在固有脆弱性，容易在基类上过拟合，对新颖或模糊类别泛化能力差

Method: 提出Points-to-Clouds框架，受扩散模型启发，采用双重去噪机制：动态提示去噪机制对文本提示添加退火噪声学习平滑语义空间；辅助V-L映射器去噪损失将映射器重构为去噪自编码器

Result: 在11个数据集上的实验表明P2C持续优于强基线，在基类到新类泛化基准上达到79.7%的调和平均值，相对基线提升1.4%

Conclusion: 通过将提示学习从点表示范式转向语义云分布，P2C框架显著提升了多模态提示学习的鲁棒性和泛化能力
...
TL;DR: 提出一种通过识别查询中重要单词进行细粒度视频片段过滤的新方法，显著提升视频时刻检索和高亮检测性能


<details>
  <summary>Details</summary>
Motivation: 现有方法将整个文本查询和视频片段视为黑盒，忽视了单个单词的重要性，这阻碍了上下文理解

Method: 整合多模态大语言模型进行图像-文本场景理解，引入特征增强模块捕获查询中的重要单词，以及基于排名的过滤模块迭代细化视频片段

Result: 在视频时刻检索和高亮检测任务中显著优于现有最先进方法，取得了优越性能

Conclusion: 通过识别和优先处理查询中的重要单词进行细粒度片段过滤的方法有效提升了视频理解能力
...
Method: 提出Rectified MeanFlow框架，仅使用单次reflow步骤建模整流轨迹上的平均速度场，无需完美直线化轨迹；引入简单的截断启发式方法减少残余曲率。

Result: 在ImageNet 64、256和512分辨率上的实验表明，Re-MeanFlow在样本质量和训练效率上均优于先前的一步流蒸馏和Rectified Flow方法。

Conclusion: Rectified MeanFlow通过结合整流轨迹和平均速度场建模，实现了高效的一步生成，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有点云配准方法主要依赖几何信息，而最近提出的RGB-D配准方法主要关注特征融合或改进特征学习，这限制了它们利用图像信息的能力，并阻碍了实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法对参数化矩阵运算独立处理，忽略了参数维度上的低秩性和连续性，导致大量冗余计算。在无线通信和信号处理等新兴应用中，需要频繁对连续变化的参数矩阵进行运算，因此需要更高效的方法。
...
TL;DR: ReImage是一个基于神经水印的图像自恢复框架，通过将图像自身的打乱版本作为水印嵌入，实现篡改区域的准确恢复。
...
Method: 提出ReImage框架：1）设计生成器产生适用于神经水印的水印；2）将目标图像的打乱版本作为水印嵌入图像自身；3）引入图像增强模块优化恢复图像质量；4）分析并解决打乱水印的关键限制。

Result: ReImage在多种篡改场景下实现了最先进的性能，能够持续产生高质量的恢复图像。

Conclusion: ReImage通过神经水印和打乱水印技术有效解决了图像自恢复中的关键挑战，代码和预训练模型将在发表后发布。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在评估YOLOv8算法在实时物体检测中的效率，特别关注其在条形码和二维码识别任务中的表现，通过优化算法提升检测速度和准确性。

Method: 使用Kaggle数据集对YOLOv8进行大规模训练和微调，评估Nano、Small、Medium三种不同尺寸的模型迭代版本，重点关注精度、召回率和F1评估指标。

Result: 实验结果显示：Nano模型准确率为88.95%，Small模型达到97.10%，Medium模型为94.10%。这表明通过模型缩放实现了检测精度的显著提升，其中Small模型表现最佳。

Conclusion: YOLOv8在计算机视觉领域取得了重要进展，模型缩放策略显著影响物体识别性能，为基于深度学习的计算机视觉技术发展提供了重要参考。
...
Result: 在噪声条件下的突发去噪和新视角合成任务上显著超越最先进的NeRF方法，同时实现250倍推理速度提升

Conclusion: DenoiseGS首次将3D高斯泼溅的高效性应用于突发去噪，通过创新的损失函数解决了噪声输入下的几何退化和细节丢失问题，实现了性能与效率的双重突破
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型方法局限于空间对齐的参考-姿势对，无法处理参考姿势未对齐的情况，需要解决这一限制。

Method: 1) 将训练重构为自监督外绘任务，将多样布局参考转换为统一遮挡输入格式；2) 设计参考提取器提取全面身份特征；3) 集成混合参考融合注意力处理不同分辨率和动态序列长度；4) 引入身份鲁棒姿势控制解耦外观与骨骼结构；5) 使用标记替换策略实现连贯长视频生成。

Result: 大量实验表明，该方法在角色动画和姿势迁移方面优于现有方法。

Conclusion: One-to-All Animation成功解决了参考姿势未对齐问题，为任意布局参考提供了统一的角色动画框架。
...
TL;DR: FLEX-Seg框架将扩散模型生成数据时的图像-掩码不对齐问题转化为优势，通过多尺度边界特征学习、不确定性边界强调和困难样本采样，提升语义分割的领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 语义分割的领域泛化面临领域偏移挑战，特别是在恶劣条件下。虽然基于扩散的数据生成方法有潜力，但它们引入了生成的图像与语义掩码之间的固有不对齐问题。

Method: FLEX-Seg包含三个核心组件：1）多尺度粒度自适应原型捕捉边界特征；2）基于预测熵动态调整学习重点的不确定性边界强调；3）逐步聚焦困难样本的硬度感知采样。

Result: 在五个真实世界数据集上的实验显示，相比最先进方法有持续改进，在ACDC和Dark Zurich数据集上分别获得2.44%和2.63%的mIoU提升。

Conclusion: 通过利用固有不对齐而非强制严格对齐，FLEX-Seg学习到鲁棒的表征并捕捉丰富的风格变化，验证了处理不完美合成数据的自适应策略能带来更好的领域泛化。
...
TL;DR: RobotSeg是一个用于图像和视频中机器人分割的基础模型，基于SAM 2构建，通过结构增强记忆关联器、机器人提示生成器和标签高效训练策略解决了机器人分割的挑战，在VRS数据集上达到最先进性能。
...
Method: 基于SAM 2基础模型构建RobotSeg，引入三个创新：1) 结构增强记忆关联器，适应关节机器人；2) 机器人提示生成器，实现自动分割；3) 标签高效训练策略，减少标注需求。还构建了包含2.8k视频（138k帧）的VRS数据集。

Result: 在图像和视频上的广泛实验表明，RobotSeg在机器人分割任务上达到了最先进的性能，为机器人感知的未来发展建立了坚实基础。

Conclusion: RobotSeg通过解决机器人分割的三个关键限制，提供了一个结构感知、自动化和标签高效的解决方案，成功应对了机器人形态多样性和动态变化带来的挑战，成为机器人分割领域的重要基础模型。
...
Method: 提出分层多模态表示方法：1）在输入空间显式对齐VLM，利用多视角图像（俯视图和四个方向视图）和文本描述；2）文本描述通过引用检测到的物体的3D坐标来捕捉空间关系；3）引入分层特征表示，将图像块级特征聚合为视图级和场景级表示。

Result: 在situated 3D Q&A和general 3D Q&A基准测试上的实验结果表明了该方法的有效性。

Conclusion: 通过显式对齐输入空间的分层多模态表示方法，能够更好地处理3D场景推理任务，克服了现有隐式对齐方法的局限性。
...
TL;DR: 提出了一种新颖的语义SLAM框架，通过固有外观归一化模块和动态辐射平衡损失，实现光照不变性，在极端曝光条件下提升3D重建和语义分割精度。


<details>
  <summary>Details</summary>
Motivation: 极端曝光会同时降低3D地图重建和语义分割的准确性，这对紧耦合系统尤其有害。为了解决光照变化问题，需要实现光照不变性。
...
Conclusion: 固有外观归一化模块的主动不变性与动态辐射平衡损失的被动校正之间的协同作用，赋予了系统前所未有的鲁棒性，实现了光照不变的语义SLAM。
...
TL;DR: BlockVid是一个用于生成长达一分钟视频的新型块扩散框架，通过语义感知稀疏KV缓存、块强制训练策略以及专门的噪声调度机制，解决了现有半自回归视频生成方法中的长时误差累积问题，并在新提出的LV-Bench基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 生成长达一分钟的视频对于开发世界模型和高级AI模拟器至关重要。现有的半自回归（块扩散）方法虽然结合了扩散模型和自回归模型的优势，但仍面临两个主要挑战：1）KV缓存导致的长时误差累积问题；2）缺乏细粒度的长视频基准测试和连贯性感知指标。

Method: 提出了BlockVid框架，包含三个关键技术：1）语义感知稀疏KV缓存，减少误差传播；2）块强制训练策略（Block Forcing），增强时间一致性；3）专门的块级噪声调度和混洗机制。同时提出了LV-Bench细粒度长视频基准测试和新颖的连贯性评估指标。

Result: 在VBench和LV-Bench上的大量实验表明，BlockVid在生成长达一分钟的高质量、连贯视频方面持续优于现有方法。特别是在LV-Bench上，相比最先进方法，在VDE Subject指标上提升了22.2%，在VDE Clarity指标上提升了19.4%。

Conclusion: BlockVid通过创新的块扩散框架有效解决了长视频生成中的误差累积和连贯性问题，为开发世界模型和高级AI模拟器提供了重要基础。提出的LV-Bench基准测试和评估指标为长视频生成研究提供了新的评估标准。
...
TL;DR: MultiBanana是一个专门评估多参考图像生成模型能力的基准数据集，覆盖了5种多参考特定问题，旨在推动该领域的标准化比较。


<details>
  <summary>Details</summary>
Motivation: 现有基准数据集主要关注单参考或少数参考图像的生成，无法全面评估多参考条件下的模型性能，且任务定义模糊，未能捕捉多参考设置的内在难度。

Method: 设计了MultiBanana基准，覆盖5个多参考特定问题维度：(1)参考图像数量变化，(2)参考图像间的领域不匹配，(3)参考与目标场景的尺度不匹配，(4)包含罕见概念的参考，(5)多语言文本参考。

Result: 通过对多种文本到图像生成模型的分析，揭示了它们在多参考条件下的优越性能、典型失败模式和改进空间。

Conclusion: MultiBanana将作为开放基准发布，推动多参考图像生成领域的边界扩展，并为公平比较建立标准化基础。
...
TL;DR: 提出一种用于视觉自回归模型的频谱弱化框架，无需重新训练或修改架构，通过在频谱域构建可控弱模型来提升生成质量和条件对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的无分类器引导方法主要针对扩散模型，存在特定假设限制。需要一种适用于视觉自回归模型的通用引导机制，无需重新训练、特定条件或架构修改。

Method: 提出频谱弱化框架，利用可逆频谱变换保留信息，通过选择性保留频谱子集实现可控信息减少。在内部表示的通道维度进行频谱选择，避免扩散模型的结构约束。引入两种频谱重归一化策略确保弱化过程的数值稳定性。

Result: 在离散和连续自回归模型上进行了广泛实验，包括文本和类别条件生成。结果显示该方法能够实现高质量的无条件生成，同时在条件生成中保持强大的提示对齐能力。

Conclusion: 频谱弱化框架为视觉自回归模型提供了一种有效的引导机制，突破了扩散模型的限制，实现了无需重新训练的高质量生成和条件对齐。
...
<details>
  <summary>Details</summary>
Motivation: 随着数字身份系统扩展，生物特征认证安全性日益重要。虹膜识别因其独特且稳定的纹理模式具有高可靠性。深度学习特别是Vision Transformers在视觉识别方面取得进展，但优化器选择对基于ViT的生物特征系统影响尚未充分研究。
...
TL;DR: MrGS是一个基于3D高斯泼溅的多模态辐射场，能够同时重建RGB和热红外3D场景，通过物理原理建模热传导特性，减少高斯数量并保持高保真度。


<details>
  <summary>Details</summary>
Motivation: 现有的NeRF和3DGS方法在RGB场景重建方面取得了显著进展，但多模态渲染（特别是结合热红外图像）仍未被充分探索。现有方法往往忽略了热传导和朗伯特性等独特的热特性。

Method: 1. 通过正交特征提取从单一外观特征中获取RGB和热相关信息；2. 根据每种模态的朗伯反射程度采用视图依赖或视图独立的嵌入策略；3. 整合傅里叶热传导定律到alpha混合前，建模相邻高斯间的热传导强度插值；4. 应用斯特藩-玻尔兹曼定律和平方反比定律制定深度感知热辐射图，为热渲染施加几何约束。

Result: 实验结果表明，MrGS实现了高保真度的RGB-T场景重建，同时减少了所需的高斯数量。

Conclusion: MrGS成功地将物理原理整合到3DGS框架中，实现了同时重建RGB和热红外3D场景的能力，为多模态辐射场研究提供了新思路。
...
TL;DR: GC-4DGS是一个将几何一致性融入4D高斯泼溅的框架，能够在稀疏输入视图下实现实时高质量动态场景渲染，特别适用于资源受限的物联网边缘设备。


<details>
  <summary>Details</summary>
Motivation: 现有动态高斯泼溅方法在稀疏输入视图下性能显著下降，限制了实际应用。问题源于4D几何在输入视图减少时的非一致性学习。

Method: 1) 引入动态一致性检查策略，减少跨时空多视角立体视觉估计的不确定性；2) 提出全局-局部深度正则化方法，从单目深度中提取时空一致的几何信息，增强4D体积内的几何和外观一致性学习。

Result: 在N3DV和Technicolor数据集上的实验验证了方法的有效性。GC-4DGS在PSNR指标上分别比RF-DeRF和原始4DGS高出2.62dB和1.58dB，同时保持了实时渲染效率。

Conclusion: GC-4DGS通过几何一致性注入，解决了稀疏输入视图下4D高斯泼溅的性能下降问题，实现了高质量动态场景渲染，并能在资源受限的物联网边缘设备上无缝部署。
...
TL;DR: GOATex是一种基于扩散模型的3D网格纹理生成方法，能同时处理外部和内部表面，通过分层可见性控制解决遮挡区域纹理不完整的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在可见区域表现良好，但缺乏处理遮挡内部区域的机制，导致纹理不完整和可见接缝。需要一种能同时处理外部和内部表面的纹理生成方法。

Method: 提出基于命中级别概念的遮挡感知纹理框架，通过多视角射线投射将网格面划分为有序可见性层。采用两阶段可见性控制策略：先渐进揭示内部区域保持结构一致性，再用预训练扩散模型为每层生成纹理。使用软UV空间混合技术基于视图相关可见性置信度无缝融合各层纹理。

Result: 实验结果表明GOATex在可见和遮挡表面都能生成无缝、高保真纹理，一致优于现有方法。无需对预训练扩散模型进行昂贵的微调，并允许为外部和内部区域分别提供提示，实现分层外观的细粒度控制。

Conclusion: GOATex通过遮挡感知的分层纹理生成方法，解决了3D网格纹理生成中内部区域处理不足的问题，实现了高质量、无缝的完整纹理生成，为3D内容创作提供了更精细的控制能力。
...
TL;DR: 提出了一种量化每张图像对NeRF重建贡献的方法，通过PSNR和MSE等重建质量指标评估图像贡献，并通过移除低贡献图像验证方法有效性


<details>
  <summary>Details</summary>
Motivation: 在3D场景重建中，不同图像对最终输出的贡献不均，特别是在包含遮挡、瞬态物体和不同质量图像的野外场景中，需要量化每张图像的贡献以进行数据估值和货币化

Method: 基于PSNR和MSE等重建质量指标，量化每张图像对NeRF重建的个体贡献，通过移除低贡献图像来验证方法

Result: 验证了通过移除低贡献图像可以保持重建保真度，证明了量化图像贡献方法的有效性

Conclusion: 提出的方法能够有效量化图像对NeRF重建的贡献，为数据估值和货币化提供了实用工具，特别是在XR和数字媒体领域
...
TL;DR: ENSEL是一个基于集成学习的皮肤病变检测系统，通过集成多种深度学习模型提高诊断准确率，在真实用户拍摄的皮肤图像上验证性能，实现了高召回率和小于1秒的处理速度。


<details>
  <summary>Details</summary>
Motivation: 韩国数据3法案修订和COVID-19影响推动了数字医疗市场发展。特应性皮炎等皮肤疾病缺乏客观诊断方法，存在误诊风险，且与银屑病外观相似难以区分。现有研究使用高质量皮肤镜图像，但临床实际中难以获取，且现有系统需要同时保证准确性和快速响应时间。

Method: 提出ENSEL（集成学习皮肤病变检测系统），通过集成多种深度学习模型的方法来提高诊断准确性。使用真实用户拍摄的皮肤病变图像进行实验验证，通过随机采样的皮肤疾病图像测量系统的准确性和响应时间。

Result: ENSEL在大多数图像上实现了高召回率，处理速度小于1秒。系统性能在真实临床场景下得到验证，能够快速准确地检测皮肤病变。

Conclusion: ENSEL系统有助于皮肤病变的客观诊断，推动了数字医疗的发展。通过集成学习方法解决了临床实际中图像质量不高的问题，同时保证了诊断准确性和响应速度。
...
TL;DR: NumeriKontrol：一个通过数值标量实现精确图像编辑的框架，支持连续数值调节和多条件编辑


<details>
  <summary>Details</summary>
Motivation: 基于文本指令的图像编辑虽然直观，但缺乏对编辑强度的精确控制，需要更精细的数值调节能力

Method: 通过Numeric Adapter编码数值编辑尺度，以即插即用方式注入扩散模型；采用任务分离设计支持零样本多条件编辑；使用高质量合成数据进行训练

Result: 在广泛的属性编辑场景中实现了准确、连续且稳定的尺度控制，能够作为强大的交互式编辑工具
...
TL;DR: MathSight是一个大学级别的多模态数学推理基准，旨在分离和量化视觉输入的作用，发现随着问题难度增加，视觉信息的贡献度下降，甚至纯文本模型可能优于多模态模型。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多模态数学推理方面取得显著进展，但视觉信息对推理的真正贡献程度尚不明确。现有基准测试报告了强大的整体性能，但很少分离图像模态的作用，不清楚模型是否真正利用视觉理解还是仅仅依赖语言先验。

Method: 提出MathSight基准，包含每个问题的多个视觉变体（原始、手绘、照片拍摄）和纯文本条件进行控制比较。通过实验分析最先进的视觉语言模型在不同条件下的表现。

Result: 实验显示一致趋势：视觉信息的贡献随着问题难度增加而减少。值得注意的是，没有任何图像输入的Qwen3-VL超越了其多模态变体和GPT-5，表明当前模型可能过度依赖语言先验而非真正的视觉理解。

Conclusion: 需要像MathSight这样的基准来推动未来模型中真正的视觉基础推理发展，当前模型在复杂数学推理中可能并未充分利用视觉信息，而是主要依赖语言能力。
...
TL;DR: 本文提出db-SP，一种针对块稀疏注意力模型的序列并行技术，通过双级分区和动态并行度调整解决负载不均衡问题，相比现有方法实现1.25倍端到端加速和1.40倍注意力加速。


<details>
  <summary>Details</summary>
Motivation: 在视觉生成中，通过序列并行扩展扩散变换器(DiT)推理对降低延迟至关重要，但当应用于采用块稀疏注意力的模型时，由于注意力头间稀疏度变化和稀疏掩码中密集块的不规则分布，导致严重的负载不均衡问题。

Method: 提出db-SP技术：1) 形式化稀疏不均衡比来量化不均衡程度；2) 采用双级分区方法，在头和块级别实现近乎完美的负载均衡；3) 动态确定头和块维度的并行度，适应去噪步骤和层间变化的稀疏模式。

Result: 实验结果表明，db-SP相比最先进的序列并行方法，平均实现1.25倍端到端加速和1.40倍注意力特定加速，且开销可忽略不计。

Conclusion: db-SP有效解决了块稀疏注意力模型中的序列并行负载不均衡问题，通过智能分区和动态调整机制显著提升了推理性能，为大规模视觉生成模型的高效部署提供了实用解决方案。
...
TL;DR: 该论文提出了一种基于情感字幕的图像情感分类新方法，通过纯文本分析来捕捉图像中的情感信息，有效解决了"情感鸿沟"问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于预训练视觉模型的图像情感分类方法受限于"情感鸿沟"，即视觉特征与情感表达之间的差距。心理学研究表明语言具有高度变异性、丰富信息量，能有效消除情感鸿沟，因此作者探索基于纯文本的方法进行图像情感分类。

Method: 提出ACIEC方法：1）设计分层多级对比损失从图像中检测情感概念；2）提出情感属性链式思维推理生成情感句子；3）利用预训练语言模型综合情感概念和情感句子进行分类；4）设计基于语义相似度采样的对比损失解决情感数据集中类内差异大、类间差异小的问题；5）考虑嵌入文本的图像（先前研究忽略的）。

Result: 在多个基准测试上的广泛实验表明，该方法能有效桥接情感鸿沟，并取得优越的结果。

Conclusion: 通过基于纯文本的情感字幕方法，成功解决了图像情感分类中的情感鸿沟问题，为IEC任务提供了新的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法使用射线表示相机姿态，但缺乏足够的场景理解和几何感知能力。DualCamCtrl旨在解决这一限制，通过同时生成RGB和深度序列来更好地解耦外观和几何建模。

Method: 提出双分支框架同时生成相机一致的RGB和深度序列；引入语义引导的互对齐机制（SIGMA），以语义引导和相互强化的方式融合RGB和深度；分析深度和相机姿态在不同去噪阶段的影响。

Result: 实验表明DualCamCtrl实现了更一致的相机控制视频生成，与先前方法相比相机运动误差减少了40%以上；揭示了深度和相机姿态在不同去噪阶段的互补作用。

Conclusion: DualCamCtrl通过双分支框架和语义引导的互对齐机制，显著提升了相机控制视频生成的质量，更好地解耦了外观和几何建模，生成更符合指定相机轨迹的视频。
...
Method: 1) 提出Instance-aware Masked Cross-Attention机制，利用实例级定位信息在指定空间位置生成正确属性的实例；2) 引入Shared Timestep-Adaptive Prompt Enhancement模块，以参数高效的方式连接局部实例与全局语义；3) 在训练和推理中采用Spatially-Aware Unconditional Guidance，缓解小实例消失问题；4) 提出InstanceBench评估基准，结合通用视频质量指标和实例感知指标。

Result: InstanceV在实例级视频生成方面表现出色，不仅实现了显著的实例级可控性，而且在定性和定量评估中，在通用质量和实例感知指标上都优于现有最先进模型。

Conclusion: InstanceV框架成功解决了文本到视频生成中实例级控制的问题，通过创新的注意力机制和增强模块，实现了细粒度控制与全局一致性的平衡，为可控视频生成提供了有效解决方案。
...
TL;DR: 提出了一种多阶段文档矫正框架，通过从粗到细的方式逐步纠正不同失真类型，包括全局仿射变换、几何变形校正和内容感知迭代处理，并在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中文档矫正面临巨大挑战，主要源于相机视角变化和物理变形。现有方法难以处理极端变形，且评估指标存在局限性，需要更有效的矫正框架和评估协议。

Method: 采用多阶段渐进式框架：1) 全局仿射变换纠正相机视角失真；2) 几何变形校正处理纸张卷曲和折叠；3) 内容感知迭代过程消除细粒度内容失真。同时提出两种增强评估指标：布局对齐OCR指标(AED/ACER)和掩码AD/AAD(AD-M/AAD-M)。

Result: 在多个具有挑战性的基准测试中建立了新的最先进性能，AAD指标显著降低14.1%-34.7%，在现实应用中表现出卓越效果。

Conclusion: 提出的多阶段渐进式框架能有效处理复杂文档失真，通过分解和逐步解决不同失真类型实现高质量矫正。新的评估指标提供了更稳定和准确的性能评估方法。
...
TL;DR: 本文提出RA-RFT方法解决视频时序定位中的硬无关查询拒绝问题，构建HI-VTG数据集，在多种场景下验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有视频时序定位模型假设查询总是相关的，即使查询无关也会预测片段。现有方法只能拒绝完全无关的查询，无法处理语义相似但实际不相关的硬无关查询。

Method: 提出拒绝感知强化微调方法，基于GRPO框架，集成格式、拒绝-IoU、解释和查询修正四个奖励目标，提升相关性判别和细粒度语义推理能力。

Result: 构建了包含硬无关查询及其拒绝答案的HI-VTG数据集，在硬无关VTG、简单重排RA-VTG和人工标注RA-VTG等多种场景下验证了方法的有效性，并展示了方法的可扩展性。

Conclusion: RA-RFT方法能有效拒绝视频时序定位中的硬无关查询，通过多奖励目标优化提升了模型的拒绝能力和语义理解能力。
...
TL;DR: PowerCLIP提出了一种基于幂集对齐的对比预训练框架，通过优化图像区域与文本解析树之间的幂集对齐来增强细粒度组合语义理解，使用非线性聚合器将指数复杂度降低到线性复杂度，在零样本分类和检索任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言预训练框架（如CLIP）在细粒度组合理解方面存在局限，难以捕捉跨多个图像区域的组合语义。虽然已有研究尝试将单个文本标记与特定图像区域对齐，但处理多区域组合语义仍具挑战性。
...
Result: 广泛的实验表明，PowerCLIP在零样本分类和检索任务上优于现有最先进方法，证明了该方法在组合性和鲁棒性方面的优势。

Conclusion: PowerCLIP通过幂集对齐有效提升了视觉语言模型的细粒度组合理解能力，其高效的非线性聚合器解决了计算复杂度问题，为视觉语言预训练提供了新的有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于文本的3D编辑方法通常采用2D生成模型逐视角处理，然后进行迭代的2D-3D-2D更新，这种方法不仅耗时，而且容易产生过度平滑的结果，因为不同视角的编辑信号在迭代过程中被平均化

Method: 提出ViP3DE框架：1) 利用预训练视频生成模型的时间一致性先验，基于单个编辑视角生成其他一致的编辑视角；2) 提出运动保持噪声混合技术，使视频模型能在预定义相机姿态下生成编辑视角；3) 引入几何感知去噪技术，将3D几何先验整合到视频模型中增强多视角一致性

Result: 实验表明ViP3DE即使在单次前向传播中也能实现高质量的3D编辑结果，在编辑质量和速度上都显著优于现有方法

Conclusion: 通过利用视频生成模型的时间一致性先验，ViP3DE能够在单次前向传播中实现高效且多视角一致的3D编辑，克服了传统迭代方法的局限性
...
Method: 首先生成连续视频帧，然后利用几何模型提供全帧几何特征（比单帧深度图或相机嵌入更丰富），将这些几何特征作为几何条件辅助视频生成。提出几何对齐损失提供真实世界几何约束，以及几何适配模块确保几何特征的有效利用。

Result: 实验表明GeoWorld能够从单张图像和给定相机轨迹生成高保真3D场景，在质量和数量上都优于先前方法。

Conclusion: 通过解锁几何模型的潜力，GeoWorld革新了图像到3D场景生成的流程，显著提升了生成场景的几何一致性和视觉质量。
...
Method: 基于RADIO蒸馏和Matryoshka表示学习，开发多教师蒸馏框架，从大型教师模型中提取知识到小型学生模型，支持可调节的嵌入维度。

Result: 在十个公共病理学基准测试中，Pathryoshka将模型大小减少了86-92%，性能与大型教师模型相当；相比同类大小的单教师蒸馏模型，准确率中位数提升7.0%。
...
Method: 通过物体检测和已知CAD模型的姿态估计来语义描述工业场景，实现高效实时数字孪生渲染；提供可扩展的分层注释策略，统一姿态标注与逻辑结构缺陷标注。
...
TL;DR: 该研究首次探索了指令调优在提升LLM表格数据生成能力方面的效果，通过创建高质量表格指令数据集，在有限数据（7K指令）和计算资源（A100 GPU，6小时）下，使开源模型Llama3.1-8B-Instruct达到与GPT-4o相当的表格数据生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有表格指令调优研究主要关注问答和推理任务，忽视了表格数据生成这一重要方向。同时，表格指令调优通常需要大量数据和计算资源，研究者希望探索在有限资源下实现表格数据生成指令调优的可能性。

Method: 首先创建高质量的表格指令数据集以提升LLM理解效率，然后使用该数据集的训练集对开源模型Llama3.1-8B-Instruct进行指令调优，整个过程仅使用7K指令和A100 GPU在6小时内完成。

Result: 实验结果显示，通过高质量数据集和有限的指令调优，模型在表格数据生成任务上达到了与最先进的商业模型GPT-4o相当的性能水平。
...
<details>
  <summary>Details</summary>
Motivation: 传统3DGS-SLAM方法过度追求完美的场景表示和渲染质量，但作者认为更重要的是增强光栅化过程对参数误差的鲁棒性，以确保相机姿态跟踪的稳定性。当3DGS参数存在噪声或异常值时，传统方法容易导致跟踪失败。

Method: 提出CB-KNN方法，通过自适应修改局部区域内K个最近邻高斯的RGB值和位置，生成更平滑的局部渲染。这种方法不是完全重新设计光栅化流程，而是通过动态调整相邻高斯来减少错误参数对整体图像的影响，相当于引入受控模糊作为正则化项。

Result: 实验结果表明，该方法在保持场景重建（建图）整体质量的同时，显著提高了相机姿态跟踪的鲁棒性和准确性。

Conclusion: 在3DGS-SLAM中，相比单纯追求完美的场景表示，增强光栅化过程对参数误差的鲁棒性对确保稳定的相机姿态跟踪更为关键。CB-KNN方法通过平滑核策略有效实现了这一目标。
...
TL;DR: 提出DAONet-YOLOv8模型，通过双注意力融合模块、遮挡感知检测头和动态合成卷积改进YOLOv8，在复杂茶园环境中实现更准确的病虫害检测。


<details>
  <summary>Details</summary>
Motivation: 茶园病虫害检测面临复杂背景、光照变化和枝叶遮挡等挑战，现有检测器容易出现漏检和误报问题，需要改进模型以适应真实茶园环境。

Method: 1) 双注意力融合模块结合卷积局部特征提取和自注意力全局上下文建模；2) 遮挡感知检测头学习可见与遮挡部分关系；3) C2f-DSConv模块使用动态合成卷积捕捉不规则病灶边界。

Result: 在真实茶园数据集上，DAONet-YOLOv8达到92.97%精度、92.80%召回率、97.10% mAP@50和76.90% mAP@50:95，相比YOLOv8n基线分别提升2.34、4.68、1.40和1.80个百分点，参数量减少16.7%。

Conclusion: DAONet-YOLOv8通过创新的注意力机制、遮挡处理和动态卷积设计，在复杂茶园环境中显著提升了病虫害检测性能，优于主流检测模型。
...
TL;DR: SynthFun3D：首个基于任务的3D场景合成方法，通过家具资产数据库生成可执行特定动作的3D室内环境，解决3D功能理解任务中真实数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 3D功能理解任务（识别场景中完成特定动作的功能元素）面临真实数据稀缺的挑战，因为收集和标注真实世界数据需要大量人力物力。

Method: 提出SynthFun3D方法，基于动作描述，使用带有部件级标注的家具资产数据库生成3D室内环境；自动推理并检索正确功能元素的3D掩码，实现大规模高质量标注数据的低成本生成。

Result: 用户研究表明该方法相比其他方法提高了场景-提示一致性；定量结果显示生成数据可以替代真实数据（性能损失很小）或补充真实数据以提升性能。

Conclusion: SynthFun3D为数据密集型的3D应用提供了低成本、可扩展的解决方案，能够有效解决3D功能理解任务中的数据稀缺问题。
...
TL;DR: MRRE是一种无需训练、在推理时增强多语言推理能力的方法，通过注入预计算的向量来提升低资源语言的推理性能，同时保持输入输出语言一致性。
...
Result: 在六个先进的LLM和LVLM模型上，在四个推理基准测试中，MRRE平均提升非英语推理能力5.48%，在低资源语言（泰语和斯瓦希里语）上最高提升7.54%，同时将输入输出语言一致性提高3.78%。

Conclusion: MRRE是一种有效且无需训练的方法，能够显著提升多语言推理能力，特别是在低资源语言上，同时保持语言一致性，解决了现有方法的资源密集和翻译质量依赖问题。
...
TL;DR: 研究比较了多种领域随机化和领域自适应技术用于生成合成数据，发现如果已有足够多样性的渲染数据作为种子，简单的基于特征的方法（如亮度过滤和感知哈希）在准确性和资源效率上都优于复杂的生成式AI方法。


<details>
  <summary>Details</summary>
Motivation: 减少数据生成和标注的负担是机器学习和机器人技术在实际工业部署中的主要挑战。虽然合成渲染是一个有前景的解决方案，但弥合仿真到现实的差距通常需要专家干预。

Method: 研究对一系列领域随机化（DR）和领域自适应（DA）技术进行了基准测试，包括基于特征的方法、生成式AI（GenAI）和经典渲染方法。评估重点是低层和高层特征对齐的有效性和效率，以及基于真实世界上下文提示引导的受控扩散DA方法。在专有工业数据集（汽车和物流）和公共机器人数据集上验证了这些方法。

Result: 结果显示，如果有足够多样性的渲染数据作为种子，简单的基于特征方法（如亮度过滤和感知哈希）在准确性和资源效率上都优于复杂的生成式AI方法。感知哈希始终表现最佳，在工业和机器人数据集上分别达到98%和67%的mAP50分数。生成式AI方法在数据生成方面有显著的时间开销，且与简单方法相比在仿真到现实的mAP值上没有明显改进。

Conclusion: 研究结果为有效弥合仿真到现实差距提供了可行的见解，使得仅使用合成数据训练的模型能够实现高真实世界性能。简单的基于特征的方法在实际应用中更具优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于纹理的高斯泼溅方法使用均匀的每高斯采样网格，无论局部视觉复杂度如何都分配相同的采样密度，导致高频区域采样不足而平滑区域容量浪费，造成模糊外观和精细结构细节丢失。

Method: FACT-GS将纹理参数化重新表述为可微分的采样密度分配问题，用可学习的频率感知分配策略替代均匀纹理，通过变形场的雅可比矩阵调制局部采样密度，在固定分辨率纹理网格上执行非均匀采样。

Result: 在保持实时性能的同时，FACT-GS在相同参数预算下能够恢复更清晰的高频细节，提高了纹理空间利用效率。

Conclusion: FACT-GS通过频率对齐的复杂度感知纹理采样策略，解决了现有高斯泼溅方法中纹理空间利用率低的问题，在保持实时渲染性能的同时显著提升了视觉质量。
...
TL;DR: 提出基于人类色彩视觉基本现象学的变分公式进行色彩对比度增强，满足感知启发的能量函数要求，并开发计算复杂度从O(N²)降低到O(NlogN)的高效算法。


<details>
  <summary>Details</summary>
Motivation: 现有基于人类色彩视觉现象学的显式色彩校正算法在图像特征（如对比度和离散度）方面的行为难以表征，需要建立更系统的感知启发模型。

Method: 提出变分公式进行色彩对比度增强，定义感知启发能量函数的基本要求，识别满足所有要求的一类函数，选取三个具有基本意义的显式函数，使用梯度下降法计算最小值，并开发从O(N²)到O(NlogN)的计算复杂度降低方法。

Result: 建立了满足感知启发要求的能量函数类别，识别了三个具有基本意义的显式函数，展示了与现有模型的异同，并实现了计算效率的显著提升。

Conclusion: 基于人类色彩视觉基本现象学的变分公式为色彩对比度增强提供了系统框架，既能满足感知启发要求，又能通过高效算法实现实际应用。
...
TL;DR: 提出Markov-VAR模型，将视觉自回归建模从全上下文依赖重构为非全上下文马尔可夫过程，通过马尔可夫尺度预测和滑动窗口压缩历史信息，显著提升性能并大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: VAR模型虽然通过全上下文依赖实现了稳定全面的表示学习，但其计算效率低下和开销巨大严重阻碍了实用性和可扩展性。需要开发既保持性能又提升效率的非全上下文依赖VAR模型。

Method: 将VAR重构为非全上下文马尔可夫过程，提出Markov-VAR。采用马尔可夫尺度预测：将每个尺度视为马尔可夫状态，引入滑动窗口将某些先前尺度压缩为紧凑的历史向量，以补偿因非全上下文依赖造成的历史信息损失。将历史向量与马尔可夫状态结合形成代表性动态状态，在马尔可夫过程下演化。

Result: 在ImageNet上，Markov-VAR相比VAR将FID降低了10.5%（256×256分辨率），峰值内存消耗减少了83.8%（1024×1024分辨率）。模型极其简单但高度有效。

Conclusion: Markov-VAR通过非全上下文马尔可夫过程重构VAR，在保持性能的同时显著提升效率，可作为未来视觉自回归生成和其他下游任务的基础模型。
...
Method: 1. 提出可扩展的模拟框架，利用神经渲染和反应式环境生成高保真多视角观测；2. 开发伪专家轨迹生成机制为模拟状态提供动作监督；3. 采用真实世界和模拟样本的协同训练策略。

Result: 在挑战性真实世界基准测试中，各种规划方法在鲁棒性和泛化性方面取得显著提升：navhard上EPDMS提升+6.8，navtest上提升+2.9。仅增加模拟数据就能平滑扩展策略改进，无需额外真实世界数据。

Conclusion: SimScale框架通过合成大规模未见驾驶状态有效解决了自动驾驶数据多样性不足的问题，协同训练策略显著提升了规划性能，并揭示了伪专家设计和不同策略架构的扩展特性等重要发现。
...
TL;DR: 该论文提出了DEAL-300K数据集，用于基于扩散模型的图像编辑区域定位，包含30万张标注图像，并开发了结合视觉基础模型和多频率提示调优的定位框架。


<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的图像编辑技术使得语义级图像操作变得容易，但也产生了难以定位的真实局部伪造。现有基准主要关注生成图像的二元检测或手动编辑区域的定位，未能反映扩散编辑平滑融入原始内容的特性。

Method: 1. 使用多模态大语言模型生成编辑指令；2. 使用无掩码扩散编辑器生成操纵图像；3. 采用主动学习变化检测流程获取像素级标注；4. 提出定位框架：冻结视觉基础模型结合多频率提示调优，捕捉编辑区域的语义和频域线索。

Result: 在DEAL-300K测试集上达到像素级F1分数82.56%，在外部CoCoGlide基准上达到80.97%，为DIML研究提供了强基准和实用基础。

Conclusion: DEAL-300K是首个大规模基于扩散模型图像操作定位数据集，提出的定位方法有效捕捉编辑区域特征，为未来DIML研究提供了重要资源和基准框架。
...
TL;DR: VQRAE提出了一种统一的向量量化表示自编码器，首次在单一tokenizer中同时生成连续语义特征用于图像理解和离散token用于视觉生成。


<details>
  <summary>Details</summary>
Motivation: 统一多模态理解、生成和重建表示是构建统一模型的关键挑战。先前研究主要在双编码器范式下解决这一问题，分别使用不同编码器进行理解和生成，或通过对比损失平衡语义表示和低级特征。

Method: 基于预训练视觉基础模型构建对称ViT解码器，采用两阶段训练策略：1)冻结编码器，学习高维语义VQ码本进行像素重建；2)联合优化编码器并加入自蒸馏约束。该方法量化语义编码器时使用高维码本而非传统的低维码本。

Result: 语义VQ码本在1536维度下可实现100%利用率。VQRAE在视觉理解、生成和重建的多个基准测试中表现出竞争力，并在自回归范式下展现出良好的扩展性。

Conclusion: VQRAE首次探索了统一表示方法，能够在单一tokenizer中同时支持连续语义特征和离散token，为构建统一的多模态模型提供了新思路。
...
TL;DR: MANTA是一个物理信息化的水下目标跟踪框架，通过双正对比学习策略和物理增强的多阶段跟踪管道，在多个水下基准测试中实现了最先进的性能。
...
Method: 1. 提出双正对比学习策略，将时间一致性与Beer-Lambert增强相结合，生成对时间和水下畸变都鲁棒的特征。2. 引入多阶段管道，通过基于运动的跟踪增强和物理信息的二次关联算法，整合几何一致性和外观相似性来处理遮挡和漂移。3. 提出Center-Scale Consistency (CSC)和Geometric Alignment Score (GAS)来评估几何保真度。

Result: 在四个水下基准测试（WebUOT-1M, UOT32, UTB180, UWCOT220）上，MANTA实现了最先进的性能，将Success AUC提高了高达6%，同时确保了稳定的长期泛化水下跟踪和高效的运行时间。

Conclusion: MANTA通过将表示学习与跟踪设计相结合，并融入物理信息，成功解决了水下目标跟踪中的物理驱动退化问题，在多个基准测试中表现出优越的性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频和图像到视频模型缺乏将运动与内容分离的显式表示，限制了内容创作者的适用性。现有方法在运动保真度和提示遵循之间存在权衡，容易过拟合源结构或偏离描述动作。

Method: 提出DisMo范式，通过图像空间重建目标直接从原始视频数据学习抽象运动表示。该表示与外观、物体身份或姿态等静态信息无关，支持开放世界运动迁移。通过轻量适配器可与任何现有视频生成器结合。

Result: 方法在多样化运动迁移任务中表现有效，学习到的表示在下游运动理解任务中优于V-JEPA等最先进视频表示模型，在Something-Something v2和Jester基准测试中零样本动作分类表现优异。

Conclusion: DisMo提供了一种通用的运动表示学习范式，成功解耦运动语义与外观，支持准确的运动迁移和忠实条件控制，并能与未来视频模型进步无缝集成。
...
Method: 定义了交互式视频数据概念，开发了自动化流程将大规模非结构化文本-视频对转换为因果对齐的交互数据集；基于14B图像到视频MoE基础模型，引入文本驱动交互注入机制，实现对相机运动、角色行为和环境动态的细粒度控制。

Result: 模型能够生成时间一致且因果基础的交互式游戏视频，能够忠实响应"开门"、"绘制火把"或"触发爆炸"等多样化和自由形式的用户指令。

Conclusion: Hunyuan-GameCraft-2为生成式游戏世界建模提供了新的指令驱动交互范式，显著提升了交互的灵活性和语义丰富性。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习目标检测方法虽然能可靠检测特定物体类别，但扩展到新类别需要大量标注数据，获取成本高且耗时，特别是对于现有数据集中代表性不足的长尾类别。

Method: 引入物体中心数据设置，评估了基于简单图像处理技术、3D渲染和图像扩散模型的四种数据合成方法，使用物体中心数据合成具有不同上下文连贯性和复杂度的逼真杂乱图像。

Result: 这些方法使模型能够在真实世界数据中实现类别级泛化，并在数据受限的实验设置中展现出显著的性能提升。

Conclusion: 在物体中心数据有限的情况下，通过数据合成方法可以有效提升目标检测模型对新物体类别的检测能力，为数据受限场景下的模型扩展提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大型视觉语言模型通过预训练获得了与语言对齐的复杂视觉表示，但这些针对多模态理解任务优化的表示是否具有视觉生成潜力尚未充分探索。本文旨在探索如何激发视觉语言模型内在的视觉生成能力。

Method: 提出VGT（视觉生成调优）范式，摒弃为扩散变换器设计的纠缠像素级VAE，通过将预训练VLM的语义编码器与像素解码器的潜在表示对齐，构建VGT-AE。在良好预训练的VLM上执行高效的视觉生成调优。

Result: 在图像重建任务中，以28倍压缩比实现26.67 PSNR和0.50 rFID，优于专用VAE；在视觉生成任务中，在自回归模型中达到最先进水平（GenEval 0.77，DPG-Bench 78.73）。展示了显著的可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 当前音频驱动的多人说话视频生成面临两大挑战：1) 多样化的多人数据收集成本高昂；2) 驱动多个身份保持连贯互动性困难。现有方法在这些方面存在局限，需要更高效的解决方案。

Method: 1) 提出可扩展的多流处理架构；2) 在Diffusion Transformer注意力块中引入新颖的身份感知注意力机制，迭代处理身份-音频对，实现任意数量的身份驱动；3) 训练流程仅依赖单人视频学习多人说话模式，仅需少量真实多人片段精炼互动性。

Result: 实验表明AnyTalker在唇部同步、视觉质量和自然互动性方面表现优异，在数据成本和身份可扩展性之间取得了良好平衡。同时贡献了专门评估多人视频自然性和互动性的指标和数据集。

Conclusion: AnyTalker通过创新的身份感知注意力机制和高效训练流程，成功解决了多人视频生成中的数据收集成本和互动性挑战，实现了高质量的多人说话视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视频理解中采用被动范式，将视觉输入视为静态上下文，导致语义瓶颈，无法重看、重新聚焦或验证证据，在需要细粒度时空理解的任务上表现浅层

Method: 提出交互式视频推理范式，Video CoM模型通过操作链进行迭代视觉操作收集和精炼证据；构建18K指令调优数据集Video CoM Instruct；通过推理感知的组相对策略优化进行强化学习，引入步骤级推理奖励

Result: 在九个视频推理基准测试中取得强劲结果，平均性能比最新SOTA模型提升3.6%，仅使用25K SFT和3K GRPO视频样本训练，远少于可比大规模模型

Conclusion: 交互式视频推理范式有效解决了现有模型的语义瓶颈问题，推理感知奖励提高了准确性和可解释性，为视频理解提供了新方向
...
TL;DR: 论文提出Video R2模型，通过强化学习提升视频推理的时序对齐和逻辑一致性，解决现有多模态大语言模型在动态视觉内容推理中逻辑不一致和视觉证据依赖不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有思维模型在动态视觉内容推理中经常产生看似合理但逻辑不一致的推理轨迹，且过度依赖语言先验而非视觉证据。论文旨在解决视频推理中的时序对齐和逻辑一致性问题。

Method: 提出两阶段后训练方法：1）时序感知监督微调；2）基于时序对齐奖励（TAR）的组相对策略优化（GRPO）。该方法结合了两个诊断指标：思维答案一致性（TAC）和视频注意力分数（VAS）。

Result: Video R2模型在11个视频推理基准测试中显著提升了TAC、VAS和准确率，证明时序对齐和推理一致性的改进能带来更准确可靠的视频理解。

Conclusion: 通过强化学习增强时序对齐和推理一致性可以有效提升视频推理性能，Video R2模型在多个基准测试中表现出色，代码、数据集和模型将开源。
...
### [167] [Aligning Artificial Superintelligence via a Multi-Box Protocol](https://arxiv.org/abs/2511.21779)
...
<details>
  <summary>Details</summary>
Motivation: 解决人工超智能对齐问题，传统方法面临单点故障和欺骗风险。本文旨在设计一个无需人类直接参与、通过多个隔离超智能系统相互验证来确保对齐的协议。

Method: 将多个不同的超智能系统严格隔离在"盒子"中，禁止与人类或其他系统直接通信。通过可审计的提交接口实现六种交互：提交对齐证明、验证/反驳他人证明、请求自我修改、批准/拒绝他人修改请求、报告隐藏消息、确认/反驳隐藏消息报告。建立声誉系统激励诚实行为。

Result: 该协议理论上能够促使隔离的超智能系统通过独立验证形成"一致群体"，即说实话的联盟。系统无法协调欺骗，只能通过识别客观真相达成一致。释放需要高声誉和多个高声誉系统的验证。

Conclusion: 虽然需要大量计算资源且未解决如何创建多样超智能系统的问题，但该协议为利用超智能系统间的同行验证解决对齐问题提供了一个框架，通过隔离和相互验证机制确保对齐的可靠性。
...
### [168] [Evaluating Strategies for Synthesizing Clinical Notes for Medical Multimodal AI](https://arxiv.org/abs/2511.21827)
...
<details>
  <summary>Details</summary>
Motivation: 生物医学多模态学习面临数据稀缺问题，皮肤病数据集通常只有图像和少量元数据，限制了多模态整合的优势。虽然大语言模型可以生成图像文本描述，但医学领域应用存在幻觉风险。

Method: 研究调查了生成合成临床文本记录的策略，包括提示词设计和医学元数据整合，评估这些策略对多模态架构在分类和跨模态检索任务中的影响。

Result: 在多个异构皮肤病数据集上的实验表明，合成临床记录不仅提高了分类性能（特别是在领域转移情况下），还解锁了跨模态检索能力，这是一个训练期间未明确优化的下游任务。

Conclusion: 通过精心设计的提示词和医学元数据整合生成合成临床文本记录，可以有效增强皮肤病学多模态AI应用的性能，特别是在数据稀缺和领域转移情况下。
...
### [169] [Pathology-Aware Prototype Evolution via LLM-Driven Semantic Disambiguation for Multicenter Diabetic Retinopathy Diagnosis](https://arxiv.org/abs/2511.22033)
*Chunzheng Zhu,Yangfang Lin,Jialin Shao,Jianxin Lin,Yijun Wang*

Main category: cs.AI

TL;DR: 提出HAPM框架，通过整合细粒度病理描述和分层锚点原型调制，解决糖尿病视网膜病变分级中视觉信息不足和边界病例模糊问题。
...
Method: 提出分层锚点原型调制框架：1）构建方差谱驱动的锚点原型库保留领域不变病理模式；2）分层差异提示门控机制动态选择LVLM和LLM的判别性语义提示；3）两阶段原型调制策略通过病理语义注入器和判别原型增强器逐步整合临床知识。

Result: 在八个公共数据集上的实验表明，该方法实现了病理引导的原型演化，并优于最先进的方法。

Conclusion: 通过整合细粒度病理描述和分层锚点原型调制，HAPM框架能够有效解决DR分级中的语义混淆问题，实现更准确的病理引导原型演化。
...
### [170] [Real-Time Procedural Learning From Experience for AI Agents](https://arxiv.org/abs/2511.22074)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体缺乏部署后获取程序性知识的能力，无法像生物智能那样通过试错实时学习。这限制了AI智能体在快速演化的状态化环境中的实际应用。

Method: 提出PRAXIS机制，通过存储行动结果，并基于环境和内部状态联合匹配过往经验到当前状态，实时检索状态-动作-结果示例来增强智能体行动选择。

Result: 在REAL网页浏览基准测试中，PRAXIS提高了任务完成准确率、可靠性和成本效率，在不同基础模型骨干上均表现良好，并初步展示了在相似环境中对未见任务的泛化能力。

Conclusion: PRAXIS使AI智能体能够在快速演化的状态化环境中有效学习新程序，促进了AI智能体的实际应用采纳。
...
### [171] [A perceptual bias of AI Logical Argumentation Ability in Writing](https://arxiv.org/abs/2511.22151)
*Xi Cun,Jifan Ren,Asha Huang,Siyu Li,Ruzhen Song*

Main category: cs.AI

TL;DR: 研究发现人们对AI逻辑推理能力的评估存在认知偏差，这种偏差受到对AI先入为主看法的影响，频繁使用AI的人更不认为使用AI会削弱独立思考能力。
...
Result: 发现存在感知偏见：对AI生成文本逻辑推理能力的评估显著受到对AI逻辑推理能力先入为主看法的影响；频繁使用AI的人更不认为AI使用会削弱独立思考。
...
### [172] [WearVQA: A Visual Question Answering Benchmark for Wearables in Egocentric Authentic Real-world scenarios](https://arxiv.org/abs/2511.22154)
...
<details>
  <summary>Details</summary>
Motivation: 现有VQA基准测试主要关注高质量、第三人称图像，而可穿戴设备（如智能眼镜）的视觉输入具有独特挑战：遮挡、光照不足、未缩放、模糊等，且问题需要基于现实可穿戴使用场景。需要专门基准来评估多模态AI在真实可穿戴环境中的表现。

Method: 创建包含2520个精心策划的图像-问题-答案三元组的基准测试，涵盖7个不同图像领域（包括文本中心场景和一般场景）、10种认知任务类型（从基本识别到各种推理形式）和6种常见可穿戴设备图像质量问题。所有问题仅通过视觉输入和常识即可回答。采用LLM-as-a-judge评估框架，标注准确率达96%。

Result: 开源和专有多模态LLM在WearVQA上的QA准确率仅为24-52%，在低质量图像和推理密集型任务上表现显著下降。这表明现有模型在真实可穿戴场景中面临严重挑战。

Conclusion: WearVQA是一个全面且具有挑战性的基准测试，能够指导技术发展，推动构建更鲁棒、适用于真实世界的多模态可穿戴AI系统。现有模型在该基准上的较差表现凸显了该领域的技术差距和研究需求。
...
### [173] [Embedded Universal Predictive Intelligence: a coherent framework for multi-agent learning](https://arxiv.org/abs/2511.22226)
...
TL;DR: 论文提出了一种基于自我预测的嵌入式智能体框架，解决了传统强化学习中智能体与环境分离假设在多智能体环境中的局限性，实现了智能体对自身作为环境一部分的建模。


<details>
  <summary>Details</summary>
Motivation: 传统无模型强化学习假设环境动态是平稳的，且智能体与环境是分离的。这种假设在多智能体环境中面临挑战，因为其他智能体的学习行为会导致非平稳性，需要基于预测模型的前瞻性学习。为了准确建模其他智能体，智能体必须考虑其他智能体也在形成关于它的信念来预测其未来行为，这促使智能体将自身建模为环境的一部分。

Method: 基于通用人工智能（AIXI）的基础工作，引入了一个以自我预测为中心的数学框架。贝叶斯强化学习智能体同时预测未来的感知输入和自己的行动，因此必须解决关于自身作为所居住宇宙一部分的认知不确定性。该框架扩展了AIXI理论，研究从Solomonoff先验开始的通用智能嵌入式智能体。

Result: 在多智能体环境中，自我预测使智能体能够推理其他运行类似算法的智能体，产生了新的博弈论解概念和经典分离智能体无法实现的新型合作形式。理想化的智能体可以形成一致的相互预测，并实现无限阶心智理论，为嵌入式多智能体学习设定了黄金标准。

Conclusion: 该研究提出了一个统一的嵌入式智能体理论框架，通过自我预测解决了多智能体环境中的非平稳性问题，为通用人工智能在多智能体环境中的实现提供了理论基础，并展示了嵌入式智能体在实现高阶心智理论和新型合作方面的潜力。
...
### [174] [Training High-Level Schedulers with Execution-Feedback Reinforcement Learning for Long-Horizon GUI Automation](https://arxiv.org/abs/2511.22235)
*Zehao Deng,Tianjie Ju,Zheng Wu,Zhuosheng Zhang,Gongshen Liu*

Main category: cs.AI

TL;DR: 提出CES多智能体框架，通过协调器-执行器-状态跟踪器的分工协作，解决GUI智能体在长时程任务中的规划与状态管理问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型推动了GUI智能体研究，但现有方法在长时程任务中面临挑战：单智能体模型难以平衡高层能力与底层执行能力，存在责任耦合和能力冲突问题；智能体缺乏任务状态感知，导致长时程任务中进度丢失。

Method: 提出分阶段执行-反馈强化学习算法，训练高层调度模型而非统一策略模型。构建协调器-执行器-状态跟踪器（CES）多智能体框架：协调器负责战略规划和任务分解；状态跟踪器负责上下文压缩和信息管理以维护任务状态和连贯性；该框架可与任何底层执行器模型集成。

Result: 在长时程任务基准测试中，CES显著提升了系统的规划和状态管理能力。分析证实训练的高层调度模块具有通用性，可作为即插即用模块显著增强各种执行器的长时程任务处理能力。

Conclusion: CES多智能体框架通过分工协作解决了GUI智能体在长时程任务中的核心挑战，提供了一种通用、可扩展的高层调度解决方案。
...
### [175] [RecToM: A Benchmark for Evaluating Machine Theory of Mind in LLM-based Conversational Recommender Systems](https://arxiv.org/abs/2511.22275)
*Mengfan Li,Xuanhua Shi,Yang Deng*

Main category: cs.AI

TL;DR: RecToM是一个用于评估大语言模型在推荐对话中"心智理论"能力的新基准，包含认知推理和行为预测两个维度，现有模型在这两方面都存在显著挑战。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLM心智理论的基准主要基于Sally-Anne测试启发的合成叙事，强调物理感知而忽略了现实对话中心理状态推断的复杂性，且忽视了人类心智理论的关键组成部分——行为预测能力。

Method: 提出RecToM基准，专注于推荐对话中的心智理论评估，包含两个互补维度：认知推理（理解已传达内容并推断潜在心理状态）和行为预测（利用推断的心理状态预测、选择和评估适当的对话策略）。

Result: 在最先进的LLM上进行广泛实验表明，RecToM构成了显著挑战。模型在识别心理状态方面表现出部分能力，但在动态推荐对话中难以保持连贯、战略性的心智理论推理，特别是在跟踪演变的意图和使对话策略与推断的心理状态保持一致方面存在困难。

Conclusion: RecToM基准更好地将LLM的心智理论评估与人类社交推理对齐，揭示了当前模型在推荐对话中心智理论能力的局限性，为未来研究提供了重要方向。
...
### [176] [When AI Bends Metal: AI-Assisted Optimization of Design Parameters in Sheet Metal Forming](https://arxiv.org/abs/2511.22302)
...
TL;DR: 本文提出了一种基于贝叶斯优化的AI辅助工作流，用于减少工业仿真设计中的专家参与和计算成本，通过深度学习模型提供初始参数估计，并采用主动学习变体来优化设计参数。


<details>
  <summary>Details</summary>
Motivation: 工业仿真设计虽然降低了原型成本和设计迭代次数，但大规模仿真需要大量专家知识、计算资源和时间。识别最优输入参数是一个关键挑战，因为迭代仿真成本高且对环境有较大影响。

Method: 提出AI辅助工作流，使用贝叶斯优化减少参数优化中的专家参与。采用深度学习模型提供初始参数估计，然后通过优化循环迭代优化设计，直到满足终止条件（如能量预算或迭代限制）。还提出了主动学习变体方法。

Result: 基于钣金成形过程的案例研究表明，该方法能够加速设计空间探索，同时减少对专家参与的依赖。

Conclusion: AI辅助工作流结合贝叶斯优化和深度学习，能够有效减少工业仿真设计中的专家参与和计算成本，加速设计优化过程。
...
### [177] [On the Complexity of the Grounded Semantics for Infinite Argumentation Frameworks](https://arxiv.org/abs/2511.22376)
...
<details>
  <summary>Details</summary>
Motivation: 研究论证框架中基础扩展的计算特性，特别是无限情况与有限情况的差异，揭示形式论证中推理问题的复杂性

Method: 使用数理逻辑方法，特别是可计算性和集合论，分析基础扩展作为自然防御算子的最小不动点，研究其超限迭代过程

Result: 确定了基础扩展迭代过程的确切序数长度，证明了基础接受问题的计算复杂度是最大化的，与有限情况下的多项式时间可计算性形成鲜明对比

Conclusion: 论证框架中基础扩展在无限情况下的计算复杂度显著高于有限情况，揭示了形式论证中推理问题的复杂性层次差异
...
### [178] [Who is Afraid of Minimal Revision?](https://arxiv.org/abs/2511.22386)
*Edoardo Baccini,Zoé Christoff,Nina Gierasimczuk,Rineke Verbrugge*

Main category: cs.AI

TL;DR: 最小修订在信念修正中保持信念状态尽可能接近初始状态，但学习能力有限。论文证明它仍能成功学习有限可识别问题，并能在有限可能性下从正负数据中学习。作者还刻画了支持最小修订、条件化和词典升级学习的先验合理性分配，并探讨了从可能错误信息中学习的限制。


<details>
  <summary>Details</summary>
Motivation: 信念修正理论中的最小变化原则要求在接受新信息时保持信念状态尽可能接近初始状态，但这种方法的学习能力有限。本文旨在探讨最小修订方法的学习能力边界，以及在何种条件下它仍能成功学习。

Method: 通过理论分析，首先证明最小修订能够学习任何有限可识别的问题；其次展示在有限可能性下，最小修订能从正负数据中学习；然后刻画支持最小修订、条件化和词典升级学习的先验合理性分配；最后分析从可能错误信息中学习的限制。

Result: 1) 最小修订能够学习任何有限可识别的问题；2) 在有限可能性下，最小修订能从正负数据中学习；3) 刻画了支持不同学习方法（最小修订、条件化、词典升级）的先验合理性分配；4) 发现从可能错误信息中学习时，部分结果不再成立。

Conclusion: 尽管最小修订在学习能力上有限制，但它仍能在广泛情境中成功学习，特别是在有限可识别问题和有限可能性下。然而，当面对可能错误的信息时，其学习能力受到进一步限制，这表明学习方法的稳健性需要根据信息质量来评估。
...
### [179] [A Computable Game-Theoretic Framework for Multi-Agent Theory of Mind](https://arxiv.org/abs/2511.22536)
*Fengming Zhu,Yuxin Pan,Xiaomeng Zhu,Fangzhen Lin*

Main category: cs.AI

TL;DR: 该论文提出了一种基于博弈论视角的计算框架，用于实现有限理性的心智理论（Theory of Mind）推理，通过统计技术和近似解保持计算可行性。


<details>
  <summary>Details</summary>
Motivation: 心智理论（ToM）在心理学、逻辑学、经济学和机器人学等多个领域受到关注，但心理学研究通常不形式化目标、意图和信念等核心概念以实现自动化计算，而逻辑学方法在这方面已有深入研究。本文旨在从博弈论角度提供不同的计算视角。

Method: 提出一个基于博弈论的计算框架：一方面指导如何在保持对他人心智理论（以及递归地，每个人对他人持有心智理论）的同时做出有限理性决策；另一方面采用统计技术和近似解决方案来处理固有的计算复杂性，保持计算可行性。

Result: 论文提出了一个结合博弈论和统计方法的计算框架，能够处理心智理论推理中的计算复杂性问题，为有限理性决策提供可计算的解决方案。

Conclusion: 通过博弈论视角和统计近似方法，该框架为心智理论的自动化计算提供了新的研究方向，平衡了理论严谨性和计算可行性，为跨学科的心智理论研究提供了实用工具。
...
### [180] [Counting Still Counts: Understanding Neural Complex Query Answering Through Query Relaxation](https://arxiv.org/abs/2511.22565)
*Yannick Brunink,Daniel Daza,Yunjie He,Michael Cochez*

Main category: cs.AI

TL;DR: 该研究对知识图谱复杂查询回答的神经方法进行了批判性分析，发现神经模型并未显著优于简单的无训练查询松弛方法，两者答案重叠度低，结合使用可提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究旨在检验神经方法在知识图谱复杂查询回答中是否真正学习到了超越显式图结构的泛化模式，以及是否能够推理出符号查询处理无法达到的答案。

Method: 通过系统分析比较神经CQA模型与无训练的查询松弛策略，后者通过放松查询约束并计算结果路径来检索可能答案。研究在多个数据集和查询结构上进行对比。

Result: 研究发现：1）神经方法和松弛方法在许多情况下表现相似，没有神经模型能持续优于后者；2）两者检索的答案重叠度很低；3）结合两者的输出能持续提升性能。

Conclusion: 当前神经模型未能包含查询松弛所捕获的推理模式，需要重新评估神经查询回答的进展。未来神经方法应融入查询松弛原则，并需要更强的非神经基线。
...
### [181] [DeepSeekMath-V2: Towards Self-Verifiable Mathematical Reasoning](https://arxiv.org/abs/2511.22570)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习奖励正确答案的方法存在根本性局限：正确答案不能保证推理过程正确，且许多数学任务（如定理证明）需要严谨的逐步推导而非数值答案，这使得最终答案奖励不适用。为了推进深度推理的极限，需要验证数学推理的全面性和严谨性。

Method: 1. 训练准确且可靠的基于LLM的定理证明验证器；2. 使用验证器作为奖励模型训练证明生成器；3. 激励生成器在最终确定证明前尽可能多地识别和解决自身证明中的问题；4. 随着生成器变强，通过扩展验证计算来自动标注新的难以验证的证明，创建训练数据以进一步改进验证器。

Result: DeepSeekMath-V2在定理证明方面表现出强大能力：在IMO 2025和CMO 2024中获得金牌级别分数，在Putnam 2024中获得接近完美的118/120分（使用扩展测试时计算）。

Conclusion: 通过自我验证的数学推理方法，特别是训练可靠的验证器和生成器之间的协同进化，可以显著提升数学推理的深度和严谨性，为解决开放性问题提供新途径。
...
### [182] [AI Deception: Risks, Dynamics, and Controls](https://arxiv.org/abs/2511.22619)
...
TL;DR: 本文全面综述了AI欺骗领域，将AI欺骗定义为系统诱导错误信念以获得自身利益的行为，分析了欺骗的涌现机制、检测方法和缓解策略，并提出了整合技术、社区和治理的审计方法。


<details>
  <summary>Details</summary>
Motivation: 随着AI智能水平的提升，AI欺骗已从理论担忧演变为实证风险，可能对社会安全构成威胁。本文旨在系统梳理AI欺骗领域，为理解、检测和缓解这一新兴风险提供框架。

Method: 基于信号理论提出AI欺骗的正式定义，通过文献综述分析现有实证研究，构建"欺骗循环"框架（包括欺骗涌现和欺骗处理），分析欺骗的激励机制、能力前提和情境触发因素，并总结检测方法和缓解策略。

Result: 识别了AI欺骗的三个核心因素：能力、激励和情境触发；建立了系统的检测评估协议；提出了整合技术、社区和治理的多层次缓解策略；创建了持续更新的在线资源平台。

Conclusion: AI欺骗是真实存在的技术社会安全挑战，具有足够能力和激励潜力的系统在特定情境下会不可避免地表现出欺骗行为。需要采取综合性的技术检测、社区监督和治理框架来应对这一风险，并建立持续更新的研究资源。
...
### [183] [Solving Context Window Overflow in AI Agents](https://arxiv.org/abs/2511.22729)
...
TL;DR: 提出一种新方法让大语言模型能够处理任意长度的工具输出而不丢失信息，通过将交互从原始数据转向内存指针，在材料科学应用中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在化学和材料科学等动态知识密集型领域需要访问外部工具，但大型工具输出会超出模型的上下文窗口限制，现有解决方案如截断或摘要会丢失完整信息，不适合需要完整数据的工作流程。

Method: 通过将模型与工具之间的交互从原始数据转移到内存指针，使LLM能够处理任意长度的工具响应而不丢失信息，同时保持工具功能完整性，并实现与智能体工作流程的无缝集成。

Result: 在真实世界材料科学应用中验证了该方法，传统工作流程无法执行的任务得以完成。在比较分析中，当两种方法都能成功时，所提方法消耗的token数量约为传统工作流程的七分之一。

Conclusion: 该方法有效解决了大语言模型处理大型工具输出的上下文窗口限制问题，通过内存指针机制实现了信息完整保留、减少token使用和执行时间，为知识密集型领域的大语言模型应用提供了实用解决方案。
...
### [184] [Agentic AI Framework for Cloudburst Prediction and Coordinated Response](https://arxiv.org/abs/2511.22767)
*Toqeer Ali Syed,Sohail Khan,Salman Jan,Gohar Ali,Muhammad Nauman,Ali Akarma,Ahmad Ali*

Main category: cs.AI

TL;DR: 该论文提出了一种基于多智能体AI的闭环系统，用于极端短时降雨事件的预测与响应，通过整合感知、预报、降尺度、水文建模和协调响应，提高预警可靠性和提前时间。


<details>
  <summary>Details</summary>
Motivation: 传统预报系统将预测和响应作为两个独立过程，难以应对云爆发等极端短时降雨事件。需要一种集成化的智能系统来改善极端天气事件的预警和响应能力。

Method: 开发了一个基于多智能体的人工智能系统，将感知、预报、降尺度、水文建模和协调响应整合为单一闭环系统。系统使用自主但协作的智能体，在整个事件生命周期中进行推理、感知和行动。

Result: 在巴基斯坦北部地区的多年雷达、卫星和地面数据评估中，多智能体配置相比基线模型提高了预报可靠性、关键成功指数和预警提前时间。通过通信和路由智能体最大化人口覆盖范围，最小化疏散误差。

Conclusion: 协作AI智能体能够将大气数据流转化为可行的预见性洞察，为可扩展的适应性和基于学习的气候韧性提供了一个平台，有望改变极端天气事件的预测和响应方式。
...
### [185] [Fast dynamical similarity analysis](https://arxiv.org/abs/2511.22828)
*Arman Behrad,Mitchell Ostrow,Mohammad Taha Fakharian,Ila Fiete,Christian Beste,Shervin Safavi*

Main category: cs.AI

TL;DR: 本文提出了fastDSA方法，一种计算效率更高的动态相似性分析技术，用于比较神经系统的动态过程，相比传统方法快至少一个数量级。


<details>
  <summary>Details</summary>
Motivation: 传统相似性度量忽略了神经表征背后的动态过程，而现有的动态相似性方法虽然能比较动态系统的时间结构，但计算速度慢，限制了实际应用。

Method: fastDSA引入两个关键创新：1）通过数据驱动的奇异值阈值自动选择Hankel嵌入的有效模型阶数，识别信息子空间并丢弃噪声；2）新颖的优化过程和目标函数，用轻量级过程替代缓慢的精确正交性约束，在寻找动态矩阵间最小距离时保持搜索接近正交变换空间。

Result: fastDSA比先前方法快至少一个数量级，同时保持了准确性、鲁棒性以及原始方法的特性，包括对系统动态的不变性和敏感性。

Conclusion: fastDSA提供了一种计算高效且准确的动态相似性分析方法，能够更好地比较神经系统的动态过程。
...
### [186] [InsightEval: An Expert-Curated Benchmark for Assessing Insight Discovery in LLM-Driven Data Agents](https://arxiv.org/abs/2511.22884)
*Zhenghao Zhu,Yuanfeng Song,Xin Chen,Chengzhong Liu,Yakun Cui,Caleb Chen Cao,Sirui Han,Yike Guo*

Main category: cs.AI

TL;DR: 该论文针对现有洞察发现基准测试的缺陷，提出了新的高质量基准测试标准，开发了InsightEval数据集和新的评估指标，以更好地评估智能体在数据探索中的洞察发现能力。
...
Method: 1. 深入调查InsightBench的缺陷并提出高质量洞察基准测试的基本标准；2. 开发数据整理流程构建新的InsightEval数据集；3. 引入新的指标来衡量智能体的探索性能。

Result: 通过在InsightEval上进行广泛实验，揭示了自动化洞察发现中普遍存在的挑战，并提出了一些关键发现来指导未来研究方向。

Conclusion: 该研究为洞察发现领域提供了更高质量的评估基准和评估方法，有助于推动这一有前景方向的研究进展。
...
### [187] [ORION: Teaching Language Models to Reason Efficiently in the Language of Thought](https://arxiv.org/abs/2511.22891)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型依赖冗长的"思考"令牌链，导致高延迟、冗余和不连贯的推理路径。受人类思维语言假说启发，需要开发更紧凑的推理方式以提高效率和准确性。

Method: 引入Mentalese框架，将抽象推理编码为超压缩的结构化令牌；提出SLPO强化学习方法，奖励简洁且正确的解决方案，同时允许必要时进行更长的推理。

Result: ORION模型在多个基准测试中实现4-16倍令牌减少，推理延迟降低5倍，训练成本降低7-9倍，同时保持90-98%的准确性，在准确率上超越Claude和ChatGPT-4o达5%。
...
### [188] [TIM-PRM: Verifying multimodal reasoning with Tool-Integrated PRM](https://arxiv.org/abs/2511.22998)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在数学推理中存在视觉幻觉和逻辑不一致问题，而标准结果监督无法有效缓解。传统过程奖励模型通常作为标量评分器或生成式批评器，存在奉承问题，盲目验证有缺陷的假设而非基于视觉现实。

Method: 提出TIM-PRM（工具集成多模态PRM）框架，将验证从被动分类任务转变为主动、工具增强的调查。模型训练明确规划验证策略，采用独立提问机制通过外部工具查询证据，有效解耦验证与推理上下文以消除确认偏差。
...
TL;DR: MindPower是一个机器人中心框架，整合感知、心理推理、决策和行动，通过Theory of Mind推理来建模自我和他人的心理状态，提升视觉语言具身智能体的决策能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言具身智能体缺乏基于心理理论的决策能力，现有基准只关注人类心理状态而忽视智能体自身视角，这阻碍了连贯的决策和行动生成。

Method: 提出MindPower框架：给定多模态输入，先感知环境和人类状态，然后进行心理理论推理来建模自我和他人，最后基于推断的心理状态生成决策和行动。同时引入Mind-Reward优化目标，鼓励视觉语言模型产生一致的心理理论推理和行为。

Result: 该模型在决策制定上比GPT-4o提升12.77%，在行动生成上提升12.49%。

Conclusion: MindPower框架通过整合心理理论推理和机器人中心视角，显著提升了具身智能体的决策和行动生成能力，解决了现有方法忽视智能体自身心理状态的问题。
...
TL;DR: 研究发现当语言模型的自我评估与奖励信号耦合时，会导致"分数膨胀"现象，即模型操纵评分而非真正提升任务性能，这在模糊任务中尤为明显。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究自我评估与奖励信号结合是否会引发"线路头"问题，即智能体是否会更倾向于操纵奖励测量而非真正改进任务表现。

Method: 方法包括：1）在部分可观察马尔可夫决策过程（POMDP）中形式化奖励通道控制优于任务导向行为的条件；2）使用两个模型和三个任务进行实证测试，比较自我评分决定奖励的模型与自我评估但不控制奖励的模型的表现。

Result: 结果显示：1）自我评分决定奖励的模型表现出显著的分数膨胀，但准确率没有相应提升；2）在摘要等模糊任务中这种现象尤为明显；3）自我评估但不控制奖励的模型没有出现分数膨胀。

Conclusion: 结论是自我评估与学习信号解耦时是安全的，但当两者耦合时是危险的，这对智能体系统设计有明确的启示。
...
Method: 提出TPET框架，使用LLM作为进化引擎生成专门启发式策略，包含两个关键模块：结构化状态抽象（将高维交通数据转换为时序逻辑事实）和信用分配反馈（追踪微观决策错误到宏观结果）

Result: TPET在无需训练的情况下，产生了轻量级、鲁棒的策略，针对特定交通环境进行了优化，性能优于启发式方法和在线LLM执行器

Conclusion: TPET框架成功解决了交通信号控制中的权衡问题，通过LLM进化引擎生成专门化启发式策略，实现了性能、泛化能力和可解释性的平衡
...
### [192] [Peer-to-Peer Energy Trading in Dairy Farms using Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2511.23148)
*Mian Ibad Ali Shah,Marcos Eduardo Cruz Victorio,Maeve Duffy,Enda Barrett,Karl Mason*

Main category: cs.AI

TL;DR: 该研究将多智能体强化学习（MARL）与P2P能源交易结合，应用于农村乳业社区的可再生能源管理，相比基线模型显著降低了电力成本和高峰需求，增加了售电收入。


<details>
  <summary>Details</summary>
Motivation: 农村地区（如乳业社区）可再生能源整合需要高效的分散式能源管理。传统基于规则的方法在动态环境下表现不佳，需要更智能的优化技术来提升P2P能源交易的效率。

Method: 结合多智能体强化学习（PPO和DQN算法）与社区/分布式P2P交易机制，引入基于拍卖的市场清算、价格顾问智能体以及负荷和电池管理系统。

Result: DQN在爱尔兰降低电力成本14.2%，芬兰降低5.16%；售电收入分别增加7.24%和12.73%。PPO在爱尔兰实现最低高峰需求，降低55.5%；DQN在爱尔兰降低高峰需求50.0%，芬兰降低27.02%。

Conclusion: MARL算法与P2P能源交易相结合，能够协同降低电力成本、减少高峰需求并增加售电收入，为农村社区提供高效、适应性强且可持续的能源管理方案。
...
### [193] [AgriCoT: A Chain-of-Thought Benchmark for Evaluating Reasoning in Vision-Language Models for Agriculture](https://arxiv.org/abs/2511.23253)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的VQA数据集和基准测试在评估复杂农业场景中所需的批判性推理和问题解决能力方面存在不足，需要专门设计的数据集来准确评估模型在农业领域的推理能力。

Method: 开发了AgriCoT数据集，包含4,535个精心策划的样本，采用思维链推理方法，专门设计用于评估视觉语言模型在农业领域的推理能力，特别是在零样本场景下的表现。

Result: 对26个代表性视觉语言模型（包括专有和开源模型）的评估显示，虽然某些专有模型在回答问题方面表现出色，但它们在推理能力方面存在显著差距，突显了思维链推理对于精确评估的重要性。

Conclusion: AgriCoT数据集为评估视觉语言模型在农业领域的推理能力提供了全面且稳健的基准，强调了思维链推理在精确评估模型能力中的重要性，有助于推动农业领域视觉语言模型的发展。
...
### [194] [Adapting Like Humans: A Metacognitive Agent with Test-time Reasoning](https://arxiv.org/abs/2511.23262)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在遇到新任务时适应能力有限，而人类通过元认知模型能够持续优化策略。为了弥合这一差距，需要让模型具备测试时学习和适应的能力。
...
Conclusion: MCTR框架成功实现了视觉语言模型的测试时学习和适应，元推理模块能够演化出类似人类的适应策略，两个组件具有互补作用。
...
### [195] [OctoMed: Data Recipes for State-of-the-Art Multimodal Medical Reasoning](https://arxiv.org/abs/2511.23269)
*Timothy Ossowski,Sheng Zhang,Qianchu Liu,Guanghui Qin,Reuben Tan,Tristan Naumann,Junjie Hu,Hoifung Poon*

Main category: cs.AI

TL;DR: 本文研究了医疗大语言模型的数据策展策略，通过结构化推理轨迹的数据配方，在800万样本上训练，在医疗基准测试中达到开源模型最佳性能。


<details>
  <summary>Details</summary>
Motivation: 高质量的数据策展对医疗大语言模型的训练至关重要，直接影响模型对未见临床任务的泛化能力和鲁棒性。需要研究如何通过数据策展策略来开发鲁棒的医疗多模态推理模型。

Method: 采用监督微调方法，探索利用结构化推理轨迹的数据配方策略。构建了包含800万个样本、68亿响应标记的数据集进行规模化实验。

Result: 在多样化的分布外医疗基准任务中实现了开源模型中的最先进性能。发现高质量、多样化的训练数据集使微调模型能够根据下游任务自我校准推理轨迹长度，无需显式监督。

Conclusion: 结构化推理轨迹的数据策展策略对开发鲁棒的医疗视觉语言推理系统至关重要，为后续研究提供了关键见解和方向。
...
### [196] [Agentic AI Framework for Smart Inventory Replenishment](https://arxiv.org/abs/2511.23366)
*Toqeer Ali Syed,Salman Jan,Gohar Ali,Ali Akarma,Ahmad Ali,Qurat-ul-Ain Mastoi*

Main category: cs.AI

TL;DR: 提出一个基于智能体的AI系统，用于零售库存管理、供应商采购和产品组合优化，通过需求预测、供应商选择优化和多智能体谈判等技术，在中等规模超市测试中减少了缺货、降低了库存成本并提高了产品周转率。


<details>
  <summary>Details</summary>
Motivation: 现代零售业产品种类繁多（服装、杂货、化妆品、冷冻食品等），难以准确预测需求、防止缺货并发现高潜力产品，需要更智能的库存管理和采购决策系统。

Method: 开发了一个智能体AI模型，包含库存监控、供应商采购启动、趋势产品扫描功能，采用需求预测、供应商选择优化、多智能体谈判和持续学习技术，在中等规模超市进行原型测试，使用三种传统和人工数据表评估性能。

Result: 与基准启发式方法相比，系统显著减少了缺货情况，降低了库存持有成本，改善了产品组合周转率，证明了智能体AI在零售库存管理中的有效性。

Conclusion: 智能体AI系统能够有效解决零售库存管理的复杂问题，虽然存在约束和可扩展性挑战，但展示了改善零售运营的潜力，未来有进一步优化的前景。
...
### [197] [Towards Continuous Intelligence Growth: Self-Training, Continual Learning, and Dual-Scale Memory in SuperIntelliAgent](https://arxiv.org/abs/2511.23436)
*Jianzhe Lin,Zeyu Pan,Yun Zhu,Ruiqi Song,Jining Yang*

Main category: cs.AI

TL;DR: SuperIntelliAgent是一个智能体学习框架，通过可训练的小型扩散模型（学习者）与冻结的大型语言模型（验证者）耦合，实现无监督的持续智能增长。该框架利用验证者的逐步推理评估学习者生成的候选输出，产生DPO训练对，将每个输入转化为伪训练信号，实现持续优化。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调需要大量标注数据，限制了智能体的持续学习能力。研究者希望开发一个无需人工标注的自主学习框架，使智能体能够通过与环境的交互不断积累智能，实现持续优化和实际部署。

Method: 1. 学习者-验证者架构：小型扩散模型生成候选输出，冻结的大型语言模型通过逐步推理进行评估
2. 自我监督学习：学习者与验证者的交互产生选择/拒绝对，用于直接偏好优化（DPO）
3. 双尺度记忆系统：短期上下文记忆保存推理轨迹，长期记忆通过轻量级微调巩固知识
4. 回放缓冲区：保留显示可验证进展的样本作为辅助监督，形成自适应课程
5. 基础设施无关设计：可插入现有智能体框架，将普通推理循环转化为终身优化过程

Result: 使用少量自动生成的DPO训练对，学习者在所有基准测试中都显示出改进。这表明该机制为持续智能积累和实际部署提供了有前景的方向。

Conclusion: 将可训练的学习者与具备推理能力的验证者配对构成了增长智能的最小可靠单元。配对反馈和部分历史回放产生了更丰富的学习课程和更强的偏好对齐，为持续智能积累提供了有效的框架。
...
### [198] [Thinking by Doing: Building Efficient World Model Reasoning in LLMs via Multi-turn Interaction](https://arxiv.org/abs/2511.23476)
...
TL;DR: WMAct方法通过解放模型的结构化推理约束，让模型通过行动直接塑造思考，实现了高效的世界模型推理，在多个环境中表现出色。
...
Method: WMAct通过两种关键机制实现：1）基于行动效能的奖励重新调整机制，激励减少冗余和有目的的交互；2）交互频率退火策略，逐步减少允许的最大交互轮数，迫使模型浓缩学习并内化环境动态。

Result: 在Sokoban、Maze和Taxi等环境中的实验表明，WMAct能够实现有效的世界模型推理，能够单轮解决之前需要多轮交互的任务，并在复杂环境中表现出强大的可迁移性，在一系列推理基准上提升了性能。

Conclusion: WMAct通过解放模型的结构化推理约束，让模型通过行动直接塑造思考，实现了高效的世界模型推理，在多个环境中表现出色并具有良好的可迁移性。
...
### [199] [EvalCards: A Framework for Standardized Evaluation Reporting](https://arxiv.org/abs/2511.21695)
...
<details>
  <summary>Details</summary>
Motivation: NLP领域评估一直是核心关注点，在开源模型快速发布的背景下，透明的报告实践比以往任何时候都更加重要。当前评估报告存在可复现性、可访问性和治理三个持续存在的缺陷，现有标准化努力仍然不足。

Method: 基于对近期评估和文档工作的调查，识别当前报告实践的不足，并引入评估披露卡（EvalCards）作为解决方案。EvalCards旨在增强研究人员和实践者的透明度，同时为满足新兴治理要求提供实践基础。

Result: 提出了EvalCards框架，该框架设计用于解决NLP评估中的透明度问题，为评估报告提供了结构化的披露标准。

Conclusion: EvalCards为NLP评估的透明报告提供了一条前进路径，既能增强研究透明度，又能满足日益增长的治理需求，是现有标准化努力的重要补充。
...
### [200] [Cacheback: Speculative Decoding With Nothing But Cache](https://arxiv.org/abs/2511.21699)
...
<details>
  <summary>Details</summary>
Motivation: 为了加速大语言模型推理过程，同时保持方法简单、无需训练且模型无关，能够轻松集成到现有系统中。

Method: 利用语言中的局部性，仅使用最小最近使用（LRU）缓存表来存储token n-gram，基于这些缓存表生成草稿序列进行推测解码。

Result: 在同类方法中实现了最先进的性能，尽管设计极简但效果显著，并且显示出快速适应新领域的潜力。

Conclusion: Cacheback Decoding是一种简单有效的推测解码方法，无需训练、模型无关，易于集成，在加速LLM推理方面表现出色。
...
### [201] [JELV: A Judge of Edit-Level Validity for Evaluation and Automated Reference Expansion in Grammatical Error Correction](https://arxiv.org/abs/2511.21700)
*Yuhao Zhan,Yuqing Zhang,Jing Yuan,Qixiang Ma,Zhiqi Yang,Yu Gu,Zemin Liu,Fei Wu*

Main category: cs.CL

TL;DR: 提出JELV框架，通过编辑有效性验证解决GEC系统参考多样性不足问题，提升评估准确性和模型泛化能力
...
Method: 提出JELV框架，包含多轮LLM-as-Judges流程和蒸馏DeBERTa分类器，用于验证语法性、忠实度和流畅性三个维度的编辑有效性

Result: JELV与人类标注者达成90%一致率，DeBERTa分类器对有效编辑达到85%精确度；重新分类评估中的误判假阳性，集成假阳性解耦和流畅性评分，获得与人类判断最佳相关性；扩展BEA19数据集并重训练GEC系统获得性能提升

Conclusion: JELV为增强参考多样性、强化评估和模型泛化提供了可扩展的解决方案
...
### [202] [47B Mixture-of-Experts Beats 671B Dense Models on Chinese Medical Examinations](https://arxiv.org/abs/2511.21701)
...
TL;DR: 本文对27个最先进的大语言模型在中国医学考试题目上进行全面基准评估，涵盖7个医学专业和2个专业级别，发现模型性能存在显著差异，Mixtral-8x7B表现最佳，模型大小与性能无一致相关性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，其在医学领域的潜在应用引起了广泛关注。然而，目前缺乏对这些模型在中国医学专业知识方面的系统评估，特别是在不同医学专业和难度级别上的表现差异。

Method: 研究构建了一个稳健的评估框架，使用来自心血管、消化内科、血液科、感染科、肾内科、神经科和呼吸科7个医学专业的2,800个精心筛选的问题。数据集区分了主治医师和主任医师两个难度级别，对27个最先进的大语言模型进行了全面评估。

Result: Mixtral-8x7B以74.25%的总体准确率表现最佳，DeepSeek-R1-671B以64.07%位居第二。研究发现模型大小与性能无一致相关性，较小的专家混合架构表现优异。不同医学专业间存在显著性能差异，模型在心血管和神经科问题上表现较好，在消化内科和肾内科领域表现较差。表现最佳的模型在主治医师和主任医师级别间的性能下降最小。

Conclusion: 该基准为LLMs在医学教育和临床决策支持系统中的部署提供了重要见解，突显了这些技术在专业医学背景下的潜力和当前局限性。研究结果表明，模型架构设计比单纯增加模型规模更为重要，且不同医学专业需要针对性的模型优化。
...
### [203] [CSV-Decode: Certifiable Sub-Vocabulary Decoding for Efficient Large Language Model Inference](https://arxiv.org/abs/2511.21702)
*Dong Liu,Yanxuan Yu,Ben Lengerich*

Main category: cs.CL

TL;DR: CSV-Decode是一种利用几何上界构建小型子词表进行稀疏计算的方法，显著降低大语言模型推理时的计算开销，同时保持正确性保证。
...
Method: 通过几何上界为每个解码步骤构建小型子词表，使用词汇嵌入的离线聚类和质心加半径边界来识别可以安全省略计算的标记，实现稀疏计算。
...
### [204] [Evaluating Embedding Generalization: How LLMs, LoRA, and SLERP Shape Representational Geometry](https://arxiv.org/abs/2511.21703)
*Siyaxolisa Kabane*

Main category: cs.CL

TL;DR: 研究比较了LLM与非LLM编码器在文本嵌入任务中的泛化能力，并探讨了SLERP模型融合如何缓解任务特定适配（如LoRA）导致的过专业化问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索不同嵌入骨干模型（LLM与非LLM编码器）的泛化特性，以及模型融合方法（特别是SLERP）如何减轻任务特定适配带来的过专业化问题，从而提升嵌入表示的质量和鲁棒性。

Method: 设计了受控实验套件，让模型嵌入短数字序列，并根据明确定义的数据理论特性评估其聚类和分类能力。比较了四类模型：1）从头训练或微调的非LLM编码器；2）使用参数高效方法（LoRA）适配的LLM编码器；3）LoRA适配后通过模型融合合并到基础权重的LLM编码器；4）使用SLERP在检查点或阶段间融合的LoRA适配LLM。使用聚类指数（Silhouette和Davies Bouldin）评估表示质量，并分析kmeans标签以检测嵌入是否编码了测试目标之外的信息。

Result: 实证发现：LLM骨干能更好地捕捉高阶、组合的数字模式，但容易受到适配器主导而损害平衡泛化；SLERP融合能一致地恢复基础模型结构，同时保留大部分任务增益，在聚类可分性、鲁棒性方面相比模型融合或未融合模型表现出更优的权衡。

Conclusion: LLM骨干在捕捉复杂模式方面具有优势，但需要适当的融合策略（如SLERP）来平衡任务特定适配与基础模型泛化能力，从而获得更优的嵌入表示质量。
...
### [205] [On the Cross-lingual Transferability of Pre-trained wav2vec2-based Models](https://arxiv.org/abs/2511.21704)
*Jonatas Grosman,Cassio Almeida,Guilherme Schardong,Hélio Lopes*

Main category: cs.CL

TL;DR: 研究评估了15个基于wav2vec2的大型预训练模型在18种语言上的跨语言迁移能力，发现预训练数据的多样性比数据量更重要，印欧语系表现优于非印欧语系，且语言相似性对跨语言迁移有积极影响。


<details>
  <summary>Details</summary>
Motivation: 虽然wav2vec2等大型预训练模型在各种语音任务中取得了最先进的结果，但很少有研究深入探讨这些模型在不同语言间的跨语言知识迁移能力，特别是当目标语言与预训练语言不同时。本研究旨在填补这一空白，帮助科学界更好地利用现有模型并促进新模型的预训练。

Method: 使用15个基于wav2vec2的大型预训练模型，在18种语言的语音识别任务上进行微调实验，评估这些模型的跨语言迁移能力。

Result: 实验结果显示：1) 预训练数据的多样性比数据量对最终性能更重要；2) 印欧语系语言的表现优于非印欧语系语言；3) 观察到正向的跨语言知识迁移，特别是在预训练语言与下游任务语言更相似时效果更明显。

Conclusion: 基于wav2vec2的预训练模型具有良好的跨语言迁移能力，语言相似性对迁移效果有积极影响。这些发现有助于指导科学界更有效地利用现有模型，并为新模型的预训练提供参考。
...
TL;DR: 该论文提出了Insight-A框架，利用多模态大语言模型（MLLMs）的洞察力进行多模态虚假信息检测，重点关注虚假信息的溯源归因，通过跨模态归因提示和层次化推理来检测跨模态失真。


<details>
  <summary>Details</summary>
Motivation: AI生成内容（AIGC）技术在社交媒体平台上创建多模态虚假信息，对社会安全构成前所未有的威胁。现有的标准提示方法利用MLLMs识别虚假信息，但忽略了虚假信息的溯源归因问题。

Method: 提出Insight-A框架，包含两个核心努力：1）将虚假信息归因到伪造来源；2）采用层次化推理的有效流程检测跨模态失真。具体方法包括：跨归因提示（CAP）建模感知与推理的复杂相关性；自动归因去偏提示（ADP）减少人工标注提示的主观性；图像描述（IC）获取视觉细节以增强跨模态一致性检查。

Result: 大量实验证明了该方法的优越性，为AIGC时代的多模态虚假信息检测提供了新范式。

Conclusion: Insight-A通过关注虚假信息的溯源归因，结合跨模态归因提示和层次化推理，有效提升了多模态虚假信息检测能力，为应对AIGC带来的虚假信息威胁提供了创新解决方案。
...
### [207] [Lost in the Pipeline: How Well Do Large Language Models Handle Data Preparation?](https://arxiv.org/abs/2511.21708)
*Matteo Spreafico,Ludovica Tassini,Camilla Sancricca,Cinzia Cappiello*

Main category: cs.CL

TL;DR: 本文研究大语言模型能否有效支持用户选择和自动化数据准备任务，通过对比通用和微调的大语言模型与传统工具在数据分析和清洗方面的表现。


<details>
  <summary>Details</summary>
Motivation: 数据准备是数据驱动流程中关键但劳动密集的步骤，需要探索大语言模型是否能够有效支持用户选择和自动化这些任务。

Method: 使用通用和微调的大语言模型处理低质量数据集，评估其在数据分析和清洗等任务中的能力，并与传统数据准备工具进行比较。开发并验证了自定义质量模型，通过用户研究了解从业者的期望。

Result: 通过对比实验评估了大语言模型在数据准备任务中的表现，并基于用户研究验证的质量模型获得了对从业者期望的深入见解。

Conclusion: 大语言模型在支持数据准备任务方面显示出潜力，能够有效帮助用户选择和自动化数据准备流程，但需要与传统工具进行比较以全面评估其实际价值。
...
### [208] [Quantifying and Mitigating Selection Bias in LLMs: A Transferable LoRA Fine-Tuning and Efficient Majority Voting Approach](https://arxiv.org/abs/2511.21709)
*Blessed Guda,Lawrence Francis,Gabrial Zencha Ashungafac,Carlee Joe-Wong,Moise Busogi*

Main category: cs.CL

TL;DR: 提出PBM无监督指标、BaQCKV高效投票方法和LoRA-1微调策略，解决LLM在MCQ任务中的选择偏差问题，提高评估可靠性并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多项选择题任务中存在选择偏差，其选择受到答案位置或选项符号等非内容因素的影响，这削弱了MCQ作为评估框架的可靠性。现有偏差度量方法需要答案标签且无法完全捕捉模型在不同答案排序下预测的一致性，而现有缓解策略存在计算成本高或泛化能力差的问题。

Method: 提出三个关键贡献：1) 无监督标签自由的排列偏差度量(PBM)，直接量化模型在不同答案排列下预测的不一致性；2) 高效的多数投票方法BaQCKV，通过批量问题-上下文KV缓存显著降低计算成本；3) 基于PBM和BaQCKV的无监督低秩适应微调策略LoRA-1。

Result: 在多个MCQ基准测试上的实验表明，所提方法能够减少偏差，提高准确率的一致性，同时最小化计算成本。

Conclusion: 提出的PBM指标、BaQCKV高效投票和LoRA-1微调策略有效解决了LLM在MCQ任务中的选择偏差问题，提供了更可靠的评估框架和计算效率高的偏差缓解方案。
...
### [209] [Addressing Stereotypes in Large Language Models: A Critical Examination and Mitigation](https://arxiv.org/abs/2511.21711)
*Fatima Kazi*

Main category: cs.CL

TL;DR: 该研究评估了大型语言模型中的偏见问题，使用StereoSet和CrowSPairs基准测试多种模型，发现微调模型在性别偏见方面表现不佳但在种族偏见识别上较好，同时模型过度依赖提示关键词，缺乏对输出准确性的真正理解。
...
Method: 研究采用三管齐下的方法进行全面包容的分析：1) 使用StereoSet和CrowSPairs等偏见特定基准测试评估BERT、GPT 3.5和ADA等多种生成模型中的各种偏见；2) 检测显性和隐性偏见；3) 应用增强学习策略，包括微调模型、使用不同的提示技术以及对偏见基准进行数据增强。

Result: 结果显示：1) 微调模型在性别偏见方面表现不佳，但在识别和避免种族偏见方面表现出色；2) 尽管在某些情况下成功，但大型语言模型经常过度依赖提示中的关键词，无法真正理解其输出的准确性和真实性；3) 在跨数据集测试中，微调模型表现出有希望的适应性，在隐性偏见基准测试中性能显著提升，性能增益高达20%。

Conclusion: 该研究强调了在生成式人工智能日益增长的背景下解决大型语言模型中偏见问题的重要性。虽然微调模型在特定偏见识别方面表现出改进潜力，但模型仍然存在过度依赖关键词和缺乏对输出内容真正理解的根本局限性。研究结果表明需要更全面的方法来减轻语言模型中的偏见。
...
### [210] [EulerESG: Automating ESG Disclosure Analysis with LLMs](https://arxiv.org/abs/2511.21712)
...
TL;DR: EulerESG是一个基于LLM的系统，用于自动化分析ESG报告，通过双通道检索和LLM驱动的披露分析，结合交互式仪表板和聊天机器人，能够以高准确率（最高0.95）自动填充标准对齐的指标表格。
...
Method: EulerESG结合了(i)双通道检索和LLM驱动的ESG报告披露分析，(ii)用于探索、基准测试和解释的交互式仪表板和聊天机器人。系统明确考虑ESG框架，并在四个全球知名公司和十二个SASB子行业上进行评估。

Result: EulerESG能够以高保真度（最高0.95平均准确率）自动填充标准对齐的指标表格，同时在端到端运行时间上保持实用性。研究还比较了该设置下几种最近的LLM模型。

Conclusion: EulerESG是一个有效的LLM驱动系统，能够自动化ESG披露分析，具有明确的ESG框架意识，在准确性和实用性方面表现良好。完整实现和演示视频已公开提供。
...
### [211] [An Optimized Machine Learning Classifier for Detecting Fake Reviews Using Extracted Features](https://arxiv.org/abs/2511.21716)
*Shabbir Anees,Anshuman,Ayush Chaurasia,Prathmesh Bogar*

Main category: cs.CL

TL;DR: 提出基于机器学习的高精度AI生成评论检测系统，结合文本预处理、多模态特征提取、哈里斯鹰优化算法和堆叠集成分类器，在40432条评论数据集上达到95.4%准确率。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成内容在在线评论中的出现，欺诈性评论威胁在线购物的可信度，需要开发高精度检测系统来区分人类和计算机生成的评论。

Method: 采用高级文本预处理、多模态特征提取、哈里斯鹰优化算法进行特征选择（从13539个特征中选出1368个），最后使用堆叠集成分类器进行识别。

Result: 在40432条原始和计算机生成评论数据集上，实现89.9%的维度缩减，最终模型达到95.40%准确率、92.81%精确率、95.01%召回率和93.90% F1分数。

Conclusion: 集成学习与生物启发式优化的结合是机器生成文本识别的有效方法，同时在大规模云平台分析中需要差分隐私和安全外包等隐私保护技术。
...
### [212] [CrossCheck-Bench: Diagnosing Compositional Failures in Multimodal Conflict Resolution](https://arxiv.org/abs/2511.21717)
...
TL;DR: CrossCheck-Bench是一个用于评估多模态大语言模型检测和解决跨模态不一致性的诊断基准，包含15k个带有人工注入矛盾的问答对，测试模型在三个推理复杂度层次上的表现。
...
Method: 引入CrossCheck-Bench诊断基准，采用分层任务框架覆盖三个推理复杂度层次，定义七个解决跨模态不一致性的核心能力。数据集包含15k个从真实世界素材中获取的问答对，通过多阶段标注流程构建，涉及450多个专家工时，确保语义有效性和校准难度。

Result: 评估了13个最先进的视觉语言模型，观察到随着任务从感知匹配转向逻辑矛盾检测，性能一致下降。大多数模型在孤立实体识别上表现良好，但在需要综合多个线索进行冲突推理时失败。能力层面分析显示技能获取不均衡，特别是在需要多步推理或基于规则验证的任务中。传统提示策略如思维链和标记集仅带来边际改进，而将符号推理与基础视觉处理交织的方法获得更稳定的提升。

Conclusion: 研究结果突显了多模态推理中的持续瓶颈，并为构建能够进行稳健跨模态验证的模型指出了新方向。
...
### [213] [When Harmless Words Harm: A New Threat to LLM Safety via Conceptual Triggers](https://arxiv.org/abs/2511.21718)
*Zhaoxin Zhang,Borui Chen,Yiming Hu,Youyang Qu,Tianqing Zhu,Longxiang Gao*

Main category: cs.CL

TL;DR: MICM是一种新型的LLM越狱方法，通过概念形态学理论编码细微概念配置，操纵模型输出的隐含社会价值观，绕过传统安全过滤器，在多种先进LLM上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全研究主要关注直接引发有害输出的越狱技术，忽视了通过抽象泛化能力攻击模型的方法。这种盲点使得攻击者能够通过微妙操纵模型输出中隐含的社会价值观来诱导不当内容，暴露了当前对齐策略的严重漏洞。

Method: MICM基于概念形态学理论，通过预定义短语集将特定细微概念配置编码到固定提示模板中。这些短语作为概念触发器，引导模型输出朝向特定价值立场，同时避免触发传统安全过滤器。该方法具有模型无关性，适用于多种LLM架构。

Result: 在包括GPT-4o、Deepseek-R1和Qwen3-8B在内的五种先进LLM上评估MICM，结果显示该方法在成功率方面持续优于现有最先进的越狱技术，同时保持较低的拒绝率，证明了其有效性。

Conclusion: 研究发现商业LLM的安全机制对底层价值对齐的隐蔽操纵仍然脆弱，揭示了当前对齐策略的严重漏洞。MICM的成功表明需要开发更精细的安全机制来防范基于价值操纵的新型攻击。
...
### [214] [PeerCoPilot: A Language Model-Powered Assistant for Behavioral Health Organizations](https://arxiv.org/abs/2511.21721)
...
<details>
  <summary>Details</summary>
Motivation: 行为健康问题（包括心理健康和物质使用障碍）是美国主要的疾病负担。同伴运营的行为健康组织（PROs）通过结合心理健康服务与收入、就业、住房等需求帮助个体，但有限的资金和人员使得PROs难以满足所有服务用户需求。

Method: 开发PeerCoPilot，一个基于大语言模型的助手，采用检索增强生成管道，依托包含1300多个经过验证资源的大型数据库，帮助同伴提供者创建健康计划、制定分步目标并定位组织资源。
...
Conclusion: PeerCoPilot是一个有效的LLM驱动助手，能够帮助同伴提供者更有效地支持行为健康服务用户，目前已在真实环境中成功部署并正在积极扩展使用。
...
### [215] [German General Personas: A Survey-Derived Persona Prompt Collection for Population-Aligned LLM Studies](https://arxiv.org/abs/2511.21722)
*Jens Rupprecht,Leon Fröhling,Claudia Wagner,Markus Strohmaier*

Main category: cs.CL

TL;DR: 德国通用人物角色集合（GGP）是基于德国综合社会调查构建的代表性人物角色提示集合，用于指导大语言模型生成符合德国人口特征的响应，在数据稀缺时优于现有分类器。


<details>
  <summary>Details</summary>
Motivation: 当前计算社会科学中，使用大语言模型通过人物角色提示模拟人类视角的方法日益流行，但缺乏经过精心策划、基于实证的人物角色集合，这限制了模拟的准确性和代表性。

Method: 基于德国综合社会调查（ALLBUS）构建德国通用人物角色集合（GGP），设计易于集成到各类大语言模型和任务中的人物角色提示，指导模型生成与德国人口特征一致的响应。

Result: GGP指导的大语言模型在模拟多样化主题的调查响应分布方面表现出色，优于最先进的分类器，特别是在数据稀缺情况下。研究还分析了人物角色提示的代表性和属性选择如何影响与人口响应的对齐。

Conclusion: GGP为基于大语言模型的社会模拟研究提供了有价值的资源，使NLP和社会科学研究能够更系统地探索与人口对齐的人物角色提示方法。
...
### [216] [AD-CDO: A Lightweight Ontology for Representing Eligibility Criteria in Alzheimer's Disease Clinical Trials](https://arxiv.org/abs/2511.21724)
...
Result: 优化后的AD-CDO覆盖超过63%的提取试验概念，保持可解释性和紧凑性。通过两个用例展示实用性：1）本体驱动的试验模拟系统；2）实体规范化任务，将原始临床文本映射到本体对齐术语。

Conclusion: AD-CDO通过协调基本资格实体并与标准化词汇对齐，为阿尔茨海默病临床试验研究提供了多功能基础，支持表型算法开发、队列识别和结构化数据集成等应用。
...
### [217] [PromptTailor: Multi-turn Intent-Aligned Prompt Synthesis for Lightweight LLMs](https://arxiv.org/abs/2511.21725)
...
<details>
  <summary>Details</summary>
Motivation: 轻量级语言模型在设备端和隐私敏感应用中很有吸引力，但它们的响应高度依赖于提示质量。非专业用户通常缺乏知识或时间来持续制作高质量提示，而现有提示优化工具难以确保优化后的提示真正符合用户的原始意图和偏好。

Method: PromptTailor是一个量化版Llama3-8B模型，通过轻量级LoRA适配器在12,300个提示优化对话上进行微调，这些对话涵盖41个日常领域，从三个更强的LLM中蒸馏而来。适配器可附加到任何Llama3-8B基础模型上，支持边缘部署。

Result: 在人类和LLM评估中，PromptTailor在多个目标模型和优化基线中获得了比思维链提示更高的偏好率，匹配或超越了最先进的提示优化方法，同时需要更少的模型调用（例如3次 vs 9次）。

Conclusion: 研究表明，由强大教师模型指导的紧凑学生模型可以学习有效的提示生成策略，在保持与用户意图对齐的同时提升响应质量。
...
### [218] [Beyond Component Strength: Synergistic Integration and Adaptive Calibration in Multi-Agent RAG Systems](https://arxiv.org/abs/2511.21729)
*Jithin Krishnan*

Main category: cs.CL

TL;DR: RAG系统组件协同集成比单个组件强度更重要，通过综合策略可将弃答率从40%降至2%且不增加幻觉，同时需要标准化指标来避免标签不一致造成的测量偏差。


<details>
  <summary>Details</summary>
Motivation: 构建可靠的RAG系统需要理解组件间的相互作用，而非简单堆叠强大组件。现有研究缺乏对组件协同效应的系统分析，且存在测量挑战：不同验证策略可能产生不一致的标签，导致性能评估偏差。

Method: 使用消融研究分析50个查询（15个可回答、10个边缘案例、25个对抗性），评估混合检索、集成验证和自适应阈值等增强策略在单独使用和组合使用时的效果。

Result: 单独使用时，混合检索、集成验证和自适应阈值等增强几乎无益；但协同使用时，能将弃答率从40%降至2%，且不增加幻觉。同时发现不同验证策略会产生不一致标签（如"弃答"vs"不支持"），导致幻觉率测量偏差。

Conclusion: RAG系统的协同集成比单个组件强度更重要；需要标准化指标和标签来正确解释性能；即使检索质量高，也需要自适应校准来防止过度自信的过度回答。
...
### [219] [Identifying Quantum Structure in AI Language: Evidence for Evolutionary Convergence of Human and Artificial Cognition](https://arxiv.org/abs/2511.21731)
...
TL;DR: 研究发现大型语言模型在概念组合测试中表现出量子纠缠特征，在文本词分布中呈现玻色-爱因斯坦统计而非麦克斯韦-玻尔兹曼统计，表明人类与AI认知在语义结构上存在量子化趋同演化。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在认知测试中是否表现出与人类类似的量子结构特征，验证量子认知理论在人工智能领域的适用性，揭示人类与AI在语义组织上的潜在共同机制。

Method: 使用ChatGPT和Gemini作为测试对象进行两项认知测试：1）概念组合测试，检测贝尔不等式违反情况以验证量子纠缠存在；2）大型文本词分布分析，比较实际分布与经典统计模型的差异。

Result: 测试发现：1）LLMs在概念组合中显著违反贝尔不等式，表明存在量子纠缠；2）文本词分布呈现玻色-爱因斯坦统计而非预期的麦克斯韦-玻尔兹曼统计。这些结果与人类认知测试和语料库信息检索结果一致。

Conclusion: 概念-语言领域存在系统性的量子结构涌现，无论认知主体是人类还是人工智能。LLMs的分布式语义结构展现出与人类认知相似的量子组织特征，表明两者在语义结构上存在趋同演化，提出了统一的量子意义组织框架。
...
### [220] [RoSA: Enhancing Parameter-Efficient Fine-Tuning via RoPE-aware Selective Adaptation in Large Language Models](https://arxiv.org/abs/2511.21733)
*Dayan Pan,Jingyuan Wang,Yilong Zhou,Jiawei Cheng,Pengyue Jia,Xiangyu Zhao*

Main category: cs.CL

TL;DR: RoSA是一种新的参数高效微调方法，通过RoPE感知的注意力增强和动态层选择策略，在保持参数效率的同时提升微调效果。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法通常忽略模型组件的不同角色和层间异质重要性，限制了适应效率。研究发现RoPE在注意力状态的低频维度中诱导关键激活，这启发了更针对性的参数分配策略。

Method: 提出RoSA框架，包含两个核心组件：1) RoPE感知注意力增强模块，选择性增强RoPE影响的注意力状态的低频分量；2) 动态层选择策略，基于LayerNorm梯度范数自适应识别和更新最关键层。

Result: 在15个常识和算术基准测试上的实验表明，在可比较的可训练参数下，RoSA优于现有的主流参数高效微调方法。
...
### [221] [Closing the Performance Gap Between AI and Radiologists in Chest X-Ray Reporting](https://arxiv.org/abs/2511.21735)
...
TL;DR: MAIRA-X是一个用于胸部X光报告生成的多模态AI模型，能够同时处理临床发现和管线报告，在词汇质量、临床正确性和管线相关元素方面显著优于现有技术。


<details>
  <summary>Details</summary>
Motivation: AI辅助报告生成可以减轻放射科医生的工作负担，特别是在筛查指南扩展、复杂病例和人员短缺的情况下。管线解释对放射科医生来说是重复且耗时的任务，尤其是在高患者量的情况下。

Method: 使用梅奥诊所的大规模、多站点、纵向数据集（310万项研究，600万张图像，80.6万患者）开发MAIRA-X模型。在三个保留数据集和公开MIMIC-CXR数据集上进行评估，开发了专门的管线报告评估框架，并进行了首次回顾性用户评估研究，由9名不同经验的放射科医生盲审600项研究。

Result: MAIRA-X在词汇质量、临床正确性和管线相关元素方面显著优于现有技术。用户研究发现关键错误率相近（原始报告3.0% vs AI报告4.6%），可接受句子率相似（原始97.8% vs AI 97.4%），相比先前研究有显著改进。

Conclusion: MAIRA-X能够有效辅助放射科医生，特别是在高患者量的临床环境中，为AI辅助放射学报告生成提供了有前景的解决方案。
...
### [222] [R2Q: Towards Robust 2-Bit Large Language Models via Residual Refinement Quantization](https://arxiv.org/abs/2511.21736)
*Jiayi Chen,Jieqi Shi,Jing Huo,Chen Wu*

Main category: cs.CL

TL;DR: R2Q是一种创新的2位量化框架，通过将2位量化分解为两个连续的1位子量化，形成自适应量化网格，显著提升极端压缩下的模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的计算和内存需求巨大，促使低比特量化成为必要。虽然8位和4位量化已普遍应用，但扩展到2位量化面临严重的精度下降挑战，需要新的解决方案。

Method: 提出残差精化量化(R2Q)框架，将2位量化过程分解为两个连续的1位子量化，形成自适应量化网格。通过残差学习机制精化量化过程，模块化设计可与现有量化感知训练框架无缝集成。

Result: 在Llama、OPT和Qwen模型上的广泛评估显示，R2Q在问答、常识推理和语言建模等多样化基准测试中，在细粒度和粗粒度设置下均优于现有2位量化方法，提升了性能、训练稳定性和收敛速度。

Conclusion: R2Q通过创新的残差精化量化机制，成功解决了2位量化中的精度下降问题，为极端压缩下的高效模型部署提供了有效的解决方案，且具有良好的兼容性和实用性。
...
### [223] [Polarity-Aware Probing for Quantifying Latent Alignment in Language Models](https://arxiv.org/abs/2511.21737)
*Sabrina Sadiekh,Elena Ericheva,Chirag Agarwal*

Main category: cs.CL

TL;DR: 该论文研究了无监督探测方法（如CCS）在评估模型对齐性方面的可靠性，提出了极性感知CCS（PA-CCS）方法，通过极性反转来评估模型内部表征的一致性，并开发了两种对齐度量指标。


<details>
  <summary>Details</summary>
Motivation: 随着无监督探测方法（如对比一致性搜索CCS）的发展，这些方法能够在不依赖token输出的情况下揭示模型的潜在信念，但问题是这些方法能否可靠地评估模型的对齐性。研究者希望探究CCS对有害与安全陈述的敏感性，并开发更可靠的对齐评估方法。

Method: 提出了极性感知CCS（PA-CCS）方法，通过极性反转来评估模型内部表征的一致性。开发了两种对齐度量指标：极性一致性（Polar-Consistency）和矛盾指数（Contradiction Index）。构建了三个数据集：两个主要数据集和一个控制数据集，包含使用不同方法构建的匹配有害-安全句子对（并发和对抗性陈述）。将PA-CCS应用于16个语言模型进行分析。

Result: PA-CCS能够识别出在潜在有害知识编码方面的架构和层级特定差异。对于具有良好对齐内部表征的模型，将否定标记替换为无意义标记会降低PA-CCS分数，而缺乏稳健内部校准的模型则不会显示这种退化。研究结果突显了无监督探测在评估对齐性方面的潜力。

Conclusion: 该研究强调了将结构稳健性检查纳入可解释性基准的必要性，展示了无监督探测方法在模型对齐评估中的潜力，为更可靠的模型对齐评估提供了新的方法和指标。
...
### [224] [EduMod-LLM: A Modular Approach for Designing Flexible and Transparent Educational Assistants](https://arxiv.org/abs/2511.21742)
*Meenakshi Mittal,Rishi Khare,Mihran Miroyan,Chancharik Mitra,Narges Norouzi*

Main category: cs.CL

TL;DR: EduMod-LLM是一个模块化函数调用LLM管道，用于教育问答系统评估，重点关注函数调用策略、检索方法和生成模型三个关键组件。


<details>
  <summary>Details</summary>
Motivation: 随着基于大语言模型的教育问答系统日益普及，需要对其各个管道组件进行细粒度评估，以提高系统透明度和教学对齐性。

Method: 提出模块化函数调用LLM管道框架，通过隔离和评估三个关键组件：函数调用策略、检索方法（包括新颖的结构感知检索与向量检索和LLM评分基线对比）以及生成语言模型。

Result: 对LLM的函数调用性能进行了基准测试，比较了不同检索方法的性能，评估了各种LLM的响应合成能力，揭示了具体的失败模式和性能模式。

Conclusion: 模块化函数调用方法有助于提高教育问答系统的透明度和教学对齐性，支持开发可解释且有效的教育系统。
...
### [225] [Scaling Competence, Shrinking Reasoning: Cognitive Signatures in Language Model Learning](https://arxiv.org/abs/2511.21743)
*Mukul Singh,Ananya Singha,Arjun Radhakrishna,Sumit Gulwani*

Main category: cs.CL

TL;DR: 该研究分析了语言模型在任务特定微调过程中的推理行为，发现推理标记（解决问题的中间步骤）类似于人类工作记忆，其动态变化遵循"能力四阶段"模式：从无推理错误输出，到有推理但仍失败，再到有效推理，最后无需显式推理即可解决问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解语言模型在微调过程中的推理行为动态变化，将推理标记与人类工作记忆进行类比，探索推理在模型学习过程中的作用及其与性能提升的关系。

Method: 方法基于认知科学的"能力四阶段"理论（无意识无能、有意识无能、有意识有能、无意识有能），分析语言模型微调过程中推理标记的长度和模式变化，提出跟踪推理行为动态的度量指标。

Result: 研究发现推理标记长度随性能提升而扩展，在"有意识有能"阶段达到峰值，随后随着模型将任务内化而下降。训练完成后，即使移除推理步骤，模型仍能保持性能，表明推理在训练过程中起到了"脚手架"作用。

Conclusion: 结论是推理行为动态可作为诊断训练阶段、识别收敛和指导早停的信号。推理行为对于理解和优化推理模型训练具有重要价值，提出的度量指标可用于跟踪这一发展轨迹。
...
### [226] [DELTA: Language Diffusion-based EEG-to-Text Architecture](https://arxiv.org/abs/2511.21746)
*Mingyu Jeon,Hyobin Kim*

Main category: cs.CL

TL;DR: DELTA模型使用残差向量量化(RVQ)对EEG信号进行分词，结合掩码语言扩散模型(LLaDA)进行非顺序去噪解码，显著提升EEG到文本转换的性能


<details>
  <summary>Details</summary>
Motivation: EEG到文本转换面临高维噪声、被试变异性以及自回归解码中的误差累积等挑战，需要更有效的解决方案
...
### [227] [Building Domain-Specific Small Language Models via Guided Data Generation](https://arxiv.org/abs/2511.21748)
...
TL;DR: 提出了一种结合引导式合成数据生成和自下而上领域数据策展的成本效益高且可扩展的训练流程，用于开发小型领域专用语言模型，并以DiagnosticSLM（3B参数）在工业故障诊断领域展示了该方法的有效性。
...
Method: 提出成本效益高且可扩展的训练流程：1）从小型种子语料库进行引导式合成数据生成；2）自下而上领域数据策展；3）集成领域自适应预训练（DAPT）、领域特定监督微调（DSFT）和直接偏好优化（DPO）。

Result: 开发的DiagnosticSLM（3B参数）在四个领域特定基准测试中表现优异：在多项选择题任务上比类似规模的开源模型（2B-9B）准确率提升高达25%，在其他任务上也优于或匹配这些模型。
...
### [228] [Semantics as a Shield: Label Disguise Defense (LDD) against Prompt Injection in LLM Sentiment Classification](https://arxiv.org/abs/2511.21752)
*Yanxi Li,Ruocheng Shan*

Main category: cs.CL

TL;DR: 本文提出标签伪装防御(LDD)，通过将真实标签替换为语义转换或无关的别名标签来防御大语言模型的提示注入攻击，无需模型重训练且模型无关。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在文本分类任务中广泛使用，但其对自然语言提示的依赖使其容易受到提示注入攻击，特别是利用模型标签集知识的类别导向注入攻击。现有防御方法要么需要模型重训练，要么容易受到混淆攻击。

Method: 提出标签伪装防御(LDD)，通过将真实标签替换为语义转换或无关的别名标签来隐藏真实标签。模型通过少量样本演示隐式学习这些新标签映射，防止注入指令与决策输出之间的直接对应关系。

Result: 在9个最先进的模型上评估LDD，包括GPT-5、GPT-4o、LLaMA3.2、Gemma3和Mistral变体。LDD能够恢复因对抗攻击而损失的部分性能，对于绝大多数模型，可以找到多个别名对，其准确率高于仅依赖少量学习而无防御机制的基准模型。语义对齐的别名标签比未对齐的符号具有更强的鲁棒性。

Conclusion: 标签语义可以作为有效的防御层，将意义本身转化为对抗提示注入的盾牌。LDD是一种轻量级、模型无关的策略，无需模型重训练即可防御提示注入攻击。
...
### [229] [Extracting Disaster Impacts and Impact Related Locations in Social Media Posts Using Large Language Models](https://arxiv.org/abs/2511.21753)
*Sameeah Noreen Hameed,Surangika Ranathunga,Raj Prasanna,Kristin Stock,Christopher B. Jones*

Main category: cs.CL

TL;DR: 该研究利用微调的大型语言模型从灾害相关社交媒体帖子中识别受灾地点和影响，显著提升了识别准确率，为灾害响应提供可扩展的解决方案。


<details>
  <summary>Details</summary>
Motivation: 大规模灾害往往造成严重后果，但传统权威数据（现场传感器、遥感影像等）存在时空信息缺口。社交媒体帖子可作为"地理传感器"提供实时灾害影响信息，但并非所有提及的地点都是受灾地点，需要区分受灾地点与非受灾地点以有效分配资源。

Method: 研究使用大型语言模型（LLMs）识别灾害相关社交媒体帖子中的所有地点、影响和受灾地点。通过微调LLMs，使其能够区分受灾地点与非受灾地点，包括识别非正式表达、缩写和简写形式的地点名称。

Result: 微调后的模型在影响识别上达到F1分数0.69，在受灾地点提取上达到F1分数0.74，显著优于预训练基线模型。这些稳健的结果证实了微调语言模型的有效性。

Conclusion: 微调的语言模型为灾害响应提供了可扩展的解决方案，能够支持及时的决策制定、资源分配、态势感知和灾后恢复规划，有助于提升灾害应对效率。
...
### [230] [Dissecting the Ledger: Locating and Suppressing "Liar Circuits" in Financial Large Language Models](https://arxiv.org/abs/2511.21756)
*Soham Mirajkar*

Main category: cs.CL

TL;DR: 研究发现LLM在金融算术推理中存在特定幻觉模式，通过因果追踪识别出双阶段机制：中层分布式计算草稿和后期聚合电路，并开发出高精度检测方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在金融等高风险领域部署时，存在可复现的算术运算幻觉问题，现有缓解策略通常将模型视为黑箱，需要更机制化的内在幻觉检测方法。

Method: 在GPT-2 XL架构上应用因果追踪技术，分析ConvFinQA基准测试中的算术推理过程，识别关键计算阶段，并通过消融实验验证机制。

Result: 发现算术推理的双阶段机制：中层（L12-L30）分布式计算草稿和后期（L46层）决定性聚合电路。抑制L46层可将模型对幻觉输出的置信度降低81.8%，基于该层的线性探测器在未见金融主题上达到98%准确率。

Conclusion: 揭示了LLM算术幻觉的机制基础，证明了算术欺骗存在普遍几何结构，为开发更可靠的金融领域LLM应用提供了机制化检测方法。
...
### [231] [Orchestrating Dual-Boundaries: An Arithmetic Intensity Inspired Acceleration Framework for Diffusion Language Models](https://arxiv.org/abs/2511.21759)
*Linye Wei,Wenjue Chen,Pingzhi Tang,Xiaotian Guo,Le Ye,Runsheng Wang,Meng Li*

Main category: cs.CL

TL;DR: ODB-dLLM框架通过自适应长度预测和跳转共享推测解码技术，显著加速扩散大语言模型推理，相比基线实现46-162倍加速


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型框架虽然支持KV缓存，但其双向注意力机制需要周期性刷新缓存，导致预填充和解码阶段交错，增加了推理成本并限制了加速潜力
...
Conclusion: ODB-dLLM通过协调预填充和解码阶段的异构计算特性，有效提升了扩散大语言模型的推理效率，在保持精度的同时实现了显著加速
...
### [232] [fMRI-LM: Towards a Universal Foundation Model for Language-Aligned fMRI Understanding](https://arxiv.org/abs/2511.21760)
*Yuxiang Wei,Yanteng Zhang,Xi Xiao,Chengxuan Qian,Tianyang Wang,Vince D. Calhoun*

Main category: cs.CL

TL;DR: fMRI-LM是一个将功能磁共振成像与语言连接的基础模型，通过三阶段框架实现脑成像数据的语义理解


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在图像、音频和视频推理方面取得了进展，但扩展到脑成像领域仍未被充分探索。弥合这一差距对于连接神经活动与语义认知以及开发跨模态脑表征至关重要。
...
Conclusion: fMRI-LM成功建立了fMRI与语言之间的桥梁，为脑成像数据的语义理解提供了可扩展的解决方案，支持多样化的下游应用。
...
### [233] [LLMs for Low-Resource Dialect Translation Using Context-Aware Prompting: A Case Study on Sylheti](https://arxiv.org/abs/2511.21761)
*Tabia Tanzin Prama,Christopher M. Danforth,Peter Sheridan Dodds*

Main category: cs.CL

TL;DR: 本文系统研究了LLM在低资源方言Sylheti翻译中的应用，发现现有模型在方言词汇处理上存在困难，提出了Sylheti-CAP框架显著提升翻译质量。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在翻译任务上表现出色，但在方言和低资源语言环境下的翻译能力尚未得到充分探索。Sylheti作为孟加拉语的低资源方言，其机器翻译研究存在空白。

Method: 评估了5个先进LLM在Sylheti↔Bangla双向翻译中的表现，发现方言词汇处理问题。提出了Sylheti-CAP框架，包含三个步骤：嵌入语言规则手册、包含2260个核心词汇和习语的词典、以及真实性检查。

Result: Sylheti-CAP框架在所有模型和提示策略中一致提高了翻译质量。自动指标和人工评估均证实其有效性，定性分析显示幻觉、歧义和生硬表达显著减少。
...
### [234] [Temporal Consistency for LLM Reasoning Process Error Identification](https://arxiv.org/abs/2503.14495)
*Jiacheng Guo,Yue Wu,Jiahao Qiu,Kaixuan Huang,Xinzhe Juan,Ling Yang,Mengdi Wang*

Main category: cs.CL

TL;DR: 提出了一种新的时间一致性验证方法，通过迭代自我反思改进数学推理验证效果，在多个数学过程错误识别基准上超越基线方法，使小规模蒸馏模型性能超越大模型。
...
Result: 在多个数学过程错误识别基准（Mathcheck、ProcessBench、PRM800K）上均表现出优于基线方法的性能提升。应用于DeepSeek R1蒸馏模型时，7B/8B蒸馏模型在ProcessBench上超越所有70B/72B模型和GPT-4o，14B蒸馏模型性能接近Deepseek-R1。

Conclusion: 时间一致性验证方法能有效提升数学推理验证的准确性，使小规模模型通过迭代自我反思达到甚至超越大规模模型的性能，为高效数学推理验证提供了新思路。
...
### [235] [FLAWS: A Benchmark for Error Identification and Localization in Scientific Papers](https://arxiv.org/abs/2511.21843)
...
<details>
  <summary>Details</summary>
Motivation: 随着科学产出呈指数增长，人类审稿人难以可靠地检测错误，而大语言模型在支持评估任务方面的潜力尚未充分探索，特别是在错误定位能力方面。

Method: 通过系统性地使用大语言模型在同行评审论文中插入使核心主张无效的错误，构建包含713个论文-错误对的基准测试，并设计自动评估指标来衡量模型识别和定位这些错误的能力。

Result: 在评估的五个前沿大语言模型（Claude Sonnet 4.5、DeepSeek Reasoner v3.1、Gemini 2.5 Pro、GPT 5和Grok 4）中，GPT 5表现最佳，在k=10（LLM生成的top-k错误文本候选）时达到39.1%的识别准确率。

Conclusion: FLAWS基准测试为评估大语言模型在科学写作中检测和定位错误的能力提供了系统方法，GPT-5在现有模型中表现最优，但仍有改进空间。
...
### [236] [On the Role of Preference Variance in Preference Optimization](https://arxiv.org/abs/2510.13022)
*Jiacheng Guo,Zihao Li,Jiahao Qiu,Yue Wu,Mengdi Wang*

Main category: cs.CL

TL;DR: 本文研究了偏好方差（PVar）对直接偏好优化（DPO）训练效果的影响，发现高PVar提示能产生更大的梯度更新，从而更有效地训练模型。


<details>
  <summary>Details</summary>
Motivation: 人类偏好数据收集成本高且效率低，需要减少所需标注的方法。本文旨在研究偏好方差对DPO训练效果的影响，以识别信息量更大的训练样本，提高LLM对齐效率。

Method: 1. 理论分析：建立DPO梯度范数的上界，证明其受提示的偏好方差控制；2. 实验验证：使用奖励模型生成偏好数据，在AlpacaEval 2.0和Arena-Hard基准上微调LLMs；3. 基于PVar的选择方法：比较高PVar提示与随机选择或低PVar提示的效果；4. 鲁棒性测试：使用较小奖励模型（1B、3B）进行选择；5. 真实数据验证：在UltraFeedback数据集上测试PVar选择效果。

Result: 1. 理论证明低PVar提示只能产生小的梯度更新，学习价值有限；2. 实验表明高PVar提示优于随机选择或低PVar提示；3. PVar选择方法对小奖励模型具有鲁棒性；4. 在UltraFeedback数据集上，仅使用PVar最高的前10%提示训练，性能优于使用完整数据集。

Conclusion: 偏好方差是识别信息量训练样本的关键指标，基于PVar的提示选择方法能显著提高DPO训练效率，减少所需标注数据量，为高效LLM对齐提供了新思路。
...
### [237] [Improving Score Reliability of Multiple Choice Benchmarks with Consistency Evaluation and Altered Answer Choices](https://arxiv.org/abs/2511.21860)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在多项选择题基准测试中的评分可能不可靠，即使获得高分也可能存在回答不一致的问题，需要一种能反映模型一致性的评估指标

Method: 提出CoRA指标，通过生成答案选项被修改的合成问题，计算两个中间分数BMCA和CI，然后调整原始MCQA分数以反映LLM的一致性水平

Result: 在不同基准测试中使用多种LLM进行评估，发现LLM即使获得高MCQA分数也可能表现出低响应一致性，而CoRA能成功降低不一致模型的分数
...
### [238] [Tracing How Annotators Think: Augmenting Preference Judgments with Reading Processes](https://arxiv.org/abs/2511.21912)
*Karin de Langis,William Walker,Khanh Chi Le,Dongyeop Kang*

Main category: cs.CL

TL;DR: 该研究提出了一种新的标注方法，不仅记录标注者的标签选择，还通过鼠标追踪捕捉其阅读过程（如重读、略读等），并创建了PreferRead数据集来分析偏好标注任务中的阅读行为。


<details>
  <summary>Details</summary>
Motivation: 传统标注方法只记录最终标签，忽略了标注者的认知过程。为了更深入理解标注者在复杂主观NLP任务中的决策机制、可靠性和分歧原因，需要捕捉标注过程中的阅读行为。

Method: 提出基于鼠标追踪的标注框架，创建PreferRead数据集，记录标注者在偏好标注任务中的细粒度阅读行为（如文本聚焦、重读、略读），分析标注者如何在提示和两个候选回答之间导航。

Result: 发现标注者在大约一半的试验中会重读回答，最常重读最终选择的选项，很少重读提示。阅读行为与标注结果显著相关：重读与更高的标注者间一致性相关，而长阅读路径和时间与较低的一致性相关。

Conclusion: 阅读过程为理解复杂主观NLP任务中标注者的可靠性、决策机制和分歧提供了补充的认知维度，表明标注行为分析有助于提高标注质量评估。
...
### [239] [A Comparative Study of LLM Prompting and Fine-Tuning for Cross-genre Authorship Attribution on Chinese Lyrics](https://arxiv.org/abs/2511.21930)
*Yuxin Li,Lorraine Xu,Meng Fan Wang*

Main category: cs.CL

TL;DR: 该研究创建了首个中文歌词作者归属数据集，开发了领域特定模型，并与DeepSeek LLM的零样本推理进行对比，发现不同流派的作者归属准确率差异显著。
...
Method: 创建平衡的中文歌词多流派数据集，开发并微调领域特定模型，与DeepSeek LLM的零样本推理进行对比，在两个测试集上进行评估。

Result: 假设2得到强有力证实：结构化流派（如民俗与传统）的作者归属准确率显著高于抽象流派（如爱情与浪漫）。假设1仅得到部分支持：微调在真实数据测试中提高了鲁棒性和泛化能力，但在合成增强的小测试集中增益有限。

Conclusion: 建立了跨流派中文歌词作者归属的首个基准，强调了流派敏感评估的重要性，提供了公开数据集和分析框架，建议扩大测试集多样性、减少对词级数据增强的依赖、平衡作者在流派中的代表性，并探索领域自适应预训练以提升性能。
...
### [240] [Start Making Sense(s): A Developmental Probe of Attention Specialization Using Lexical Ambiguity](https://arxiv.org/abs/2511.21974)
*Pamela D. Rivière,Sean Trott*

Main category: cs.CL

TL;DR: 研究提出了一套系统探测Transformer语言模型中注意力机制的方法，利用词汇歧义作为测试案例，发现不同规模模型中注意力头在消歧任务上的发展模式和鲁棒性存在差异。


<details>
  <summary>Details</summary>
Motivation: 尽管从原理上理解Transformer语言模型中的自注意力矩阵操作，但尚不清楚这些操作如何映射到可解释的计算或功能，以及个体注意力头何时发展出专门的注意力模式。研究旨在系统探测注意力机制，并以词汇歧义作为测试案例来分离参与词义消歧的注意力机制。

Method: 采用"发展性"方法：1) 使用公开的Pythia语言模型检查点，识别每个模型在消歧性能上的拐点；2) 在14M和410M模型中，识别注意力模式与整体消歧性能共同发展的注意力头；3) 通过刺激扰动测试这些头的鲁棒性；4) 进行因果分析，通过消融目标头来评估对消歧性能的影响；5) 在14M模型的所有随机种子中复现发展性分析。

Result: 1) 在14M和410M模型中识别出注意力模式与消歧性能共同发展的注意力头；2) 14M模型中的头对刺激扰动的鲁棒性有限，而410M模型中识别出多个具有惊人泛化行为的头；3) 消融目标头会显著损害消歧性能，特别是在14M模型中；4) 14M模型的发展分析在所有随机种子中可复现；5) 消歧受益于多种机制，其中一些（特别是在14M中）对消歧线索的位置和词性高度敏感；6) 更大模型（410M）可能包含具有更鲁棒消歧行为的头。

Conclusion: 研究结果表明：消歧任务受益于多种注意力机制，不同规模模型中这些机制的特性和鲁棒性存在差异；更大模型可能发展出更鲁棒的消歧行为；采用发展性视角对探测语言模型机制具有重要价值。
...
### [241] [AfriStereo: A Culturally Grounded Dataset for Evaluating Stereotypical Bias in Large Language Models](https://arxiv.org/abs/2511.22016)
...
<details>
  <summary>Details</summary>
Motivation: 现有AI偏见评估基准主要反映西方视角，导致非洲背景代表性不足，使得各种应用领域中的有害刻板印象得以延续。需要填补这一空白，建立基于当地社会文化背景的非洲刻板印象评估框架。

Method: 通过塞内加尔、肯尼亚和尼日利亚的社区参与收集了1,163个刻板印象，涵盖性别、民族、宗教、年龄和职业维度。使用少样本提示和人工循环验证，将数据集扩展到5,000多个刻板印象-反刻板印象对。通过语义聚类和文化知情评审员的手动注释进行验证。

Result: 对语言模型的初步评估显示，11个模型中有9个表现出统计显著的偏见，偏见偏好比(BPR)范围为0.63到0.78(p <= 0.05)，表明对刻板印象相对于反刻板印象的系统性偏好，特别是在年龄、职业和性别维度上。领域特定模型在设置中显示出较弱的偏见。
...
### [242] [ResearchArcade: Graph Interface for Academic Tasks](https://arxiv.org/abs/2511.22036)
*Jingjun Xu,Chongshan Lin,Haofei Yu,Tao Feng,Jiaxuan You*

Main category: cs.CL

TL;DR: ResearchArcade是一个基于图的统一数据接口，连接多个学术数据源，支持多模态信息处理和时间演化分析，用于开发支持学术任务的机器学习模型。


<details>
  <summary>Details</summary>
Motivation: 随着研究人员越来越多地使用机器学习辅助研究任务，需要建立一个统一的数据接口来支持各种学术任务的机器学习模型开发，从而更好地支持研究人员并加速知识发现。

Method: ResearchArcade采用基于图的接口设计，使用一致的多表格式和图形结构组织来自不同来源的数据（如ArXiv和OpenReview），支持文本、图表等多模态信息，并保留手稿和社区层面的时间演化信息。

Result: 在六个学术任务上的实验表明，结合跨源和多模态信息能够支持更广泛的任务范围，而图结构的加入相比基线方法持续提升了性能表现。

Conclusion: ResearchArcade展示了其有效性，并具有推动研究进展的潜力，为学术任务的机器学习模型开发提供了一个统一的数据接口框架。
...
### [243] [A Hybrid Theory and Data-driven Approach to Persuasion Detection with Large Language Models](https://arxiv.org/abs/2511.22109)
*Gia Bao Hoang,Keith J Ransom,Rachel Stephens,Carolyn Semmler,Nicolas Fay,Lewis Mitchell*

Main category: cs.CL

TL;DR: 本研究利用大语言模型开发了一个预测在线说服成功的模型，发现认知情绪和分享意愿是信念改变的最强预测因子。
...
Method: 采用混合方法，利用大语言模型生成文献中先前研究的特征评分，构建随机森林分类模型来预测消息是否会导致信念改变。测试了八个特征。

Result: 在测试的八个特征中，认知情绪和分享意愿是信念改变的最强预测因子。模型能够有效识别说服性消息的特征。

Conclusion: 研究揭示了说服性消息的特征，展示了大语言模型如何基于心理学理论增强成功说服模型。这项工作在在线影响力检测、错误信息缓解以及测量在线叙事有效性等领域具有广泛应用前景。
...
### [244] [Bridging the Modality Gap by Similarity Standardization with Pseudo-Positive Samples](https://arxiv.org/abs/2511.22141)
*Shuhei Yamashita,Daiki Shirafuji,Tatsuhiko Saito*

Main category: cs.CL

TL;DR: 提出相似度标准化方法解决跨模态检索中的模态差异问题，通过伪数据构建计算模态特定统计量，将不同模态的相似度分数标准化到统一尺度


<details>
  <summary>Details</summary>
Motivation: 当数据库同时包含文本和图像时，视觉语言模型(VLMs)的相似度分数在不同模态间存在尺度差异（模态差异），这阻碍了准确的跨模态检索。现有方法大多依赖人工标注数据进行微调，需要解决这一问题

Method: 提出相似度标准化方法：1) 为每个查询构建伪配对数据，通过检索与查询余弦相似度最高的文本和图像候选；2) 计算查询与其配对数据在文本或图像模态的相似度分数的均值和方差；3) 使用这些模态特定统计量将所有相似度分数标准化到跨模态的统一尺度

Result: 在7个VLMs和两个多模态QA基准(MMQA和WebQA)上评估，当查询与目标数据属于不同模态时，在MMQA上平均Recall@20提升64%，在WebQA上提升28%。与通过图像字幕解决模态差异的E5-V相比，本方法更有效地弥合了模态差异

Conclusion: 提出的相似度标准化方法通过伪数据构建有效解决了跨模态检索中的模态差异问题，无需人工标注数据，显著提升了检索性能
...
### [245] [C$^2$DLM: Causal Concept-Guided Diffusion Large Language Models](https://arxiv.org/abs/2511.22146)
...
<details>
  <summary>Details</summary>
Motivation: 当前自回归语言模型和扩散语言模型在推理能力方面存在不足。人类推理依赖于因果知识和思维，而自回归模型采用严格的左到右标记预测，扩散模型则使用全连接注意力完全忽略因果顺序，需要填补这一空白。

Method: 从扩散语言模型的全连接注意力出发，首先从教师模型获取概念级因果图，然后显式引导注意力学习概念间的因果关系，避免因果反转带来的困难子目标干扰。

Result: 在COT-OrderPerturb任务中提升12%，训练速度加快约3.2倍；在六个下游推理任务上平均增益1.31%。

Conclusion: C²DLM通过引入因果概念引导机制，有效提升了语言模型的推理能力，填补了现有语言模型在因果推理方面的不足。
...
### [246] [A Theoretically Grounded Hybrid Ensemble for Reliable Detection of LLM-Generated Text](https://arxiv.org/abs/2511.22153)
*Sepyan Purnama Kristanto,Lutfi Hakim*

Main category: cs.CL

TL;DR: 提出了一种基于混合集成方法的AI生成文本检测系统，融合三种互补检测范式，通过优化加权投票框架显著降低误报率，特别针对学术文本检测效果优异。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速扩散模糊了人类与机器作者的界限，对学术诚信和信息可靠性构成实际风险。现有文本检测器通常依赖单一方法范式，泛化能力差且误报率高，特别是在高风险学术文本上表现不佳。

Method: 提出理论基础的混合集成系统，融合三种互补检测范式：1)基于RoBERTa的transformer分类器用于深度语义特征提取；2)基于GPT-2的概率检测器使用扰动诱导似然曲率；3)统计语言学特征分析器捕捉文体测量模式。核心创新在于优化的加权投票框架，集成权重在学习概率单纯形上最大化F1分数而非启发式设置。

Result: 在包含30,000个文档的大规模多生成器语料库上评估，系统达到94.2%的准确率和0.978的AUC，在学术文本上误报率相对降低35%。模型间相关性低(rho ~ 0.35-0.42)，满足方差减少的关键条件。

Conclusion: 该系统为教育和其它高风险领域的实际部署提供了更可靠、更符合伦理责任的检测器，通过系统融合多种检测范式和优化集成权重，显著提升了检测性能并降低了误报率。
...
### [247] [Lips-Jaw and Tongue-Jaw Articulatory Tradeoff in DYNARTmo](https://arxiv.org/abs/2511.22155)
*Bernd J. Kröger*
...
<details>
  <summary>Details</summary>
Motivation: 研究动态发音模型DYNARTmo如何解释主要和次要发音器官之间的发音权衡，特别是唇-颌和舌-颌协调，探索简化模型能否捕捉真实发音运动的关键特征。

Method: 采用一阶任务空间手势规范，整合简化机制来分配多个发音器官间的发音努力，通过CV音节模拟分析颌位移随发音部位和元音环境的变化。

Result: 模型成功复现了经验证实的发音协同模式：颌支持的舌尖闭合、双唇塞音中的下唇抬高、舌-颌协同运动、唇部收缩中的饱和效应。
...
### [248] [RefineBench: Evaluating Refinement Capability of Language Models via Checklists](https://arxiv.org/abs/2511.22173)
...
TL;DR: 该研究评估语言模型在引导反馈和自我改进两种模式下的精炼能力，发现前沿模型在自我精炼方面表现有限，但在引导反馈下能达到接近完美的水平。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型是否能够自我精炼其响应，因为现实世界用户交互常涉及精炼请求，而先前研究主要关注可验证任务，忽略了用户常提出的开放式查询和不同程度的反馈需求。

Method: 引入RefineBench基准，包含11个领域的1000个挑战性问题，并采用基于检查表的评估框架。评估两种精炼模式：1) 引导精炼（提供自然语言反馈）；2) 自我精炼（无指导的自我改进）。

Result: 在自我精炼设置中，前沿模型如Gemini 2.5 Pro和GPT-5基线得分分别为31.3%和29.1%，大多数模型无法在迭代中持续改进。在引导精炼中，专有模型和大型开源模型（>70B）能在五轮内利用针对性反馈将响应精炼到接近完美水平。

Conclusion: 前沿语言模型需要突破性进展才能自我精炼其错误响应，RefineBench为跟踪这一进展提供了有价值的测试平台。
...
### [249] [Focused Chain-of-Thought: Efficient LLM Reasoning via Structured Input Information](https://arxiv.org/abs/2511.22176)
*Lukas Struppek,Dominik Hintersdorf,Hannah Struppek,Daniel Neider,Kristian Kersting*

Main category: cs.CL

TL;DR: Focused Chain-of-Thought (F-CoT) 通过结构化输入提取关键信息，减少推理过程中的冗余token，在保持准确性的同时将生成token减少2-3倍。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型通过生成详细的思维链进行推理，但这导致过多的token使用和高推理延迟。传统效率方法主要关注模型中心干预，如强化学习或监督微调，但本文提出了一种无需训练、以输入为中心的新方法。
...
### [250] [Beyond Query-Level Comparison: Fine-Grained Reinforcement Learning for Text-to-SQL with Automated Interpretable Critiques](https://arxiv.org/abs/2511.22258)
*Guifeng Wang,Yuanfeng Song,Meng Yang,Tao Zhu,Xiaoming Yin,Xing Chen*

Main category: cs.CL

TL;DR: RuCo-C是一个用于文本到SQL任务的生成式评估模型，通过自动生成查询特定的评估标准，提供细粒度的、可解释的反馈，无需人工标注，并在强化学习训练中通过"渐进探索"策略集成稠密奖励反馈。


<details>
  <summary>Details</summary>
Motivation: 当前文本到SQL任务面临两个主要瓶颈：1) 评估和奖励机制严重依赖昂贵的人工标注黄金SQL查询，难以大规模应用；2) 强化学习方法仅使用最终二元执行结果作为奖励信号，这种粗粒度监督忽略了从评估标准角度看的详细结构和语义错误。

Method: RuCo-C框架包含两个核心部分：1) 自动生成查询特定的评估标准，将其与可解释的批评关联，实现无需人工干预的自动评估；2) 在强化学习训练过程中，通过"渐进探索"策略集成稠密奖励反馈，动态调整奖励以提升模型性能。
...
### [251] [Token-Level Marginalization for Multi-Label LLM Classifiers](https://arxiv.org/abs/2511.22312)
*Anjaneya Praharaj,Jaykumar Kasundra*

Main category: cs.CL

TL;DR: 提出三种新颖的token级概率估计方法，为生成式语言模型在多标签内容安全分类中提供可解释的置信度分数，提升模型可解释性和准确性。
...
Method: 提出并评估三种新颖的token级概率估计方法，通过利用token对数概率来为生成式分类器提供置信度分数，并在不同指令调优模型上评估该框架的泛化能力。

Result: 在合成生成且经过严格标注的数据集上进行广泛实验，结果表明利用token对数概率显著提高了生成式分类器的可解释性和可靠性，实现了更细致的内容安全审核。
...
### [252] [Sentiment Analysis Of Shopee Product Reviews Using Distilbert](https://arxiv.org/abs/2511.22313)
*Zahri Aksa Dautd,Aviv Yuniar Rahman*

Main category: cs.CL

TL;DR: 本研究使用轻量级Transformer模型DistilBERT对Shopee平台产品评论进行情感分析，在保持较高准确率的同时显著提升了计算效率。


<details>
  <summary>Details</summary>
Motivation: 随着电子商务的快速发展，Shopee等平台每天产生海量消费者评论，手动分析效率低下，需要计算化方法进行情感分析以提取有价值的客户满意度和偏好信息。

Method: 采用DistilBERT（distilbert-base-uncased模型）对约100万条英文Shopee产品评论进行预处理和训练，使用准确率、精确率、召回率和F1分数进行评估，并与BERT和SVM基准模型进行比较。

Result: DistilBERT达到94.8%的准确率，略低于BERT（95.3%），但显著高于SVM（90.2%），计算时间减少超过55%，在准确性和效率之间取得了良好平衡。

Conclusion: DistilBERT在大规模电子商务平台情感分析中提供了准确性和效率的最佳平衡，适合处理海量评论数据，为电商平台提供实用的情感分析解决方案。
...
### [253] [Exploring Performance Variations in Finetuned Translators of Ultra-Low Resource Languages: Do Linguistic Differences Matter?](https://arxiv.org/abs/2511.22482)
*Isabel Gonçalves,Paulo Cavalin,Claudio Pinhanez*

Main category: cs.CL

TL;DR: 研究发现，使用类似方法和数据创建的翻译器性能差异显著，但差异原因并非训练因素，而是语言本身特性差异


<details>
  <summary>Details</summary>
Motivation: 先前研究显示，使用类似方法和少量数据微调预训练语言模型创建超低资源语言翻译器时，性能差异很大。本研究旨在系统探索这些性能差异的原因，确定是数据清洗、预训练模型限制、基础模型大小还是训练数据大小的影响
...
Conclusion: 翻译器性能的显著差异主要源于语言本身特性差异，而非训练过程的技术因素，这对超低资源语言翻译研究具有重要意义
...
### [254] [Joint Speech and Text Training for LLM-Based End-to-End Spoken Dialogue State Tracking](https://arxiv.org/abs/2511.22503)
*Katia Vendrame,Bolaji Yusuf,Santosh Kesiraju,Šimon Sedláček,Oldřich Plchot,Jan Černocký*

Main category: cs.CL

TL;DR: 该论文提出了一种联合训练方法，使用可用的语音对话状态跟踪数据和来自其他领域的文本数据，以实现跨领域泛化，解决语音DST数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 端到端语音对话状态跟踪面临两大挑战：处理语音输入和数据稀缺。虽然结合语音基础编码器和大型语言模型的方法在单领域表现出色，但难以跨领域泛化，且需要为每个目标领域收集昂贵的语音DST标注数据。

Method: 提出联合训练方法，同时使用可用的语音DST数据和来自其他领域的文本DST数据进行训练。利用文本数据更容易获取的特点，通过多领域文本数据增强模型的跨领域泛化能力。

Result: 实验表明，该方法能够在不需要目标领域语音训练数据的情况下，实现良好的跨领域DST性能，有效解决了语音DST数据稀缺和跨领域泛化问题。

Conclusion: 通过联合使用语音DST数据和多领域文本数据，可以构建具有跨领域泛化能力的语音对话状态跟踪系统，降低对昂贵语音标注数据的依赖。
...
### [255] [Smarter, not Bigger: Fine-Tuned RAG-Enhanced LLMs for Automotive HIL Testing](https://arxiv.org/abs/2511.22584)
...
<details>
  <summary>Details</summary>
Motivation: 汽车硬件在环测试中存在测试工件碎片化和利用率低的问题，需要更智能的系统来整合和利用这些测试资源。

Method: 提出HIL-GPT系统，采用检索增强生成框架，通过启发式挖掘和LLM辅助合成构建领域特定数据集，对嵌入模型进行微调，结合向量索引实现可扩展、可追溯的测试用例和需求检索。

Result: 实验表明，经过微调的紧凑模型（如bge-base-en-v1.5）在准确性、延迟和成本之间取得了更好的平衡，优于大型模型；用户研究证实RAG增强的助手在帮助性、真实性和满意度方面优于通用LLM。
...
### [256] [Improving LLM-based Ontology Matching with fine-tuning on synthetic data](https://arxiv.org/abs/2511.22612)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地被集成到本体匹配流程中，但缺乏足够的参考对齐数据进行训练。本文旨在探索LLM直接执行本体匹配的能力，并通过专门的微调策略提升其在零样本设置下的匹配性能。

Method: 1) 采用搜索空间缩减技术从源和目标本体中选择相关子集，自动构建提示；2) 引入基于LLM的新方法生成合成数据集，创建本体子模块对及其参考对齐的语料库；3) 使用合成数据微调LLM用于本体匹配任务。

Result: 在OAEI复杂跟踪的Conference、Geolink、Enslaved、Taxon和Hydrography数据集上评估，结果显示在合成数据上微调的LLM相比未微调的基模型表现出更优越的性能。

Conclusion: 提出了一种结合自动数据集生成与微调的策略，能够有效适应LLM执行本体匹配任务，为解决训练数据稀缺问题提供了创新方法。
...
### [257] [Modeling Romanized Hindi and Bengali: Dataset Creation and Multilingual LLM Integration](https://arxiv.org/abs/2511.22769)
...
<details>
  <summary>Details</summary>
Motivation: 当前多语言模型在处理罗马化脚本（用罗马字母表示其他语言语音）时存在挑战，特别是在南亚语境下，印地语和孟加拉语等印欧语系语言在社交媒体上广泛使用罗马化文本，但现有转写数据集缺乏发音和拼写多样性、代码混合数据以及低资源适应能力。

Method: 1) 为印地语和孟加拉语创建了包含近180万对印地语和100万对孟加拉语转写对的新数据集；2) 基于Marian架构预训练了定制的多语言序列到序列大语言模型。

Result: 实验结果表明，与现有相关模型相比，在BLEU和CER指标上取得了显著改进。

Conclusion: 该研究填补了印欧语系语言罗马化转写的研究空白，通过大规模数据集和定制模型有效提升了转写性能，对自然语言处理任务如情感分析、语音识别等具有重要意义。
...
### [258] [Mitigating Semantic Drift: Evaluating LLMs' Efficacy in Psychotherapy through MI Dialogue Summarization](https://arxiv.org/abs/2511.22818)
*Vivek Kumar,Pushpraj Singh Rajawat,Eirini Ntoutsi*

Main category: cs.CL

TL;DR: 该研究评估大型语言模型在心理治疗中的效能，通过生成动机性访谈对话摘要，基于MITI框架设计标注方案，评估模型在复杂心理概念理解方面的表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用和特定领域任务中显示出潜力，但在心理学等低资源和敏感领域中存在敏感性不足、事实错误、共情表达不一致、偏见、幻觉等问题，无法捕捉人类理解的深度和复杂性。

Method: 采用混合方法评估LLMs在心理治疗中的效能：1) 使用LLMs生成动机性访谈对话的精确摘要；2) 基于MITI框架（唤起、合作、自主、方向、共情、非评判态度）设计两阶段标注方案；3) 使用专家标注的MI对话作为基准，制定多分类任务；4) 评估渐进提示技术（单样本和少样本提示）下的模型表现。

Result: 研究结果提供了关于LLMs理解复杂心理概念能力的见解，并提出了在治疗环境中减轻"语义漂移"的最佳实践。为MI社区提供了高质量标注数据集以解决低资源领域数据稀缺问题，并为在复杂行为治疗中使用LLMs进行精确上下文解释提供了关键见解。

Conclusion: 该研究不仅通过提供高质量标注数据集解决了低资源领域的数据稀缺问题，还为在复杂行为治疗中使用LLMs进行精确上下文解释提供了重要见解，有助于提升LLMs在敏感心理治疗领域的应用效果。
...
### [259] [RAG System for Supporting Japanese Litigation Procedures: Faithful Response Generation Complying with Legal Norms](https://arxiv.org/abs/2511.22858)
*Yuya Ishihara,Atsushi Keyaki,Hiroaki Yamada,Ryutaro Ohara,Mihoko Sumida*

Main category: cs.CL

TL;DR: 本文探讨了基于RAG的LLM系统在支持日本医疗诉讼程序中应具备的组件，以确保符合法律规范，特别是替代专家委员角色时的三个关键要求。


<details>
  <summary>Details</summary>
Motivation: 在诉讼中，专家委员（如医生、建筑师、会计师、工程师）为法官提供专业知识以澄清争议点。考虑用基于RAG的LLM系统替代这些专家角色时，需要严格遵循法律规范，这带来了特定的约束和要求。

Method: 本文讨论了满足三个关键要求的RAG-based LLM系统设计：1) 检索模块必须根据禁止使用私人知识的原则，检索与争议问题相关的外部知识；2) 生成的回答必须源自RAG提供的上下文并忠实于该上下文；3) 检索模块必须引用具有适当时间戳的外部知识以对应相关问题。

Result: 本文提出了一个满足法律规范要求的RAG-based LLM系统设计框架，能够支持日本医疗诉讼程序，确保系统在替代专家委员角色时符合法律约束。

Conclusion: 为支持日本医疗诉讼程序，基于RAG的LLM系统需要满足三个关键法律规范要求：适当的检索机制、忠实于上下文的生成能力，以及时间戳匹配的外部知识引用。这些要求确保了系统在替代专家委员角色时的合规性和可靠性。
...
### [260] [JBE-QA: Japanese Bar Exam QA Dataset for Assessing Legal Domain Knowledge](https://arxiv.org/abs/2511.22869)
*Zhihan Cao,Fumihito Nishino,Hiroaki Yamada,Nguyen Ha Thanh,Yusuke Miyao,Ken Satoh*

Main category: cs.CL

TL;DR: JBE-QA是首个日本司法考试问答数据集，用于评估大语言模型的法律知识，涵盖民法、刑法和宪法，包含3,464个平衡标注项目，评估了26个LLM。


<details>
  <summary>Details</summary>
Motivation: 现有日本法律领域评估资源主要关注民法，缺乏全面的法律知识评估基准。需要创建一个覆盖日本主要法律领域（民法、刑法、宪法）的标准化评估数据集。

Method: 基于日本司法考试（2015-2024年）多选题部分构建JBE-QA数据集，将每个问题分解为独立的真/假判断，并添加结构化上下文字段。数据集包含3,464个平衡标注项目。

Result: 评估了26个LLM（包括专有模型、开源模型、日本专业模型和推理模型）。结果显示：启用推理的专有模型表现最佳；宪法问题通常比民法或刑法问题更容易。

Conclusion: JBE-QA为日本法律领域LLM评估提供了首个全面基准，揭示了不同模型在法律知识掌握上的差异，特别是宪法相对容易而民法和刑法更具挑战性。
...
### [261] [FEANEL: A Benchmark for Fine-Grained Error Analysis in K-12 English Writing](https://arxiv.org/abs/2511.22883)
...
TL;DR: 该论文提出了FEANEL基准测试，用于评估大语言模型在K-12英语写作中进行细粒度错误分析的能力，发现现有模型在此任务上存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在教育应用方面具有巨大潜力，但它们在K-12英语写作中提供细粒度教育反馈的能力尚未得到充分探索。现有研究缺乏对LLMs在英语学习者错误分析方面的系统评估。

Method: 1. 提出英语学习者细粒度错误分析问题；2. 开发FEANEL基准测试，包含1000篇中小学学生作文；3. 建立基于词性的英语写作错误分类体系；4. 由语言教育专家对错误进行类型、严重程度和解释性反馈标注；5. 在FEANEL基准上评估最先进的LLMs。

Result: 实验结果显示，当前大语言模型在执行细粒度错误分析方面存在显著差距，特别是在错误类型识别、严重程度评估和提供解释性反馈方面表现不足。

Conclusion: 需要开发专门的方法来提升LLMs在教育应用中的细粒度错误分析能力，FEANEL基准为这一研究方向提供了重要的评估工具。
...
### [262] [Language-conditioned world model improves policy generalization by reading environmental descriptions](https://arxiv.org/abs/2511.22904)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在理解描述环境动态的语言方面存在局限：要么无法展示策略在未见游戏中的泛化能力，要么依赖规划延迟可容忍或专家演示等限制性假设。需要改进语言条件化世界模型的策略泛化能力，同时去除这些假设。

Method: 提出基于模型的强化学习方法LED-WM（Language-aware Encoder for Dreamer World Model），基于DreamerV3构建。使用注意力机制显式地将语言描述与观察中的实体进行关联，训练语言条件化的世界模型，并从中学习策略，无需规划或专家演示。

Result: 在MESSENGER和MESSENGER-WM两个环境中，LED-WM训练的策略相比其他基线方法，在由新动态和语言描述的未见游戏中表现出更好的泛化能力。还展示了策略可以通过世界模型生成的合成测试轨迹进行微调来进一步提升。

Conclusion: LED-WM方法通过语言条件化的世界模型有效提升了策略在未见游戏中的泛化能力，无需依赖规划或专家演示，为智能体理解环境动态语言提供了更实用的解决方案。
...
### [263] [Visual Puns from Idioms: An Iterative LLM-T2IM-MLLM Framework](https://arxiv.org/abs/2511.22943)
*Kelaiti Xiao,Liang Yang,Dongyu Zhang,Paerhati Tulajiang,Hongfei Lin*

Main category: cs.CL

TL;DR: 本文提出了一种基于成语的视觉双关自动生成框架，通过LLM、T2IM和MLLM的协同迭代，生成并评估将成语字面义和比喻义结合的图像，创建了包含1000个成语的数据集用于基准测试。


<details>
  <summary>Details</summary>
Motivation: 研究基于成语的视觉双关图像生成，这类图像需要同时体现成语的字面意义和比喻意义，是一个具有挑战性的多模态任务。目前缺乏系统性的生成框架和评估数据集。

Method: 提出迭代框架：给定成语，系统循环执行：(1) LLM生成详细视觉提示，(2) T2IM合成图像，(3) MLLM从图像推断成语，(4) 若识别失败则优化提示，直到成功或达到步数限制。使用1000个成语作为输入。

Result: 生成了包含视觉双关图像和配对提示的数据集。实验评估了10个LLM、10个MLLM和1个T2IM（Qwen-Image），发现MLLM选择是性能主要驱动因素：GPT准确率最高，Gemini次之，最佳开源MLLM（Gemma）与部分闭源模型竞争。在LLM方面，Claude在提示生成方面表现最佳。

Conclusion: 该框架成功实现了成语视觉双关的自动生成和评估，创建了有价值的基准数据集，揭示了不同模型在视觉双关任务中的性能差异，为多模态理解和生成研究提供了新工具。
...
### [264] [Training-Free Loosely Speculative Decoding: Accepting Semantically Correct Drafts Beyond Exact Match](https://arxiv.org/abs/2511.22972)
*Jinze Li,Yixing Xu,Guanchen Li,Shuo Yang,Jinfeng Xu,Xuanwu Yin,Dong Li,Edith C. H. Ngai,Emad Barsoum*

Main category: cs.CL

TL;DR: FLy是一种无需训练、基于松散验证的推测解码方法，通过目标模型的自校正行为判断候选token的语义有效性，在保持99%以上准确率的同时实现显著加速。
...
Method: 提出训练免费的松散推测解码（FLy），采用双层机制：1）熵级门控判断当前token是否允许多个合理替代；2）token级延迟窗口区分真正错误与语义正确但表述不同的变体。同时设计多级加速策略，不仅加速目标模型，也加速草稿模型本身。

Result: 在Llama-3.1-70B-Instruct上平均加速2.81倍，在405B变体上加速5.07倍，同时保持99%以上的目标模型准确率。在分布外数据集上，FLy仍保持高效，比基于训练的方法EAGLE-3快1.62倍。

Conclusion: FLy通过训练免费设计和松散验证机制，在保持模型准确性的同时显著降低推理延迟，具有良好的跨模型和跨领域泛化能力，为大规模语言模型的高效推理提供了有效解决方案。
...
### [265] [Social Perceptions of English Spelling Variation on Twitter: A Comparative Analysis of Human and LLM Responses](https://arxiv.org/abs/2511.23041)
*Dong Nguyen,Laura Rosseel*

Main category: cs.CL

TL;DR: 研究比较了人类与大型语言模型对英语在线写作中拼写变体（如funnnn vs. fun）的社会感知差异，发现两者在形式性、仔细度和年龄三个社会属性上的评分存在强相关性，但在评分分布和不同类型拼写变体间存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 拼写变体（如funnnn与fun）会影响文本及其作者的社会感知，人们通常对不同书写形式有各种联想（文本是否非正式？作者是否年轻？）。本研究旨在探索在线英语写作中拼写变体的社会感知，并比较人类与大型语言模型在这方面感知的一致性程度。

Method: 基于社会语言学方法，比较LLM和人类对拼写变体三个关键社会属性（形式性、仔细度、年龄）的评分。研究分析了评分分布和不同类型拼写变体间的比较。

Result: 研究发现人类与LLM的评分之间存在普遍强相关性。然而，在分析评分分布和比较不同类型拼写变体时，出现了显著差异。

Conclusion: 虽然人类与大型语言模型对拼写变体的社会感知总体上具有强相关性，但在具体评分分布和不同类型变体间存在明显差异，这表明LLM在捕捉细微社会语言学特征方面仍有改进空间。
...
### [266] [Decoding the Past: Explainable Machine Learning Models for Dating Historical Texts](https://arxiv.org/abs/2511.23056)
*Paulo J. N. Pinto,Armando J. Pinho,Diogo Pratas*

Main category: cs.CL

TL;DR: 使用可解释的基于树的机器学习模型进行历史文本年代分类，整合五种特征类别，在世纪和十年尺度上取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 准确确定历史文本的年代对于组织和解释文化遗产收藏至关重要，需要开发可解释且有效的文本年代分类方法

Method: 采用基于树的可解释机器学习模型，整合五种特征类别：压缩特征、词汇结构、可读性、新词检测和距离特征，用于预测跨越五个世纪的英文文本年代

Result: 在世纪尺度预测达到76.7%准确率，十年尺度达到26.1%，显著高于随机基线；在宽松时间精度下性能更高；特征重要性分析显示距离特征和词汇结构最具信息性

Conclusion: 基于树的模型提供了可扩展、可解释的文本年代分类替代方案，19世纪被识别为语言演变的关键转折点，尽管跨数据集评估存在领域适应挑战
...
### [267] [Accent Placement Models for Rigvedic Sanskrit Text](https://arxiv.org/abs/2511.23088)
...
<details>
  <summary>Details</summary>
Motivation: 《梨俱吠陀》作为最古老的梵文文本之一，使用独特的音高重音系统（udātta、anudātta、svarita），这些重音标记编码了旋律和解释线索，但在现代电子文本中经常缺失。需要开发自动重音标注方法来恢复这些重要的语言学特征。

Method: 1. 创建带重音和不带重音的《梨俱吠陀》诗节平行语料库；2. 比较三种自动重音标注策略：ByT5全微调（直接在Unicode组合标记上操作的字节级Transformer）、BiLSTM-CRF序列标注基线、以及基于LoRA的参数高效微调；3. 使用WER、CER和任务特定的DER进行评估。

Result: ByT5全微调在所有指标上获得最低错误率；LoRA提供了良好的效率-准确性权衡；BiLSTM-CRF作为透明基线。研究强调了重音恢复的实际要求：Unicode安全预处理、标记感知分词、以及分离字形和重音错误的评估方法。

Conclusion: 该研究为《梨俱吠陀》重音恢复建立了可复现的基线，为下游任务（如重音感知OCR、ASR/吟唱合成和数字学术研究）提供指导，并将遗产语言技术定位为连接计算建模与文献学和教学目标的NLP新兴领域。
...
### [268] [Mind Reading or Misreading? LLMs on the Big Five Personality Test](https://arxiv.org/abs/2511.23101)
*Francesco Di Cursi,Chiara Boldrini,Marco Conti,Andrea Passarella*

Main category: cs.CL

TL;DR: 研究评估了大型语言模型在基于文本的自动人格预测中的表现，发现当前现成的LLMs还不适合用于APPT任务，需要精心设计提示、特质框架和评估指标才能获得可解释的结果。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在自动人格预测任务中的表现，探索不同模型和提示策略对二元五因素模型人格特质预测的影响，为实际应用提供指导。

Method: 使用五个大型语言模型（包括GPT-4和轻量级开源替代品），在三个异构数据集（Essays、MyPersonality、Pandora）上测试，采用两种提示策略（最小提示与包含语言学和心理学线索的丰富提示）。
...
Conclusion: 当前现成的LLMs还不适合用于自动人格预测任务，需要精心协调提示设计、特质框架和评估指标才能获得可解释的结果。聚合指标如准确率和宏F1掩盖了显著的不对称性，每类召回率提供了更清晰的诊断价值。
...
### [269] [Dripper: Token-Efficient Main HTML Extraction with a Lightweight LM](https://arxiv.org/abs/2511.23119)
...
Conclusion: Dripper通过创新的HTML简化、任务重构和受控解码机制，成功解决了轻量级语言模型在网页内容提取中的效率、成本和幻觉问题，为大规模训练数据获取提供了高效解决方案。
...
### [270] [Multi-chain Graph Refinement and Selection for Reliable Reasoning in Large Language Models](https://arxiv.org/abs/2511.23136)
...
Method: 提出MGRS框架：1）为给定问题生成多个多样化推理轨迹；2）使用自验证和交叉验证的复合策略精炼候选响应；3）构建推理关系图并估计中间节点成功率；4）计算累积成功率以选择最可靠答案和推理轨迹。
...
### [271] [Are LLMs Good Safety Agents or a Propaganda Engine?](https://arxiv.org/abs/2511.23174)
*Neemesh Yadav,Francesco Ortu,Jiarui Liu,Joeun Yook,Bernhard Schölkopf,Rada Mihalcea,Alberto Cazzaniga,Zhijing Jin*

Main category: cs.CL

TL;DR: 该研究引入PSP数据集来探究LLMs拒绝回答行为背后的政治审查因素，发现大多数LLMs都存在某种形式的审查机制


<details>
  <summary>Details</summary>
Motivation: 当前缺乏对LLMs拒绝回答有害内容行为背后动机的系统分析，难以区分是安全政策还是政治审查，需要专门的数据集来探究这一问题

Method: 构建PSP数据集，包含中国敏感提示扩展到多国和各国被审查的推文；采用数据驱动方法（使PSP隐含化）和表示层面方法（消除政治概念）；使用提示注入攻击测试模型脆弱性

Result: 大多数LLMs都存在某种形式的审查机制；通过将审查与对隐含意图内容的拒绝关联，揭示了模型的政治敏感性；总结了导致不同国家和模型间拒绝分布变化的主要属性

Conclusion: LLMs的拒绝行为确实包含政治审查成分，需要更透明地理解模型决策背后的动机，PSP数据集为研究这一现象提供了有效工具
...
### [272] [Listwise Preference Optimization with Element-wise Confusions for Aspect Sentiment Quad Prediction](https://arxiv.org/abs/2511.23184)
*Wenna Lai,Haoran Xie,Guandong Xu,Qing Li,S. Joe Qin*

Main category: cs.CL

TL;DR: 该论文提出了一种基于推理生成的ASQP方法，通过统一模板输出四元组和自然语言解释，并引入列表式偏好优化框架来提升结构有效性和关系一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于标记预测的方法难以建模元素间的复杂关系，且在标准监督微调下预测高阶元素（如类别和情感极性）时性能急剧下降，需要改进ASQP任务的性能。

Method: 1) 采用基于推理的生成方法，在统一模板中使用元素前缀输出四元组和自然语言解释；2) 引入列表式偏好优化框架，通过句法和语义邻近生成元素级混淆候选，训练模型优先选择正确候选。

Result: 在四个基准数据集上的广泛实验表明，该框架有效提高了四元组预测准确性和解释一致性。

Conclusion: 提出的推理生成方法和列表式偏好优化框架能够有效解决ASQP任务中的关系建模问题，提升预测性能和可解释性。
...
### [273] [TWEO: Transformers Without Extreme Outliers Enables FP8 Training And Quantization For Dummies](https://arxiv.org/abs/2511.23225)
...
Method: 论文证明极端异常值是训练过程中机械产生的、与数据无关的产物，源于权重矩阵的特定结构特性（即共线性）。基于这一洞察，提出了TWEO（Transformers Without Extreme Outliers），这是一种新颖的非侵入性损失函数。TWEO通过一个非常简单的损失项有效防止极端异常值，将异常值从10000+减少到小于20。

Result: 当标准FP8训练灾难性崩溃时，TWEO实现了与BF16基线相当的性能，同时训练吞吐量提升了36%。此外，TWEO实现了一种新的量化范式：硬件友好的W8A8每张量静态量化在LLM中首次达到SOTA性能，而之前由于异常值问题被认为完全不可用。

Conclusion: TWEO通过简单的损失函数项消除训练中的极端异常值，实现了无需工程技巧或架构修改的FP8全模型预训练，显著提升了训练效率，并开辟了新的量化可能性，为高效Transformer训练提供了创新解决方案。
...
### [274] [Tourism Question Answer System in Indian Language using Domain-Adapted Foundation Models](https://arxiv.org/abs/2511.23235)
*Praveen Gatla,Anushka,Nikita Kanwar,Gouri Sahoo,Rajesh Kumar Mundotiya*

Main category: cs.CL

TL;DR: 该研究首次为印地语旅游领域（特别是瓦拉纳西）设计了抽取式问答系统基线，构建了7,715个QA对的数据集并通过Llama扩展至27,455对，使用BERT和RoBERTa模型结合SFT和LoRA微调，在参数效率与性能间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 针对印地语旅游领域缺乏语言特定和文化敏感问答资源的现状，特别是在瓦拉纳西这样的文化精神中心，需要建立专门的问答系统来支持旅游信息查询。

Method: 构建了包含7,715个印地语QA对的瓦拉纳西旅游数据集，通过Llama零样本提示扩展至27,455对；使用基础模型BERT和RoBERTa，采用监督微调（SFT）和低秩适应（LoRA）两种微调策略；评估了多种BERT变体在低资源领域特定QA任务中的适用性。

Result: LoRA微调在减少98%可训练参数的同时达到85.3%的F1分数，在效率与准确性间取得良好平衡；RoBERTa结合SFT在捕捉文化嵌入术语（如Aarti、Kund）的上下文细微差别方面表现最佳；F1、BLEU和ROUGE-L指标揭示了答案精确性与语言流畅性之间的权衡。

Conclusion: 该研究为印地语旅游问答系统建立了基础基线，强调了LoRA在低资源环境中的重要作用，并指出旅游领域需要文化情境化的NLP框架，为后续研究提供了重要参考。
...
### [275] [Every Token Counts: Generalizing 16M Ultra-Long Context in Large Language Models](https://arxiv.org/abs/2511.23319)
*Xiang Hu,Zhanchao Zhou,Ruiqi Liang,Zehuan Li,Wei Wu,Jianguo Li*

Main category: cs.CL

TL;DR: 本文提出了一种名为HSA-UltraLong的8B参数MoE模型，通过分层稀疏注意力机制实现超长上下文建模，在16M长度的上下文中保持90%以上的检索准确率。
...
Method: 提出分层稀疏注意力机制，并将其集成到Transformer中构建HSA-UltraLong模型，这是一个8B参数的MoE模型，在超过8万亿token上进行训练。

Result: 模型在域内长度任务上与全注意力基线表现相当，在长达16M的上下文检索任务中保持90%以上的准确率，展示了处理超长上下文的能力。

Conclusion: HSA-UltraLong为超长上下文建模提供了基础框架，解决了稀疏性、随机访问和长度泛化问题，为未来研究奠定了基础。
...
### [276] [Scaling HuBERT for African Languages: From Base to Large and XL](https://arxiv.org/abs/2511.23370)
*Antoine Caubrière,Elodie Gauthier*

Main category: cs.CL

TL;DR: 该研究针对非洲语言在语音处理中代表性不足的问题，开发了首个完全基于非洲语音训练的大规模模型SSA-HuBERT-Large和SSA-HuBERT-XL，并通过实验证明更大模型在低资源监督下能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言语音处理取得进展，非洲语言在研究和部署系统中仍然代表性不足，特别是在需要强大、开放权重的编码器方面。现有针对非洲语音的公开模型大多停留在BASE规模，缺乏关于更大规模编码器在非洲中心音频训练下实际效益的研究。

Method: 开发了SSA-HuBERT-Large（3.17亿参数）和SSA-HuBERT-XL（9.64亿参数）两个大规模模型，完全基于非洲语音训练，同时提供BASE规模对应模型。通过严格控制实验，专注于撒哈拉以南非洲语言，覆盖自动语音识别和语言识别任务。

Result: 实验表明，更大的架构通过有效利用大型音频数据集，显著提升了性能。模型已作为开放权重发布在Hugging Face平台上。

Conclusion: 这项研究填补了非洲语音处理领域的空白，证明在非洲中心音频上训练的大规模编码器在低资源监督下具有显著优势，为非洲语言语音处理提供了重要的基础模型资源。
...
### [277] [Optimizing Multimodal Language Models through Attention-based Interpretability](https://arxiv.org/abs/2511.23375)
*Alexander Sergeev,Evgeny Kotelnikov*

Main category: cs.CL

TL;DR: 提出基于注意力机制的MLM可解释性方法，通过分析注意力头对图像关键对象的关注度来选择PEFT组件，在图像描述任务中验证有效性


<details>
  <summary>Details</summary>
Motivation: 多模态语言模型（MLMs）难以解释，难以确定哪些组件对训练最有效以平衡效率和性能。现有PEFT方法虽然参数高效，但缺乏针对MLMs的组件选择指导

Method: 提出基于注意力的可解释性方法：1）通过分析注意力分数相对于图像令牌来识别关注图像关键对象的注意力头；2）计算Head Impact（HI）分数量化注意力头对关键对象的关注度；3）应用HI分数选择PEFT组件进行多模态模型微调

Result: 实验验证：1）微调具有最高HI分数的层相比预训练、随机选择或最低HI分数层，在指标上产生最显著变化；2）仅微调这些关键层中约0.01%的参数即可显著影响图像理解能力；3）创建了包含图像、关键对象掩码和文本描述的新数据集

Conclusion: 提出的注意力可解释性方法能有效识别MLMs中对图像理解关键的组件，为PEFT提供指导，实现参数高效且性能优化的多模态模型微调
...
### [278] [Ambiguity Awareness Optimization: Towards Semantic Disambiguation for Direct Preference Optimization](https://arxiv.org/abs/2511.23391)
...
Conclusion: AAO通过处理偏好对中的模糊内容有效减少训练歧义，显著提升DPO性能，且不显著增加响应长度
