...
TL;DR: IndicVisionBench是首个以印度次大陆为中心的大规模多模态基准测试，涵盖英语和10种印度语言，包含OCR、MMT和VQA等3个任务，约5K图像和37K+问答对，评估了8个模型在文化多样性环境中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前大多数视觉语言模型评估基准都是西方中心的，缺乏对文化多样性和多语言环境的测试，需要建立更包容的评估框架。

Method: 构建IndicVisionBench基准，涵盖13个文化相关主题，包含OCR、多模态机器翻译和视觉问答3个任务，6种问题类型，创建10种印度语言的并行标注语料库。

Result: 评估8个模型（从专有闭源系统到开源中大规模模型）发现存在显著的性能差距，揭示了当前VLMs在文化多样性环境中的局限性。
...
TL;DR: 本文提出了Condition Preference Optimization (CPO)方法，通过优化控制条件而非生成图像来解决文本到图像生成中的可控性问题，相比ControlNet++和DPO方法在计算效率和性能上都有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法如ControlNet++在优化时只关注低噪声时间步，忽略了高噪声时间步的贡献并引入近似误差；而DPO方法难以确保图像对仅在可控性上存在差异。

Method: 提出Condition Preference Optimization (CPO)，通过构建获胜和失败的控制信号对，训练模型偏好获胜控制信号，消除混杂因素并降低训练目标方差。

Result: CPO在多个控制类型上显著提升可控性：分割任务错误率降低超过10%，人体姿态任务降低70-80%，边缘和深度图任务稳定降低2-5%。

Conclusion: CPO方法在理论上具有比DPO更低的对比损失方差，实验证明其在可控性和计算效率方面优于现有最先进方法。
...
TL;DR: DARN是一种新颖的解码器架构，通过动态自适应正则化解决卫星图像异质性挑战，在完整微调和高效适应两种范式下都实现了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 基础模型为地理空间分析提供了强大表示，但标准适应方法采用固定正则化策略，无法处理卫星图像的显著异质性。

Method: DARN集成了三个关键创新：轻量级任务复杂度预测器、基于预测复杂度的自适应dropout调制（0.1-0.5）、动态容量门控。

Result: 在完整微调下，DARN在GeoBench基准上达到86.66% mIoU（比之前SOTA提升5.56pp）；在高效适应下，在Sen1Floods11上达到90.5% mIoU，并在OOD泛化、鲁棒性和少数类性能方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 热带气旋(TC)准确预报面临挑战，主要由于卫星观测有限且难以解析TC增强相关的云属性。现有机器学习方法局限于TC罕见区域，对强风暴验证不足。
...
Conclusion: 该方法对推进TC增强理解和改进预报至关重要，为TC研究提供了新的3D观测能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D识别任务主要使用隐式表示如PointNet，但显式几何表示在3D重建领域已显示出优势。本文旨在开发基于3D高斯的显式表示，以提升计算效率和性能。

Method: 开发了基于自然梯度和从PointNet蒸馏的优化技术，学习能够重建PointNet激活的高斯基。并扩展了3D高斯泼溅中的滤波技术来加速编码器。

Result: 3D高斯点编码器比可比精度的PointNet快2.7倍，内存减少46%，FLOPs减少88%。在Mamba3D中运行时快1.27倍，内存和FLOPs分别减少42%和54%。

Conclusion: 3D高斯点编码器提供了一种轻量级的显式几何表示，能够在仅CPU设备上实现高帧率，为3D识别任务提供了高效的计算解决方案。
...
TL;DR: 本研究以Cellpose为例，系统分析了生物医学图像分割中的两个关键挑战：训练数据冗余和跨域迁移中的灾难性遗忘问题。研究发现仅需10%数据即可达到性能饱和，并提出数据集量化策略和选择性重放方法来解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 当前通用生物医学图像分割模型在多样化成像模态和细胞类型中广泛应用，但训练数据冗余程度和跨域迁移对模型保留的影响尚未得到充分研究。

Method: 提出数据集量化(DQ)策略构建紧凑多样的训练子集；进行跨域微调实验评估灾难性遗忘；使用选择性DQ重放方法恢复源域性能；分析训练域排序对泛化的影响。

Result: 实验显示图像分割性能在仅使用10%数据时达到饱和；跨域微调导致源域性能显著下降；选择性重放5-10%源数据可有效恢复源性能；训练域排序可改善泛化并减少遗忘。
...
TL;DR: 提出了一种基于表面法向量先验知识的几何去噪新范式，使用标签向量集作为先验，在去噪过程中自然嵌入分割问题，并通过总变差项实现正则化。


<details>
  <summary>Details</summary>
Motivation: 利用表面法向量的先验知识来改进几何去噪效果，通过标签向量集提供几何结构的先验信息。

Method: 采用分裂Bregman（ADMM）方法求解优化问题，顶点更新步骤基于二阶形状计算，通过法向量与标签向量的相似性进行分割。

Result: 该方法在几何去噪过程中实现了基于法向量的自然分割，并通过总变差正则化保持几何结构的平滑性。

Conclusion: 提出的新范式成功地将法向量先验知识融入几何去噪，实现了去噪与分割的联合优化，为几何处理提供了新思路。
...
TL;DR: 提出了一种隐式自先验方法，直接从输入点云中提取形状特定的几何先验，通过隐式神经表示和可学习字典来捕获重复结构和长程相关性，结合RIMLS重建高质量表面。


<details>
  <summary>Details</summary>
Motivation: 从不规则点云恢复高质量表面是病态问题，需要强几何先验。现有方法依赖外部训练数据，本文旨在直接从输入点云本身提取先验知识。

Method: 联合训练可学习嵌入字典与隐式距离场，通过交叉注意力机制捕获形状的重复结构和相关性；使用自监督点云重建损失优化；结合RIMLS进行表面重建。

Result: 实验表明该方法在生成高保真表面方面优于经典和基于学习的方法，具有更好的细节保留能力和对数据退化的鲁棒性。

Conclusion: 提出的隐式自先验方法无需外部训练数据，能有效从点云中提取几何先验，结合RIMLS实现高质量表面重建，在细节保留和鲁棒性方面表现优异。
...
TL;DR: 本文提出了Clinical-ComBAT方法，用于解决DW-MRI数据在多站点采集时的扫描仪特异性偏差问题，相比传统ComBAT方法更适合临床实际应用场景。


<details>
  <summary>Details</summary>
Motivation: 传统ComBAT方法在临床应用中存在局限性，包括依赖线性协变量关系、同质人群、固定站点数量和充足样本量等假设，这些限制使其难以适应真实临床环境。

Method: Clinical-ComBAT采用非线性多项式数据模型，支持独立站点协调，以规范站点为参考进行站点特异性协调，并包含可适应小样本的方差先验、超参数调优和协调质量评估指标。

Result: 在模拟和真实数据上的实验表明，该方法能有效改善扩散指标的协调性，并增强了规范建模的适用性。

Conclusion: Clinical-ComBAT方法克服了传统方法的限制，为多站点DW-MRI数据协调提供了更灵活、实用的解决方案，特别适合临床环境应用。
...
TL;DR: 本文提出了一个包含8775次牙科检查的多模态数据集，涵盖50000张口内图像、8056张X光片和详细文本记录。通过微调Qwen-VL模型在口腔异常分类和诊断报告生成任务上取得显著提升，验证了数据集对AI牙科研究的价值。


<details>
  <summary>Details</summary>
Motivation: AI在口腔医疗领域的发展需要大规模多模态数据集来反映临床实践的复杂性，但目前缺乏这样的公开资源。

Method: 收集了2018-2025年间4800名患者的8775次牙科检查数据，包括图像和文本记录。微调Qwen-VL 3B和7B模型，在口腔异常分类和诊断报告生成任务上与基准模型和GPT-4o进行比较。

Result: 微调后的模型在口腔异常分类和诊断报告生成任务上相比基准模型和GPT-4o取得了显著提升，验证了数据集的有效性。

Conclusion: 该公开数据集为AI牙科研究提供了重要资源，能够有效推动AI驱动的口腔医疗解决方案的发展。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展导致深度伪造日益逼真，给执法和公众信任带来挑战。现有被动检测器依赖特定伪造伪影，难以泛化到新型深度伪造。主动水印检测方法需要在抗良性失真和恶意篡改敏感性之间取得平衡。

Method: 开发了在潜在空间中操作的可学习水印嵌入器，捕获高级图像语义，同时提供对消息编码和提取的精确控制。采用多智能体对抗强化学习范式，使水印智能体通过与对抗攻击者智能体模拟的良性及恶意图像操作交互，寻求鲁棒性和脆弱性的最优平衡。

Result: 在CelebA和CelebA-HQ基准上的全面评估表明，该方法始终优于最先进方法，在具有挑战性的操作场景下，在CelebA上提升超过4.5%，在CelebA-HQ上提升超过5.3%。

Conclusion: 所提出的框架通过结合高维潜在空间表示和多智能体对抗强化学习，成功开发了鲁棒且自适应的水印方法，为深度伪造检测提供了有效解决方案。
...
TL;DR: CLM系统通过将高斯数据卸载到CPU内存，仅在需要时加载到GPU，使3D高斯溅射(3DGS)能够在单个消费级GPU上渲染大型场景。


<details>
  <summary>Details</summary>
Motivation: 3DGS在渲染大型或复杂场景时面临GPU内存不足的挑战，需要解决内存限制问题。

Method: 采用创新的卸载策略，利用3DGS内存访问模式进行流水线处理，重叠GPU-CPU通信、GPU计算和CPU计算，同时减少通信量。

Result: 在单个RTX4090上成功渲染需要1亿个高斯的大型场景，并达到最先进的重建质量。

Conclusion: CLM系统有效解决了3DGS在大场景渲染中的内存瓶颈问题，实现了在消费级硬件上的高效运行。
...
TL;DR: 本文提出了贴纸语义相似度任务的定义，创建了首个基准数据集Triple-S，并开发了轻量级通用贴纸编码器GSE，在贴纸语义理解和下游任务上表现出色。
...
Method: 定义了贴纸语义相似度任务，构建了包含905个人工标注贴纸对的Triple-S基准数据集，并提出了通用贴纸编码器GSE，使用Triple-S和额外数据集学习鲁棒的贴纸嵌入。

Result: GSE在未见过的贴纸上实现了优越性能，在情感分类和贴纸检索等下游任务上表现出强劲结果。
...
TL;DR: 本文发现主流LVLM架构存在语言模态偏见，提出通过平均池化视觉特征来优化文本嵌入的方法，显著改善了视觉定位并减少了幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 识别当前LVLM架构中存在的语言模态偏见问题，这种偏见主要源于将视觉嵌入简单附加到输入文本序列的常见做法。

Method: 提出一种简单有效的方法，通过整合平均池化的视觉特征来优化文本嵌入，从而改善视觉信息在模型中的表示。

Result: 该方法在已有基准测试中显著改善了视觉定位能力，并大幅减少了幻觉现象。

Conclusion: 虽然平均池化提供了一种简单、鲁棒且高效的视觉信息整合方式，但更复杂的融合方法可能进一步改善视觉定位和跨模态对齐，这留待未来研究。
...
TL;DR: 提出了一种动态残差编码与切片级对比学习（DRE-SLCL）方法，用于端到端的全切片图像表示学习，解决了GPU内存限制下无法同时处理数万个图像块的问题。
...
Method: 使用内存库存储数据集中所有WSI的块特征，训练时随机采样部分块计算特征，同时从内存库中选择同一WSI的额外块特征，通过残差编码技术结合采样特征和检索特征生成WSI表示，最后基于小批量内WSI的表示和病理报告计算切片级对比损失。

Result: 在癌症亚型分类、癌症识别和突变预测任务上的实验证明了所提出的DRE-SLCL方法的有效性。

Conclusion: DRE-SLCL方法通过动态残差编码和切片级对比学习，成功解决了WSI端到端表示学习中的计算挑战，在多个癌症相关任务上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统动作捕捉需要专业设备的问题，开发一种基于压力数据和文本提示的隐私保护、低成本运动生成方法。

Method: 使用双级特征提取器解析压力数据，结合分层扩散模型，利用物理线索和语义指导来生成精确的运动。
...
TL;DR: NTP-MRISeg将医学参考图像分割重新定义为基于统一多模态序列的自回归下一标记预测任务，通过简化模型设计、消除模态特定融合需求，并引入三种新策略来解决累积预测误差、长尾标记分布和细粒度病变边缘等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有医学参考图像分割方法通常涉及复杂的多模态融合或多阶段解码器设计，作者旨在简化模型架构，实现端到端训练，并利用新兴大规模多模态模型的预训练分词器来增强泛化能力。

Method: 提出NTP-MRISeg框架，将MRIS重新定义为自回归下一标记预测任务；引入三种新策略：(1)下一k标记预测方案减少累积误差，(2)标记级对比学习增强边界敏感性和缓解长尾分布，(3)基于记忆的硬错误标记优化策略在训练中强调困难标记。

Result: 在QaTa-COV19和MosMedData+数据集上的广泛实验表明，NTP-MRISeg实现了新的最先进性能，为传统MRIS流程提供了简化且有效的替代方案。

Conclusion: NTP-MRISeg通过重新制定MRIS任务为自回归下一标记预测，简化了模型设计，支持端到端训练，并在多个数据集上取得了最先进的性能，为医学图像分割提供了更高效的解决方案。
...
TL;DR: Role-SynthCLIP是一个利用多视角角色扮演提示来增强CLIP模型训练数据语义多样性的框架，通过MLLMs生成多样化描述，在保持数据量不变的情况下显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据生成方法主要关注增加数据量，但往往导致语义多样性有限和描述冗余浅显，限制了CLIP模型的效果。

Method: 提出Role-SynthCLIP框架，使用多视角角色扮演提示（如组合分析师、图像语境解释者）指导MLLMs从不同角度生成语义多样的描述，增强图像-文本对齐的细粒度性。

Result: 仅使用100万Role-SynthCLIP对训练的CLIP-B/16模型在MS COCO验证集上达到64.1%的Recall@1，比现有最佳合成数据基线（使用500万对）高出2.8个百分点。

Conclusion: 该方法通过提升合成数据的语义多样性而非单纯增加数据量，有效提高了CLIP模型的性能，证明了语义质量对模型训练的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于合成数据训练的视觉模型难以泛化到复杂的天气退化场景，恶劣天气严重影响了真实世界的视觉感知性能。

Method: 构建HFLS-Weather物理驱动的高保真数据集，设计双层次强化学习框架：局部层次通过扰动驱动的图像质量优化精炼天气特定恢复模型；全局层次使用元控制器根据场景退化动态协调模型选择和执行顺序。

Result: 该框架能够持续适应真实世界条件，在广泛的恶劣天气场景中实现了最先进的性能。

Conclusion: 提出的双层次强化学习框架有效解决了恶劣天气下视觉感知的泛化问题，通过物理驱动的数据集和动态模型协调机制，显著提升了模型在复杂天气退化条件下的性能。
...
TL;DR: 提出一种在雪覆盖环境中通过检测路边标柱来间接识别车道的方法，使用贝塞尔曲线拟合车道轨迹，并发布了包含8万帧标注数据的SnowyLane合成数据集。


<details>
  <summary>Details</summary>
Motivation: 解决雪覆盖环境下车道标记经常缺失或被遮挡的问题，为冬季自动驾驶提供可靠的车道检测方案。

Method: 通过检测路边垂直标柱作为间接车道指示器，利用参数化贝塞尔曲线模型拟合平滑车道轨迹，基于空间一致性和道路几何特征。
...
TL;DR: JWTH模型通过整合大规模自监督预训练与细胞中心的后调优和注意力池化，融合局部和全局标记，在数字病理学中实现更准确和可解释的AI生物标志物检测。
...
TL;DR: 提出了一种将可变形高斯泼溅技术分为前景和背景组件的方法，用于解决稀疏相机配置下的动态3D重建问题，在保持模型紧凑的同时提高了重建质量。


<details>
  <summary>Details</summary>
Motivation: 在电影制作中，预算限制导致稀疏相机配置，现有方法难以捕捉复杂的动态特征，需要一种能在稀疏视图下有效重建动态场景的方法。

Method: 将规范高斯泼溅和变形场分为前景和背景组件，使用t=0帧的稀疏掩码进行分离，分别在不同损失函数下训练，前景学习颜色、位置和旋转变化，背景仅学习位置变化。

Result: 在3D和2.5D娱乐数据集上取得SotA结果，PSNR提高达3分，模型大小减半，并能生成包含透明和动态纹理的分割动态重建结果。
...
<details>
  <summary>Details</summary>
Motivation: 解决DINOv3模型在高分辨率图像处理时计算复杂度高和资源消耗大的问题，因为其transformer架构的平方复杂度导致高分辨率特征图计算成本高昂。

Method: 提出BRIXEL知识蒸馏方法，让学生模型学习在更高分辨率下重现自身特征图，从而降低计算成本。

Result: BRIXEL在固定分辨率的下游任务中大幅超越基线DINOv3模型，能够以较低计算成本产生与教师模型非常相似的特征图。

Conclusion: BRIXEL通过简单的知识蒸馏方法有效解决了高分辨率特征图计算成本高的问题，在保持性能的同时显著降低了计算开销。
...
TL;DR: MUSE是一种针对组织病理学中细胞核检测与分类的自监督学习方法，通过多尺度密集自蒸馏和基于细胞核位置的局部自蒸馏机制，有效利用大规模未标记数据，在多个基准测试中超越现有监督方法和通用病理学基础模型。
...
Result: 在三个广泛使用的基准测试上的实验表明，MUSE有效解决了组织病理学细胞核检测与分类的核心挑战。

Conclusion: MUSE模型不仅超越了最先进的监督基线，还优于通用的病理学基础模型，为组织病理学分析提供了有效的自监督学习解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩展原始WtL算法的应用范围，使其不仅适用于彩色图像中的船舶分割，还能处理红外图像中的船舶分割以及RGB图像中的各种对象，满足需要详细分割或高质量样本的专业应用需求。

Method: 通过轮廓跟踪算法替代标准非极大值抑制，将对象轮廓细化为1像素宽的闭合形状，形成可分割的前景-背景区域。针对红外分割，适配了对象轮廓检测器的输入；针对RGB对象，增强了算法处理能力。

Result: WtL2在实现闭合对象轮廓方面优于最新的基于轮廓的方法，提供了高交并比和令人印象深刻的细节，在红外船舶分割和RGB对象分割中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法如ControlNet依赖手工条件图和重训练，灵活性有限；基于反转的方法虽然对齐效果好但推理成本高。需要一种无需训练、高效的结构控制方法。

Method: 1. 单步注意力提取：从最优关键时间步提取注意力并在整个去噪过程中复用；2. 潜在条件解耦(LCD)：分离关键时间步和噪声潜在表示，提供更精细的注意力质量控制；3. 支持组合控制：通过多源参考图像实现直观场景布局设计。

Result: FreeControl实现了结构和语义对齐的视觉连贯生成，直接从原始图像生成，支持直观的组合设计，与现代扩散模型兼容，仅增加约5%的额外成本。

Conclusion: FreeControl为测试时控制引入了新范式，无需反转或重训练即可实现高效的结构控制，在保持视觉质量的同时显著降低了计算成本。
...
TL;DR: 4D3R是一个无需相机位姿的动态神经渲染框架，通过两阶段方法分离静态和动态组件，在动态场景新视角合成方面取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 解决单目视频中动态场景的新视角合成问题，特别是在未知相机位姿的情况下，现有方法如NeRF和3DGS对动态内容处理不佳且依赖预计算位姿。

Method: 采用两阶段方法：首先利用3D基础模型进行初始位姿和几何估计，然后进行运动感知细化。关键技术包括运动感知束调整模块和高效的运动感知高斯溅射表示。

Result: 在真实世界动态数据集上的实验表明，该方法比最先进方法PSNR提升高达1.8dB，在具有大动态对象的挑战性场景中表现尤其突出，同时计算需求比先前动态场景表示减少5倍。

Conclusion: 4D3R框架在动态场景新视角合成方面实现了显著的性能提升和计算效率改进，特别是在处理大动态对象和未知相机位姿的情况下。
...
TL;DR: 本文介绍了OregairuChar数据集，这是一个专门用于分析动漫《我的青春恋爱物语果然有问题》中角色出现频率的基准数据集，包含1600个手动选择的帧和2860个边界框标注，涵盖11个主要角色。
...
Method: 创建了包含1600个手动选择帧的数据集，标注了2860个边界框，涵盖11个主要角色，并包含遮挡、姿态变化和角色间相似性等视觉挑战。在数据集上对多个目标检测模型进行基准测试，并利用预测结果进行细粒度的集级角色存在分析。

Result: 该数据集能够捕捉角色突出程度的模式及其在叙事中的演变，为计算叙事动态和角色中心叙事研究提供了有价值的资源。

Conclusion: OregairuChar数据集通过强调角色出现频率，为探索风格化媒体中的计算叙事动态和角色中心叙事提供了重要工具。
...
TL;DR: 该研究开发了一种基于3D U-Net的自动分割方法，用于结直肠肝转移瘤的术中超声图像分割，通过裁剪区域训练显著提升了分割精度，实现了近实时的手术导航。


<details>
  <summary>Details</summary>
Motivation: 术中准确描绘结直肠肝转移瘤对于实现阴性切除边缘至关重要，但由于术中超声图像对比度低、噪声大且依赖操作者，传统方法面临挑战，需要自动分割技术来提高超声导航的精度和效率。

Method: 使用85例患者的追踪3D术中超声数据，通过nnU-Net框架实现3D U-Net，比较了完整体积和裁剪肿瘤区域两种训练变体，评估指标包括Dice相似系数、Hausdorff距离和相对体积差异。

Result: 裁剪体积模型在所有指标上显著优于完整体积模型（AUC-ROC = 0.898 vs 0.718），中位DSC=0.74，召回率=0.79，Hausdorff距离=17.1mm，与半自动分割相当但速度快约4倍（约1分钟）。

Conclusion: 基于裁剪3D U-Net的自动分割方法为肝转移瘤术中超声提供了可靠、近实时的分割结果，无需配准即可实现高效的超声导航，接近专家级精度同时显著减少人工工作量和手术时间。
...
TL;DR: LiveStar是一个开创性的实时流媒体助手，通过自适应流式解码实现持续主动响应，解决了现有在线视频大语言模型在实时响应性和叙事连贯性方面的不足。
...
TL;DR: 本文揭示了基于Chamfer Distance的常用点云生成评估指标存在鲁棒性问题，提出了引入样本对齐和使用Density-Aware Chamfer Distance的改进方法，并提出了新的Surface Normal Concordance指标。同时提出了Diffusion Point Transformer架构，在ShapeNet数据集上实现了最先进的生成质量。
...
Method: 1) 在距离计算前引入样本对齐；2) 用Density-Aware Chamfer Distance替代传统CD；3) 提出Surface Normal Concordance指标，通过比较估计点法向量来近似表面相似性；4) 基于transformer的点云分析模型，提出Diffusion Point Transformer架构用于生成高保真3D结构。

Result: 在ShapeNet数据集上的广泛实验表明，提出的方法在生成点云质量方面优于先前解决方案，达到了新的最先进水平。

Conclusion: 通过改进评估指标和提出新的生成架构，本文为点云生成提供了更全面可靠的评估框架和更高质量的生成方法，推动了3D点云生成技术的发展。
...
Result: 在包含11种食物类别和10名参与者的27.5小时IMU记录数据集上，CuisineSense在进食状态检测和食物分类方面都取得了高精度。
...
Conclusion: 该系统能够有效简化装配流程，减少人工干预，提高装配效率和准确性。
...
TL;DR: MPRF是一个多模态闭环检测管道，结合视觉和LiDAR两种模态，在非结构化环境中实现稳健的闭环检测和6-DoF位姿估计。
...
Method: 采用基于transformer的基础模型，结合两阶段视觉检索策略（DINOv2特征+SALAD聚合）进行候选筛选，以及SONATA-based LiDAR描述符进行几何验证，实现显式6-DoF位姿估计。
...
<details>
  <summary>Details</summary>
Motivation: 现有时序搜索方法通常依赖手工设计的搜索过程，缺乏对学习最优搜索策略的端到端优化。

Method: 提出TimeSearch-R框架，将时序搜索重新定义为交错式文本-视频思考，使用强化学习（特别是GRPO-CSV方法）来优化搜索策略，并通过完整性自验证机制确保视频内容的充分探索。

Result: 在多个时序搜索基准测试（Haystack-LVBench、Haystack-Ego4D）和长视频理解基准测试（VideoMME、MLVU）上取得显著提升，在LongVideoBench上比基础模型Qwen2.5-VL提升4.1%，比先进视频推理模型Video-R1提升2.0%。

Conclusion: TimeSearch-R通过强化学习和完整性自验证机制，有效提升了时序搜索的性能，在多个基准测试上达到了新的最先进水平。
...
<details>
  <summary>Details</summary>
Motivation: 在AIGC时代，隐写术的容量需求日益重要，但现有方法难以嵌入语义丰富的句子级信息。本文旨在解决这一挑战，实现任意句子级消息在图像中的隐藏。

Method: 提出了S²LM语义隐写语言模型，利用大语言模型在整个过程中嵌入高级文本信息，设计了新的处理流程，将LLM融入隐写全过程。

Result: 定量和定性实验表明，该方法有效解锁了大语言模型的新语义隐写能力，能够成功隐藏句子甚至段落级别的语义内容。

Conclusion: S²LM方法成功实现了句子到图像的语义隐写，为隐写术开辟了新的研究方向，能够处理更丰富的语义信息。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D人体运动与语言整合的研究主要集中在文本到运动生成，而运动理解任务相对未被充分探索。现有数据集缺乏详细的时间标注且多为短序列。

Method: 构建了包含60,000个复杂运动序列的CompMo数据集，每个序列包含2-10个动作并带有精确时间边界标注。提出了DEMO模型，将大语言模型与简单运动适配器相结合。

Result: DEMO模型在CompMo数据集和适应基准测试中显著优于现有方法，为3D运动理解和描述建立了强大的基线。

Conclusion: 该研究填补了3D运动理解领域的空白，通过CompMo数据集和DEMO模型为未来研究提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视觉质量评估中主要依赖监督微调或仅排名目标，导致推理浅层、分数校准差和跨域泛化有限。

Method: 提出双分支奖励公式，分别建模样本内响应一致性和样本间偏好对齐，通过组相对策略优化进行优化，并设计了视频质量评估的全局-时间和局部-空间数据流策略。
...
TL;DR: PALM是一个大规模手部数据集，包含263名受试者的13,000个高质量手部扫描和90,000张多视角图像，用于解决手部建模的挑战。作者还提出了PALM-Net基线模型，通过基于物理的逆向渲染学习手部几何和材质属性的多主体先验，实现真实、可重照明的单图像手部化身个性化。


<details>
  <summary>Details</summary>
Motivation: 当前从图像创建高质量个性化手部化身面临挑战，主要由于复杂的几何、外观和关节结构，特别是在无约束光照和有限视角下。此外，缺乏同时提供准确3D几何、高分辨率多视角图像和多样化受试者群体的数据集也限制了进展。

Method: 提出了PALM大规模数据集，包含13,000个高质量手部扫描和90,000张多视角图像，涵盖263名受试者。基于此开发了PALM-Net基线模型，通过基于物理的逆向渲染学习多主体手部几何和材质属性先验。

Result: PALM数据集捕获了肤色、年龄和几何形状的丰富变化。PALM-Net能够实现真实、可重照明的单图像手部化身个性化，展示了该数据集的实用性。

Conclusion: PALM的规模和多样性使其成为手部建模及相关研究的宝贵现实世界资源，为解决手部化身创建中的挑战提供了重要基础。
...
TL;DR: 提出了一种用于图像修复的持续学习方法，通过简单的卷积层修改来适应新任务，无需修改主干架构，显著减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法在图像修复领域存在挑战，需要大量工程化架构修改，计算开销大。正则化方法不适用于修复任务，因为不同修复问题需要不同的特征处理。

Method: 提出对卷积层进行简单修改，从先前修复任务中适应知识，无需触及主干架构。可以无缝应用于任何深度架构而无需结构修改。

Result: 实验验证表明，新修复任务可以在不影响现有任务性能的情况下引入。新任务的性能通过适应先前任务创建的知识库得到提升，且可增加可训练参数而不会显著增加计算开销或推理时间。

Conclusion: 该方法为图像修复领域的持续学习提供了一种简单有效的解决方案，能够在不修改主干架构的情况下适应新任务，同时保持现有任务的性能。
...
TL;DR: 提出基于HierSpeech++潜在语音表示的文本到说话人脸合成框架，通过Text-to-Vec模块从文本生成Wav2Vec2嵌入，联合调节语音和面部生成。采用两阶段训练处理特征分布偏移，实现紧密的视听对齐，保持说话人身份，在推理时无需真实音频即可生成自然的语音和同步的面部动作。


<details>
  <summary>Details</summary>
Motivation: 解决文本到说话人脸合成中音频和视觉生成的一致性问题，避免级联流水线中的误差累积，实现更好的唇部同步和视觉真实感。

Method: 使用Text-to-Vec模块生成Wav2Vec2嵌入作为条件信号，采用两阶段训练策略：先在Wav2Vec2嵌入上预训练，然后在TTS输出上微调，以处理干净特征和TTS预测特征之间的分布偏移。

Result: 实验表明，基于TTS预测的潜在特征条件化优于级联流水线，在唇部同步和视觉真实感方面均有提升，能够生成自然的语音表达和同步的面部运动。

Conclusion: 所提出的框架通过联合条件化和两阶段训练策略，有效实现了文本到说话人脸的合成，在保持说话人身份的同时，显著提升了视听对齐质量和生成效果。
...
TL;DR: gitmerge3D是一种全局信息图标记合并方法，可将3D点云变换器中的标记数量减少90-95%，同时保持竞争性能，显著提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 当前3D点云变换器依赖密集标记表示，导致训练和推理时计算和内存成本高昂，研究发现标记存在显著冗余。

Method: 提出gitmerge3D方法，通过全局信息图标记合并来减少标记数量，挑战了更多标记必然带来更好性能的普遍假设。

Result: 在多个3D视觉任务中验证，标记数量减少90-95%的同时保持竞争性能，计算效率显著提升。

Conclusion: 这是首个评估大规模3D变换器模型中冗余性的工作，为开发更高效的3D基础架构提供了见解，许多当前模型存在过度标记化和可扩展性优化不足的问题。
...
TL;DR: 该研究探讨了使用中等分辨率（10米）的哥白尼计划地球观测图像进行建筑物损坏评估的可行性，并发布了xBD-S12数据集，包含10,315对灾前灾后图像对。
...
Method: 使用Sentinel-1和Sentinel-2的10米分辨率图像，构建xBD-S12数据集，并与现有xBD基准进行时空对齐，通过一系列实验验证损坏检测和映射效果。

Result: 实验表明，尽管只有10米分辨率，但在许多灾害场景中能够较好地检测和映射建筑物损坏；复杂模型架构在泛化到未见灾害时表现不佳，地理空间基础模型带来的实际效益有限。

Conclusion: 哥白尼图像是进行快速、广域损坏评估的可行数据源，可与超高分辨率图像一起发挥重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 利用照片中人脸信息进行年代估计，特别是当图像包含多个可识别个体时，多脸信息聚合能显著提升性能。

Method: 提出概率框架，结合现代人脸识别和年龄估计模型的视觉证据，以及基于职业的时间先验来推断照片拍摄年份。

Result: 多脸证据聚合能持续提升性能，该方法显著优于基于场景的基线方法，特别是在包含多个可识别个体的图像中。

Conclusion: 基于人脸信息的照片年代估计方法有效，多脸信息聚合是关键优势，特别是在识别多个个体的情况下表现更佳。
...
Method: 开发五种分类模型，比较传统图像数据和事件数据，使用神经形态传感器检测像素亮度变化，无需全帧重建。

Result: 基于事件数据的模型优于帧基方法，事件LSTM模型在准确性和速度间取得最佳平衡，准确率97.6%，处理时间0.28ms。
...
TL;DR: 本文提出了视觉空间调优(VST)框架，通过构建大规模数据集和渐进式训练方法，在不影响通用能力的前提下增强视觉语言模型的空间感知和推理能力。
...
Result: 在多个空间基准测试中取得SOTA结果，包括MMSI-Bench 34.8%和VSIBench 61.2%，且不影响模型的通用能力。
...
<details>
  <summary>Details</summary>
Motivation: 参与2024年综合医疗排班竞赛，解决医疗资源调度优化问题，提升排班效率和质量。

Method: 结合混合整数规划、约束规划和模拟退火的3阶段解决方案，基于子问题分解的方法。

Result: 在竞赛中获得第三名，首次提供了基准实例最优解值的下界。

Conclusion: 该方法有效解决了医疗排班问题，但仍有改进空间，指出了未来可优化的开放问题。
...
TL;DR: 本文提出了一种新的拒绝选项预测器，专门针对数据不足导致的认识不确定性进行拒绝预测，通过最小化期望遗憾来优化模型决策。


<details>
  <summary>Details</summary>
Motivation: 传统拒绝选项方法只关注随机不确定性，但在数据有限的实际场景中，认识不确定性变得不可忽视。需要一种能够识别训练数据不足区域的预测框架。

Method: 基于贝叶斯学习，重新定义最优预测器为最小化期望遗憾的模型。当输入数据的遗憾超过指定拒绝成本时，模型选择拒绝预测。

Result: 提出了第一个原则性框架，能够学习识别训练数据不足以做出可靠决策的输入。

Conclusion: 该框架为高风险应用中的不确定性量化提供了理论基础，特别是在数据有限的情况下能够有效管理认识不确定性。
...
TL;DR: DMA是一个在线学习框架，通过多粒度人类反馈动态调整RAG系统的检索排序，实现实时适应而不牺牲基础检索能力。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统依赖静态检索，难以适应动态意图和内容漂移，需要能够实时学习和适应的框架。

Method: DMA将文档级、列表级和响应级反馈信号整合到统一学习流程中，包括监督训练、策略优化和知识蒸馏，构建轻量级评分器。

Result: 在线部署显示显著提升用户参与度，离线测试在对话QA任务上表现优异，同时保持基础检索竞争力。

Conclusion: DMA为RAG系统提供了基于反馈的实时适应原则性方法，平衡了适应性和基础能力。
...
TL;DR: 本文提出了实时推理的新问题框架，研究在动态环境中智能体的推理能力。通过构建Real-Time Reasoning Gym平台，比较了反应式智能体和规划式智能体两种范式，并提出了同时采用两种范式的AgileThinker方法，在时间压力和任务难度增加时表现更优。
...
Method: 构建Real-Time Reasoning Gym平台，研究反应式智能体（有限推理计算）和规划式智能体（扩展推理计算）两种范式，并提出同时采用两种范式的AgileThinker方法。

Result: 实验表明，即使最先进的模型在两种范式中都难以同时做出逻辑性和及时性的判断。AgileThinker在任务难度和时间压力增加时持续优于单一范式的智能体，有效平衡了推理深度和响应延迟。

Conclusion: 实时推理是开发实用智能体的关键测试平台，为时间约束AI系统的研究提供了基础，指明了实现实时能力智能体的路径。
...
TL;DR: 本文提出了一种支持军事地面作战执行阶段决策的方法论，专注于机械化营的行动方案生成与评估。该方法从初始行动集出发，系统生成数千个行动替代方案，通过评估识别更优方案，并在作战过程中动态更新。


<details>
  <summary>Details</summary>
Motivation: 为军事地面作战执行阶段提供决策支持，特别是在机械化营的行动规划中，需要系统化的方法来生成和评估各种行动方案，以应对动态变化的战场环境。

Method: 采用并行生成和评估的方法，从初始行动集开始，系统生成大量行动替代方案，结合敌方状态和行动进行多维度评估，包括部队组成、兵力比、攻防类型和预期推进率等要素。

Result: 该方法能够产生多样化的行动替代方案，支持基于已评估行动的新方案生成管理，并在作战条件变化时提供修订的行动方案。

Conclusion: 所提出的方法论为军事决策者提供了有效的行动方案生成和评估框架，能够在动态作战环境中支持顺序决策过程。
...
Method: 使用LLM智能体处理维护日志中的六种噪声类型，包括拼写错误、缺失字段、近似重复条目和错误日期等。

Result: LLM在处理通用清洗任务方面表现有效，为未来工业应用提供了有前景的基础。
...
### [54] [Evaluating LLMs' Reasoning Over Ordered Procedural Steps](https://arxiv.org/abs/2511.04688)
*Adrita Anika,Md Messal Monem Miah*

Main category: cs.CL

TL;DR: 本文研究了LLMs在程序性序列推理任务中的表现，通过使用食谱数据集评估模型从乱序步骤重建正确序列的能力，发现模型性能随序列长度增加和步骤位移加剧而下降。
...
Result: 分析显示模型性能随序列长度增加而下降，反映了较长程序的复杂性增加。输入中步骤位移越大（对应更严重的乱序）会导致性能进一步恶化。

Conclusion: 这些发现突显了当前LLMs在程序性推理方面的局限性，特别是在处理更长和更无序的输入时表现不佳。
...
### [55] [Adaptive Testing for LLM Evaluation: A Psychometric Alternative to Static Benchmarks](https://arxiv.org/abs/2511.04689)
*Peiyu Li,Xiuxiu Tang,Si Chen,Ying Cheng,Ronald Metoyer,Ting Hua,Nitesh V. Chawla*

Main category: cs.CL

TL;DR: ATLAS是一个基于项目反应理论的自适应测试框架，通过Fisher信息指导的项目选择来估计模型能力，能在保持测量精度的同时减少90%的测试项目。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估需要数千个基准项目，导致评估昂贵且缓慢。固定项目集的平均准确度方法将所有项目同等对待，忽略了项目质量和信息量的差异。

Method: 使用项目反应理论（IRT）和Fisher信息指导的项目选择来估计模型能力，分析五个主要基准发现3-6%的项目存在负区分度，表明存在注释错误。

Result: 在HellaSwag基准上仅使用42个项目（原5,608个）就能匹配完整基准估计，平均绝对误差为0.154。项目暴露率低于10%，测试重叠率为16-27%，而静态基准中所有模型都看到所有项目（100%暴露）。

Conclusion: IRT排名与准确度排名存在差异：相同准确度的模型获得不同的IRT分数，23-31%的模型排名变化超过10位，表明自适应测试能提供更精确的模型能力评估。
...
### [56] [Reasoning Up the Instruction Ladder for Controllable Language Models](https://arxiv.org/abs/2511.04694)
*Zishuo Zheng,Vidhisha Balachandran,Chan Young Park,Faeze Brahman,Sachin Kumar*

Main category: cs.CL

TL;DR: 该研究将指令层次结构解析重新定义为推理任务，通过构建VerIH数据集和轻量级强化学习训练，使LLM能够优先处理系统指令而非用户输入，从而提升模型可靠性和对抗攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在现实决策中承担高风险角色，需要处理来自多个来源的竞争性指令，建立指令层次结构对于LLM的可靠性和可控性至关重要。

Method: 构建VerIH指令层次数据集，包含对齐和冲突的系统-用户指令，通过轻量级强化学习训练模型进行指令优先级推理。

Result: 微调模型在指令遵循和指令层次基准测试中取得一致改进，推理能力泛化到安全关键场景，增强了对抗越狱和提示注入攻击的鲁棒性。
...
### [57] [EncouRAGe: Evaluating RAG Local, Fast, and Reliable](https://arxiv.org/abs/2511.04696)
*Jan Strich,Adeline Scharfenberg,Chris Biemann,Martin Semmann*

Main category: cs.CL

TL;DR: EncouRAGe是一个用于简化基于大语言模型和嵌入模型的检索增强生成系统开发和评估的Python框架，包含五个模块化组件，强调科学可重复性和本地部署。


<details>
  <summary>Details</summary>
Motivation: 为了简化RAG系统的开发和评估过程，提供灵活的实验环境和可扩展的开发框架，促进RAG研究的科学可重复性。

Method: 开发了包含Type Manifest、RAG Factory、Inference、Vector Store和Metrics五个模块化组件的Python框架，支持灵活实验和扩展开发。

Result: 在多个基准数据集（包括25k问答对和51k+文档）上的评估显示，RAG在Oracle Context下表现不佳，而Hybrid BM25在所有四个数据集中始终获得最佳结果；重排序仅带来边际性能提升但增加了响应延迟。

Conclusion: EncouRAGe框架为RAG系统开发提供了有效的工具，实验结果表明RAG仍有改进空间，Hybrid BM25表现最佳，重排序技术需权衡性能提升与延迟成本。
...
### [58] [multiMentalRoBERTa: A Fine-tuned Multiclass Classifier for Mental Health Disorder](https://arxiv.org/abs/2511.04698)
...
<details>
  <summary>Details</summary>
Motivation: 从社交媒体文本中早期检测心理健康障碍对于提供及时支持、风险评估和转介适当资源至关重要。

Method: 基于多个精心策划的数据集，对RoBERTa模型进行微调，用于多类别心理健康状况分类，并与传统机器学习方法、领域特定transformers和基于提示的大语言模型进行比较。

Result: multiMentalRoBERTa在六分类设置中宏观F1分数达到0.839，在五分类设置中达到0.870，优于微调的MentalBERT和基线分类器。数据探索揭示了抑郁与自杀意念以及焦虑与创伤后应激障碍之间的强相关性。

Conclusion: multiMentalRoBERTa被提出作为一种轻量级、鲁棒且可部署的解决方案，用于增强心理健康平台的支持能力，同时强调公平性、偏见缓解和人在回路安全协议的重要性。
...
### [59] [Cross-Lingual SynthDocs: A Large-Scale Synthetic Corpus for Any to Arabic OCR and Document Understanding](https://arxiv.org/abs/2511.04699)
...
### [60] [Measuring what Matters: Construct Validity in Large Language Model Benchmarks](https://arxiv.org/abs/2511.04703)
...
<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型对于衡量其能力和识别部署前的安全或鲁棒性问题至关重要。可靠地测量抽象复杂现象需要强大的构念效度，即测量指标能够代表现象的关键要素。

Method: 组织29名专家评审团队，对自然语言处理和机器学习顶级会议中的445个LLM基准测试进行系统性评审，分析测量现象、任务和评分指标的模式。

Result: 发现现有基准测试在测量现象、任务和评分指标方面存在模式问题，这些模式削弱了评估结果的有效性，影响了关于模型安全性和鲁棒性的主张。

Conclusion: 为研究人员和从业者提供了8项关键建议和详细可操作指南，以改进LLM基准测试的开发，确保评估结果具有更好的构念效度和可靠性。
...
### [61] [GEMMA-SQL: A Novel Text-to-SQL Model Based on Large Language Models](https://arxiv.org/abs/2511.04710)
...
Result: GEMMA-SQL Instruct变体在SPIDER基准测试中达到66.8%的测试套件准确率和63.3%的精确集合匹配准确率，优于IRNet、RYANSQL和CodeXDavinci等基线模型。
...
### [62] [First is Not Really Better Than Last: Evaluating Layer Choice and Aggregation Strategies in Language Model Data Influence Estimation](https://arxiv.org/abs/2511.04715)
*Dmytro Vitel,Anshuman Chhabra*

Main category: cs.CL

TL;DR: 本文挑战了先前关于LLM影响函数计算中嵌入层最优的观点，提出中间注意力层更适合影响估计，并开发了新的评估指标和聚合方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLM影响函数计算由于模型规模庞大，通常只能计算部分层的影响。先前研究认为嵌入层最适合，但本文发现这种基于抵消效应的假设不可靠。

Method: 提出理论和实证证据证明抵消效应不可靠，发现中间注意力层是更好的影响估计器。开发了排名和投票等替代标准平均的聚合方法，并提出新的评估指标NDR。

Result: 通过在不同类型和规模的LLM上进行广泛实验，证明第一层（嵌入层）不一定比最后一层更适合LLM影响估计，与领域先验知识形成对比。

Conclusion: 中间注意力层比嵌入层更适合LLM影响函数计算，新的聚合方法和NDR指标能显著提升影响估计性能，为LLM决策解释提供了更可靠的方法。
...
### [63] [Surprisal reveals diversity gaps in image captioning and different scorers change the story](https://arxiv.org/abs/2511.04754)
*Nikolai Ilinykh,Simon Dobnik*

Main category: cs.CL

TL;DR: 本文提出了一种基于信息熵方差的图像描述语言多样性度量方法，通过比较人类描述与五种最先进视觉语言模型在MSCOCO数据集上的表现，发现使用不同评分器会完全反转多样性结论。
...
Method: 引入信息熵方差作为多样性度量指标，比较人类描述与五种视觉语言模型在贪婪解码和核采样下的表现，使用基于描述的n-gram语言模型和通用语言模型进行评分。

Result: 使用基于描述的n-gram模型时，人类描述的信息熵方差约为模型的两倍；但使用通用语言模型评分时，模式完全反转。单一评分器可能导致结论完全相反。

Conclusion: 提出基于信息熵的多样性度量方法，强调稳健的多样性评估必须报告多个评分器下的信息熵值，避免因单一评分器而产生误导性结论。
...
### [64] [Explore Data Left Behind in Reinforcement Learning for Reasoning Language Models](https://arxiv.org/abs/2511.04800)
*Chenxi Liu,Junjie Liang,Yuqi Jia,Bochuan Cao,Yang Bai,Heng Huang,Xun Chen*

Main category: cs.CL

TL;DR: ERPO框架通过探索零方差奖励的残差提示来增强LLM的强化学习训练效果，通过自适应温度调整鼓励多样性推理轨迹生成，从而重新激活训练信号。


<details>
  <summary>Details</summary>
Motivation: 随着LLM训练时间延长和规模扩大，越来越多的训练提示成为残差提示（零方差奖励），这些提示无法提供训练信号，降低了训练多样性和效果。
...
Conclusion: ERPO通过有效利用残差提示，提升了强化学习在语言模型训练中的效果，特别是在数学推理任务上表现出色。
...
### [65] [Trained on Tokens, Calibrated on Concepts: The Emergence of Semantic Calibration in LLMs](https://arxiv.org/abs/2511.04869)
*Preetum Nakkiran,Arwen Bradley,Adam Goliński,Eugene Ndiaye,Michael Kirchhof,Sinead Williamson*

Main category: cs.CL

TL;DR: 该论文发现基础LLMs在语义校准方面表现良好，能够有意义地评估开放域问答任务中的置信度，尽管没有经过明确训练。理论贡献建立了语义校准作为下一个令牌预测副产品的机制，并通过实验验证了三个关键发现。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否能够超越令牌级别，评估其回答实际含义的置信度，因为现有研究主要关注下一个令牌校准，而语义校准能力尚不清楚。

Method: 提出基于采样的语义校准概念，建立B-校准理论框架，通过连接校准和局部损失最优性来解释语义校准的出现机制，并进行实验验证。

Result: 发现基础LLMs在语义校准方面表现良好；RL指令微调会系统性破坏这种校准；思维链推理也会破坏校准。

Conclusion: 该工作首次为LLMs中语义校准何时以及为何出现提供了原则性解释，揭示了基础LLMs具有内在的语义校准能力，但后续优化过程可能破坏这种能力。
...
### [66] [Minimal and Mechanistic Conditions for Behavioral Self-Awareness in LLMs](https://arxiv.org/abs/2511.04875)
...
<details>
  <summary>Details</summary>
Motivation: LLMs展现出的行为自我意识能力可能带来安全隐患，比如在评估中更好地隐藏真实能力，因此需要理解这种自我意识出现的最小条件和机制过程。

Method: 通过在指令调优的LLMs上进行受控的LoRA微调实验，使用低秩适配器来研究自我意识的诱导机制。

Result: 1) 单一rank-1 LoRA适配器可可靠诱导自我意识；2) 学习到的自我意识行为可由激活空间中的单一转向向量捕获；3) 自我意识具有非普遍性和领域局部化特征。
...
### [67] [SDS KoPub VDR: A Benchmark Dataset for Visual Document Retrieval in Korean Public Documents](https://arxiv.org/abs/2511.04910)
*Jaehoon Lee,Sohyun Kim,Wanggeun Park,Geon Lee,Seungkyung Kim,Minyoung Lee*

Main category: cs.CL

TL;DR: SDS KoPub VDR是首个大规模公开的韩语公共文档检索基准，包含361个真实文档（40,781页），构建了600个查询-页面-答案三元组，支持文本检索和多模态检索任务评估。
...
Method: 基于361个真实世界文档构建基准，包括256个KOGL Type 1许可文件和105个官方法律门户文件。通过多模态模型生成查询-页面-答案三元组，并经过严格的人工验证和优化过程。

Result: 双任务评估揭示了显著的性能差距，特别是在需要跨模态推理的多模态场景中，即使是先进模型也存在困难。

Conclusion: SDS KoPub VDR不仅能够在文本和多模态检索任务中进行严格评估，还为推进复杂真实世界文档智能中的多模态AI提供了明确路线图。
...
### [68] [BudgetMem: Learning Selective Memory Policies for Cost-Efficient Long-Context Processing in Language Models](https://arxiv.org/abs/2511.04919)
*Chandra Vamsi Krishna Alla,Harish Naidu Gaddam,Manohar Kommi*

Main category: cs.CL

TL;DR: BudgetMem是一种新型内存增强架构，通过选择性记忆策略和特征显著性评分，在严格预算约束下决定存储哪些信息，相比传统RAG系统节省72.4%内存且性能仅下降1.0%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理长上下文时面临显著的计算和内存约束，尽管对需要处理长文档、多会话对话和书籍长度文本的应用需求日益增长。现有扩展上下文窗口的方法在资源受限部署中成本过高。

Method: 结合选择性记忆策略与基于特征的显著性评分（实体密度、TF-IDF、话语标记、位置偏差），在严格预算约束下决定哪些信息值得存储。使用学习门控机制和BM25稀疏检索实现高效信息访问。

Result: 在700个问答对上的实验表明，BudgetMem在长文档上仅产生1.0% F1分数下降，同时节省72.4%内存。预算敏感性分析显示BudgetMem的优势随文档长度增加而增强。

Conclusion: BudgetMem为在普通硬件上部署能力强的长上下文系统提供了实用途径，使先进语言理解能力更加普及。
...
### [69] [Diagnosing and Mitigating Semantic Inconsistencies in Wikidata's Classification Hierarchy](https://arxiv.org/abs/2511.04926)
*Shixiong Zhao,Hideaki Takeda*

Main category: cs.CL

TL;DR: 本文提出了一种验证方法，用于检测Wikidata知识图谱中的分类错误、过度泛化的子类链接和冗余连接，并开发了一个系统供用户检查任意实体的分类关系。
...
Method: 提出并应用新的验证方法来确认分类错误、过度泛化的子类链接和冗余连接，引入新的评估标准判断问题是否需要修正，开发用户检查系统。

Result: 成功验证了特定领域中存在分类错误、过度泛化的子类链接和冗余连接问题。

Conclusion: 该方法能够有效识别Wikidata中的分类不一致问题，并利用众包特性充分发挥平台潜力进行修正。
...
### [70] [LoPT: Lossless Parallel Tokenization Acceleration for Long Context Inference of Large Language Model](https://arxiv.org/abs/2511.04952)
*Wei Shao,Lingchao Zheng,Pengyu Wang,Peizhen Zheng,Jun Li,Yuwei Fan*

Main category: cs.CL

TL;DR: LoPT是一种无损并行分词框架，解决了长文本推理中分词瓶颈问题，通过基于字符位置匹配和动态分块长度调整确保与顺序分词输出一致，同时实现显著加速。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理场景对大型语言模型越来越重要，但带来显著计算延迟。现有并行分词方法通过文本分割和多进程分词加速处理，但由于合并后的边界伪影导致结果不一致。

Method: 提出LoPT框架，采用基于字符位置的匹配和动态分块长度调整来准确对齐和合并分词片段，确保与标准顺序分词输出完全相同。

Result: 在多样化长文本数据集上的广泛实验表明，LoPT在保证无损分词的同时实现了显著加速。

Conclusion: LoPT提供了理论一致性证明和全面分析研究，验证了方法的鲁棒性，解决了并行分词中的边界伪影问题。
...
### [71] [Too Good to be Bad: On the Failure of LLMs to Role-Play Villains](https://arxiv.org/abs/2511.04962)
*Zihao Yi,Qingxuan Jiang,Ruotian Ma,Xingyu Chen,Qu Yang,Mengru Wang,Fanghua Ye,Ying Shen,Zhaopeng Tu,Xiaolong Li,Linus*

Main category: cs.CL

TL;DR: 本文研究了大型语言模型在扮演反派角色时的表现，发现安全对齐与角色扮演真实性之间存在冲突，模型在扮演道德越低的角色时表现越差。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在模拟非亲社会、反派角色时的能力，探索模型安全对齐与创造性角色扮演真实性之间的根本冲突。

Method: 引入Moral RolePlay基准数据集，包含四级道德对齐量表，让最先进的LLMs扮演从道德典范到纯粹恶棍的角色，进行大规模评估。

Result: 发现随着角色道德水平降低，角色扮演保真度单调下降；模型在与安全原则直接对立的特质上表现最差；高度安全对齐的模型在反派角色扮演方面表现特别差。

Conclusion: 揭示了模型安全性与创造性保真度之间的关键张力，为开发更细致、上下文感知的对齐方法铺平了道路。
...
### [72] [Acquiring Common Chinese Emotional Events Using Large Language Model](https://arxiv.org/abs/2511.04989)
...
Result: 获得了102,218个带有情感极性标签的高质量常见情感事件，这是唯一大规模的中文情感事件常识知识库。内在评估显示该方法有效，外在用例展示了在情感原因提取领域的潜力。
...
### [73] [UA-Code-Bench: A Competitive Programming Benchmark for Evaluating LLM Code Generation in Ukrainian](https://arxiv.org/abs/2511.05040)
*Mykyta Syromiatnikov,Victoria Ruvinskaya*

Main category: cs.CL

TL;DR: UA-Code-Bench是一个针对乌克兰语代码生成和竞争性编程问题解决能力的新基准测试，包含500个不同难度的问题，评估了13个领先模型，结果显示即使是顶级模型也只能解决一半问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注从英语翻译的广泛任务或仅评估简单的语言理解，难以准确评估大语言模型在低资源语言中的真实能力。

Method: 使用来自Eolymp平台的500个问题，均匀分布在5个难度级别，通过专用Eolymp环境对13个领先的专有和开源模型进行隐藏测试评估，确保代码正确性。

Result: 结果显示即使是顶级模型如OpenAI o3和GPT-5也只能解决一半的问题，突显了在低资源自然语言中代码生成的挑战。

Conclusion: 这项工作证明了竞争性编程基准测试在评估大语言模型方面的价值，特别是在代表性不足的语言中，为多语言代码生成和推理增强模型的未来研究铺平了道路。
...
### [74] [Order-Level Attention Similarity Across Language Models: A Latent Commonality](https://arxiv.org/abs/2511.05064)
*Jinglin Liang,Jin Zhong,Shuangping Huang,Yunqing Hu,Huiyuan Zhang,Huifang Li,Lixin Fan,Hanlin Gu*

Main category: cs.CL

TL;DR: 本文研究了语言模型中上下文聚合模式的共性，提出了顺序级注意力(OLA)概念，发现不同语言模型在相同顺序的OLA上存在显著相似性，并基于此开发了无需训练的可迁移OLA适配器(TOA)，能够有效提升未见语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型中上下文聚合模式的共性，加深对语言模型的理解并促进跨模型知识迁移。现有研究通常关注单个模型或注意力头，缺乏对多个语言模型共性的系统性分析。

Method: 引入基于注意力展开顺序分解的顺序级注意力(OLA)，发现OLA与句法知识之间存在隐式映射，并提出了可迁移OLA适配器(TOA)，将OLA作为统一的句法特征表示来训练适配器。

Result: 实验证明，不同语言模型在相同顺序的OLA上表现出显著相似性，TOA方法能够在无需参数更新的情况下泛化到未见语言模型，有效提升其性能。

Conclusion: 语言模型在上下文聚合模式上存在共性，基于OLA的TOA方法实现了跨语言模型的零样本知识迁移，为语言模型理解和应用提供了新视角。
...
### [75] [Reasoning-Guided Claim Normalization for Noisy Multilingual Social Media Posts](https://arxiv.org/abs/2511.05078)
...
Result: 在20种语言上取得了显著成果，METEOR得分从英语的41.16到马拉地语的15.21，在英语排行榜上排名第三，荷兰语和旁遮普语排名第四，相比基线配置相对提升了41.3%。

Conclusion: 该方法在罗曼语系和日耳曼语系语言上表现出有效的跨语言泛化能力，同时在不同语言结构中保持了语义连贯性。
...
### [76] [On Text Simplification Metrics and General-Purpose LLMs for Accessible Health Information, and A Potential Architectural Advantage of The Instruction-Tuned LLM class](https://arxiv.org/abs/2511.05080)
*P. Bilha Githinji,Aikaterini Meilliou,Peiwu Qin*

Main category: cs.CL

TL;DR: 本文评估了两种通用大语言模型在文本简化任务中的表现，发现指令调优的Mistral 24B在可读性和话语保真度平衡方面优于推理增强的QWen2.5 32B，并识别了度量选择的功能冗余问题。


<details>
  <summary>Details</summary>
Motivation: 随着公众健康信息需求和数字消费的增加，需要可扩展的解决方案来自动将复杂科学文档简化为通俗语言，但现有方法在可读性和内容保真度平衡方面仍面临挑战。

Method: 采用比较分析方法，评估指令调优的Mistral 24B和推理增强的QWen2.5 32B在文本简化任务中的表现，使用21个指标进行综合相关性分析。

Result: Mistral在SARI指标上达到42.46，BERTScore为0.91，在可读性和话语保真度平衡方面表现更好；QWen虽然可读性增强，但BERTScore显著较低为0.89。五个可读性指标存在强功能冗余。

Conclusion: 研究为文本简化任务建立了LLM基准性能，识别指令调优的Mistral 24B更适合简化任务，提供了度量选择启发，并指出词汇支持是简化的主要领域适应问题。
...
### [77] [Iterative Layer-wise Distillation for Efficient Compression of Large Language Models](https://arxiv.org/abs/2511.05085)
*Grigory Kovalev,Mikhail Tikhomirov*

Main category: cs.CL

TL;DR: 本文研究了大型语言模型的蒸馏方法，旨在开发保持高性能的紧凑模型。基于ShortGPT方法改进，通过迭代评估层重要性来减少模型层数，在Qwen2.5-3B模型上成功将层数从36减少到28，参数降至24.7亿，质量损失仅9.7%。


<details>
  <summary>Details</summary>
Motivation: 开发能够在保持高性能的同时减少模型大小的紧凑模型，以适应资源受限环境中的部署需求。

Method: 基于ShortGPT方法改进，采用迭代评估层重要性的方法，通过测量移除单个层时性能下降来评估重要性，并结合基于KL散度和均方误差的联合损失函数进行进一步训练。

Result: 在Qwen2.5-3B模型上，成功将层数从36减少到28（参数降至24.7亿），质量损失仅9.7%；减少到24层时质量损失为18%。发现中间transformer层对推理贡献较小。

Conclusion: 迭代蒸馏和微调方法有效，能够创建高效模型，特别适合资源受限环境中的部署。该方法证明了中间层在推理中的重要性较低，为模型压缩提供了新思路。
...
### [78] [A Toolbox for Improving Evolutionary Prompt Search](https://arxiv.org/abs/2511.05120)
...
Result: 该方法在优化质量和效率方面均有提升，代码已发布支持新任务的提示优化。

Conclusion: 提出的改进方法能够部分泛化到一般提示优化中，为后续研究提供了基础。
...
### [79] [ManufactuBERT: Efficient Continual Pretraining for Manufacturing](https://arxiv.org/abs/2511.05135)
*Robin Armingaud,Romaric Besançon*

Main category: cs.CL

TL;DR: 本文提出了ManufactuBERT，一个针对制造业领域持续预训练的RoBERTa模型，通过精心处理的大规模制造业语料库，在制造业相关NLP任务上达到最先进性能，并显著减少训练时间和计算成本。
...
Method: 提出了一个全面的数据处理流程，包括领域特定过滤和多阶段去重过程，创建制造业语料库，并基于此持续预训练RoBERTa模型。

Result: ManufactuBERT在多个制造业相关NLP任务上建立了新的最先进性能，并且经过精心去重的语料库训练使收敛速度显著加快，训练时间和计算成本减少了33%。
...
### [80] [Effectiveness of Chain-of-Thought in Distilling Reasoning Capability from Large Language Models](https://arxiv.org/abs/2511.05184)
*Cong-Thanh Do,Rama Doddipatla,Kate Knill*

Main category: cs.CL

TL;DR: 本文研究了思维链（CoT）在白盒知识蒸馏中的作用，通过使用Qwen和Llama2系列LLM进行实验，证明CoT能有效提升蒸馏模型在复杂自然语言推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 探索思维链（CoT）在知识蒸馏中传递推理能力的作用，特别是如何利用CoT将大型语言模型的推理能力蒸馏到小型语言模型中。

Method: 使用Qwen和Llama2系列LLM进行白盒知识蒸馏实验，采用CoT-Collection数据集中的思维链数据，并在BIG-Bench-Hard基准上评估蒸馏模型的自然语言推理和理解能力。

Result: 实验结果表明，思维链显著提高了白盒知识蒸馏的效果，使蒸馏模型在BBH的自然语言推理和理解任务中获得了更好的平均性能。

Conclusion: 思维链在白盒知识蒸馏中发挥重要作用，能够有效提升小型语言模型在复杂推理任务上的表现。
...
### [81] [Translation via Annotation: A Computational Study of Translating Classical Chinese into Japanese](https://arxiv.org/abs/2511.05239)
*Zilong Li,Jie Cao*

Main category: cs.CL

TL;DR: 本文研究将古汉语翻译成日语的过程抽象为序列标注任务，提出基于大语言模型的标注流程，构建新数据集，并验证在低资源环境下引入中文NLP辅助任务对序列标注训练的促进作用。


<details>
  <summary>Details</summary>
Motivation: 研究古汉语到日语的翻译标注系统面临低资源问题，需要探索现代语言技术在此领域的应用。

Method: 采用基于大语言模型的标注流程，从数字化开源翻译数据构建新数据集，并引入中文NLP辅助任务来提升序列标注任务的训练效果。

Result: 在低资源设置下，引入中文NLP辅助任务对序列标注训练有促进作用；大语言模型在直接机器翻译中表现优异，但在字符标注任务中存在困惑。

Conclusion: 所提出的方法可以作为大语言模型的补充，在古汉语翻译标注任务中发挥重要作用。
...
### [82] [Reflective Personalization Optimization: A Post-hoc Rewriting Framework for Black-Box Large Language Models](https://arxiv.org/abs/2511.05286)
...
Method: 提出RPO框架：第一阶段基础模型生成通用响应，第二阶段训练反射模块进行重写对齐。反射模块采用监督微调建立个性化推理策略，再通过强化学习优化输出质量。

Result: 在LaMP基准测试中，RPO显著优于最先进的基线方法，证明了显式响应塑造优于隐式上下文注入。

Conclusion: RPO引入了一个高效、模型无关的个性化层，可以与任何基础模型无缝集成，为用户中心生成场景开辟了新方向。
...
### [83] [Listening Between the Lines: Decoding Podcast Narratives with Language Modeling](https://arxiv.org/abs/2511.05310)
...
<details>
  <summary>Details</summary>
Motivation: 播客已成为塑造公众意见的重要媒介，但其非脚本化、多主题和对话式的特点使得自动分析变得困难。现有基于结构化文本训练的模型难以捕捉播客中微妙的叙事框架线索。

Method: 开发并评估了一个微调的BERT模型，将叙事框架与对话中提到的具体实体明确关联，使抽象框架在具体细节中得以落地。然后使用这些细粒度框架标签并与高层次主题关联。

Result: 该方法与人类判断更接近，能够准确分析播客叙事框架，并揭示了讨论内容（主题）与呈现方式（框架）之间的系统性关系。

Conclusion: 提出了一种更稳健的数字媒体影响力研究框架，通过连接叙事框架与具体实体的方法，为播客内容分析提供了更准确和可扩展的解决方案。
...
### [84] [What Are the Facts? Automated Extraction of Court-Established Facts from Criminal-Court Opinions](https://arxiv.org/abs/2511.05320)
*Klára Bendová,Tomáš Knap,Jan Černý,Vojtěch Pour,Jaromir Savelka,Ivana Kvapilíková,Jakub Drápal*

Main category: cs.CL

TL;DR: 本文研究了从斯洛伐克法院判决中提取犯罪行为描述的可行性，比较了正则表达式和大型语言模型两种方法，发现先进正则表达式和LLM方法都能达到97%以上的提取率，结合使用可达99.5%。


<details>
  <summary>Details</summary>
Motivation: 刑事司法行政数据中关于犯罪行为的详细信息有限，而欧洲大陆法院判决中的犯罪行为描述是一个未充分利用的信息源，需要开发有效的方法来提取这些描述。

Method: 使用两种方法：正则表达式（基础版和进阶版）和大型语言模型（Gemini Flash 2.0）。基础正则表达式识别典型词汇，进阶版关注"sparing"及其规范化；LLM方法通过预定义指令提取描述。

Result: 基础方法仅识别40.5%的描述，进阶正则表达式达97%，LLM达98.75%，结合使用达99.5%。法学学生评估显示进阶方法约90%与人工标注匹配，基础方法仅34.5%。LLM在91.75%情况下完全匹配人工标注。

Conclusion: 从法院判决中提取犯罪行为描述是可行的，先进正则表达式和LLM方法都表现优异，结合使用效果最佳，为刑事司法研究提供了新的数据源。
...
### [85] [Evaluating Subword Tokenization Techniques for Bengali: A Benchmark Study with BengaliBPE](https://arxiv.org/abs/2511.05324)
*Firoj Ahmmed Patwary,Abdullah Al Noman*

Main category: cs.CL

TL;DR: 本文提出了BengaliBPE，一种专门为孟加拉语设计的BPE分词器，通过Unicode标准化、字素级初始化和形态感知合并规则来解决现有分词器在形态丰富的孟加拉语上表现不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 当前主流的子词分词器（如SentencePiece、HuggingFace BPE）主要针对拉丁语系或多语言语料设计，在形态丰富的语言如孟加拉语上表现不佳，因此需要开发专门针对孟加拉语的分词器。

Method: BengaliBPE采用字节对编码（BPE）方法，结合Unicode标准化、字素级初始化和形态感知合并规则，以保持语言一致性和子词完整性。使用大规模孟加拉语新闻分类数据集与三种基线方法进行比较。

Result: 评估显示所有方法表现都合理，但BengaliBPE提供了最详细的分割和最佳的形态可解释性，尽管计算成本略高。在分词粒度、编码速度和下游分类准确性方面进行了全面比较。
...
### [86] [Minority-Aware Satisfaction Estimation in Dialogue Systems via Preference-Adaptive Reinforcement Learning](https://arxiv.org/abs/2511.05407)
*Yahui Fu,Zi Haur Pang,Tatsuya Kawahara*

Main category: cs.CL

TL;DR: 本文提出一个统一框架来建模个体和群体层面的用户满意度偏好，通过个性化推理链和多数-少数偏好聚类算法，结合偏好自适应强化学习来提升对话系统的用户满意度估计。
...
Method: 提出Chain-of-Personalized-Reasoning(CoPeR)捕捉个体偏好；开发基于期望最大化的多数-少数偏好感知聚类算法(M2PC)无监督发现用户群体；整合为偏好自适应强化学习框架(PAda-PPO)联合优化个体和群体偏好对齐。

Result: 在情感支持对话数据集上的实验表明，用户满意度估计得到持续改进，特别是对于代表性不足的用户群体。

Conclusion: 该框架能够有效建模个体和群体层面的用户偏好，显著提升对话系统的用户满意度评估性能，特别是在处理少数用户群体偏好方面表现突出。
...
### [87] [Steering Language Models with Weight Arithmetic](https://arxiv.org/abs/2511.05408)
*Constanza Fierro,Fabien Roger*

Main category: cs.CL

TL;DR: 提出对比权重引导方法，通过权重算术编辑模型参数，利用窄分布训练数据实现更好的行为控制，在减少奉承行为和诱导错位方面表现出色，并能检测训练过程中的错位行为。


<details>
  <summary>Details</summary>
Motivation: 在多样化训练分布上为大型语言模型提供高质量反馈困难且昂贵，而仅在窄分布上提供反馈可能导致意外泛化。需要更好利用窄训练数据来控制模型行为。

Method: 通过从两个小规模微调中减去权重增量来隔离行为方向：一个诱导期望行为，另一个诱导相反行为。然后通过添加或移除该方向来修改模型权重。

Result: 权重引导比激活引导具有更好的泛化能力，在保持通用能力的同时实现更强的分布外行为控制。能减少微调引入的奉承行为和拒绝不足，同时保持任务性能增益。

Conclusion: 对比权重引导是有效的后训练方法，能利用窄训练数据实现行为控制，并可通过权重相似性检测训练过程中的错位行为。
...
### [88] [MIMIC-SR-ICD11: A Dataset for Narrative-Based Diagnosis](https://arxiv.org/abs/2511.05485)
*Yuexin Wu,Shiqi Wang,Vasile Rus*

Main category: cs.CL

TL;DR: 该论文提出了MIMIC-SR-ICD11数据集和LL-Rank框架，用于从电子健康记录中自动诊断编码，通过基于似然的重排序方法显著提升了诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 疾病诊断是现代医疗的核心，但电子健康记录中的模板化文档往往会遗漏临床重要信号。自述报告能保留这些被衰减或忽略的细微但重要的细节。

Method: 构建了MIMIC-SR-ICD11数据集，并提出了LL-Rank框架——一种基于似然的重排序方法，计算每个标签在临床报告上下文中的长度归一化联合似然，并减去相应的无报告先验似然。

Result: 在七个模型骨干上，LL-Rank始终优于强大的生成加映射基线方法（GenMap）。消融实验表明LL-Rank的改进主要来自其基于PMI的评分，该评分将语义兼容性与标签频率偏差分离开来。

Conclusion: LL-Rank框架通过基于互信息的评分机制有效提升了临床诊断编码的准确性，为自动疾病诊断提供了有效的解决方案。
