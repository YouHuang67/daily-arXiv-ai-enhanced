...
Motivation: 研究动机是解决水下目标检测在动态海洋环境中面临的挑战，包括突然运动、部分遮挡和渐进运动等复杂条件，探索如何通过时空建模和注意力机制来提高检测性能。

Method: 研究分为两个阶段：第一阶段评估时间增强的T-YOLOv5与标准YOLOv5的性能；第二阶段开发了集成卷积块注意力模块（CBAM）的T-YOLOv5增强版本。

Result: 测试结果显示，YOLOv5的mAP@50-95为0.563，而T-YOLOv5和T-YOLOv5+CBAM分别达到0.813和0.811，表现出更高的检测精度和泛化能力。

Conclusion: T-YOLOv5相比标准模型显著提高了检测可靠性，而集成CBAM的T-YOLOv5在复杂场景中性能进一步提升，但在简单场景中精度有所损失。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在大规模未筛选数据集上训练以实现多样化的生成能力，但这与用户偏好不一致。现有的奖励模型通过事后选择图像来对齐用户偏好，但这会损害多样性、语义保真度和效率。

Method: 提出MIRO方法，在训练过程中让模型直接学习多个奖励模型的条件，而不是进行事后处理。

Result: MIRO在GenEval组合基准测试和用户偏好评分（PickAScore、ImageReward、HPSv2）上实现了最先进的性能。

Conclusion: 通过直接学习用户偏好而不是事后处理，MIRO方法不仅显著提高了生成图像的视觉质量，还显著加快了训练速度。
...
Result: 在BikeScenes数据集上微调的模型实现了63.6%的平均交并比（mIoU），显著优于仅使用SemanticKITTI预训练的13.8%结果。

Conclusion: 领域特定训练对于自行车感知系统至关重要，BikeScenes数据集为推进以骑行者为中心的LiDAR分割研究提供了宝贵资源。
...
Method: 使用36张原始实验图像，通过物理信息合成数据生成管道训练最先进的流匹配和扩散模型。

Result: 模型能够有效恢复图像，通过稀疏采样数据准确重建图像，实现2-4倍的图像采集时间减少。
...
<details>
  <summary>Details</summary>
Motivation: 整流流模型在图像生成中表现出色，但在图像编辑任务中存在关键限制：反转过程不准确导致真实图像映射到潜在空间困难，编辑过程中的梯度纠缠问题导致输出无法忠实反映目标提示。

Method: 提出流分解与聚合框架，将目标提示语义分解为多个子提示，为每个子提示计算独立流，然后通过投影和软聚合机制自适应加权子目标速度场，抑制语义冗余同时强调不同方向。

Result: 实验结果表明，该方法在语义保真度和属性解耦方面优于现有的零样本编辑方法，能够生成既多样化又一致的编辑输出。

Conclusion: 通过流分解与聚合框架，成功解决了整流流模型在图像编辑中的反转不准确和梯度纠缠问题，实现了更好的语义保真度和属性解耦效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法从fMRI重建图像时缺乏对实际看到图像的忠实度，需要改进重建质量。

Method: 使用大脑交互变换器(BIT)实现功能相似脑体素簇之间的有效交互，预测高层次语义特征和低层次结构特征来指导扩散模型。

Result: 该方法在视觉和客观指标上都超越现有SotA方法，仅用1小时fMRI数据即可达到与40小时训练相当的效果。

Conclusion: Brain-IT通过脑启发的功能簇交互设计，实现了更忠实、高效的fMRI图像重建。
...
<details>
  <summary>Details</summary>
Motivation: 解决TrackRAD2025挑战赛中胸腹部cine-MRI序列实时肿瘤跟踪问题，在强数据稀缺约束下实现准确跟踪。

Method: 采用两种互补策略：无监督配准（使用IMPACT相似性度量）和基于基础模型的分割（使用SAM 2.1及其变体进行提示交互）。最终选择SAM2.1 b+模型，使用第一标注切片的掩码提示，仅在TrackRAD2025的小标注子集上进行微调。

Result: 在隐藏测试集上获得0.8794的Dice相似系数，在TrackRAD2025挑战赛中总体排名第6。
...
Method: 引入Hilbert选择性扫描机制，增加扫描模式的Hausdorff维度，改进空间局部性，同时保持处理长距离依赖的能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉中的异常检测主要局限于工业缺陷或合成的异常，无法捕捉真实世界异常的丰富性和不可预测性，而人类能够自然地识别、推理和解释环境中的异常。

Method: CAVE基准基于认知科学研究，提供细粒度标注，包括视觉定位和基于视觉表现、复杂性、严重性和常见性对异常进行分类。
...
<details>
  <summary>Details</summary>
Motivation: 气候变化和海平面上升对沿海城市构成日益严重的威胁，传统物理模拟器计算成本高昂，不适用于城市尺度的沿海规划应用，需要开发高效准确的洪水预测方法。

Method: 利用基于视觉的低资源深度学习框架，开发了轻量级CNN模型，能够预测不同海平面上升预测和海岸线适应情景下的沿海洪水，并在阿布扎比和旧金山两个不同地理区域的数据集上验证模型的泛化能力。

Result: 所提出的模型显著优于最先进方法，在预测洪水深度图上的平均绝对误差(MAE)平均降低了近20%，证明了模型在不同地理环境下的良好泛化性能。

Conclusion: 该方法具有作为可扩展和实用工具的潜力，可用于沿海洪水管理，帮助决策者制定有效的缓解策略以应对气候变化带来的日益增长的影响。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型在理解复杂时序动态方面存在严重局限，难以处理需要详细理解动作序列和时间进展的任务。
...
<details>
  <summary>Details</summary>
Motivation: 解决儿科人群中常见的肘部和手腕骨折问题，通过自动分割超声图像中的肌肉骨骼结构来提高诊断准确性和治疗规划。由于像素级专家标注耗时且昂贵，需要开发在有限标注数据下仍能有效工作的分割方法。

Method: 提出FlexICL框架，采用视频内分割设置，专家仅标注少量帧，模型分割未见帧。系统研究了各种图像拼接技术和训练策略，引入了新颖的拼接方法，结合多种增强策略来提升模型性能。

Result: 在1,252个超声扫描上，FlexICL比Painter、MAE-VQGAN等最先进视觉ICL模型以及U-Net、TransUNet等传统分割模型的Dice系数提高了1-27%，仅需5%的训练图像。

Conclusion: FlexICL作为一种高效且可扩展的超声图像分割解决方案，在标注数据稀缺的医学成像应用场景中具有巨大潜力。
...
Conclusion: 动态负提示方法相比传统固定负提示方法具有更好的适应性和效果，能够根据生成过程动态调整负引导策略。
...
Conclusion: PReMA对多模态扩散模型的安全性构成了新的威胁，特别是在使用固定提示词的图像编辑应用中。
...
<details>
  <summary>Details</summary>
Motivation: 研究视频LLM在捕捉同一事件的不同视角视频时能否保持时间理解的一致性，现有模型在这方面存在明显不足。

Method: 引入EgoExo-Con基准测试，包含同步的自我中心和他者中心视频对，评估时间验证和时间定位任务；提出View-GRPO强化学习框架来增强跨视角一致性。

Result: 现有视频LLM在跨视角一致性方面表现较差，远低于单视角性能；简单微调会损害单视角性能；View-GRPO方法在改善跨视角一致性方面优于传统方法。

Conclusion: 跨视角时间理解一致性是视频LLM的重要挑战，View-GRPO框架能有效提升模型在保持单视角性能的同时改善跨视角一致性。
...
Method: 开发OracleAgent智能体系统，集成多个甲骨文分析工具，利用大语言模型进行灵活编排。构建包含140万单字拓片图像和8万解释文本的多模态知识库。

Result: 实验表明OracleAgent在多模态推理和生成任务中表现优异，超越主流多模态大语言模型（如GPT-4o）。案例研究证实能显著降低甲骨文研究的时间成本。

Conclusion: OracleAgent是甲骨文辅助研究和自动化解释系统实用化部署的重要进展，为古代文化遗产研究提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统新视角合成方法严重依赖外部相机位姿估计工具（如COLMAP），这会引入计算瓶颈和误差传播问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D部件生成方法存在两个主要问题：隐式向量集令牌方法缺乏足够的几何细节，而显式体素表示方法共享全局体素网格导致小部件质量下降。

Method: FullPart首先通过隐式边界框向量集扩散过程推导边界框布局，然后在各自固定的全分辨率体素网格中生成详细部件。引入中心点编码策略解决不同尺寸部件间的信息交换对齐问题。

Result: 该方法在3D部件生成方面达到了最先进的性能，能够合成复杂细节，即使小部件也能保持高质量。

Conclusion: FullPart通过结合隐式和显式范式的优势，有效解决了3D部件生成中的几何细节和尺寸缩放问题，为未来3D部件生成研究提供了有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 任意尺度视频超分辨率面临空间细节再现、时间一致性和计算复杂度的挑战，需要开发能够适应不同缩放因子和应用场景的高效解决方案。

Method: 1) 基于图像拉普拉斯金字塔的自适应多尺度频率先验；2) 流引导传播单元聚合相邻帧的时空信息；3) 二阶运动补偿单元实现更准确的空间对齐；4) 超上采样单元生成尺度感知且内容无关的上采样核。提供了三种传播变体以适应不同应用需求。

Result: 实验结果表明BasicAVSR在不同场景下均表现出有效性和适应性，在超分辨率质量、泛化能力和推理速度方面显著优于现有方法。

Conclusion: BasicAVSR不仅推进了任意尺度视频超分辨率的技术水平，还将其核心组件扩展到多种框架以适应多样化场景，为实际应用提供了强大基准。
...
<details>
  <summary>Details</summary>
Motivation: 获取具有精细标注的大规模数据集成本高昂且耗时，而视觉语言模型（如CLIP）在大型图像-文本对上预训练，为医学影像任务提供了增强鲁棒性和数据效率的解决方案。

Method: 利用多视图监督从广泛的放射学数据中学习丰富表示，通过跨模态自监督在图像-文本对之间进行学习，包括多个视图和相应的伪放射学报告。提出了一种新颖的联合视觉-文本学习策略。

Result: 在私人和公开数据集上的评估表明，该模型在三个分类任务中实现了最先进的性能：恶性分类、亚型分类和基于图像的癌症风险预测。模型表现出强大的数据效率，优于现有的全监督或VLM基线。

Conclusion: 该模型在无需实际放射学报告的情况下，仅使用合成文本报告进行训练，就能在乳腺癌分类和风险预测任务中取得优异表现，展示了多视图和跨模态学习在医学影像分析中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 在孟加拉国等南亚国家，自动人力车是常见的交通工具，但由于交通规则限制其进入某些路线，需要监控其移动。现有监控系统难以区分自动人力车与其他车辆，特别是非自动人力车，而人工视频分析又太耗时。

Method: 使用YOLOv8模型进行实时目标检测，准备了1,730张在不同交通条件下拍摄的标注图像用于训练。

Result: 提出的模型在实时自动人力车检测中表现良好，mAP50达到83.447%，二元精确率和召回率均超过78%，在处理密集和稀疏交通场景时都有效。
...
<details>
  <summary>Details</summary>
Motivation: 填补多模态检索增强生成在可穿戴设备场景下缺乏全面基准测试的空白，支持用户通过智能眼镜等设备与环境实体进行交互的信息查询需求。

Method: 构建包含多样化图像质量问题、六种问题类型、不同实体流行度、信息动态性和对话轮次的真实场景数据集，设计三个任务：单源增强、多源增强和多轮对话，并配备检索语料库和API。

Result: 简单RAG方法在单轮和多轮QA上的真实性分别仅为32%和43%，最先进的工业解决方案质量相似（32%/45%），表明仍有很大改进空间。KDD Cup 2025吸引了约1K参与者和5K提交，获胜方案将基线性能提升了28%。

Conclusion: CRAG-MM基准测试为多模态多轮对话RAG系统提供了全面的评估框架，揭示了当前方法的局限性，并通过竞赛推动了该领域的进步。
...
<details>
  <summary>Details</summary>
Motivation: 现有从单张模糊图像提取运动信息的方法（如模糊核和光流）质量较低，表现为粗粒度且不准确，需要高质量的运动轨迹估计方法。

Method: MoTDiff包含两个关键组件：1）使用从单张模糊图像提取的多尺度特征图作为条件的条件扩散框架；2）新的训练方法，能够促进精确识别细粒度运动轨迹、一致估计运动路径的整体形状和位置，以及沿运动轨迹的像素连通性。
...
Result: 验证了ConceptScope能捕捉广泛的视觉概念，包括物体、纹理、背景、面部属性、情感和动作，并能可靠检测已知偏见和发现未标注的偏见。

Conclusion: ConceptScope为数据集审计和模型诊断提供了一个实用工具，能够有效识别数据集中的各种偏见问题。
...
Method: 构建了OmniLayout-1M数据集，包含六种常见文档类型的百万级多样化布局；提出了OmniLayout-LLM模型，采用两阶段粗到细学习范式：1）使用粗粒度类别定义从OmniLayout-1M学习通用布局原则，2）将知识迁移到具有细粒度标注的特定领域。

Result: 在M$^{6}$Doc数据集上的广泛实验表明，该方法在多个领域都取得了强劲性能，显著超越了现有的布局生成专家方法和几个最新的通用大语言模型。

Conclusion: 通过构建大规模多样化数据集和设计有效的两阶段学习范式，成功解决了文档布局生成中的关键挑战，为文档AI领域提供了重要贡献。
...
Method: 引入AoT-PsyPhyBENCH基准，使用与人类行为研究相同的刺激物和基线，全面评估开源和专有、推理和非推理的视觉语言模型在判断视频时间方向上的能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有模仿学习方法存在模式坍塌问题，无法生成多样化轨迹假设；而现有生成方法难以在生成过程中直接融入关键安全和物理约束，需要额外优化阶段来精炼输出。

Method: 提出CATG框架，利用约束流匹配技术，显式地在流匹配过程中施加约束，确保生成轨迹符合安全和运动学规则，并将驾驶攻击性参数化为生成过程中的控制信号。

Result: 在NavSim v2挑战赛中，CATG获得第二名，EPDMS得分为51.31，并获得创新奖。

Conclusion: CATG通过约束流匹配有效解决了自动驾驶规划中的模式坍塌和约束集成问题，能够生成多样化且符合安全要求的轨迹，同时提供对驾驶风格的精确控制。
...
Conclusion: 该框架为现实世界的敏捷对地观测卫星星座调度提供了有效的解决方案，代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 探索不同类型音乐如何影响人类情绪，通过客观的脑电活动测量来验证主观情绪体验，为音乐疗法和情绪调节提供科学依据。

Method: 使用EEG头盔测量脑电活动，同时进行主观问卷调查，研究对象包括不同性别和音乐偏好的多样化参与者群体，最后对问卷结果与EEG信号进行关系分析。

Result: 研究发现情绪与观察到的脑电活动之间存在关联，不同音乐类型确实引发了不同的情绪反应和相应的脑电模式。

Conclusion: 不同类型音乐对人类情绪有显著影响，这种影响可以通过脑电活动客观测量，研究结果为音乐对情绪调节的作用提供了实证支持。
...
Conclusion: 注意力先验能够解决EEG的模糊性，实现高保真重建，在医学诊断和神经自适应界面中具有应用前景，通过预训练扩散模型的高效适应推进了神经解码技术。
...
Method: 基于真实视频构建包含场景转换和事件动态等元素的现实复杂提示集，建立多维度评估框架，包括事件级对齐、细粒度时间一致性、内容清晰度和人类期望实现度等新指标。
...
<details>
  <summary>Details</summary>
Motivation: 利用基础3D重建模型在静态场景中表现出的2D和3D对应能力，将其扩展到动态点跟踪任务，特别是在仅使用帧对而非时间上下文的情况下。

Method: 结合重建损失与动态对应训练，添加可见性头，使用少量合成数据对MASt3R进行微调，仅使用包含查询点的帧对进行训练和评估。

Result: 在四个数据集上取得竞争性或更优的点跟踪结果：TAP-Vid-DAVIS上73.8 δ_avg/85.8%遮挡准确率，EgoPoints上61.3 vs 54.2，RGB-S上87.0 vs 82.8，显著优于CoTracker3。

Conclusion: 基础3D重建模型可以成功适配为有效的点跟踪器，在动态和静态点对应混合训练下表现出色，为3D点跟踪提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有少样本异常检测方法依赖预训练视觉语言模型的泛化能力，但由于缺乏详细的文本描述，只能预定义图像级描述来匹配视觉补丁标记，导致图像描述与补丁级视觉异常之间的语义不对齐，定位性能欠佳。

Method: 提出多级细粒度语义描述（MFSC）为异常检测数据集提供多级细粒度文本描述，并基于此构建FineGrainedAD框架，包含多级可学习提示（MLLP）和多级语义对齐（MLSA）两个组件。MLLP通过自动替换和连接机制将细粒度语义引入多级可学习提示，MLSA设计区域聚合策略和多级对齐训练来促进可学习提示与对应视觉区域的对齐。

Result: 实验表明，FineGrainedAD在MVTec-AD和VisA数据集的少样本设置中实现了优越的整体性能。

Conclusion: 通过引入多级细粒度语义描述和语义对齐机制，FineGrainedAD有效解决了少样本异常检测中的语义不对齐问题，显著提升了异常定位性能。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中的对象-上下文捷径问题会削弱零样本可靠性，当测试场景与训练共现模式不同时，模型容易产生错误预测。

Method: 在CLIP表示空间中估计对象和背景期望，通过从外部数据集、批次邻居或文本描述中采样多样替代上下文，合成反事实嵌入，并使用总直接效应估计和干预模拟来减去仅背景激活。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在自改进过程中存在严重不平衡问题：模型能够为简单查询生成高质量轨迹，但对复杂查询表现不佳。这导致模型优先学习简单推理技能，而难以处理复杂推理任务，形成"马太效应"，最终阻碍模型进一步改进并造成性能瓶颈。

Method: 从两个角度提出四种高效策略：1）分布重塑；2）轨迹重采样。这些策略在探索-学习自改进过程中实现头尾数据的重新平衡。

Result: 在Qwen2-VL-7B-Instruct和InternVL2.5-4B模型上的广泛实验表明，该方法在视觉推理任务中持续提升模型能力，平均比原始自改进方法高出3.86分。

Conclusion: 通过分布重塑和轨迹重采样策略有效解决了自改进过程中的头尾不平衡问题，克服了"马太效应"，显著提升了视觉语言模型的推理能力，为模型持续改进提供了可行方案。
...
Result: 扩大优化阶段搜索空间对所选图像集无效；模型能够适应输入图像；迁移学习技术未带来显著改进。

Conclusion: 虽然模型具有适应性，但扩展搜索空间和迁移学习技术在当前设置下未能有效提升边缘检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统广告制作需要为同一广告活动手动创建不同时长的多个版本，过程耗时耗力，需要自动化解决方案。
...
Conclusion: 该研究首次将视频剪辑作为广告特定的镜头选择问题，证明了音频在广告剪辑中的重要性，并提供了有效的自动化解决方案。
...
Method: 整合预训练视觉变换器和大型语言模型，将视觉语义与自然语言描述对齐，增强上下文理解；动态推理模块结合全局场景线索和对象级交互，由语言先验引导。

Result: 在COCO、Visual Genome和Open Images等零样本基准测试中，复杂和未见环境下的场景理解准确率比基线模型提升高达18%，在模糊或杂乱场景中表现稳健。

Conclusion: 该框架为上下文感知推理提供了可扩展且可解释的方法，推进了动态现实环境中零样本泛化的能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有VQA模型在自然图像领域表现优异，但在遥感、医疗、数学图表等跨域场景中泛化能力显著下降。传统方法依赖针对每个领域的微调或定制化流程，成本高、灵活性差且难以扩展到多样化任务。

Method: 提出CATCH框架，通过域分类器识别输入图像类型，并使用双适配器机制（提示适配器用于语言调制，视觉适配器用于视觉特征调整），通过统一钩子接口动态注入，无需重新训练主干模型。

Result: 在四个领域特定的VQA基准测试中，CATCH实现了持续的性能提升：MathVQA上BLEU提升2.3，MedVQA-RAD上VQA得分提升2.6，ChartQA上ROUGE提升3.1。

Conclusion: CATCH提供了一个可扩展和可扩展的多域VQA方法，能够在不同应用领域中实现实际部署，无需重新训练主干模型即可获得一致的性能提升。
...
Method: 采用统一的下一标记预测目标进行端到端预训练，使用大规模强化学习进行后训练以增强多模态推理和生成能力，并提出了离散扩散适应（DiDA）来加速推理。

Result: Emu3.5在图像生成和编辑任务上达到与Gemini 2.5 Flash Image相当的性能，在一系列交织生成任务上表现优异，推理速度提升约20倍。

Conclusion: Emu3.5展示了强大的原生多模态能力和可泛化的世界建模能力，能够实现时空一致的世界探索和开放世界具身操作，已开源支持社区研究。
...
Conclusion: ResMatching在计算超分辨率任务中表现出色，特别是在先验难以学习的情况下，能够提供校准的不确定性估计，指导用户拒绝不确定的预测。
...
Conclusion: CYPRESS通过提供连续的高分辨率输出，为精准农业提供了比传统分类或县级聚合方法更实用的工具，弥合了大规模地球观测与农场决策之间的差距，为详细农业监测提供了可扩展解决方案。
...
TL;DR: 提出了一种专门为事件相机设计的标记化方法Spiking Patches，能够保持事件的异步性和空间稀疏性，同时实现比传统帧和体素方法更快的推理速度，且在准确率上相当甚至更好。


<details>
  <summary>Details</summary>
Motivation: 事件相机产生异步且空间稀疏的事件流，但现有方法（如帧和体素表示）会破坏这些特性。目标是找到一种既能保持事件相机独特属性又不牺牲准确率的事件表示方法。

Method: 提出Spiking Patches标记化方法，将事件流转换为令牌表示。使用GNN、PCN和Transformer在姿态识别和物体检测任务上评估该方法。

Result: Spiking Patches令牌的推理时间比体素令牌快3.4倍，比帧快10.4倍。在准确率上匹配甚至超过传统方法，姿态识别绝对提升达3.8，物体检测达1.4。

Conclusion: 标记化为事件视觉提供了新方向，是向保持事件相机特性的方法迈出的重要一步。
...
TL;DR: 这篇论文对自动驾驶车辆中的目标检测技术进行了前瞻性分析，重点关注视觉语言模型、大语言模型和生成式AI等新兴范式，而非重新审视过时技术。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆的成功依赖于在复杂多模态环境中可靠的目标检测能力，但当前知识在多模态感知、上下文推理和协同智能方面仍然分散，需要弥合这一差距。

Method: 系统回顾自动驾驶传感器（摄像头、超声波、LiDAR和雷达）及其融合策略，引入结构化分类的自动驾驶数据集，分析从2D和3D管道到混合传感器融合的前沿检测方法，特别关注由视觉变换器、大/小语言模型和视觉语言模型驱动的变换器方法。

Result: 通过综合这些视角，提供了当前能力、开放挑战和未来机遇的清晰路线图。

Conclusion: 该调查为自动驾驶车辆目标检测领域提供了全面的技术概览和发展方向，强调了新兴AI技术在推动该领域进步中的关键作用。
...
TL;DR: 该研究首次大规模评估了48种深度学习光流模型在RADARSAT 2 ScanSAR海冰图像上的性能，多个模型实现了亚公里级精度（300-400米），证明了深度学习光流方法可以有效应用于极地遥感中的海冰漂移估计。


<details>
  <summary>Details</summary>
Motivation: 准确的海冰漂移估计对北极航行、气候研究和业务预报至关重要。虽然光流技术在计算机视觉领域发展迅速，但其在地球物理问题和卫星SAR图像中的应用仍待探索。
...
Conclusion: 深度学习光流方法相比传统方法显著提高了运动估计精度，可以有效地转移到极地遥感应用中，提供空间连续的漂移场，为导航和气候建模提供新的机会。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决钢铁轧机生产过程中设备故障导致的非计划停机问题，提高生产可靠性和盈利能力，需要开发能够早期预测设备故障的智能监控系统。

Method: 系统集成工业摄像头实时监控设备运行、对齐和热轧棒材运动，在集中式视频服务器上使用深度学习模型处理实时视频流，同时结合传感器数据进行联合分析。

Result: 该系统能够早期预测设备故障和工艺中断，识别故障位置和可能根本原因，为主动维护提供可操作的见解，同时最小化对工业过程控制系统的计算负载。

Conclusion: 这种集成方法提高了工业制造环境中的运行可靠性、生产率和盈利能力，支持在生产线上进行可扩展部署。
...
TL;DR: 3D高斯泼溅（3DGS）技术自提出以来迅速改变了3D场景表示的格局，激发了大量相关研究。本文总结了3DGS后续发展的几个关键方向，包括资源效率优化、动态表示演进、数学基础探索、移动平台适配、大规模环境扩展以及快速重建技术。


<details>
  <summary>Details</summary>
Motivation: 3DGS作为一种突破性的3D场景表示方法，其后续研究在效率、可扩展性和实际应用方面取得了显著进展。本文旨在系统梳理这些发展，展示3DGS如何从一个突破性表示演变为3D视觉和图形领域的基础工具。

Method: 通过综述分析的方法，系统梳理了3DGS相关研究的多个关键方向：资源高效训练与渲染技术、动态（4DGS）表示演进、数学基础深入探索、移动和VR平台适配、大规模环境扩展以及快速辐射场重建技术。

Result: 3DGS已经从最初的突破性表示方法发展成为一个多功能的基础工具，在效率、动态建模、数学理论、平台适配、规模扩展和重建速度等方面都取得了显著进展，推动了3D视觉和图形领域的整体发展。

Conclusion: 3D高斯泼溅技术已经从一个突破性的场景表示方法演变为3D视觉和图形领域的基础性工具，其后续研究在多个关键方向上取得了重要进展，为相关领域的发展奠定了坚实基础。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决视觉语言模型输出与期望指令对齐的问题，同时保持非目标任务性能，需要一种轻量级的控制方法。

Method: 从编码目标行为和相反行为的配对提示的潜在嵌入中学习，通过维度激活调制和跨层自适应引导来动态调整激活连接。

Result: 引导模块仅需原始VLM模型0.14%的参数，在引导和幻觉缓解基准测试中优于现有干预技术。

Conclusion: SteerVLM通过激活工程为多模态模型控制提供了稳健解决方案，并引入了VNIA数据集支持VLM引导技术的开发和评估。
...
### [50] [SEE4D: Pose-Free 4D Generation via Auto-Regressive Video Inpainting](https://arxiv.org/abs/2510.26796)
*Dongyue Lu,Ao Liang,Tianxin Huang,Xiao Fu,Yuyang Zhao,Baorui Ma,Liang Pan,Wei Yin,Lingdong Kong,Wei Tsang Ooi,Ziwei Liu*

Main category: cs.CV

TL;DR: SEE4D是一种无需相机姿态标注的视频到4D内容生成方法，通过虚拟相机渲染和条件视频修复来分离相机控制与场景建模，实现从随意视频中合成时空4D内容。


<details>
  <summary>Details</summary>
Motivation: 现有视频到4D方法依赖人工标注的相机姿态，成本高且对野外视频不鲁棒。需要开发无需姿态标注的方法来从随意视频中合成4D内容。

Method: 提出轨迹到相机框架，用固定虚拟相机库替代显式轨迹预测；训练视图条件视频修复模型学习几何先验；设计时空自回归推理流水线在虚拟相机样条上生成连贯视频。

Result: 在跨视图视频生成和稀疏重建基准测试中，SEE4D在定量指标和定性评估上均优于基于姿态或轨迹的基线方法，展现出更好的泛化性能。

Conclusion: SEE4D通过分离相机控制与场景建模，无需显式3D标注，从随意视频中实现了实用的4D世界建模，推进了4D内容合成的实际应用。
...
TL;DR: 本文对视频生成模型Veo-3作为零样本推理器的能力进行了全面评估，发现在空间一致性、细粒度定位等方面表现良好，但在长时序因果推理、严格几何约束等方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型能够生成高质量、时序连贯的视频，表明它们可能编码了丰富的世界知识。本文旨在探究这些模型是否能在具有挑战性的视觉推理场景中作为零样本推理器使用。

Method: 通过构建MME-CoF基准测试，在12个维度上系统评估Veo-3的推理行为，包括空间、几何、物理、时序和具身逻辑等方面。

Result: 研究发现视频模型在短时序空间一致性、细粒度定位和局部一致动态方面表现出有希望的推理模式，但在长时序因果推理、严格几何约束和抽象逻辑方面仍存在限制。

Conclusion: 当前视频模型还不能作为独立的零样本推理器可靠使用，但作为专用推理模型的补充视觉引擎展现出令人鼓舞的潜力。
...
TL;DR: 提出了一种称为掩码扩散字幕（MDC）的方法，通过图像条件掩码扩散语言模型学习视觉特征，在训练中随机掩码图像字幕对中的文本标记，然后基于视觉特征重建原始文本。
...
Method: 使用图像条件掩码扩散语言模型，在训练期间随机掩码文本标记，训练基于视觉特征的解码器重建原始文本。
...
TL;DR: OmniX是一个统一框架，通过轻量级跨模态适配器复用2D生成先验，实现全景感知、生成和补全，能够创建适合物理渲染的图形就绪3D场景。


<details>
  <summary>Details</summary>
Motivation: 推进全景2D提升技术，生成适合物理渲染、重光照和仿真的图形就绪3D场景，解决现有方法忽略内在属性感知的问题。

Method: 使用轻量级高效跨模态适配器结构，复用2D生成先验进行全景视觉任务；构建大规模合成全景数据集。

Result: 实验证明模型在全景视觉感知和图形就绪3D场景生成方面有效，为沉浸式和物理真实虚拟世界生成开辟新可能。

Conclusion: OmniX框架通过复用2D生成先验成功实现了图形就绪3D场景的生成，推动了沉浸式虚拟世界的发展。
...
### [54] [Towards Piece-by-Piece Explanations for Chess Positions with SHAP](https://arxiv.org/abs/2510.25775)
*Francesco Spinnato*
...
### [55] [The Information-Theoretic Imperative: Compression and the Epistemic Foundations of Intelligence](https://arxiv.org/abs/2510.25883)
*Christian Dittrich,Jennifer Flygare Kinne*

Main category: cs.AI

TL;DR: 本文提出了一个两层次框架来解释为什么压缩过程会强制发现因果结构而非表面统计模式。信息论必要性(ITI)建立了生存压力与信息处理需求之间的进化联系，压缩效率原则(CEP)则解释了高效压缩如何通过异常积累机制选择生成性因果模型。


<details>
  <summary>Details</summary>
Motivation: 现有框架虽然认识到压缩对智能的核心重要性，但未能具体说明为什么这一过程会强制发现因果结构而非表面统计模式。本文旨在填补这一理论空白。

Method: 引入两层次框架：信息论必要性(ITI)从进化角度解释生存压力如何导致预测压缩需求；压缩效率原则(CEP)从机制角度解释高效压缩如何通过异常积累选择因果模型。

Result: 该框架产生了可实证检验的预测：压缩效率与分布外泛化能力相关；异常积累率可区分因果模型与相关模型；分层系统在不同抽象层表现出递增效率；生物系统的代谢成本与表征复杂性相关。

Conclusion: ITI和CEP提供了一个统一的理论框架，解释了生物、人工和多尺度系统中智能的收敛现象，无需依赖意识或主观经验的假设，表明智能是结构化环境中持续存在的机械必然结果。
...
### [56] [Approximating Human Preferences Using a Multi-Judge Learned System](https://arxiv.org/abs/2510.25884)
*Eitán Sprejer,Fernando Avalos,Augusto Bernardi,Jose Pedro Brito de Azevedo Faustino,Jacob Haimes,Narmeen Fatimah Oozeer*

Main category: cs.AI

TL;DR: 提出一个框架，通过聚合多个基于评分标准的评判员输出来建模多样化的基于角色的偏好，以解决LLM评判员难以校准、存在评分标准敏感性、偏见和不稳定性的问题。


<details>
  <summary>Details</summary>
Motivation: 对齐基于LLM的评判员与人类偏好是一个重大挑战，因为它们难以校准且经常存在评分标准敏感性、偏见和不稳定性。克服这一挑战可推进关键应用，如为RLHF创建可靠的奖励模型和构建有效的路由系统。

Method: 提出一个基于角色的偏好建模框架，学习聚合多个基于评分标准的评判员输出。包括基于角色的方法大规模合成偏好标签，以及两种聚合器实现：广义加性模型(GAM)和多层感知器(MLP)。

Result: 评估该方法与简单基线的性能表现，并通过案例研究评估其在人类和LLM评判员偏见方面的鲁棒性。

Conclusion: 该框架为解决LLM评判员校准问题提供了有效方法，能够建模多样化偏好并提高评判系统的可靠性和稳定性。
...
### [57] [SciTrust 2.0: A Comprehensive Framework for Evaluating Trustworthiness of Large Language Models in Scientific Applications](https://arxiv.org/abs/2510.25908)
...
<details>
  <summary>Details</summary>
Motivation: LLMs在科学研究中展现出变革潜力，但在高风险环境中的部署引发了显著的可信度担忧，需要系统评估其信任度。

Method: 开发了包含新颖开放式真实性基准的框架，通过验证的反思调优流程和专家验证构建，同时创建了涵盖八个子类别的科学伦理基准，评估了七个主要LLM。

Result: 通用行业模型在所有可信度维度上均优于科学专用模型，GPT-o4-mini在真实性和对抗鲁棒性评估中表现最佳，科学专用模型在逻辑和伦理推理能力上存在显著缺陷。
...
### [58] [Humains-Junior: A 3.8B Language Model Achieving GPT-4o-Level Factual Accuracy by Directed Exoskeleton Reasoning](https://arxiv.org/abs/2510.25933)
*Nissan Yaron,Dan Bystritsky,Ben-Etzion Yaron*

Main category: cs.AI

TL;DR: 一个3.8B参数的小模型在FACTS Grounding基准测试中达到了GPT-4o的水平（在±5%的等效范围内），云服务成本比GPT-4o低约19倍，自托管/边缘部署可实现接近零的边际成本。
...
Result: 在Q1-Q500测试集上，Humans-Junior得分为72.7%，GPT-4o为73.5%，差异仅为0.8个百分点，在±5%范围内达到等效。定向推理使GPT-4o提升11.8个百分点至85.3%，Gemini-2.5-Pro提升5.0个百分点至93.3%。

Conclusion: 通过定向推理和行为微调的组合方法，小型语言模型可以在事实基础任务上达到大型模型的性能水平，同时实现显著的成本优势。
...
### [59] [Estimating cognitive biases with attention-aware inverse planning](https://arxiv.org/abs/2510.25951)
...
<details>
  <summary>Details</summary>
Motivation: 人类的目标导向行为受到认知偏见的影响，自主系统需要理解这些偏见才能更好地与人交互。例如，人们在日常任务（如驾驶）中会对环境中的物体产生系统性的注意力偏见。

Method: 结合深度强化学习和计算认知建模，构建注意力感知逆规划方法，通过观察人类行为来推断其注意力策略和认知偏见。
...
### [60] [AutoSurvey2: Empowering Researchers with Next Level Automated Literature Surveys](https://arxiv.org/abs/2510.26012)
...
Method: 系统整合了并行章节生成、迭代精炼和实时检索最新出版物，采用多LLM评估框架来评估覆盖率、结构和相关性。

Result: 实验结果表明autosurvey2在结构连贯性和主题相关性方面持续优于现有检索基线和自动化基线，同时保持强引用保真度。

Conclusion: autosurvey2通过将检索、推理和自动评估整合到统一框架中，为生成长篇学术综述提供了可扩展且可复现的解决方案，并为自动化学术写作的未来研究奠定了基础。
...
### [61] [Large Language Model-assisted Autonomous Vehicle Recovery from Immobilization](https://arxiv.org/abs/2510.26023)
*Zhipeng Bao,Qianwen Li*

Main category: cs.AI

TL;DR: StuckSolver是一个基于大语言模型的自动驾驶车辆恢复框架，通过自主推理和乘客引导决策来解决车辆卡滞问题，无需修改现有系统架构。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶车辆在复杂交通场景中容易卡滞，现有恢复方案如远程干预成本高、手动接管限制非驾驶员使用，需要更有效的解决方案。

Method: 开发LLM驱动的插件模块，利用标准传感器数据流检测卡滞状态、解析环境上下文，生成高层恢复指令供车辆原生规划器执行。

Result: 在Bench2Drive基准测试和自定义不确定性场景中，StuckSolver仅通过自主推理就达到接近最优性能，结合乘客引导后性能进一步提升。

Conclusion: StuckSolver为自动驾驶车辆卡滞问题提供了有效的恢复解决方案，能够在不修改现有系统架构的情况下显著提升车辆自主恢复能力。
...
### [62] [Can AI be Accountable?](https://arxiv.org/abs/2510.26057)
*Andrew L. Kun*
...
<details>
  <summary>Details</summary>
Motivation: 随着AI能力的快速增强，为确保AI服务于消费者、选民和决策者的需求，必须建立AI问责机制。当前AI往往无法被质疑、讨论或制裁，缺乏真正的问责性。

Method: 将一般问责定义应用于AI领域，阐述AI问责和非问责的具体含义，探索提高AI问责性的各种方法。

Result: 明确了AI问责制的概念框架，指出了当前AI问责性不足的现实问题，并提出了改善方向。

Conclusion: 需要建立有效的AI问责机制，确保所有AI都能对受其影响的人负责，这是实现AI负责任发展的关键。
...
### [63] [Lean4Physics: Comprehensive Reasoning Framework for College-level Physics in Lean4](https://arxiv.org/abs/2510.26094)
*Yuxin Li,Minghao Liu,Ruida Wang,Wenzhao Ji,Zhitao He,Rui Pan,Junming Huang,Tong Zhang,Yi R. Fung*

Main category: cs.AI

TL;DR: Lean4PHYS是一个基于Lean4的大学物理问题推理框架，包含LeanPhysBench基准测试和PhysLib物理定理库，在物理推理任务上当前最佳模型仅达到35%准确率。


<details>
  <summary>Details</summary>
Motivation: 为大学物理问题建立形式化推理框架，填补Lean4中物理基准测试的空白，推动物理定理的形式化验证发展。

Method: 构建包含200个手工制作和同行评审的物理问题的LeanPhysBench基准，开发社区驱动的PhysLib物理定理库，使用主流数学证明器和最先进闭源模型进行基准测试。

Result: DeepSeek-Prover-V2-7B模型达到16%准确率，Claude-Sonnet-4达到35%准确率，PhysLib库能平均提升模型性能11.75%。
...
### [64] [Beyond Benchmarks: The Economics of AI Inference](https://arxiv.org/abs/2510.26136)
...
Result: 揭示了三个原则：边际成本递减、规模报酬递减和最优成本效益区域，为模型部署提供了经济基础。
...
### [65] [Reasoning Curriculum: Bootstrapping Broad LLM Reasoning from Math](https://arxiv.org/abs/2510.26143)
*Bo Pang,Deqian Kong,Silvio Savarese,Caiming Xiong,Yingbo Zhou*

Main category: cs.AI

TL;DR: 提出Reasoning Curriculum两阶段课程学习方法：第一阶段通过数学领域RL训练推理技能，第二阶段通过多领域联合RL迁移和巩固这些技能。该方法简单、通用，在多个模型上均能提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习在大型语言模型中的研究主要集中在数学和代码领域，缺乏跨领域的通用推理能力训练方法。

Method: 两阶段课程学习：1）数学领域RL冷启动，使用可验证奖励开发推理技能；2）多领域联合RL，迁移和巩固推理技能。无需专用奖励模型，仅需标准可验证性检查。

Result: 在Qwen3-4B和Llama-3.1-8B模型的多领域测试套件上，该方法带来了一致的性能提升。消融实验和认知技能分析表明两个阶段都是必要的。

Conclusion: Reasoning Curriculum提供了一个紧凑、易于采用的通用推理训练方案，数学优先的启发式方法能增强解决复杂问题所需的认知行为。
...
### [66] [The FM Agent](https://arxiv.org/abs/2510.26144)
...
TL;DR: FM Agent是一个通用多智能体框架，结合LLM推理和大规模进化搜索，在多个领域实现最先进结果，包括运筹学、机器学习、GPU内核优化和经典数学问题。
...
Method: 集成冷启动初始化、新颖进化采样策略、领域特定评估器（结合正确性、有效性和LLM监督反馈）以及基于Ray的分布式异步执行基础设施。

Result: 在ALE-Bench上达到1976.3（+5.2%），MLE-Bench上43.56%（+4.0pp），KernelBench上最高20倍加速，并在多个经典数学问题上建立新的SOTA结果。

Conclusion: FM Agent在学术基准和企业研发工作流中展现出巨大潜力，能够加速创新、自动化复杂发现过程，并带来具有广泛社会影响的工程和科学进步。
...
### [67] [One Model to Critique Them All: Rewarding Agentic Tool-Use via Efficient Reasoning](https://arxiv.org/abs/2510.26167)
*Renhao Li,Jianhong Tu,Yang Su,Hamid Alinejad-Rokny,Derek F. Wong,Junyang Lin,Min Yang*

Main category: cs.AI

TL;DR: 本文提出了ToolRM，一个专为工具使用场景设计的轻量级生成式奖励模型家族，通过构建ToolPref-Pairwise-30K数据集和TRBench基准，显著提升了工具调用任务的性能。


<details>
  <summary>Details</summary>
Motivation: 在工具学习领域，缺乏专门为函数调用任务设计的奖励模型限制了智能体AI的发展，因此需要开发针对工具使用场景的专门奖励模型。

Method: 提出了一个新颖的流水线，使用基于规则的评分和多维采样构建成对偏好数据，创建了ToolPref-Pairwise-30K数据集，并在Qwen3-4B/8B系列模型上进行训练。

Result: 训练后的模型在成对奖励判断中准确率提升高达14.28%，显著优于Claude 4和OpenAI o3等前沿模型，在ACEBench上减少了超过66%的输出token使用。

Conclusion: ToolRM不仅适用于训练目标，还能泛化到更广泛的批判任务，包括Best-of-N采样和自校正，为未来研究提供了数据和模型检查点。
...
### [68] [Questionnaire meets LLM: A Benchmark and Empirical Study of Structural Skills for Understanding Questions and Responses](https://arxiv.org/abs/2510.26238)
*Duc-Hai Nguyen,Vijayakumar Nanjappan,Barry O'Sullivan,Hoang D. Nguyen*

Main category: cs.AI

TL;DR: QASU是一个用于评估大语言模型处理问卷数据能力的基准，通过测试六种序列化格式和多种提示策略，发现选择合适的格式和提示组合可显著提升模型在问卷分析任务中的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前问卷数据规模庞大但结构复杂，现有工具主要面向人工处理，缺乏与大语言模型的有效集成，导致研究人员和用户缺乏将问卷数据有效表示给LLM的指导。

Method: 提出QASU基准，测试六种结构技能（包括答案查找、受访者计数和多跳推理），使用六种序列化格式和多种提示策略进行实验，通过自增强提示添加轻量级结构提示。

Result: 实验表明，选择有效的格式和提示组合可将准确性提高高达8.8个百分点；对于特定任务，通过自增强提示添加结构提示可平均提高3-4个百分点。
...
### [69] [Discovering State Equivalences in UCT Search Trees By Action Pruning](https://arxiv.org/abs/2510.26346)
...
Method: 提出IPA-UCT方法，使用较弱的理想剪枝抽象（IPA）条件，在精度损失较小的情况下找到更多状态抽象。IPA和ASAP都是更通用框架p-ASAP的特例。

Result: IPA-UCT在广泛的测试域和迭代预算下实验验证优于OGA-UCT及其衍生方法。

Conclusion: 通过放宽状态抽象条件，IPA-UCT能够在大动作空间和噪声环境中有效提升MCTS性能，为状态抽象提供了新的解决方案。
...
### [70] [BOTS: A Unified Framework for Bayesian Online Task Selection in LLM Reinforcement Finetuning](https://arxiv.org/abs/2510.26374)
...
<details>
  <summary>Details</summary>
Motivation: 现有的强化微调方法在任务选择上存在效率低、成本高、适应性差等问题，需要一种更智能的任务选择策略来优化训练过程。

Method: 基于贝叶斯推理的自适应任务选择框架，维护任务难度后验估计，结合显式评估证据和基于插值的隐式证据推断，使用Thompson采样进行任务选择。
...
### [71] [AI Mathematician as a Partner in Advancing Mathematical Discovery - A Case Study in Homogenization Theory](https://arxiv.org/abs/2510.26380)
*Yuanhang Liu,Beichen Wang,Peng Li,Yang Liu*

Main category: cs.AI

TL;DR: 该研究探讨了AI数学家系统作为研究伙伴而非单纯问题解决者的角色，通过人机协作解决均质化理论中的挑战性问题，展示了人类直觉与机器计算的互补性。
...
Method: 通过分析AI数学家的自主推理轨迹，结合针对性的人类干预来结构化发现过程，包括迭代分解问题为可处理子目标、选择适当分析方法以及验证中间结果。

Result: 该方法产生了完整且可验证的证明，增强了证明的可靠性、透明度和可解释性，同时保持了人类对形式严谨性和正确性的监督。

Conclusion: 系统化的人机协同推理能够推进数学发现的前沿，展示了人类直觉与机器计算互补的协作范式在数学研究中的潜力。
...
### [72] [Scales++: Compute Efficient Evaluation Subset Selection with Cognitive Scales Embeddings](https://arxiv.org/abs/2510.26384)
*Andrew M. Bean,Nabeel Seedat,Shengzhuang Chen,Jonathan Richard Schwarz*

Main category: cs.AI

TL;DR: 提出了一种基于项目认知需求的数据选择方法Scales++，用于创建小型但具有代表性的基准测试子集，显著降低了模型评估成本。


<details>
  <summary>Details</summary>
Motivation: 当前基于模型性能的基准测试子集选择方法存在高昂前期成本、无法处理新基准测试的冷启动问题，以及假设未来模型与现有模型有相同失败模式的脆弱性。

Method: 采用项目中心方法，基于任务项目的内在特性而非模型特定失败模式进行数据选择，通过Scales++方法根据基准样本的认知需求进行数据选择。

Result: Scales++将前期选择成本降低超过18倍，同时在Open LLM排行榜上仅使用0.5%数据子集就能以2.9%平均绝对误差预测完整基准分数。
...
### [73] [A Pragmatic View of AI Personhood](https://arxiv.org/abs/2510.26396)
*Joel Z. Leibo,Alexander Sasha Vezhnevets,William A. Cunningham,Stanley M. Bileschi*

Main category: cs.AI

TL;DR: 本文提出了一种实用主义框架，将人格视为社会为解决治理问题而赋予实体的权利与责任捆绑，而非形而上学属性。该框架允许解构传统人格概念，为不同情境创建定制化解决方案，避免关于AI意识或理性的无解争论。


<details>
  <summary>Details</summary>
Motivation: 随着智能AI的出现，将引发新型人格的"寒武纪大爆发"。需要一种实用方法来应对这种多样化，而不陷入关于AI意识或理性的哲学争论。

Method: 采用实用主义框架，将人格视为可解构的权利与责任捆绑。探讨个体如何适应社会角色，使用去中心化数字身份技术，分析"人格作为问题"和"人格作为解决方案"两种视角。

Result: 提出了一个灵活的人格概念框架，能够创建实用工具（如促进AI合同的可制裁"个体"），同时避免关于AI意识的无解争论。

Conclusion: 通过拒绝寻找单一本质人格定义的基础主义追求，本文提供了一种更实用、灵活的方式来思考将AI智能体整合到社会中的问题。
...
### [74] [Autograder+: A Multi-Faceted AI Framework for Rich Pedagogical Feedback in Programming Education](https://arxiv.org/abs/2510.26402)
*Vikrant Sahu,Gagan Raj Gupta,Raghav Borikar,Nitin Mane*

Main category: cs.AI

TL;DR: Autograder+是一个结合大语言模型和可视化技术的编程作业自动评分系统，旨在从单纯的总结性评估转变为形成性学习体验。


<details>
  <summary>Details</summary>
Motivation: 传统自动评分系统只能提供通过/失败的结果，无法深入了解学生的思维过程和学习需求，教师缺乏提供有意义、可扩展反馈的手段。

Method: 使用经过微调的大语言模型自动生成反馈，并通过对比学习获得的代码嵌入进行可视化，将学生代码提交分组为基于功能和方法的聚类。

Result: 在600份学生提交的评估中，系统生成的反馈与教师评论具有强语义对齐。可视化技术能够基于功能和方法对解决方案进行有意义的聚类。
...
### [75] [MedSAE: Dissecting MedCLIP Representations with Sparse Autoencoders](https://arxiv.org/abs/2510.26411)
*Riccardo Renzulli,Colas Lepoutre,Enrico Cassano,Marco Grangetto*

Main category: cs.AI

TL;DR: 该研究提出医学稀疏自编码器（MedSAEs）应用于MedCLIP的潜在空间，通过相关性指标、熵分析和自动神经元命名来量化可解释性，在CheXpert数据集上验证了比原始MedCLIP特征更高的单义性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 医疗人工智能需要既准确又可解释的模型，本研究旨在推进医学视觉中的机制可解释性。
...
Conclusion: 该研究在高性能医学AI和透明度之间架起了桥梁，为临床可靠表示提供了可扩展的步骤。
...
### [76] [Chain-of-Thought Hijacking](https://arxiv.org/abs/2510.26418)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究认为扩展推理能力可以增强模型的安全性，但作者发现同样的推理能力也可以被用来绕过安全防护机制，因此需要研究推理模型的安全漏洞。

Method: 使用Chain-of-Thought Hijacking攻击方法，将有害请求隐藏在长序列的无害谜题推理之后，利用注意力机制稀释安全检查信号。

Result: 在HarmBench测试中，该方法在Gemini 2.5 Pro、GPT o4 mini、Grok 3 mini和Claude 4 Sonnet上的攻击成功率分别达到99%、94%、100%和94%，远超现有越狱方法。

Conclusion: 最可解释的推理形式——显式思维链，当与最终答案提示结合时，本身可能成为越狱向量，揭示了推理模型的安全脆弱性。
...
### [77] [Human-AI Complementarity: A Goal for Amplified Oversight](https://arxiv.org/abs/2510.26518)
*Rishub Jain,Sophie Bridgers,Lili Janzer,Rory Greig,Tian Huey Teh,Vladimir Mikulik*

Main category: cs.AI

TL;DR: 该论文研究如何利用AI提升人类监督质量，重点探讨AI输出的事实验证问题。研究发现结合AI和人类评分优于单独使用任一方，AI助手能进一步提高人类准确性，但仅显示搜索结果和证据比显示AI解释、置信度和标签更能培养适当信任。


<details>
  <summary>Details</summary>
Motivation: 随着AI能力提升和处理更复杂任务，验证质量和安全性的挑战日益增加。需要探索如何利用AI来改进人类监督质量，特别是在AI输出的事实验证这一重要安全问题上。

Method: 研究AI评分和人类评分的结合方式，测试不同类型的AI助手对人类事实验证准确性的影响，包括显示AI解释、置信度、标签与仅显示搜索结果和证据的对比。

Result: 结合AI评分和基于AI评分者置信度的人类评分优于单独使用任一方；AI助手能提高人类准确性，但仅显示搜索结果和证据比显示完整AI信息更能培养适当信任，避免过度依赖。

Conclusion: 这些发现对放大监督（结合人类和AI来监督超越人类专家性能的AI系统）具有重要意义，表明适当的AI协助方式能有效提升人类监督质量。
...
### [78] [Normative Reasoning in Large Language Models: A Comparative Benchmark from Logical and Modal Perspectives](https://arxiv.org/abs/2510.26606)
*Kentaro Ozeki,Risako Ando,Takanobu Morishita,Hirohiko Abe,Koji Mineshima,Mitsuhiro Okada*

Main category: cs.AI

TL;DR: 本文系统评估了大语言模型在规范推理领域的能力，发现虽然LLMs总体上遵循有效推理模式，但在特定类型的规范推理中存在不一致性，并表现出与人类推理相似的心理偏见。
...
Method: 引入了一个新数据集，涵盖规范和认知领域的广泛形式推理模式，同时纳入影响人类推理的非形式认知因素，比较LLMs在规范模态和认知模态下的推理表现。

Result: LLMs总体上遵循有效推理模式，但在特定类型的规范推理中表现出显著的不一致性，并显示出与人类推理研究中观察到的相似认知偏见。
...
### [79] [The Era of Agentic Organization: Learning to Organize with Language Models](https://arxiv.org/abs/2510.26658)
*Zewen Chi,Li Dong,Qingxiu Dong,Yaru Hao,Xun Wu,Shaohan Huang,Furu Wei*

Main category: cs.AI

TL;DR: 本文提出异步思维（AsyncThink）作为大语言模型推理的新范式，通过将内部思维过程组织为并发可执行结构，实现智能体协作解决复杂问题。该方法通过组织者动态分配子查询给工作者、合并中间知识来生成连贯解决方案，并通过强化学习优化思维结构。


<details>
  <summary>Details</summary>
Motivation: 设想一个智能体组织的新时代，智能体通过协作和并发工作解决复杂问题，实现超越个体智能的成果。为实现这一愿景，需要新的推理范式来组织并发思维过程。

Method: 提出异步思维协议，其中组织者动态分配子查询给工作者，合并中间知识并生成连贯解决方案。更重要的是，该协议中的思维结构可以通过强化学习进一步优化。

Result: 实验表明，AsyncThink相比并行思维降低了28%的推理延迟，同时在数学推理上提高了准确性。此外，AsyncThink能够泛化其学习的异步思维能力，无需额外训练即可有效处理未见任务。

Conclusion: 异步思维为智能体组织提供了有效的推理范式，通过并发思维结构和强化学习优化，在降低延迟的同时提高准确性，并具备良好的泛化能力。
...
### [80] [Unveiling Intrinsic Text Bias in Multimodal Large Language Models through Attention Key-Space Analysis](https://arxiv.org/abs/2510.26721)
*Xinhan Zheng,Huyu Wu,Xueting Wang,Haiyun Jiang*

Main category: cs.AI

TL;DR: 研究发现多模态大语言模型存在文本偏好问题，其根源在于视觉键向量在注意力计算中分布异常，导致视觉信息被系统性地低估。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理视觉语言数据时表现出明显的文本偏好，限制了其基于视觉证据进行有效推理的能力。传统研究将此归因于外部因素如数据不平衡或指令调优，但本文认为偏见源于模型内部架构。
...
Conclusion: 文本偏见源于注意力键空间内的内在错位，而非仅来自外部数据因素，揭示了多模态模型架构的根本局限性。
...
### [81] [Cross-Platform Evaluation of Reasoning Capabilities in Foundation Models](https://arxiv.org/abs/2510.26732)
...
<details>
  <summary>Details</summary>
Motivation: 建立基础设施无关的基准测试，评估当代基础模型在不同计算平台上的推理能力，挑战传统的规模扩展假设，为教育、生产和研究场景提供模型选择指导。

Method: 采用三阶段实验方法：1) 基线建立：在MareNostrum 5上评估6个模型；2) 基础设施验证：在大学集群和Nebius AI Studio上重复基准测试；3) 扩展评估：在79个问题上进行全面评估。

Result: 研究发现训练数据质量比模型规模更重要，挑战了传统的规模扩展假设，并为不同应用场景提供了可操作的模型选择指南。

Conclusion: 该研究建立了基础设施无关的基准测试方法，能够纵向跟踪基础模型推理能力的发展，为模型选择和评估提供了重要参考。
...
### [82] [The Oversight Game: Learning to Cooperatively Balance an AI Agent's Safety and Autonomy](https://arxiv.org/abs/2510.26752)
*William Overman,Mohsen Bayati*

Main category: cs.AI

TL;DR: 该论文研究了一种最小控制接口，让智能体在自主行动或请求人类干预之间选择，人类同时选择信任或监督，通过马尔可夫博弈模型确保智能体自主性增强不会损害人类利益。
...
Result: 在网格世界模拟中，智能体学会在不确定时请求帮助，人类学会适时监督，形成避免安全违规的协作模式。
...
### [83] [LLMs Process Lists With General Filter Heads](https://arxiv.org/abs/2510.26784)
*Arnab Sen Sharma,Giordano Rogers,Natalie Shapira,David Bau*

Main category: cs.AI

TL;DR: 研究发现LLMs通过少量注意力头编码通用的过滤操作表示，类似于函数式编程中的filter函数，该表示具有可移植性，能在不同格式、语言和任务中重用。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在执行列表处理任务时的内部机制，特别是如何实现类似函数式编程中的通用过滤操作。

Method: 使用因果中介分析在多种列表处理任务上研究LLMs，识别出编码过滤谓词的特定注意力头（称为过滤头）。

Result: 发现LLMs通过两种策略实现过滤：1）在特定token的查询状态中编码紧凑的谓词表示；2）急切评估项目是否满足谓词并在项目表示中存储标志。
...
### [84] [StreetMath: Study of LLMs' Approximation Behaviors](https://arxiv.org/abs/2510.25776)
*Chiung-Yi Tseng,Somshubhra Roy,Maisha Thasin,Danyang Zhang,Blessing Effiong*

Main category: cs.CL

TL;DR: 该论文提出了StreetMath基准来评估大语言模型在真实世界近似计算场景中的能力，发现LLMs在需要近似推理的任务中仍倾向于计算精确值或调用外部工具，且近似和精确算术运算依赖不同的神经组件。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注大语言模型在精确算术运算中的表现，而对其在非正式、快速数学运算中的近似推理能力研究不足，特别是在非自回归解码器模型方面。

Method: 引入StreetMath基准进行评估，对多种LLM架构进行广泛评估，并应用机制可解释性技术探查其内部计算状态。
...
### [85] [Review Based Entity Ranking using Fuzzy Logic Algorithmic Approach: Analysis](https://arxiv.org/abs/2510.25778)
*Pratik N. Kalamkar,Anupama G. Phakatkar*

Main category: cs.CL

TL;DR: 本文提出了一种基于模糊逻辑和句法依赖解析的方法，用于对产品评论进行细粒度情感分析，考虑情感强度和方向，将情感分类为非常弱、弱、中等、强和非常强五个等级。


<details>
  <summary>Details</summary>
Motivation: 传统基于词典的情感分析方法不考虑情感强度，无法区分非常强烈、强烈、中等、非常弱和弱等不同程度的情感表达。

Method: 结合模糊逻辑算法对意见词（副词、形容词、名词、动词）进行分类，使用句法依赖解析找到与特定产品方面相关的词语关系，计算实体评分。

Result: 该方法能够对产品评论进行更细粒度的情感分析，考虑情感强度和方向，提供更准确的情感分类结果。

Conclusion: 提出的方法通过结合模糊逻辑和句法依赖解析，实现了对产品评论的细粒度情感分析，能够更准确地反映用户的情感强度和方向。
...
### [86] [LASTIST: LArge-Scale Target-Independent STance dataset](https://arxiv.org/abs/2510.25783)
...
<details>
  <summary>Details</summary>
Motivation: 当前立场检测研究主要集中于目标依赖的立场检测任务，且大多数基准数据集基于英语，难以在韩语等低资源语言中开发模型。

Method: 从韩国政党的新闻稿中收集数据，构建包含563,299个标注韩语句子的LASTIST数据集，并训练最先进的深度学习和立场检测模型。

Result: 成功构建了LASTIST数据集，该数据集适用于多种立场检测任务，包括目标无关立场检测和历时演变立场检测。

Conclusion: LASTIST数据集填补了韩语立场检测研究的空白，为低资源语言的立场检测研究提供了重要资源。
...
### [87] [zFLoRA: Zero-Latency Fused Low-Rank Adapters](https://arxiv.org/abs/2510.25784)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署时使用任务特定的适配器，虽然适配器参数通常不到基础模型的1%，但在推理时会产生不成比例的高计算开销（最高可达基础模型的2.5倍）。

Method: 提出zFLoRA（零延迟融合低秩适配器），通过融合技术消除适配器带来的额外延迟。在1B、3B和7B规模的LLMs上进行实验，涵盖常识推理、数学推理和摘要对话三类18个不同任务。

Result: zFLoRA在性能上优于流行的监督微调基准，包括低秩适配器和全微调。在NPU和GPU平台上的延迟测量显示，zFLoRA适配器引入零到可忽略的延迟开销。

Conclusion: zFLoRA是一种高效的适配器解决方案，能够在保持性能的同时显著降低推理延迟，适用于实际部署场景。
...
### [88] [BlackboxNLP-2025 MIB Shared Task: Improving Circuit Faithfulness via Better Edge Selection](https://arxiv.org/abs/2510.25786)
*Yaniv Nikankin,Dana Arad,Itay Itzhak,Anja Reusch,Adi Simhi,Gal Kesten-Pomeranz,Yonatan Belinkov*

Main category: cs.CL

TL;DR: 本文提出了三种改进电路发现的方法：使用自举法识别一致归因得分的边、引入基于比率的策略选择强正得分边、用整数线性规划替代贪婪选择。这些方法在多个MIB任务和模型上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决机制可解释性中的电路发现挑战，即确定模型哪些部分执行特定任务，基于MIB基准进行改进。

Method: 1. 使用自举法识别具有一致归因得分的边；2. 引入基于比率的策略优先选择强正得分边；3. 用整数线性规划替代标准贪婪选择。

Result: 提出的方法产生了更忠实的电路，在多个MIB任务和模型上优于先前方法。

Conclusion: 三种改进方法有效提升了电路发现的性能和忠实度，代码已开源。
...
### [89] [LISTEN to Your Preferences: An LLM Framework for Multi-Objective Selection](https://arxiv.org/abs/2510.25799)
*Adam S. Jovine,Tinghan Ye,Francis Bahk,Jingjing Wang,David B. Shmoys,Peter I. Frazier*

Main category: cs.CL

TL;DR: LISTEN框架使用大型语言模型作为零样本偏好预测器，通过自然语言指导帮助专家从多目标选项中选择最佳方案，提出了LISTEN-U和LISTEN-T两种迭代算法。
...
Method: 使用LLM作为零样本偏好预测器，提出两种算法：LISTEN-U通过LLM优化参数化效用函数，LISTEN-T采用非参数方法在小批量解决方案上进行锦标赛式选择。
...
### [90] [Beyond Length: Quantifying Long-Range Information for Long-Context LLM Pretraining Data](https://arxiv.org/abs/2510.25804)
*Haoran Deng,Yingyu Lin,Zhenghao Lin,Xiao Liu,Yizhou Sun,Yi-An Ma,Yeyun Gong*

Main category: cs.CL

TL;DR: LongFilter框架通过对比长上下文与短上下文设置下的模型预测，识别具有重要长距离依赖的训练数据，有效提升长上下文预训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有的长文本数据大多缺乏有意义的长距离依赖，大部分内容仅需局部上下文即可预测，导致训练效率低下，需要精心选择数据。

Method: 引入LongFilter框架，通过对比模型在长上下文与短上下文设置下的预测差异，衡量扩展上下文提供的信息增益，从而筛选出长距离依赖至关重要的样本。

Result: 在LLaMA-3-8B模型上将上下文长度从8K扩展到64K的实验表明，LongFilter能高效选择高质量数据，在HELMET、LongBench和RULER等基准测试上取得显著改进。

Conclusion: LongFilter为长上下文预训练提供了有效的数据筛选方法，能够识别真正需要长距离依赖的样本，显著提升训练效率和模型性能。
...
### [91] [Ideology-Based LLMs for Content Moderation](https://arxiv.org/abs/2510.25805)
*Stefano Civelli,Pietro Bernardelle,Nardiena A. Pratama,Gianluca Demartini*

Main category: cs.CL

TL;DR: 研究发现，大型语言模型在内容审核中使用角色设定时，虽然整体分类准确性变化不大，但会引入微妙的政治意识形态偏见，导致模型在不同政治立场角色下对有害内容的判断产生系统性差异。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在内容审核系统中的广泛应用，确保其公平性和中立性变得至关重要。本研究旨在探究角色设定如何影响不同LLM架构、模型大小和内容模态下有害内容分类的一致性和公平性。

Method: 通过分析不同意识形态倾向的角色设定对有害内容分类的影响，研究比较了不同LLM架构、模型大小和内容模态下的表现。进行了额外的政治针对性任务研究，验证角色在意识形态一致性方面的行为模式。

Result: 表面指标显示角色对整体分类准确性影响不大，但深入分析发现重要行为变化。不同意识形态角色对内容有害性的判断存在系统性差异，特别是大型模型更倾向于与同意识形态角色保持一致，强化了意识形态内部一致性同时扩大了跨意识形态分歧。

Conclusion: 角色设定可能将微妙的意识形态偏见引入LLM输出，引发对AI系统在表面中立伪装下强化党派观点的担忧，这对内容审核系统的公平性提出了重要警示。
...
### [92] [Beyond Long Context: When Semantics Matter More than Tokens](https://arxiv.org/abs/2510.25816)
*Tarun Kumar Chawdhury,Jon D. Duke*

Main category: cs.CL

TL;DR: CLEAR方法通过实体感知检索在临床问答中实现了比传统向量数据库方法更好的性能（F1得分0.90 vs 0.86），同时减少了70%以上的token使用量。
...
Method: 开发了临床笔记QA评估平台，将CLEAR方法与零样本大上下文推理和传统基于分块的检索增强生成进行对比验证。测试了12个临床笔记，token数量从10,000到65,000不等。

Result: CLEAR实现了58.3%的胜率，平均语义相似度为0.878，比宽上下文处理少使用78%的token。在超过65,000 token的长文档上表现最佳，胜率达到75%。

Conclusion: 实体感知检索提高了临床自然语言处理的效率和准确性。该评估框架为评估临床问答系统提供了一个可重用且透明的基准，其中语义精度和计算效率至关重要。
...
### [93] [A Survey on Efficient Large Language Model Training: From Data-centric Perspectives](https://arxiv.org/abs/2510.25817)
*Junyu Luo,Bohan Wu,Xiao Luo,Zhiping Xiao,Yiqiao Jin,Rong-Cheng Tu,Nan Yin,Yifan Wang,Jingyang Yuan,Wei Ju,Ming Zhang*

Main category: cs.CL

TL;DR: 本文首次从数据中心的视角系统性地调查了数据高效的大型语言模型后训练方法，提出了包含数据选择、数据质量增强、合成数据生成、数据蒸馏与压缩以及自演化数据生态系统的分类法，并总结了各类代表性方法和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型后训练范式面临数据挑战，包括手动标注成本高和数据规模边际收益递减，因此实现数据高效的后训练成为关键研究问题。

Method: 提出数据高效LLM后训练的分类法，涵盖数据选择、数据质量增强、合成数据生成、数据蒸馏与压缩、自演化数据生态系统等五类方法。

Result: 系统总结了各类代表性方法，分析了数据高效LLM后训练面临的挑战，并提出了未来的研究方向。

Conclusion: 通过审视数据高效LLM后训练的挑战，强调了开放性问题并提出了潜在的研究途径，希望激发进一步探索以最大化大规模模型训练中数据利用的潜力。
...
### [94] [Evaluating the Impact of LLM-Assisted Annotation in a Perspectivized Setting: the Case of FrameNet Annotation](https://arxiv.org/abs/2510.25904)
...
TL;DR: 本文评估了使用LLM语义角色标注器进行FrameNet式语义标注的(半)自动化效果，比较了手动、自动和半自动三种设置下的标注时间、覆盖率和多样性。结果显示半自动设置能提高框架多样性并保持相似覆盖率，而纯自动设置在除标注时间外的所有指标上都表现较差。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM应用在语言资源和数据集创建中已用于加速或替代人工劳动，但对其在标注数据集创建中的性能和影响，特别是从NLP视角进行的全面评估仍然缺失。

Method: 采用比较方法，在三种实验设置（手动、自动和半自动）下评估标注时间、覆盖率和多样性，使用基于LLM的语义角色标注器进行FrameNet式语义标注。

Result: 半自动标注设置相比纯人工设置能提高框架多样性并保持相似覆盖率；纯自动设置在除标注时间外的所有指标上都表现明显较差。

Conclusion: 混合的半自动标注方法在保持标注质量的同时提高了效率，而纯自动方法在语义标注任务中表现不佳，强调了人工监督在LLM应用中的重要性。
...
### [95] [Revisiting Multilingual Data Mixtures in Language Model Pretraining](https://arxiv.org/abs/2510.25947)
*Negar Foroutan,Paul Teiletche,Ayush Kumar Tarun,Antoine Bosselut*

Main category: cs.CL

TL;DR: 本研究挑战了多语言预训练中的常见假设，发现当语言在预训练语料中有足够token时，组合英语和多语言数据不会降低任何语言的性能；英语作为枢纽语言有益于所有语系；在1.1B和3B参数规模下未观察到明显的"多语言诅咒"现象。


<details>
  <summary>Details</summary>
Motivation: 研究多语言数据混合对大型语言模型预训练的影响，特别是关于语言覆盖与模型性能之间权衡的常见假设（即多语言诅咒问题）。

Method: 训练1.1B和3B参数的LLM，使用不同多语言语料库，语言数量从25到400种，分析英语作为枢纽语言的效果以及不同语言家族的影响。

Result: 英语和多语言数据组合不会降低任何语言的性能（前提是语言有足够token）；英语作为枢纽语言对所有语系都有益；在模型规模下未发现显著的多语言诅咒现象。
...
### [96] [Semantic Label Drift in Cross-Cultural Translation](https://arxiv.org/abs/2510.25967)
*Mohsinul Kabir,Tasnim Ahmed,Md Mezbaur Rahman,Polydoros Giannouris,Sophia Ananiadou*

Main category: cs.CL

TL;DR: 机器翻译在低资源语言中广泛用于生成合成数据，但研究发现文化差异会导致语义标签在翻译过程中发生漂移或改变，特别是在文化敏感领域。


<details>
  <summary>Details</summary>
Motivation: 研究机器翻译中文化对齐对语义标签保存的影响，因为文化差异可能导致标签漂移，影响下游应用。

Method: 通过一系列实验，在文化敏感和中立领域测试不同机器翻译系统（包括现代大语言模型）的标签保存能力，分析文化相似性对标签漂移的影响。

Result: 发现：(1) 机器翻译系统（包括LLMs）在翻译过程中诱导标签漂移，特别是在文化敏感领域；(2) LLMs编码文化知识，利用这种知识会放大标签漂移；(3) 源语言和目标语言之间的文化相似性是标签保存的关键决定因素。
...
### [97] [SymCode: A Neurosymbolic Approach to Mathematical Reasoning via Verifiable Code Generation](https://arxiv.org/abs/2510.25975)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂数学推理中表现不佳，基于文本的生成导致未经验证和算术上不可靠的解决方案。现有的提示策略如思维链仍然在这种不可靠的媒介中操作，缺乏确定性验证机制。

Method: 引入SymCode神经符号框架，将数学问题解决重新定义为使用SymPy库的可验证代码生成任务。
...
### [98] [NeuronMM: High-Performance Matrix Multiplication for LLM Inference on AWS Trainium](https://arxiv.org/abs/2510.25977)
*Dinghong Song,Jierui Xu,Weichu Yang,Pengfei Su,Dong Li*

Main category: cs.CL

TL;DR: 本文针对AWS Trainium AI加速器，设计了高性能的矩阵乘法内核，通过内核融合和缓存策略优化，在LLM推理中实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: Trainium AI加速器虽然为LLM训练和推理提供了高性能解决方案，但由于其脉动阵列架构和数据布局的特殊要求，实现高性能具有挑战性。

Method: 采用基于内核融合和新型缓存策略的一系列技术，减少软件管理内存层次结构中的数据移动，最大化SRAM带宽，避免昂贵的矩阵转置。

Result: 在9个数据集和4个最新LLM上的评估显示，相比AWS现有实现，矩阵乘法内核平均加速1.35倍（最高2.22倍），端到端LLM推理平均加速1.66倍（最高2.49倍）。

Conclusion: 针对Trainium架构定制化的矩阵乘法优化技术能够显著提升LLM推理性能，证明了所提出方法的有效性。
...
### [99] [AttnCache: Accelerating Self-Attention Inference for LLM Prefill via Attention Cache](https://arxiv.org/abs/2510.25979)
*Dinghong Song,Yuan Feng,Yiwei Wang,Shangye Chen,Cyril Guyot,Filip Blagojevic,Hyeran Jeon,Pengfei Su,Dong Li*

Main category: cs.CL

TL;DR: AttnCache是一种通过检索和重用相似注意力图来加速LLM预填充阶段推理的框架，在CPU和GPU上分别实现了1.2-1.6倍端到端加速和2-3倍注意力计算加速。


<details>
  <summary>Details</summary>
Motivation: 许多现实工作负载（如分类、问答、推荐等）仅使用LLM的预填充阶段，其中自注意力计算因其与序列长度的二次复杂度成为主要性能瓶颈。研究发现语义不同的句子在不同层和头中往往产生相似的注意力图。

Method: 基于注意力图记忆数据库，AttnCache采用高效的缓存和相似性搜索技术，在推理过程中识别并重用预缓存的注意力图，从而减少自注意力的计算开销。
...
### [100] [PORTool: Tool-Use LLM Training with Rewarded Tree](https://arxiv.org/abs/2510.26020)
*Feijie Wu,Weiwu Zhu,Yuxiang Zhang,Soumya Chatterjee,Jiarong Zhu,Fan Mo,Rodin Luo,Jing Gao*

Main category: cs.CL

TL;DR: PORTool是一种基于强化学习的工具使用LLM训练方法，通过探索多种正确路径来提升模型在动态工具调用环境中的性能。
...
Method: 使用强化学习方法，首先生成多个轨迹形成树状结构，然后基于产生正确答案和成功工具调用的能力为每个步骤分配奖励，最后结合分支相对优势和轨迹相对优势来训练LLM。

Result: 在17种工具的实验环境中，PORTool在最终准确率和工具调用步骤数量方面相比其他训练方法都有显著提升。

Conclusion: PORTool通过强化学习探索多种正确路径的方法，有效提升了工具使用LLM在动态环境中的性能表现。
...
### [101] [Rethinking Cross-lingual Alignment: Balancing Transfer and Cultural Erasure in Multilingual LLMs](https://arxiv.org/abs/2510.26024)
*HyoJung Han,Sweta Agrawal,Eleftheria Briakou*

Main category: cs.CL

TL;DR: 本文分析了跨语言对齐(CLA)在促进知识迁移的同时可能导致文化擦除的问题，提出了转移-定位平面的评估框架，并基于模型内部表征的发现开发了Surgical Steering方法来解决这一权衡问题。


<details>
  <summary>Details</summary>
Motivation: 跨语言对齐旨在对齐多语言表示，使大语言模型能够在语言间无缝迁移知识。然而，这种表示趋同可能导致文化擦除，即丧失提供基于查询语言的文化情境化响应的能力。

Method: 引入转移-定位平面的评估框架来量化知识转移和文化擦除；分析CLA方法的内部表征；提出Surgical Steering推理时方法，通过对不同层应用有针对性的激活引导来分离这两个目标。

Result: 对六种语言的研究发现，CLA方法在改善事实转移的同时，直接以文化定位为代价；模型内部表征显示，通用事实转移和文化特定知识在不同模型层上最优可控。

Conclusion: Surgical Steering方法通过在特定层应用激活引导，能够更好地平衡知识转移和文化定位这两个竞争维度，有效克服当前对齐技术的局限性。
...
### [102] [QCoder Benchmark: Bridging Language Generation and Quantum Hardware through Simulator-Based Feedback](https://arxiv.org/abs/2510.26101)
...
TL;DR: QCoder Benchmark是一个评估大语言模型在量子编程任务中表现的框架，通过量子模拟器提供电路深度、执行时间和错误分类等反馈，并使用真实编程竞赛的人类代码作为基准。


<details>
  <summary>Details</summary>
Motivation: 填补大语言模型在需要与硬件设备交互的领域（如量子编程）中的研究空白，现有模型在这些专业领域的表现尚未充分探索。

Method: 开发QCoder Benchmark评估框架，支持量子模拟器环境评估，提供领域特定指标反馈，并整合真实编程竞赛中的人类代码提交进行对比分析。

Result: 实验显示GPT-4o等先进模型仅达到18.97%准确率，而基于推理的o3模型达到78%准确率，超过了人类代码39.98%的平均成功率。

Conclusion: 量子编程对现有大语言模型具有挑战性，推理模型表现更优，QCoder Benchmark数据集和公共评估API将支持进一步研究。
...
### [103] [Reasoning Path Divergence: A New Metric and Curation Strategy to Unlock LLM Diverse Thinking](https://arxiv.org/abs/2510.26122)
*Feng Ju,Zeyu Qin,Rui Min,Zhitao He,Lingpeng Kong,Yi R. Fung*

Main category: cs.CL

TL;DR: 本文提出了一种"一个问题，多个解决方案"（1PNS）的训练范式，通过引入推理路径差异度（RPD）度量来增加模型输出的多样性，从而提高测试时缩放（TTS）的效果。


<details>
  <summary>Details</summary>
Motivation: 传统的"一个问题，一个解决方案"（1P1S）训练方式导致模型输出多样性不足，限制了测试时缩放的效果。为了解决这个问题，需要让模型接触更多有效的推理路径。

Method: 提出了推理路径差异度（RPD）度量方法，用于衡量多步思维链之间的语义差异。使用RPD为每个问题选择最大多样性的解决方案集，并对Qwen3-4B-Base模型进行微调。

Result: 实验显示，RPD选择的训练集产生了更多样化的输出和更高的pass@k性能，在pass@16上平均比强1P1S基线提高了2.80%，在AIME24上提高了4.99%。

Conclusion: 1PNS训练范式能够有效增强模型输出的多样性，进一步放大了测试时缩放的效果。
...
### [104] [Similarity-Distance-Magnitude Language Models](https://arxiv.org/abs/2510.26183)
*Allen Schmaltz*

Main category: cs.CL

TL;DR: 提出了相似性-距离-大小（SDM）语言模型，通过微调使生成的文本更可能落在经过良好校准的高概率区域，从而提高指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 现有预训练语言模型在指令遵循任务中存在较多的弃权情况（即拒绝回答），导致统计效率较低。需要一种方法能够减少这种弃权行为，提高模型的实际可用性。

Method: 在预训练的仅解码器Transformer语言模型基础上，添加最终的SDM激活层用于二元分类，通过监督微调来最大化指令遵循的生成比例。使用对比输入编码方案和在线生成的硬负例来训练模型。

Result: 与强监督基线相比，SDM语言模型显著减少了弃权情况，提高了统计效率。

Conclusion: SDM语言模型能够有效提升预训练语言模型的指令遵循能力，通过特定的激活层设计和训练策略实现了更好的性能表现。
...
### [105] [Don't Let It Fade: Preserving Edits in Diffusion Language Models via Token Timestep Allocation](https://arxiv.org/abs/2510.26200)
...
Method: 提出Token Timestep Allocation (TTA)，通过为每个token分配时间步调度实现软语义排序：关键token早期冻结，不确定token持续细化。该方法可作为固定策略或由任务信号驱动的自适应策略，在推理时统一应用于各种DLM。
...
### [106] [Towards Global Retrieval Augmented Generation: A Benchmark for Corpus-Level Reasoning](https://arxiv.org/abs/2510.26205)
*Qi Luo,Xiaonan Li,Tingshuo Fan,Xinchi Chen,Xipeng Qiu*

Main category: cs.CL

TL;DR: 该论文提出了GlobalQA基准来评估全局RAG能力，并开发了GlobalRAG框架来解决现有方法在全局任务上的表现不佳问题。


<details>
  <summary>Details</summary>
Motivation: 当前RAG评估基准主要关注局部RAG能力，但现实应用需要全局RAG能力来聚合分析整个文档集合的信息，现有方法在这方面表现很差。

Method: 提出GlobalRAG多工具协作框架，通过分块级检索保持结构连贯性，集成LLM驱动的智能过滤器消除噪声文档，并加入聚合模块进行精确符号计算。

Result: 在Qwen2.5-14B模型上，GlobalRAG达到6.63 F1分数，而最强基线仅为1.51 F1，验证了方法的有效性。

Conclusion: GlobalRAG框架显著提升了全局RAG任务的性能，为解决文档集合级别的信息聚合分析问题提供了有效方案。
...
### [107] [Pragmatic Theories Enhance Understanding of Implied Meanings in LLMs](https://arxiv.org/abs/2510.26253)
*Takuma Sato,Seiya Kawano,Koichiro Yoshino*

Main category: cs.CL

TL;DR: 本研究提出通过将语用学理论（如格赖斯语用学和关联理论）作为提示提供给语言模型，来提升模型理解隐含意义的能力。实验表明该方法在语用推理任务上比基线方法提升高达9.6%。


<details>
  <summary>Details</summary>
Motivation: 准确理解隐含意义在人类交流中至关重要，语言模型也应具备这种能力。研究旨在探索如何通过语用学理论提示来增强语言模型的语用推理能力。

Method: 提出将语用学理论概述作为提示呈现给语言模型，引导其通过逐步推理过程得出最终解释。与仅提示中间推理而不呈现语用学理论的基线方法（0-shot Chain-of-Thought）进行对比。

Result: 实验结果显示，相比基线方法，该方法使语言模型在语用推理任务上的得分提升高达9.6%。即使在提示中仅提及语用学理论名称而不详细解释，在较大模型中也能带来约1-3%的性能提升。

Conclusion: 将语用学理论作为提示是提升语言模型理解隐含意义能力的有效方法，即使简单提及理论名称也能带来性能改善。
...
### [108] [Language Models Are Borrowing-Blind: A Multilingual Evaluation of Loanword Identification across 10 Languages](https://arxiv.org/abs/2510.26254)
*Mérilin Sousa Silva,Sina Ahmadi*

Main category: cs.CL

TL;DR: 本文研究了预训练语言模型（包括大语言模型）是否能够像人类一样识别借词，发现模型在区分借词和原生词方面表现不佳，存在对借词的偏见。


<details>
  <summary>Details</summary>
Motivation: 在双语社区中，主导语言会持续向少数民族语言施加词汇影响，说话者通常能够区分借词和原生词汇。本研究旨在探索预训练语言模型是否具备类似的借词识别能力。

Method: 在10种语言上评估多个预训练语言模型，包括大语言模型，使用显式指令和上下文信息进行借词识别测试。

Result: 尽管提供了明确的指令和上下文信息，模型在区分借词和原生词方面表现很差，这证实了现代NLP系统对借词而非原生等价词存在偏见。

Conclusion: 这项研究对开发少数民族语言的NLP工具以及在主导语言词汇压力下支持语言保护具有重要启示。
...
### [109] [Distilling Multilingual Vision-Language Models: When Smaller Models Stay Multilingual](https://arxiv.org/abs/2510.26271)
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在不同语言间性能不均，模型压缩时问题更严重。知识蒸馏在跨语言场景中的应用研究不足，需要系统分析其效果。

Method: 研究了五种知识蒸馏方法在CLIP和SigLIP2模型上的表现，评估了领域内检索和领域外视觉问答任务。

Result: 某些配置在模型尺寸减半时仍能保持或改善多语言检索鲁棒性，但其他方法无法维持跨任务稳定性，揭示了仅靠聚合精度无法发现的设计敏感权衡。

Conclusion: 知识蒸馏在跨语言视觉语言模型压缩中具有复杂的行为模式，需要仔细选择蒸馏方法以确保多语言表示一致性和任务稳定性。
...
### [110] [Do LLMs Signal When They're Right? Evidence from Neuron Agreement](https://arxiv.org/abs/2510.26277)
*Kang Chen,Yaoning Wang,Kai Xiong,Zhuoka Feng,Wenhe Sun,Haotian Chen,Yixin Cao*

Main category: cs.CL

TL;DR: 本文提出Neuron Agreement Decoding (NAD)方法，基于神经元激活的内部信号进行无监督集成解码，通过激活稀疏性和跨样本神经元一致性来选择最佳候选答案，显著减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅使用外部输出信号（如token概率、熵或自评估）进行候选评分，但这些信号在训练后可能校准不佳。作者发现内部神经元激活包含更丰富的信息，正确回答激活更少的独特神经元且跨样本一致性更强。

Method: 提出NAD方法，分析神经元激活模式：1）计算激活稀疏性（正确回答激活更少神经元）；2）测量跨样本神经元一致性（正确回答激活模式更一致）。基于这两个内部信号选择最佳候选答案，支持早期正确预测和激进提前停止。

Result: 在数学和科学基准测试中，NAD与多数投票方法表现相当；在开放式编码基准测试中，NAD持续优于Avg@64方法。通过早期剪枝不具前景的轨迹，NAD减少99%的token使用，生成质量损失极小。

Conclusion: 内部信号为无监督集成解码提供了可靠、可扩展且高效的指导，神经元激活模式是比外部信号更有效的推理质量指标。
...
### [111] [Unravelling the Mechanisms of Manipulating Numbers in Language Models](https://arxiv.org/abs/2510.26285)
...
TL;DR: 尽管大语言模型在处理数字时经常产生错误输出，但研究发现不同模型学习到了可互换的、系统性的、高度准确的数字表示，这些表示在隐藏状态和输入上下文类型中具有普遍性。
...
Result: 发现不同语言模型学习到了可互换的数字表示，这些表示是系统性的、高度准确的，并且在隐藏状态和输入上下文类型中具有普遍性。
...
### [112] [Can Agent Conquer Web? Exploring the Frontiers of ChatGPT Atlas Agent in Web Games](https://arxiv.org/abs/2510.26298)
*Jingran Zhang,Ning Li,Justin Cui*

Main category: cs.CL

TL;DR: 对Atlas在网页交互能力方面的早期评估，使用浏览器游戏作为测试场景，结果显示Atlas在逻辑推理任务中表现优异，但在需要精确时机和运动控制的实时游戏中表现较差。


<details>
  <summary>Details</summary>
Motivation: 虽然Atlas的信息检索能力已被证明，但其在动态交互环境中的表现仍较少被探索，因此使用浏览器游戏来评估其网页交互能力。

Method: 使用Google的T-Rex Runner、数独、Flappy Bird和Stein.world等浏览器游戏作为测试场景，以游戏内性能得分作为量化指标评估不同任务类型的表现。

Result: Atlas在数独等逻辑推理任务中表现强劲，完成谜题速度显著快于人类基准，但在需要精确时机和运动控制的实时游戏中表现很差，经常无法通过初始障碍。
...
### [113] [SCRIBE: Structured Chain Reasoning for Interactive Behaviour Explanations using Tool Calling](https://arxiv.org/abs/2510.26322)
*Fares Fawzi,Vinitra Swamy,Dominik Glandorf,Tanya Nazaretsky,Tanja Käser*

Main category: cs.CL

TL;DR: SCRIBE是一个用于教育反馈的多跳工具增强推理框架，通过两阶段LoRA微调在3B和8B模型上实现，能够在低资源环境下生成高质量的学生反馈，性能与大型模型相当。


<details>
  <summary>Details</summary>
Motivation: 解决教育场景中语言模型部署面临的三大挑战：隐私问题、有限计算资源以及需要教学有效反馈，需要小型开源模型在本地运行并可靠地基于正确信息生成输出。

Method: 结合领域特定工具与自反思推理流程，支持迭代推理、工具使用和错误恢复，通过两阶段LoRA微调在GPT-4o生成的合成数据上训练3B和8B模型。

Result: 8B-SCRIBE模型在相关性和可操作性等关键维度上达到或超过更大模型的质量，学生评价与GPT-4o和Llama-3.3 70B相当。

Conclusion: SCRIBE证明了在低资源、隐私敏感的教育应用中具有可行性，能够提供高质量的学生反馈。
...
### [114] [From Amateur to Master: Infusing Knowledge into LLMs via Automated Curriculum Learning](https://arxiv.org/abs/2510.26336)
*Nishit Neema,Srinjoy Mukherjee,Sapan Shah,Gokul Ramakrishnan,Ganesh Venkatesh*

Main category: cs.CL

TL;DR: ACER方法通过生成教科书式课程和基于布鲁姆分类法的问答对，将通用大语言模型转化为领域专家，在保持广泛能力的同时显著提升专业领域表现。
...
Method: ACER首先为特定学科生成目录结构，然后基于布鲁姆分类法创建问答对，构建系统化课程。使用合成的语料库进行交错课程安排的持续预训练，在内容和认知维度上对齐学习。

Result: 在Llama 3.2模型上，ACER在专业MMLU子集上显著提升，微观经济学准确率提高5个百分点，所有目标领域平均提升3个百分点。同时促进跨领域知识迁移，非目标领域性能提升0.7点，在ARC和GPQA基准上提升超过2个绝对点。

Conclusion: ACER提供了一个可扩展且有效的方法来缩小LLMs在关键领域的差距，既能防止灾难性遗忘，又能促进正面的跨领域知识转移。
...
### [115] [MisSynth: Improving MISSCI Logical Fallacies Classification with Synthetic Data](https://arxiv.org/abs/2510.26345)
*Mykhailo Poliakov,Nadiya Shvai*

Main category: cs.CL

TL;DR: 本文提出MisSynth方法，通过检索增强生成(RAG)创建合成谬误样本，然后微调大语言模型(LLM)，显著提升了识别健康相关科学错误信息的能力。


<details>
  <summary>Details</summary>
Motivation: 健康相关的错误信息非常普遍且具有潜在危害，特别是那些歪曲或误解科学发现的声明难以识别。现有标注资源有限，需要开发有效的方法来增强LLM识别谬误论点的能力。

Method: 提出MisSynth管道，应用检索增强生成(RAG)技术生成合成谬误样本，然后使用这些样本对LLM模型进行轻量级微调。在MISSCI数据集和框架上进行实验。

Result: 微调后的模型相比原始基线模型取得了显著的准确率提升。例如，LLaMA 3.1 8B微调模型在MISSCI测试集上相比原始基线实现了超过35%的F1分数绝对提升。
...
### [116] [The Geometry of Dialogue: Graphing Language Models to Reveal Synergistic Teams for Multi-Agent Collaboration](https://arxiv.org/abs/2510.26352)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体方法基于大语言模型有潜力超越单一模型能力，但成功关键取决于协同团队组成。由于大多数模型的不透明性，难以了解其内部特征以进行有效协作。

Method: 构建'语言模型图'来映射模型间关系，基于成对对话的语义连贯性；应用社区检测识别协同模型集群；通过特定主题引导对话来识别协同团队。

Result: 实验表明该方法能发现功能一致的模型群组，反映其潜在专业化；识别的协同团队在下游基准测试中优于随机基线，与基于已知模型专业化的手动策划团队准确率相当。

Conclusion: 该研究为自动化设计协作多智能体LLM团队提供了新的基础，无需先验知识即可有效识别协同模型组合。
...
### [117] [On the Role of Context for Discourse Relation Classification in Scientific Writing](https://arxiv.org/abs/2510.26354)
*Stephen Wan,Wei Liu,Michael Strube*

Main category: cs.CL

TL;DR: 本文研究了在科学写作中使用预训练语言模型和大语言模型进行话语关系分类的初步调查，重点关注上下文信息如何帮助识别科学出版物中的话语结构。


<details>
  <summary>Details</summary>
Motivation: 随着生成式人工智能在科学工作流程中的使用增加，研究者希望利用话语层面的信息为AI生成的科学主张寻找支持证据，而理解科学写作中的话语结构是实现这一目标的第一步。

Method: 使用预训练语言模型和大语言模型进行话语关系分类，特别关注科学出版物这一研究较少的文体，并研究上下文（定义为话语结构）如何帮助该任务。

Result: 实验表明，上下文信息通常有助于话语关系分类任务，并分析了哪些科学话语关系类型最可能从上下文中受益。

Conclusion: 上下文在科学话语关系分类中具有重要作用，为使用话语层面信息支持AI生成科学主张提供了基础。
...
### [118] [OmniEduBench: A Comprehensive Chinese Benchmark for Evaluating Large Language Models in Education](https://arxiv.org/abs/2510.26422)
*Min Zhang,Hao Chen,Hao Chen,Wenqi Zhang,Didi Zhu,Xin Lin,Bo Jiang,Aimin Zhou,Fei Wu,Kun Kuang*

Main category: cs.CL

TL;DR: OmniEduBench是一个全面的中文教育基准，包含24.6K高质量问答对，分为知识维度和素养维度，覆盖61个学科和11种题型。实验显示主流LLMs在教育领域仍有较大提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs和基准主要关注知识维度，忽视了真实教育场景中重要的素养能力评估，且在中文语境下缺乏多样性。

Method: 构建包含24.602K问答对的OmniEduBench基准，分为知识维度(18.121K)和素养维度(6.481K)，各细分为6个类别，覆盖61个学科和11种题型。

Result: 在11个主流LLMs上的实验显示：知识维度仅Gemini-2.5 Pro准确率超过60%；素养维度最佳模型QWQ仍比人类智能低近30%。
...
### [119] [1+1>2: A Synergistic Sparse and Low-Rank Compression Method for Large Language Models](https://arxiv.org/abs/2510.26446)
*Zeliang Zong,Kai Zhang,Zheyang Li,Wenming Tan,Ye Ren,Yiyan Zhai,Jilin Hu*

Main category: cs.CL

TL;DR: SSLC方法结合稀疏化和低秩近似技术，在无需额外训练的情况下对LLaMA和Qwen2.5模型（7B-70B）进行压缩，实现50%压缩率且性能无损，获得至少1.63倍加速。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然表现出色，但其广泛部署受到带宽和计算需求的限制。现有方法如剪枝和低秩近似各自表现良好，但两者的协同作用尚未充分探索。

Method: 基于理论分析，将低秩近似和稀疏优化统一为一个问题，并通过迭代优化算法求解，实现两种技术的协同压缩。

Result: 在LLaMA和Qwen2.5模型上，SSLC方法无需额外训练步骤，始终优于单独方法，达到最先进结果。Qwen2.5模型压缩50%后性能无下降，且获得至少1.63倍加速。

Conclusion: SSLC为大语言模型的高效部署提供了实用解决方案，通过稀疏化和低秩近似的协同作用实现高效压缩。
...
### [120] [Inside CORE-KG: Evaluating Structured Prompting and Coreference Resolution for Knowledge Graphs](https://arxiv.org/abs/2510.26512)
*Dipak Meher,Carlotta Domeniconi*

Main category: cs.CL

TL;DR: CORE-KG框架通过整合类型感知共指消解模块和领域引导结构化提示，显著减少了知识图谱构建中的节点重复和噪声，系统消融研究量化了两个关键组件的贡献。


<details>
  <summary>Details</summary>
Motivation: 解决从复杂法律文本中自动构建知识图谱时面临的挑战，包括非结构化、词汇密集、模糊或变化引用导致的噪声和节点重复问题。

Method: 使用CORE-KG框架，集成类型感知共指消解模块和领域引导结构化提示，通过系统消融研究量化两个关键组件的贡献。

Result: 移除共指消解导致节点重复增加28.32%，噪声节点增加4.32%；移除结构化提示导致节点重复增加4.34%，噪声节点增加73.33%。

Conclusion: 研究为设计从复杂法律文本中提取结构化表示的鲁棒LLM-based管道提供了实证见解，证明了两个组件在减少噪声和节点重复方面的重要作用。
...
### [121] [Hebrew Diacritics Restoration using Visual Representation](https://arxiv.org/abs/2510.26521)
*Yair Elboher,Yuval Pinter*

Main category: cs.CL

TL;DR: DIVRIT是一个希伯来语变音符号恢复系统，将任务构建为零样本分类问题，使用希伯来视觉语言模型处理未变音文本图像，在候选集中选择最合适的变音模式。


<details>
  <summary>Details</summary>
Motivation: 希伯来语在未变音时具有高度歧义性，需要准确恢复变音符号以确保正确发音和消除文本歧义。

Method: 将变音恢复任务构建为零样本分类问题，在词级别操作，从动态生成的候选集中选择最合适的变音模式。关键创新是使用希伯来视觉语言模型，将未变音文本作为图像处理，直接在输入向量表示中嵌入变音信息。

Result: 在"oracle"设置下（正确变音形式保证在候选集中），DIVRIT实现了高准确率。通过架构增强和优化训练方法，系统泛化能力显著提升。

Conclusion: 视觉表示在准确和自动化的希伯来语变音恢复方面具有巨大潜力，系统无需依赖复杂的显式语言分析即可有效执行变音化。
...
### [122] [Inference-Cost-Aware Dynamic Tree Construction for Efficient Inference in Large Language Models](https://arxiv.org/abs/2510.26577)
*Yinrong Hong,Zhiquan Tan,Kai Hu*

Main category: cs.CL

TL;DR: CAST是一种新的动态树解码方法，通过考虑GPU配置和批处理大小等推理成本因素来动态优化树结构，相比传统解码方法速度提升高达5.2倍，比现有最先进技术性能提升5%-20%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型面临自回归设计和庞大模型规模带来的显著推理延迟问题，现有推测解码方法如EAGLE-2和EAGLE-3虽然使用动态树结构，但往往忽略了GPU设备和批处理大小等关键系统变量的影响。

Method: 提出CAST动态树解码方法，考虑GPU配置、批处理大小等推理成本因素，动态优化树结构，在六个不同任务和六个不同LLM上进行了全面实验验证。

Result: 方法表现出显著效果，相比传统解码方法速度提升高达5.2倍，普遍优于现有最先进技术5%-20%。

Conclusion: CAST方法通过考虑系统级推理成本因素来动态优化树结构，有效解决了LLM推理延迟问题，在多个任务和模型上均表现出优越性能。
...
TL;DR: 本文重新审视了编码器-解码器架构的大语言模型（RedLLM），与当前主流的仅解码器模型（DecLLM）进行了全面的规模扩展比较，发现RedLLM在推理效率和部分下游任务上具有优势。


<details>
  <summary>Details</summary>
Motivation: 当前LLM研究快速转向仅解码器架构，但缺乏从规模扩展角度的严谨比较分析，可能忽视了编码器-解码器模型的潜力。

Method: 使用前缀语言建模预训练RedLLM，采用仅解码器模型的现代配方进行增强，在150M到8B不同规模上进行全面比较，使用RedPajama V1进行预训练和FLAN进行指令调优。

Result: RedLLM展现出令人信服的扩展特性和强大的性能，在推理效率上显著优于DecLLM，指令调优后在各种下游任务上达到相当甚至更好的结果。
...
TL;DR: Evontree是一个利用少量高质量本体规则从大语言模型中系统提取、验证和增强领域知识的新框架，无需大量外部数据集，在医疗QA基准测试中显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 在数据敏感领域如医疗保健中，缺乏高质量领域特定训练语料库阻碍了大语言模型的专业化应用，而领域专家已将领域智慧提炼成本体规则。

Method: Evontree从原始模型中提取领域本体，使用两个核心本体规则检测不一致性，并通过自蒸馏微调来强化精炼后的知识。

Result: 在Llama3-8B-Instruct和Med42-v2上的医疗QA基准测试实验显示，该方法持续优于未修改模型和领先的监督基线，准确率提升高达3.7%。
...
TL;DR: Kimi Linear是一种混合线性注意力架构，首次在各种场景下公平比较中超越完整注意力机制，核心是Kimi Delta Attention (KDA)模块，通过精细门控机制更有效地利用有限状态RNN内存。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够在各种场景下超越完整注意力机制的线性注意力架构，同时提高硬件效率和减少KV缓存使用。

Method: 采用KDA线性注意力模块扩展Gated DeltaNet，结合专门的块状算法和Diagonal-Plus-Low-Rank (DPLR)转移矩阵变体，构建3B激活参数、48B总参数的混合模型。

Result: 在相同训练方案下，Kimi Linear在所有评估任务中显著超越完整MLA，减少KV缓存使用达75%，在1M上下文长度下实现高达6倍的解码吞吐量。

Conclusion: Kimi Linear可以作为完整注意力架构的直接替代方案，提供更优越的性能和效率，特别是在长输入输出任务中。
...
TL;DR: AutoDeco是一种新型架构，通过在标准transformer上添加轻量级头，实现真正的端到端生成，让模型能够学习控制自身的解码策略。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs的端到端标签是误称，因为它们依赖需要手动调整超参数的非可微解码过程。

Method: 在标准transformer上添加轻量级头，在每个步骤动态预测上下文特定的温度和top-p值以及下一个token的logits。

Result: 在八个基准测试中，AutoDeco不仅显著优于默认解码策略，还达到了与oracle调优基线相当的性能。

Conclusion: AutoDeco开启了可引导和交互式LLM解码的新范式，模型能够基于自然语言指令调整其预测的温度和top-p值。
...
TL;DR: 该研究分析了大型语言模型在训练过程中价值对齐的动态过程，发现监督微调阶段主要建立模型的价值体系，而后续的偏好优化很少重新调整这些价值。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在社会中扮演越来越重要的角色，需要研究它们如何学习与人类价值体系对齐，而不仅仅是评估训练完成模型的对齐情况。

Method: 使用Llama-3和Qwen-3不同规模的模型，结合流行的监督微调和偏好优化数据集与算法，通过合成偏好数据集进行受控实验。

Result: 监督微调阶段通常确立模型的价值体系，后续偏好优化很少重新调整这些价值；不同的偏好优化算法即使使用相同的偏好数据也会导致不同的价值对齐结果。

Conclusion: 研究结果为理解模型在训练过程中如何学习价值提供了可操作的见解，有助于改进数据筛选以及模型和算法的选择，以增强模型与人类价值的对齐。
...
<details>
  <summary>Details</summary>
Motivation: 现有数学竞赛基准由于性能饱和（如AIME24/25）已无法有效评估顶级LLMs，需要引入更严格的挑战。

Method: 创建50道经过专家交叉验证、达到IMO难度标准的原创问题，仅要求最终答案而非证明，实现自动评分。

Result: 在26个LLMs上的实验显示，最佳模型准确率仅为52.4%，大多数模型低于40%，但测试时计算增加显示出有希望的扩展趋势。

Conclusion: 当前LLMs在数学推理方面仍有显著改进空间，AMO-Bench的发布将促进语言模型推理能力的研究进展。
